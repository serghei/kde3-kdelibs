...
ASpellDict::ASpellDict(const QString &lang) : Dictionary(lang)
{
...
QStringList ASpellDict::suggest(const QString &word)
{
    ...
    /* ASpell is expecting length of a string in char representation */
    /* word.length() != word.utf8().length() for nonlatin strings    */
    const AspellWordList *suggestions = aspell_speller_suggest(m_speller, word.utf8(), word.utf8().length());
...
bool ASpellDict::checkAndSuggest(const QString &word, QStringList &suggestions)
{
    bool c = check(word);
    if(c)
...
bool ASpellDict::storeReplacement(const QString &bad, const QString &good)
{
    /* ASpell is expecting length of a string in char representation */
    /* word.length() != word.utf8().length() for nonlatin strings    */
    return aspell_speller_store_replacement(m_speller, bad.utf8(), bad.utf8().length(), good.utf8(), good.utf8().length());
}
...
bool ASpellDict::addToPersonal(const QString &word)
{
    ...
    /* ASpell is expecting length of a string in char representation */
    /* word.length() != word.utf8().length() for nonlatin strings    */
    aspell_speller_add_to_personal(m_speller, word.utf8(), word.utf8().length());
...
bool ASpellDict::addToSession(const QString &word)
{
    /* ASpell is expecting length of a string in char representation */
    /* word.length() != word.utf8().length() for nonlatin strings    */
    return aspell_speller_add_to_session(m_speller, word.utf8(), word.utf8().length());
}
