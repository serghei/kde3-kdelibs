...
HSpellDict::HSpellDict(const QString &lang) : Dictionary(lang)
{
...
bool HSpellDict::check(const QString &word)
{
    ...
    int correct = hspell_check_word(m_speller, wordISO, &preflen); // this going to be removed
...
bool HSpellDict::checkAndSuggest(const QString &word, QStringList &suggestions)
{
    bool c = check(word);
    if(c)
