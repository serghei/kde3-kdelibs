...
void DomainBrowserPrivate::customEvent(QCustomEvent *event)
{
    ...
    if(event->type() == QEvent::User + SD_ADDREMOVE)
    {
        ...
        if(aev->m_op == AddRemoveEvent::Add)
            m_owner->gotNewDomain(aev->m_domain);
        else
            m_owner->gotRemoveDomain(aev->m_domain);
    }
}
...
DomainBrowser::DomainBrowser(QObject *parent) : QObject(parent)
{
    ...
    connect(KApplication::kApplication(), SIGNAL(kipcMessage(int, int)), this, SLOT(domainListChanged(int, int)));
}
...
void DomainBrowser::startBrowse()
{
    ...
    {
        DNSServiceRef ref;
        if(DNSServiceEnumerateDomains(&ref, kDNSServiceFlagsBrowseDomains, 0, domain_callback, reinterpret_cast< void * >(d))
...
void domain_callback(DNSServiceRef, DNSServiceFlags flags, uint32_t, DNSServiceErrorType errorCode, const char *replyDomain, void *context)
{
    ...
    {
        AddRemoveEvent arev((flags & kDNSServiceFlagsAdd) ? AddRemoveEvent::Add : AddRemoveEvent::Remove, QString::null, QString::null,
