...
KBuildServiceFactory::KBuildServiceFactory(KSycocaFactory *serviceTypeFactory, KBuildServiceGroupFactory *serviceGroupFactory)
    : KServiceFactory(), m_serviceDict(977), m_dupeDict(977), m_serviceTypeFactory(serviceTypeFactory), m_serviceGroupFactory(serviceGroupFactory)
{
...
KSycocaEntry *KBuildServiceFactory::createEntry(const QString &file, const char *resource)
{
    QString name = file;
...
void KBuildServiceFactory::saveOfferList(QDataStream &str)
{
    ...
    bool isNumber;
    for(QDictIterator< KSycocaEntry::Ptr > itserv(*m_entryDict); itserv.current(); ++itserv)
    {
    ...
    for(QDictIterator< KSycocaEntry::Ptr > it(*(m_serviceTypeFactory->entryDict())); it.current(); ++it)
    {
        ...
        for(KService::List::ConstIterator it2 = services.begin(); it2 != services.end(); ++it2)
        {
...
void KBuildServiceFactory::saveInitList(QDataStream &str)
{
    ...
    for(KService::List::Iterator it = initList.begin(); it != initList.end(); ++it)
    {
        str << (Q_INT32)(*it)->offset();
    }
}

void KBuildServiceFactory::addEntry(KSycocaEntry *newEntry, const char *resource)
{
