...
Kded::Kded(bool checkUpdates, bool new_startup)
    : DCOPObject("kbuildsycoca"), DCOPObjectProxy(), b_checkUpdates(checkUpdates), m_needDelayedCheck(false), m_newStartup(new_startup)
{
    _self = this;
    QCString cPath;
...
bool Kded::process(const QCString &obj, const QCString &fun, const QByteArray &data, QCString &replyType, QByteArray &replyData)
{
    if(obj == "ksycoca")
...
KDEDModule *Kded::loadModule(const KService *s, bool onDemand)
{
    ...
    if(s && !s->library().isEmpty())
    {
        ...
        if(!lib)
        {
            kdWarning() << k_funcinfo << "Could not load library. [ " << loader->lastErrorMessage() << " ]" << endl;
            libname.prepend("lib");
...
void Kded::slotApplicationRemoved(const QCString &appId)
{
    ...
    if(windowIds)
    {
        for(QValueList< long >::ConstIterator it = windowIds->begin(); it != windowIds->end(); ++it)
        {
            long windowId = *it;
...
void Kded::updateDirWatch()
{
    ...
    QObject::connect(m_pDirWatch, SIGNAL(dirty(const QString &)), this, SLOT(update(const QString &)));
    QObject::connect(m_pDirWatch, SIGNAL(created(const QString &)), this, SLOT(update(const QString &)));
    QObject::connect(m_pDirWatch, SIGNAL(deleted(const QString &)), this, SLOT(dirDeleted(const QString &)));

    // For each resource
    for(QStringList::ConstIterator it = m_allResourceDirs.begin(); it != m_allResourceDirs.end(); ++it)
    {
        readDirectory(*it);
    }
}
...
void Kded::crashHandler(int)
{
    ...
    qWarning(
        ...
        DCOPClient::postMortemSender(), DCOPClient::postMortemObject(), DCOPClient::postMortemFunction());
}
...
bool Kded::process(const QCString &fun, const QByteArray &data, QCString &replyType, QByteArray &replyData)
{
    if(fun == "recreate()")
    {
        if(!m_recreateBusy)
...
KUpdateD::KUpdateD()
{
    ...
    QObject::connect(m_pDirWatch, SIGNAL(dirty(const QString &)), this, SLOT(slotNewUpdateFile()));
...
static KCmdLineOptions options[] = {{"check", I18N_NOOP("Check Sycoca database only once"), 0}, {"new-startup", "Internal", 0}, KCmdLineLastOption};
...
public:
    ...
    {
        startup = true;
        dcopClient()->connectDCOPSignal("DCOPServer", "", "terminateKDE()", objId(), "quit()", false);
    }
...
{
    KAboutData aboutData("kded", I18N_NOOP("KDE Daemon"), "$Id: kded.cpp 711061 2007-09-11 09:42:51Z tpatzig $",
    ...
    QObject::connect(client, SIGNAL(applicationRemoved(const QCString &)), kded, SLOT(slotApplicationRemoved(const QCString &)));
