...
static void track(const QString &menuId, const QString &menuName, QDict< KService > *includeList, QDict< KService > *excludeList,
                  QDict< KService > *itemList, const QString &comment)
{
...
void VFolderMenu::includeItems(QDict< KService > *items1, QDict< KService > *items2)
{
...
void VFolderMenu::matchItems(QDict< KService > *items1, QDict< KService > *items2)
{
...
void VFolderMenu::excludeItems(QDict< KService > *items1, QDict< KService > *items2)
{
    for(QDictIterator< KService > it(*items2); it.current(); ++it)
    {
...
void VFolderMenu::mergeMenu(SubMenu *menu1, SubMenu *menu2, bool reversePriority)
{
    if(m_track)
    {
        track(m_trackId, menu1->name, &(menu1->items), &(menu1->excludeItems), &(menu2->items),
              QString("Before MenuMerge w. %1 (incl)").arg(menu2->name));
        track(m_trackId, menu1->name, &(menu1->items), &(menu1->excludeItems), &(menu2->excludeItems),
              QString("Before MenuMerge w. %1 (excl)").arg(menu2->name));
    }
    if(reversePriority)
    {
    ...
    {
        track(m_trackId, menu1->name, &(menu1->items), &(menu1->excludeItems), &(menu2->items),
              QString("After MenuMerge w. %1 (incl)").arg(menu2->name));
        track(m_trackId, menu1->name, &(menu1->items), &(menu1->excludeItems), &(menu2->excludeItems),
              QString("After MenuMerge w. %1 (excl)").arg(menu2->name));
    }
...
void VFolderMenu::insertService(SubMenu *parentMenu, const QString &name, KService *newService)
{
    int i = name.find('/');
...
#define FOR_ALL_APPLICATIONS(it)                                                                                                                     \
    ...
    {                                                                                                                                                \
        for(QDictIterator< KService > it(info->applications); it.current(); ++it)                                                                    \
        {
...
KService *VFolderMenu::findApplication(const QString &relPath)
{
    for(appsInfo *info = m_appsInfoStack.first(); info; info = m_appsInfoStack.next())
    {
...
void VFolderMenu::addApplication(const QString &id, KService *service)
{
...
void VFolderMenu::buildApplicationIndex(bool unusedOnly)
{
    ...
    for(; appsInfo_it != m_appsInfoList.end(); ++appsInfo_it)
    {
        ...
        for(QDictIterator< KService > it(info->applications); it.current();)
        {
            ...
            for(QStringList::ConstIterator it2 = cats.begin(); it2 != cats.end(); ++it2)
            {
...
QString VFolderMenu::locateMenuFile(const QString &fileName)
{
    ...
    if(!xdgMenuPrefix.isEmpty())
    {
        ...
        QString baseName = QDir::cleanDirPath(m_docInfo.baseDir + fileInfo.dirPath() + "/" + fileNameOnly);
...
QString VFolderMenu::locateDirectoryFile(const QString &fileName)
{
    ...
    QString tmp;
    for(QStringList::ConstIterator it = m_directoryDirs.begin(); it != m_directoryDirs.end(); ++it)
    {
        tmp = (*it) + fileName;
...
void VFolderMenu::processCondition(QDomElement &domElem, QDict< KService > *items)
{
...
void VFolderMenu::processKDELegacyDirs()
{
    ...
    (void)KGlobal::dirs()->findAllResources("apps", QString::null,
                                            true, // Recursive!
                                            true, // uniq
                                            relFiles);
    for(QStringList::ConstIterator it = relFiles.begin(); it != relFiles.end(); ++it)
    {
...
void VFolderMenu::layoutMenu(VFolderMenu::SubMenu *menu, QStringList defaultLayout)
{
...
void VFolderMenu::markUsedApplications(QDict< KService > *items)
{
...
VFolderMenu::SubMenu *VFolderMenu::parseMenu(const QString &file, bool forceLegacyLoad)
{
