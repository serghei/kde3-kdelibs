...
bool KBuildSycoca::build()
{
    ...
    for(KSycocaFactory *factory = m_lstFactories->first(); factory; factory = m_lstFactories->next())
    {
        ...
        for(KSycocaResourceList::ConstIterator it1 = list->begin(); it1 != list->end(); ++it1)
        {
    ...
    bool uptodate = true;
    // For all resources
    for(QStringList::ConstIterator it1 = allResources.begin(); it1 != allResources.end(); ++it1)
    {
        g_changed = false;
        ...
        (void)KGlobal::dirs()->findAllResources(g_resource, QString::null,
                                                true, // Recursive!
                                                true, // uniq
                                                relFiles);
        ...
        for(g_factory = m_lstFactories->first(); g_factory; g_factory = m_lstFactories->next(), g_entryDict = entryDictList->next())
        {
            // For each resource the factory deals with
            ...
            for(KSycocaResourceList::ConstIterator it2 = list->begin(); it2 != list->end(); ++it2)
            {
                ...
                for(QStringList::ConstIterator it3 = relFiles.begin(); it3 != relFiles.end(); ++it3)
                {
    ...
    if(result || bMenuTest)
    {
        ...
        connect(g_vfolder, SIGNAL(newService(const QString &, KService **)), this, SLOT(slotCreateEntry(const QString &, KService **)));
        ...
        *g_allResourceDirs += g_vfolder->allDirectories();
        ...
        disconnect(g_vfolder, SIGNAL(newService(const QString &, KService **)), this, SLOT(slotCreateEntry(const QString &, KService **)));
    ...
    for(QDictIterator< KService > it(menu->items); it.current(); ++it)
    {
        if(bMenuTest)
        {
            if(!menu->isDeleted && !it.current()->noDisplay())
                printf("%s\t%s\t%s\n", caption.local8Bit().data(), it.current()->menuId().local8Bit().data(),
                       locate("apps", it.current()->desktopEntryPath()).local8Bit().data());
        }
        else
        {
...
bool KBuildSycoca::recreate()
{
    ...
    if(database->status() != 0)
    {
        ...
        if(!silent)
            KMessageBox::error(
                0, i18n("Error creating database '%1'.\nCheck that the permissions are correct on the directory and the disk is not full.\n")
    ...
    if(build()) // Parse dirs
    {
        ...
        if(!database->close())
        {
            ...
            if(!silent)
                KMessageBox::error(
                    0, i18n("Error writing database '%1'.\nCheck that the permissions are correct on the directory and the disk is not full.\n")
                           .arg(path.local8Bit().data()),
                    i18n("KBuildSycoca"));
#endif
            return false;
...
bool KBuildSycoca::checkDirTimestamps(const QString &dirname, const QDateTime &stamp, bool top)
{
    ...
    for(QFileInfoListIterator it(*list); it.current() != NULL; ++it)
    {
...
static KCmdLineOptions options[] = {{"nosignal", I18N_NOOP("Do not signal applications to update"), 0},
...
extern "C" KDE_EXPORT int kdemain(int argc, char **argv)
{
    ...
    KAboutData d(appName, I18N_NOOP("KBuildSycoca"), appVersion, I18N_NOOP("Rebuilds the system configuration cache."), KAboutData::License_GPL,
    ...
    QLabel progress(QString("<p><br><nobr>    %1    </nobr><br>").arg(i18n("Reloading KDE configuration, please wait...")), 0, "",
                    Qt::WType_Dialog | Qt::WStyle_DialogBorder | Qt::WStyle_Customize | Qt::WStyle_Title);
    QString capt = i18n("KDE Configuration Manager");
    if(!silent)
    {
        if(KMessageBox::No
           == KMessageBox::questionYesNo(0, i18n("Do you want to reload KDE configuration?"), capt, i18n("Reload"), i18n("Do Not Reload")))
            return 0;
    }
    ...
    while(true)
    {
        ...
        while(dcopClient->isApplicationRegistered(appName))
        {
            ...
            obj->connect(dcopClient, SIGNAL(applicationRemoved(const QCString &)), SLOT(deleteLater()));
    ...
    if(incremental || !checkfiles)
    {
        ...
        if((current_update_sig != ksycoca_update_sig) || (current_language != ksycoca_language) || (KSycoca::self()->timeStamp() == 0))
        {
            incremental = false;
            checkfiles = true;
            delete KSycoca::self();
    ...
    if(checkfiles && (!checkstamps || !KBuildSycoca::checkTimestamps(filestamp, oldresourcedirs)))
    {
        ...
        if(incremental)
        {
            ...
            // For each factory
            for(KSycocaFactory *factory = factories->first(); factory; factory = factories->next())
            {
