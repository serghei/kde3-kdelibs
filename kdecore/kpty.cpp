...
struct KPtyPrivate
{
    KPtyPrivate() : xonXoff(false), utf8(false), masterFd(-1), slaveFd(-1)
    {
...
gotpty:
    ...
    if(((st.st_uid != getuid()) || (st.st_mode & (S_IRGRP | S_IXGRP | S_IROTH | S_IWOTH | S_IXOTH))) && !chownpty(true))
    {
        kdWarning(175) << "chownpty failed for device " << ptyName << "::" << d->ttyName << "\nThis means the communication can be eavesdropped."
                       << endl;
    }
