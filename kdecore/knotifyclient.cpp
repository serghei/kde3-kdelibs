...
static bool canAvoidStartupEvent(const QString &event, const QString &appname, int present)
{
    ...
    static KConfig *configfile =
        appname != "ksmserver" ? new KConfig(appname + ".eventsrc", true, false) : new KConfig("knotify.eventsrc", true, false);
    static KConfig *eventsfile =
        appname != "ksmserver" ? new KConfig(appname + "/eventsrc", true, false, "data") : new KConfig("knotify/eventsrc", true, false, "data");
    ...
    if((ev1 == -2 && ev2 == -2) // unknown
       ...
       || (ev1 == -2 && ev2 > 0))
    { // not configured, has default presentation
        checkAvoid = false;
        return false;
    }
    return true;
...
static int sendNotifyEvent(const QString &message, const QString &text, int present, int level, const QString &sound, const QString &file, int winId)
{
    ...
    ds << message << appname << text << sound << file << present << level << winId << uniqueId;
...
int KNotifyClient::userEvent(const QString &text, int present, int level, const QString &sound, const QString &file)
{
...
int KNotifyClient::userEvent(int winId, const QString &text, int present, int level, const QString &sound, const QString &file)
{
