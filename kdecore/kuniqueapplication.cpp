...
static KCmdLineOptions kunique_options[] = {{"nofork", "Don't run in the background.", 0}, KCmdLineLastOption};
...
KUniqueApplication::KUniqueApplication(bool allowStyles, bool GUIenabled, bool configUnique)
    : KApplication(allowStyles, GUIenabled, initHack(configUnique)), DCOPObject(KCmdLineArgs::about->appName())
{
...
#ifdef Q_WS_X11
KUniqueApplication::KUniqueApplication(Display *display, Qt::HANDLE visual, Qt::HANDLE colormap, bool allowStyles, bool configUnique)
    : KApplication(display, visual, colormap, allowStyles, initHack(configUnique)), DCOPObject(KCmdLineArgs::about->appName())
{
    d = new KUniqueApplicationPrivate;
    d->processingRequest = false;
    d->firstInstance = true;
...
bool KUniqueApplication::process(const QCString &fun, const QByteArray &data, QCString &replyType, QByteArray &replyData)
{
    if(fun == "newInstance()")
    {
...
void KUniqueApplication::delayRequest(const QCString &fun, const QByteArray &data)
{
...
void KUniqueApplication::processDelayed()
{
    if(dcopClient()->isSuspended())
    {
        // Try again later.
