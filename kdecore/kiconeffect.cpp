...
QString KIconEffect::fingerprint(int group, int state) const
{
    ...
    if(cached.isEmpty())
    {
        ...
        cached += ':';
        cached += mTrans[group][state] ? QString::fromLatin1("trans") : QString::fromLatin1("notrans");
...
QImage KIconEffect::apply(QImage image, int group, int state) const
{
    ...
    return apply(image, mEffect[group][state], mValue[group][state], mColor[group][state], d->mColor2[group][state], mTrans[group][state]);
}
...
QPixmap KIconEffect::apply(QPixmap pixmap, int group, int state) const
{
    ...
    return apply(pixmap, mEffect[group][state], mValue[group][state], mColor[group][state], d->mColor2[group][state], mTrans[group][state]);
}

QPixmap KIconEffect::apply(QPixmap pixmap, int effect, float value, const QColor col, bool trans) const
{
    return apply(pixmap, effect, value, col, KGlobalSettings::baseColor(), trans);
}

QPixmap KIconEffect::apply(QPixmap pixmap, int effect, float value, const QColor col, const QColor col2, bool trans) const
{
    QPixmap result;
...
void KIconEffect::deSaturate(QImage &img, float value)
{
    ...
    for(i = 0; i < pixels; i++)
    {
        ...
        data[i] = qRgba(color.red(), color.green(), color.blue(), qAlpha(data[i]));
    }
}
...
void KIconEffect::visualActivate(QWidget *widget, QRect rect)
{
