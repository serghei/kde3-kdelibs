...
static QString encode(const QString &segment, int encoding_offset, int encoding_hint, bool isRawURI = false)
{
    ...
    for(int i = 0; i < old_length; i++)
    {
        ...
        if((character <= 32) || (character >= 127) || strchr(encode_string, character))
        {
    ...
    for(int i = 0; i < old_length; i++)
    {
        ...
        if((character < 32) ||      // Low ASCII
           ((character == '%') &&   // The escape character itself
            ...
            (hex2int(segment[i + 1].unicode()) != -1) && (hex2int(segment[i + 2].unicode()) != -1))
...
static QString cleanpath(const QString &_path, bool cleanDirSeparator, bool decodeDots)
{
    ...
    bool slash = (len && path[len - 1] == '/') || (len > 1 && path[len - 2] == '/' && path[len - 1] == '.');
    ...
    while(pos && (pos = path.findRev('/', --pos)) != -1)
    {
        ...
        else
        {
            ...
            if((len || !cleanDirSeparator) && (len != 1 || path[pos + 1] != '.'))
            {
                if(!cdUp)
...
QDataStream &operator<<(QDataStream &s, const KURL &a)
{
    ...
    s << a.m_strProtocol << a.m_strUser << a.m_strPass << a.m_strHost << a.m_strPath << a.m_strPath_encoded << QueryForWire << a.m_strRef_encoded
...
QDataStream &operator>>(QDataStream &s, KURL &a)
{
    ...
    s >> a.m_strProtocol >> a.m_strUser >> a.m_strPass >> a.m_strHost >> a.m_strPath >> a.m_strPath_encoded >> QueryFromWire >> a.m_strRef_encoded
        >> malf >> a.m_iPort;
...
KURL::KURL(const KURL &_u, const QString &_rel_url, int encoding_hint)
{
    ...
    if(!_u.m_strHost.isEmpty() && !rUrl.isEmpty() && rUrl.find(_u.m_strProtocol, 0, false) == 0 && rUrl[len] == ':'
       && (rUrl[len + 1] != '/' || (rUrl[len + 1] == '/' && rUrl[len + 2] != '/')))
    {
...
void KURL::parse(const QString &_url, int encoding_hint)
{
    ...
    // Proceed with :// :/ or :
    while(pos < len && (isalpha((int)buf[pos]) || isdigit((int)buf[pos]) || buf[pos] == '+' || buf[pos] == '-'))
        pos++;
...
void KURL::parseRawURI(const QString &_url, int encoding_hint)
{
    ...
    { // can't happen, the caller checked all this already
        reset();
        m_strProtocol = _url;
        m_iUriMode = Invalid;
        return;
...
void KURL::parseURL(const QString &_url, int encoding_hint)
{
    ...
    while((pos < len) && (buf[pos] != '@') && (buf[pos] != '/') && (buf[pos] != '?') && (buf[pos] != '#'))
        pos++;
...
bool KURL::operator==(const KURL &_u) const
{
    ...
    if(m_strProtocol == _u.m_strProtocol && m_strUser == _u.m_strUser && m_strPass == _u.m_strPass && m_strHost == _u.m_strHost
       && m_strPath == _u.m_strPath &&
       // The encoded path may be null, but the URLs are still equal (David)
       (m_strPath_encoded.isNull() || _u.m_strPath_encoded.isNull() || m_strPath_encoded == _u.m_strPath_encoded)
       && m_strQuery_encoded == _u.m_strQuery_encoded && m_strRef_encoded == _u.m_strRef_encoded && m_iPort == _u.m_iPort)
    {
        return true;
    }
...
KURL KURL::join(const KURL::List &lst)
{
    ...
    for(KURL::List::ConstIterator it = first; it != lst.end(); --it)
    {
        ...
        if(it != first)
        {
            if(!u.m_strRef_encoded)
                u.m_strRef_encoded = tmp.url();
            else
                u.m_strRef_encoded += "#" + tmp.url(); // Support more than one suburl thingy
        }
        tmp = u;
    }
...
QString KURL::fileName(bool _strip_trailing_slash) const
{
    QString fname;
    if(hasSubURL())
    { // If we have a suburl, then return the filename from there
...
QString KURL::directory(bool _strip_trailing_slash_from_result, bool _ignore_trailing_slash_in_path) const
{
...
{
    if((url.protocol() != base_url.protocol()) || (url.host() != base_url.host()) || (url.port() && url.port() != base_url.port())
       || (url.hasUser() && url.user() != base_url.user()) || (url.hasPass() && url.pass() != base_url.pass()))
    {
...
KURL::URIMode KURL::uriModeForProtocol(const QString &protocol)
{
    ...
    KURL::URIMode mode = Auto;
#endif
        if(protocol == "ed2k" || protocol == "sig2dat" || protocol == "slsk" || protocol == "data")
            mode = RawURI;
