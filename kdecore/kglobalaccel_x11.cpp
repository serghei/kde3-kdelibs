...
KGlobalAccelPrivate::KGlobalAccelPrivate() : KAccelBase(KAccelBase::NATIVE_KEYS), m_blocked(false), m_blockingDisabled(false), m_suspended(false)
{
    if(all_accels == NULL)
...
void KGlobalAccelPrivate::blockShortcuts(bool block)
{
    if(all_accels == NULL)
        return;
    for(QValueList< KGlobalAccelPrivate * >::ConstIterator it = all_accels->begin(); it != all_accels->end(); ++it)
    {
...
bool KGlobalAccelPrivate::grabKey(const KKeyServer::Key &key, bool bGrab, KAccelAction *pAction)
{
    if(!key.code())
    {
        kdWarning(125) << "KGlobalAccelPrivate::grabKey( " << key.key().toStringInternal() << ", " << bGrab << ", \""
                       << (pAction ? pAction->name().latin1() : "(null)") << "\" ): Tried to grab key with null code." << endl;
        return false;
    }
...
#endif
            if(bGrab)
                XGrabKey(qt_xdisplay(), keyCodeX, keyModX | irrelevantBitsMask, qt_xrootwin(), True, GrabModeAsync, GrabModeSync);
            else
...
bool KGlobalAccelPrivate::x11KeyPress(const XEvent *pEvent)
{
    ...
    kdDebug(125) << "x11KeyPress: seek " << key.toStringInternal()
                 << QString(" keyCodeX: %1 state: %2 keyModX: %3").arg(codemod.code, 0, 16).arg(pEvent->xkey.state, 0, 16).arg(codemod.mod, 0, 16)
                 << endl;
    ...
    if(!m_rgCodeModToAction.contains(codemod))
    {
        ...
        for(CodeModMap::ConstIterator it = m_rgCodeModToAction.begin(); it != m_rgCodeModToAction.end(); ++it)
        {
            ...
            kdDebug(125) << "\tcode: " << QString::number(it.key().code, 16) << " mod: " << QString::number(it.key().mod, 16)
                         << (pAction ? QString(" name: \"%1\" shortcut: %2").arg(pAction->name()).arg(pAction->shortcut().toStringInternal())
                                     : QString::null)
                         << endl;
        }
#endif
