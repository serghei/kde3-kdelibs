...
static ModInfo g_rgModInfo[KKey::MOD_FLAG_COUNT] = {{KKey::SHIFT, Qt::SHIFT, X11_ONLY(ShiftMask) I18N_NOOP("Shift"), QString()},
...
// Special Names List
static const SymName g_rgSymNames[] = {{XK_ISO_Left_Tab, "Backtab"},
...
#ifdef Q_WS_X11
static SymVariation g_rgSymVariation[] = {{'/', XK_KP_Divide, false}, {'*', XK_KP_Multiply, false},    {'-', XK_KP_Subtract, false},
                                          {'+', XK_KP_Add, false},    {XK_Return, XK_KP_Enter, false}, {0, 0, false}};
...
bool initializeMods()
{
    ...
    for(int i = Mod2MapIndex; i < 8; i++)
    {
        ...
        switch(keySymX)
        {
            case XK_Num_Lock:
                g_modXNumLock = mask;
                break; // Normally Mod2Mask
            case XK_Super_L:
            case XK_Super_R:
                g_rgModInfo[3].modX = mask;
                break; // Win key, Normally Mod4Mask
            case XK_Meta_L:
            case XK_Meta_R:
                if(!g_rgModInfo[3].modX)
                    g_rgModInfo[3].modX = mask;
                break; // Win alternate
            case XK_Scroll_Lock:
                g_modXScrollLock = mask;
                break; // Normally Mod5Mask
            case XK_Mode_switch:
...
// If it's a unicode character,
    ...
    else if(m_sym < 0x3000)
    {
        ...
        if((c.latin1() && c.isLetterOrNumber()) || (bUserSpace && !c.isSpace()))
            return c;
    }
...
QString Sym::toStringInternal() const
{
