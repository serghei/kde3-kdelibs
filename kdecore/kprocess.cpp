...
void KProcess::setEnvironment(const QString &name, const QString &value)
{
...
void KProcess::setupEnvironment()
{
    ...
    {
        setenv(QFile::encodeName(it.key()).data(), QFile::encodeName(it.data()).data(), 1);
    }
    if(!d->wd.isEmpty())
    {
...
int KProcess::commSetupDoneP()
{
    ...
    if(communication & Stdin)
    {
        ...
        QObject::connect(innot, SIGNAL(activated(int)), this, SLOT(slotSendData(int)));
    }
    ...
    if(communication & Stdout)
    {
        ...
        Q_CHECK_PTR(outnot);
        QObject::connect(outnot, SIGNAL(activated(int)), this, SLOT(slotChildOutput(int)));
        if(communication & NoRead)
            suspend();
    }
    ...
    if(communication & Stderr)
    {
        ...
        Q_CHECK_PTR(errnot);
        QObject::connect(errnot, SIGNAL(activated(int)), this, SLOT(slotChildError(int)));
    }
...
int KProcess::commSetupDoneC()
{
    ...
    {
        if(dup2(out[1], STDOUT_FILENO) < 0 || setsockopt(out[1], SOL_SOCKET, SO_LINGER, (char *)&so, sizeof(so)))
            ok = 0;
    ...
    {
        if(dup2(err[1], STDERR_FILENO) < 0 || setsockopt(err[1], SOL_SOCKET, SO_LINGER, (char *)&so, sizeof(so)))
            ok = 0;
    }
...
KShellProcess::KShellProcess(const char *shellname) : KProcess()
{
