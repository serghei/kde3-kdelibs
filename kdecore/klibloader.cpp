...
KLibFactory::KLibFactory(QObject *parent, const char *name) : QObject(parent, name)
{
}
...
KLibrary::~KLibrary()
{
    ...
    if(m_objs.count() > 0)
    {
        ...
        for(; it.current(); ++it)
        {
            ...
            disconnect(it.current(), SIGNAL(destroyed()), this, SLOT(slotObjectDestroyed()));
        }
...
KLibFactory *KLibrary::factory()
{
    ...
    connect(m_factory, SIGNAL(objectCreated(QObject *)), this, SLOT(slotObjectCreated(QObject *)));
...
KLibWrapPrivate::KLibWrapPrivate(KLibrary *l, void *h) : lib(l), ref_count(1), handle(h), name(l->name()), filename(l->fileName())
{
    unload_mode = UNKNOWN;
...
KLibLoader::KLibLoader(QObject *parent, const char *name) : QObject(parent, name)
{
    s_self = this;
...
KLibrary *KLibLoader::library(const char *name)
{
    ...
    connect(wrap->lib, SIGNAL(destroyed()), this, SLOT(slotLibraryDestroyed()));
...
void KLibLoader::close_pending(KLibWrapPrivate *wrap)
{
    ...
    while((wrap = d->loaded_stack.first()))
    {
        ...
        if(d->unload_mode != KLibLoaderPrivate::UNLOAD && wrap->unload_mode != KLibWrapPrivate::UNLOAD)
            break;
        ...
        if(!deleted_one)
        {
            ...
            {
                kapp->clipboard()->setText(kapp->clipboard()->text(QClipboard::Selection), QClipboard::Selection);
            }
            if(kapp->clipboard()->ownsClipboard())
            {
                kapp->clipboard()->setText(kapp->clipboard()->text(QClipboard::Clipboard), QClipboard::Clipboard);
            }
        }
