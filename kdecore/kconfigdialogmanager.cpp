...
bool KConfigDialogManager::parseChildren(const QWidget *widget, bool trackChanges)
{
    ...
    QObject *object;
    for(QPtrListIterator< QObject > it(*listOfChildren); (object = it.current()); ++it)
    {
        ...
        if(widgetName && (strncmp(widgetName, "kcfg_", 5) == 0))
        {
            ...
            if(item)
            {
                ...
                else
                {
                    ...
                    if(cb && cb->editable())
                        connect(cb, SIGNAL(textChanged(const QString &)), this, SIGNAL(widgetModified()));
                }
            }
            else
            {
        ...
        else if(widgetName)
        {
            ...
            if(changedIt != changedMap.end())
            {
                if((!d->insideGroupBox || !childWidget->inherits("QRadioButton")) && !childWidget->inherits("QGroupBox"))
...
void KConfigDialogManager::updateWidgets()
{
    ...
    QWidget *widget;
    for(QDictIterator< QWidget > it(d->knownWidget); (widget = it.current()); ++it)
    {
