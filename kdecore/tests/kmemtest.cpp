...
void showMem()
{
    ...
    while(!done)
    {
        fgets(buf, 256, fs);
        buf[256] = 0;
        if((strncmp(buf, "VmLib", 5) == 0) || (strncmp(buf, "VmData", 6) == 0) || (strncmp(buf, "VmSize", 6) == 0) || (strncmp(buf, "VmExe", 5) == 0)
           || (strncmp(buf, "VmRSS", 5) == 0) || (strncmp(buf, "VmLck", 5) == 0))
            printf("%s", buf);
...
long showTotalMem()
{
    ...
    while(!done)
    {
        ...
        if(strncmp(buf, "Mem:", 4) == 0)
        {
            ...
            long buffers = 0;
            long cached = 0;
            sscanf(buf, "Mem: %ld %ld %ld %ld %ld %ld", &total, &used, &free, &shared, &buffers, &cached);
