...
QString KCompletion::makeCompletion(const QString &string)
{
    ...
    if(myCompletionMode == KGlobalSettings::CompletionShell && string == myLastString)
    {
    ...
    if(myCompletionMode == KGlobalSettings::CompletionPopup || myCompletionMode == KGlobalSettings::CompletionPopupAuto)
    {
        findAllCompletions(string, &d->matches, myHasMultipleMatches);
    ...
    if(!string.isEmpty())
    { // only emit match when string is not empty
...
void KCompletion::findAllCompletions(const QString &string, KCompletionMatchesWrapper *matches, bool &hasMultipleMatches) const
{
...
void KCompletion::extractStringsFromNode(const KCompTreeNode *node, const QString &beginning, KCompletionMatchesWrapper *matches,
...
void KCompletion::extractStringsFromNodeCI(const KCompTreeNode *node, const QString &beginning, const QString &restString,
                                           KCompletionMatchesWrapper *matches) const
{
    ...
    if(child1)
        extractStringsFromNodeCI(child1, beginning + *child1, newRest, matches);
    ...
    if(ch1.isLetter())
    {
        ...
        if(ch1 != ch2)
        {
            ...
            if(child2)
                extractStringsFromNodeCI(child2, beginning + *child2, newRest, matches);
        }
    }
}
...
void KCompletion::doBeep(BeepMode mode) const
{
    ...
    switch(mode)
    {
        case Rotation:
            ...
            break;
        case PartialMatch:
            if(myCompletionMode == KGlobalSettings::CompletionShell || myCompletionMode == KGlobalSettings::CompletionMan)
            {
...
KCompletionMatches::KCompletionMatches(bool sort_P) : _sorting(sort_P)
{
}

KCompletionMatches::KCompletionMatches(const KCompletionMatchesWrapper &matches) : _sorting(matches.sorting())
{
