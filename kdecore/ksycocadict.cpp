...
KSycocaDict::KSycocaDict() : d(0), mStr(0), mOffset(0)
{
}

KSycocaDict::KSycocaDict(QDataStream *str, int offset) : d(0), mStr(str), mOffset(offset)
{
...
void KSycocaDict::add(const QString &key, KSycocaEntry *payload)
{
    if(key.isEmpty())
...
uint KSycocaDict::hashKey(const QString &key)
{
    int l = key.length();
    register uint h = 0;
    ...
    for(int pass = 1; pass <= 2; pass++)
    {
        ...
        for(uint i = 0; i < mHashTableSize; i++)
        {
            if(hashTable[i].duplicates)
            {
                ...
                /*kdDebug(7011) << QString("Duplicate lists: Offset = %1 list_size = %2")
                 * .arg(hashTable[i].duplicate_offset,8,16).arg(dups->count()) << endl;
    */
