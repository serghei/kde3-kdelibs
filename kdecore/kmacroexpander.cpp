...
public:
    KMacroMapExpander(const QMap< KT, VT > &map, QChar c = '%') : KMacroExpanderBase(c), macromap(map)
    {
    }
...
template < class VT > class KMacroMapExpander< QChar, VT > : public KMacroExpanderBase {

public:
    KMacroMapExpander(const QMap< QChar, VT > &map, QChar c = '%') : KMacroExpanderBase(c), macromap(map)
    {
    }
...
template < class VT > int KMacroMapExpander< QChar, VT >::expandPlainMacro(const QString &str, uint pos, QStringList &ret)
{
...
template < class VT > int KMacroMapExpander< QChar, VT >::expandEscapedMacro(const QString &str, uint pos, QStringList &ret)
{
...
template < class VT > class KMacroMapExpander< QString, VT > : public KMacroExpanderBase {

public:
    KMacroMapExpander(const QMap< QString, VT > &map, QChar c = '%') : KMacroExpanderBase(c), macromap(map)
    {
    }
...
template < class VT > int KMacroMapExpander< QString, VT >::expandPlainMacro(const QString &str, uint pos, QStringList &ret)
{
    ...
    if(!sl)
        return 0;
    QMapConstIterator< QString, VT > it = macromap.find(QConstString(str.unicode() + pos, sl).string());
    if(it != macromap.end())
    {
        ret += it.data();
        return sl;
...
template < class VT > int KMacroMapExpander< QString, VT >::expandEscapedMacro(const QString &str, uint pos, QStringList &ret)
{
    if(str[pos + 1] == escapeChar())
    {
        ret += QString(escapeChar());
    ...
    if(!sl)
        return 0;
    QMapConstIterator< QString, VT > it = macromap.find(QConstString(str.unicode() + rpos, sl).string());
    if(it != macromap.end())
    {
        ret += it.data();
        return rsl;
...
int KCharMacroExpander::expandPlainMacro(const QString &str, uint pos, QStringList &ret)
{
...
int KCharMacroExpander::expandEscapedMacro(const QString &str, uint pos, QStringList &ret)
{
    if(str[pos + 1] == escapeChar())
    {
        ret += QString(escapeChar());
...
int KWordMacroExpander::expandPlainMacro(const QString &str, uint pos, QStringList &ret)
{
    if(isIdentifier(str[pos - 1].unicode()))
        return 0;
    uint sl;
...
int KWordMacroExpander::expandEscapedMacro(const QString &str, uint pos, QStringList &ret)
{
    if(str[pos + 1] == escapeChar())
    {
        ret += QString(escapeChar());
...
template < class KT, class VT > inline QString TexpandMacros(const QString &ostr, const QMap< KT, VT > &map, QChar c)
{
    QString str(ostr);
...
template < class KT, class VT > inline QString TexpandMacrosShellQuote(const QString &ostr, const QMap< KT, VT > &map, QChar c)
{
    QString str(ostr);
    KMacroMapExpander< KT, VT > kmx(map, c);
