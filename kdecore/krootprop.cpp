...
void KRootProp::sync()
{
    ...
    XChangeProperty(qt_xdisplay(), qt_xrootwin(), atom, XA_STRING, 8, PropModeReplace, (const unsigned char *)propString.utf8().data(),
                    propString.length());
    XFlush(qt_xdisplay());
}
...
void KRootProp::setProp(const QString &rProp)
{
    ...
    {
        unsigned char *buf = 0;
        if(XGetWindowProperty(qt_xdisplay(), qt_xrootwin(), atom, offset, 256, False, XA_STRING, &type, &format, &nitems, &bytes_after, &buf)
               == Success
           && buf)
        {
...
QString KRootProp::readEntry(const QString &rKey, const QString &pDefault) const
{
...
QFont KRootProp::readFontEntry(const QString &rKey, const QFont *pDefault) const
{
    QFont aRetFont;
    QFont aDefFont;
...
QColor KRootProp::readColorEntry(const QString &rKey, const QColor *pDefault) const
{
    QColor aRetColor;
    ...
    nGreen = aValue.mid(nIndex1 + 1, nIndex2 - nIndex1 - 1).toInt(&bOK);
