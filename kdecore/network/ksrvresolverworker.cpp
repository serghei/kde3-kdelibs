...
bool KSrvResolverWorker::run()
{
    ...
    else
    {
        ...
        for(mapit = myResults.begin(); mapit != myResults.end(); ++mapit)
        {
            ...
            // start the resolvers
            for(QValueList< Entry >::Iterator it = entries.begin(); it != entries.end(); ++it)
            {
                Entry &e = *it;
...
bool KSrvResolverWorker::postprocess()
{
    ...
    for(mapit = myResults.begin(), mapend = myResults.end(); mapit != mapend; ++mapit)
    {
        QValueList< Entry >::Iterator it = (*mapit).entries.begin(), end = (*mapit).entries.end();
        for(; it != end; ++it)
        {
            Entry &e = *it;
