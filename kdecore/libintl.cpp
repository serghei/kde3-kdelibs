...
char *k_nl_find_msg(struct kde_loaded_l10nfile *domain_file, const char *msgid)
{
    ...
    if(domain->hash_size > 2 && domain->hash_tab != NULL)
    {
        ...
        if(W(domain->must_swap, domain->orig_tab[nstr - 1].length) == len
           && strcmp(msgid, domain->data + W(domain->must_swap, domain->orig_tab[nstr - 1].offset)) == 0)
...
static inline unsigned long hash_string(const char *str_param)
{
...
void k_nl_load_domain(struct kde_loaded_l10nfile *domain_file)
{
    int fd;
    struct stat st;
    ...
    if(fstat(fd, &st) != 0 || st.st_size < (off_t)sizeof(struct mo_file_header))
    {
    ...
    data = (struct mo_file_header *)mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, fd, 0);
