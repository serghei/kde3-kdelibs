...
#define MMAP(addr, size, prot, flags)                                                                                                                \
    ((dev_zero_fd < 0) ? (dev_zero_fd = open("/dev/zero", O_RDWR), mmap((addr), (size), (prot), (flags), dev_zero_fd, 0))                            \
...
static void do_check_malloc_state()
{
    ...
    for(i = 1; i < NBINS; ++i)
    {
        ...
        for(p = last(b); p != b; p = p->bk)
        {
            ...
            if(i >= 2)
            {
                ...
                assert(idx == i);
                /* lists are sorted */
                assert(p->bk == b || (unsigned long)chunksize(p->bk) >= (unsigned long)chunksize(p));
            }
            /* chunk is followed by a legal chain of inuse chunks */
            for(q = next_chunk(p); (q != av->top && inuse(q) && (unsigned long)(chunksize(q)) >= MINSIZE); q = next_chunk(q))
    ...
    assert((unsigned long)(av->sbrked_mem) <= (unsigned long)(av->max_sbrked_mem));

    assert((unsigned long)(av->mmapped_mem) <= (unsigned long)(av->max_mmapped_mem));

    assert((unsigned long)(av->max_total_mem) >= (unsigned long)(av->mmapped_mem) + (unsigned long)(av->sbrked_mem));
}
#endif
        ...
        else if(!chunk_is_mmapped(p))
        {
            ...
            */
            ...
            if((unsigned long)(size) >= FASTBIN_CONSOLIDATION_THRESHOLD)
            {
                ...
                if((unsigned long)(chunksize(av->top)) >= (unsigned long)(av->trim_threshold))
...
#endif
{
    ...
    */
    ...
    if((unsigned long)(nb) >= (unsigned long)(av->mmap_threshold) && (av->n_mmaps < av->n_mmaps_max))
    {
    ...
    */
    ...
    assert((old_top == initial_top(av) && old_size == 0) || ((unsigned long)(old_size) >= MINSIZE && prev_inuse(old_top)));
...
#endif
{
    ...
    if(!chunk_is_mmapped(oldp))
    {
        ...
        else
        {
            ...
            if(next == av->top && (unsigned long)(newsize = oldsize + chunksize(next)) >= (unsigned long)(nb + MINSIZE))
            {
            ...
            else if(next != av->top && !inuse(next) && (unsigned long)(newsize = oldsize + chunksize(next)) >= (unsigned long)(nb))
            {
                newp = oldp;
                unlink(next, bck, fwd);
            }
    ...
    */
    ...
    { /* misaligned */
        ...
        */
        ...
        brk = (char *)mem2chunk(((unsigned long)(m + alignment - 1)) & -((signed long)alignment));
...
static void *sbrk(long size)
{
    ...
    assert((char *)g_last->top_reserved - g_last->reserve_size <= (char *)g_last->top_committed && g_last->top_committed <= g_last->top_reserved
                        ...
                        while(VirtualQuery(memory_info.BaseAddress, &memory_info, sizeof(memory_info)))
                        {
                            ...
                            printf("Query %p %d %s\n", memory_info.BaseAddress, memory_info.RegionSize,
                                   memory_info.State == MEM_FREE
                                       ? "FREE"
                                       : (memory_info.State == MEM_RESERVE ? "RESERVED" : (memory_info.State == MEM_COMMIT ? "COMMITTED" : "?")));
#endif
                            /* Region is free, well aligned and big enough: we are done */
                            if(memory_info.State == MEM_FREE && (unsigned)memory_info.BaseAddress % g_regionsize == 0
...
static int cpuinfo(int whole, unsigned long *kernel, unsigned long *user)
{
    if(whole)
    {
        ...
        int rc = GetProcessTimes(GetCurrentProcess(), (FILETIME *)&creation64, (FILETIME *)&exit64, (FILETIME *)&kernel64, (FILETIME *)&user64);
        if(!rc)
        {
            *kernel = 0;
            *user = 0;
    ...
    {
        __int64 creation64, exit64, kernel64, user64;
        int rc = GetThreadTimes(GetCurrentThread(), (FILETIME *)&creation64, (FILETIME *)&exit64, (FILETIME *)&kernel64, (FILETIME *)&user64);
        if(!rc)
        {
            *kernel = 0;
            *user = 0;
