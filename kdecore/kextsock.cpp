...
static bool process_flags(int flags, int &socktype, int &familyMask, int &outflags)
{
    ...
    /* check other flags */
    outflags = (flags & KExtendedSocket::passiveSocket ? KResolver::Passive : 0) | (flags & KExtendedSocket::canonName ? KResolver::CanonName : 0)
...
// default constructor
KExtendedSocket::KExtendedSocket() : sockfd(-1), d(new KExtendedSocketPrivate)
{
}

// constructor with hostname
KExtendedSocket::KExtendedSocket(const QString &host, int port, int flags) : sockfd(-1), d(new KExtendedSocketPrivate)
{
...
// same
KExtendedSocket::KExtendedSocket(const QString &host, const QString &service, int flags) : sockfd(-1), d(new KExtendedSocketPrivate)
{
...
bool KExtendedSocket::isIPv6Only()
{
    ...
    int on;
    socklen_t onsiz = sizeof(on);
    if(getsockopt(sockfd, IPPROTO_IPV6, IPV6_V6ONLY, (char *)&on, &onsiz) == -1)
    {
        setError(IO_UnspecifiedError, errno);
        return false;
    }
...
int KExtendedSocket::startAsyncLookup()
{
    ...
    if(!d->resRemote.isRunning())
    {
        ...
        QObject::connect(&d->resRemote, SIGNAL(finished(KResolverResults)), this, SLOT(dnsResultsReady()));
    ...
    if((d->flags & passiveSocket) == 0 && !d->resLocal.isRunning())
    {
        ...
        QObject::connect(&d->resLocal, SIGNAL(finished(KResolverResults)), this, SLOT(dnsResultsReady()));
    ...
    {
        d->status = lookupDone;
        emit lookupFinished(d->resRemote.results().count() + d->resLocal.results().count());
    }
    return 0;
}
...
int KExtendedSocket::accept(KExtendedSocket *&sock)
{
    ...
    if(newfd == -1)
    {
        ...
        kdWarning(170) << "Error accepting on socket " << sockfd << ":" << perror << endl;
        return -1;
    }
...
int KExtendedSocket::connect()
{
    ...
    KResolverResults remote = d->resRemote.results(), local = d->resLocal.results();
    ...
    for(it = remote.begin(), it2 = local.begin(); it != remote.end(); ++it)
    {
        ...
        if(doingtimeout && KSocks::self()->hasWorkingAsyncConnect())
        {
            ...
            setBufferSize(d->flags & inputBufferedSocket ? -1 : 0, d->flags & outputBufferedSocket ? -1 : 0);
...
void KExtendedSocket::closeNow()
{
    ...
    emit closed(closedNow | (readBufferSize() != 0 ? availRead : 0) | (writeBufferSize() != 0 ? dirtyWrite : 0));
}
        ...
        if(d->inMaxSize == -1 || cursize < (unsigned)d->inMaxSize)
        {
            do
            {
                ...
                else if(len == 0)
                {
                    ...
                    d->qsnIn = d->qsnOut = NULL;
                    d->status = done;
                    emit closed(involuntary | (readBufferSize() ? availRead : 0) | (writeBufferSize() ? dirtyWrite : 0));
                    return;
                }
                else
                {
...
void KExtendedSocket::connectionEvent()
{
    ...
    for(; d->current < remote.count(); d->current++)
    {
        ...
        if(local.count() != 0)
        {
            ...
            errno = 0;
            sockfd = ::socket(remote[d->current].family(), remote[d->current].socketType(), remote[d->current].protocol());
            setError(IO_ConnectError, errno);
            errcode = errno;
            if(sockfd == -1)
                continue; // cannot create this socket
            ...
            setIPv6Only(d->ipv6only);
            cleanError();
            if(KSocks::self()->bind(sockfd, local[localidx].address(), local[localidx].length()) == -1)
            {
                ::close(sockfd);
                sockfd = -1;
                continue;
        ...
        if(KSocks::self()->hasWorkingAsyncConnect())
            setBlockingMode(false);
        if(KSocks::self()->connect(sockfd, remote[d->current].address(), remote[d->current].length()) == -1)
        {
            if(errno != EWOULDBLOCK && errno != EINPROGRESS)
            {
                setError(IO_ConnectError, errno);
...
int KExtendedSocket::resolve(sockaddr *sock, ksocklen_t len, QString &host, QString &port, int flags)
{
...
int KExtendedSocket::resolve(::KSocketAddress *sock, QString &host, QString &port, int flags)
{
    return resolve(sock->data, sock->datasize, host, port, flags);
}

QPtrList< KAddressInfo > KExtendedSocket::lookup(const QString &host, const QString &port, int userflags, int *error)
{
    kdDebug(170) << "Deprecated function called:" << k_funcinfo << endl;
