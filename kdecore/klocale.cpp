...
void KLocale::initLanguageList(KConfig *config, bool useEnv)
{
    ...
    QStringList languageList;
    if(useEnv)
        languageList += QStringList::split(':', QFile::decodeName(::getenv("KDE_LANG")));
...
{
    for(QValueList< KCatalogue >::Iterator it = d->catalogues.begin(); it != d->catalogues.end(); ++it)
    {
...
{
    for(QValueList< KCatalogue >::ConstIterator it = d->catalogues.begin(); it != d->catalogues.end(); ++it)
    {
...
{
    const char *pluralFormString = I18N_NOOP(
...
void KLocale::initFormat()
{
    ...
    KSimpleConfig entry(locate("locale", QString::fromLatin1("l10n/%1/entry.desktop").arg(m_country)), true);
...
#define readConfigBoolEntry(key, default, save)                                                                                                      \
    ...
    readConfigBoolEntry("PositivePrefixCurrencySymbol", true, m_positivePrefixCurrencySymbol);
    readConfigBoolEntry("NegativePrefixCurrencySymbol", true, m_negativePrefixCurrencySymbol);
    readConfigNumEntry("PositiveMonetarySignPosition", (int)BeforeQuantityMoney, m_positiveMonetarySignPosition, SignPosition);
    readConfigNumEntry("NegativeMonetarySignPosition", (int)ParensAround, m_negativeMonetarySignPosition, SignPosition);
    ...
    readConfigNumEntry("MeasureSystem", (int)Metric, d->measureSystem, MeasureSystem);
    ...
    KSimpleConfig language(locate("locale", QString::fromLatin1("%1/entry.desktop").arg(m_language)), true);
...
#define read3ConfigBoolEntry(key, default, save)                                                                                                     \
    ...
    read3ConfigBoolEntry("DateMonthNamePossessive", false, d->dateMonthNamePossessive);
...
QString KLocale::catalogueFileName(const QString &language, const KCatalogue &catalog)
{
    QString path = QString::fromLatin1("%1/LC_MESSAGES/%2.mo").arg(language).arg(catalog.name());
...
bool KLocale::setLanguage(const QStringList &languages)
{
    ...
    for(QStringList::Iterator it = languageList.fromLast(); it != languageList.begin(); --it)
    {
...
bool KLocale::isApplicationTranslatedInto(const QString &language)
{
    ...
    // a stat
    QString sFileName = QString::fromLatin1("%1/LC_MESSAGES/%2.mo").arg(language).arg(appName);
...
void KLocale::splitLocale(const QString &aStr, QString &language, QString &country, QString &chrset)
{
    QString str = aStr;
...
QString KLocale::monthName(int i, bool shortName) const
{
    if(shortName)
        ...
        {
            case 1:
                return translate("January", "Jan");
            case 2:
                return translate("February", "Feb");
            case 3:
                return translate("March", "Mar");
            case 4:
                return translate("April", "Apr");
            case 5:
                return translate("May short", "May");
            case 6:
                return translate("June", "Jun");
            case 7:
                return translate("July", "Jul");
            case 8:
                return translate("August", "Aug");
            case 9:
                return translate("September", "Sep");
            case 10:
                return translate("October", "Oct");
            case 11:
                return translate("November", "Nov");
            case 12:
                return translate("December", "Dec");
        }
    else
        switch(i)
        {
...
QString KLocale::monthNamePossessive(int i, bool shortName) const
{
    if(shortName)
        ...
        {
            case 1:
                return translate("of January", "of Jan");
            case 2:
                return translate("of February", "of Feb");
            case 3:
                return translate("of March", "of Mar");
            case 4:
                return translate("of April", "of Apr");
            case 5:
                return translate("of May short", "of May");
            case 6:
                return translate("of June", "of Jun");
            case 7:
                return translate("of July", "of Jul");
            case 8:
                return translate("of August", "of Aug");
            case 9:
                return translate("of September", "of Sep");
            case 10:
                return translate("of October", "of Oct");
            case 11:
                return translate("of November", "of Nov");
            case 12:
                return translate("of December", "of Dec");
        }
    else
        switch(i)
        {
            case 1:
                return translate("of January");
            case 2:
                return translate("of February");
            case 3:
                return translate("of March");
            case 4:
                return translate("of April");
            case 5:
                return translate("of May long", "of May");
            case 6:
                return translate("of June");
            case 7:
                return translate("of July");
            case 8:
                return translate("of August");
            case 9:
                return translate("of September");
            case 10:
                return translate("of October");
            case 11:
                return translate("of November");
            case 12:
                return translate("of December");
        }
...
void KLocale::updateCatalogues()
{
    ...
    for(QValueList< KCatalogue >::Iterator it = d->catalogues.begin(); it != d->catalogues.end();)
    {
    ...
    for(QStringList::ConstIterator itLangs = d->languageList.begin(); itLangs != d->languageList.end(); ++itLangs)
    {
        for(QStringList::ConstIterator itNames = d->catalogNames.begin(); itNames != d->catalogNames.end(); ++itNames)
        {
...
QString KLocale::translate_priv(const char *msgid, const char *fallback, const char **translated, int *pluralType) const
{
    if(pluralType)
    {
...
QString KLocale::translate(const char *singular, const char *plural, unsigned long n) const
{
...
QString KLocale::translateQt(const char *context, const char *source, const char *message) const
{
    if(!source || !source[0])
    {
        kdWarning() << "KLocale: trying to look up \"\" in catalog. "
...
{
    // some defaults
    QString currency = symbol.isNull() ? currencySymbol() : symbol;
    if(precision < 0)
        precision = fracDigits();
    ...
    if(neg ? negativePrefixCurrencySymbol() : positivePrefixCurrencySymbol())
    {
        res.prepend(' ');
...
QString KLocale::formatNumber(const QString &numStr, bool round, int precision) const
{
    ...
    if((round && precision < 0) || !QRegExp("^[+-]?\\d+(\\.\\d+)*(e[+-]?\\d+)?$").exactMatch(tmpString))
        return numStr;
...
{
    if((timeFormat().contains(QString::fromLatin1("%I")) > 0) || (timeFormat().contains(QString::fromLatin1("%l")) > 0))
        return true;
    else
        return false;
}
...
QString KLocale::formatDateTime(const QDateTime &pDateTime, bool shortFormat, bool includeSeconds) const
{
...
const char *KLocale::encoding() const
{
    ...
    if(0 == qstrcmp("System", codecForEncoding()->name()))
    {
        ...
        if(GetLocaleInfoA(MAKELCID(MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), SORT_DEFAULT), LOCALE_IDEFAULTANSICODEPAGE, d->win32SystemEncoding + 3,
...
QStringList KLocale::languagesTwoAlpha() const
{
    ...
    for(QStringList::ConstIterator it = origList.begin(); it != origList.end(); ++it)
    {
        QString lang = *it;
        QStringList langLst;
        ...
        for(QStringList::ConstIterator langIt = langLst.begin(); langIt != langLst.end(); ++langIt)
        {
...
bool KLocale::setCharset(const QString &)
{
    return true;
}
QString KLocale::charset() const
{
    return QString::fromLatin1("UTF-8");
}
