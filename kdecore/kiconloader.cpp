...
void KIconThemeNode::queryIcons(QStringList *result, int size, KIcon::Context context) const
{
    // add the icons of this theme to it
    *result += theme->queryIcons(size, context);
}

void KIconThemeNode::queryIconsByContext(QStringList *result, int size, KIcon::Context context) const
{
    // add the icons of this theme to it
    *result += theme->queryIconsByContext(size, context);
}

KIcon KIconThemeNode::findIcon(const QString &name, int size, KIcon::MatchType match) const
{
...
{
    KIconLoaderDebug(KIconLoader *l, const QString &a) : loader(l), appname(a), valid(true)
    {
    }
...
KIconLoader::KIconLoader(const QString &_appname, KStandardDirs *_dirs)
{
    ...
    for(QValueList< KIconLoaderDebug >::Iterator it = kiconloaders->begin(); it != kiconloaders->end();)
    {
        if((*it).loader == this)
...
void KIconLoader::init(const QString &_appname, KStandardDirs *_dirs)
{
    ...
    d->mpDirs->addResourceType("appicon", KStandardDirs::kde_default("data") + appname + "/pics/");
    // ################## KDE4: consider removing the toolbar directory
    d->mpDirs->addResourceType("appicon", KStandardDirs::kde_default("data") + appname + "/toolbar/");
...
{
#ifdef KICONLOADER_CHECKS
    for(QValueList< KIconLoaderDebug >::Iterator it = kiconloaders->begin(); it != kiconloaders->end(); ++it)
    {
        if((*it).loader == this)
        {
            (*it).valid = false;
...
QString KIconLoader::removeIconExtensionInternal(const QString &name) const
{
    ...
    if(name != name_noext)
    {
        kdDebug(264) << "Application " << KGlobal::instance()->instanceName() << " loads icon " << name << " with extension." << endl;
    }
#endif
...
KIcon KIconLoader::findMatchingIcon(const QString &name, int size) const
{
    ...
    /* JRT: To follow the XDG spec, the order in which we look for an
       ...
       */
    for(KIconThemeNode *themeNode = d->links.first(); themeNode; themeNode = d->links.next())
    {
...
inline QString KIconLoader::unknownIconPath(int size) const
{
    ...
    if(!icon.isValid())
    {
        kdDebug(264) << "Warning: could not find \"Unknown\" icon for size = " << size << endl;
        return QString::null;
    }
    return icon.path;
}
...
QPixmap KIconLoader::loadIcon(const QString &_name, KIcon::Group group, int size, int state, QString *path_store, bool canReturnNull) const
{
    QString name = _name;
    QPixmap pix;
    QString key;
    ...
    if(group == KIcon::User)
    {
        ...
        QString path = (absolutePath) ? name : iconPath(name, KIcon::User, canReturnNull);
        if(path.isEmpty())
        {
            if(canReturnNull)
                return pix;
    ...
    if((path_store != 0L) || noEffectKey != d->lastImageKey)
    {
        // No? load it.
        KIcon icon;
    ...
    if(overlay)
    {
        ...
        if((overlay & KIcon::LockOverlay) && ((ovl = loadOverlay(theme->lockOverlay(), size)) != 0L))
            KIconEffect::overlay(*img, *ovl);
        if((overlay & KIcon::LinkOverlay) && ((ovl = loadOverlay(theme->linkOverlay(), size)) != 0L))
            KIconEffect::overlay(*img, *ovl);
        if((overlay & KIcon::ZipOverlay) && ((ovl = loadOverlay(theme->zipOverlay(), size)) != 0L))
            KIconEffect::overlay(*img, *ovl);
        if((overlay & KIcon::ShareOverlay) && ((ovl = loadOverlay(theme->shareOverlay(), size)) != 0L))
            KIconEffect::overlay(*img, *ovl);
    ...
    if(favIconOverlay)
    {
        ...
        int x = img->width() - favIcon.width() - 1, y = img->height() - favIcon.height() - 1;
        if(favIcon.depth() != 32)
            favIcon = favIcon.convertDepth(32);
        if(img->depth() != 32)
            *img = img->convertDepth(32);
        for(int line = 0; line < favIcon.height(); ++line)
        {
            QRgb *fpos = reinterpret_cast< QRgb * >(favIcon.scanLine(line));
            QRgb *ipos = reinterpret_cast< QRgb * >(img->scanLine(line + y)) + x;
            for(int i = 0; i < favIcon.width(); ++i, ++fpos, ++ipos)
...
QStringList KIconLoader::queryIconsByContext(int group_or_size, KIcon::Context context) const
{
    QStringList result;
    if(group_or_size >= KIcon::LastGroup)
    {
...
class KIconFactory : public QIconFactory {
public:
    KIconFactory(const QString &iconName_P, KIcon::Group group_P, int size_P, KIconLoader *loader_P);
    KIconFactory(const QString &iconName_P, KIcon::Group group_P, int size_P, KIconLoader *loader_P, bool canReturnNull);
...
QIconSet KIconLoader::loadIconSet(const QString &name, KIcon::Group g, int s, bool canReturnNull, bool immediateExistenceCheck)
{
    if(!d->delayedLoading)
...
QIconSet KIconLoader::loadIconSetNonDelayed(const QString &name, KIcon::Group g, int s, bool canReturnNull)
{
    QIconSet iconset;
...
KIconFactory::KIconFactory(const QString &iconName_P, KIcon::Group group_P, int size_P, KIconLoader *loader_P)
...
KIconFactory::KIconFactory(const QString &iconName_P, KIcon::Group group_P, int size_P, KIconLoader *loader_P, bool canReturnNull_P)
    : iconName(iconName_P), group(group_P), size(size_P), loader(loader_P), canReturnNull(canReturnNull_P)
{
    setAutoDelete(true);
}
...
QPixmap *KIconFactory::createPixmap(const QIconSet &, QIconSet::Size, QIconSet::Mode mode_P, QIconSet::State)
{
    ...
    { // active and normal icon are usually the same
        if(loader->iconEffect()->fingerprint(group, KIcon::ActiveState) == loader->iconEffect()->fingerprint(group, KIcon::DefaultState))
...
QPixmap DesktopIcon(const QString &name, int force_size, int state, KInstance *instance)
{
...
QPixmap BarIcon(const QString &name, int force_size, int state, KInstance *instance)
{
    KIconLoader *loader = instance->iconLoader();
...
QPixmap SmallIcon(const QString &name, int force_size, int state, KInstance *instance)
{
    KIconLoader *loader = instance->iconLoader();
...
QPixmap MainBarIcon(const QString &name, int force_size, int state, KInstance *instance)
{
    KIconLoader *loader = instance->iconLoader();
