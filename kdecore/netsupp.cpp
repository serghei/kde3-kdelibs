...
int kde_getaddrinfo(const char *name, const char *service, const struct addrinfo *hint, struct kde_addrinfo **result)
{
    ...
    if(hint && (hint->ai_family == PF_UNIX))
    {
        ...
        // "*" or "localhost"
        if(name != NULL && !(name[0] == '\0' || (name[0] == '*' && name[1] == '\0') || strcmp("localhost", name) == 0))
...
#ifdef AF_INET6
    else if(h->h_addrtype == AF_INET6 && (hint->ai_family == AF_INET6 || hint->ai_family == AF_UNSPEC))
...
static int make_inet(const char *name, int portnum, int protonum, struct addrinfo *p, const struct addrinfo *hint, struct addrinfo **result)
{
    ...
    do
    {
        ...
        if(name != NULL)
        {
            ...
            if(hint->ai_family == AF_INET6 || (hint->ai_family == AF_UNSPEC && strchr(name, ':') != NULL))
            {
                // yes, this is IPv6
...
int getaddrinfo(const char *name, const char *serv, const struct addrinfo *hint, struct addrinfo **result)
{
    ...
    if(hint == NULL || result == NULL)
        return EAI_BADFLAGS;
    if(hint->ai_family != AF_UNSPEC && hint->ai_family != AF_UNIX && hint->ai_family != AF_INET
#ifdef AF_INET6
       && hint->ai_family != AF_INET6
#endif
       )
        return EAI_FAMILY;
    if(hint->ai_socktype != 0 && hint->ai_socktype != SOCK_STREAM && hint->ai_socktype != SOCK_DGRAM)
        return EAI_SOCKTYPE;
    ...
    if(hint->ai_family == AF_UNIX || hint->ai_family == AF_UNSPEC)
    {
        ...
        else
        {
            ...
            if(hint->ai_family == AF_UNIX || ((name != NULL && *name == '/') || (serv != NULL && *serv == '/')))
            {
                *result = p;
...
int getnameinfo(const struct sockaddr *sa, ksocklen_t salen, char *host, size_t hostlen, char *serv, size_t servlen, int flags)
{
    ...
    else if(s.sa->sa_family == AF_INET)
    {
        ...
        {
            // have to do lookup
            struct hostent *h = gethostbyaddr((const char *)&s.sin->sin_addr, sizeof(s.sin->sin_addr), AF_INET);
    ...
    else if(s.sa->sa_family == AF_INET6)
    {
        ...
        {
            // have to do lookup
            struct hostent *h = gethostbyaddr((const char *)&s.sin->sin_addr, sizeof(s.sin->sin_addr), AF_INET6);
            if(h == NULL && flags & NI_NAMEREQD)
                return 1;
            else if(h == NULL)
                inet_ntop(AF_INET6, &s.sin6->sin6_addr, host, hostlen);
...
const char *inet_ntop(int af, const void *cp, char *buf, size_t len)
{
    ...
    if(af == AF_INET6)
    {
        ...
        if(KDE_IN6_IS_ADDR_V4MAPPED(p) || KDE_IN6_IS_ADDR_V4COMPAT(p))
            sprintf(buf2, "::%s%u.%u.%u.%u", KDE_IN6_IS_ADDR_V4MAPPED(p) ? "ffff:" : "", buf[12], buf[13], buf[14], buf[15]);
        else
        {
