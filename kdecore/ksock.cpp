...
public:
    ...
    KSocketPrivate() : readNotifier(0), writeNotifier(0)
    {
    }
};
...
// posterity.  Really.
KSocket::KSocket(int _sock) : sock(_sock), d(new KSocketPrivate)
{
...
KSocket::KSocket(const char *_host, unsigned short int _port, int _timeout) : sock(-1), d(new KSocketPrivate)
{
    connect(_host, _port, _timeout);
}

KSocket::KSocket(const char *_path) : sock(-1), d(new KSocketPrivate)
{
    connect(_path);
}
...
void KSocket::enableWrite(bool _state)
{
    if(_state)
    {
        if(!d->writeNotifier)
        {
            ...
            QObject::connect(d->writeNotifier, SIGNAL(activated(int)), this, SLOT(slotWrite(int)));
        }
        else
...
bool KSocket::initSockaddr(ksockaddr_in *server_name, const char *hostname, unsigned short int port, int domain)
{
    ...
    QPtrList< KAddressInfo > list = KExtendedSocket::lookup(hostname, QString::number(port), KExtendedSocket::ipv4Socket);
...
KServerSocket::KServerSocket(const char *_path, bool _bind) : sock(-1)
{
...
KServerSocket::KServerSocket(unsigned short int _port, bool _bind) : sock(-1)
{
    d = new KServerSocketPrivate();
    d->bind = _bind;
...
bool KServerSocket::init(const char *_path)
{
    ...
    KExtendedSocket *ks = new KExtendedSocket(QString::null, _path, KExtendedSocket::passiveSocket | KExtendedSocket::unixSocket);
    d->ks = ks;
...
bool KServerSocket::init(unsigned short int _port)
{
    d->port = _port;
    KExtendedSocket *ks;
    ks = new KExtendedSocket(QString::null, _port, KExtendedSocket::passiveSocket | KExtendedSocket::inetSocket);
    d->ks = ks;
