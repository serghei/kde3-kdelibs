...
{
    static const uchar iqm[] = {0x00, 0x00, 0x00, 0x00, 0xdc, 0x07, 0x00, 0xd8, 0x00, 0x00, 0x00, 0x10, 0x01, 0x00, 0x00, 0x38}; // \'"$`<>|;&(){}*?#
    ...
    for(uint pos = 0;;)
    {
        ...
        if(firstword)
        {
            if(c == '_' || (c >= 'A' && c <= 'Z') || (c >= 'a' && c <= 'z'))
            {
                ...
                do
                    cc = args[pos2++];
                while(cc == '_' || (cc >= 'A' && cc <= 'Z') || (cc >= 'a' && cc <= 'z') || (cc >= '0' && cc <= '9'));
                if(cc == '=')
                    goto metaerr;
            }
        }
            ...
            else
            {
                if(c == '\\')
                {
                    if(pos >= args.length())
                        goto quoteerr;
                    c = args.unicode()[pos++];
                    if(!c.isSpace() && !((flags & AbortOnMeta) ? isMeta(c) : isQuoteMeta(c)))
                        cret += '\\';
                }
...
{
    static const uchar iqm[] = {0xff, 0xff, 0xff, 0xff, 0xdd, 0x07, 0x00, 0xd8,
                                0x00, 0x00, 0x00, 0x10, 0x01, 0x00, 0x00, 0x38}; // 0-32 \'"$`<>|;&(){}*?#
