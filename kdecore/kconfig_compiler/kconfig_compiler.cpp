...
static const KCmdLineOptions options[] = {{"d", 0, 0},
...
public:
    ...
    CfgEntry(const QString &group, const QString &type, const QString &key, const QString &name, const QString &label, const QString &whatsThis,
             const QString &code, const QString &defaultValue, const QValueList< Choice > &choices, bool hidden)
        : mGroup(group)
        , mType(type)
        , mKey(key)
        , mName(name)
    ...
    void setChoices(const QValueList< Choice > &d)
    {
        mChoices = d;
    }
...
static void preProcessDefault(QString &defaultValue, const QString &name, const QString &type, const QValueList< CfgEntry::Choice > &choices,
                              QString &code)
{
    ...
    else if((type == "StringList" || type == "PathList") && !defaultValue.isEmpty())
    {
        ...
        {
            cpp << "  default" << name << ".append( QString::fromUtf8( \"" << *it << "\" ) );" << endl;
        }
    ...
    else if(type == "IntList")
    {
        ...
        for(it = defaults.begin(); it != defaults.end(); ++it)
        {
            cpp << "  default" << name << ".append( " << *it << " );" << endl;
        }
        defaultValue = "default" + name;
    }
}
...
CfgEntry *parseEntry(const QString &group, const QDomElement &element)
{
    ...
    CfgEntry *result = new CfgEntry(group, type, key, name, label, whatsThis, code, defaultValue, choices, hidden == "true");
    if(!param.isEmpty())
    {
...
static QString itemDeclaration(const CfgEntry *e)
{
    ...
    QString fCap = e->name();
    fCap[0] = fCap[0].upper();
    return "  KConfigSkeleton::Item" + itemType(e->type()) + "  *item" + fCap
...
{
    QString t = "new KConfigSkeleton::Item" + itemType(type) + "( currentGroup(), " + key + ", " + varPath(name) + param;
    if(type == "Enum")
        t += ", values" + name;
...
QString paramString(const QString &group, const QValueList< Param > &parameters)
{
    ...
    QString arguments;
    int i = 1;
    for(QValueList< Param >::ConstIterator it = parameters.begin(); it != parameters.end(); ++it)
    {
...
{
    KAboutData aboutData("kconfig_compiler", I18N_NOOP("KDE .kcfg compiler"), "0.3", I18N_NOOP("KConfig Compiler"), KAboutData::License_LGPL);
    aboutData.addAuthor("Cornelius Schumacher", 0, "schumacher@kde.org");
    aboutData.addAuthor("Waldo Bastian", 0, "bastian@kde.org");
    aboutData.addAuthor("Zack Rusin", 0, "zack@kde.org");
    aboutData.addCredit("Reinhold Kainhofer", "Fix for parametrized entries", "reinhold@kainhofer.com", "http://reinhold.kainhofer.com");
    aboutData.addCredit("Duncan Mac-Vicar P.", "dpointer support", "duncan@kde.org", "http://www.mac-vicar.com/~duncan");
  ...
  for( cfg = entries.first(); cfg; cfg = entries.next() ) {
    ...
    h << "#ifndef " << (!nameSpace.isEmpty() ? nameSpace.upper() + "_" : "") << className.upper() << "_H" << endl;
    h << "#define " << (!nameSpace.isEmpty() ? nameSpace.upper() + "_" : "") << className.upper() << "_H" << endl << endl;
    ...
    for(e = entries.first(); e; e = entries.next())
    {
        ...
        if(!values.isEmpty())
        {
            if(globalEnums)
            {
                ...
                cppPreamble +=
                    "const char* const " + className + "::" + enumName(e->param()) + "ToString[] = " + "{ \"" + values.join("\", \"") + "\" };\n";
            }
            else
            {
                ...
                h << "    };" << endl;
                cppPreamble += "const char* const " + className + "::" + enumName(e->param()) + "::enumToString[] = " + "{ \"" + values.join("\", \"")
                               + "\" };\n";
            }
        }
    }
    ...
    for(e = entries.first(); e; e = entries.next())
    {
        ...
        if(itemAccessors)
        {
            h << endl;
            h << "    /**" << endl;
            h << "      Get Item object corresponding to " << n << "()" << endl;
            h << "    */" << endl;
            h << "    Item" << itemType(e->type()) << " *" << getFunction(n) << "Item(";
            if(!e->param().isEmpty())
            {
                h << " " << cppType(e->paramType()) << " i ";
            }
    ...
    if(!cfgFileName.isEmpty())
        cpp << " QString::fromLatin1( \"" << cfgFileName << "\" ";
    if(cfgFileNameArg)
        cpp << " config ";
    if(!cfgFileName.isEmpty())
        cpp << ") ";
    ...
    for(e = entries.first(); e; e = entries.next())
    {
        ...
        if(e->type() == "Enum")
        {
            cpp << "  QValueList<KConfigSkeleton::ItemEnum::Choice> values" << e->name() << ";" << endl;
            QValueList< CfgEntry::Choice > choices = e->choices();
            QValueList< CfgEntry::Choice >::ConstIterator it;
            for(it = choices.begin(); it != choices.end(); ++it)
            {
        ...
        {
            // Normal case
            cpp << "  " << itemPath(e) << " = " << newItem(e->type(), e->name(), key, e->defaultValue()) << endl;
        ...
        else
        {
            ...
            for(int i = 0; i <= e->paramMax(); i++)
            {
                ...
                cpp << "  " << itemVarStr << " = " << newItem(e->type(), e->name(), paramString(key, e, i), defaultStr, QString("[%1]").arg(i))
                    << endl;
    ...
    if(dpointer)
    {
        ...
        for(e = entries.first(); e; e = entries.next())
        {
            ...
            {
                cpp << endl;
                cpp << "KConfigSkeleton::Item" << itemType(e->type()) << " *" << getFunction(n, className) << "Item(";
                if(!e->param().isEmpty())
                {
                    cpp << " " << cppType(e->paramType()) << " i ";
                }
