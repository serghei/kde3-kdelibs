...
class KDebugEntry {
public:
    KDebugEntry(int n, const QCString &d)
    {
        number = n;
        descr = d;
    }
...
{
    kDebugPrivate() : oldarea(0), config(0)
    {
    }
...
static void kDebugBackend(unsigned short nLevel, unsigned int nArea, const char *data)
{
    ...
    if((nLevel == KDEBUG_FATAL) && (!kDebug_data->config || kDebug_data->config->readNumEntry("AbortFatal", 1)))
        abort();
}
...
kdbgstream &kdbgstream::operator<<(const QWidget *widget)
{
    ...
    else
    {
        ...
        string += "geometry=" + QString().setNum(widget->width()) + "x" + QString().setNum(widget->height()) + "+" + QString().setNum(widget->x())
...
{
    static const char *const s_penStyles[] = {"NoPen", "SolidLine", "DashLine", "DotLine", "DashDotLine", "DashDotDotLine"};
...
{
    static const char *const s_brushStyles[] = {"NoBrush",       "SolidPattern",  "Dense1Pattern", "Dense2Pattern", "Dense3Pattern",
...
QString kdBacktrace(int levels)
{
    ...
    for(int i = 0; i < n; ++i)
        s += QString::number(i) + QString::fromLatin1(": ") + QString::fromLatin1(strings[i]) + QString::fromLatin1("\n");
    s += "]\n";
    if(strings)
        free(strings);
#endif
