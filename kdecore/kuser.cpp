...
public:
    ...
    KUserPrivate(long _uid, long _gid, const QString &_loginname, const QString &_fullname, const QString &_room, const QString &_workPhone,
                 const QString &_homePhone, const QString &_homedir, const QString &_shell)
        : valid(true)
        , uid(_uid)
        , gid(_gid)
...
KUser::KUser(const KUser &user) : d(user.d)
{
}
...
void KUser::fillPasswd(struct passwd *p)
{
    if(p)
    {
        ...
        d = new KUserPrivate(p->pw_uid, p->pw_gid, QString::fromLocal8Bit(p->pw_name), (gecosList.size() > 0) ? gecosList[0] : QString::null,
                             (gecosList.size() > 1) ? gecosList[1] : QString::null, (gecosList.size() > 2) ? gecosList[2] : QString::null,
                             (gecosList.size() > 3) ? gecosList[3] : QString::null, QString::fromLocal8Bit(p->pw_dir),
...
public:
    ...
    KUserGroupPrivate(long _gid, const QString &_name, const QValueList< KUser > &_users) : valid(true), gid(_gid), name(_name), users(_users)
    {
    }
};
...
KUserGroup::KUserGroup(const KUserGroup &group) : d(group.d)
{
}
...
void KUserGroup::fillGroup(struct group *p)
{
    ...
    d = new KUserGroupPrivate(p->gr_gid, QString::fromLocal8Bit(p->gr_name), users);
}
