...
KVMAllocator::Block *KVMAllocator::allocate(size_t _size)
{
    ...
    while(it != d->free_blocks.end())
    {
        if(it.data().size > _size)
        {
            ...
            Block block;
            kdDebug(180) << "VM alloc: using block from free list " << (long)free_block.start << " size =" << (long)free_block.size
                         << " request = " << _size << endl;
 ...
 */
void KVMAllocator::free(Block *block_p)
{
    Block block = *block_p;
    if(block.mmap)
    {
    ...
    if(before != d->free_blocks.end())
    {
        ...
        {
            // Merge blocks.
            kdDebug(180) << "VM merging: Block " << (long)block_before.start << " with " << (long)block.start << " (before)" << endl;
    ...
    if(after != d->free_blocks.end())
    {
        ...
        {
            // Merge blocks.
            kdDebug(180) << "VM merging: Block " << (long)block.start << " with " << (long)block_after.start << " (after)" << endl;
 ...
 */
void KVMAllocator::copy(void *dest, Block *src, int _offset, size_t length)
{
    (void)copyBlock(dest, src, _offset, length);
}

bool KVMAllocator::copyBlock(void *dest, Block *src, int _offset, size_t length)
{
 ...
 */
void KVMAllocator::copy(Block *dest, void *src, int _offset, size_t length)
{
    (void)copyBlock(dest, src, _offset, length);
}

bool KVMAllocator::copyBlock(Block *dest, void *src, int _offset, size_t length)
{
...
void *KVMAllocator::map(Block *block)
{
    ...
    void *result = mmap(0, block->length, PROT_READ | PROT_WRITE, MAP_SHARED, d->tempfile->handle(), block->start);
    block->mmap = result;
    return block->mmap;
}
