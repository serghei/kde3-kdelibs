...
KAccelAction::KAccelAction(const QString &sName, const QString &sLabel, const QString &sWhatsThis, const KShortcut &cutDef3, const KShortcut &cutDef4,
...
bool KAccelAction::init(const QString &sName, const QString &sLabel, const QString &sWhatsThis, const KShortcut &rgCutDefaults3,
                        const KShortcut &rgCutDefaults4, const QObject *pObjSlot, const char *psMethodSlot, bool bConfigurable, bool bEnabled)
{
    m_sName = sName;
    m_sLabel = sLabel;
...
bool KAccelActions::init(KConfigBase &config, const QString &sGroup)
{
    ...
    for(uint i = 0; it != mapEntry.end(); ++it, i++)
    {
        ...
        m_prgActions[i] = new KAccelAction(it.key(), it.key(), it.key(), cuts, cuts, 0, 0, // pObjSlot, psMethodSlot,
    ...
    for(uint i = 0; i < m_nSize; i++)
    {
        ...
        if(pAction && pAction->m_bConfigurable)
        {
            ...
            if(pAction2)
            {
                ...
                kdDebug(125) << "\t" << pAction->m_sName << " found: " << sOld << " => " << pAction2->m_cut.toStringInternal() << " = "
...
KAccelAction *KAccelActions::actionPtr(KKeySequence cut)
{
    ...
    {
        if(m_prgActions[i] == 0)
            kdWarning(125) << "KAccelActions::actionPtr( " << cut.toStringInternal() << " ): encountered null pointer at m_prgActions[" << i << "]"
                           << endl;
...
KAccelAction *KAccelActions::insert(const QString &sAction, const QString &sLabel, const QString &sWhatsThis, const KShortcut &rgCutDefaults3,
                                    ...
                                    bool bEnabled)
{
    ...
    KAccelAction *pAction =
        new KAccelAction(sAction, sLabel, sWhatsThis, rgCutDefaults3, rgCutDefaults4, pObjSlot, psMethodSlot, bConfigurable, bEnabled);
    insertPtr(pAction);
...
*/
bool KAccelActions::writeActions(const QString &sGroup, KConfigBase *pConfig, bool bWriteAll, bool bGlobal) const
{
