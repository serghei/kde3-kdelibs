...
static unsigned long windows_properties[2] = {NET::ClientList | NET::ClientListStacking | NET::NumberOfDesktops | NET::DesktopGeometry
                                                  | NET::DesktopViewport | NET::CurrentDesktop | NET::DesktopNames | NET::ActiveWindow | NET::WorkArea
                                                  | NET::KDESystemTrayWindows,
                                              NET::WM2ShowingDesktop};

static unsigned long desktop_properties[2] = {NET::NumberOfDesktops | NET::DesktopGeometry | NET::DesktopViewport | NET::CurrentDesktop
                                                  | NET::DesktopNames | NET::ActiveWindow | NET::WorkArea | NET::KDESystemTrayWindows,
                                              NET::WM2ShowingDesktop};
...
public:
    KWinModulePrivate(int _what)
        : QWidget(0, 0)
        , NETRootInfo4(qt_xdisplay(), _what >= KWinModule::INFO_WINDOWS ? windows_properties : desktop_properties, 2, -1, false)
    ...
    struct StrutData
    {
        StrutData(WId window_, const NETStrut &strut_, int desktop_) : window(window_), strut(strut_), desktop(desktop_){};
...
KWinModule::KWinModule(QObject *parent) : QObject(parent, "kwin_module")
{
    init(INFO_ALL);
}

KWinModule::KWinModule(QObject *parent, int what) : QObject(parent, "kwin_module")
{
    init(what);
}
...
QSize KWinModulePrivate::numberOfViewports(int desktop) const
{
    ...
    QSize s(netdesktop.width / QApplication::desktop()->width(), netdesktop.height / QApplication::desktop()->height());
...
QPoint KWinModulePrivate::currentViewport(int desktop) const
{
    ...
    return QPoint(1 + (netviewport.x / QApplication::desktop()->width()), 1 + (netviewport.y / QApplication::desktop()->height()));
}
...
bool KWinModulePrivate::x11Event(XEvent *ev)
{
    if(ev->xany.window == qt_xrootwin())
    {
        ...
        {
            for(QPtrListIterator< KWinModule > mit(modules); mit.current(); ++mit)
                emit(*mit)->currentDesktopViewportChanged(currentDesktop(), currentViewport(currentDesktop()));
        }
...
{
    for(QValueList< StrutData >::Iterator it = strutWindows.begin(); it != strutWindows.end(); ++it)
        if((*it).window == w)
        {
...
QRect KWinModule::workArea(const QValueList< WId > &exclude, int desktop) const
{
    ...
    for(it1 = d->windows.begin(); it1 != d->windows.end(); ++it1)
    {
        ...
        for(; it2 != d->strutWindows.end(); ++it2)
            if((*it2).window == *it1)
                break;
...
void KWinModule::doNotManage(const QString &title)
{
    ...
    arg << title;
    kapp->dcopClient()->call("kwin", "", "doNotManage(QString)", data, replyType, replyData);
}
