...
{
    KConfigGroup g(KGlobal::config(), "KDE");
    return g.readNumEntry("VisualActivateSpeed", KDE_DEFAULT_VISUAL_ACTIVATE_SPEED);
}
...
QFont KGlobalSettings::largeFont(const QString &text)
{
    ...
    for(QStringList::ConstIterator it = fam.begin(); it != fam.end(); ++it)
    {
...
QRect KGlobalSettings::desktopGeometry(const QPoint &point)
{
    ...
    {
        KConfigGroup group(KGlobal::config(), "Windows");
        if(group.readBoolEntry("XineramaEnabled", true) && group.readBoolEntry("XineramaPlacementEnabled", true))
        {
...
{
    KConfigGroup g(KGlobal::config(), "KDE");
    return g.readBoolEntry("ShowIconsOnPushButtons", KDE_DEFAULT_ICON_ON_PUSHBUTTON);
}
...
{
    KConfigGroup g(KGlobal::config(), "KDE");
    return g.readBoolEntry("OpaqueResize", KDE_DEFAULT_OPAQUE_RESIZE);
}

int KGlobalSettings::buttonLayout()
{
    KConfigGroup g(KGlobal::config(), "KDE");
    return g.readNumEntry("ButtonLayout", KDE_DEFAULT_BUTTON_LAYOUT);
}
