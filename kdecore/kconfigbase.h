...
public:
     ...
     *  when writing it back.
     */
    void writeEntry(const QString &pKey, double nValue, bool bPersistent = true, bool bGlobal = false, char format = 'g', int precision = 6,
                    bool bNLS = false);
    /**
     * Writes a (key/value) pair.
     * Same as above, but writes a floating-point value.
     ...
     *  when writing it back.
     */
    void writeEntry(const char *pKey, double nValue, bool bPersistent = true, bool bGlobal = false, char format = 'g', int precision = 6,
                    bool bNLS = false);
     ...
     * @since 3.1.3
     */
    void writePathEntry(const QString &pKey, const QStringList &rValue, char sep = ',', bool bPersistent = true, bool bGlobal = false,
                        bool bNLS = false);
    /**
     * writePathEntry() overridden to accept a list of paths (strings).
     *
     ...
     * @since 3.1.3
     */
    void writePathEntry(const char *pKey, const QStringList &rValue, char sep = ',', bool bPersistent = true, bool bGlobal = false,
                        bool bNLS = false);
...
private:
    ...
    void writeEntry(const char *pKey, const QString &rValue, bool bPersistent, bool bGlobal, bool bNLS, bool bExpand);
    void writeEntry(const char *pKey, const QStringList &rValue, char sep, bool bPersistent, bool bGlobal, bool bNLS, bool bExpand);
};
...
public:
    ...
    KConfigGroupSaver(KConfigBase *config, QString group)
        ...
        : _config(config),
          _oldgroup(config->group())
    {
        _config->setGroup(group);
    }

    KConfigGroupSaver(KConfigBase *config, const char *group) : _config(config), _oldgroup(config->group())
    {
        _config->setGroup(group);
    }

    KConfigGroupSaver(KConfigBase *config, const QCString &group) : _config(config), _oldgroup(config->group())
    {
        _config->setGroup(group);
    }
