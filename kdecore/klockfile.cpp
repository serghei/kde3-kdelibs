...
#define FIELD_EQ(what) (st_buf1.what == st_buf2.what)
    return FIELD_EQ(st_dev) && FIELD_EQ(st_ino) && FIELD_EQ(st_uid) && FIELD_EQ(st_gid) && FIELD_EQ(st_nlink);
#undef FIELD_EQ
}
...
static KLockFile::LockResult lockFile(const QString &lockFile, KDE_struct_stat &st_buf, bool &linkCountSupport)
{
    ...
    (*(uniqueFile.textStream())) << QString::number(getpid()) << endl << instanceName << endl << hostname << endl;
    uniqueFile.close();
...
static KLockFile::LockResult deleteStaleLock(const QString &lockFile, KDE_struct_stat &st_buf, bool &linkCountSupport)
{
    ...
    if(!linkCountSupport && (KDE_lstat(lckFile, &st_buf2) == 0) && statResultIsEqual(st_buf, st_buf2))
    {
