...
public:
    ...
    void calculateArc(bool relative, QMemArray< ArtBpath > &vec, int &index, double &curx, double &cury, double angle, double x, double y, double r1,
                      double r2, bool largeArcFlag, bool sweepFlag)
    {
    ...
    static void art_vpath_render_bez(ArtVpath **p_vpath, int *pn, int *pn_max, double x0, double y0, double x1, double y1, double x2, double y2,
                                     double x3, double y3, double flatness)
    {
        ...
        do
        {
            ...
            switch(bez[bez_index].code)
            {
                ...
                case ART_END2:
                    ...
                    break;
                case ART_CURVETO:
                    art_vpath_render_bez(&vec, &vec_n, &vec_n_max, x, y, bez[bez_index].x1, bez[bez_index].y1, bez[bez_index].x2, bez[bez_index].y2,
                                         bez[bez_index].x3, bez[bez_index].y3, flatness);
                    x = bez[bez_index].x3;
                    y = bez[bez_index].y3;
                    break;
            }
    ...
    static void art_rgb_affine_run(int *p_x0, int *p_x1, int y, int src_width, int src_height, const double affine[6])
    {
        int x0, x1;
        double z;
        double x_intercept;
    ...
    static void art_rgba_rgba_affine(art_u8 *dst, int x0, int y0, int x1, int y1, int dst_rowstride, const art_u8 *src, int src_width, int src_height,
                                     int src_rowstride, const double affine[6])
    {
        int x, y;
        double inv[6];
        ...
        for(y = y0; y < y1; y++)
        {
            ...
            run_x0 = x0;
            run_x1 = x1;
            art_rgb_affine_run(&run_x0, &run_x1, y, src_width, src_height, inv);
...
void KSVGIconPainter::drawImage(double x, double y, QImage &image)
{
    ...
    d->helper->art_rgba_rgba_affine(d->helper->m_buffer, 0, 0, d->helper->m_width, d->helper->m_height, d->helper->m_rowstride, image.bits(),
                                    image.width(), image.height(), image.width() * 4, affine);
}
...
QWMatrix KSVGIconPainter::parseTransform(const QString &transform)
{
    ...
    for(; it != end; ++it)
    {
        ...
        else if(subtransform[0] == "matrix")
        {
            if(params.count() >= 6)
            {
                result.setMatrix(params[0].toDouble(), params[1].toDouble(), params[2].toDouble(), params[3].toDouble(), params[4].toDouble(),
                                 params[5].toDouble());
            }
        }
    }
