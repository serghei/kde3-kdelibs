...
KCatalogue::KCatalogue(const QString &name, const QString &language) : d(new KCataloguePrivate)
{
    d->name = name;
    d->language = language;
    ...
    QString path = QString::fromLatin1("%1/LC_MESSAGES/%2.mo").arg(d->language).arg(d->name);

    setFileName(locate("locale", path));
}

KCatalogue::KCatalogue(const KCatalogue &rhs) : d(new KCataloguePrivate)
{
    *this = rhs;
}
