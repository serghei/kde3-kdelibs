...
KKey::KKey(uint key, uint modFlags)
{
    init(key, modFlags);
}
KKey::KKey(int keyQt)
...
KKey::KKey(const QKeySequence &seq)
{
    init(seq);
}
KKey::KKey(const QKeyEvent *pEvent)
{
    init(pEvent);
}
...
bool KKey::init(int keyQt)
{
    ...
    // if( sym.initQt( keyQt )
    if(KKeyServer::keyQtToSym(keyQt, m_sym) && KKeyServer::keyQtToMod(keyQt, m_mod))
        return true;
    else
    {
        m_sym = 0;
...
bool KKey::init(const QKeyEvent *pEvent)
{
    ...
    if(pEvent->state() & Qt::ShiftButton)
        keyQt |= Qt::SHIFT;
    if(pEvent->state() & Qt::ControlButton)
        keyQt |= Qt::CTRL;
    if(pEvent->state() & Qt::AltButton)
        keyQt |= Qt::ALT;
    if(pEvent->state() & Qt::MetaButton)
        keyQt |= Qt::META;
    return init(keyQt);
}
...
bool KKey::init(const QString &sSpec)
{
    ...
    kdDebug(125) << "KKey::init( \"" << sSpec << "\" ):"
                 << " m_sym = " << QString::number(m_sym, 16) << ", m_mod = " << QString::number(m_mod, 16) << endl;
...
KKeySequence::KKeySequence(const QKeySequence &seq)
{
    init(seq);
}
...
KKeySequence::KKeySequence(const KKeySequence &seq)
{
    init(seq);
}
KKeySequence::KKeySequence(const QString &s)
{
    init(s);
}
...
KShortcut::KShortcut(const KKeySequence &seq)
{
    init(seq);
}
...
KShortcut::KShortcut(const char *ps)
{
    init(QString(ps));
}
...
bool KShortcut::contains(const KKeyNative &keyNative) const
{
    ...
    for(uint i = 0; i < count(); i++)
    {
        if(!m_rgseq[i].isNull() && m_rgseq[i].count() == 1 && m_rgseq[i].key(0) == key)
            return true;
    }
    return false;
}
...
bool KShortcut::append(const KShortcut &cut)
{
    ...
    for(uint i = 0; i < co; i++)
    {
        if(!contains(cut.seq(i)))
            seqs++;
    }
    if(seqs > MAX_SEQUENCES)
        return false;
