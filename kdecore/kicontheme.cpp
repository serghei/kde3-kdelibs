...
KIconTheme::KIconTheme(const QString &name, const QString &appName)
{
    ...
    if(!appName.isEmpty() && (name == "crystalsvg" || name == "hicolor" || name == "locolor"))
    {
    ...
    for(it = icnlibs.begin(); it != icnlibs.end(); ++it)
    {
        ...
        {
            themeDirs += cDir;
            if(mDir.isEmpty() && (KStandardDirs::exists(cDir + "index.desktop") || KStandardDirs::exists(cDir + "index.theme")))
                mDir = cDir;
        }
    }
...
QStringList KIconTheme::queryIcons(int size, KIcon::Context context) const
{
    ...
    for(; dirs.current(); ++dirs)
    {
        ...
        if((dir->type() == KIcon::Scalable) && (size >= dir->minSize()) && (size <= dir->maxSize()))
        {
            result += dir->iconList();
            continue;
        }
        if((dir->type() == KIcon::Threshold) && (abs(size - dir->size()) < dir->threshold()))
            result += dir->iconList();
    }
...
KIcon KIconTheme::iconPath(const QString &name, int size, KIcon::MatchType match) const
{
    ...
    for(; dirs.current(); ++dirs)
    {
        ...
        if(match == KIcon::MatchExact)
        {
            ...
            if((dir->type() == KIcon::Scalable) && ((size < dir->minSize()) || (size > dir->maxSize())))
                continue;
            if((dir->type() == KIcon::Threshold) && (abs(dir->size() - size) > dir->threshold()))
                continue;
        }
        else
        {
...
QString KIconTheme::current()
{
    ...
    *_theme = config->readEntry("Theme", defaultThemeName());
    if(*_theme == QString::fromLatin1("hicolor"))
