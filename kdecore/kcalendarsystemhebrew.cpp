...
static QString num2heb(int num, bool includeMillenium)
{
    const QChar decade[] = {0x05D8, 0x05D9, 0x05DB, 0x05DC, 0x05DE, 0x05E0, 0x05E1, 0x05E2, 0x05E4, 0x05E6};
    QString result;
...
// Ok
KCalendarSystemHebrew::KCalendarSystemHebrew(const KLocale *locale) : KCalendarSystem(locale)
{
}
...
// Ok
QString KCalendarSystemHebrew::monthName(const QDate &date, bool shortName) const
{
    return monthName(month(date), year(date), shortName);
}

// Ok
QString KCalendarSystemHebrew::monthNamePossessive(const QDate &date, bool shortName) const
{
...
// ### Fixme
QString KCalendarSystemHebrew::monthNamePossessive(int month, int year, bool shortName) const
{
...
// Ok
QString KCalendarSystemHebrew::weekDayName(const QDate &date, bool shortName) const
{
...
int KCalendarSystemHebrew::hndays(int mon, int year) const
{
    ...
    if(mon == 8 /*IYYAR*/ || mon == 10 /*TAMUZ*/ || mon == 12 /*ELUL*/ || mon == 4 /*TEVET*/ || mon == 14 /*ADAR 2*/
       || (mon == 6 /*ADAR*/ && !is_leap_year(year)) || (mon == 2 /*CHESHVAN*/ && !long_cheshvan(year))
...
static int heb2num(const QString &str, int &iLength)
{
    ...
    {
        c = s[pos];
        if(s.length() > pos && (s[pos + 1] == QChar('\'') || s[pos + 1] == QChar('\"')))
        {
            iLength++;
            s.remove(pos + 1, 1);
        }
        ...
        {
            if(s.length() > pos && s[pos + 1] >= QChar(0x05D0) && s[pos + 1] <= QChar(0x05EA))
        ...
        {
            if(s.length() > pos && s[pos + 1] >= QChar(0x05D0) && s[pos + 1] <= QChar(0x05EA) && s[pos + 1] != QChar(0x05D5)
