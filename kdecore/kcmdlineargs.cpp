...
public:
    KCmdLineParsedOptions() : QAsciiDict< QCString >(7)
    {
    }
...
public:
    KCmdLineParsedArgs() : QStrList(true)
    {
    }
    QDataStream &save(QDataStream &s) const
    {
...
void KCmdLineArgs::init(int _argc, char **_argv, const char *_appname, const char *programName, const char *_description, const char *_version,
                        bool noKApp)
{
    init(_argc, _argv, new KAboutData(_appname, programName, _version, _description), noKApp);
}

void KCmdLineArgs::init(int _argc, char **_argv, const char *_appname, const char *_description, const char *_version, bool noKApp)
{
    init(_argc, _argv, new KAboutData(_appname, _appname, _version, _description), noKApp);
}

void KCmdLineArgs::initIgnore(int _argc, char **_argv, const char *_appname)
{
    init(_argc, _argv, new KAboutData(_appname, _appname, "unknown", "KDE Application", false));
    ignoreUnknown = true;
}

void KCmdLineArgs::init(const KAboutData *ab)
{
...
void KCmdLineArgs::init(int _argc, char **_argv, const KAboutData *_about, bool noKApp)
{
    argc = _argc;
    argv = _argv;
...
void KCmdLineArgs::addCmdLineOptions(const KCmdLineOptions *options, const char *name, const char *id, const char *afterId)
{
    if(!argsList)
...
void KCmdLineArgs::findOption(const char *_opt, QCString opt, int &i, bool _enabled, bool &moreOptions)
{
...
void KCmdLineArgs::printQ(const QString &msg)
{
    QCString localMsg = msg.local8Bit();
    fprintf(stdout, "%s", localMsg.data());
}

void KCmdLineArgs::parseAllArgs()
{
    ...
    for(int i = 1; i < argc; i++)
    {
        ...
        if((argv[i][0] == '-') && argv[i][1] && inOptions)
        {
            ...
            else if((::qstrcmp(option, "version") == 0) || (::qstrcmp(option, "v") == 0))
            {
                printQ(QString("Qt: %1\n").arg(qVersion()));
...
void KCmdLineArgs::usage(const QString &error)
{
...
void KCmdLineArgs::usage(const char *id)
{
    enable_i18n();
    ...
    if(!(args->id) && (args->options) && (args->options->name) && (args->options->name[0] != '+'))
    {
        ...
        while(args)
        {
            ...
            while(option && option->name)
            {
                ...
                if(name[0] == '+')
                {
                    ...
                    printQ(optionFormatString.arg(name, -25).arg(description));
                }
                else
                {
                    if(!hasOptions)
                    ...
                    else
                    {
                        ...
                        if(!option->def)
                        {
                            printQ(optionFormatString.arg(opt, -25).arg(description));
                        }
                        else
                        {
                            printQ(optionFormatStringDef.arg(opt, -25).arg(description).arg(option->def));
                        }
                        opt = "";
                    }
                }
 ...
 */
KCmdLineArgs::KCmdLineArgs(const KCmdLineOptions *_options, const char *_name, const char *_id) : options(_options), name(_name), id(_id)
{
    parsedOptionList = 0;
    parsedArgList = 0;
...
void KCmdLineArgs::save(QDataStream &ds) const
{
    uint count = 0;
    if(parsedOptionList)
...
void KCmdLineArgs::load(QDataStream &ds)
{
    if(!parsedOptionList)
...
void KCmdLineArgs::setOption(const QCString &opt, bool enabled)
{
    if(isQt)
    {
...
void KCmdLineArgs::setOption(const QCString &opt, const char *value)
{
    if(isQt)
    {
...
QCString KCmdLineArgs::getOption(const char *_opt) const
{
    QCString *value = 0;
    if(parsedOptionList)
    {
...
bool KCmdLineArgs::isSet(const char *_opt) const
{
    // Look up the default.
    const char *opt_name;
    const char *def;
...
void KCmdLineArgs::addArgument(const char *argument)
{
    if(!parsedArgList)
        parsedArgList = new KCmdLineParsedArgs;
