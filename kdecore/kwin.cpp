/* This file is part of the KDE libraries
    ...
    $Id: kwin.cpp 715236 2007-09-21 15:08:48Z mueller $
...
ContextWidget::ContextWidget() : QWidget(0, 0)
{
    kwin_net_create_atoms();
    ...
    XGrabPointer(qt_xdisplay(), qt_xrootwin(), true,
                 (uint)(ButtonPressMask | ButtonReleaseMask | PointerMotionMask | EnterWindowMask | LeaveWindowMask), GrabModeAsync, GrabModeAsync,
...
void KWin::activateWindow(WId win, long time)
{
    ...
    if(time == 0)
        time = qt_x_user_time;
    info.setActiveWindow(win, NET::FromApplication, time, kapp->activeWindow() ? kapp->activeWindow()->winId() : 0);
#endif // Q_WS_X11 ...
...
void KWin::setMainWindow(QWidget *subwindow, WId mainwindow)
{
    ...
    if(mainwindow != 0)
    {
        ...
        */
        if(qt_cast< QDialog * >(subwindow) != NULL && subwindow->parentWidget() == NULL && kapp->mainWidget() != NULL)
        {
...
KWin::Info KWin::info(WId win)
{
    Info w;
#ifdef Q_WS_X11
    NETWinInfo inf(qt_xdisplay(), win, qt_xrootwin(), NET::WMState | NET::WMStrut | NET::WMWindowType | NET::WMName | NET::WMVisibleName
                                                          | NET::WMDesktop | NET::WMPid | NET::WMKDEFrameStrut | NET::XAWMState);
...
QPixmap KWin::icon(WId win, int width, int height, bool scale, int flags)
{
    ...
    if(flags & WMHints)
    {
        ...
        if(p != None)
        {
            ...
            XGetGeometry(qt_xdisplay(), p, &root, &x, &y, &w, &h, &border_w, &depth);
            if(w > 0 && h > 0)
            {
                QPixmap pm(w, h, depth);
                // Always detach before doing something behind QPixmap's back.
                pm.detach();
                XCopyArea(qt_xdisplay(), p, pm.handle(), qt_xget_temp_gc(qt_xscreen(), depth == 1), 0, 0, w, h, 0, 0);
                if(p_mask != None)
                {
                    QBitmap bm(w, h);
                    XCopyArea(qt_xdisplay(), p_mask, bm.handle(), qt_xget_temp_gc(qt_xscreen(), true), 0, 0, w, h, 0, 0);
                    pm.setMask(bm);
                }
                if(scale && width > 0 && height > 0 && !pm.isNull() && ((int)w != width || (int)h != height))
                {
    ...
    if(flags & ClassHint)
    {
        ...
        if(result.isNull())
        {
            ...
            if(XGetClassHint(qt_xdisplay(), win, &hint))
            {
                ...
                QPixmap pm = KGlobal::instance()->iconLoader()->loadIcon(className.lower(), KIcon::Small, iconWidth, KIcon::DefaultState, 0, true);
                if(scale && !pm.isNull())
                    result.convertFromImage(pm.convertToImage().smoothScale(width, height));
                else
                    result = pm;
    ...
    if(flags & XApp)
    {
        ...
        if(result.isNull())
        {
            QPixmap pm = KGlobal::instance()->iconLoader()->loadIcon("xapp", KIcon::Small, iconWidth, KIcon::DefaultState, 0, true);
            if(scale && !pm.isNull())
                result.convertFromImage(pm.convertToImage().smoothScale(width, height));
            else
                result = pm;
...
void KWin::setExtendedStrut(WId win, int left_width, int left_start, int left_end, int right_width, int right_start, int right_end, int top_width,
                            int top_start, int top_end, int bottom_width, int bottom_start, int bottom_end)
{
#ifdef Q_WS_X11
    NETWinInfo info(qt_xdisplay(), win, qt_xrootwin(), 0);
...
KWin::WindowInfo::WindowInfo(WId win, unsigned long properties, unsigned long properties2)
{
    ...
    d->ref = 1;
    if(properties == 0)
        properties = NET::WMState | NET::WMStrut | NET::WMWindowType | NET::WMName | NET::WMVisibleName | NET::WMIconName | NET::WMVisibleIconName
                     | NET::WMDesktop | NET::WMPid | NET::WMKDEFrameStrut | NET::XAWMState | NET::WMGeometry;
...
KWin::WindowInfo::WindowInfo() : d(NULL)
{
}
...
KWin::WindowInfo::WindowInfo(const WindowInfo &wininfo) : d(wininfo.d)
{
    if(d != NULL)
        ++d->ref;
}
...
{
#ifdef Q_WS_X11
    kdWarning((d->info->passedProperties()[NETWinInfo::PROTOCOLS] & NET::WMState) == 0, 176) << "Pass NET::WMState to KWin::windowInfo()" << endl;
    return d->info->state();
#else
    return 0;
#endif
...
{
#ifdef Q_WS_X11
    kdWarning((d->info->passedProperties()[NETWinInfo::PROTOCOLS] & NET::XAWMState) == 0, 176) << "Pass NET::XAWMState to KWin::windowInfo()" << endl;
...
{
#ifdef Q_WS_X11
    kdWarning((d->info->passedProperties()[NETWinInfo::PROTOCOLS] & NET::WMStrut) == 0, 176) << "Pass NET::WMStrut to KWin::windowInfo()" << endl;
    return d->info->strut();
#else
    NETStrut n;
    return n;
...
{
#ifdef Q_WS_X11
    kdWarning((d->info->passedProperties()[NETWinInfo::PROTOCOLS] & NET::WMWindowType) == 0, 176) << "Pass NET::WMWindowType to KWin::windowInfo()"
                                                                                                  << endl;
...
{
#ifdef Q_WS_X11
    kdWarning((d->info->passedProperties()[NETWinInfo::PROTOCOLS] & NET::WMVisibleName) == 0, 176) << "Pass NET::WMVisibleName to KWin::windowInfo()"
                                                                                                   << endl;
    return d->info->visibleName() && d->info->visibleName()[0] != '\0' ? QString::fromUtf8(d->info->visibleName()) : name();
#else
    return QString("name");
#endif
}
...
{
#ifdef Q_WS_X11
    kdWarning((d->info->passedProperties()[NETWinInfo::PROTOCOLS] & NET::WMName) == 0, 176) << "Pass NET::WMName to KWin::windowInfo()" << endl;
    return d->name_;
#else
    return QString();
#endif
...
{
#ifdef Q_WS_X11
    kdWarning((d->info->passedProperties()[NETWinInfo::PROTOCOLS] & NET::WMIconName) == 0, 176) << "Pass NET::WMIconName to KWin::windowInfo()"
                                                                                                << endl;
    if(d->info->iconName() && d->info->iconName()[0] != '\0')
        return QString::fromUtf8(d->info->iconName());
    if(!d->iconic_name_.isEmpty())
...
{
#ifdef Q_WS_X11
    kdWarning((d->info->passedProperties()[NETWinInfo::PROTOCOLS] & NET::WMDesktop) == 0, 176) << "Pass NET::WMDesktop to KWin::windowInfo()" << endl;
...
{
#ifdef Q_WS_X11
    kdWarning((d->info->passedProperties()[NETWinInfo::PROTOCOLS] & NET::WMGeometry) == 0, 176) << "Pass NET::WMGeometry to KWin::windowInfo()"
                                                                                                << endl;
    return d->geometry_;
#else
...
{
#ifdef Q_WS_X11
    kdWarning((d->info->passedProperties()[NETWinInfo::PROTOCOLS2] & NET::WM2WindowRole) == 0, 176) << "Pass NET::WM2WindowRole to KWin::windowInfo()"
                                                                                                    << endl;
...
QString KWin::readNameProperty(WId win, unsigned long atom)
{
    ...
    if(XGetTextProperty(qt_xdisplay(), win, &tp, atom) != 0 && tp.value != NULL)
    {
        ...
        else if(XmbTextPropertyToTextList(qt_xdisplay(), &tp, &text, &count) == Success && text != NULL && count > 0)
        {
