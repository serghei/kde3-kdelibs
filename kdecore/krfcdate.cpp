...
static const struct
{
    const char tzName[4];
    int tzOffset;
} known_zones[] = {{"UT", 0},     {"GMT", 0},    {"EST", -300}, {"EDT", -240}, {"CST", -360},    {"CDT", -300},
                   {"MST", -420}, {"MDT", -360}, {"PST", -480}, {"PDT", -420}, {{0, 0, 0, 0}, 0}};

time_t KRFCDate::parseDate(const QString &_date)
{
    if(_date.isEmpty())
        return 0;
...
time_t KRFCDate::parseDateISO8601(const QString &input_)
{
    if(input_.isEmpty())
        return 0;
...
int KRFCDate::localUTCOffset()
{
    ...
    unsigned int timeUTC = ymdhms_to_seconds(tM->tm_year + 1900, tM->tm_mon + 1, tM->tm_mday, tM->tm_hour, tM->tm_min, tM->tm_sec);

    tM = localtime(&timeNow);
    unsigned int timeLocal = ymdhms_to_seconds(tM->tm_year + 1900, tM->tm_mon + 1, tM->tm_mday, tM->tm_hour, tM->tm_min, tM->tm_sec);
...
QCString KRFCDate::rfc2822DateString(time_t utcTime, int utcOffset)
{
    ...
    dateStr.sprintf("%s, %02d %s %04d %02d:%02d:%02d %c%02d%02d", day_names[tM->tm_wday], tM->tm_mday, month_names[tM->tm_mon], tM->tm_year + 1900,
                    tM->tm_hour, tM->tm_min, tM->tm_sec, sgn, z / 60 % 24, z % 60);
