...
static void tokenize(list< QCString > &res, const QCString &text, char tr, bool strict = false)
{
    ...
    if(text.isEmpty())
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "tokenize: text is an empty string, done." << endl;
        return;
    }
    while(zwei != -1)
    {
    ...
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "tokenize: partened in " << res.size() << " parts.\n";
    // -----
...
KeyValueMap::KeyValueMap() : data(new StringStringMap)
{
    // ###########################################################################
    // ###########################################################################
}

KeyValueMap::KeyValueMap(const KeyValueMap &orig) : data(new StringStringMap(*orig.data))
{
    // ###########################################################################
    // ###########################################################################
}
...
void KeyValueMap::clear()
{
    ...
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::clear: erasing map contents ... " << endl;
    // -----
...
bool KeyValueMap::fill(const QString &filename, bool force, bool relax)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    if(file.open(IO_ReadOnly))
    {
        ...
        while(!stream.eof())
        {
            ...
            if(!line.isEmpty() /* && !stream.eof() */ && !isComment(line))
            {
                ...
                {
                    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::fill: could not insert line " << line << ".\n"; // ignore this case further
                }
            }
        }
        file.close();
    ...
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::fill: cannot open file " << filename << endl;
        return false;
    }
    // ###########################################################################
}

bool KeyValueMap::save(const QString &filename, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::save: saving data to -->" << filename << "<--.\n";
    ...
    if(!force)
    {
        if(file.exists())
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::save: file exists but may not." << endl;
            return false;
        }
    }
    ...
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::save: could not open file -->%s<-- for saving." << filename.utf8() << endl;
        return false;
    }
    // ###########################################################################
    return true;
...
bool KeyValueMap::save(QTextStream &file, int count)
{
    ...
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::save: saving data to given output stream." << endl;
    StringStringMap::iterator pos;
    bool ret = true;
...
bool KeyValueMap::erase(const QCString &key)
{
    // ###########################################################################
...
bool KeyValueMap::parseComplexString(const QCString &orig,
    ...
    const                                              // complex string in the original
{
    ...
    QCString mod;
    int count = 1;
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::parseComplexString: parsing the string -->" << orig << "<--.\n";
    // -----
    if(orig.isEmpty())
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::parseComplexString: string is empty.\n"
                                           "                                 "
    ...
    for(;;)
    {
        ...
        { // handle special characters
            ...
            kdDebug(GUARD, KAB_KDEBUG_AREA)
                    .form(
                        "KeyValueMap::parseComplexString: found "
...
QCString KeyValueMap::makeComplexString(const QCString &orig)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::makeComplexString: coding the string\n           -->" << orig
    ...
    // -----
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::makeComplexString: result is\n           -->" << temp << "<--.\n";
    return temp;
    // ###########################################################################
}

bool KeyValueMap::getRaw(const QCString &key, QCString &value) const
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::getRaw: trying to get raw value for key \"" << key << "\" ...\n";
    ...
    {
        value = (*pos).second;
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "in KeyValueMap, value is " << value << endl;
        return true;
    }
    // ###########################################################################
}

bool KeyValueMap::insertRaw(const QCString &key, const QCString &value, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insertRaw: inserting uncoded value " << value << " for key " << key << endl;
    int n = 0;
    // -----
    ...
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insertRaw: success" << (n == 0 ? "" : " (forced)") << endl;
        return true;
    }
    else
    {
...
bool KeyValueMap::insert(const QCString &key, const QCString &value, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insert: inserting value\n           -->" << value
                                    << "<-- \""
                                       "                     for key\n           -->"
                                    << key << "<--.\n";
...
bool KeyValueMap::insertLine(QCString line, bool force, bool relax, bool encode)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insertLine: inserting line -->" << line << "<--.\n";
    int index;
    QCString key;
    QCString value;
    ...
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insertLine: line is empty." << endl;
        return false;
    }
    // -----
    index = line.find('=');
    ...
    if(!key.isEmpty() && (relax == true ? 1 : !value.isEmpty()))
    {
        ...
        { // while loading from a already coded file:
...
bool KeyValueMap::get(const QCString &key, QCString &value) const
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[string]: "
                                       "trying to get value for key \""
                                    << key << "\" ... " << endl;
    QCString raw;
    QCString temp;
    // -----
    ...
    else
    {
                                        ...
                                        << endl;
        {
            ...
            {
                kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[string]: complex string found." << endl;
                value = temp;
            }
            else
            {
                kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[string]: this is no complex string." << endl;
...
bool KeyValueMap::insert(const QCString &key, const QString &value, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insert[QString]: trying to "
                                       "insert \""
                                    << (!value.isNull() ? "true" : "false") << "\" for key\n           -->" << v << "<--.\n";
    return insert(key, v, force);
    // ###########################################################################
}

bool KeyValueMap::get(const QCString &key, QString &value) const
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[QString]: trying to get "
                                       "a QString value for key "
                                    << key << endl;
    QCString v;
    // ----- get string representation:
    if(!get(key, v))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[QString]: key " << key << " not in KeyValueMap.\n";
        return false;
    }
    // ----- find its state:
...
bool KeyValueMap::insert(const QCString &key, const bool &value, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insert[bool]: trying to "
                                       "insert \""
                                    << (value == true ? "true" : "false") << "\" for key\n           -->" << key << "<--.\n";
    return insert(key, value == true ? "true" : "false", force);
    // ###########################################################################
}


bool KeyValueMap::get(const QCString &key, bool &value) const
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    if(!get(key, v))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[bool]: key " << key << " not in KeyValueMap.";
        return false;
    }
    // ----- find its state:
    v = v.stripWhiteSpace();
    ...
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[bool]: failure, unknown value." << endl;
    // -----
    return false;
    // ###########################################################################
}
...
bool KeyValueMap::insert(const QCString &key, const long &value, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
...
bool KeyValueMap::get(const QCString &key, long &value) const
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    if(!get(key, v))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[int]: key " << key << " not in KeyValueMap.\n";
        return false;
    }
    // -----
    temp = v.toLong(&ok);
...
bool KeyValueMap::insert(const QCString &key, const list< long > &values, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    // -----
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insert[long int list]: "
                                       "constructed string value is "
                                    << value << endl;
    return insert(key, value, force);
    // ###########################################################################
}

bool KeyValueMap::get(const QCString &key, list< long > &values) const
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[long int list]: trying "
                                       "to decode int list for key "
                                    << key << endl;
    ...
    if(!get(key, value))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[long int list]: no such key." << endl;
        return false;
    }
    tokenize(tokens, value, ',');
    if(tokens.empty())
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[long int list]: no tokens." << endl;
        return false;
    }
    // -----
...
bool KeyValueMap::insert(const QCString &key, const list< int > &values, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
...
bool KeyValueMap::get(const QCString &key, list< int > &values) const
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[int list]: trying to "
                                       "decode int list for key "
                                    << key << endl;
    ...
    if(!get(key, value))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[int list]: no such key." << endl;
        return false;
    }
    tokenize(tokens, value, ',');
    if(tokens.empty())
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[int list]: no tokens." << endl;
        return false;
    }
    // -----
    for(pos = tokens.begin(); pos != tokens.end(); ++pos)
...
bool KeyValueMap::insert(const QCString &key, const double &value, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA)
            .form(
                "KeyValueMap::insert[double]: trying to "
...
bool KeyValueMap::get(const QCString &key, double &value) const
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    if(!get(key, v))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[int]: key " << key << " not in "
                                                                                   "KeyValueMap."
                                        << endl;
        return false;
    }
...
bool KeyValueMap::get(const QCString &key, list< QCString > &values) const
{
    register bool GUARD;
    GUARD = false;
    ...
    if(!getRaw(key, raw))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[list<string>]: key " << key << " not in KeyValueMap." << endl;
        return false;
    }
    // -----
    for(;;)
    { // ----- parten the string down into a list, find special characters:
        ...
        if(second != -1)
        {
            ...
            { // ----- we are completely finished
                kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[list<string>]: list end found." << endl;
                break;
            }
        }
        else
...
bool KeyValueMap::insert(const QCString &key, const list< QCString > &values, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
...
bool KeyValueMap::get(const QCString &key, QStrList &values) const
{
    register bool GUARD;
    GUARD = false;
    ...
    if(!getRaw(key, raw))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[QStrList]: key " << key << " not in KeyValueMap." << endl;
        return false;
    }
    // -----
    for(;;)
...
bool KeyValueMap::insert(const QCString &key, const QStrList &values, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insert[QStrList]: coding string list." << endl;
    QCString value = "\"";
    QCString temp;
    unsigned int count;
    // ----- create coded string list:
    ...
    value += "\""; // finish the string
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insert[QStrList]: result of coding is %s." << value << endl;
    // ----- insert it without coding:
    return insertRaw(key, value, force);
    // ###########################################################################
}
...
bool KeyValueMap::get(const QCString &key, QStringList &values) const
{
    register bool GUARD;
    GUARD = false;
    ...
    if(!get(key, temp))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[QStringList]: key " << key << " not in KeyValueMap." << endl;
        return false;
    }
    // ----- do the conversion:
...
bool KeyValueMap::insert(const QCString &key, const QStringList &values, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::insert[QStringList]: coding QStringList." << endl;
...
bool KeyValueMap::insert(const QCString &key, const list< double > &values, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
...
bool KeyValueMap::get(const QCString &key, list< double > &values) const
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[double list]: trying to "
                                       "decode double list for key "
                                    << key << endl;
    ...
    if(!get(key, value))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[double list]: no such key." << endl;
        return false;
    }
    // -----
    tokenize(tokens, value, ',');
    for(pos = tokens.begin(); pos != tokens.end(); ++pos)
    {
        ...
        {
            kdDebug() << "KeyValueMap::get[double list]: conversion error for " << *pos << endl;
        }
    }
    // -----
...
bool KeyValueMap::insert(const QCString &key, const QDate &value, bool force)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
...
bool KeyValueMap::get(const QCString &key, QDate &date) const
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    if(!get(key, values))
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[QDate]: no such key." << endl;
        return false;
    }
    if(values.size() != 3)
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[QDate]: more or less than 3 values." << endl;
        return false;
    }
    y = values.front();
    values.pop_front();
    ...
    // -----
    if(y != 0 || m != 0 || d != 0)
        temp.setYMD(y, m, d); // avoid QDate messages
    if(!temp.isValid() && !temp.isNull())
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[QDate]: no valid date." << endl;
        return false;
    }
    else
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "KeyValueMap::get[QDate]: done." << endl;
        date = temp;
        return true;
    }
    // ###########################################################################
...
bool Section::add(const QCString &name)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::add: adding section \"" << name << "\" to "
                                                                                    "this section ..."
                                    << endl;
    Section *section;
    bool rc;
...
bool Section::add(const QCString &name, Section *section)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::add: added section " << name << " successfully.\n";
        return true;
    }
    else
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::add: failed to add section " << name << ", section already exists.\n";
        return false;
    }
    // ###########################################################################
}

bool Section::find(const QCString &name, StringSectionMap::iterator &result)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
...
bool Section::remove(const QCString &name)
{
    // ###########################################################################
    StringSectionMap::iterator pos;
    // -----
...
bool Section::find(const QCString &name, Section *&section)
{
    // ###########################################################################
    StringSectionMap::iterator pos;
    // -----
...
void Section::insertIndentSpace(QTextStream &file, int level)
{
    // ###########################################################################
    int i, j;
    // -----
...
bool Section::save(QTextStream &stream, int level)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    for(pos = sections.begin(); pos != sections.end(); ++pos)
    {
        ...
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::save: saved section \"" << (*pos).first << "\".\n";
        }
        insertIndentSpace(stream, level);
...
bool Section::readSection(QTextStream &file, bool finish)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    for(;;)
    {
        ...
        { // ----- verify it:
            if(file.eof())
            {
                kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::readSection: EOF, line is \"" << line << "\".\n";
                if(!line.isEmpty())
                {
                ...
                {
                    kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::readSection: EOF (no error)." << endl;
                    return true;
                }
            }
        }
...
bool Section::isBeginOfSection(QCString line)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
...
bool Section::isEndOfSection(QCString line)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::isEndOfSection: is " << line << " the end of"
                                                                                 " a section?"
                                    << endl;
    int first = 1, second;
    QCString temp;
    ...
    if(line[0] != '[' || line[line.length() - 1] != ']')
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::isBeginOfSection: does not match." << endl;
        return false;
    }
    // ----- find the word inside the brackets:
    for(first = 1; line[first] == ' '; ++first)
    ...
    if(temp == "END")
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::isBeginOfSection: yes, it is." << endl;
        return true;
    }
    else
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::isBeginOfSection: no, it is not." << endl;
        return false;
    }
    // ###########################################################################
}

QCString Section::nameOfSection(const QCString &line)
{
    register bool GUARD;
    GUARD = false;
    // ###########################################################################
    ...
    { // empty section names are not allowed
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "Section::isBeginOfSection: too short or empty line." << endl;
        return "";
    }
...
QConfigDB::QConfigDB(QWidget *parent, const char *name) : QWidget(parent, name), timer(0), readonly(true), locked(false), mtime(new QDateTime)
{
    // ###########################################################################
    hide();
    // ###########################################################################
...
bool QConfigDB::get(const list< QCString > &key, KeyValueMap *&map)
{
    register bool GUARD;
    GUARD = false;
    // ############################################################################
    ...
    if(key.empty())
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "ConfigDB::get: path is empty, returning toplevel section." << endl;
        map = top.getKeys();
        return true;
    }
...
bool QConfigDB::createSection(const list< QCString > &key)
{
    // ############################################################################
    Section *section = &top;
    unsigned int index;
...
bool QConfigDB::createSection(const QCString &desc)
{
    // ############################################################################
...
bool QConfigDB::get(const QCString &key, KeyValueMap *&map)
{
    // ############################################################################
...
list< QCString > QConfigDB::stringToKeylist(const QCString &desc)
{
    register bool GUARD;
    GUARD = false;
    ...
    if(desc.isEmpty())
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::stringToKeylist: path is empty." << endl;
        return key;
    }
    for(;;)
    {
        ...
        if(second == -1)
        {
            if((unsigned)first < desc.length() + 1)
            {
                ...
                kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::stringToKeylist: found last part " << temp << endl;
                key.push_back(temp);
            }
            break;
        }
        temp = desc.mid(first, second - first);
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::stringToKeylist: found part " << temp << endl;
        key.push_back(temp);
        first = second + 1;
    }
    // -----
...
bool QConfigDB::get(const QCString &key, Section *&section)
{
    // ############################################################################
...
bool QConfigDB::get(const list< QCString > &key, Section *&section)
{
    register bool GUARD;
    GUARD = false;
    // ############################################################################
...
int QConfigDB::IsLocked(const QString &file)
{
    register bool GUARD;
    GUARD = false;
    // ############################################################################
    ...
    if(access(QFile::encodeName(lockfile), F_OK) == 0)
    {
        ...
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::IsLocked: cannot open the lockfile." << endl;
            return -1;
        }
        return pid;
    }
...
bool QConfigDB::lock(const QString &file)
{
    register bool GUARD;
    GUARD = false;
    // ############################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::lock: locking the file " << file << endl;
    QString lockfile = file + ".lock";
    QFile f(lockfile);
    // -----
    if(access(QFile::encodeName(lockfile), F_OK) == 0)
...
bool QConfigDB::unlock()
{
    ...
    GUARD = false;
    // ############################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::unlock: unlocking the file " << filename << endl;
    ...
    if(access(QFile::encodeName(lockfile), F_OK | W_OK) == 0)
    {
        ...
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::unlock: lockfile deleted." << endl;
...
void QConfigDB::CleanLockFiles(int)
{
    register bool GUARD;
    GUARD = false;
    // ############################################################################
    ...
    kdDebug(GUARD, KAB_KDEBUG_AREA)
            .form(
                "QConfigDB::CleanLockFiles: removing %i "
                "remaining lockfiles.",
                LockFiles.size())
        << endl;
    for(pos = LockFiles.begin(); pos != LockFiles.end(); ++pos)
    {
        ...
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "                          " << *pos << " removed.\n";
            LockFiles.erase(pos);
            --pos;
        }
        else
...
bool QConfigDB::CheckLockFile(const QString &file)
{
    register bool GUARD;
    GUARD = false;
    // ############################################################################
...
bool QConfigDB::checkFileChanged()
{
    register bool GUARD;
    GUARD = false;
    // ############################################################################
    ...
    if(file.exists())
    {
        ...
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::checkFileChanged: file has been changed.n" << endl;
            emit(fileChanged());
            return true;
        }
        else
    ...
    else
    {
        ...
        { // it existed, and now it does no more
            emit(fileChanged());
            return true;
        }
    }
...
bool QConfigDB::setFileName(const QString &filename_, bool mustexist, bool readonly_)
{
    register bool GUARD;
    GUARD = false;
    // ############################################################################
    ...
    { // ----- it is stale:
        ...
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::setFileName: removed stale lockfile." << endl;
        }
        else
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::setFileName: cannot remove stale lockfile." << endl;
            return false;
        }
    }
    // -----
    if(mustexist)
    {
        ...
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::setFileName: permission granted." << endl;
            if(!readonly_)
            { //       we need r/w access:
                if(lock(filename_))
                {
    ...
    else
    {
        ...
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::setFileName: permission granted, new file." << endl;
            readonly = readonly_;
            filename = filename_;
            if(!readonly)
            {
                if(!lock())
                {
                    kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::setFileName: could not lock the file." << endl;
                    return false;
                }
            }
            storeFileAge();
...
bool QConfigDB::save(const char *header, bool force)
{
    register bool GUARD;
    GUARD = true;
    // ############################################################################
    kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::save: saving database -->" << filename << "<--.\n";
    bool wasRO = false;
    bool rc;
    // -----
    if(checkFileChanged())
    {
        kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::save: file is newer, not saving." << endl;
        return false;
    }
    if(force && isRO())
    {
        ...
        {
            wasRO = true;
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::save: switched to (forced) r/w mode." << endl;
        }
        else
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::save: cannot switch to (forced) r/w mode." << endl;
            return false;
        }
    }
    // ----- now save it:
    if(!isRO())
    {
        ...
        if(file.open(IO_WriteOnly))
        {
            ...
            stream << '#' << " [File created by QConfigDB object " << version() << "]" << endl;
            if(!top.save(stream)) // traverse tree
            {
                kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::save: error saving subsections." << endl;
            }
            storeFileAge();
            file.close();
            rc = true;
        ...
        {
            kdDebug() << "QConfigDB::save: error opening file \"" << filename << "\" for writing.\n";
            rc = false;
        }
    }
    else
    ...
    if(wasRO) // only true if we switched to forced r/w mode here
    {
        ...
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::save: reset (forced) r/w mode." << endl;
        }
        else
        {
            kdDebug(GUARD, KAB_KDEBUG_AREA) << "QConfigDB::save: cannot reset (forced) r/w mode." << endl;
            rc = false;
        }
    }
    // -----
