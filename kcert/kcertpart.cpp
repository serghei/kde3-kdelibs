...
KX509Item::KX509Item(KListViewItem *parent, KSSLCertificate *x) : KListViewItem(parent, 0L)
{
    setup(x);
}

KX509Item::KX509Item(KListView *parent, KSSLCertificate *x) : KListViewItem(parent)
{
    setup(x);
}
...
KPKCS12Item::KPKCS12Item(KListViewItem *parent, KSSLPKCS12 *x) : KListViewItem(parent, 0L)
{
    cert = x;
    if(x)
    {
...
KCertPart::KCertPart(QWidget *parentWidget, const char *widgetName, QObject *parent, const char *name, const QStringList & /*args*/)
    : KParts::ReadWritePart(parent, name)
{
    ...
    _baseGrid = new QGridLayout(_frame, 15, 9, KDialog::marginHint(), KDialog::spacingHint());
    ...
    QGridLayout *tabGrid = new QGridLayout(tab, 4, 5, KDialog::marginHint(), KDialog::spacingHint());
    ...
    tabGrid = new QGridLayout(tab, 4, 5, KDialog::marginHint(), KDialog::spacingHint());
    ...
    connect(_sideList, SIGNAL(selectionChanged(QListViewItem *)), this, SLOT(slotSelectionChanged(QListViewItem *)));
    setReadWrite(true);
}
...
bool KCertPart::openFile()
{
    ...
    if(whatType == "application/x-pkcs12")
    {
        ...
        while(!_p12)
        {
            ...
            if(!_p12)
            {
                rc = KMessageBox::warningContinueCancel(_frame, i18n("The certificate file could not be loaded. Try a different password?"),
                                                        i18n("Certificate Import"), i18n("Try Different"));
    ...
    else if(whatType == "application/x-x509-ca-cert" || whatType == "application/binary-certificate")
    {
        FILE *fp;
        bool isPEM = false;
        ...
        for(int i = 0; i < KOSSL::self()->OPENSSL_sk_num(sx5i); i++)
        {
            ...
            { // something else - maybe a CA file
                KSSLCertificate *xc = KSSLCertificate::fromX509(x5i->x509);
                if(xc->x509V3Extensions().certTypeCA())
                    new KX509Item(_parentCA, xc);
                else
...
void KCertPart::slotImport()
{
    ...
    else if(_ca)
    {
        ...
        _signers->addCA(_ca->toString(), _ca->x509V3Extensions().certTypeSSLCA(), _ca->x509V3Extensions().certTypeEmailCA(),
...
void KCertPart::slotImportAll()
{
    ...
    for(KPKCS12Item *t = dynamic_cast< KPKCS12Item * >(_parentP12->firstChild()); t; t = dynamic_cast< KPKCS12Item * >(t->nextSibling()))
    {
        if(t)
        {
            _p12 = t->cert;
    ...
    for(KX509Item *t = dynamic_cast< KX509Item * >(_parentCA->firstChild()); t; t = dynamic_cast< KX509Item * >(t->nextSibling()))
    {
        if(t)
        {
            _ca = t->cert;
