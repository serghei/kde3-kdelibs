...
int main(int argc, char **argv)
{
    ...
    KAboutData aboutData("meinproc", I18N_NOOP("XML-Translator"), "$Revision: 649205 $", I18N_NOOP("KDE Translator for XML"));
    ...
    if(index)
    {
        ...
        if(style_sheet != NULL)
        {
            ...
            if(res != NULL)
            {
                ...
                int wi = 0;
                for(PairList::ConstIterator it = list.begin(); it != list.end(); ++it, ++wi)
                    fprintf(stdout, "w\t%s\t%d\t%d\n", (*it).word.utf8().data(), 1000 * wi / (int)list.count(), (*it).base);
    ...
    else
    {
        ...
        else
        {
            ...
            while(true)
            {
                ...
                QString filename = output.mid(filename_index, output.find("\"", filename_index) - filename_index);
