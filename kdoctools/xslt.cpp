...
QString transform(const QString &pat, const QString &tss, const QValueVector< const char * > &params)
{
    QString parsed;
...
QString splitOut(const QString &parsed, int index)
{
    ...
    while(true)
    {
        ...
        //        kdDebug() << "FILENAME " << startindex << " " << endindex << " " << inside << " " << parsed.mid(startindex + 18, 15)<< " " <<
        //        parsed.length() << endl;
...
QString lookForCache(const QString &filename)
{
    ...
    if(readCache(filename, cache + "cache.bz2", output))
        return output;
    if(readCache(filename, locateLocal("cache", "kio_help" + cache + "cache.bz2"), output))
        return output;
