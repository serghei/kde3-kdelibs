...
{
    data(fromUnicode(QString("<html><head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=%1\"></head>\n"
                             "%2</html>")
                         .arg(QTextCodec::codecForLocale()->name())
                         .arg(t)));
}

HelpProtocol *slave = 0;

HelpProtocol::HelpProtocol(bool ghelp, const QCString &pool, const QCString &app) : SlaveBase(ghelp ? "ghelp" : "help", pool, app), mGhelp(ghelp)
{
    slave = this;
}
