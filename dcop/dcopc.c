...
void dcop_process_message(IceConn iceConn, IcePointer clientObject, int opcode, unsigned long length, Bool swap, IceReplyWaitInfo *replyWait,
                          Bool *replyWaitRet);
...
static IcePoVersionRec DCOPVersions[] = {{DCOPVersionMajor, DCOPVersionMinor, dcop_process_message}};
                          ...
                          Bool *replyWaitRet)
{
    ...
    IceReadMessageHeader(dcop_ice_conn, sizeof(struct DCOPMsg), struct DCOPMsg, pMsg);
...
Bool dcop_send_signal(const char *receiving_app, const char *object, const char *function, char *data, int dataLength)
{
    char *pos = 0L;
    char *header = 0L;
     ...
     */
    IceGetHeader(dcop_ice_conn, dcop_major_opcode, DCOPSend, sizeof(struct DCOPMsg), struct DCOPMsg, pMsgPtr);
    ...
    headerLength =
        strlen(sAnonymous) + 1 + strlen(receiving_app) + 1 + strlen(object) + 1 + strlen(function) + 1 + 4 * 5; /* 4 string lengths + 1 int */
...
Bool dcop_call(const char *appId, const char *remApp, const char *remObjId, const char *remFun, const char *data, int dataLength, char **replyType,
               char **replyData, int *replyDataLength)
{
    ...
    IceGetHeader(dcop_ice_conn, dcop_major_opcode, DCOPCall, sizeof(struct DCOPMsg), struct DCOPMsg, pMsg);
...
Bool dcop_attach_internal(Bool register_as_anonymous)
{
    fprintf(stderr, "dcop_attach_internal(%s)\n", register_as_anonymous ? "True" : "False");

    if(False == dcop_ice_register())
        return False;
    if(False == dcop_connect())
        return False;
    if(False == dcop_protocol_setup())
        return False;
...
char *dcop_register(const char *app_name, Bool add_pid)
{
    ...
    callStatus = dcop_call(dcop_requested_name, "DCOPServer", "", /* Object irrelevant */
                           "registerAs(QCString)", data, dataLength, &replyType, &replyData, &replyLen);
...
{
    dcop_major_opcode = IceRegisterForProtocolSetup((char *)("DCOP"), (char *)DCOPVendorString, (char *)DCOPReleaseString, 1,    /* What's this ? */
                                                    DCOPVersions, DCOPAuthCount, (char **)DCOPAuthNames, DCOPClientAuthProcs, 0L /* What's this ? */
                                                    );
...
Bool dcop_connect()
{
    ...
    dcop_ice_conn = IceOpenConnection(dcopServer, context, False, dcop_major_opcode, sizeof(errBuf), errBuf);
...
Bool dcop_protocol_setup()
{
    ...
    status = IceProtocolSetup(dcop_ice_conn, dcop_major_opcode, clientData, True, &(majorVersion), &(minorVersion), &(vendor), &(release),
                              BUFFER_SIZE, errBuf);

    return ((status == IceProtocolSetupSuccess) && (IceConnectionStatus(dcop_ice_conn) == IceConnectAccepted));
}
