...
KAudioPlayStream::KAudioPlayStream(KArtsServer *server, const QString title, QObject *p, const char *n)
    : QObject(p, n), d(new KAudioPlayStreamPrivate(server, title, this))
{
    kdDebug(400) << k_funcinfo << endl;
}
...
KByteSoundProducer::KByteSoundProducer(KAudioPlayStream *impl, float minBufferTime, int rate, int bits, int channels, const char *title)
    : _samplingRate(rate), _channels(channels), _bits(bits), _packets(7), _title(title), _impl(impl)
{
    ...
    {
        _packets++;
        streamBufferTime = (float)(_packets * packetCapacity * 1000) / (float)(_samplingRate * _channels * 2);
...
void KByteSoundProducer::streamStart()
{
    outdata.setPull(_packets, packetCapacity);
}
void KByteSoundProducer::streamEnd()
{
    outdata.endPull();
}
