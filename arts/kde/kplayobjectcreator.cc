...
bool KDE::PlayObjectCreator::create(const KURL &url, bool createBUS, const QObject *receiver, const char *slot)
{
    ...
    connect(this, SIGNAL(playObjectCreated(Arts::PlayObject)), receiver, slot);
    ...
    if(!url.isLocalFile())
    {
        ...
        connect(instream_impl, SIGNAL(mimeTypeFound(const QString &)), this, SLOT(slotMimeType(const QString &)));
    ...
    emit playObjectCreated(
        m_server.createPlayObjectForURL(std::string(QFile::encodeName(url.path())), std::string(mimetype->name().latin1()), createBUS));
    return true;
}
...
void KDE::PlayObjectCreator::slotMimeType(const QString &mimetype)
{
    ...
    playObject = m_server.createPlayObjectForStream(m_instream, std::string(mimetype_copy.latin1()), m_createBUS);
