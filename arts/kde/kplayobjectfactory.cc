...
KPlayObject *KPlayObjectFactory::createPlayObject(const KURL &_url, const QString &mimetype, bool createBUS)
{
    ...
    if(!m_server.isNull())
    {
        if(mimetype == "application/octet-stream" && m_allowStreaming)
        {
            ...
            return new KPlayObject(m_server.createPlayObjectForURL(string(QFile::encodeName(url.path())), string(mimetype.latin1()), createBUS),
                                   false);
    }
    else
...
KDE::PlayObject *KDE::PlayObjectFactory::createPlayObject(const KURL &_url, const QString &mimetype, bool createBUS)
{
    ...
    {
        kdWarning(400)
            << "KDE::PlayObjectFactory was instructed to use a Synth_AMAN_PLAY for output but the program also asked for a Synth_BUS_UPLINK" << endl;
        createBUS = false;
    }
    ...
    if(url.isLocalFile() || !d->allowStreaming
       || (url.protocol() == "audiocd" && mimetype == "application/x-cda" && mimeTypes().contains("application/x-cda")))
    {
        // we rely on the delivered mimetype if it's a local file
        d->playObj =
            new KDE::PlayObject(d->server.createPlayObjectForURL(string(QFile::encodeName(url.path())), string(mimetype.latin1()), createBUS), false);
    }
    else
    {
...
QStringList KDE::PlayObjectFactory::mimeTypes(void)
{
    ...
    QStringList results;
    for(vector< Arts::TraderOffer >::iterator offer = offers->begin(); offer != offers->end(); ++offer)
    {
        vector< string > *mimetypes = (*offer).getProperty("MimeType");

        for(vector< string >::iterator mimetype = mimetypes->begin(); mimetype != mimetypes->end(); ++mimetype)
        {
