...
public:
    Stream(SoundServer server, int rate, int bits, int channels, string name)
        : server(server), _finished(false), isAttached(false), _samplingRate(rate), _bits(bits), _channels(channels), pos(0), _name(name)
    {
    ...
    virtual int stream_get(arts_parameter_t param)
    {
        switch(param)
        {
            ...
            case ARTS_P_TOTAL_LATENCY:
                return stream_get(ARTS_P_SERVER_LATENCY) + stream_get(ARTS_P_BUFFER_TIME);
...
class Receiver : public ByteSoundReceiver_skel, public StdSynthModule, virtual public Stream {
    /*
...
public:
    Receiver(SoundServer server, int rate, int bits, int channels, string name) : Stream(server, rate, bits, channels, name)
    {
...
class Sender : public ByteSoundProducerV2_skel, public StdSynthModule, virtual public Stream {
    /*
     * FIXME: bsWrapper is a more or less ugly trick to be able to use
     * this object although not using smartwrappers to access it
     */
...
public:
    Sender(SoundServer server, int rate, int bits, int channels, string name) : Stream(server, rate, bits, channels, name)
    {
...
extern "C" ARTSC_EXPORT int arts_backend_write(arts_stream_t stream, const void *buffer, int count)
{
    if(!ArtsCApi::the())
        return ARTS_E_NOINIT;
...
extern "C" ARTSC_EXPORT int arts_backend_stream_set(arts_stream_t stream, arts_parameter_t param, int value)
{
    if(!ArtsCApi::the())
        return ARTS_E_NOINIT;
...
extern "C" ARTSC_EXPORT int arts_backend_stream_get(arts_stream_t stream, arts_parameter_t param)
{
    if(!ArtsCApi::the())
        return ARTS_E_NOINIT;
