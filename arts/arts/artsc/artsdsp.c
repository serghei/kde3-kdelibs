...
typedef void *(*orig_mmap_ptr)(void *start, size_t length, int prot, int flags, int fd, off_t offset);
...
static int is_sound_device(const char *pathname)
{
    ...
    if(strcmp(pathname, "/dev/dsp") == 0)
        return 1;
    if(strcmp(pathname, "/dev/sound/dsp") == 0)
        return 1;
    return 0;
}
    ...
    else if(sndfd != -1)
    {
        ...
        switch(request)
        {
            ...
            case SNDCTL_DSP_GETISPACE: /* _SIOR ('P',13, audio_buf_info) */
                ...
                {
                    audiop->fragstotal = stream ? arts_stream_get(stream, ARTS_P_PACKET_COUNT) : 10;
                    audiop->fragsize = stream ? arts_stream_get(stream, ARTS_P_PACKET_SIZE) : 16384;
                    audiop->bytes = stream ? arts_stream_get(stream, ARTS_P_BUFFER_SPACE) : 16384;
...
void *mmap(void *start, size_t length, int prot, int flags, int fd, off_t offset)
{
    ...
    else
    {
        ...
        if(mmapemu && length > 0)
        {
            ...
            artsdspdebug("aRts: /dev/dsp mmap (unsupported, try -m option)...\n");
    }
    return (void *)-1;
}
