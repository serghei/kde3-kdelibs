...
static void arts_backend_ref()
{
    if(backend.refcnt == 0)
    {
        ...
        if(backend.handle)
        {
            backend.init = (backend_init_ptr)dlsym(backend.handle, "arts_backend_init");
            backend.suspend = (backend_suspend_ptr)dlsym(backend.handle, "arts_backend_suspend");
            backend.suspended = (backend_suspended_ptr)dlsym(backend.handle, "arts_backend_suspended");
            backend.free = (backend_free_ptr)dlsym(backend.handle, "arts_backend_free");
            backend.play_stream = (backend_play_stream_ptr)dlsym(backend.handle, "arts_backend_play_stream");
            backend.record_stream = (backend_record_stream_ptr)dlsym(backend.handle, "arts_backend_record_stream");
            backend.close_stream = (backend_close_stream_ptr)dlsym(backend.handle, "arts_backend_close_stream");
            backend.write = (backend_write_ptr)dlsym(backend.handle, "arts_backend_write");
            backend.read = (backend_read_ptr)dlsym(backend.handle, "arts_backend_read");
            backend.stream_set = (backend_stream_set_ptr)dlsym(backend.handle, "arts_backend_stream_set");
            backend.stream_get = (backend_stream_get_ptr)dlsym(backend.handle, "arts_backend_stream_get");
        }

        if(backend.handle && backend.init && backend.free && backend.play_stream && backend.record_stream && backend.close_stream && backend.write
           && backend.read && backend.stream_set && backend.stream_get && backend.suspend)
            backend.available = 1;
        else
            backend.available = 0;
    }
...
int arts_suspended()
{
    ...
    if(backend.available && backend.suspended)
        rc = backend.suspended();
    return rc;
}
...
arts_stream_t arts_play_stream(int rate, int bits, int channels, const char *name)
{
    ...
    if(backend.available)
        rc = backend.play_stream(rate, bits, channels, name);
    return rc;
}
...
arts_stream_t arts_record_stream(int rate, int bits, int channels, const char *name)
{
    ...
    if(backend.available)
        rc = backend.record_stream(rate, bits, channels, name);
    return rc;
}
...
int arts_read(arts_stream_t stream, void *buffer, int count)
{
    ...
    if(backend.available)
        rc = backend.read(stream, buffer, count);
    return rc;
}
...
int arts_write(arts_stream_t stream, const void *buffer, int count)
{
    ...
    if(backend.available)
        rc = backend.write(stream, buffer, count);
    return rc;
}
...
int arts_stream_set(arts_stream_t stream, arts_parameter_t param, int value)
{
    ...
    if(backend.available)
        rc = backend.stream_set(stream, param, value);
    return rc;
}
...
int arts_stream_get(arts_stream_t stream, arts_parameter_t param)
{
    ...
    if(backend.available)
        rc = backend.stream_get(stream, param);
    return rc;
}
...
const char *arts_error_text(int errorcode)
{
    switch(errorcode)
    {
        ...
        case ARTS_E_NOBACKEND:
            return "loading the aRts backend \"" ARTSC_BACKEND "\" failed";
        case ARTS_E_NOIMPL:
