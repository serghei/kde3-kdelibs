 ...
 * else
 */
string createTypeCode(string type, const string &name, long model, string indent = "")
{
    string result = "";
    ...
    else if(type == "float")
    {
        ...
        if(model == MODEL_ARG_SEQ)
            result = "const std::vector<float>&";
        if(model == MODEL_RESULT)
            result = "float";
        if(model == MODEL_RESULT_SEQ)
            result = "std::vector<float> *";
        if(model == MODEL_STREAM)
            result = "float *" + name;
        if(model == MODEL_MSTREAM)
            result = "float **" + name;
        if(model == MODEL_ASTREAM)
            result = "Arts::FloatAsyncStream " + name;
        if(model == MODEL_AMSTREAM)
            assert(false);
        if(model == MODEL_ASTREAM_PACKETPTR)
            result = "Arts::DataPacket<float> *";
        if(model == MODEL_READ)
    ...
    {
        if(model == MODEL_MEMBER)
            result = "Arts::mcopbyte";
        if(model == MODEL_MEMBER_SEQ)
            result = "std::vector<Arts::mcopbyte>";
        if(model == MODEL_ARG)
            result = "Arts::mcopbyte";
        if(model == MODEL_ARG_SEQ)
            result = "const std::vector<Arts::mcopbyte>&";
        if(model == MODEL_RESULT)
            result = "Arts::mcopbyte";
        if(model == MODEL_RESULT_SEQ)
            result = "std::vector<Arts::mcopbyte> *";
        ...
        if(model == MODEL_ASTREAM_PACKETPTR)
            result = "Arts::DataPacket<Arts::mcopbyte> *";
    }
    else if(type == "long")
    {
        if(model == MODEL_MEMBER)
    ...
    else if(isStruct(type))
    {
        ...
        if(model == MODEL_RES_READ)
        {
            result = indent + "if(!result) return " + type + "(); // error occurred\n";
    ...
    else if(isEnum(type))
    {
        ...
        if(model == MODEL_RES_READ)
        {
            result = indent + "if(!result) return (" + type + ")0; // error occurred\n";
            result += indent + type + " returnCode = (" + type + ")result->readLong();\n";
            result += indent + "delete result;\n";
            result += indent + "return returnCode;\n";
        }
        if(model == MODEL_REQ_READ)
            result = indent + type + " " + name + " = (" + type + ")request->readLong();\n";
        if(model == MODEL_WRITE)
            result = "stream.writeLong(" + name + ")";
        if(model == MODEL_WRITE_SEQ)
    ...
    if((model & ~MODEL_SEQ) == MODEL_MEMBER || (model & ~MODEL_SEQ) == MODEL_ARG)
    {
        result += " ";
        result += name;
    }
...
string buildInheritanceList(const InterfaceDef &interface, const string &append)
{
    ...
    for(ii = interface.inheritedInterfaces.begin(); ii != interface.inheritedInterfaces.end(); ii++)
    {
        if(!first)
            result += ",\n\t";
        else
...
const char *generated_disclaimer = "/* this file was generated by the MCOP idl compiler - DO NOT EDIT */\n\n";
...
void doEnumHeader(FILE *header)
{
    ...
    for(edi = enums.begin(); edi != enums.end(); edi++)
    {
        ...
        if(fromInclude(ed.name))
            continue; // should come from the include

        nspace.setFromSymbol(ed.name);
        string ename = nspace.printableForm(ed.name);
        if(ename == "_anonymous_")
            ename = "";
...
void createStubCode(FILE *source, string iface, string method, const MethodDef &md)
{
    ...
    fprintf(source, "%s %s_stub::%s(%s)\n", rc.c_str(), iface.c_str(), method.c_str(), params.c_str());
    fprintf(source, "{\n");
    ...
    if(md.flags & methodTwoway)
    {
        ...
        fprintf(source,
                "\trequest = Arts::Dispatcher::the()->"
                "createRequest(requestID,_objectID,methodID);\n");
    }
    else
    {
        fprintf(source,
                "\tArts::Buffer *request = Arts::Dispatcher::the()->"
    ...
    if(md.flags & methodTwoway)
    {
        ...
        fprintf(source, "%s", createTypeCode(md.type, "", MODEL_RES_READ, "\t").c_str());
    }
    fprintf(source, "}\n\n");
}
...
void createDispatchFunction(FILE *source, long mcount, const InterfaceDef &d, const MethodDef &md, string name)
{
    ...
    else
    {
        if(md.type != "void")
        {
            cerr << "method " << md.name << " in interface " << d.name << " is declared oneway, but not void" << endl;
            exit(1);
        }
    }
...
void checkSymbolDefinition(const string &name, const string &type, const InterfaceDef &where, map< string, string > &defs)
{
    ...
    {
        cerr << idl_filename << ": warning: " << xwhere << " collides with " << mapentry << endl;
    }
}
...
void doInterfacesHeader(FILE *header)
{
    ...
    for(ii = interfaces.begin(); ii != interfaces.end(); ii++)
    {
        ...
        if(inherits.empty())
            inherits = "virtual public Arts::Object_base";
        ...
        fprintf(header,
                "\tstatic %s_base *_create(const std::string& subClass"
                " = \"%s\");\n",
                iname.c_str(), d.name.c_str());
        fprintf(header, "\tstatic %s_base *_fromString(const std::string& objectref);\n", iname.c_str());
        fprintf(header,
                "\tstatic %s_base *_fromReference(Arts::ObjectReference ref,"
                " bool needcopy);\n\n",
                iname.c_str());

        fprintf(header,
                "\tstatic %s_base *_fromDynamicCast(const Arts::Object&"
                " object);\n",
                iname.c_str());
        ...
        if(inherits.empty())
            inherits = "virtual public Arts::Object_stub";
        ...
        if(inherits.empty())
            inherits = "virtual public Arts::Object_skel";

        fprintf(header,
                "class ARTS_EXPORT %s_skel : virtual public %s_base,"
                " %s {\n",
                iname.c_str(), iname.c_str(), inherits.c_str());
        ...
        for(ai = d.attributes.begin(); ai != d.attributes.end(); ai++)
        {
            ...
            if((ad.flags & attributeStream) && (ad.flags & streamAsync))
            {
                ...
                string ptype = createTypeCode(ad.type, "", MODEL_ASTREAM_PACKETPTR);
        ...
        for(ai = d.attributes.begin(); ai != d.attributes.end(); ai++)
        {
            ...
            if((ad.flags & attributeAttribute) && (ad.flags & streamOut)
               && (ad.type == "byte" || ad.type == "float" || ad.type == "long" || ad.type == "string" || ad.type == "boolean" || ad.type == "*byte"
                   || ad.type == "*float" || ad.type == "*long" || ad.type == "*string" || isEnum(ad.type)))
            {
        ...
        fprintf(header,
                "\tvoid dispatch(Arts::Buffer *request, Arts::Buffer *result,"
                "long methodID);\n");
        ...
        */
        ...
        for(i = all.begin(); i != all.end(); i++)
        {
            ...
            for(mi = id.methods.begin(); mi != id.methods.end(); mi++)
            {
                ...
                {
                    fc.mname = md.name;
                    fprintf(header, "\tinline %s %s(%s);\n", fc.result.c_str(), md.name.c_str(), fc.params.c_str());
                }
    ...
    for(fi = forwardCode.begin(); fi != forwardCode.end(); fi++)
    {
        if(fi->constructor)
        {
            fprintf(header, "inline %s::%s(%s)\n", fi->fullifacename.c_str(), fi->mname.c_str(), fi->params.c_str());
        ...
        {
            fprintf(header, "inline %s %s::%s(%s)\n", fi->result.c_str(), fi->fullifacename.c_str(), fi->mname.c_str(), fi->params.c_str());
            fprintf(header, "{\n");
            fprintf(header,
                    "\t%s _cache?static_cast<%s*>(_cache)->%s(%s):"
                    "static_cast<%s*>(_method_call())->%s(%s);\n",
                    fi->result == "void" ? "" : "return", fi->baseclass.c_str(), fi->mname.c_str(), fi->callparams.c_str(), fi->baseclass.c_str(),
        ...
        for(interIt = interfaces.begin(); interIt != interfaces.end(); interIt++)
        {
            ...
            if(parent.name == (*si))
            {
                ...
                for(ai = parent.attributes.begin(); ai != parent.attributes.end(); ai++)
                {
                    ...
                    {
                        if(((ai->flags & streamIn) && (dir == defaultIn)) || ((ai->flags & streamOut) && (dir == defaultOut)))
                        {
                            found = true;
                            foundPos = ai;
                            break;
...
void doInterfacesSource(FILE *source)
{
    ...
    for(ii = interfaces.begin(); ii != interfaces.end(); ii++)
    {
        ...
        fprintf(source,
                "%s_base *%s_base::_fromReference(Arts::ObjectReference r,"
                " bool needcopy)\n",
                d.name.c_str(), d.name.c_str());
        fprintf(source, "{\n");
        fprintf(source, "\t%s_base *result;\n", d.name.c_str());
        fprintf(source, "\tresult = (%s_base *)Arts::Dispatcher::the()->connectObjectLocal(r,\"%s\");\n", d.name.c_str(), d.name.c_str());
        ...
        fprintf(source,
                "%s_stub::%s_stub(Arts::Connection *connection, "
                "long objectID)\n",
                d.name.c_str(), iname.c_str());
        ...
        string methodTableString = formatMultiLineString(methodTable.toString("MethodTable"), "        ");
        ...
        {
            fprintf(source,
                    "void %s_skel::notify(const Arts::Notification "
                    "&notification)\n",
                    d.name.c_str());
            fprintf(source, "{\n");
            for(ai = d.attributes.begin(); ai != d.attributes.end(); ai++)
            {
                AttributeDef &ad = *ai;
                if((ad.flags & (attributeStream | streamAsync)) == (attributeStream | streamAsync))
                {
                    const char *fname = (ad.flags & streamIn) ? "process" : "request";
                    string packettype = createTypeCode(ad.type, "", MODEL_ASTREAM_PACKETPTR);

                    fprintf(source, "\tif(%s.notifyID() == notification.ID)\n", ad.name.c_str());
                    fprintf(source, "\t\t%s_%s((%s)notification.data);\n", fname, ad.name.c_str(), packettype.c_str());
                }
            }
            fprintf(source, "}\n\n");
            ...
            {
                AttributeDef &ad = *ai;
                if((ad.flags & (attributeStream | streamAsync | streamOut)) == (attributeStream | streamAsync | streamOut))
                {
                    string packettype = createTypeCode(ad.type, "", MODEL_ASTREAM_PACKETPTR);
                    fprintf(source, "void %s_skel::request_%s(%s)\n", d.name.c_str(), ad.name.c_str(), packettype.c_str());
                    fprintf(source, "{\n");
                    fprintf(source,
                            "	assert(false); // this default is for "
        ...
        else if(state == filenameFind)
        {
            switch(*i++)
            {
                ...
                case '<':
                    ...
                    break;
                default:
                    cerr << "bad char after #include statement" << endl;
    ...
    while((c = getopt(argc, argv, "I:P:C:te:")) != -1)
    {
        switch(c)
        {
            case 'I':
                ...
                break;
            case 'P':
                packetTypes.push_back(optarg);
                break;
            case 'C':
                customIncludes.push_back(optarg);
                break;
            case 't':
                makeTypeInfo = true;
                break;
            case 'e':
                includedNames.push_back(optarg);
                break;
            default:
                exit_usage(argv[0]);
                break;
