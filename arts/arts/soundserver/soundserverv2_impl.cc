...
SoundServerV2_impl::SoundServerV2_impl() : _sampleStorage(SampleStorage(MCOPUtils::createFilePath("artsd-samples"), true))
{
    checkNewObjects();
}
...
PlayObject SoundServerV2_impl::createPlayObjectForURL(const std::string &url, const std::string &mimetype, bool createBUS)
{
    ...
    if(!objectType.empty())
    {
        ...
        if(result.loadMedia(url))
        {
            ...
            arts_warning("couldn't load file %s", url.c_str());
    }
    else
        arts_warning("mimetype %s unsupported", mimetype.c_str());
...
PlayObject SoundServerV2_impl::createPlayObjectForStream(InputStream instream, const std::string &mimetype, bool createBUS)
{
    ...
    if(!objectType.empty())
    {
        ...
        arts_warning("mimetype %s unsupported for streaming", mimetype.c_str());
...
void SoundServerV2_impl::checkNewObjects()
{
    ...
    if(mkdir(dir.c_str(), 0755) != 0)
    {
        ...
        {
            arts_warning(
                "can't create directory %s to fill it with"
                " trader data (%s)",
                dir.c_str(), why.c_str());
            return;
        }
...
void SoundServerV2_impl::rebuildTraderCache(const string &directory, vector< TraderOffer > *offers)
{
    vector< TraderOffer >::iterator i;
