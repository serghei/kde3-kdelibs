...
namespace Arts {
class FileInputStream_impl : virtual public FileInputStream_skel, virtual public StdSynthModule {
protected:
    string _filename;
    int age, fd;
...
public:
    ...
    bool eof()
    {
        return (fd < 0 || position >= _size) && (wqueue.size() == PACKET_COUNT);
    }
    bool seekOk()
    {
        return true;
...
const unsigned int FileInputStream_impl::PACKET_COUNT = 8;

const unsigned int FileInputStream_impl::PACKET_SIZE = 8192;
}
