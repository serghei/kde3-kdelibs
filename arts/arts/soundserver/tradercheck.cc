...
void TraderCheck::run()
{
    ...
    for(i = allObjects->begin(); i != allObjects->end(); i++)
    {
        ...
        if(haveProperty(offer, "Type"))
        {
            ...
            check(haveProperty(offer, "TypeFile"), "trader entries with a Type entry MUST have a TypeFile entry");

            check(!haveProperty(offer, "Language"), "trader entries with a Type entry MUST NOT have a Language entry");
        }
        else if(haveProperty(offer, "Language") || haveProperty(offer, "Library"))
        {
            ...
            else
            {
                ...
                for(ii = ifaces.begin(); ii != ifaces.end(); ii++)
                {
                    switch(ii->second)
                    {
                        ...
                        case 1:
                            check(false, "missing interface %s in Interface entry", ii->first.c_str());
                            break;

                        case 2:
                            check(false, "given unimplemented(?) interface %s in Interface entry", ii->first.c_str());
                            break;
                        ...
                        default:
                            check(false, "given interface %s in Interface entry more than once?", ii->first.c_str());
                            break;
                    }
                }
            }
            ...
            {
                check(getSingleProperty(offer, "Language") == "C++", "trader entries with a Library entry SHOULD have a Language=C++ entry");
            }

            if(getSingleProperty(offer, "Language") == "C++")
            {
                string library = getSingleProperty(offer, "Library");
                check(!library.empty(), "entries with a Language entry must have a Library entry");
                check(findFile(MCOPUtils::extensionPath(), library), "Library entry MUST be loadable via extension path");
            }

            check(haveProperty(offer, "Interface"), "entries with Language/Library MUST have an Interface entry");
        }
        else
        {
