...
public:
     ...
     */
    GSLPlayObject_impl() : leftPlay_(DataHandlePlay::null()), rightPlay_(DataHandlePlay::null()), state_(posIdle)
    {
    }

    float speed()
    {
        return leftPlay_.speed();
    }
    ...
    void streamStart()
    {
        ...
        arts_debug("GSLPlayObject: streamStart(%d channels) playing %d, %d", dhandle_.channelCount(), leftPlay_.channelIndex(),
    ...
    bool loadMedia(const std::string &filename)
    {
        ...
        if(dhandle_.isLoaded())
        {
            ...
            if(dhandle_.channelCount() > 2)
                arts_debug(
                    "GSLPlayObject: can't play more than 2 channels, "
                    "playing first two!");
    ...
    poTime currentTime()
    {
        ...
        float timesec = leftPlay_.pos() / dhandle_.channelCount() / dhandle_.mixerFrequency();
        float timems = (timesec - floor(timesec)) * 1000.0;

        return poTime((long)timesec, (long)timems, (long)leftPlay_.pos(), "samples");
    }
    ...
    poTime overallTime()
    {
        ...
        return poTime((long)timesec, (long)timems, (long)sampleCount(), "samples");
    }
