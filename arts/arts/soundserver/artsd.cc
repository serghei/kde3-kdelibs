...
static void exitListAudioIO()
{
    ...
    {
        fprintf(stderr, "  %-10s%s\n", AudioIO::queryAudioIOParamStr(i, AudioIO::name), AudioIO::queryAudioIOParamStr(i, AudioIO::fullName));
    }
    fprintf(stderr, "\n");
    exit(0);
}
...
static void handleArgs(int argc, char **argv)
{
    ...
    while((optch = getopt(argc, argv, "r:p:nuF:S:hD:dl:a:Ab:s:m:vNw:fV:c:")) > 0)
    {
        ...
        {
            case 'p':
                cfgPort = atoi(optarg); // setting a port => network transparency
            case 'n':
                cfgServers = static_cast< Dispatcher::StartServer >(cfgServers | Dispatcher::startTCPServer);
                break;
            case 'a':
                cfgAudioIO = optarg;
                break;
            ...
            case 'b':
                ...
                break;
            case 's':
                cfgAutoSuspend = atoi(optarg);
                break;
            case 'F':
                cfgFragmentCount = atoi(optarg);
                break;
            case 'S':
                cfgFragmentSize = atoi(optarg);
                break;
            case 'D':
                cfgDeviceName = optarg;
                break;
            case 'd':
                cfgFullDuplex = true;
                break;
            ...
            case 'c':
                ...
                break;
            case 'u':
                cfgServers = static_cast< Dispatcher::StartServer >(cfgServers | Dispatcher::noAuthentication);
                break;
            case 'A':
                cmdListAudioIO = true;
                break;
...
static bool publishReferences(SoundServerV2 server, AudioManager audioManager, bool silent)
{
    ObjectManager *om = ObjectManager::the();
    bool result;

    result = om->addGlobalReference(server, "Arts_SoundServerV2") && om->addGlobalReference(server, "Arts_SoundServer")
             && om->addGlobalReference(server, "Arts_SimpleSoundServer") && om->addGlobalReference(server, "Arts_PlayObjectFactory")
             && om->addGlobalReference(audioManager, "Arts_AudioManager");

    if(!result && !silent)
    {
        cerr << "Error: Can't add object reference (probably artsd is already running)." << endl
             << "       If you are sure it is not already running, remove the relevant files:" << endl
             << endl
...
int main(int argc, char **argv)
{
    ...
    /* apply configuration */
    if(cfgAudioIO)
        AudioSubSystem::the()->audioIO(cfgAudioIO);
    if(cfgSamplingRate)
        AudioSubSystem::the()->samplingRate(cfgSamplingRate);
    if(cfgFragmentCount)
        AudioSubSystem::the()->fragmentCount(cfgFragmentCount);
    if(cfgFragmentSize)
        AudioSubSystem::the()->fragmentSize(cfgFragmentSize);
    if(cfgFullDuplex)
        AudioSubSystem::the()->fullDuplex(cfgFullDuplex);
    if(cfgDeviceName)
        AudioSubSystem::the()->deviceName(cfgDeviceName);
    if(cfgBits)
        AudioSubSystem::the()->format(cfgBits);
    ...
    if(!publishReferences(server, audioManager, true))
    {
        ...
        if(!publishReferences(server, audioManager, false))
            return 1;
    }
