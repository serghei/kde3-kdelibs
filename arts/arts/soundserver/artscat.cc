...
class Sender : public ByteSoundProducerV2_skel, public StdSynthModule, public IONotify {
protected:
    FILE *pfile;
    int pfd;
    int packets;
...
public:
    Sender(FILE *input, float minStreamBufferTime) : pfile(input), waiting(false)
    {
        ...
        {
            packets++;
            streamBufferTime = (float)(packets * packetCapacity * 1000) / (float)(samplingRate() * channels() * 2);
