...
/* --- prototypes --- */
static GslLong dh_mad_coarse_seek(GslDataHandle *data_handle, GslLong voffset);
...
static gboolean check_frame_validity(MadHandle *handle, struct mad_header *header)
{
...
static gboolean read_next_frame_header(MadHandle *handle)
{
    ...
    {
        if(!MAD_RECOVERABLE(handle->stream.error) || handle->stream.error == MAD_ERROR_LOSTSYNC)
        {
            /* read on */
...
static gboolean /* FALSE: handle->eof || handle->error != NULL */
    pcm_frame_read(MadHandle *handle, gboolean synth)
{
    gboolean succeeded = TRUE;
...
static guint *create_seek_table(MadHandle *handle, guint *n_seeks_p)
{
    guint *seeks = NULL;
    ...
    do
    {
        while(read_next_frame_header(handle))
        {
            ...
            if(0)
            {
                ...
                mad_synth_frame(&handle->synth, &handle->frame);
                MAD_DEBUG("frame(%u) PCM:%u => FILE:%u FDIFF:%d (%x %x %x) br:%lu time:%ld/%lu mode:%u ext:%u flags:0x%x phase:%u", i,
                          i * handle->frame_size, this_pos, this_pos - seeks[MAX(i, 1) - 1], handle->stream.this_frame[0],
                          handle->stream.this_frame[1], (handle->stream.this_frame[1] >> 1) & 3, handle->frame.header.bitrate,
                          handle->frame.header.duration.seconds, handle->frame.header.duration.fraction, handle->frame.header.mode,
                          handle->frame.header.mode_extension, handle->frame.header.flags, handle->synth.phase);
            }
        }
...
static GslErrorType dh_mad_open(GslDataHandle *dhandle, GslDataHandleSetup *setup)
{
    ...
    if(setup->n_channels < 1 || setup->n_channels > MAX_CHANNELS || handle->frame_size < 1 || handle->sample_rate < 1)
        goto OPEN_FAILED;
...
static GslLong dh_mad_read(GslDataHandle *dhandle, GslLong voffset, /* in values */
                           GslLong n_values, gfloat *values)
{
    MadHandle *handle = (MadHandle *)dhandle;
    GslLong pos = voffset / dhandle->setup.n_channels;
    gboolean frame_read_ok = TRUE;

    if(pos < handle->pcm_pos || pos >= handle->pcm_pos + handle->pcm_length + SEEK_BY_READ_AHEAD(handle) * handle->frame_size)
    {
        GslLong tmp;
    ...
    if(!frame_read_ok)
    {
        if(handle->stream.error == MAD_ERROR_BADDATAPTR)
        {
            ...
            if(handle->accumulate_state_frames < 10)
            {
                ...
                MAD_DEBUG("retrying seek with accumulate_state_frames=%d", handle->accumulate_state_frames);
            ...
            {
                MAD_DEBUG("synthesizing frame failed, accumulate_state_frames is already %u: %s", handle->accumulate_state_frames, handle->error);
                return -1;
            }
        }
        else
    ...
    {
        MAD_MSG(GSL_ERROR_READ_FAILED, "pcm position screwed (pos: %lu, handle-pos: %lu), aborting read", pos, handle->pcm_pos);
        return -1;
    }
}

static GslLong dh_mad_coarse_seek(GslDataHandle *dhandle, GslLong voffset)
{
    MadHandle *handle = (MadHandle *)dhandle;
    ...
    if(pos < handle->pcm_pos || pos >= handle->pcm_pos + handle->pcm_length + SEEK_BY_READ_AHEAD(handle))
    {
        GslLong offs = pos;
        guint i, file_pos;
      ...
      handle->synth.phase = ((i + 1) * (handle->frame_size / 32)) % 16;
        ...
        MAD_DEBUG("seek-done: at %lu (f:%lu) want %lu (f:%lu) got %lu (f:%lu) diff %ld (diff-requested %ld)", opos, opos / handle->frame_size, pos,
                  pos / handle->frame_size, handle->pcm_pos, handle->pcm_pos / handle->frame_size, handle->pcm_pos - opos, pos - opos);
    }

    return handle->pcm_pos * dhandle->setup.n_channels;
}

static void dh_mad_close(GslDataHandle *data_handle)
{
...
static void dh_mad_destroy(GslDataHandle *data_handle)
{
    MadHandle *handle = (MadHandle *)data_handle;
...
static GslDataHandleFuncs dh_mad_vtable = {
    dh_mad_open, dh_mad_read, dh_mad_close, dh_mad_destroy, dh_mad_coarse_seek,
};

static GslDataHandle *dh_mad_new(const gchar *file_name, gboolean skip_seek_keep_open)
{
    MadHandle *handle;
    gboolean success;
