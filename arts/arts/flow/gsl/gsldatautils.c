    ...
    if(pos < peekbuf->start || pos >= peekbuf->end)
    {
        ...
        for(k = peekbuf->start; k < peekbuf->end; k += inc)
        {
            ...
            { /* pathologic */
                ...
                inc = 1;
                gsl_message_send(GSL_MSG_DATA_HANDLE, "PeekBuffer", GSL_ERROR_READ_FAILED, "unable to read from data handle (%p)", dhandle);
            }
        }
    }
...
gint /* errno */
    gsl_data_handle_dump(GslDataHandle *dhandle, gint fd, GslWaveFormatType format, guint byte_order)
{
    GslLong l, offs = 0;
...
static void write_bytes(gint fd, guint n_bytes, const void *bytes)
{
    gint errold = errno;
    guint j;
...
static void write_uint32_le(gint fd, guint32 val)
{
    val = GUINT32_TO_LE(val);
    write_bytes(fd, 4, &val);
}

static void write_uint16_le(gint fd, guint16 val)
{
...
gint /* errno */
    gsl_data_handle_dump_wav(GslDataHandle *dhandle, gint fd, guint n_bits, guint n_channels, guint sample_freq)
{
    ...
    return gsl_data_handle_dump(dhandle, fd, n_bits == 16 ? GSL_WAVE_FORMAT_SIGNED_16 : GSL_WAVE_FORMAT_UNSIGNED_8, G_LITTLE_ENDIAN);
}

gboolean gsl_data_detect_signal(GslDataHandle *handle, GslLong *sigstart_p, GslLong *sigend_p)
{
    ...
    GslDataPeekBuffer peek_buffer = {
        +1 /* incremental direction */, 0,
    };
    ...
    for(; k < handle->setup.n_values; k++)
    {
        ...
        needx = (ABS(level_4 + current - (level_0 + level_1 + level_2 + level_3) / 2) * ABS(level_4 - mean) * ABS(current - mean));
        /* shift */
        level_0 = level_1;
        level_1 = level_2;
        level_2 = level_3;
...
GslLong gsl_data_find_sample(GslDataHandle *dhandle, gfloat min_value, gfloat max_value, GslLong start_offset, gint direction)
{
    ...
    if(gsl_data_handle_open(dhandle) != GSL_ERROR_NONE || start_offset >= dhandle->setup.n_values)
        return -1;
...
static inline gdouble tailmatch_score_loop(GslDataHandle *shandle, GslDataHandle *dhandle, GslLong start, gdouble worst_score)
{
