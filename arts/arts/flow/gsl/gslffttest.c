...
/* --- prototypes --- */
static void reference_power2_fftc(unsigned int n_values, const double *rivalues_in, double *rivalues_out, int esign);
static void fill_rand(guint n, double *a);
static double diff(guint m, guint p, double *a1, double *a2, const gchar *str);


/* --- variables --- */
static double ref_fft_in[MAX_FFT_SIZE] = {
    0,
};
static double ref_fft_aout[MAX_FFT_SIZE] = {
    0,
};
static double ref_fft_sout[MAX_FFT_SIZE] = {
    0,
};
static double work_fft_in[MAX_FFT_SIZE] = {
    0,
};
static double work_fft_aout[MAX_FFT_SIZE] = {
    0,
};
static double work_fft_sout[MAX_FFT_SIZE] = {
    0,
};
...
static double diff(guint m, guint p, double *a1, double *a2, const gchar *str)
{
    ...
    for(n = 0; n < m; n++)
    {
        ...
        if(n < p)
            g_print("%3u:%.3f) % 19.9f - % 19.9f = % 19.9f (% 19.9f)\n", n, ((float)n) / (float)m, a1[n], a2[n], a1[n] - a2[n], a1[n] / a2[n]);
        d += a;
        max = MAX(max, a);
        min = MIN(min, a);
    }
    ...
    g_print("noise: %u %u %u\n", g_bit_storage(1. / min), g_bit_storage(m / d), g_bit_storage(1. / max));
    return d;
}
...
static inline void reference_bitreverse_fft2analysis(const unsigned int n, const double *X, double *Y)
{
...
static inline void reference_bitreverse_fft2synthesis(const unsigned int n, const double *X, double *Y)
{
    const unsigned int n2 = n >> 1, n1 = n + n2, max = n >> 2;
    unsigned int i, r;
    double scale = n;
...
static void reference_power2_fftc(unsigned int n_values, const double *rivalues_in, double *rivalues, int esign)
{
