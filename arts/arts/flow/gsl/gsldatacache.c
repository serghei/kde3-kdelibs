...
static GslDataCacheNode *data_cache_new_node_L(GslDataCache *dcache, gsize offset, guint pos, gboolean demand_load);


/* --- variables --- */
static GslMutex global_dcache_mutex = {
    0,
};
static GslCond global_dcache_cond_node_filled = {
    0,
};
...
void gsl_data_cache_open(GslDataCache *dcache)
{
    ...
    if(!dcache->open_count)
    {
        ...
        if(error)
        {
            ...
            gsl_message_send(GSL_MSG_DATA_CACHE, "Open", error, "failed to open \"%s\": %s", dcache->dhandle->name, gsl_strerror(error));
        }
        else
        {
            dcache->open_count = 1;
...
static inline GslDataCacheNode **data_cache_lookup_nextmost_node_L(GslDataCache *dcache, gsize offset)
{
    if(dcache->n_nodes > 0)
    {
...
static inline GslDataCacheNode *data_cache_new_node_L(GslDataCache *dcache, gsize offset, guint pos, gboolean demand_load)
{
    ...
    if(left_node)
    {
        ...
        if(offset < left_node_offset + left_node_size)
        {
            ...
            memcpy(data, left_node_data + offset - left_node_offset, left_node_copy * sizeof(GslDataType));
    ...
    do
    {
        ...
        if(result < 0)
        {
            gsl_message_send(GSL_MSG_DATA_CACHE, "ReadAhead", GSL_ERROR_READ_FAILED, "reading from \"%s\"", dcache->dhandle->name);
            break;
        }
        else
        {
...
GslDataCacheNode *gsl_data_cache_ref_node(GslDataCache *dcache, gsize offset, GslDataCacheRequest load_request)
{
...
static gboolean                                                         /* still locked */
    data_cache_free_olders_Lunlock(GslDataCache *dcache, guint max_lru) /* how many lru nodes to keep */
{
...
void gsl_data_cache_unref_node(GslDataCache *dcache, GslDataCacheNode *node)
{
    ...
    if(!node->ref_count && (node->age + AGE_EPSILON <= dcache->max_age || dcache->max_age < AGE_EPSILON))
