...
#define ENGINE_NODE_IS_CONSUMER(node) ((node)->is_consumer && (node)->output_nodes == NULL)
...
typedef enum { ENGINE_FLOW_JOB_NOP, ENGINE_FLOW_JOB_SUSPEND, ENGINE_FLOW_JOB_RESUME, ENGINE_FLOW_JOB_ACCESS, ENGINE_FLOW_JOB_LAST } EngineFlowJobType;
typedef struct
{
...
static void _engine_node_insert_flow_job(EngineNode *node, EngineFlowJob *fjob)
{
...
static inline EngineFlowJob *_engine_node_pop_flow_job(EngineNode *node, guint64 tick_stamp)
{
...
static inline guint64 _engine_node_peek_flow_job_stamp(EngineNode *node)
{
    EngineFlowJob *fjob = node->flow_jobs;

    if_reject(fjob) return fjob->any.tick_stamp;
