...
EXTC gpointer gsl_arts_mutex_new()
{
    return new Arts::Mutex;
}

EXTC void gsl_arts_mutex_free(gpointer mutex)
{
    Arts::Mutex *m = static_cast< Arts::Mutex * >(mutex);
    delete m;
}

EXTC void gsl_arts_mutex_lock(gpointer mutex)
{
    Arts::Mutex *m = static_cast< Arts::Mutex * >(mutex);
    m->lock();
}

EXTC gboolean gsl_arts_mutex_trylock(gpointer mutex)
{
    Arts::Mutex *m = static_cast< Arts::Mutex * >(mutex);
    return m->tryLock();
}

EXTC void gsl_arts_mutex_unlock(gpointer mutex)
{
    Arts::Mutex *m = static_cast< Arts::Mutex * >(mutex);
    m->unlock();
}

EXTC gpointer gsl_arts_cond_new()
{
    return new Arts::ThreadCondition;
}

EXTC void gsl_arts_cond_free(gpointer cond)
{
    Arts::ThreadCondition *c = static_cast< Arts::ThreadCondition * >(cond);
    delete c;
}

EXTC void gsl_arts_cond_signal(gpointer cond)
{
    Arts::ThreadCondition *c = static_cast< Arts::ThreadCondition * >(cond);
    c->wakeOne();
}

EXTC void gsl_arts_cond_broadcast(gpointer cond)
{
    Arts::ThreadCondition *c = static_cast< Arts::ThreadCondition * >(cond);
    c->wakeAll();
}

EXTC void gsl_arts_cond_wait(gpointer cond, gpointer mutex)
{
    Arts::ThreadCondition *c = static_cast< Arts::ThreadCondition * >(cond);
    Arts::Mutex *m = static_cast< Arts::Mutex * >(mutex);
...
EXTC void gsl_arts_cond_timed_wait(gpointer /*cond*/, gpointer /*mutex*/, GTimeVal * /*time*/)
{
    arts_assert(false);
}
...
public:
    ...
    GslArtsThread(gpointer (*func)(gpointer data), gpointer data) : func(func), data(data)
    {
        gthread.data = 0;
    }
    void run()
...
// This is C++:
GThread *gsl_arts_thread_create_full_CPP(gpointer (*func)(gpointer data), gpointer data, gulong /*stack_size*/, gboolean /*joinable*/,
                                         gboolean /*bound*/, GThreadPriority /*priority*/, GError ** /*error*/)
{
...
// This is C:
extern "C" GThread *gsl_arts_thread_create_full(gpointer (*func)(gpointer data), gpointer data, gulong stack_size, gboolean joinable, gboolean bound,
                                                GThreadPriority priority, GError **error)
{
    return gsl_arts_thread_create_full_CPP(func, data, stack_size, joinable, bound, priority, error);
}

EXTC gpointer gsl_arts_thread_self()
{
...
EXTC void gsl_arts_thread_init(gpointer /*arg*/)
{
}
