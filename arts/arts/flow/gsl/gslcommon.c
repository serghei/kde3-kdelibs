...
static inline gpointer low_alloc(gsize mem_size)
{
    gpointer mem;
...
static inline void low_free(gsize mem_size, gpointer mem)
{
    if(mem_size >= TS8_SIZE && mem_size / 8 < SIMPLE_CACHE_SIZE)
    {
        guint cell;
...
gpointer gsl_alloc_memblock(gsize block_size)
{
    guint8 *cmem;
    gsize *debug_size;
...
void gsl_free_memblock(gsize block_size, gpointer mem)
{
    gsize *debug_size;
    guint8 *cmem;
...
gpointer gsl_alloc_memblock0(gsize block_size)
{
...
static void gsl_free_node_list(gpointer mem, gsize node_size)
{
    struct
    {
        gpointer next, data;
...
/* --- ring (circular-list) --- */
static inline GslRing *gsl_ring_prepend_i(GslRing *head, gpointer data)
{
...
GslRing *gsl_ring_prepend(GslRing *head, gpointer data)
{
    return gsl_ring_prepend_i(head, data);
}

GslRing *gsl_ring_prepend_uniq(GslRing *head, gpointer data)
{
    GslRing *walk;
...
GslRing *gsl_ring_append(GslRing *head, gpointer data)
{
    GslRing *ring;
...
GslRing *gsl_ring_concat(GslRing *head1, GslRing *head2)
{
    GslRing *tail1, *tail2;
...
GslRing *gsl_ring_remove_node(GslRing *head, GslRing *node)
{
    if(!head)
...
GslRing *gsl_ring_remove(GslRing *head, gpointer data)
{
    GslRing *walk;
...
GslRing *gsl_ring_find(GslRing *head, gconstpointer data)
{
    GslRing *ring;
...
GslRing *gsl_ring_nth(GslRing *head, guint n)
{
    GslRing *ring = head;
...
gpointer gsl_ring_nth_data(GslRing *head, guint n)
{
    GslRing *ring = head;
...
gpointer gsl_ring_pop_head(GslRing **head_p)
{
    gpointer data;
...
gpointer gsl_ring_pop_tail(GslRing **head_p)
{
    gpointer data;
...
GslRing *gsl_ring_insert_sorted(GslRing *head, gpointer data, GCompareFunc func)
{
    gint cmp;
...
static GslCond global_thread_cond = {
    0,
};
...
static inline ThreadData *thread_data_from_gsl_thread(GslThread *thread)
{
...
static gpointer thread_wrapper(gpointer arg)
{
...
static ThreadData *create_tdata(void)
{
    ThreadData *tdata;
    glong d_long;
    gint error;
...
GslThread *gsl_thread_new(GslThreadFunc func, gpointer user_data)
{
    ...
    if(tdata)
    {
        ...
        gthread = g_thread_create_full(thread_wrapper, tdata, 0, joinable, FALSE, G_THREAD_PRIORITY_NORMAL, &gerror);
    }
...
static void thread_wakeup_I(ThreadData *tdata)
{
    guint8 data = 'W';
    gint r;
...
static void default_mutex_init(GslMutex *mutex)
{
...
static int default_mutex_trylock(GslMutex *mutex)
{
    return g_mutex_trylock(mutex->mutex_pointer) ? 0 : -1;
}

static void default_mutex_lock(GslMutex *mutex)
{
...
static void default_mutex_unlock(GslMutex *mutex)
{
    g_mutex_unlock(mutex->mutex_pointer);
}

static void default_mutex_destroy(GslMutex *mutex)
{
    g_mutex_free(mutex->mutex_pointer);
    memset(mutex, 0, sizeof(*mutex));
}

static void default_rec_mutex_init(GslRecMutex *rec_mutex)
{
    rec_mutex->depth = 0;
...
static int default_rec_mutex_trylock(GslRecMutex *rec_mutex)
{
...
static void default_rec_mutex_lock(GslRecMutex *rec_mutex)
{
    gpointer self = gsl_thread_self();
...
static void default_rec_mutex_unlock(GslRecMutex *rec_mutex)
{
    gpointer self = gsl_thread_self();
    ...
    if(rec_mutex->owner == self && rec_mutex->depth > 0)
    {
        ...
        g_warning("unable to unlock recursive mutex with self %p != %p or depth %u < 1", rec_mutex->owner, self, rec_mutex->depth);
}

static void default_rec_mutex_destroy(GslRecMutex *rec_mutex)
{
...
static void default_cond_init(GslCond *cond)
{
    cond->cond_pointer = g_cond_new();
}

static void default_cond_wait(GslCond *cond, GslMutex *mutex)
{
    /* infinite wait */
    g_cond_wait(cond->cond_pointer, mutex->mutex_pointer);
}

static void default_cond_signal(GslCond *cond)
{
    g_cond_signal(cond->cond_pointer);
}

static void default_cond_broadcast(GslCond *cond)
{
    g_cond_broadcast(cond->cond_pointer);
}

static void default_cond_destroy(GslCond *cond)
{
    g_cond_free(cond->cond_pointer);
}

static void default_cond_wait_timed(GslCond *cond, GslMutex *mutex, gulong abs_secs, gulong abs_usecs)
{
    GTimeVal gtime;
...
GslMutexTable gsl_mutex_table = {
    default_mutex_init,       default_mutex_lock,        default_mutex_trylock,   default_mutex_unlock,
    default_mutex_destroy,    default_rec_mutex_init,    default_rec_mutex_lock,  default_rec_mutex_trylock,
    default_rec_mutex_unlock, default_rec_mutex_destroy, default_cond_init,       default_cond_signal,
    default_cond_broadcast,   default_cond_wait,         default_cond_wait_timed, default_cond_destroy,
};

void gsl_cond_wait_timed(GslCond *cond, GslMutex *mutex, glong max_useconds)
{
    if(max_useconds < 0)
...
const gchar *gsl_strerror(GslErrorType error)
{
    switch(error)
    {
        ...
        case GSL_ERROR_INTERNAL:
            return "Internal error (please report)";
        case GSL_ERROR_UNKNOWN:
            return "Unknown error";
        case GSL_ERROR_IO:
            return "I/O error";
        case GSL_ERROR_PERMS:
            return "Insufficient permission";
        case GSL_ERROR_BUSY:
            return "Resource currently busy";
        case GSL_ERROR_EXISTS:
            return "Resource exists already";
        case GSL_ERROR_TEMP:
            return "Temporary error";
        case GSL_ERROR_EOF:
            return "File empty or premature EOF";
        case GSL_ERROR_NOT_FOUND:
            return "Resource not found";
        case GSL_ERROR_OPEN_FAILED:
            return "Open failed";
        case GSL_ERROR_SEEK_FAILED:
            return "Seek failed";
        case GSL_ERROR_READ_FAILED:
            return "Read failed";
        case GSL_ERROR_WRITE_FAILED:
            return "Write failed";
        case GSL_ERROR_FORMAT_INVALID:
            return "Invalid format";
        case GSL_ERROR_FORMAT_UNKNOWN:
            return "Unknown format";
        case GSL_ERROR_DATA_CORRUPT:
            return "Data corrupt";
        case GSL_ERROR_CONTENT_GLITCH:
            return "Data glitch (junk) detected";
        case GSL_ERROR_NO_RESOURCE:
            return "Out of memory, disk space or similar resource";
        case GSL_ERROR_CODEC_FAILURE:
...
static const GDebugKey gsl_static_debug_keys[] = {
    {"notify", GSL_MSG_NOTIFY}, {"dcache", GSL_MSG_DATA_CACHE}, {"dhandle", GSL_MSG_DATA_HANDLE}, {"loader", GSL_MSG_LOADER},
    {"osc", GSL_MSG_OSC},       {"engine", GSL_MSG_ENGINE},     {"jobs", GSL_MSG_JOBS},           {"fjobs", GSL_MSG_FJOBS},
    {"sched", GSL_MSG_SCHED},   {"master", GSL_MSG_MASTER},     {"slave", GSL_MSG_SLAVE},
};

static const gchar *reporter_name(GslDebugFlags reporter)
{
    switch(reporter)
    {
        case GSL_MSG_NOTIFY:
            return "Notify";
        case GSL_MSG_DATA_CACHE:
            return "DataCache";
        case GSL_MSG_DATA_HANDLE:
            return "DataHandle";
        case GSL_MSG_LOADER:
            return "Loader";
        case GSL_MSG_OSC:
...
void gsl_message_send(GslDebugFlags reporter, const gchar *section, GslErrorType error, const gchar *messagef, ...)
{
    struct
    {
    ...
    g_printerr("GSL-%s%s%s: %s%s%s\n", msg->reporter_name, msg->section ? ":" : "", msg->section ? msg->section : "", msg->message,
               msg->error_str ? ": " : "", msg->error_str ? msg->error_str : "");
}
...
void gsl_debug(GslDebugFlags reporter, const gchar *section, const gchar *format, ...)
{
    ...
    if(reporter & gsl_debug_flags)
    {
        ...
        va_end(args);
        g_printerr("DEBUG:GSL-%s%s%s: %s\n", reporter_name(reporter), section ? ":" : "", section ? section : "", string);
        g_free(string);
    }
}

void gsl_auxlog_push(GslDebugFlags reporter, const gchar *section)
{
    ThreadData *tdata = thread_data_from_gsl_thread(gsl_thread_self());
...
void gsl_auxlog_message(GslErrorType error, const gchar *format, ...)
{
    ThreadData *tdata = thread_data_from_gsl_thread(gsl_thread_self());
    GslDebugFlags reporter = GSL_MSG_NOTIFY;
    const gchar *section = NULL;
...
GslErrorType gsl_check_file(const gchar *file_name, const gchar *mode)
{
    guint access_mask = 0;
    ...
    if(check_file || check_dir || check_link)
    {
        ...
        if((check_file && !S_ISREG(st.st_mode)) || (check_dir && !S_ISDIR(st.st_mode)) || (check_link && !S_ISLNK(st.st_mode)))
...
GslErrorType gsl_error_from_errno(gint sys_errno, GslErrorType fallback)
{
    switch(sys_errno)
    {
        case ELOOP:
        ...
        case ENOTDIR:
        case ENOENT:
            return GSL_ERROR_NOT_FOUND;
        case EROFS:
        case EPERM:
        case EACCES:
            return GSL_ERROR_PERMS;
        case ENOMEM:
        case ENOSPC:
        case EFBIG:
        case ENFILE:
        case EMFILE:
            return GSL_ERROR_NO_RESOURCE;
        case EISDIR:
        case ESPIPE:
        case EIO:
            return GSL_ERROR_IO;
        case EEXIST:
            return GSL_ERROR_EXISTS;
        case ETXTBSY:
        case EBUSY:
            return GSL_ERROR_BUSY;
        case EAGAIN:
        case EINTR:
            return GSL_ERROR_TEMP;
        case EINVAL:
        case EFAULT:
        case EBADF:
            return GSL_ERROR_INTERNAL;
        default:
            return fallback;
    }
}
...
static guint get_n_processors(void)
{
...
const GslConfig *gsl_get_config(void)
{
    return gsl_config;
}
...
void gsl_init(const GslConfigValue values[], GslMutexTable *mtable)
{
    const GslConfigValue *config = values;
    static GslConfig pconfig = {
        /* DEFAULTS */
        1,           /* n_processors */
