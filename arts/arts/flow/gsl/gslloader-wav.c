...
static GslErrorType wav_read_header(gint fd, WavHeader *header)
{
    guint n_bytes;
...
static GslErrorType wav_read_fmt_header(gint fd, FmtHeader *header)
{
    guint n_bytes;
    ...
    if(header->format != 1 /* PCM */ || header->n_channels > 2 || header->n_channels < 1)
    {
    ...
    {
        WAV_DEBUG("invalid byte_per_second (%u!=%u) or byte_per_sample (%u!=%u)", header->byte_per_second,
                  header->sample_freq * header->byte_per_sample, header->byte_per_sample, (header->bit_per_sample + 7) / 8 * header->n_channels);
        return GSL_ERROR_FORMAT_INVALID;
    }
    if(header->length > 16)
    {
...
static GslErrorType wav_read_data_header(gint fd, DataHeader *header, guint byte_alignment)
{
    guint n_bytes;
    ...
    {
        WAV_DEBUG("invalid data length (%u) or alignment (%u)", header->data_length, header->data_length % byte_alignment);
        return GSL_ERROR_FORMAT_INVALID;
    }
...
static GslWaveFileInfo *wav_load_file_info(gpointer data, const gchar *file_name, GslErrorType *error_p)
{
...
static void wav_free_file_info(gpointer data, GslWaveFileInfo *file_info)
{
...
static GslWaveDsc *wav_load_wave_dsc(gpointer data, GslWaveFileInfo *file_info, guint nth_wave, GslErrorType *error_p)
{
    FileInfo *fi = (FileInfo *)file_info;
    ...
    *error_p = wav_read_data_header(fi->fd, &data_header, data_width * fmt_header.n_channels);
    ...
    {
        case 8:
            format = GSL_WAVE_FORMAT_UNSIGNED_8;
            break;
        case 12:
            format = GSL_WAVE_FORMAT_SIGNED_12;
            break;
        case 16:
    ...
    if(0)
        WAV_DEBUG("n_channels: %d sample_freq: %d bit_width: %u", fmt_header.n_channels, fmt_header.sample_freq, fmt_header.bit_per_sample);
...
static void wav_free_wave_dsc(gpointer data, GslWaveDsc *wave_dsc)
{
...
static GslDataHandle *wav_create_chunk_handle(gpointer data, GslWaveDsc *wave_dsc, guint nth_chunk, GslErrorType *error_p)
{
    WaveDsc *dsc = (WaveDsc *)wave_dsc;
    ...
    dhandle =
        gsl_wave_handle_new(dsc->wdsc.file_info->file_name, dsc->wdsc.n_channels, dsc->format, G_LITTLE_ENDIAN, dsc->data_offset, dsc->n_values);
    return dhandle;
}
