...
/* --- functions --- */
static GslLoader *loader_find_by_name(const gchar *name)
{
    GslLoader *loader;
    ...
    if(loader->magic_specs)
    {
        ...
        for(i = 0; loader->magic_specs[i]; i++)
        {
            if(loader->extensions)
                ...
                {
                    magic = gsl_magic_create(loader, loader->priority, loader->extensions[j], loader->magic_specs[i]);
                    gsl_magic_list = gsl_ring_append(gsl_magic_list, magic);
                }
            else
            {
                magic = gsl_magic_create(loader, loader->priority, NULL, loader->magic_specs[i]);
                gsl_magic_list = gsl_ring_append(gsl_magic_list, magic);
            }
        }
    }
...
GslWaveFileInfo *gsl_wave_file_info_load(const gchar *file_name, GslErrorType *error_p)
{
...
GslWaveDsc *gsl_wave_dsc_load(GslWaveFileInfo *wave_file_info, guint nth_wave, GslErrorType *error_p)
{
    GslErrorType error = GSL_ERROR_NONE;
    GslWaveDsc *wdsc;
    GslLoader *loader;
...
GslDataHandle *gsl_wave_handle_create(GslWaveDsc *wave_dsc, guint nth_chunk, GslErrorType *error_p)
{
    GslErrorType error = GSL_ERROR_NONE;
    ...
    dhandle = loader->create_chunk_handle(loader->data, wave_dsc, nth_chunk, &error);
    if(error && dhandle)
    {
        /* loaders shouldn't do this */
...
GslWaveChunk *gsl_wave_chunk_create(GslWaveDsc *wave_dsc, guint nth_chunk, GslErrorType *error_p)
{
    GslDataHandle *dhandle;
    GslDataCache *dcache;
    GslWaveChunk *wchunk;
    ...
    wchunk =
        gsl_wave_chunk_new(dcache, wave_dsc->chunks[nth_chunk].osc_freq, wave_dsc->chunks[nth_chunk].mix_freq, wave_dsc->chunks[nth_chunk].loop_type,
                           wave_dsc->chunks[nth_chunk].loop_start, wave_dsc->chunks[nth_chunk].loop_end, wave_dsc->chunks[nth_chunk].loop_count);
