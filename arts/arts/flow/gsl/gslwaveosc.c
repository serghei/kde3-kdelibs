...
/* --- prototype --- */
static void wave_osc_transform_filter(GslWaveOscData *wosc, gfloat play_freq);
...
/* --- functions --- */
gboolean gsl_wave_osc_process(GslWaveOscData *wosc, guint n_values, const gfloat *freq_in, const gfloat *mod_in, const gfloat *sync_in,
                              gfloat *mono_out)
{
    guint mode = 0;
    ...
    if_reject(!wosc->config.wchunk_from_freq) return FALSE;
    ...
    if(wosc->y[0] != 0.0 && !(fabs(wosc->y[0]) > GSL_SIGNAL_EPSILON && fabs(wosc->y[0]) < GSL_SIGNAL_KAPPA))
    {
        guint i;
    ...
    wosc->done =
        (wosc->block.is_silent && /* FIXME, let filter state run out? */
         ((wosc->block.play_dir < 0 && wosc->block.offset < 0) || (wosc->block.play_dir > 0 && wosc->block.offset > wosc->wchunk->wave_length)));
...
void gsl_wave_osc_set_filter(GslWaveOscData *wosc, gfloat play_freq, gboolean clear_state)
{
    ...
    if_reject(!wosc->config.wchunk_from_freq) return;
    ...
    if(istep != wosc->istep)
    {
        ...
        gfloat filt_fact = CLAMP(1. / step, 1. / (empiric_filter_stability_limit * zero_padding), 1. / zero_padding /* spectrum half */);
...
void gsl_wave_osc_config(GslWaveOscData *wosc, GslWaveOscConfig *config)
{
    ...
    if(wosc->config.wchunk_data != config->wchunk_data || wosc->config.wchunk_from_freq != config->wchunk_from_freq
    ...
    else
    {
        ...
        if(wosc->config.cfreq != config->cfreq || wosc->config.start_offset != config->start_offset)
        {
