...
/* --- prototypes --- */
static Magic *magic_create(gchar *magic_string, const gchar *original);
static gboolean magic_match_file(BFile *bfile, Magic *magics);
static gboolean bfile_open(BFile *bfile, const gchar *file_name);
static gboolean bfile_read(BFile *bfile, guint offset, void *mem, guint n_bytes);
    ...
    if(bfile_open(&bfile, file_name))
    {
        ...
        if(!rmagic && extension)
            for(node = magic_list; node; node = gsl_ring_walk(magic_list, node))
            {
                ...
                if(!magic->extension || strcmp(magic->extension, extension) != 0 || rpriority < magic->priority
        ...
        if(!rmagic && !extension)
            for(node = magic_list; node; node = gsl_ring_walk(magic_list, node))
            {
                ...
                if(rpriority < magic->priority || (rmagic && rpriority == magic->priority))
                    continue;
                if(magic_match_file(&bfile, magic->match_list))
                {
                    rmagic = magic;
...
static gboolean magic_parse_test(Magic *magic, const gchar *string)
{
...
static gboolean magic_parse_type(Magic *magic, const gchar *string)
{
    gchar *f = NULL;
...
static gboolean magic_parse_offset(Magic *magic, gchar *string)
{
    gchar *f = NULL;
...
static gboolean magic_check_data(Magic *magic, MagicData *data)
{
    gint cmp = 0;
...
static inline gboolean magic_read_data(BFile *bfile, Magic *magic, MagicData *data)
{
...
static gboolean magic_match_file(BFile *bfile, Magic *magics)
{
    ...
    do
    {
        ...
        if(!magic_read_data(bfile, magics, &data) || !magic_check_data(magics, &data))
            return FALSE;
        magics = magics->next;
    } while(magics);
