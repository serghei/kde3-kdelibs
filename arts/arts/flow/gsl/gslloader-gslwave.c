...
static const char *wave_tokens_512[] = {
    "wave", "chunk", "name",    "byte_order", "format",    "n_channels", "mix_freq", "osc_freq",   "midi_note",
    "file", "index", "boffset", "n_values",   "loop_type", "loop_start", "loop_end", "loop_count",
};
static const char *wave_tokens_768[] = {
    "big_endian", "big",         "little_endian", "little", "signed_8", "signed_12", "signed_16",
    "unsigned_8", "unsigned_12", "unsigned_16",   "float",  "none",     "jump",      "pingpong",
};


/* --- functions --- */
static const gchar *gsl_wave_token(GslWaveTokenType token)
{
    if(token >= 768)
    {
        token -= 768;
...
static GTokenType gslwave_skip_rest_statement(GScanner *scanner, guint level)
{
...
static GslWaveFileInfo *gslwave_load_file_info(gpointer data, const gchar *_file_name, GslErrorType *error_p)
{
    FileInfo *fi = NULL;
...
static void gslwave_free_file_info(gpointer data, GslWaveFileInfo *file_info)
{
...
static guint gslwave_parse_chunk_dsc(GScanner *scanner, GslWaveChunkDsc *chunk)
{
    ...
    do
        switch(g_scanner_get_next_token(scanner))
        {
            ...
            case GSL_WAVE_TOKEN_MIX_FREQ:
                ...
                {
                    case G_TOKEN_FLOAT:
                        chunk->mix_freq = scanner->value.v_float;
                        break;
                    case G_TOKEN_INT:
                        chunk->mix_freq = scanner->value.v_int;
                        break;
                    default:
            ...
            case GSL_WAVE_TOKEN_OSC_FREQ:
                ...
                {
                    case G_TOKEN_FLOAT:
                        chunk->osc_freq = scanner->value.v_float;
                        break;
                    case G_TOKEN_INT:
                        chunk->osc_freq = scanner->value.v_int;
                        break;
                    default:
                        return G_TOKEN_FLOAT;
                }
            ...
            case GSL_WAVE_TOKEN_MIDI_NOTE:
                ...
                chunk->osc_freq = gsl_temp_freq(gsl_get_config()->kammer_freq, scanner->value.v_int - gsl_get_config()->midi_kammer_note);
                break;
            ...
            case GSL_WAVE_TOKEN_LOOP_TYPE:
                ...
                switch(g_scanner_get_next_token(scanner))
                {
                    case GSL_WAVE_TOKEN_NONE:
                        chunk->loop_type = GSL_WAVE_LOOP_NONE;
                        break;
                    case GSL_WAVE_TOKEN_JUMP:
                        chunk->loop_type = GSL_WAVE_LOOP_JUMP;
                        break;
                    case GSL_WAVE_TOKEN_PINGPONG:
                        chunk->loop_type = GSL_WAVE_LOOP_PINGPONG;
                        break;
                    default:
                        return GSL_WAVE_TOKEN_JUMP;
                }
                break;
...
static guint gslwave_parse_wave_dsc(GScanner *scanner, WaveDsc *dsc, const gchar *wave_name)
{
    parse_or_return(scanner, '{');
    do
        switch(g_scanner_get_next_token(scanner))
        {
            ...
            case GSL_WAVE_TOKEN_CHUNK:
                ...
                if(dsc->wdsc.chunks[i].osc_freq >= dsc->wdsc.chunks[i].mix_freq / 2.)
                    g_scanner_error(scanner, "wave chunk \"%s\" mixing frequency is invalid: mix_freq=%f osc_freq=%f",
                                    dsc->wdsc.chunks[i].loader_data1 ? (gchar *)dsc->wdsc.chunks[i].loader_data1 : "", dsc->wdsc.chunks[i].mix_freq,
            ...
            case GSL_WAVE_TOKEN_BYTE_ORDER:
                ...
                switch(token)
                {
                    ...
                    case GSL_WAVE_TOKEN_LITTLE:
                        dsc->byte_order = G_LITTLE_ENDIAN;
                        break;
                    case GSL_WAVE_TOKEN_BIG_ENDIAN:
                    case GSL_WAVE_TOKEN_BIG:
                        dsc->byte_order = G_BIG_ENDIAN;
                        break;
                    default:
                        return GSL_WAVE_TOKEN_LITTLE_ENDIAN;
                }
                break;
            case GSL_WAVE_TOKEN_FORMAT:
                ...
                switch(token)
                {
                    case GSL_WAVE_TOKEN_SIGNED_8:
                        dsc->format = GSL_WAVE_FORMAT_SIGNED_8;
                        break;
                    case GSL_WAVE_TOKEN_SIGNED_12:
                        dsc->format = GSL_WAVE_FORMAT_SIGNED_12;
                        break;
                    case GSL_WAVE_TOKEN_SIGNED_16:
                        dsc->format = GSL_WAVE_FORMAT_SIGNED_16;
                        break;
                    case GSL_WAVE_TOKEN_UNSIGNED_8:
                        dsc->format = GSL_WAVE_FORMAT_UNSIGNED_8;
                        break;
                    case GSL_WAVE_TOKEN_UNSIGNED_12:
                        dsc->format = GSL_WAVE_FORMAT_UNSIGNED_12;
                        break;
                    case GSL_WAVE_TOKEN_UNSIGNED_16:
                        dsc->format = GSL_WAVE_FORMAT_UNSIGNED_16;
                        break;
                    case GSL_WAVE_TOKEN_FLOAT:
                        dsc->format = GSL_WAVE_FORMAT_FLOAT;
                        break;
                    default:
                        return GSL_WAVE_TOKEN_SIGNED_16;
                }
                break;
            ...
            case GSL_WAVE_TOKEN_MIX_FREQ:
                ...
                {
                    case G_TOKEN_FLOAT:
                        dsc->dfl_mix_freq = scanner->value.v_float;
                        break;
                    case G_TOKEN_INT:
                        dsc->dfl_mix_freq = scanner->value.v_int;
                        break;
                    default:
                        return G_TOKEN_FLOAT;
                }
...
static void gslwave_wave_dsc_free(WaveDsc *dsc)
{
    guint i;
...
static GslWaveDsc *gslwave_load_wave_dsc(gpointer data, GslWaveFileInfo *file_info, guint nth_wave, GslErrorType *error_p)
{
    GScanner *scanner;
    WaveDsc *dsc;
    guint token, i;
...
static void gslwave_free_wave_dsc(gpointer data, GslWaveDsc *wave_dsc)
{
...
static GslDataHandle *gslwave_load_singlechunk_wave(GslWaveFileInfo *fi, const gchar *wave_name, GslErrorType *error_p)
{
    GslWaveDsc *wdsc;
    guint i;
...
static GslDataHandle *gslwave_create_chunk_handle(gpointer data, GslWaveDsc *wave_dsc, guint nth_chunk, GslErrorType *error_p)
{
    WaveDsc *dsc = (WaveDsc *)wave_dsc;
    ...
    if(chunk->loader_data1) /* file_name */
    {
        ...
        if(cfi)
        {
             ...
             * stack overflow
             */
            dhandle = gslwave_load_singlechunk_wave(cfi, chunk->loader_data2, /* wave_name */
                                                    error_p);
        ...
        dhandle = gsl_wave_handle_new(string,                                                                   /* file_name */
                                      dsc->wdsc.n_channels, dsc->format, dsc->byte_order, chunk->loader_offset, /* byte_offset */
...
void _gsl_init_loader_gslwave(void)
{
    ...
    static const gchar *mime_types[] = {
        "audio/x-gslwave", NULL,
    };
    static const gchar *magics[] = {
        "0 string #GslWave", NULL,
    };
