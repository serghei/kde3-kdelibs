...
static void free_node(EngineNode *node)
{
    guint j;
...
static void free_job(GslJob *job)
{
...
static void free_flow_job(EngineFlowJob *fjob)
{
    switch(fjob->fjob_id)
    {
...
static inline gfloat **const_values_lookup_nextmost(ConstValuesArray *array, gfloat key_value)
{
...
static inline guint upper_power2(guint number)
{
    return gsl_alloc_upper_power2(MAX(number, 8));
}

static inline void const_values_insert(ConstValuesArray *array, guint index, gfloat *value_block)
{
    if(array->n_nodes == 0)
    {
...
GslJob *_engine_pop_job(void) /* (glong max_useconds) */
{
...
void _engine_unset_schedule(EngineSchedule *sched)
{
    ...
    if_reject(pqueue_n_nodes || pqueue_n_cycles) g_warning(G_STRLOC ": schedule(%p) still busy", sched);
