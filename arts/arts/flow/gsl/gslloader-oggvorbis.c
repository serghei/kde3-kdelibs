...
/* --- functions --- */
static GslWaveFileInfo *oggv_load_file_info(gpointer data, const gchar *file_name, GslErrorType *error_p)
{
...
static void oggv_free_file_info(gpointer data, GslWaveFileInfo *file_info)
{
...
static GslWaveDsc *oggv_load_wave_dsc(gpointer data, GslWaveFileInfo *file_info, guint nth_wave, GslErrorType *error_p)
{
    FileInfo *fi = (FileInfo *)file_info;
...
static void oggv_free_wave_dsc(gpointer data, GslWaveDsc *wdsc)
{
    g_free(wdsc->name);
    g_free(wdsc->chunks);
...
static GslDataHandle *oggv_create_chunk_handle(gpointer data, GslWaveDsc *wdsc, guint nth_chunk, GslErrorType *error_p)
{
    ...
    dhandle = gsl_data_handle_new_ogg_vorbis(fi->wfi.file_name, wdsc->chunks[0].loader_offset); /* lbitstream */
    if(!dhandle)
        *error_p = GSL_ERROR_OPEN_FAILED;
    return dhandle;
}
...
void _gsl_init_loader_oggvorbis(void)
{
    ...
    static const gchar *mime_types[] = {
        "application/x-ogg", "audio/x-vorbis", "audio/x-ogg", NULL,
    };
