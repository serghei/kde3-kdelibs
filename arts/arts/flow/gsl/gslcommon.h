...
{ GSL_MSG_NOTIFY = 1 << 0,
  ...
  GSL_MSG_SCHED = 1 << 8,
  GSL_MSG_MASTER = 1 << 9,
  GSL_MSG_SLAVE = 1 << 10 } GslDebugFlags;
extern const GDebugKey *gsl_debug_keys;
extern const guint gsl_n_debug_keys;
void gsl_debug(GslDebugFlags reporter, const gchar *section, const gchar *format, ...) G_GNUC_PRINTF(3, 4);
...
void gsl_message_send(GslDebugFlags reporter, const gchar *section, /* maybe NULL */
                      GslErrorType error,                           /* maybe 0 */
                      const gchar *messagef, ...) G_GNUC_PRINTF(4, 5);
...
struct _GslMutexTable
{
    ...
    void (*cond_wait)(GslCond *cond, GslMutex *mutex);
    void (*cond_wait_timed)(GslCond *cond, GslMutex *mutex, gulong abs_secs, gulong abs_usecs);
...
void gsl_free_memblock(gsize size, gpointer memblock);
