...
/* --- functions --- */
static inline void osc_update_pwm_offset(GslOscData *osc, gfloat pulse_mod) /* -1..+1 */
{
...
#define GSL_INCLUDER_TABLE                                                                                                                           \
    static void (*osc_process_table[])(GslOscData *, guint, const gfloat *, const gfloat *, const gfloat *, const gfloat *, gfloat *, gfloat *)
...
#define GSL_INCLUDER_TABLE                                                                                                                           \
    static void (*osc_process_pulse_table[])(GslOscData *, guint, const gfloat *, const gfloat *, const gfloat *, const gfloat *, gfloat *, gfloat *)
#define GSL_INCLUDER_FILE "gsloscillator-aux.c"
#include "gslincluder.c"
#undef OSC_INCLUDER_FLAGS
...
/* --- functions --- */
static inline void osc_process(GslOscData *osc, guint n_values, guint mode, const gfloat *ifreq, const gfloat *imod, const gfloat *isync,
                               const gfloat *ipwm, gfloat *mono_out, gfloat *sync_out)
{
    ...
    if(mode & OSC_FLAG_PULSE_OSC)
        osc_process_pulse_table[mode & ~OSC_FLAG_PULSE_OSC](osc, n_values, ifreq, imod, isync, ipwm, mono_out, sync_out);
    else
        osc_process_table[mode](osc, n_values, ifreq, imod, isync, NULL, mono_out, sync_out);
}

void gsl_osc_process(GslOscData *osc, guint n_values, const gfloat *ifreq, const gfloat *imod, const gfloat *isync, gfloat *mono_out,
                     gfloat *sync_out)
{
    ...
    osc_process(osc, n_values, 0, ifreq, imod, isync, NULL, mono_out, sync_out);
}

void gsl_osc_process_pulse(GslOscData *osc, guint n_values, const gfloat *ifreq, const gfloat *imod, const gfloat *isync, const gfloat *ipwm,
                           gfloat *mono_out, gfloat *sync_out)
{
    g_return_if_fail(osc != NULL);
    g_return_if_fail(n_values > 0);
    g_return_if_fail(mono_out != NULL);
    ...
    if(!(osc->last_mode & OSC_FLAG_PULSE_OSC))
        osc->last_mode = OSC_FLAG_INVAL;
    osc_process(osc, n_values, OSC_FLAG_PULSE_OSC, ifreq, imod, isync, ipwm, mono_out, sync_out);
}
