...
{
    return memset(g_malloc(n_bytes), 0, n_bytes);
}
gpointer g_realloc(gpointer mem, gulong n_bytes)
{
    void *p = realloc(mem, n_bytes);
    GSL_ASSERT(p != 0);
    return p;
}
...
gchar *gsl_g_convert(const gchar *str, gsize len,                                                                               /* gssize */
                     const gchar *to_codeset, const gchar *from_codeset, gsize *bytes_read, gsize *bytes_written, void **error) /* GError */
{
...
static void gsl_g_snprintf(gchar *out_buffer, size_t size, const gchar *format, ...)
{
    gchar *buffer;
    va_list args;
...
static GString *g_string_new(const gchar *s)
{
...
static GString *g_string_append_c(GString *gstr, int ch)
{
...
static void g_string_free(GString *gstr, int freeme)
{
    if(freeme)
        g_free(gstr->str);
    g_free(gstr);
...
/* --- defines --- */
#define to_lower(c)                                                                                                                                  \
    ((guchar)(((((guchar)(c)) >= 'A' && ((guchar)(c)) <= 'Z') * ('a' - 'A')) | ((((guchar)(c)) >= 192 && ((guchar)(c)) <= 214) * (224 - 192))        \
...
static GScannerConfig g_scanner_config_template = {
    ...
    (G_CSET_a_2_z "_" G_CSET_A_2_Z G_CSET_DIGITS G_CSET_LATINS G_CSET_LATINC) /* cset_identifier_nth */,
...
static void g_scanner_msg_handler(GScanner *scanner, gchar *message, gint is_error);
...
GTokenType g_scanner_get_next_token(GScanner *scanner)
{
    ...
    if(scanner->next_token != G_TOKEN_NONE)
    {
        ...
        g_scanner_get_token_i(scanner, &scanner->token, &scanner->value, &scanner->line, &scanner->position);
...
void g_scanner_unexp_token(GScanner *scanner, GTokenType expected_token, const gchar *identifier_spec, const gchar *symbol_spec,
                           const gchar *symbol_name, const gchar *message, gint is_error)
{
    ...
    switch(scanner->token)
    {
        ...
        /* fall through */
        case G_TOKEN_SYMBOL:
            if(expected_token == G_TOKEN_SYMBOL || (scanner->config->symbol_2_token && expected_token > G_TOKEN_LAST))
                print_unexp = FALSE;
            if(symbol_name)
                g_snprintf(token_string, token_string_len, "%s%s `%s'", print_unexp ? "" : "invalid ", symbol_spec, symbol_name);
            else
        ...
        case G_TOKEN_IDENTIFIER_NULL:
            if(expected_token == G_TOKEN_IDENTIFIER || expected_token == G_TOKEN_IDENTIFIER_NULL)
                print_unexp = FALSE;
            g_snprintf(token_string, token_string_len, "%s%s `%s'", print_unexp ? "" : "invalid ", identifier_spec,
        ...
        case G_TOKEN_STRING:
            if(expected_token == G_TOKEN_STRING)
                print_unexp = FALSE;
            g_snprintf(token_string, token_string_len, "%s%sstring constant \"%s\"", print_unexp ? "" : "invalid ",
    ...
    switch(expected_token)
    {
        ...
        case G_TOKEN_INT:
            g_snprintf(expected_string, expected_string_len, "%snumber (integer)", scanner->token == G_TOKEN_INT ? "valid " : "");
            break;

        case G_TOKEN_FLOAT:
            g_snprintf(expected_string, expected_string_len, "%snumber (float)", scanner->token == G_TOKEN_FLOAT ? "valid " : "");
            break;

        case G_TOKEN_STRING:
            g_snprintf(expected_string, expected_string_len, "%sstring constant", scanner->token == G_TOKEN_STRING ? "valid " : "");
            break;

        case G_TOKEN_IDENTIFIER:
        case G_TOKEN_IDENTIFIER_NULL:
            g_snprintf(expected_string, expected_string_len, "%s%s",
                       (scanner->token == G_TOKEN_IDENTIFIER_NULL || scanner->token == G_TOKEN_IDENTIFIER ? "valid " : ""), identifier_spec);
            break;
...
static void g_scanner_get_token_i(GScanner *scanner, GTokenType *token_p, GTokenValue *value_p, guint *line_p, guint *position_p)
{
    ...
    } while(((*token_p > 0 && *token_p < 256) && strchr(scanner->config->cset_skip_characters, *token_p))
            || (*token_p == G_TOKEN_CHAR && strchr(scanner->config->cset_skip_characters, value_p->v_char))
            || (*token_p == G_TOKEN_COMMENT_MULTI && scanner->config->skip_comment_multi)
...
static void g_scanner_get_token_ll(GScanner *scanner, GTokenType *token_p, GTokenValue *value_p, guint *line_p, guint *position_p)
{
    ...
    do /* while (ch != 0) */
    {
        ...
        switch(ch)
        {
            ...
            case '/':
                if(!config->scan_comment_multi || g_scanner_peek_next_char(scanner) != '*')
                    goto default_case;
                        ...
                        switch(ch)
                        {
                            ...
                            case 'E':
                                if((token != G_TOKEN_HEX && !config->scan_float)
                                   || (token != G_TOKEN_HEX && token != G_TOKEN_OCTAL && token != G_TOKEN_FLOAT && token != G_TOKEN_INT))
                                {
                                    token = G_TOKEN_ERROR;
                                    value.v_error = G_ERR_NON_DIGIT_IN_CONST;
                                    in_number = FALSE;
    ...
    if(token == G_TOKEN_IDENTIFIER)
    {
        ...
        if(token == G_TOKEN_IDENTIFIER && config->scan_identifier_NULL && strlen(value.v_identifier) == 4)
        {
            ...
            {
                if((value.v_identifier[0] == null_upper[0] || value.v_identifier[0] == null_lower[0])
                   && (value.v_identifier[1] == null_upper[1] || value.v_identifier[1] == null_lower[1])
                   && (value.v_identifier[2] == null_upper[2] || value.v_identifier[2] == null_lower[2])
...
#error need to implement special handling for long double
                        ...
                        if(c == 'f' && u_double.mpn.biased_exponent > 0 && u_double.mpn.biased_exponent < 2047)
                        {
                    ...
                    default:
                        if(may_warn)
                            g_warning(G_STRLOC ": unable to handle `%c' while parsing format", c);
                        break;
                }
