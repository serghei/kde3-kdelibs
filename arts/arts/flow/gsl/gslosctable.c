 ...
 */
#define CACHE_MATCH_FREQ(usr_mfreq, cache_mfreq) (fabs((cache_mfreq)*44107 - (usr_mfreq)*44107) < OSC_FREQ_EPSILON)
...
static gint osc_table_entry_locs_cmp(gconstpointer bsearch_node1, /* key */
                                     gconstpointer bsearch_node2);
static void osc_wave_extrema_pos(guint n_values, const gfloat *values, guint *minp_p, guint *maxp_p);
void gsl_osc_cache_debug_dump(void);


/* --- variables --- */
static GBSearchArray *cache_entries = NULL;
static const GBSearchConfig cache_taconfig = {sizeof(OscTableEntry *), cache_table_entry_locs_cmp, 0};
static const GBSearchConfig osc_taconfig = {sizeof(OscTableEntry *), osc_table_entry_locs_cmp, 0};
...
static OscTableEntry *cache_table_entry_lookup_best(GslOscWaveForm wave_form, guint8 *filter_func, gfloat mfreq)
{
    ...
    if(ep2)
    {
         ...
         */
        if(ep1 && ((*ep1)->wave_form != wave_form || (*ep1)->filter_func != filter_func || (*ep1)->mfreq < mfreq))
            ep1 = NULL;
        if(ep3 && ((*ep3)->wave_form != wave_form || (*ep3)->filter_func != filter_func || (*ep3)->mfreq < mfreq))
            ep3 = NULL;
        if((*ep2)->wave_form != wave_form || (*ep2)->filter_func != filter_func || (*ep2)->mfreq < mfreq)
        {
...
static OscTableEntry *osc_table_entry_lookup_best(const GslOscTable *table, gfloat mfreq, gfloat *min_mfreq)
{
    ...
    if_reject(!ep) return NULL; /* ugh, bad */
    ...
    if(mfreq > (*ep)->mfreq) /* need better filter */
    {
        ...
        if(i + 1 < g_bsearch_array_get_n_nodes(table->entry_array))
            ...
            OSC_DEBUG("lookup mismatch, aliasing possible: want_freq=%f got_freq=%f", mfreq * table->mix_freq, (*ep)->mfreq * table->mix_freq);
    }
...
static guint wave_table_size(GslOscWaveForm wave_form, gfloat mfreq)
{
...
static void fft_filter(guint n_values, gfloat *values, /* [0..n_values], n_values/2 complex values */
                       gdouble scale_window, double (*window)(double))
{
    guint i;
...
static OscTableEntry *cache_table_ref_entry(GslOscWaveForm wave_form, double (*filter_func)(double), gfloat mfreq)
{
...
static void cache_table_unref_entry(OscTableEntry *e)
{
...
GslOscTable *gsl_osc_table_create(gfloat mix_freq, GslOscWaveForm wave_form, double (*filter_func)(double), guint n_freqs, const gfloat *freqs)
{
    ...
    for(i = 0; i < n_freqs; i++)
    {
        ...
        if(!e || fabs(e->mfreq * table->mix_freq - mfreq * table->mix_freq) > OSC_FREQ_EPSILON)
        {
            ...
            OSC_DEBUG("not inserting existing entry (freq=%f) for freq %f (nyquist=%f)", e->mfreq * table->mix_freq, mfreq * table->mix_freq,
                      nyquist);
    }
