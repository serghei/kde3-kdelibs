...
#define GSL_FLOAT_IS_NORMAL(f) (GSL_FLOAT_PARTS(f).mpn.biased_exponent > 0 && GSL_FLOAT_PARTS(f).mpn.biased_exponent < 255)
#define GSL_FLOAT_IS_SUBNORMAL(f) (GSL_FLOAT_PARTS(f).mpn.biased_exponent == 0 && GSL_FLOAT_PARTS(f).mpn.mantissa != 0)
...
#define GSL_DOUBLE_IS_NORMAL(d) (GSL_DOUBLE_PARTS(d).mpn.biased_exponent > 0 && GSL_DOUBLE_PARTS(d).mpn.biased_exponent < 2047)
#define GSL_DOUBLE_IS_SUBNORMAL(d)                                                                                                                   \
    (GSL_DOUBLE_PARTS(d).mpn.biased_exponent == 0 && (GSL_DOUBLE_PARTS(d).mpn.mantissa_low != 0 || GSL_DOUBLE_PARTS(d).mpn.mantissa_high != 0))
#define GSL_DOUBLE_IS_NANINF(d) (GSL_DOUBLE_PARTS(d).mpn.biased_exponent == 2047)
#define GSL_DOUBLE_IS_NAN(d) (GSL_DOUBLE_IS_NANINF(d) && (GSL_DOUBLE_PARTS(d).mpn.mantissa_low != 0 || GSL_DOUBLE_PARTS(d).mpn.mantissa_high != 0))
#define GSL_DOUBLE_IS_INF(d) (GSL_DOUBLE_IS_NANINF(d) && GSL_DOUBLE_PARTS(d).mpn.mantissa_low == 0 && GSL_DOUBLE_PARTS(d).mpn.mantissa_high == 0)
...
#define _GSL_DOUBLE_INF_BYTES                                                                                                                        \
    {                                                                                                                                                \
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf0, 0x7f                                                                                               \
    }
#define _GSL_DOUBLE_NAN_BYTES                                                                                                                        \
    {                                                                                                                                                \
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xf8, 0x7f                                                                                               \
    }
#define _GSL_FLOAT_INF_BYTES                                                                                                                         \
    {                                                                                                                                                \
        0x00, 0x00, 0x80, 0x7f                                                                                                                       \
    }
#define _GSL_FLOAT_NAN_BYTES                                                                                                                         \
    {                                                                                                                                                \
        0x00, 0x00, 0xc0, 0x7f                                                                                                                       \
    }
...
static const union {
    unsigned char c[8];
    double d;
} _gsl_dnan_union = {_GSL_DOUBLE_NAN_BYTES};
static const union {
    unsigned char c[8];
    double d;
} _gsl_dinf_union = {_GSL_DOUBLE_INF_BYTES};
static const union {
    unsigned char c[4];
    float f;
} _gsl_fnan_union = {_GSL_FLOAT_NAN_BYTES};
static const union {
    unsigned char c[4];
    float f;
} _gsl_finf_union = {_GSL_FLOAT_INF_BYTES};
...
static inline int G_GNUC_CONST gsl_ftoi(register float f)
{
    int r;
...
static inline int G_GNUC_CONST gsl_dtoi(register double f)
{
    int r;
...
static inline int G_GNUC_CONST gsl_ftoi(register float v)
{
    return v < -0.0 ? v - 0.5 : v + 0.5;
}
static inline int G_GNUC_CONST gsl_dtoi(register double v)
{
    return v < -0.0 ? v - 0.5 : v + 0.5;
}
#endif
