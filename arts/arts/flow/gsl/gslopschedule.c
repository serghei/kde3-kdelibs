...
static inline void unschedule_node(EngineSchedule *sched, EngineNode *node)
{
    guint leaf_level;
...
static inline void unschedule_cycle(EngineSchedule *sched, GslRing *ring)
{
    guint leaf_level;
    GslRing *walk;
...
static void _engine_schedule_debug_dump(EngineSchedule *sched)
{
    g_printerr("sched(%p) = {\n", sched);
    if(sched)
    {
...
void _engine_schedule_destroy(EngineSchedule *sched)
{
    g_return_if_fail(sched != NULL);
    g_return_if_fail(sched->secured == FALSE);
    g_return_if_fail(sched->in_pqueue == FALSE);
...
static void _engine_schedule_grow(EngineSchedule *sched, guint leaf_level)
{
...
void _engine_schedule_node(EngineSchedule *sched, EngineNode *node, guint leaf_level)
{
    g_return_if_fail(sched != NULL);
    g_return_if_fail(sched->secured == FALSE);
...
void _engine_schedule_cycle(EngineSchedule *sched, GslRing *cycle_nodes, guint leaf_level)
{
    GslRing *walk;
...
void _engine_schedule_restart(EngineSchedule *sched)
{
    g_return_if_fail(sched != NULL);
...
void _engine_schedule_secure(EngineSchedule *sched)
{
    g_return_if_fail(sched != NULL);
    g_return_if_fail(sched->secured == FALSE);
...
static void schedule_advance(EngineSchedule *sched)
{
...
EngineNode *_engine_schedule_pop_node(EngineSchedule *sched)
{
...
GslRing *_engine_schedule_pop_cycle(EngineSchedule *sched)
{
    g_return_val_if_fail(sched != NULL, NULL);
    g_return_val_if_fail(sched->secured == TRUE, NULL);
    g_return_val_if_fail(sched->cur_leaf_level <= sched->leaf_levels, NULL);
...
void _engine_schedule_unsecure(EngineSchedule *sched)
{
    g_return_if_fail(sched != NULL);
    g_return_if_fail(sched->secured == TRUE);
    g_return_if_fail(sched->in_pqueue == FALSE);
...
/* --- depth scheduling --- */
static GslRing *merge_untagged_node_lists_uniq(GslRing *ring1, GslRing *ring2)
{
    GslRing *walk;
...
static gboolean resolve_cycle(EngineCycle *cycle, EngineNode *node, GslRing **cycle_nodes_p)
{
    if(node != cycle->last)
        return FALSE;
...
static gboolean master_resolve_cycles(EngineQuery *query, EngineNode *node)
{
    GslRing *walk;
...
static void query_add_cycle(EngineQuery *query, EngineNode *dep, EngineNode *node)
{
...
static void query_merge_cycles(EngineQuery *query, EngineQuery *child_query, EngineNode *node)
{
    GslRing *walk;
...
static void subschedule_query_node(EngineSchedule *schedule, EngineNode *node, EngineQuery *query)
{
...
void _engine_schedule_consumer_node(EngineSchedule *schedule, EngineNode *node)
{
    EngineQuery query = {
        0,
    };
