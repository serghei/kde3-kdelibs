...
/* --- functions --- */
static GslErrorType ov_errno_to_error(gint ov_errno, GslErrorType fallback)
{
    switch(ov_errno)
    {
        case OV_EOF:
            return GSL_ERROR_EOF;
        case OV_EBADLINK:
        case OV_EBADPACKET:
        case OV_HOLE:
            return GSL_ERROR_DATA_CORRUPT;
        case OV_EREAD:
            return GSL_ERROR_READ_FAILED;
        case OV_ENOSEEK:
            return GSL_ERROR_SEEK_FAILED;
        case OV_EFAULT:
        case OV_EIMPL:
            return GSL_ERROR_CODEC_FAILURE;
        case OV_EINVAL:
...
static size_t rfile_read(void *ptr, size_t size, size_t nmemb, void *datasource)
{
    GslRFile *rfile = datasource;
    return gsl_rfile_read(rfile, size * nmemb, ptr);
}

static int rfile_seek(void *datasource, ogg_int64_t offset, int whence)
{
    GslRFile *rfile = datasource;
    GslLong l;
    switch(whence)
...
static int rfile_close(void *datasource)
{
    GslRFile *rfile = datasource;
...
static long rfile_tell(void *datasource)
{
    GslRFile *rfile = datasource;
...
static ov_callbacks rfile_ov_callbacks = {
    rfile_read, rfile_seek, rfile_close, rfile_tell,
};

static GslErrorType dh_vorbis_open(GslDataHandle *data_handle, GslDataHandleSetup *setup)
{
...
static GslLong dh_vorbis_coarse_seek(GslDataHandle *dhandle, GslLong voffset)
{
    ...
    if(pos < vhandle->pcm_pos || pos >= vhandle->pcm_pos + vhandle->pcm_length + SEEK_BY_READ_AHEAD(vhandle))
    {
...
static void read_packet(VorbisHandle *vhandle)
{
    gfloat **pcm = NULL;
    gint stream_id, i;
...
static GslLong dh_vorbis_read(GslDataHandle *dhandle, GslLong voffset, /* in values */
...
static void dh_vorbis_close(GslDataHandle *dhandle)
{
    VorbisHandle *vhandle = (VorbisHandle *)dhandle;
...
static void dh_vorbis_destroy(GslDataHandle *data_handle)
{
    VorbisHandle *vhandle = (VorbisHandle *)data_handle;
...
static GslDataHandleFuncs dh_vorbis_vtable = {
    dh_vorbis_open, dh_vorbis_read, dh_vorbis_close, dh_vorbis_destroy, dh_vorbis_coarse_seek,
};
