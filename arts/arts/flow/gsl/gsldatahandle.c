...
GslErrorType gsl_data_handle_open(GslDataHandle *dhandle)
{
    ...
    if(dhandle->open_count == 0)
    {
        ...
        if(!error && (dhandle->setup.n_values < 0 || dhandle->setup.n_channels < 1 || dhandle->setup.bit_depth < 1))
        {
            g_warning("internal error in data handle open() (%p): nv=%ld nc=%u bd=%u", dhandle->vtable->open, dhandle->setup.n_values,
                      dhandle->setup.n_channels, dhandle->setup.bit_depth);
...
GslLong gsl_data_handle_read(GslDataHandle *dhandle, GslLong value_offset, GslLong n_values, gfloat *values)
{
    GslLong l;
...
GslLong gsl_data_handle_length(GslDataHandle *dhandle)
{
    GslLong l;
...
guint gsl_data_handle_n_channels(GslDataHandle *dhandle)
{
    guint n;
...
guint gsl_data_handle_bit_depth(GslDataHandle *dhandle)
{
    guint n;
...
const gchar *gsl_data_handle_name(GslDataHandle *dhandle)
{
    g_return_val_if_fail(dhandle != NULL, NULL);
...
/* --- const memory handle --- */
static GslErrorType mem_handle_open(GslDataHandle *dhandle, GslDataHandleSetup *setup)
{
...
static void mem_handle_close(GslDataHandle *dhandle)
{
    /* MemHandle *mhandle = (MemHandle*) dhandle; */
}

static void mem_handle_destroy(GslDataHandle *dhandle)
{
    MemHandle *mhandle = (MemHandle *)dhandle;
...
static GslLong mem_handle_read(GslDataHandle *dhandle, GslLong voffset, GslLong n_values, gfloat *values)
{
    MemHandle *mhandle = (MemHandle *)dhandle;
...
GslDataHandle *gsl_data_handle_new_mem(guint n_channels, guint bit_depth, GslLong n_values, const gfloat *values, void (*free)(gpointer values))
{
    static GslDataHandleFuncs mem_handle_vtable = {
        mem_handle_open, mem_handle_read, mem_handle_close, mem_handle_destroy,
    };
    MemHandle *mhandle;
    gboolean success;
...
/* --- chain handle --- */
static GslErrorType chain_handle_open(GslDataHandle *dhandle, GslDataHandleSetup *setup)
{
...
static void chain_handle_close(GslDataHandle *dhandle)
{
    ChainHandle *chandle = (ChainHandle *)dhandle;
...
/* --- reversed handle --- */
static void reverse_handle_destroy(GslDataHandle *data_handle)
{
...
static GslLong reverse_handle_read(GslDataHandle *dhandle, GslLong voffset, GslLong n_values, gfloat *values)
{
...
GslDataHandle *gsl_data_handle_new_reverse(GslDataHandle *src_handle)
{
    static GslDataHandleFuncs reverse_handle_vtable = {
        chain_handle_open, reverse_handle_read, chain_handle_close, reverse_handle_destroy,
    };
...
/* --- cut handle --- */
static GslErrorType cut_handle_open(GslDataHandle *dhandle, GslDataHandleSetup *setup)
{
...
static void cut_handle_destroy(GslDataHandle *data_handle)
{
...
static GslLong cut_handle_read(GslDataHandle *dhandle, GslLong voffset, GslLong n_values, gfloat *values)
{
    CutHandle *chandle = (CutHandle *)dhandle;
...
static GslDataHandle *gsl_data_handle_new_translate(GslDataHandle *src_handle, GslLong cut_offset, GslLong n_cut_values, GslLong tail_cut)
{
    static GslDataHandleFuncs cut_handle_vtable = {
        cut_handle_open, cut_handle_read, chain_handle_close, cut_handle_destroy,
    };
    CutHandle *chandle;
    gboolean success;
 ...
 */
GslDataHandle *gsl_data_handle_new_cut(GslDataHandle *src_handle, GslLong cut_offset, GslLong n_cut_values)
{
 ...
 * the end.
 */
GslDataHandle *gsl_data_handle_new_crop(GslDataHandle *src_handle, GslLong n_head_cut, GslLong n_tail_cut)
{
...
/* --- insert handle --- */
static GslErrorType insert_handle_open(GslDataHandle *dhandle, GslDataHandleSetup *setup)
{
...
static void insert_handle_destroy(GslDataHandle *data_handle)
{
...
static GslLong insert_handle_read(GslDataHandle *data_handle, GslLong voffset, GslLong n_values, gfloat *values)
{
    InsertHandle *ihandle = (InsertHandle *)data_handle;
    GslLong l, orig_n_values = n_values;

    if(voffset < ihandle->src_handle->setup.n_values && voffset < ihandle->paste_offset)
    {
...
GslDataHandle *gsl_data_handle_new_insert(GslDataHandle *src_handle, guint paste_bit_depth, GslLong insertion_offset, GslLong n_paste_values,
                                          const gfloat *paste_values, void (*free)(gpointer values))
{
    static GslDataHandleFuncs insert_handle_vtable = {
        insert_handle_open, insert_handle_read, chain_handle_close, insert_handle_destroy,
    };
...
/* --- loop handle --- */
static GslErrorType loop_handle_open(GslDataHandle *dhandle, GslDataHandleSetup *setup)
{
...
static void loop_handle_destroy(GslDataHandle *data_handle)
{
...
static GslLong loop_handle_read(GslDataHandle *data_handle, GslLong voffset, GslLong n_values, gfloat *values)
{
    LoopHandle *lhandle = (LoopHandle *)data_handle;

    if(voffset < lhandle->loop_start)
        return gsl_data_handle_read(lhandle->src_handle, voffset, MIN(lhandle->loop_start - voffset, n_values), values);
    else
    {
        ...
        return gsl_data_handle_read(lhandle->src_handle, lhandle->loop_start + noffset, MIN(lhandle->loop_width - noffset, n_values), values);
    }
}

GslDataHandle *gsl_data_handle_new_looped(GslDataHandle *src_handle, GslLong loop_first, GslLong loop_last)
{
    static GslDataHandleFuncs loop_handle_vtable = {
        loop_handle_open, loop_handle_read, chain_handle_close, loop_handle_destroy,
    };
    LoopHandle *lhandle;
    gboolean success;
...
/* --- dcache handle --- */
static void dcache_handle_destroy(GslDataHandle *data_handle)
{
...
static GslErrorType dcache_handle_open(GslDataHandle *dhandle, GslDataHandleSetup *setup)
{
...
static void dcache_handle_close(GslDataHandle *data_handle)
{
    DCacheHandle *dhandle = (DCacheHandle *)data_handle;
...
static GslLong dcache_handle_read(GslDataHandle *data_handle, GslLong voffset, GslLong n_values, gfloat *values)
{
    DCacheHandle *dhandle = (DCacheHandle *)data_handle;
...
GslDataHandle *gsl_data_handle_new_dcached(GslDataCache *dcache)
{
    static GslDataHandleFuncs dcache_handle_vtable = {
        dcache_handle_open, dcache_handle_read, dcache_handle_close, dcache_handle_destroy,
    };
...
/* --- wave handle --- */
static inline const guint G_GNUC_CONST wave_format_bit_depth(const GslWaveFormatType format)
{
    switch(format)
    {
...
static void wave_handle_destroy(GslDataHandle *data_handle)
{
...
static GslErrorType wave_handle_open(GslDataHandle *data_handle, GslDataHandleSetup *setup)
{
    WaveHandle *whandle = (WaveHandle *)data_handle;
...
static void wave_handle_close(GslDataHandle *dhandle)
{
...
static GslLong wave_handle_read(GslDataHandle *data_handle, GslLong voffset, GslLong n_values, gfloat *values)
{
    WaveHandle *whandle = (WaveHandle *)data_handle;
            ...
            switch(whandle->format)
            {
                case GSL_WAVE_FORMAT_UNSIGNED_16:
                    if(whandle->byte_order != G_BYTE_ORDER)
                        for(i = 0; i < l; i++)
                        {
                            int v = GUINT16_SWAP_LE_BE(u16[i]);
                            v -= 32768;
                            values[i] = v * (1. / 32768.);
                        }
...
GslDataHandle *gsl_wave_handle_new(const gchar *file_name, guint n_channels, GslWaveFormatType format, guint byte_order, GslLong byte_offset,
                                   GslLong n_values)
{
    static GslDataHandleFuncs wave_handle_vtable = {
        wave_handle_open, wave_handle_read, wave_handle_close, wave_handle_destroy,
    };
    WaveHandle *whandle;
...
GslWaveFormatType gsl_wave_format_from_string(const gchar *string)
{
    ...
    if((string[0] == 'u' || string[0] == 'U') && (string[1] == 'n' || string[1] == 'N'))
    {
        is_unsigned = TRUE;
        string += 2;
    }
