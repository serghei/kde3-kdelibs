...
/* --- variables --- */
static gfloat static_zero_block[STATIC_ZERO_SIZE] = {
    0,
}; /* FIXME */


/* --- functions --- */
static inline void wpos_step(GslWaveChunk *wchunk, WPos *wpos)
{
    wpos->pos += wpos->dir;
    if(wpos->loop_count)
    {
        ...
        {
            if(wpos->dir < 0 && wpos->pos == wchunk->loop_first + wpos->dir)
            {
                wpos->loop_count--;
                wpos->dir = -wpos->dir;
...
static void fill_block(GslWaveChunk *wchunk, gfloat *block, GslLong offset, guint length, gboolean backward, guint loop_count)
{
...
static gfloat *create_block_for_offset(GslWaveChunk *wchunk, GslLong offset, guint length)
{
    ...
    offset -= padding;
    j = ((wchunk->wave_length - one - offset) - (wchunk->pploop_ends_backwards ? wchunk->loop_first : wave_last - wchunk->loop_last));
    if(j >= 0)
    {
        k = j / loop_width;
...
static void setup_pblocks(GslWaveChunk *wchunk)
{
    GslLong padding = wchunk->n_pad_values;
...
static inline GslWaveChunkMem *wave_identify_offset(GslWaveChunk *wchunk, Iter *iter)
{
...
static void wave_chunk_setup_loop(GslWaveChunk *wchunk)
{
    ...
    switch(loop_type)
    {
        case GSL_WAVE_LOOP_JUMP:
            ...
            if(loop_last >= wchunk->length || loop_first >= loop_last)
                goto CASE_DONT_LOOP;
...
GslWaveChunk *gsl_wave_chunk_new(GslDataCache *dcache, gfloat osc_freq, gfloat mix_freq, GslWaveLoopType loop_type, GslLong loop_first,
                                 GslLong loop_last, guint loop_count)
{
    GslWaveChunk *wchunk;
...
GslWaveChunk *_gsl_wave_chunk_copy(GslWaveChunk *wchunk)
{
    ...
    return gsl_wave_chunk_new(wchunk->dcache, wchunk->osc_freq, wchunk->mix_freq, wchunk->loop_type, wchunk->loop_first, wchunk->loop_last,
                              wchunk->loop_count);
}
