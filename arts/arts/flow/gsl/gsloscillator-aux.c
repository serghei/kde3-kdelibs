...
static void GSL_INCLUDER_FUNC(GslOscData *osc, guint n_values, const gfloat *ifreq, const gfloat *mod_in, const gfloat *sync_in, const gfloat *pwm_in,
 ...
 */
#if(ISYNC1_OSYNC0) /* input sync only */
        {
            ...
            if_reject(GSL_SIGNAL_RAISING_EDGE(last_sync_level, sync_level)) cur_pos = sync_pos;
 ...
 */
#if(WITH_FREQ)
        {
            ...
            if(GSL_SIGNAL_FREQ_CHANGED(last_freq_level, freq_level))
            {
                ...
                else pos_inc = gsl_dtoi(freq_level * gsl_cent_factor(osc->config.fine_tune) * wave->freq_to_step);
                posm_strength = pos_inc * osc->config.fm_strength;
                self_posm_strength = pos_inc * osc->config.self_fm_strength;
