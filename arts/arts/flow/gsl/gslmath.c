...
/* --- functions --- */
static inline char *pretty_print_double(char *str, double d)
{
    char *s = str;
...
char *gsl_complex_list(unsigned int n_points, GslComplex *points, const char *indent)
{
    static unsigned int rbi = 0;
    static char *rbuffer[RING_BUFFER_LENGTH] = {
        NULL,
    };
    ...
    rbi++;
    if(rbi >= RING_BUFFER_LENGTH)
        rbi -= RING_BUFFER_LENGTH;
    if(rbuffer[rbi] != NULL)
        g_free(rbuffer[rbi]);
    s = tbuffer;
...
char *gsl_poly_str(unsigned int degree, double *a, const char *var)
{
    static unsigned int rbi = 0;
    static char *rbuffer[RING_BUFFER_LENGTH] = {
        NULL,
...
char *gsl_poly_str1(unsigned int degree, double *a, const char *var)
{
    static unsigned int rbi = 0;
    static char *rbuffer[RING_BUFFER_LENGTH] = {
        NULL,
...
void gsl_complex_gnuplot(const char *file_name, unsigned int n_points, GslComplex *points)
{
...
double gsl_temp_freq(double kammer_freq, int halftone_delta)
{
    double factor;
...
void gsl_poly_from_re_roots(unsigned int degree, double *a, GslComplex *roots)
{
    unsigned int i;
...
void gsl_cpoly_from_roots(unsigned int degree, GslComplex *c, GslComplex *roots)
{
    unsigned int i;
...
void gsl_poly_complex_roots(unsigned int degree, double *a, /* [0..degree] (degree+1 elements) */
...
double gsl_ellip_rf(double x, double y, double z)
{
    return rf(x, y, z);
}

double gsl_ellip_F(double phi, double ak)
{
    return ellf(phi, ak);
}

double gsl_ellip_sn(double u, double emmc)
{
    double sn;
...
double gsl_ellip_asn(double y, double emmc)
{
    return y * rf(1.0 - y * y, 1.0 - y * y * (1.0 - emmc), 1.0);
}

GslComplex gsl_complex_ellip_asn(GslComplex y, GslComplex emmc)
{
    return gsl_complex_mul(y, rfC(gsl_complex_sub(gsl_complex(1.0, 0), gsl_complex_mul(y, y)),
                                  gsl_complex_sub(gsl_complex(1.0, 0), gsl_complex_mul3(y, y, gsl_complex_sub(gsl_complex(1.0, 0), emmc))),
                                  gsl_complex(1.0, 0)));
}

GslComplex gsl_complex_ellip_sn(GslComplex u, GslComplex emmc)
{
    GslComplex sn;
...
double gsl_bit_depth_epsilon(guint n_bits)
{
    ...
    static const double bit_epsilons[] = {
        .4900000000000000000000000000000000000000, .2490000000000000000000000000000000000000, .1249000000000000000000000000000000000000,
        .0624900000000000000000000000000000000000, .0312490000000000000000000000000000000000, .0156249000000000000000000000000000000000,
        .0078124900000000000000000000000000000000, .0039062490000000000000000000000000000000, .0019531249000000000000000000000000000000,
        .0009765624900000000000000000000000000000, .0004882812490000000000000000000000000000, .0002441406249000000000000000000000000000,
        .0001220703124900000000000000000000000000, .0000610351562490000000000000000000000000, .0000305175781249000000000000000000000000,
        .0000152587890624900000000000000000000000, .0000076293945312490000000000000000000000, .0000038146972656249000000000000000000000,
        .0000019073486328124900000000000000000000, .0000009536743164062490000000000000000000, .0000004768371582031249000000000000000000,
        .0000002384185791015624900000000000000000, .0000001192092895507812490000000000000000, .0000000596046447753906249000000000000000,
        ...
        .0000000037252902984619140624900000000000, .0000000018626451492309570312490000000000, .0000000009313225746154785156249000000000,
        .0000000004656612873077392578124900000000, .0000000002328306436538696289062490000000,
    };
...
static inline double DMIN(double d1, double d2)
{
    return d1 < d2 ? d1 : d2;
}
static inline double DMAX(double d1, double d2)
{
    return d1 > d2 ? d1 : d2;
}
static inline double DSQR(double d)
{
...
static void poldiv(double u[], int n, double v[], int nv, double q[], double r[])
...
static void laguer(GslComplex a[], int m, GslComplex *x, int *its)
    ...
    { /* Loop over iterations up to allowed maximum. */
        *its = iter;
        ...
        if(iter % MT)
            ...
            *x = gsl_complex_sub(*x, gsl_complex_rmul(frac[iter / MT], dx));
...
static void zroots(GslComplex a[], int m, GslComplex roots[], int polish)
/* Given the degree m and the m+1 complex coefficients a[0..m] of the polynomial mi=0 a (i)xi,
...
static void qroot(double p[], int n, double *b, double *c, double eps)
...
than eps. The routine poldiv §5.3 is used. */
{
    ...
    for(iter = 1; iter <= ITMAX; iter++)
    {
        ...
        *b += (delb = (r * sc - s * rc) * dv);
        *c += (delc = (-r * sb + s * rb) * dv);
        if((fabs(delb) <= eps * fabs(*b) || fabs(*b) < TINY) && (fabs(delc) <= eps * fabs(*c) || fabs(*c) < TINY))
        {
...
static void sncndnC(GslComplex uu, GslComplex emmc, GslComplex *sn_p, GslComplex *cn_p, GslComplex *dn_p)
{
    ...
    if(emc.re || emc.im) /* gsl_complex_abs (emc)) */
    {
        ...
        for(i = 1; i <= 13; i++)
        {
            ...
            if(gsl_complex_abs(gsl_complex_sub(a, emc)) <= gsl_complex_abs(gsl_complex_mul(gsl_complex(SNCNDN_CA, 0), a)))
                break;
...
RF_BIG at most one fifth the machine overflow limit. */
{
    ...
    if(DMIN(DMIN(x, y), z) < 0.0 || DMIN(DMIN(x + y, x + z), y + z) < RF_TINY || DMAX(DMAX(x, y), z) > RF_BIG)
...
static GslComplex rfC(GslComplex x, GslComplex y, GslComplex z)
{
    ...
    if(DMIN(DMIN(gsl_complex_abs(x), gsl_complex_abs(y)), gsl_complex_abs(z)) < 0.0)
        nrerror("rf: x,y,z have to be positive");
    if(DMIN(DMIN(gsl_complex_abs(x) + gsl_complex_abs(y), gsl_complex_abs(x) + gsl_complex_abs(z)), gsl_complex_abs(y) + gsl_complex_abs(z))
       < RF_TINY)
        nrerror("rf: only one of x,y,z may be 0");
    ...
    return gsl_complex_div(gsl_complex_add3(gsl_complex(1.0, 0),
                                            gsl_complex_mul(e2, gsl_complex_sub3(gsl_complex_mul(RFC_C1, e2), RFC_C2, gsl_complex_mul(RFC_C3, e3))),
