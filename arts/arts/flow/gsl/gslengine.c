 ...
 */
GslJob *gsl_job_access(GslModule *module, GslAccessFunc access_func, gpointer data, GslFreeFunc free_func)
{
    GslJob *job;
 ...
 * gsl_flow_job_access
 */
GslJob *gsl_flow_job_access(GslModule *module, guint64 tick_stamp, GslAccessFunc access_func, gpointer data, GslFreeFunc free_func)
{
    GslJob *job;
    EngineFlowJob *fjob;
...
/* --- initialization --- */
static void slave(gpointer data)
{
    gboolean run = TRUE;
 ...
 */
void gsl_engine_init(gboolean run_threaded, guint block_size, guint sample_freq, guint sub_sample_mask)
{
