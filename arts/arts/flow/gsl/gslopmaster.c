...
#define NODE_FLAG_RECONNECT(node)                                                                                                                    \
    G_STMT_START                                                                                                                                     \
    {                                                                                                                                                \
        (node)->reconnected = (node)->module.klass->reconnect != NULL;                                                                               \
    }                                                                                                                                                \
    G_STMT_END
...
static inline guint64 toyprof_elapsed(ToyprofStamp fstamp, ToyprofStamp lstamp)
{
...
const gfloat gsl_engine_master_zero_block[GSL_STREAM_MAX_VALUES] = {
    0,
}; /* FIXME */
...
/* --- functions --- */
static void add_consumer(EngineNode *node)
{
...
static void remove_consumer(EngineNode *node)
{
...
static void master_idisconnect_node(EngineNode *node, guint istream)
{
    EngineNode *src_node = node->inputs[istream].src_node;
    guint ostream = node->inputs[istream].src_stream;
    gboolean was_consumer;

    g_assert(ostream < ENGINE_NODE_N_OSTREAMS(src_node) && src_node->outputs[ostream].n_outputs > 0); /* these checks better pass */
...
static void master_jdisconnect_node(EngineNode *node, guint jstream, guint con)
{
    EngineNode *src_node = node->jinputs[jstream][con].src_node;
    guint i, ostream = node->jinputs[jstream][con].src_stream;
    gboolean was_consumer;

    g_assert(ostream < ENGINE_NODE_N_OSTREAMS(src_node) && node->module.jstreams[jstream].n_connections > 0
...
static void master_disconnect_node_outputs(EngineNode *src_node, EngineNode *dest_node)
{
    gint i, j;
...
static void master_process_job(GslJob *job)
{
    switch(job->job_id)
    {
        ...
        case ENGINE_JOB_JCONNECT:
            ...
            node->module.jstreams[jstream].values =
                g_renew(const gfloat *, node->module.jstreams[jstream].values, node->module.jstreams[jstream].n_connections + 1);
        ...
        case ENGINE_JOB_JDISCONNECT:
            ...
            for(con = 0; con < node->module.jstreams[jstream].n_connections; con++)
                if(node->jinputs[jstream][con].src_node == src_node && node->jinputs[jstream][con].src_stream == ostream)
                    break;
        ...
        case ENGINE_JOB_ADD_POLL:
            ...
            if(job->data.poll.n_fds + master_n_pollfds > GSL_ENGINE_MAX_POLLFDS)
                g_error("adding poll job exceeds maximum number of poll-fds (%u > %u)", job->data.poll.n_fds + master_n_pollfds,
                        GSL_ENGINE_MAX_POLLFDS);
            ...
            if(poll)
            {
                ...
                if(poll_last->n_fds)
                {
                    ...
                    g_memmove(poll_last->fds, poll_last->fds + poll_last->n_fds,
                              ((guint8 *)(master_pollfds + master_n_pollfds)) - ((guint8 *)(poll_last->fds + poll_last->n_fds)));
                ...
                g_warning(G_STRLOC ": failed to remove unknown poll function %p(%p)", job->data.poll.poll_func, job->data.poll.data);
            break;
        default:
...
static void master_poll_check(glong *timeout_p, gboolean check_with_revents)
{
    ...
    for(poll = master_poll_list; poll; poll = poll->next)
    {
        ...
        if(poll->poll_func(poll->data, gsl_engine_block_size(), &timeout, poll->n_fds, poll->n_fds ? poll->fds : NULL, check_with_revents)
           || timeout == 0)
        {
...
static inline guint64 master_handle_flow_jobs(EngineNode *node, guint64 max_tick)
{
...
static void master_process_locked_node(EngineNode *node, guint n_values)
{
    ...
    while(node->counter < final_counter)
    {
        ...
        for(i = 0; i < ENGINE_NODE_N_OSTREAMS(node); i++)
        {
            ...
            if(node->module.ostreams[i].values != node->outputs[i].buffer + diff)
                memcpy(node->outputs[i].buffer + diff, node->module.ostreams[i].values, (new_counter - node->counter) * sizeof(gfloat));
        }
...
static void master_process_flow(void)
{
    ...
    if(master_schedule)
    {
        ...
        while(node)
        {
            ...
            if_reject(profile_modules) toyprof_stamp(profile_stamp1);
        ...
        if_reject(profile_modules)
        {
            if(profile_node)
            {
                if(profile_maxtime > profile_modules)
                    g_print("Excess Node: %p  Duration: %lu usecs     ((void(*)())%p)         \n", profile_node, profile_maxtime,
                            profile_node->module.klass->process);
                else
                    g_print("Slowest Node: %p  Duration: %lu usecs     ((void(*)())%p)         \r", profile_node, profile_maxtime,
                            profile_node->module.klass->process);
            }
        }
...
static void master_reschedule_flow(void)
{
    EngineNode *node;
...
gboolean _engine_master_prepare(GslEngineLoop *loop)
{
    ...
    MAS_DEBUG("PREPARE: need_dispatch=%u timeout=%6ld n_fds=%u", need_dispatch, loop->timeout, loop->n_fds);
...
void _engine_master_thread(gpointer data)
{
    ...
    g_assert(sizeof(struct pollfd) == sizeof(GPollFD) && G_STRUCT_OFFSET(GPollFD, fd) == G_STRUCT_OFFSET(struct pollfd, fd)
