...
bool AudioIOALSA::open()
{
    ...
    {
        if((err = snd_pcm_open(&m_pcm_playback, _deviceName.c_str(), SND_PCM_STREAM_PLAYBACK, SND_PCM_NONBLOCK)) < 0)
        {
            _error = "device: ";
    ...
    {
        if((err = snd_pcm_open(&m_pcm_capture, _deviceName.c_str(), SND_PCM_STREAM_CAPTURE, SND_PCM_NONBLOCK)) < 0)
        {
            _error = "device: ";
            _error += _deviceName.c_str();
    ...
    if(((_direction & directionWrite) && setPcmParams(m_pcm_playback)) || ((_direction & directionRead) && setPcmParams(m_pcm_capture)))
    {
        snd_pcm_close(m_pcm_playback);
...
void AudioIOALSA::close()
{
    ...
    audio_read_pds.pfds = NULL;
    audio_write_pds.pfds = NULL;
    audio_read_pds.nfds = 0;
    audio_write_pds.nfds = 0;
}
...
void AudioIOALSA::notifyIO(int fd, int type)
{
    ...
    if(m_pcm_playback)
    {
        ...
        if(todo & AudioSubSystem::ioWrite)
        {
            ...
            snd_pcm_poll_descriptors_revents(m_pcm_playback, audio_write_pds.pfds, audio_write_pds.nfds, &revents);
            if(!(revents & POLLOUT))
                todo &= ~AudioSubSystem::ioWrite;
        }
    }
    if(m_pcm_capture)
    {
        ...
        {
            unsigned short revents;
            snd_pcm_poll_descriptors_revents(m_pcm_capture, audio_read_pds.pfds, audio_read_pds.nfds, &revents);
            if(!(revents & POLLIN))
                todo &= ~AudioSubSystem::ioRead;
        }
    }

    if(type & IOType::except)
        todo |= AudioSubSystem::ioExcept;
...
int AudioIOALSA::setPcmParams(snd_pcm_t *pcm)
{
    ...
    if(abs((int)rate - (int)_samplingRate) > (int)tolerance)
    {
        ...
        char details[80];
        sprintf(details, " (requested rate %d, got rate %d)", _samplingRate, rate);
        _error += details;
        return 1;
    }
    _samplingRate = rate;
