...
private:
    ...
    public:
        ...
        void setParent(AudioIOOSSThreaded *instance)
        {
            parent = instance;
        }
...
{
    static const char *device[] = {"/dev/dsp",        /* Linux (and lots of others) */
...
{
    arts_return_val_if_fail(format == AFMT_U8 || format == AFMT_S16_LE || format == AFMT_S16_BE, 16);
...
bool AudioIOOSSThreaded::open()
{
    ...
    string caps = "";
    if(device_caps & DSP_CAP_DUPLEX)
        caps += "duplex ";
    if(device_caps & DSP_CAP_REALTIME)
        caps += "realtime ";
    if(device_caps & DSP_CAP_BATCH)
        caps += "batch ";
    if(device_caps & DSP_CAP_COPROC)
        caps += "coproc ";
    if(device_caps & DSP_CAP_TRIGGER)
        caps += "trigger ";
    if(device_caps & DSP_CAP_MMAP)
        caps += "mmap ";
    artsdebug("device capabilities: revision%d %s", device_caps & DSP_CAP_REVISION, caps.c_str());
    ...
    {
        char details[80];
        sprintf(details, " (_format = %d, asked driver to give %d, got %d)", _format, requestedFormat, gotFormat);
    ...
    if(abs(speed - _samplingRate) > tolerance)
    {
        ...
        char details[80];
        sprintf(details, " (requested rate %d, got rate %d)", _samplingRate, speed);
        _error += details;
