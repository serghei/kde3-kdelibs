    ...
    for(bi = _busList.begin(); bi != _busList.end(); bi++)
    {
        ...
        for(ci = bus->clients.begin(); ci != bus->clients.end(); ci++)
        {
            if(*ci == client)
            {
                ...
                {
                    client->snode()->devirtualize("left", bus->left._node(), "invalue");
                    client->snode()->devirtualize("right", bus->right._node(), "invalue");
                }
                return;
            }
        }
    ...
    for(bi = _busList.begin(); bi != _busList.end(); bi++)
    {
        ...
        for(si = bus->servers.begin(); si != bus->servers.end(); si++)
        {
            if(*si == server)
            {
                ...
                {
                    server->snode()->devirtualize("left", bus->left._node(), "outvalue");
                    server->snode()->devirtualize("right", bus->right._node(), "outvalue");
                }
                return;
            }
        }
...
class Synth_BUS_UPLINK_impl : public Synth_BUS_UPLINK_skel, public StdSynthModule, public BusClient {
    BusManager *bm;
...
class Synth_BUS_DOWNLINK_impl : public Synth_BUS_DOWNLINK_skel, public StdSynthModule, public BusClient {
    bool running, active, relink;
    BusManager *bm;
