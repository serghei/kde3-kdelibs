...
DataHandle::DataHandle(GslDataHandle *handle) : handle_(handle)
{
}

DataHandle::DataHandle(const DataHandle &other)
{
    copyFrom(other);
}
...
gint DataHandle::open()
{
    ...
    arts_debug("open()ing datahandle (open_count before: %d)..", handle_->open_count);
...
void DataHandle::close()
{
    ...
    arts_debug("close()ing datahandle (open_count before: %d)..", handle_->open_count);
...
DataHandle DataHandle::createCropped(GslLong headCutValueCount, GslLong tailCutValueCount)
{
    arts_return_val_if_fail(handle_ != 0, null());

    return DataHandle(gsl_data_handle_new_crop(handle_, headCutValueCount, tailCutValueCount));
}

DataHandle DataHandle::createCut(GslLong cutOffset, GslLong cutValueCount)
{
    arts_return_val_if_fail(handle_ != 0, null());

    return DataHandle(gsl_data_handle_new_cut(handle_, cutOffset, cutValueCount));
}
...
{
    arts_debug("wanna have cache with padding %d for each of %d channels..", gsl_get_config()->wave_chunk_padding, channelCount());
    return gsl_data_cache_from_dhandle(handle_, gsl_get_config()->wave_chunk_padding * channelCount());
}
...
WaveChunkDescription::WaveChunkDescription(const GslWaveDsc *parent, guint index) : parent_(parent), parentIndex_(index)
{
    if(index > parent->n_chunks)
    {
        arts_debug(
            "wrong index given to WaveChunkDescription constructor, "
            "using 0 instead..");
        parentIndex_ = 0;
    }
}
...
// internal, private
WaveDescription::WaveDescription(GslWaveFileInfo *parent, guint index, const std::string &name)
    : parentInfo_(parent), name_(name), parentIndex_(index), desc_(0), error_(GSL_ERROR_NONE)
{
...
WaveDescription::WaveDescription(const WaveDescription &other) : desc_(0), error_(GSL_ERROR_NONE)
{
    copyFrom(other);
}
...
WaveFileInfo::WaveFileInfo(const std::string &filename) : info_(0), error_(GSL_ERROR_NONE), filename_(filename)
{
...
WaveDataHandle::WaveDataHandle() : DataHandle(NULL), oscillatorFrequency_(0), mixerFrequency_(0)
{
}

WaveDataHandle::WaveDataHandle(const GslWaveDsc *waveDesc, guint chunkIndex) : DataHandle(NULL), oscillatorFrequency_(0), mixerFrequency_(0)
{
    handle_ = gsl_wave_handle_create(const_cast< GslWaveDsc * >(waveDesc), chunkIndex, &error_);
...
WaveDataHandle::WaveDataHandle(const std::string &filename, guint waveIndex, guint chunkIndex)
    : DataHandle(NULL), oscillatorFrequency_(0), mixerFrequency_(0)
{
