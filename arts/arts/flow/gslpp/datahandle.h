...
public:
     ...
     */
    DataHandle createCropped(GslLong headCutValueCount, GslLong tailCutValueCount);

    /**
     * Create a new data handle by removing the values
     * [cutOffset..cutOffset+cutValueCount-1].
     */
    DataHandle createCut(GslLong cutOffset, GslLong cutValueCount);
...
public:
    WaveDataHandle(const std::string &filename, guint waveIndex = 0, guint chunkIndex = 0);

    static WaveDataHandle null()
    {
        return WaveDataHandle();
    }
