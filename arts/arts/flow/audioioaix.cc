...
AudioIOAIX::AudioIOAIX()
{
    ...
    if(fd >= 0)
    {
        ...
        param(format) = (audioStatus.bits_per_sample == 8) ? 8 : ((audioStatus.flags & AUDIO_BIG_ENDIAN) ? 17 : 16);
    }
}
...
bool AudioIOAIX::open()
{
    ...
    if(abs(audioInit.srate - _samplingRate) > tolerance)
    {
        ...
        char details[80];
        sprintf(details, " (requested rate %d, got rate %ld)", _samplingRate, audioInit.srate);
        _error += details;
