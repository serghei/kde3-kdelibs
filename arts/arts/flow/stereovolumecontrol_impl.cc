...
class StereoVolumeControl_impl : virtual public StereoVolumeControl_skel, virtual public StdSynthModule {
    float _scaleFactor;
...
public:
    StereoVolumeControl_impl() : _scaleFactor(1.0), _currentVolumeLeft(0.0), _currentVolumeRight(0.0), _virtualized(false), _calcVolume(false)
    {
        virtualize();
    }
    ...
    */
    void calculateBlock(unsigned long samples)
    {
        if(_scaleFactor == 1.0)
        {
            if(_calcVolume)
            {
                 ...
                 */
                if(fabs(_currentVolumeLeft) < 0.001)
                    _currentVolumeLeft = 0.0;
                if(fabs(_currentVolumeRight) < 0.001)
    ...
    {
        return (_currentVolumeLeft + _currentVolumeRight < 0.001) ? asSuspend : asNoSuspend;
    }
};
