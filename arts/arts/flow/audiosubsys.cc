        ...
        if(can_write > 0)
        {
            ...
            if(len != can_write)
                arts_fatal(
                    ...
                    " (see aRts FAQ)",
                    len, can_write, errno, strerror(errno));
...
void AudioSubSystem::adjustDuplexBuffers()
{
    ...
    if(fsize > 0 && fcount > 0) // not all AudioIO classes need to support this
    {
        ...
        int currentOffset = rBuffer.size() + wBuffer.size() + canRead + max((fsize * fcount) - canWrite, 0);

        d->adjustDuplexOffset[d->adjustDuplexOffsetIndex++ & 3] = currentOffset;
        if(d->adjustDuplexOffsetIndex <= 4)
            return;

        int avgOffset;
        avgOffset = d->adjustDuplexOffset[0] + d->adjustDuplexOffset[1] + d->adjustDuplexOffset[2] + d->adjustDuplexOffset[3];
        avgOffset /= 4;
