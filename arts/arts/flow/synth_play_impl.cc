...
public:
    ...
    void streamInit()
    {
        ...
        format = as->format();
        bits = as->bits();
        arts_debug("audio format is %d Hz, %d bits, %d channels", as->samplingRate(), bits, channels);
    }
    ...
    void calculateBlock(unsigned long samples)
    {
        ...
        {
            if(format == 8)
                convert_stereo_2float_i8(samples, invalue_left, invalue_right, outblock);
            else if(format == 16)
                convert_stereo_2float_i16le(samples, invalue_left, invalue_right, outblock);
            else if(format == 17)
                convert_stereo_2float_i16be(samples, invalue_left, invalue_right, outblock);
            else if(format == 32)
            {
            ...
            arts_warning("channels != 1 && channels != 2?");
    ...
    void notifyIO(int fd, int type)
    {
        ...
        if(type & IOType::read)
            asType |= AudioSubSystem::ioRead;
        if(type & IOType::write)
            asType |= AudioSubSystem::ioWrite;
        assert(asType != 0);
