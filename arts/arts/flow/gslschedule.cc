...
public:
    ...
    static gboolean gslCheck(gpointer /* data */, guint /* n_values */, glong * /* timeout_p */, guint /* n_fds */, const GPollFD * /* fds */,
...
Port::Port(const string &name, void *ptr, long flags, StdScheduleNode *parent)
    : _name(name), _ptr(ptr), _flags((AttributeType)flags), parent(parent), _dynamicPort(false)
{
...
AudioPort::AudioPort(const string &name, void *ptr, long flags, StdScheduleNode *parent) : Port(name, ptr, flags, parent)
{
    destcount = 0;
    sourcemodule = 0;
    source = 0;
...
{
    if(source)
        return; // Error, should not happen (See BR70028)
    ...
    gsl_trans_add(trans, gsl_job_connect(source->parent->gslModule, source->gslEngineChannel, parent->gslModule, gslEngineChannel));
    gsl_trans_commit(trans);
}

void AudioPort::disconnect(Port *psource)
{
    if(!source || source != psource->audioPort())
        return; // Error, should not happen (See BR70028)
    assert(source);
    ...
    // GSL disconnect
    GslTrans *trans = gsl_trans_open();
    gsl_trans_add(trans, gsl_job_disconnect(parent->gslModule, gslEngineChannel));
    gsl_trans_commit(trans);
}

// --------- MultiPort ----------

MultiPort::MultiPort(const string &name, void *ptr, long flags, StdScheduleNode *parent) : Port(name, ptr, flags, parent)
{
    conns = 0;
    nextID = 0;
    initConns();
...
void StdScheduleNode::gslProcess(GslModule *module, guint n_values)
{
    ...
    for(j = 0; j < node->inConnCount; j++)
    {
        if(node->inConn[j]->gslIsConstant)
            *((float **)node->inConn[j]->_ptr) = gsl_engine_const_values(node->inConn[j]->gslConstantValue);
        else
...
void StdScheduleNode::rebuildConn()
{
    ...
    for(c = 0; c < inConnCount; c++)
    {
        if(inConn[c]->source)
        {
            gsl_trans_add(trans, gsl_job_connect(inConn[c]->source->parent->gslModule, inConn[c]->source->gslEngineChannel,
                                                 inConn[c]->parent->gslModule, inConn[c]->gslEngineChannel));
        }
    }
    for(c = 0; c < outConnCount; c++)
    {
        ...
        for(ci = outConn[c]->autoDisconnect.begin(); ci != outConn[c]->autoDisconnect.end(); ci++)
        {
            AudioPort *dest = (*ci)->audioPort();
            if(dest)
            {
                gsl_trans_add(trans, gsl_job_connect(outConn[c]->parent->gslModule, outConn[c]->gslEngineChannel, dest->parent->gslModule,
...
{
    if(target == "StdScheduleNode")
        return (StdScheduleNode *)this;
    return 0;
}
...
void StdScheduleNode::accessModule()
{
    ...
    if(!module)
        arts_warning(
            "Error using interface %s in the flowsystem: only objects"
...
void StdScheduleNode::suspend()
{
    if(running)
    {
        accessModule();
        suspended = true;
        if((module->autoSuspend() & asSuspendMask) == asSuspendStop)
            stop();
    }
}
...
void StdScheduleNode::restart()
{
    if(suspended)
    {
        accessModule();
        suspended = false;
        if(!running && (module->autoSuspend() & asSuspendMask) == asSuspendStop)
            start();
    }
}
...
void StdScheduleNode::virtualize(const std::string &port, ScheduleNode *implNode, const std::string &implPort)
{
...
void StdScheduleNode::devirtualize(const std::string &port, ScheduleNode *implNode, const std::string &implPort)
{
    StdScheduleNode *impl = (StdScheduleNode *)implNode->cast("StdScheduleNode");
    if(impl)
    {
...
void StdScheduleNode::connect(const string &port, ScheduleNode *dest, const string &destport)
{
...
void StdScheduleNode::disconnect(const string &port, ScheduleNode *dest, const string &destport)
{
    RemoteScheduleNode *rsn = dest->remoteScheduleNode();
    if(rsn)
    {
...
StdFlowSystem::StdFlowSystem()
{
    ...
    {
        GslConfigValue values[3] = {{"wave_chunk_padding", 8},
                                    {
        ...
        do
        {
            ...
            if(!node->suspendTag)
            {
                ...
                for(c = 0; c < node->outConnCount; c++)
                {
                    ...
                    for(ci = node->outConn[c]->autoDisconnect.begin(); ci != node->outConn[c]->autoDisconnect.end(); ci++)
                    {
                        AudioPort *dest = (*ci)->audioPort();
                        if(dest)
...
{
    StdScheduleNode *sn = (StdScheduleNode *)node._node()->cast("StdScheduleNode");
    sn->start();
}
...
void StdFlowSystem::connectObject(Object sourceObject, const string &sourcePort, Object destObject, const std::string &destPort)
{
    arts_debug("connect port %s to %s", sourcePort.c_str(), destPort.c_str());
    StdScheduleNode *sn = (StdScheduleNode *)sourceObject._node()->cast("StdScheduleNode");
    assert(sn);

    Port *port = sn->findPort(sourcePort);
    assert(port);

    StdScheduleNode *destsn = (StdScheduleNode *)destObject._node()->cast("StdScheduleNode");
    if(destsn)
    {
...
void StdFlowSystem::disconnectObject(Object sourceObject, const string &sourcePort, Object destObject, const std::string &destPort)
{
...
void StdFlowSystem::setFloatValue(Object node, const std::string &port, float value)
{
    StdScheduleNode *sn = (StdScheduleNode *)node._node()->cast("StdScheduleNode");
    assert(sn);
...
FlowSystemReceiver StdFlowSystem::createReceiver(Object object, const string &port, FlowSystemSender sender)
{
    StdScheduleNode *sn = (StdScheduleNode *)object._node()->cast("StdScheduleNode");
