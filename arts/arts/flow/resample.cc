...
#define compose_16le(first, second) (((((second) + 128) & 0xff) << 8) + (first))

#define compose_16be(first, second) (((((first) + 128) & 0xff) << 8) + (second))
...
Resampler::Resampler(Refiller *refiller) : dropBytes(0), refiller(refiller), pos(0.0), step(1.0), channels(2), bits(16), block(0), haveBlock(-1)
{
...
void Resampler::ensureRefill()
{
    ...
    if(block == 0)
    {
        missing = bufferSize + sampleSize - refiller->read(buffer, bufferSize + sampleSize);
...
void Resampler::run(float *left, float *right, unsigned long samples)
{
    ...
    else if(channels == 1 && interpolate)
    {
        while(i < samples)
        {
            ...
            left[i] = right[i] = fbuffer[offset] * (1.0 - error) + fbuffer[offset + 1] * error;
            RESAMPLER_STEP();
        }
    }
    else if(channels == 2)
