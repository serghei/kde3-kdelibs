...
bool AudioIOSun::open()
{
    ...
    if(auinfo.play.precision != (uint_t)((_format == 8) ? 8 : 16)
       || (param(direction) == 3 && auinfo.record.precision != (uint_t)((_format == 8) ? 8 : 16)))
    {
        char play_details[80];
        char record_details[80];
        sprintf(play_details, " (_format = %d, asked driver to give %d, got %d)", _format, _format, auinfo.play.precision);
        if(param(direction) == 3)
            sprintf(record_details, " (_format = %d, asked driver to give %d, got %d)", _format, _format, auinfo.record.precision);
    ...
    if((auinfo.play.encoding != (uint_t)AUDIO_ENCODING_LINEAR) || (param(direction) == 3 && auinfo.record.encoding != (uint_t)AUDIO_ENCODING_LINEAR))
    {
        char play_encoding[80], record_encoding[80];
        sprintf(play_encoding, "(%d bits, %d encoding)", auinfo.play.precision, auinfo.play.encoding);
        sprintf(record_encoding, "(%d bits, %d encoding)", auinfo.record.precision, auinfo.record.encoding);

        _error = "Can't set playback and/or record format";
        _error += "requested format was ";
        _error += (_format == 8) ? "8-bit AUDIO_ENCODING_LINEAR" : "16-bit AUDIO_ENCODING_LINEAR";
    ...
    if(abs(int(auinfo.play.sample_rate - _samplingRate)) > tolerance)
    {
        ...
        char details[80];
        sprintf(details, " (requested rate %d, got rate %d)", _samplingRate, auinfo.play.sample_rate);
        _error += details;
    ...
    if(param(direction) == 3 && abs(int(auinfo.record.sample_rate - _samplingRate)) > tolerance)
    {
...
int AudioIOSun::getParam(AudioParam p)
{
    ...
    switch(p)
    {
        ...
        case canWrite:
            if(ioctl(audio_fd, AUDIO_GETINFO, &auinfo) < 0)
                return (0);
            count = SUN_MAX_BUFFER_SIZE - (bytesWritten - (auinfo.play.samples * bytesPerSample));
            return count;
