...
VPortConnection::VPortConnection(VPort *source, VPort *dest, Style style) : source(source), dest(dest), style(style)
{
    ...
    {
#ifdef VPORT_DEBUG
        arts_debug("emit a connection consumer = %s, producer = %s", dest->name(), source->name());
#endif
...
bool VPort::makeVirtualizeParams(VPort *forward, VPort *&source, VPort *&dest, VPortConnection::Style &style)
{
    source = dest = 0;
    // masquerading
...
void VPort::virtualize(VPort *forward)
{
    ...
    {
#ifdef VPORT_DEBUG
        cout << "virtualize ... source (producer) is " << source->name() << " dest (consumer) is " << dest->name() << endl;
#endif
...
void VPort::devirtualize(VPort *forward)
{
    ...
    if(makeVirtualizeParams(forward, source, dest, style))
    {
        ...
        {
            if((*i)->source == source && (*i)->dest == dest && (*i)->style == style)
            {
                delete(*i);
                return;
            }
...
void VPort::expandHelper(VPortConnection *conn, int state, VPort *current, VPort *source, VPort *dest, bool remove)
{
    list< VPortConnection * >::iterator ci;
    ...
    else if(state == 3) /* state 3: input port expansion */
    {
        ...
        if(current->outgoing.empty() && conn == 0)
        {
            if(remove)
            {
                ...
                {
                    if((*ci)->source == source && (*ci)->dest == dest && (*ci)->style == VPortConnection::vcTransport)
                    {
                        delete(*ci);
                        removeOk = true;
                    }
...
const char *VPort::name()
{
    if(_name.empty())
    {
        _name = port->parent->object()->_interfaceName() + "." + port->name();
    }
    return _name.c_str();
}
