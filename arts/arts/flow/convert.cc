...
#define compose_16le(ptr) ((((*((ptr) + 1) + 128) & 0xff) << 8) + *(ptr))

#define compose_16be(ptr) ((((*(ptr) + 128) & 0xff) << 8) + *((ptr) + 1))

#define conv_16_float(x) ((float)((x)-32768) / 32768.0)

#define convert_16le_float(x) conv_16_float(compose_16le(&(x)))
#define convert_16be_float(x) conv_16_float(compose_16be(&(x)))

#define conv_8_float(x) ((float)((x)-128) / 128.0)

#define convert_8_float(x) ((float)((x)-128) / 128.0)
...
#define mk_converter(from_format, to_format)                                                                                                         \
    void Arts::convert_mono_##from_format##_##to_format(unsigned long samples, datatype_##from_format *from, datatype_##to_format *to)               \
    {                                                                                                                                                \
    ...
    void Arts::interpolate_mono_##from_format##_##to_format(unsigned long samples, double startpos, double speed, datatype_##from_format *from,      \
    ...
    {                                                                                                                                                \
        ...
        {                                                                                                                                            \
            ...
            *to = (convert_##from_format##_##to_format(from[position])) * (1.0 - error)                                                              \
                  + (convert_##from_format##_##to_format(from[position + datasize_##from_format])) * error;                                          \
            to += datasize_##to_format;                                                                                                              \
            flpos += speed;                                                                                                                          \
            samples--;                                                                                                                               \
        }                                                                                                                                            \
    }                                                                                                                                                \
    void Arts::convert_stereo_i##from_format##_2##to_format(unsigned long samples, datatype_##from_format *from, datatype_##to_format *left,         \
    ...
    void Arts::interpolate_stereo_i##from_format##_2##to_format(unsigned long samples, double startpos, double speed, datatype_##from_format *from,  \
                                                                datatype_##to_format *left, datatype_##to_format *right)                             \
    {                                                                                                                                                \
        double flpos = startpos;                                                                                                                     \
        while(samples)                                                                                                                               \
        {                                                                                                                                            \
            ...
            double error = flpos - floor(flpos);                                                                                                     \
            *left = (convert_##from_format##_##to_format(from[position])) * (1.0 - error)                                                            \
                    + (convert_##from_format##_##to_format(from[position + 2 * datasize_##from_format])) * error;                                    \
            left += datasize_##to_format;                                                                                                            \
            position += datasize_##from_format;                                                                                                      \
            *right = (convert_##from_format##_##to_format(from[position])) * (1.0 - error)                                                           \
                     + (convert_##from_format##_##to_format(from[position + 2 * datasize_##from_format])) * error;                                   \
            right += datasize_##to_format;                                                                                                           \
            flpos += speed;                                                                                                                          \
            samples--;                                                                                                                               \
...
mk_converter(8, float) mk_converter(16le, float) mk_converter(16be, float) mk_converter(float, float)
...
unsigned long Arts::uni_convert_stereo_2float(unsigned long samples,     // number of required samples
                                              ...
                                              )
{
    ...
    if(doSamples)
    {
        if(fromChannels == 1)
        {
            if(fromBits == uni_convert_float_ne)
            {
                interpolate_mono_float_float(doSamples, startposition, speed, (float *)from, left);
            }
            else if(fromBits == uni_convert_s16_be)
            {
                interpolate_mono_16be_float(doSamples, startposition, speed, from, left);
            }
            else if(fromBits == uni_convert_s16_le)
            {
                interpolate_mono_16le_float(doSamples, startposition, speed, from, left);
            }
            else
            {
                interpolate_mono_8_float(doSamples, startposition, speed, from, left);
            }
        ...
        else if(fromChannels == 2)
        {
            if(fromBits == uni_convert_float_ne)
            {
                interpolate_stereo_ifloat_2float(doSamples, startposition, speed, (float *)from, left, right);
            }
            else if(fromBits == uni_convert_s16_be)
            {
                interpolate_stereo_i16be_2float(doSamples, startposition, speed, from, left, right);
            }
            else if(fromBits == uni_convert_s16_le)
            {
                interpolate_stereo_i16le_2float(doSamples, startposition, speed, from, left, right);
            }
            else
            {
                interpolate_stereo_i8_2float(doSamples, startposition, speed, from, left, right);
            }
        }
        else
        {
