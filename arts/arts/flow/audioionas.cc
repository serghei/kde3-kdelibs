...
static AuBool eventHandler(AuServer *, AuEvent *ev, AuEventHandlerRec *handler);
...
public:
    ...
    friend AuBool eventHandler(AuServer *, AuEvent *ev, AuEventHandlerRec *handler);
};
...
static AuBool Arts::eventHandler(AuServer *, AuEvent *ev, AuEventHandlerRec *handler)
{
...
bool AudioIONAS::open()
{
    ...
    aud = AuOpenServer(_device.compare("null") == 0 ? NULL : _device.c_str(), 0, NULL, 0, NULL, &server_msg);
    if(aud == NULL)
    {
        _error = "device ";
        _error += _device;
    ...
    for(i = 0; i < AuServerNumDevices(aud); i++)
    {
        ...
        if(AuDeviceKind(devattr) == AuComponentKindPhysicalOutput && AuDeviceNumTracks(devattr) == _channels)
        {
    ...
    AuMakeElementImportClient(&elements[0], _samplingRate, _format == 8 ? AuFormatLinearUnsigned8 : AuFormatLinearSigned16LSB, _channels, AuTrue,
                              _buf_samples * _fragmentCount, _buf_samples * (_fragmentCount) / 2, 0, NULL);
    AuMakeElementExportDevice(&elements[1], 0, device, _samplingRate, AuUnlimitedSamples, 0, NULL);
    AuSetElements(aud, flow, AuTrue, 2, elements, NULL);
    AuRegisterEventHandler(aud, AuEventHandlerIDMask, 0, flow, eventHandler, (AuPointer) this);
