...
public:
    DataHandle_impl(GSL::DataHandle dhandle = GSL::DataHandle::null()) : dhandle_(dhandle)
    {
    ...
    GSL::DataHandle createCropped(long headCutValueCount, long tailCutValueCount)
    {
        return dhandle_.createCropped(headCutValueCount, tailCutValueCount);
    }

    GSL::DataHandle createCut(long cutOffset, long cutValueCount)
    {
...
class ReversedDataHandle_impl : virtual public DataHandle_impl, virtual public ReversedDataHandle_skel {
...
class CroppedDataHandle_impl : virtual public DataHandle_impl, virtual public CroppedDataHandle_skel {
    void init(DataHandle sourceHandle, long headCutValueCount, long tailCutValueCount)
    {
        DataHandle_impl *impl = dynamic_cast< DataHandle_impl * >(sourceHandle._base());
...
class CutDataHandle_impl : virtual public DataHandle_impl, virtual public CutDataHandle_skel {
    void init(DataHandle sourceHandle, long cutOffset, long cutValueCount)
    {
        DataHandle_impl *impl = dynamic_cast< DataHandle_impl * >(sourceHandle._base());
...
class DataHandlePlay_impl : virtual public DataHandlePlay_skel, virtual public StdSynthModule {
protected:
    DataHandle dhandle_;
...
public:
    DataHandlePlay_impl()
        ...
        , wosc_(NULL)
        , mixerFrequency_(0)
        , // FIXME: doc says soundserver's freq
        channelIndex_(0)
        , speed_(1.0f)
        , pos_(0)
        , finished_(false)
    ...
    void handle(DataHandle handle)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if(errno_)
                    arts_debug("DataHandlePlay got error from GSL::DataHandle.open(): '%s'", strerror(errno_));
            }
        }
    }
    ...
    long pos()
    {
        return wosc_ ? wosc_->block.offset : 0;
    }
    void pos(long p)
    {
        if(pos() != p)
    ...
    void createWaveChunk()
    {
        ...
        if(!gslDHandle_.isNull() && gslDHandle_.isOpen())
        {
            ...
            {
                wchunk_ = gsl_wave_chunk_new(dcache, OSCILLATOR_FREQUENCY, mixerFrequency_, GSL_WAVE_LOOP_NONE, 0, 0, 0);
...
class WaveDataHandle_impl : virtual public DataHandle_impl, virtual public WaveDataHandle_skel {
...
public:
    ...
    bool load(const string &filename, long waveIndex, long chunkIndex)
    {
