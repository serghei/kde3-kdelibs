...
public:
    Buffer b;
    AttributeDataPacket(GenericDataChannel *channel) : GenericDataPacket(channel)
    {
        size = 0;
        b.writeLong(0);
    }
...
bool Object_skel::_initAttribute(const AttributeDef &attribute)
{
    ...
    for(i = _internalData->attributeSlots.begin(); i != _internalData->attributeSlots.end(); i++)
    {
        AttributeSlotBind *b = *i;
        if(b->method == "_set_" + attribute.name || b->method == attribute.name + "_changed")
        {
...
void Object_skel::_defaultNotify(const Notification &notification)
{
    ...
    for(i = slots.begin(); i != slots.end(); i++)
    {
        if((*i)->notifyID() == notification.ID)
        {
            ...
            for(mti = _internalData->methodTable.begin(); mti != _internalData->methodTable.end(); mti++)
            {
                if(mti->methodDef.name == (*i)->method)
                {
                    ...
                    while(params.remaining())
                    {
                        ...
                        else if(mti->dispatchStyle == MTE::dfDynamic)
                        {
                            ...
                            mti->dispFunc.dynamicDispatcher(mti->object, methodID, &params, &result);
                        }
                        else
                        {
                            arts_assert(0);
...
void Object_skel::_emit_changed(const char *attrib, const AnyConstRef &value)
{
    ...
    for(i = slots.begin(); i != slots.end(); i++)
    {
        if((*i)->method == attrib)
        {
            AttributeDataPacket *adp = (AttributeDataPacket *)(*i)->createPacket(1);
            adp->add(value);
            adp->send();
            return;
        }
...
bool Object_skel::_generateSlots(const std::string &name, const std::string &interface)
{
    InterfaceDef d = _queryInterface(interface);
    vector< string >::iterator ii;
    for(ii = d.inheritedInterfaces.begin(); ii != d.inheritedInterfaces.end(); ii++)
    {
        if(_generateSlots(name, *ii))
            return true;
    }
    ...
    for(ai = d.attributes.begin(); ai != d.attributes.end(); ai++)
    {
        ...
        {
            if((ai->flags & streamIn && ai->name == name) || (ai->flags & streamOut && ai->name + "_changed" == name))
            {
                _initAttribute(*ai);
                return true;
            }
...
bool Object_skel::_QueryInitStreamFunc(Object_skel *skel, const std::string &name)
{
    /*
     * this function
     *
    ...
    if(!result)
    {
        arts_warning("used stream %s on object %s, which doesn't seem to exist", name.c_str(), skel->_interfaceName().c_str());
    }
    return result;
}
...
Object_base::Object_base() : _deleteOk(false), _scheduleNode(0), _nextNotifyID(1), _refCnt(1)
{
...
Object_base::~Object_base()
{
    ...
    {
        arts_fatal(
            "reference counting violation - you may not call delete "
    ...
    for(ai = _internalData->attributeSlots.begin(); ai != _internalData->attributeSlots.end(); ai++)
    {
        delete(*ai);
    }
    ...
    if(!_scheduleNode)
    {
        switch(_location())
        {
            case objectIsLocal:
            {
                ...
                _scheduleNode->initStream("QueryInitStreamFunc", (void *)Object_skel::_QueryInitStreamFunc, -1);
            }
            break;
...
void Object_skel::_referenceClean()
{
    if(_remoteSendCount > 0)
    {
        ...
        else
        {
            ...
            arts_debug(
                "_referenceClean: found unused object marked by "
...
void Object_skel::_addMethod(DispatchFunction disp, void *obj, const MethodDef &md)
{
...
void Object_skel::_addMethod(OnewayDispatchFunction disp, void *obj, const MethodDef &md)
{
    Arts::ObjectInternalData::MethodTableEntry me;
...
void Object_skel::_addMethod(DynamicDispatchFunction disp, void *obj, const MethodDef &md)
{
    Arts::ObjectInternalData::MethodTableEntry me;
...
long Object_skel::_addCustomMessageHandler(OnewayDispatchFunction handler, void *obj)
{
    if(!_internalData->methodTableInit)
    {
        // take care that the object base methods are at the beginning
...
void Object_skel::_dispatch(Buffer *request, Buffer *result, long methodID)
{
    ...
    const ObjectInternalData::MethodTableEntry &me = _internalData->methodTable[methodID];
...
long Object_skel::_lookupMethod(const MethodDef &md)
{
    ...
    arts_warning(
        "_lookupMethod %s %s failed this might be caused by "
...
{
    long methodID = _lookupMethodFast(
        "method:"
        "000000125f6973436f6d70617469626c65576974680000000008626f6f6c65616e00000000020000000100000007737472696e67000000000e696e746572666163656e616d65"
        "000000000000000000");
    long requestID;
    Arts::Buffer *request, *result;
    request = Arts::Dispatcher::the()->createRequest(requestID, _objectID, methodID);
...
std::string Arts::Object_stub::_addChild(Arts::Object child, const std::string &name)
{
    long methodID = _lookupMethodFast(
        "method:"
        "0000000a5f6164644368696c640000000007737472696e67000000000200000002000000076f626a65637400000000066368696c64000000000000000007737472696e670000"
        "0000056e616d65000000000000000000");
    long requestID;
    Arts::Buffer *request, *result;
    request = Arts::Dispatcher::the()->createRequest(requestID, _objectID, methodID);
...
{
    long methodID = _lookupMethodFast(
        "method:0000000d5f72656d6f76654368696c640000000008626f6f6c65616e00000000020000000100000007737472696e6700000000056e616d65000000000000000000");
    long requestID;
    Arts::Buffer *request, *result;
    request = Arts::Dispatcher::the()->createRequest(requestID, _objectID, methodID);
    request->writeString(name);
...
{
    long methodID = _lookupMethodFast(
        "method:0000000a5f6765744368696c6400000000076f626a65637400000000020000000100000007737472696e6700000000056e616d65000000000000000000");
    long requestID;
    Arts::Buffer *request, *result;
    request = Arts::Dispatcher::the()->createRequest(requestID, _objectID, methodID);
    request->writeString(name);
...
std::vector< std::string > *Arts::Object_stub::_queryChildren()
{
    ...
    if(!result)
        return _returnCode; // error occurred
...
{
    long methodID =
        _lookupMethodFast("method:000000115f6765745f5f666c6f7753797374656d0000000011417274733a3a466c6f7753797374656d00000000020000000000000000");
    long requestID;
    Arts::Buffer *request, *result;
    request = Arts::Dispatcher::the()->createRequest(requestID, _objectID, methodID);
    request->patchLength();
...
public:
    ...
    void shutdown()
    {
        Object_stub::_cleanupMethodCache();
    }
