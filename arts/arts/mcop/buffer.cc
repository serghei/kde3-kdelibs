...
void Buffer::writeString(const string &s)
{
    ...
    writeLong(len);
    contents.insert(contents.end(), reinterpret_cast< const unsigned char * >(s.c_str()), reinterpret_cast< const unsigned char * >(s.c_str() + len));
}
...
long Buffer::readLong()
{
    ...
    if(remaining() >= 4)
    {
        result = (contents[rpos] << 24) + (contents[rpos + 1] << 16) + (contents[rpos + 2] << 8) + contents[rpos + 3];
        rpos += 4;
    }
    else
    {
...
unsigned char Buffer::fromHexNibble(char c)
{
    ...
    if(uc >= '0' && uc <= '9')
        return uc - (unsigned char)'0';
    if(uc >= 'a' && uc <= 'f')
        return uc + 10 - (unsigned char)'a';
    if(uc >= 'A' && uc <= 'F')
        return uc + 10 - (unsigned char)'A';
...
bool Buffer::fromString(const string &data, const string &name)
{
    ...
    if(stringncmp(data, start, start.size()) != 0)
        return false;
    contents.clear();
    ...
    while(di != data.end())
    {
        ...
        if(h >= 16 || l >= 16)
            return false; // no proper hex digit
