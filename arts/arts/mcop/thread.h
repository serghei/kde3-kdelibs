...
public:
     ...
     */
    inline Mutex(bool recursive = false) : impl(recursive ? SystemThreads::the()->createRecMutex_impl() : SystemThreads::the()->createMutex_impl())
    {
    }
...
public:
    ThreadCondition() : impl(SystemThreads::the()->createThreadCondition_impl())
    {
    }
