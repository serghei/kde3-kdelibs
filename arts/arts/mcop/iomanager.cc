...
public:
    ...
    inline IONotify *notify()
    {
        return _notify;
    };
    inline void remove(int types)
    {
        _types &= ~types;
    }
};
...
static void iomanager_debug_latency_end()
{
    if(iomanager_debug_latency_time.tv_sec)
    {
        ...
        float diff = (end.tv_usec - iomanager_debug_latency_time.tv_usec) / 1000.0 + (end.tv_sec - iomanager_debug_latency_time.tv_sec) * 1000.0;
...
TimeWatcher::TimeWatcher(int _milliseconds, TimeNotify *notify) : milliseconds(_milliseconds), _notify(notify), active(false), destroyed(false)
{
...
{
    if(nextNotify.tv_sec > reference.tv_sec)
        return false;
    if(nextNotify.tv_sec < reference.tv_sec)
        return true;
