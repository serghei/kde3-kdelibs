...
static int create_link(const char *file, const char *tmp_dir)
{
    int result;
...
static int build_link(const char *tmp_prefix, const char *kde_prefix)
{
    struct passwd *pw_ent;
    char kde_tmp_dir[PATH_MAX + 1];
    char user_tmp_dir[PATH_MAX + 1];
...
unsigned long MCOPUtils::makeIID(const string &interfaceName)
{
    ...
    if(!iidmapobj)
        iidmapobj = new map< string, unsigned long >;
...
const vector< string > *MCOPUtils::extensionPath()
{
    ...
    if(!result)
        result = readPath("ExtensionPath", EXTENSION_DIR);
    return result;
}
...
const vector< string > *MCOPUtils::traderPath()
{
    ...
    if(!result)
    {
        ...
        const char *home = getenv("HOME");
        if(home)
            result->push_back(home + string("/.mcop/trader-cache"));
    }
    return result;
}
...
string MCOPUtils::mcopDirectory()
{
    ...
    if(mkdir(mcopDirectory.c_str(), 0755) != 0)
    {
        ...
        {
            arts_warning("can't create directory %s (%s)", mcopDirectory.c_str(), why.c_str());
