...
public:
    DispatcherWakeUpHandler()
    {
        ...
        Dispatcher::the()->ioManager()->watchFD(wakeUpPipe[wReceive], IOType::read | IOType::reentrant, this);
    }
...
Dispatcher::Dispatcher(IOManager *ioManager, StartServer startServer)
{
    ...
    if(!envOk)
    {
        string globalCommName = MCOPUtils::readConfigEntry("GlobalComm", "Arts::TmpGlobalComm");
    ...
    if(!arts_md5_auth_set_cookie(secretCookie.c_str()))
    {
         ...
         */
        arts_warning("bad md5 secret-cookie obtained from %s - replacing it", globalComm()._interfaceName().c_str());
        ...
        if(!arts_md5_auth_set_cookie(cookie))
            arts_fatal(
                "error initializing md5 secret cookie "
    ...
    for(i = secretCookie.begin(); i != secretCookie.end(); i++)
        *i = 'y';
...
Dispatcher::~Dispatcher()
{
    ...
    {
        cerr << "warning: leaving MCOP Dispatcher and still " << Object_base::_objectCount() << " object references alive." << endl;
    ...
    if(Type::_typeCount())
    {
        cerr << "warning: leaving MCOP Dispatcher and still " << Type::_typeCount() << " types alive." << endl;
    }

    if(GenericDataPacket::_dataPacketCount())
    {
        cerr << "warning: leaving MCOP Dispatcher and still " << GenericDataPacket::_dataPacketCount() << " data packets alive." << endl;
    }
    ...
    switch(conn->connState())
    {
        ...
        case Connection::expectServerHello:
            if(messageType == mcopServerHello)
            {
                ...
                if(!valid)
                    break; // invalid hello received -> forget it
        ...
        case Connection::expectClientHello:
            if(messageType == mcopClientHello)
            {
                ...
                if(valid
                   && ((c.authProtocol == "md5auth" && c.authData == conn->cookie()) || (c.authProtocol == "noauth" && d->allowNoAuthentication)))
                {
                    ...
                    if(!d->accept)
                    {
                        ...
                        d->accept->hints.push_back("GlobalComm=" + d->globalComm.toString());
                        d->accept->hints.push_back("InterfaceRepo=" + d->interfaceRepo.toString());
                    }
...
{
    char *buffer;
    buffer = arts_strdup_printf("%s-%04x-%08lx", MCOPUtils::getFullHostname().c_str(), getpid(), time(0));
    serverID = buffer;
    free(buffer);
}
...
string Dispatcher::objectToString(long objectID)
{
    ...
    if(unixServer)
        oref.urls.push_back(unixServer->url());
    if(tcpServer)
        oref.urls.push_back(tcpServer->url());
...
void *Dispatcher::connectObjectLocal(ObjectReference &reference, const string &interface)
{
...
Connection *Dispatcher::connectUrl(const string &url)
{
    ...
    if(conn)
    {
        ...
        while((conn->connState() != Connection::established) && !conn->broken())
        {
            if(isMainThread)
                _ioManager->processOneEvent(true);
            else
...
void Dispatcher::handleConnectionClose(Connection *connection)
{
    ...
    for(l = 0; l < objectPool.max(); l++)
    {
        ...
        if(skel)
            skel->_disconnectRemote(connection);
    }
...
{
    if(SystemThreads::the()->isMainThread())
        return;
