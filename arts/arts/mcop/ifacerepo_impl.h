...
class InterfaceRepo_impl : virtual public InterfaceRepoV2_skel {
    ...
    public:
        EnumEntry(Buffer &stream, long moduleID) : EnumDef(stream), Entry(moduleID)
        {
        }
    };
    class TypeEntry : public TypeDef, public Entry {
    public:
        TypeEntry(Buffer &stream, long moduleID) : TypeDef(stream), Entry(moduleID)
        {
        }
    };
    class InterfaceEntry : public InterfaceDef, public Entry {
    public:
        InterfaceEntry(Buffer &stream, long moduleID) : InterfaceDef(stream), Entry(moduleID){};
    };
