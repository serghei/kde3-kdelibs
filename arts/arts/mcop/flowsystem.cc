...
void RemoteScheduleNode::virtualize(const std::string &, ScheduleNode *, const std::string &)
{
    // (de)virtualization of ports is only implemented locally
    assert(false);
}

void RemoteScheduleNode::devirtualize(const std::string &, ScheduleNode *, const std::string &)
{
    // (de)virtualization of ports is only implemented locally
    assert(false);
}
...
void RemoteScheduleNode::connect(const std::string &port, ScheduleNode *remoteNode, const std::string &remotePort)
{
    ...
    else if(flags & streamIn)
    {
        ...
        remoteFs.connectObject(remoteNode->nodeObject(), remotePort, nodeObject(), port);
    }
    else
    {
        assert(false);
...
void RemoteScheduleNode::disconnect(const std::string &port, ScheduleNode *remoteNode, const std::string &remotePort)
{
    arts_return_if_fail(remoteNode != 0);
    ...
    else if(flags & streamIn)
    {
        ...
        remoteFs.disconnectObject(remoteNode->nodeObject(), remotePort, nodeObject(), port);
    }
    else
    {
        assert(false);
