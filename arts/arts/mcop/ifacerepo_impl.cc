...
long InterfaceRepo_impl::insertModule(const ModuleDef &newModule)
{
    ...
    for(ii = newModule.interfaces.begin(); ii != newModule.interfaces.end(); ii++)
    {
        Buffer b;
        ii->writeType(b);
    ...
    for(ti = newModule.types.begin(); ti != newModule.types.end(); ti++)
    {
        Buffer b;
        ti->writeType(b);
    ...
    for(ei = newModule.enums.begin(); ei != newModule.enums.end(); ei++)
    {
        Buffer b;
        ei->writeType(b);
...
InterfaceDef InterfaceRepo_impl::queryInterface(const string &name)
{
    ...
    {
        arts_warning(
            "InterfaceRepo: no information about the interface %s "
            "is known",
            name.c_str());
    }
...
TypeDef InterfaceRepo_impl::queryType(const string &name)
{
    ...
    arts_warning("InterfaceRepo: no information about the type %s is known.", name.c_str());
    return TypeDef();
}
...
EnumDef InterfaceRepo_impl::queryEnum(const string &name)
{
    ...
    arts_warning("InterfaceRepo: no information about the enum %s is known.", name.c_str());
    return EnumDef();
}
...
vector< string > *InterfaceRepo_impl::queryChildren(const std::string &name)
{
    ...
    for(ii = interfaces.begin(); ii != interfaces.end(); ii++)
    {
        ...
        for(j = (*ii)->inheritedInterfaces.begin(); j != (*ii)->inheritedInterfaces.end() && !found; j++)
        {
            if(*j == name)
            {
