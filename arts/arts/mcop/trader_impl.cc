...
{
    TraderRestriction(const string &key, const string &value) : key(key), value(value)
    {
    }
...
protected:
    TraderHelper();
    ~TraderHelper();
    void addDirectory(const string &directory, const string &iface = "", map< pair< dev_t, ino_t >, bool > *dirsDone = 0);
...
// TraderOffer
//----------------------------------------------------------------------------
TraderOffer_impl::TraderOffer_impl(const string &interfaceName, const string &filename) : _interfaceName(interfaceName)
{
...
void TraderHelper::addDirectory(const string &directory, const string &iface, map< pair< dev_t, ino_t >, bool > *dirsDone)
{
    ...
    while((de = readdir(dir)) != 0)
    {
        ...
        {
            if(strlen(de->d_name) > 10 && strncmp(&de->d_name[strlen(de->d_name) - 10], ".mcopclass", 10) == 0)
            {
                // cut .mcopclass for currentIface;
                currentIface = currentIface.substr(0, currentIface.size() - 10);
                allOffers.push_back(new TraderOffer_impl(currentIface, currentEntry));
            }
        }
    }
...
vector< TraderOffer > *TraderHelper::doQuery(const vector< TraderRestriction > &query)
{
