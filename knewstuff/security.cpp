    ...
    *readProcess << "gpg"
    ...
    if(!readProcess->start(KProcess::NotifyOnExit, true))
        KMessageBox::error(0L, i18n("<qt>Cannot start <i>gpg</i> and retrieve the available keys. Make sure that <i>gpg</i> is installed, otherwise "
                                    "verification of downloaded resources will not be possible.</qt>"));
    else
        m_gpgRunning = true;
}
    ...
    while(procIO->readln(data, true) != -1)
    {
        switch(m_runMode)
        {
            ...
            case Verify:
                ...
                else if(data.startsWith("NO_PUBKEY"))
                {
                    m_result &= SIGNED_BAD_CLEAR;
                    m_result |= UNKNOWN;
                }
                else if(data.startsWith("BADSIG"))
                {
                    m_result |= SIGNED_BAD;
                    QString id = data.section(" ", 1, 1).right(8);
                    if(!m_keys.contains(id))
                ...
                else if(data.startsWith("TRUST_ULTIMATE"))
                {
                    m_result &= SIGNED_BAD_CLEAR;
                    m_result |= TRUSTED;
                }
            ...
            case Sign:
                if(data.find("passphrase.enter") != -1)
                {
                    ...
                    int result = KPasswordDialog::getPassword(
                        password, i18n("<qt>Enter passphrase for key <b>0x%1</b>, belonging to<br><i>%2&lt;%3&gt;</i>:</qt>")
                                      .arg(m_secretKey)
                                      .arg(key.name)
                                      .arg(key.mail));
                ...
                else if(data.find("BAD_PASSPHRASE") != -1)
                {
                    m_result |= BAD_PASSPHRASE;
                }
                break;
...
void Security::slotCheckValidity()
{
    ...
    *verifyProcess << "gpg"
                   ...
                   << "--status-fd=2"
                   << "--command-fd=0"
                   << "--verify" << f.dirPath() + "/signature" << m_fileName;
    ...
    {
        KMessageBox::error(0L, i18n("<qt>Cannot start <i>gpg</i> and check the validity of the file. Make sure that <i>gpg</i> is installed, "
                                    "otherwise verification of downloaded resources will not be possible.</qt>"));
...
void Security::slotSignFile()
{
    ...
    *signProcess << "gpg"
                 ...
                 << "--no-tty"
                 << "--detach-sign"
                 << "-u" << m_secretKey << "-o" << f.dirPath() + "/signature" << m_fileName;
    ...
    {
        KMessageBox::error(0L, i18n("<qt>Cannot start <i>gpg</i> and sign the file. Make sure that <i>gpg</i> is installed, otherwise signing of the "
