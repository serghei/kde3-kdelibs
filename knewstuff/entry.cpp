...
{
    if(d(this)->mNameMap.isEmpty())
        return QString::null;

    if(!d(this)->mNameMap[lang].isEmpty())
        return d(this)->mNameMap[lang];
    else
    {
        QStringList langs = KGlobal::locale()->languageList();
        for(QStringList::Iterator it = langs.begin(); it != langs.end(); ++it)
            if(!d(this)->mNameMap[*it].isEmpty())
                return d(this)->mNameMap[*it];
    }
    if(!d(this)->mNameMap[QString::null].isEmpty())
        return d(this)->mNameMap[QString::null];
    else
        return *(mSummaryMap.begin());
}
...
Entry::Entry() : mRelease(0), mReleaseDate(QDate::currentDate()), mRating(0), mDownloads(0)
{
}

Entry::Entry(const QDomElement &e) : mRelease(0), mRating(0), mDownloads(0)
{
    parseDomElement(e);
}
...
QString Entry::summary(const QString &lang) const
{
    ...
    if(!mSummaryMap[lang].isEmpty())
        return mSummaryMap[lang];
    else
    {
        QStringList langs = KGlobal::locale()->languageList();
        for(QStringList::Iterator it = langs.begin(); it != langs.end(); ++it)
            if(!mSummaryMap[*it].isEmpty())
                return mSummaryMap[*it];
    }
    if(!mSummaryMap[QString::null].isEmpty())
        return mSummaryMap[QString::null];
    else
        return *(mSummaryMap.begin());
}
...
KURL Entry::payload(const QString &lang) const
{
    ...
    if(payload.isEmpty())
    {
        QStringList langs = KGlobal::locale()->languageList();
        for(QStringList::Iterator it = langs.begin(); it != langs.end(); ++it)
            if(!mPayloadMap[*it].isEmpty())
                return mPayloadMap[*it];
    }
    if(payload.isEmpty())
        payload = mPayloadMap[QString::null];
...
KURL Entry::preview(const QString &lang) const
{
    ...
    if(preview.isEmpty())
    {
        QStringList langs = KGlobal::locale()->languageList();
        for(QStringList::Iterator it = langs.begin(); it != langs.end(); ++it)
            if(!mPreviewMap[*it].isEmpty())
                return mPreviewMap[*it];
    }
    if(preview.isEmpty())
        preview = mPreviewMap[QString::null];
...
void Entry::parseDomElement(const QDomElement &element)
{
    ...
    for(n = element.firstChild(); !n.isNull(); n = n.nextSibling())
    {
        ...
        if(e.tagName() == "email")
            setAuthorEmail(e.text().stripWhiteSpace()); /* kde-look; change on server! */
...
QDomElement Entry::createDomElement(QDomDocument &doc, QDomElement &parent)
{
    ...
    addElement(doc, entry, "releasedate", releaseDate().toString(Qt::ISODate));
...
QDomElement Entry::addElement(QDomDocument &doc, QDomElement &parent, const QString &tag, const QString &value)
{
