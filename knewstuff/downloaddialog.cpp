...
public:
    NumSortListViewItem(QListView *parent, QString label1, QString label2 = QString::null, QString label3 = QString::null,
...
public:
    DateSortListViewItem(QListView *parent, QString label1, QString label2 = QString::null, QString label3 = QString::null,
                         QString label4 = QString::null, QString label5 = QString::null, QString label6 = QString::null,
                         QString label7 = QString::null, QString label8 = QString::null)
        : KListViewItem(parent, label1, label2, label3, label4, label5, label6, label7, label8)
    {
...
DownloadDialog::DownloadDialog(Engine *engine, QWidget *)
    : KDialogBase(KDialogBase::IconList, i18n("Get Hot New Stuff"), KDialogBase::Close, KDialogBase::Close)
{
    init(engine);
}

DownloadDialog::DownloadDialog(QWidget *) : KDialogBase(KDialogBase::IconList, i18n("Get Hot New Stuff"), KDialogBase::Close, KDialogBase::Close)
{
    init(0);
}
...
DownloadDialog::DownloadDialog(Engine *engine, QWidget *, const QString &caption)
    : KDialogBase(KDialogBase::IconList, (caption.isNull() ? i18n("Get Hot New Stuff") : caption), KDialogBase::Close, KDialogBase::Close)
{
    init(engine);
}
...
void DownloadDialog::addProvider(Provider *p)
{
    ...
    if(!ret)
        pix = KGlobal::iconLoader()->loadIcon("knewstuff", KIcon::Panel);
...
void DownloadDialog::slotResult(KIO::Job *job)
{
    ...
    for(QDomNode pn = knewstuff.firstChild(); !pn.isNull(); pn = pn.nextSibling())
    {
        ...
        if(stuff.tagName() == "stuff")
        {
            ...
            {
                if((!m_filter.isEmpty()) && (entry->type() != m_filter))
                    continue;
            }
...
int DownloadDialog::installStatus(Entry *entry)
{
    ...
    else
    {
        ...
        if(!date.isValid())
            installed = 0;
        else if(date < entry->releaseDate())
            installed = -1;
        else
            installed = 1;
    }
...
void DownloadDialog::addEntry(Entry *entry, const QStringList &variants)
{
    ...
    if(installed > 0)
        pix = KGlobal::iconLoader()->loadIcon("ok", KIcon::Small);
    else if(installed < 0)
        pix = KGlobal::iconLoader()->loadIcon("history", KIcon::Small);
    else
        pix = QPixmap();
    ...
    {
        KListViewItem *tmp_r = new NumSortListViewItem(lv_r, entry->name(lang), entry->version(), QString("%1").arg(entry->rating()));
        tmp_r->setPixmap(0, pix);
    }
    if(variants.contains("downloads"))
    {
        KListViewItem *tmp_d = new NumSortListViewItem(lv_d, entry->name(lang), entry->version(), QString("%1").arg(entry->downloads()));
        tmp_d->setPixmap(0, pix);
    }
    if(variants.contains("latest"))
    {
        KListViewItem *tmp_l =
            new DateSortListViewItem(lv_l, entry->name(lang), entry->version(), KGlobal::locale()->formatDate(entry->releaseDate()));
...
{
    if((m_curtab == 0) && (lv_r->selectedItem()))
        return lv_r->selectedItem();
    if((m_curtab == 1) && (lv_d->selectedItem()))
        return lv_d->selectedItem();
    if((m_curtab == 2) && (lv_l->selectedItem()))
...
void DownloadDialog::slotInstalled(KIO::Job *job)
{
    ...
    if(ret)
    {
        ...
        KMessageBox::error(this, i18n("Installation failed."), i18n("Installation"));
    d->m_lvtmp_r->setEnabled(true);
    d->m_lvtmp_l->setEnabled(true);
    d->m_lvtmp_d->setEnabled(true);
...
void DownloadDialog::loadProvider(Provider *p)
{
    ...
    for(QMap< QString, QStringList >::Iterator it = urls.begin(); it != urls.end(); it++)
    {
        ...
        connect(variantjob, SIGNAL(data(KIO::Job *, const QByteArray &)), SLOT(slotData(KIO::Job *, const QByteArray &)));
    }
