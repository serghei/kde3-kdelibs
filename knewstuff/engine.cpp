...
Engine::Engine(KNewStuff *newStuff, const QString &type, QWidget *parentWidget)
    : mParentWidget(parentWidget), mDownloadDialog(0), mUploadDialog(0), mProviderDialog(0), mUploadProvider(0), d(new Private), mType(type)
{
...
Engine::Engine(KNewStuff *newStuff, const QString &type, const QString &providerList, QWidget *parentWidget)
...
void Engine::download()
{
    ...
    connect(mProviderLoader, SIGNAL(providersLoaded(Provider::List *)), SLOT(getMetaInformation(Provider::List *)));
...
void Engine::getMetaInformation(Provider::List *providers)
{
    ...
    {
        if(p->downloadUrl().isEmpty())
            continue;

        KIO::TransferJob *job = KIO::get(p->downloadUrl(), false, false);
        connect(job, SIGNAL(result(KIO::Job *)), SLOT(slotNewStuffJobResult(KIO::Job *)));
        connect(job, SIGNAL(data(KIO::Job *, const QByteArray &)), SLOT(slotNewStuffJobData(KIO::Job *, const QByteArray &)));
            ...
            else
            {
                ...
                for(p = knewstuff.firstChild(); !p.isNull(); p = p.nextSibling())
                {
                    ...
                    if(stuff.tagName() != "stuff")
                        continue;
                    if(stuff.attribute("type", mType) != mType)
                        continue;
...
void Engine::slotDownloadJobResult(KIO::Job *job)
{
    ...
    if(d->mNewStuff->install(mDownloadDestination))
    {
        ...
        {
            KMessageBox::information(mParentWidget, i18n("Successfully installed hot new stuff."));
        }
    }
    else if(!d->mIgnoreInstallResult)
    {
        KMessageBox::error(mParentWidget, i18n("Failed to install hot new stuff."));
    }
}
...
void Engine::upload(const QString &fileName, const QString &previewName)
{
    ...
    connect(mProviderLoader, SIGNAL(providersLoaded(Provider::List *)), SLOT(selectUploadProvider(Provider::List *)));
...
void Engine::upload(Entry *entry)
{
    ...
    if(mUploadProvider->noUpload())
    {
        ...
        {
            int result = KMessageBox::questionYesNo(mParentWidget, text, caption, i18n("Upload Info"), KStdGuiItem::close());
    ...
    {
        int result = KMessageBox::questionYesNo(mParentWidget, text, caption, i18n("&Upload"), KStdGuiItem::cancel());
        if(result == KMessageBox::Yes)
        {
            ...
            connect(job, SIGNAL(result(KIO::Job *)), SLOT(slotUploadPayloadJobResult(KIO::Job *)));
        }
        else
        {
            emit uploadFinished(false);
...
bool Engine::createMetaFile(Entry *entry)
{
    ...
    kdDebug() << "--DOM START--" << endl << doc.toString() << "--DOM_END--" << endl;
...
void Engine::slotUploadPayloadJobResult(KIO::Job *job)
{
    ...
    connect(newJob, SIGNAL(result(KIO::Job *)), SLOT(slotUploadPreviewJobResult(KIO::Job *)));
}
...
void Engine::slotUploadPreviewJobResult(KIO::Job *job)
{
    ...
    connect(newJob, SIGNAL(result(KIO::Job *)), SLOT(slotUploadMetaJobResult(KIO::Job *)));
}
...
void Engine::slotUploadMetaJobResult(KIO::Job *job)
{
    ...
    KMessageBox::information(mParentWidget, i18n("Successfully uploaded new stuff."));
    emit uploadFinished(true);
}
