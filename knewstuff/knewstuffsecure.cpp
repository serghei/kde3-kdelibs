...
KNewStuffSecure::KNewStuffSecure(const QString &type, QWidget *parentWidget) : KNewStuff(type, parentWidget)
{
    m_tempDir = 0L;
...
bool KNewStuffSecure::install(const QString &fileName)
{
    ...
    if(tar.open(IO_ReadOnly))
    {
        ...
        ok = false;
    if(!ok)
        KMessageBox::error(parentWidget(), i18n("There was an error with the downloaded resource tarball file. Possible causes are damaged archive "
                                                "or invalid directory structure in the archive."),
...
void KNewStuffSecure::slotValidated(int result)
{
    ...
    else
    {
        ...
        {
            signatureStr = i18n("The resource was signed with key <i>0x%1</i>, belonging to <i>%2 &lt;%3&gt;</i>.")
                               .arg(key.id.right(8))
                               .arg(key.name)
                               .arg(key.mail);
        }
    ...
    if(!valid)
    {
        ...
        if(KMessageBox::warningContinueCancel(parentWidget(), i18n("<qt>There is a problem with the resource file you have downloaded. The errors "
                                                                   "are :<b>%1</b><br>%2<br><br>Installation of the resource is <b>not "
                                                                   "recommended</b>.<br><br>Do you want to proceed with the installation?</qt>")
                                                                  .arg(errorString)
                                                                  .arg(signatureStr),
                                              i18n("Problematic Resource File"))
           == KMessageBox::Continue)
            valid = true;
    }
    else
        KMessageBox::information(parentWidget(), i18n("<qt>%1<br><br>Press OK to install it.</qt>").arg(signatureStr), i18n("Valid Resource"),
    ...
    else
    {
        if(result & Security::BAD_PASSPHRASE)
        {
            if(KMessageBox::warningContinueCancel(
                   parentWidget(),
                   i18n("There are no keys usable for signing or you did not entered the correct passphrase.\nProceed without signing the resource?"))
               == KMessageBox::Cancel)
            {
