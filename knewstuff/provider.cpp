...
void Provider::parseDomElement(const QDomElement &element)
{
    ...
    if(!iconurl.isValid())
        iconurl.setPath(element.attribute("icon"));
    setIcon(iconurl);
...
ProviderLoader::ProviderLoader(QWidget *parentWidget) : QObject(parentWidget)
{
...
void ProviderLoader::load(const QString &type, const QString &providersList)
{
    ...
    if(providersUrl.isEmpty())
    {
        ...
        QString server = cfg->readEntry("MasterServer", "http://korganizer.kde.org");
...
void ProviderLoader::slotJobResult(KIO::Job *job)
{
    ...
    kdDebug() << "--PROVIDERS-START--" << endl << mJobData << "--PROV_END--" << endl;
