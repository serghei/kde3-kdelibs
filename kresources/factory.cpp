...
Factory::Factory(const QString &resourceFamily) : mResourceFamily(resourceFamily)
{
    KTrader::OfferList plugins = KTrader::self()->query("KResources/Plugin", QString("[X-KDE-ResourceFamily] == '%1'").arg(resourceFamily));
...
ConfigWidget *Factory::configWidget(const QString &type, QWidget *parent)
{
    ...
    if(!factory)
    {
        kdDebug(5650) << "KRES::Factory::configWidget(): Factory creation failed " << KLibLoader::self()->lastErrorMessage() << endl;
        return 0;
    }
    ...
    if(!pluginFactory)
    {
        kdDebug(5650) << "KRES::Factory::configWidget(): no plugin factory." << endl;
        return 0;
    }
...
Resource *Factory::resource(const QString &type, const KConfig *config)
{
    ...
    if(!factory)
    {
        kdDebug(5650) << "KRES::Factory::resource(): Factory creation failed " << KLibLoader::self()->lastErrorMessage() << endl;
        return 0;
    }
    ...
    if(!resource)
    {
        kdDebug(5650) << "'" << ptr->library() << "' is not a " + mResourceFamily + " plugin." << endl;
        return 0;
    }
