...
protected:
    PageNode(KCModuleInfo *info, PageNode *parent) : m_type(KCM), m_parent(parent), m_visible(true), m_dirty(true)
    {
        m_value.kcm = info;
        m_dialog = parent->m_dialog;
    }

    PageNode(GroupInfo &group, PageNode *parent) : m_type(Group), m_parent(parent), m_visible(true), m_dirty(true)
    {
...
public:
    PageNode(Dialog *dialog) : m_type(Root), m_dialog(dialog), m_parent(0), m_visible(true), m_dirty(true)
    {
    }
    ...
    int weight() const
    {
        int w = (KCM == m_type) ? m_value.kcm->weight() : m_value.group->weight;
    ...
    QString name() const
    {
        ...
        return (KCM == m_type) ? m_value.kcm->moduleName() : m_value.group->name;
    }
    ...
    void addToDialog(KCMultiDialog *dlg)
    {
        ...
        if(Group == m_type && 0 == m_value.group->page)
        {
            ...
            if(!m_value.group->icon.isNull())
                icon = SmallIcon(m_value.group->icon, IconSize(KIcon::Small));
            QVBox *page = dlg->addVBoxPage(m_value.group->name, QString::null, icon);
    ...
    bool insert(GroupInfo &group)
    {
        if(group.parentid.isNull())
        {
            if(Root == m_type)
            {
                ...
                kdFatal(700) << "wrong PageNode insertion" << kdBacktrace() << endl;
        }
...
public:
    DialogPrivate(Dialog *parent) : dlg(0), pagetree(parent)
    {
    }
...
Dialog::Dialog(QWidget *parent, const char *name) : QObject(parent, name), d(new DialogPrivate(this))
{
...
Dialog::Dialog(ContentInListView content, QWidget *parent, const char *name) : QObject(parent, name), d(new DialogPrivate(this))
{
    d->parentwidget = parent;
...
Dialog::Dialog(const QStringList &components, QWidget *parent, const char *name) : QObject(parent, name), d(new DialogPrivate(this))
{
    d->parentwidget = parent;
    d->staticlistview = true;
...
Dialog::Dialog(const QStringList &components, ContentInListView content, QWidget *parent, const char *name)
    : QObject(parent, name), d(new DialogPrivate(this))
{
    d->parentwidget = parent;
    d->staticlistview = (content == Static);
    d->services = instanceServices() + parentComponentsServices(components);
...
{
    // read .setdlg files
    QString setdlgpath = locate("appdata", KGlobal::instance()->instanceName() + ".setdlg");
    QStringList setdlgaddon = KGlobal::dirs()->findAllResources("appdata", "ksettingsdialog/*.setdlg");
    ...
    d->dlg = new KCMultiDialog(dialogface, i18n("Configure"), d->parentwidget);
    ...
    if(!d->staticlistview)
        d->dlg->addButtonBelowList(i18n("Select Components..."), this, SLOT(configureTree()));
