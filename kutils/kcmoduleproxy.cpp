...
KCModule *KCModuleProxy::realModule() const
{
    ...
    { /* We got the name we requested, because no one was before us,
       * or, it was an random application which had picked that name */
        ...
        d->kcm = KCModuleLoader::loadModule(moduleInfo(), KCModuleLoader::Inline, d->withFallback, that, name(), d->args);

        connect(d->kcm, SIGNAL(changed(bool)), SLOT(moduleChanged(bool)));
        connect(d->kcm, SIGNAL(destroyed()), SLOT(moduleDestroyed()));
        connect(d->kcm, SIGNAL(quickHelpChanged()), SIGNAL(quickHelpChanged()));
        QWhatsThis::add(that, d->kcm->quickHelp());

        d->topLayout->addWidget(d->kcm);

        if(!d->rootInfo &&                                                                   /* If it's already done */
           moduleInfo().needsRootPrivileges() /* root, anyone? */ && !KUser().isSuperUser()) /* Not necessary if we're root */
        {
            ...
            if(msg.isEmpty())
                d->rootInfo->setText(
                    i18n("<b>Changes in this section requires root access.</b><br />"
                         "Click the \"Administrator Mode\" button to "
                         "allow modifications."));
            else
                d->rootInfo->setText(msg);

            QWhatsThis::add(d->rootInfo, i18n("This section requires special permissions, probably "
    ...
    else
    {
        ...
        connect(d->dcopClient, SIGNAL(applicationRemoved(const QCString &)), SLOT(applicationRemoved(const QCString &)));
        ...
        if(d->dcopClient->call(d->dcopName, d->dcopName, "applicationName()", data, replyType, replyData))
        {
            stream >> result;

            d->kcm = KCModuleLoader::reportError(KCModuleLoader::Inline, i18n("Argument is application name",
...
void KCModuleProxy::runAsRoot()
{
    ...
    QPalette pal(red);
    pal.setColor(QColorGroup::Background, colorGroup().background());
    ...
    if(!kdesu.isEmpty())
    {
        ...
        *d->rootProcess << "--nonewdcop"
                        << "-n"
                        << "-d" << QString("-i%1").arg(moduleInfo().icon());

        *d->rootProcess << QString("%1 %2 --embed-proxy %3 --lang %4")
                               .arg(locate("exe", "kcmshell"))
                               .arg(cmd)
                               .arg(d->embedWidget->winId())
...
KCModuleProxy::KCModuleProxy(const KService::Ptr &service, bool withFallback, QWidget *parent, const char *name, const QStringList &args)
    : QWidget(parent, name)
{
...
KCModuleProxy::KCModuleProxy(const KCModuleInfo &info, bool withFallback, QWidget *parent, const char *name, const QStringList &args)
    : QWidget(parent, name)
{
    init(info);
    d->args = args;
...
KCModuleProxy::KCModuleProxy(const QString &serviceName, bool withFallback, QWidget *parent, const char *name, const QStringList &args)
    : QWidget(parent, name)
{
...
void KCModuleProxy::callRootModule(const QCString &function)
{
    ...
    if(!kapp->dcopClient()->call(d->dcopName, d->dcopName, function, sendData, replyType, replyData, true, -1))
...
QString KCModuleProxy::quickHelp() const
{
    ...
    else
    {
        ...
        if(kapp->dcopClient()->call(d->dcopName, d->dcopName, "quickHelp()", data, replyType, replyData))
...
{
    return realModule() ? realModule()->buttons() : KCModule::Help | KCModule::Default | KCModule::Apply;
}
