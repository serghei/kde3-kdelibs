...
KPluginListViewToolTip::KPluginListViewToolTip(QWidget *parent, KListView *lv) : QToolTip(parent), m_listView(lv)
{
}
...
{
    KPluginSelectionWidgetPrivate(KPluginSelector *_kps, const QString &_cat, KConfigGroup *_config)
        : widgetstack(0)
        , kps(_kps)
        , config(_config)
        , tooltip(0)
...
KPluginSelectionWidget::KPluginSelectionWidget(const QValueList< KPluginInfo * > &plugininfos, KPluginSelector *kps, QWidget *parent,
                                               const QString &catname, const QString &category, KConfigGroup *config, const char *name)
    : QWidget(parent, name), d(new KPluginSelectionWidgetPrivate(kps, catname, config))
{
...
{
    for(QMap< QCheckListItem *, KPluginInfo * >::ConstIterator it = d->pluginInfoMap.begin(); it != d->pluginInfoMap.end(); ++it)
...
QWidget *KPluginSelectionWidget::insertKCM(QWidget *parent, const KCModuleInfo &moduleinfo)
{
    KCModuleProxy *module = new KCModuleProxy(moduleinfo, false, parent);
    ...
    QStringList *parentComponents = new QStringList(moduleinfo.service()->property("X-KDE-ParentComponents").toStringList());
...
void KPluginSelectionWidget::updateConfigPage(KPluginInfo *plugininfo, bool checked)
{
...
public:
    KPluginSelectorPrivate() : frame(0), tabwidget(0), widgetstack(0), hideconfigpage(false)
    {
    }
...
void KPluginSelector::checkNeedForTabWidget()
{
    ...
    if(!d->tabwidget && d->pswidgets.size() == 1)
    {
        ...
        if(w)
        {
            ...
            connect(d->tabwidget, SIGNAL(currentChanged(QWidget *)), w, SLOT(tabWidgetChanged(QWidget *)));
        }
    }
}
...
static QValueList< KPluginInfo * > kpartsPluginInfos(const QString &instanceName)
{
    ...
    const QStringList desktopfilenames = KGlobal::dirs()->findAllResources("data", instanceName + "/kpartplugins/*.desktop", true, false);
    return KPluginInfo::fromFiles(desktopfilenames);
}

void KPluginSelector::addPlugins(const QString &instanceName, const QString &catname, const QString &category, KConfig *config)
{
...
void KPluginSelector::addPluginsInternal(const QValueList< KPluginInfo * > plugininfos, const QString &catname, const QString &category,
                                         KConfigGroup *cfgGroup)
{
    KPluginSelectionWidget *w;
    if(d->tabwidget)
    {
        w = new KPluginSelectionWidget(plugininfos, this, d->tabwidget, catname, category, cfgGroup);
        d->tabwidget->addTab(w, catname);
        connect(d->tabwidget, SIGNAL(currentChanged(QWidget *)), w, SLOT(tabWidgetChanged(QWidget *)));
    }
    else
        w = new KPluginSelectionWidget(plugininfos, this, d->frame, catname, category, cfgGroup);
    w->setMinimumSize(200, 200);
    connect(w, SIGNAL(changed(bool)), this, SIGNAL(changed(bool)));
    connect(w, SIGNAL(configCommitted(const QCString &)), this, SIGNAL(configCommitted(const QCString &)));
    d->pswidgets += w;
}

void KPluginSelector::addPlugins(const KInstance *instance, const QString &catname, const QString &category, KConfig *config)
{
    if(!config)
...
void KPluginSelector::addPlugins(const QValueList< KPluginInfo * > &plugininfos, const QString &catname, const QString &category, KConfig *config)
{
    checkNeedForTabWidget();
...
{
    for(QValueList< KPluginSelectionWidget * >::Iterator it = d->pswidgets.begin(); it != d->pswidgets.end(); ++it)
    {
        (*it)->load();
    }
}
...
void KPluginSelector::defaults()
{
    ...
    KCModuleProxy *kcm = (KCModuleProxy *)pluginconfig->qt_cast("KCModuleProxy");
    if(kcm)
    {
        kdDebug(702) << "call KCModule::defaults() for the plugins KCM" << endl;
        kcm->defaults();
        return;
    }
    ...
    // one KCM
    QObjectList *kcms = pluginconfig->queryList("KCModuleProxy", 0, false, false);
