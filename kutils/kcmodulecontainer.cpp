...
public:
    KCModuleContainerPrivate(const QStringList &mods)
        : modules(mods), tabWidget(0), buttons(0), hasRootKCM(false), btnRootMode(0), btnLayout(0), topLayout(0)
    {
    }
...
/***********************************************************************/
KCModuleContainer::KCModuleContainer(QWidget *parent, const char *name, const QString &mods) : KCModule(parent, name)
{
    d = new KCModuleContainerPrivate(QStringList::split(",", QString(mods).remove(" ")));
    init();
}

KCModuleContainer::KCModuleContainer(QWidget *parent, const char *name, const QStringList &mods)
...
void KCModuleContainer::addModule(const QString &module)
{
    ...
    {
        kdDebug(713) << "KCModuleContainer: module '" << module << "' was not found and thus not loaded" << endl;
        return;
    }
...
void KCModuleContainer::tabSwitched(QWidget *module)
{
    ...
    if(mod->moduleInfo().needsRootPrivileges() && !mod->rootMode())
    {
        ...
        connect(mod, SIGNAL(childClosed()), SLOT(rootExited()));
    }
    else
