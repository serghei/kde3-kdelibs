...
public:
    KPluginInfoPrivate() : hidden(false), enabledbydefault(false), pluginenabled(false), config(0), kcmservicesCached(false)
    {
    }
...
KPluginInfo::KPluginInfo(const QString &filename, const char *resource) : d(new KPluginInfoPrivate)
{
    ...
    if(filename.endsWith(QString::fromAscii(".desktop")))
    {
        ...
        d->enabledbydefault = file.readBoolEntry("X-KDE-PluginInfo-EnabledByDefault", false);
    }
...
KPluginInfo::KPluginInfo(const KService::Ptr service) : d(new KPluginInfoPrivate)
{
    d->service = service;
    ...
    d->dependencies = service->property("X-KDE-PluginInfo-Depends").toStringList();
...
{
    QStringList files = KGlobal::dirs()->findAllResources("data", name + "/kpartplugins/*.desktop", true, false);
...
const QValueList< KService::Ptr > &KPluginInfo::kcmServices() const
{
    ...
    {
        d->kcmservices = KTrader::self()->query("KCModule", "'" + d->pluginName + "' in [X-KDE-ParentComponents]");
        kdDebug(703) << "found " << d->kcmservices.count() << " offers for " << d->pluginName << endl;
