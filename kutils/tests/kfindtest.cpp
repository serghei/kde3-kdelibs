...
void KFindTest::find(const QString &pattern, long options)
{
    ...
    connect(m_find, SIGNAL(highlight(const QString &, int, int)), SLOT(slotHighlight(const QString &, int, int)));
...
{
    m_hits.append("line: \"" + text + "\", index: " + QString::number(index) + ", length: " + QString::number(matchedLength) + "\n");
}

void KFindTest::slotHighlight(int id, int index, int matchedLength)
{
    m_hits.append("line: \"" + m_text[id] + "\", index: " + QString::number(index) + ", length: " + QString::number(matchedLength) + "\n");
}
