...
void KReplaceTest::replace(const QString &pattern, const QString &replacement, long options)
{
    ...
    // of found text.
    connect(m_replace, SIGNAL(highlight(const QString &, int, int)), this, SLOT(slotHighlight(const QString &, int, int)));
    // Connect findNext signal - called when pressing the button in the dialog
    connect(m_replace, SIGNAL(findNext()), this, SLOT(slotReplaceNext()));
    // Connect replace signal - called when doing a replacement
    connect(m_replace, SIGNAL(replace(const QString &, int, int, int)), this, SLOT(slotReplace(const QString &, int, int, int)));
...
{
    kdDebug() << "slotHighlight Index:" << matchingIndex << " Length:" << matchedLength << " Substr:" << str.mid(matchingIndex, matchedLength)
              << endl;
...
{
    kdDebug() << "slotReplace index=" << replacementIndex << " replacedLength=" << replacedLength << " matchedLength=" << matchedLength
              << " text=" << text.left(50) << endl;
...
int main(int argc, char **argv)
{
    ...
    testReplaceBackRef(KReplaceDialog::BackReference | KReplaceDialog::FindBackwards | KReplaceDialog::PromptOnReplace,
                       KDialogBase::User3); // replace
    testReplaceBackRef(KReplaceDialog::BackReference | KReplaceDialog::FindBackwards | KReplaceDialog::PromptOnReplace,
                       KDialogBase::User1); // replace all
