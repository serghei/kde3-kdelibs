...
public:
    KCMError(const QString &msg, const QString &details, QWidget *parent) : KCModule(parent, "KCMError")
    {
...
KCModule *KCModuleLoader::load(const KCModuleInfo &mod, const QString &libname, KLibLoader *loader, ErrorReporting report, QWidget *parent,
                               const char *name, const QStringList &args)
{
    ...
    if(lib)
    {
        ...
        if(lib->hasSymbol(QFile::encodeName(initSym)))
        {
            ...
            {
                KCModule *module =
                    KParts::ComponentFactory::createInstanceFromFactory< KCModule >(factory, parent, name ? name : mod.handle().latin1(), args);
                if(module)
                    return module;
            }
            // else do a fallback
    ...
    return reportError(report, i18n("The specified library %1 could not be found.").arg(mod.library()), QString::null, parent);
    return 0;
}
...
KCModule *KCModuleLoader::loadModule(const KCModuleInfo &mod, ErrorReporting report, bool withfallback, QWidget *parent, const char *name,
                                     const QStringList &args)
{
    ...
    if(!mod.service())
    {
        return reportError(report, i18n("The module %1 could not be found.").arg(mod.moduleName()),
                           i18n("<qt><p>The diagnostics is:<br>The desktop file %1 could not be found.</qt>").arg(mod.fileName()), parent);
    }
    ...
    if(!mod.library().isEmpty())
    {
        ...
        if(module)
            return module;
        return reportError(report, i18n("The module %1 could not be loaded.").arg(mod.moduleName()), QString::null, parent);
    }
    ...
    {
        return reportError(report, i18n("The module %1 is not a valid configuration module.").arg(mod.moduleName()),
                           i18n("<qt><p>The diagnostics is:<br>The desktop file %1 does not specify a library.</qt>").arg(mod.fileName()), parent);
    }

    return 0;
}

KCModule *KCModuleLoader::loadModule(const QString &module, QWidget *parent, const char *name, const QStringList &args)
{
    return loadModule(KCModuleInfo(module), None, false, parent, name, args);
}

KCModule *KCModuleLoader::loadModule(const QString &module, ErrorReporting report, QWidget *parent, const char *name, const QStringList &args)
{
...
{
    KMessageBox::detailedError(parent, i18n("There was an error loading the module."),
                               i18n("<qt><p>The diagnostics is:<br>%1"
...
KCModule *KCModuleLoader::reportError(ErrorReporting report, const QString &text, QString details, QWidget *parent)
{
    if(details.isNull())
        details = i18n(
