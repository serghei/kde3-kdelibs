 ...
 */
    ...
    if(info.synth_type == SYNTH_TYPE_SAMPLE && info.synth_subtype == SAMPLE_TYPE_AWE32)
    {
...
void SynthOut::noteOn(uchar chn, uchar note, uchar vel)
{
    ...
    {
        SEQ_START_NOTE(device, map->channel(chn), map->key(chn, chnpatch[chn], note), vel);
    }
#ifdef SYNTHOUTDEBUG
...
{
    SEQ_STOP_NOTE(device, map->channel(chn), map->key(chn, chnpatch[chn], note), 0);
#ifdef SYNTHOUTDEBUG
