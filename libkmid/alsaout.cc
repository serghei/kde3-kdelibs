...
void AlsaOut::openDev(int)
{
    ...
    if(di->client < 0)
    {
        fprintf(stderr, "Couldn't get client id");
        return;
    };
    ...
    int port = snd_seq_create_simple_port(di->handle, NULL, SND_SEQ_PORT_CAP_WRITE | SND_SEQ_PORT_CAP_SUBS_WRITE | SND_SEQ_PORT_CAP_READ,
...
void AlsaOut::eventInit(snd_seq_event_t *ev)
{
    ...
    if(!di->src)
    {
        fprintf(stderr, "AlsaOut::eventInit : no source\n");
        return;
    }
    ev->source = *di->src;
    if(!di->tgt)
    {
        fprintf(stderr, "AlsaOut::eventInit : no target\n");
        return;
    }
    ev->dest = *di->tgt;
