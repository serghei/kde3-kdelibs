...
MidiOut::MidiOut(int d)
{
    ...
    if(map == NULL)
    {
        printfdebug("ERROR : midiOut : Map is NULL\n");
        return;
    };
    _ok = 1;
}
...
const char *MidiOut::deviceName(void) const
{
    switch(deviceType())
    {
        ...
        case(KMID_AWE):
            return "AWE";
        case(KMID_ALSA):
            return reinterpret_cast< const AlsaOut * >(this)->deviceName();
    }
    return "Unknown";
}
...
{
    if(deviceType() == KMID_ALSA)
    { // XXX : sync should be virtual after next bic
