...
DeviceManager::DeviceManager(int def)
{
    ...
    if(def == -1)
    {
        ...
        if(!KGlobal::_instance)
            tmp_instance = new KInstance("nonKDEapp");
            ...
            if(ioctl(seqfd, SNDCTL_SYNTH_INFO, &synthinfo[i]) != -1)
            {
                ...
                switch(synthinfo[i].synth_subtype)
                {
                    ...
                    case(SAMPLE_TYPE_GUS):
                        ...
                        break;
                    default:
                        printf("default subtype\n");
                        break;
                }
#endif
                if(synthinfo[i].synth_type == SYNTH_TYPE_FM)
                    device[i + n_midi] = new FMOut(i, synthinfo[i].nr_voices);
                else if((synthinfo[i].synth_type == SYNTH_TYPE_SAMPLE) && (synthinfo[i].synth_subtype == SAMPLE_TYPE_GUS))
...
const char *DeviceManager::midiMapFilename(void)
{
    ...
    if(default_dev >= n_total)
        return "";
    return (device[default_dev] != NULL) ? device[default_dev]->midiMapFilename() : "";
}
