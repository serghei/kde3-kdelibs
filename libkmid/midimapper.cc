...
void MidiMapper::loadFile(const char *name)
{
    ...
    while(!feof(fh))
    {
        ...
        {
            if(strncmp(&s[7], "PATCHMAP", 8) == 0)
                readPatchmap(fh);
            else if(strncmp(&s[7], "KEYMAP", 6) == 0)
                readKeymap(fh, s);
            else if(strncmp(&s[7], "CHANNELMAP", 10) == 0)
                readChannelmap(fh);
            else
            {
...
{
    Keymap *km = keymaps;
    while((km != NULL) && (strcmp(km->name, n) != 0))
        km = km->next;
    return km;
}
...
void MidiMapper::readOptions(FILE *fh)
{
    ...
    while(!fin)
    {
        ...
        else if(strncmp(s, "MapExpressionToVolumeEvents", 27) == 0)
            mapExpressionToVolumeEvents = 1;
...
uchar MidiMapper::key(uchar chn, uchar pgm, uchar note)
{
    ...
    if(patchKeymap[pgm] != NULL)
        notemapped = patchKeymap[pgm]->key[note];
    if(channelKeymap[chn] != NULL)
        notemapped = channelKeymap[chn]->key[note];
    return notemapped;
}

uchar MidiMapper::patch(uchar chn, uchar pgm)
{
    return (channelPatchForced[chn] == -1) ? patchmap[pgm] : (uchar)channelPatchForced[chn];
}
...
{
    if((mapExpressionToVolumeEvents) && (ctl == 11))
        ctl = 7;
}
