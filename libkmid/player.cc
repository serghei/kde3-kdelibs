/**************************************************************************
    ...
    $Id: player.cc 539843 2006-05-11 20:08:40Z mueller $
        ...
        switch(ev->command)
        {
            case(MIDI_NOTEON):
                if(ev->vel == 0)
                    ...
                    na->add((ulong)minTime, ev->chn, 1, ev->note);
                break;
            case(MIDI_NOTEOFF):
                na->add((ulong)minTime, ev->chn, 0, ev->note);
                break;
...
#endif
                                strncpy(pspev->text, (char *)ev->data, (ev->length >= sizeof(lasttext)) ? sizeof(lasttext) - 1 : (ev->length));
...
// playing=0;
        ...
        {
            case(MIDI_NOTEON):
                midi->noteOn(ev->chn, ev->note, ev->vel);
                break;
            case(MIDI_NOTEOFF):
                midi->noteOff(ev->chn, ev->note, ev->vel);
                break;
            case(MIDI_KEY_PRESSURE):
                midi->keyPressure(ev->chn, ev->note, ev->vel);
                break;
            case(MIDI_PGM_CHANGE):
                if(!ctl->forcepgm[ev->chn])
                    midi->chnPatchChange(ev->chn, (ctl->gm == 1) ? (ev->patch) : (MT32toGM[ev->patch]));
                break;
            case(MIDI_CHN_PRESSURE):
                midi->chnPressure(ev->chn, ev->vel);
                break;
            case(MIDI_PITCH_BEND):
                midi->chnPitchBender(ev->chn, ev->d1, ev->d2);
                break;
            case(MIDI_CTL_CHANGE):
                midi->chnController(ev->chn, ev->ctl, ev->d1);
                break;
            case(MIDI_SYSTEM_PREFIX):
                if((ev->command | ev->chn) == META_EVENT)
                {
...
void MidiPlayer::setPos(ulong gotomsec, MidiStatus *midistat)
{
    ...
    {
        if(ctl->forcepgm[i])
            midistat->chnPatchChange(i, ctl->pgm[i]);
    }
    ...
    while(likeplaying)
    {
        ...
        if(likeplaying)
        {
            ...
            switch(ev->command)
            {
                ...
                case(MIDI_PGM_CHANGE):
                    if(!ctl->forcepgm[ev->chn])
                        midistat->chnPatchChange(ev->chn, ev->patch);
                    break;
                case(MIDI_CHN_PRESSURE):
                    midistat->chnPressure(ev->chn, ev->vel);
                    break;
                case(MIDI_PITCH_BEND):
                    midistat->chnPitchBender(ev->chn, ev->d1, ev->d2);
                    break;
                case(MIDI_CTL_CHANGE):
                    midistat->chnController(ev->chn, ev->ctl, ev->d1);
                    break;
                case(MIDI_SYSTEM_PREFIX):
                    if((ev->command | ev->chn) == META_EVENT)
                    {
...
void MidiPlayer::debugSpecialEvents(void)
{
    ...
    {
        printf("t:%d ticks:%d diff:%ld abs:%ld s:%s tempo:%ld\n", pspev->type, pspev->ticks, pspev->diffmilliseconds, pspev->absmilliseconds,
               pspev->text, pspev->tempo);
        pspev = pspev->next;
    }
}
