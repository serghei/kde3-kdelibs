...
FunctionPrototypeImp::FunctionPrototypeImp(ExecState *exec) : InternalFunctionImp((FunctionPrototypeImp *)0)
{
    Value protect(this);
    putDirect(toStringPropertyName, new FunctionProtoFuncImp(exec, this, FunctionProtoFuncImp::ToString, 0, toStringPropertyName), DontEnum);
    static const Identifier applyPropertyName("apply");
    putDirect(applyPropertyName, new FunctionProtoFuncImp(exec, this, FunctionProtoFuncImp::Apply, 2, applyPropertyName), DontEnum);
    static const Identifier callPropertyName("call");
    putDirect(callPropertyName, new FunctionProtoFuncImp(exec, this, FunctionProtoFuncImp::Call, 1, callPropertyName), DontEnum);
...
FunctionProtoFuncImp::FunctionProtoFuncImp(ExecState * /*exec*/, FunctionPrototypeImp *funcProto, int i, int len, const Identifier &_ident)
...
Value FunctionProtoFuncImp::call(ExecState *exec, Object &thisObj, const List &args)
{
    ...
    switch(id)
    {
        case ToString:
        {
            ...
            {
                DeclaredFunctionImp *fi = static_cast< DeclaredFunctionImp * >(thisObj.imp());
                return String("function " + fi->name().ustring() + "(" + fi->parameterString() + ") " + fi->body->toCode());
            }
            else if(thisObj.inherits(&InternalFunctionImp::info) && !static_cast< InternalFunctionImp * >(thisObj.imp())->name().isNull())
            {
        ...
        case Apply:
        {
            ...
            {
                if(argArray.isA(ObjectType) && (Object::dynamicCast(argArray).inherits(&ArrayInstanceImp::info)
...
FunctionObjectImp::FunctionObjectImp(ExecState * /*exec*/, FunctionPrototypeImp *funcProto) : InternalFunctionImp(funcProto)
{
    Value protect(this);
...
Object FunctionObjectImp::construct(ExecState *exec, const List &args)
{
    ...
    while(i < len)
    {
        while(*c == ' ' && i < len)
            c++, i++;
        if(Lexer::isIdentLetter(c->uc))
        { // else error
            param = UString(c, 1);
            c++, i++;
            while(i < len && (Lexer::isIdentLetter(c->uc) || Lexer::isDecimalDigit(c->uc)))
            {
                param += UString(c, 1);
                c++, i++;
            }
        ...
        Object err = Error::create(exec, SyntaxError, I18N_NOOP("Syntax error in parameter list"), -1);
        exec->setException(err);
        return err;
    }
