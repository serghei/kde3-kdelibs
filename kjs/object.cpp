...
Value Object::call(ExecState *exec, Object &thisObj, const List &args)
{
    ...
    if(++depth > KJS_MAX_STACK)
    {
        ...
        Object err = Error::create(exec, RangeError, "Exceeded maximum function call depth.");
...
ObjectImp::ObjectImp(const Object &proto) : _proto(static_cast< ObjectImp * >(proto.imp())), _internalValue(0L)
{
    // fprintf(stderr,"ObjectImp::ObjectImp %p\n",(void*)this);
}

ObjectImp::ObjectImp(ObjectImp *proto) : _proto(proto), _internalValue(0L)
{
}
...
Value ObjectImp::getPropertyByIndex(ExecState *exec, unsigned propertyName) const
{
    return get(exec, Identifier::from(propertyName));
}

// ECMA 8.6.2.2
void ObjectImp::put(ExecState *exec, const Identifier &propertyName, const Value &value, int attr)
{
...
// delme
void ObjectImp::putPropertyByIndex(ExecState *exec, unsigned propertyName, const Value &value, int attr)
{
...
Value ObjectImp::defaultValue(ExecState *exec, Type hint) const
{
    ...
    if(v.type() == ObjectType)
    {
        ...
        { // spec says "not primitive type" but ...
            ...
            if(defType == UnspecifiedType || defType == UndefinedType || defType == NullType || defType == BooleanType || defType == StringType
               || defType == NumberType)
            {
                return def;
            }
...
Object Error::create(ExecState *exec, ErrorType errtype, const char *message, int lineno, int sourceId)
{
#ifdef KJS_VERBOSE
