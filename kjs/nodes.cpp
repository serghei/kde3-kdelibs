...
#define KJS_ABORTPOINT                                                                                                                               \
    if(exec->dynamicInterpreter()->imp()->debugger() && exec->dynamicInterpreter()->imp()->debugger()->imp()->aborted())                             \
        return Completion(Normal);
...
Value Node::throwError(ExecState *exec, ErrorType e, const char *msg, const Value &v, const Node *expr) const
{
...
Reference AccessorNode2::evaluateReference(ExecState *exec) const
{
    ...
    {
        UString s = "Attempted to access '" + ident.ustring() + "' property on %s object (result of expression %s)";
...
Value FunctionCallNode::evaluate(ExecState *exec) const
{
    ...
    if(thisVal.type() == ObjectType && Object::dynamicCast(thisVal).inherits(&ActivationImp::info))
        thisVal = Null();
...
Node *AddNode::create(Node *t1, Node *t2, char op)
{
    ...
    // fold constants
    if((t1->type() == NumberType || t1->type() == BooleanType) && (t2->type() == NumberType || t2->type() == BooleanType))
    {
...
Value RelationalNode::evaluate(ExecState *exec) const
{
    ...
    else if(oper == OpIn)
    {
        ...
        if(v2.type() != ObjectType)
            return throwError(exec, TypeError, "Value %s (result of expression %s) is not an object. Cannot be used with IN expression.", v2, expr2);
    ...
    {
        if(v2.type() != ObjectType)
            return throwError(exec, TypeError, "Value %s (result of expression %s) is not an object. Cannot be used with instanceof operator.", v2,
                              expr2);
...
StatListNode::StatListNode(StatementNode *s) : statement(s), list(this)
{
    setLoc(s->firstLine(), s->lastLine(), s->code());
}

StatListNode::StatListNode(StatListNode *l, StatementNode *s) : statement(s), list(l->list)
{
    l->list = this;
...
VarDeclNode::VarDeclNode(const Identifier &id, AssignExprNode *in, Type t) : varType(t), ident(id), init(in)
{
}
...
ForInNode::ForInNode(Node *l, Node *e, StatementNode *s) : init(0L), lexpr(l), expr(e), varDecl(0L), statement(s)
{
}

ForInNode::ForInNode(const Identifier &i, AssignExprNode *in, Node *e, StatementNode *s) : ident(i), init(in), expr(e), statement(s)
{
...
Completion ContinueNode::execute(ExecState *exec)
{
    ...
    if(ident.isEmpty() && !exec->context().imp()->seenLabels()->inIteration())
        return Completion(Throw, throwError(exec, SyntaxError, "continue used outside of iteration statement"));
    else if(!ident.isEmpty() && !exec->context().imp()->seenLabels()->contains(ident))
        return Completion(Throw, throwError(exec, SyntaxError, "Label %s not found in containing block. Can't continue.", ident));
    else
...
Completion BreakNode::execute(ExecState *exec)
{
    ...
    if(ident.isEmpty() && !exec->context().imp()->seenLabels()->inIteration() && !exec->context().imp()->seenLabels()->inSwitch())
        return Completion(Throw, throwError(exec, SyntaxError, "break used outside of iteration or switch statement"));
    else if(!ident.isEmpty() && !exec->context().imp()->seenLabels()->contains(ident))
        return Completion(Throw, throwError(exec, SyntaxError, "Label %s not found in containing block. Can't break.", ident));
    else
...
Completion LabelNode::execute(ExecState *exec)
{
    ...
    {
        return Completion(Throw, throwError(exec, SyntaxError, "Duplicated label %s found.", label));
    };
...
FunctionBodyNode::FunctionBodyNode(SourceElementsNode *s) : BlockNode(s)
{
