...
RegExpPrototypeImp::RegExpPrototypeImp(ExecState *exec, ObjectPrototypeImp *objProto, FunctionPrototypeImp *funcProto) : ObjectImp(objProto)
{
    Value protect(this);
    ...
    putDirect(compilePropertyName, new RegExpProtoFuncImp(exec, funcProto, RegExpProtoFuncImp::Compile, 1, compilePropertyName), DontEnum);
}

// ------------------------------ RegExpProtoFuncImp ---------------------------

RegExpProtoFuncImp::RegExpProtoFuncImp(ExecState * /*exec*/, FunctionPrototypeImp *funcProto, int i, int len, const Identifier &_ident)
...
RegExpImp::RegExpImp(RegExpPrototypeImp *regexpProto) : ObjectImp(regexpProto), reg(0L)
{
}
...
void RegExpImp::setRegExp(RegExp *r)
{
    ...
    putDirect("global", (r->flags() & RegExp::Global) ? BooleanImp::staticTrue : BooleanImp::staticFalse, DontDelete | ReadOnly | DontEnum);
    putDirect("ignoreCase", (r->flags() & RegExp::IgnoreCase) ? BooleanImp::staticTrue : BooleanImp::staticFalse, DontDelete | ReadOnly | DontEnum);
    putDirect("multiline", (r->flags() & RegExp::Multiline) ? BooleanImp::staticTrue : BooleanImp::staticFalse, DontDelete | ReadOnly | DontEnum);
...
RegExpObjectImp::RegExpObjectImp(ExecState * /*exec*/, FunctionPrototypeImp *funcProto, RegExpPrototypeImp *regProto)
...
RegExp *RegExpObjectImp::makeEngine(ExecState *exec, const UString &p, const Value &flagsInput)
{
    ...
    for(int pos = 0; pos < flags.size(); ++pos)
    {
        switch(flags[pos].unicode())
        {
            ...
            default:
            {
                Object err = Error::create(exec, SyntaxError, "Invalid regular expression flags");
                exec->setException(err);
                return 0;
            }
        }
    ...
    if(!re->isValid())
    {
        Object err = Error::create(exec, SyntaxError, "Invalid regular expression");
        exec->setException(err);
        delete re;
        return 0;
    }
