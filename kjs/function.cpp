...
UString decodeURI(ExecState *exec, UString string, UString reservedSet)
{
    ...
    for(int k = 0; k < string.size(); k++)
    {
        ...
        else
        {
            ...
            {
                k++;
                if((UChar(string[k]) != UChar('%')) || !decodeHex(string[k + 1], string[k + 2], &B) || ((B & 0xC0) != 0x80))
                {
                    Object err = Error::create(exec, URIError);
                    exec->setException(err);
                    free(decbuf);
            ...
            else if(n == 3)
            {
                ...
                if(V < 0x800 || V == 0xFFFE || V == 0xFFFF || (V >= 0xD800 && V <= 0xDFFF))
                    V = replacementChar;
                C = UChar((unsigned short)V);
            }
            else
...
FunctionImp::FunctionImp(ExecState *exec, const Identifier &n)
    : InternalFunctionImp(static_cast< FunctionPrototypeImp * >(exec->lexicalInterpreter()->builtinFunctionPrototype().imp()))
    , param(0L)
    , line0(-1)
    , line1(-1)
    , sid(-1)
...
Value FunctionImp::call(ExecState *exec, Object &thisObj, const List &args)
{
    ...
    ContextImp ctx(globalObj, exec->dynamicInterpreter()->imp(), thisObj, sid, codeType(), exec->context().imp(), this, &args);
...
DeclaredFunctionImp::DeclaredFunctionImp(ExecState *exec, const Identifier &n, FunctionBodyNode *b, const ScopeChain &sc)
...
// ECMA 10.1.6
ActivationImp::ActivationImp(FunctionImp *function, const List &arguments) : _function(function), _arguments(true), _argumentsObject(0)
{
...
GlobalFuncImp::GlobalFuncImp(ExecState * /*exec*/, FunctionPrototypeImp *funcProto, int i, int len, const Identifier &_ident)
    ...
    switch(id)
    {
        ...
        { // eval()
            ...
            else
            {
                ...
                // enter a new execution context
                ContextImp ctx(exec->dynamicInterpreter()->globalObject(), exec->dynamicInterpreter()->imp(), thisObj, source->sid, EvalCode,
                               exec->context().imp());
        ...
        { // ECMA 15.1.2.2
            ...
            bool leading0 = false;
            if((base == 0 || base == 16) && (*startptr == '0' && (startptr[1] == 'x' || startptr[1] == 'X')))
            {
                startptr += 2;
                base = 16;
            }
        ...
        case UnEscape:
        {
            ...
            while(k < len)
            {
                ...
                {
                    if(Lexer::isHexDigit((c + 2)->uc) && Lexer::isHexDigit((c + 3)->uc) && Lexer::isHexDigit((c + 4)->uc)
                       && Lexer::isHexDigit((c + 5)->uc))
                    {
