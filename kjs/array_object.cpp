...
Value ArrayInstanceImp::getPropertyByIndex(ExecState *exec, unsigned index) const
{
    if(index > MAX_INDEX)
...
void ArrayInstanceImp::putPropertyByIndex(ExecState *exec, unsigned index, const Value &value, int attr)
{
...
struct CompareWithCompareFunctionArguments
{
    CompareWithCompareFunctionArguments(ExecState *e, ObjectImp *cf)
        : exec(e), compareFunction(cf), globalObject(e->dynamicInterpreter()->globalObject())
    {
...
static int compareWithCompareFunctionForQSort(const void *a, const void *b)
{
    ...
    double compareResult = args->compareFunction->call(args->exec, args->globalObject, args->arguments).toNumber(args->exec);
...
// ECMA 15.4.4
ArrayPrototypeImp::ArrayPrototypeImp(ExecState * /*exec*/, ObjectPrototypeImp *objProto) : ArrayInstanceImp(objProto, 0)
{
    Value protect(this);
...
ArrayObjectImp::ArrayObjectImp(ExecState *exec, FunctionPrototypeImp *funcProto, ArrayPrototypeImp *arrayProto) : InternalFunctionImp(funcProto)
{
    Value protect(this);
