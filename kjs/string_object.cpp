...
StringInstanceImp::StringInstanceImp(ObjectImp *proto) : ObjectImp(proto)
{
    setInternalValue(String(""));
}

StringInstanceImp::StringInstanceImp(ObjectImp *proto, const UString &string) : ObjectImp(proto)
{
...
*/
// ECMA 15.5.4
StringPrototypeImp::StringPrototypeImp(ExecState * /*exec*/, ObjectPrototypeImp *objProto) : StringInstanceImp(objProto)
{
    Value protect(this);
    ...
    switch(id)
    {
        ...
        case Replace:
                ...
                do
                {
                    ...
                    if(!o1.isValid())
                    {
                        ...
                        for(int i = 0; (i = rstr.find(UString("$"), i)) != -1; i++)
                        {
                            ...
                            {
                                rstr = rstr.substr(0, i) + s.substr((*ovector)[2 * pos], (*ovector)[2 * pos + 1] - (*ovector)[2 * pos])
                                       + rstr.substr(i + 2);
                                i += (*ovector)[2 * pos + 1] - (*ovector)[2 * pos] - 1; // -1 offsets i++
                            }
                        }
                    }
                    else // 2nd arg is a function call. Spec from
                         // http://devedge.netscape.com/library/manuals/2000/javascript/1.5/reference/string.html#1194258
                    {
                        ...
                        for(unsigned int sub = 1; sub <= reg->subPatterns(); ++sub)
                            l.append(String(s.substr((*ovector)[2 * sub], (*ovector)[2 * sub + 1] - (*ovector)[2 * sub])));
            ...
            { // First arg is a string
                ...
                {
                    u3 = s.substr(0, pos) + a1.toString(exec) + s.substr(pos + len);
                    result = String(u3);
                }
            }
            break;
...
StringObjectImp::StringObjectImp(ExecState *exec, FunctionPrototypeImp *funcProto, StringPrototypeImp *stringProto) : InternalFunctionImp(funcProto)
{
    Value protect(this);
...
StringObjectFuncImp::StringObjectFuncImp(ExecState * /*exec*/, FunctionPrototypeImp *funcProto) : InternalFunctionImp(funcProto)
{
    Value protect(this);
