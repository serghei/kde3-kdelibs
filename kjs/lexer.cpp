...
int Lexer::lex()
{
    ...
    while(!done)
    {
        ...
        switch(state)
        {
            ...
            case InEscapeSequence:
                ...
                {
                    if(current >= '0' && current <= '3' && isOctalDigit(next1) && isOctalDigit(next2))
                    {
    ...
    if(state != Identifier && state != IdentifierOrKeyword && convertNextIdentifier)
        convertNextIdentifier = false;
    ...
    switch(state)
    {
        ...
        case IdentifierOrKeyword:
            if((token = Lookup::find(&mainTable, buffer16, pos16)) < 0)
            {
                case Identifier:
                    ...
                    if(convertNextIdentifier)
                    {
                        convertNextIdentifier = false;
#ifdef KJS_VERBOSE
                        UString debugstr(buffer16, pos16);
                        fprintf(stderr, "Anonymous function hack: eating identifier %s\n", debugstr.ascii());
#endif
                        token = FUNCEXPRIDENT;
                    }
                    else
            ...
            if(token == FUNCTION && (lastToken == '=' || lastToken == ',' || lastToken == '(' || lastToken == ':' || lastToken == RETURN))
...
{
    return (c == ' ' || c == '\t' || c == 0x0b || c == 0x0c || c == 0xa0);
}
...
int Lexer::matchPunctuator(unsigned short c1, unsigned short c2, unsigned short c3, unsigned short c4)
{
    if(c1 == '>' && c2 == '>' && c3 == '>' && c4 == '=')
    {
        shift(4);
...
unsigned short Lexer::convertOctal(unsigned short c1, unsigned short c2, unsigned short c3) const
{
...
UChar Lexer::convertUnicode(unsigned short c1, unsigned short c2, unsigned short c3, unsigned short c4)
{
