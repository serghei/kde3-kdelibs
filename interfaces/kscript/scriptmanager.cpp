...
KScriptManager::KScriptManager(QObject *parent, const char *name) : QObject(parent, name), KScriptClientInterface()
{
}
...
void KScriptManager::runScript(const QString &scriptName, QObject *context, const QVariant &arg)
{
    ...
    if(newScript)
    {
        ...
        if(!m_scriptCache[scriptName])
        {
            ...
            KScriptInterface *ksif =
                KParts::ComponentFactory::createInstanceFromQuery< KScriptInterface >("KScriptRunner/KScriptRunner", scriptType, this);
            if(ksif)
            {
