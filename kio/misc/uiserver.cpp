...
//-----------------------------------------------------------------------------
ListProgress::ListProgress(QWidget *parent, const char *name) : KListView(parent, name)
{
...
void ListProgress::readSettings()
{
    ...
    for(int i = 0; i < TB_MAX; i++)
    {
        ...
        if(m_lpcc[i].width == 0)
            m_lpcc[i].width = defaultColumnWidth[i];
...
int UIServer::newJob(QCString observerAppId, bool showProgress)
{
    ...
    for(; it.current(); ++it)
    {
        if(it.current()->itemBelow() == 0L)
        { // this will find the end of list
            break;
        }
    }
    ...
    connect(item, SIGNAL(jobCanceled(ProgressItem *)), SLOT(slotJobCanceled(ProgressItem *)));
...
void UIServer::slotUpdate()
{
    ...
    statusBar()->changeItem(i18n("Remaining Size", " Rem. Size: %1 ").arg(KIO::convertSize(iTotalSize)), ID_TOTAL_SIZE);
    statusBar()->changeItem(i18n("Remaining Time", " Rem. Time: %1 ").arg(KIO::convertSeconds(totalRemTime)), ID_TOTAL_TIME);
    statusBar()->changeItem(i18n(" %1/s ").arg(KIO::convertSize(iTotalSpeed)), ID_TOTAL_SPEED);
}
...
{
    kdDebug(7024) << "UIServer::openPassDlg: User= " << info.username << ", Msg= " << info.prompt << endl;
    KIO::AuthInfo inf(info);
    int result = KIO::PasswordDialog::getNameAndPassword(inf.username, inf.password, &inf.keepPassword, inf.prompt, inf.readOnly, inf.caption,
...
QByteArray UIServer::open_RenameDlg(int id, const QString &caption, const QString &src, const QString &dest, int mode, unsigned long sizeSrc,
                                    unsigned long sizeDest, unsigned long ctimeSrc, unsigned long ctimeDest, unsigned long mtimeSrc,
                                    unsigned long mtimeDest)
{
    return open_RenameDlg64(id, caption, src, dest, mode, sizeSrc, sizeDest, ctimeSrc, ctimeDest, mtimeSrc, mtimeDest);
}
