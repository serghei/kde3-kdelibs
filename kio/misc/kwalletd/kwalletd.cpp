...
KWalletD::KWalletD(const QCString &name) : KDEDModule(name), _failed(0)
{
    srand(time(0));
    ...
    connect(KApplication::dcopClient(), SIGNAL(applicationRemoved(const QCString &)), this, SLOT(slotAppUnregistered(const QCString &)));
...
void KWalletD::openAsynchronous(const QString &wallet, const QCString &returnObject, uint wId)
{
    ...
    if(!_enabled || !QRegExp("^[A-Za-z0-9]+[A-Za-z0-9\\s\\-_]*$").exactMatch(wallet))
    {
    ...
    if(rc == -1)
    {
        ...
        if((isPath && QFile::exists(wallet)) || (!isPath && KWallet::Backend::exists(wallet)))
        {
            ...
            if(0 != pwless || !b->isOpen())
            {
                ...
                if(appid.isEmpty())
                {
                    kpd->setPrompt(i18n("<qt>KDE has requested to open the wallet '<b>%1</b>'. Please enter the password for this wallet below.")
                                       .arg(QStyleSheet::escape(wallet)));
                }
                else
                {
                    kpd->setPrompt(i18n("<qt>The application '<b>%1</b>' has requested to open the wallet '<b>%2</b>'. Please enter the password for "
                                        "this wallet below.")
                                       .arg(QStyleSheet::escape(appid))
                                       .arg(QStyleSheet::escape(wallet)));
                }
                brandNew = false;
        ...
        else if(wallet == KWallet::Wallet::LocalWallet() || wallet == KWallet::Wallet::NetworkWallet())
        {
            ...
            {
                kpd->setPrompt(
                    i18n("KDE has requested to open the wallet. This is used to store sensitive data in a secure fashion. Please enter a password to "
                         "use with this wallet or click cancel to deny the application's request."));
            }
            else
            {
                kpd->setPrompt(
                    i18n("<qt>The application '<b>%1</b>' has requested to open the KDE wallet. This is used to store sensitive data in a secure "
                         "fashion. Please enter a password to use with this wallet or click cancel to deny the application's request.")
                        .arg(QStyleSheet::escape(appid)));
            }
            brandNew = true;
            kpd->setButtonOK(KGuiItem(i18n("&Open"), "fileopen"));
        ...
        else
        {
            ...
            if(appid.length() == 0)
            {
                kpd->setPrompt(i18n("<qt>KDE has requested to create a new wallet named '<b>%1</b>'. Please choose a password for this wallet, or "
                                    "cancel to deny the application's request.")
                                   .arg(QStyleSheet::escape(wallet)));
            }
            else
            {
                kpd->setPrompt(i18n("<qt>The application '<b>%1</b>' has requested to create a new wallet named '<b>%2</b>'. Please choose a "
                                    "password for this wallet, or cancel to deny the application's request.")
                                   .arg(QStyleSheet::escape(appid))
                                   .arg(QStyleSheet::escape(wallet)));
            }
            brandNew = true;
        ...
        while(!b->isOpen())
        {
            ...
            if(kpd->exec() == KDialog::Accepted)
            {
                ...
                if(!b->isOpen())
                {
                    kpd->setPrompt(i18n("<qt>Error opening the wallet '<b>%1</b>'. Please try again.<br>(Error code %2: %3)")
                                       .arg(QStyleSheet::escape(wallet))
                                       .arg(rc)
                                       .arg(KWallet::Backend::openRCToString(rc)));
                    kpd->clearPassword();
                }
            }
            else
...
bool KWalletD::isAuthorizedApp(const QCString &appid, const QString &wallet, WId w)
{
    ...
    if(!implicitAllow(wallet, thisApp))
    {
        ...
        {
            dialog->setLabel(i18n("<qt>The application '<b>%1</b>' has requested access to the open wallet '<b>%2</b>'.")
                                 .arg(QStyleSheet::escape(QString(appid)))
                                 .arg(QStyleSheet::escape(wallet)));
        }
...
void KWalletD::doTransactionChangePassword(const QCString &appid, const QString &wallet, uint wId)
{
    ...
    if(!it.current())
    {
        ...
        if(-1 == handle)
        {
            KMessageBox::sorryWId(wId, i18n("Unable to open wallet. The wallet must be opened in order to change the password."),
...
{
    for(QMap< QCString, QValueList< int > >::Iterator i = _handles.begin(); i != _handles.end(); ++i)
    {
...
void KWalletD::notifyFailures()
{
    if(!_showingFailureNotify)
    {
        ...
        KMessageBox::information(0, i18n("There have been repeated failed attempts to gain access to a wallet. An application may be misbehaving."),
                                 i18n("KDE Wallet Service"));
        _showingFailureNotify = false;
    }
}
...
void KWalletD::closeAllWallets()
{
    ...
    for(QMap< QString, QCString >::Iterator it = _passwords.begin(); it != _passwords.end(); ++it)
    {
        it.data().fill(0);
    }
    _passwords.clear();
