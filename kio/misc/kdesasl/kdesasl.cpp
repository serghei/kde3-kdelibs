...
KDESasl::KDESasl(const QString &aUser, const QString &aPass, const QString &aProtocol)
{
    mProtocol = aProtocol;
    mUser = aUser;
    mPass = aPass;
...
{
    if(aMethods.contains("DIGEST-MD5"))
        mMethod = "DIGEST-MD5";
    else if(aMethods.contains("CRAM-MD5"))
        mMethod = "CRAM-MD5";
    else if(aMethods.contains("PLAIN"))
        mMethod = "PLAIN";
    else if(aMethods.contains("LOGIN"))
        mMethod = "LOGIN";
    else
        mMethod = QCString();
    return mMethod;
...
QByteArray KDESasl::getCramMd5Response(const QByteArray &aChallenge)
{
    ...
    for(i = 0; i < 64; i++)
        XorOpad[i] = secret[i] ^ 0x5C;
    QByteArray XorIpad(64);
    for(i = 0; i < 64; i++)
        XorIpad[i] = secret[i] ^ 0x36;
    KMD5 md5;
    md5.update(XorIpad);
...
QByteArray KDESasl::getDigestMd5Response(const QByteArray &aChallenge)
{
    ...
    while(a < aChallenge.size())
    {
        ...
        if(qstrnicmp(aChallenge.data() + a, "realm=", 6) == 0)
            realm = str;
        else if(qstrnicmp(aChallenge.data() + a, "nonce=", 6) == 0)
            nonce = str;
        else if(qstrnicmp(aChallenge.data() + a, "qop=", 4) == 0)
            qop = str;
...
QByteArray KDESasl::getBinaryResponse(const QByteArray &aChallenge, bool aBase64)
{
    ...
    if(qstricmp(mMethod, "PLAIN") == 0)
        return getPlainResponse();
    if(qstricmp(mMethod, "LOGIN") == 0)
    ...
    //    return
    //    getDigestMd5Response(QCString("realm=\"elwood.innosoft.com\",nonce=\"OA6MG9tEQGm2hh\",qop=\"auth\",algorithm=md5-sess,charset=utf-8"));
    return QByteArray();
}
