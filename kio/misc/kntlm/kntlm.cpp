...
QString KNTLM::getString(const QByteArray &buf, const SecBuf &secbuf, bool unicode)
{
    ...
    if(offset > buf.size() || offset + len > buf.size())
        return QString::null;
...
bool KNTLM::getAuth(QByteArray &auth, const QByteArray &challenge, const QString &user, const QString &password, const QString &domain,
                    const QString &workstation, bool forceNTLM, bool forceNTLMv2)
{
...
QByteArray KNTLM::getNTLMv2Response(const QString &target, const QString &user, const QString &password, const QByteArray &targetInformation,
