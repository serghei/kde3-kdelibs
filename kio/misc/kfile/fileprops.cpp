...
FileProps::FileProps(const QString &path, const QStringList &suppliedGroups) : m_dirty(false)
{
...
QString FileProps::getValue(const QString &group, const QString &key) const
{
    KFileMetaInfoGroup g = m_info->group(group);
    return FileProps::createKeyValue(g, key);
}

bool FileProps::setValue(const QString &group, const QString &key, const QString &value)
{
    KFileMetaInfoGroup g = m_info->group(group);
    bool wasAdded = false;
    if(!g.isValid())
...
QString FileProps::createKeyValue(const KFileMetaInfoGroup &g, const QString &key)
{
    static const int MAX_SPACE = 25;
    KFileMetaInfoItem item = g.item(key);

    QString result("%1");
    result = result.arg((item.isValid() ? item.translatedKey() : key) + ":", -MAX_SPACE);
...
// static
QStringList FileProps::createKeyValueList(const KFileMetaInfoGroup &g, const QStringList &keys)
{
    QStringList result;
...
// kfile --mimetype --listsupported --listavailable --listpreferred --listwritable --getValue "key" --setValue "key=value" --allValues
// --preferredValues --dialog --quiet file [file...]
// "key" may be a list of keys, separated by commas
static KCmdLineOptions options[] = {{"m", 0, 0}, // short option for --mimetype
                                    {"nomimetype", I18N_NOOP("Do not print the mimetype of the given file(s)"), 0},

                                    {"ls", 0, 0}, // short option for --listsupported
                                    {"listsupported <mimetype>", I18N_NOOP("List all supported metadata keys of the given file(s). "
                                                                           "If mimetype is not specified, the mimetype of the given "
                                                                           "files is used."),
                                     "file"},

                                    {"lp", 0, 0}, // short option for --listpreferred
                                    {"listpreferred <mimetype>", I18N_NOOP("List all preferred metadata keys of the given file(s). "
                                                                           "If mimetype is not specified, the mimetype of the given "
                                                                           "files is used."),
                                     "file"},

                                    {"la", 0, 0}, // short option for --listavailable
                                    {"listavailable", I18N_NOOP("List all metadata keys which have a value in the given "
                                                                "file(s)."),
                                     0},

                                    {"sm", 0, 0}, // short option for --supportedMimetypes
                                    {"supportedMimetypes", I18N_NOOP("Prints all mimetypes for which metadata support is "
                                                                     "available."),
                                     0},

                                    {"q", 0, 0}, // short option for --quiet
                                    {"quiet", I18N_NOOP("Do not print a warning when more than one file was given "
                                                        "and they do not all have the same mimetype."),
                                     0},

                                    {"av", 0, 0}, // short option for --allValues
                                    {"allValues", I18N_NOOP("Prints all metadata values, available in the given "
                                                            "file(s)."),
                                     0},

                                    {"pv", 0, 0}, // short option for --preferredValues
                                    {"preferredValues", I18N_NOOP("Prints the preferred metadata values, available in the "
                                                                  "given file(s)."),
                                     0},

                                    {"dialog", I18N_NOOP("Opens a KDE properties dialog to allow viewing and "
                                                         "modifying of metadata of the given file(s)"),
                                     0},

                                    {"getValue <key>", I18N_NOOP("Prints the value for 'key' of the given file(s). 'key' "
                                                                 "may also be a comma-separated list of keys"),
                                     0},

                                    {"setValue <key=value>", I18N_NOOP("Attempts to set the value 'value' for the metadata key "
                                                                       "'key' for the given file(s)"),
                                     0},

                                    {"!groups <arguments>", I18N_NOOP("The group to get values from or set values to"), 0},

                                    {"+[files]", I18N_NOOP("The file (or a number of files) to operate on."), 0},
                                    KCmdLineLastOption};
...
static void printSupportedMimeTypes()
{
    ...
    {
        cout << i18n("No support for metadata extraction found.").local8Bit() << endl;
        return;
    }
...
static KFileItemList *fileItemList(const KCmdLineArgs *args)
{
    ...
    for(int i = 0; i < args->count(); i++)
        items->append(new KFileItem(KFileItem::Unknown, KFileItem::Unknown, args->url(i)));
    return items;
}
...
static void printMimeTypes(const KCmdLineArgs *args)
{
    for(int i = 0; i < args->count(); i++)
    {
        ...
        cout << args->arg(i) << ": " << mt->comment().local8Bit() << " (" << mt->name().local8Bit() << ")" << endl;
    }
}
...
{
    // kfile --mimetype --supportedMimetypes --listsupported --listavailable --listpreferred --listwritable --getValue "key" --setValue "key=value"
    // --allValues --preferredValues --dialog --quiet file [file...]
    // "key" may be a list of keys, separated by commas
    ...
    for(; (props = it.current()); ++it)
    {
        ...
        if(args->isSet("setValue"))
        {
            ...
            QStringList groups = props->userSuppliedGroups() ? props->groupsToUse() : props->supportedGroups();
...
{
    KAboutData about("kfile", I18N_NOOP("kfile"), KFILEVERSION, I18N_NOOP("A commandline tool to read and modify metadata of files."),
                     KAboutData::License_LGPL, "(c) 2002, Carsten Pfeiffer", 0 /*text*/, "http://devel-home.kde.org/~pfeiffer/", "pfeiffer@kde.org");

    about.addAuthor("Carsten Pfeiffer", 0, "pfeiffer@kde.org", "http://devel-home.kde.org/~pfeiffer/");
    ...
    for(int i = 0; i < files; i++)
    {
        ...
        else
        {
            if(!quiet)
            {
                cerr << args->arg(i) << ": " << i18n("Cannot determine metadata").local8Bit() << endl;
            }
            delete props;
        }
    }
