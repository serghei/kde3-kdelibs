...
public:
    ...
    {
        return String(m_address.ipAddress().toString());
    }
...
private:
    Address(const QString &host, bool numeric)
    {
        ...
        KNetwork::KResolverResults addresses = KNetwork::KResolver::resolve(host, QString::null, flags, KNetwork::KResolver::IPv4Family);
...
struct IsResolvable : public Function
{
    virtual Value call(ExecState *exec, Object &, const List &args)
    {
        ...
        try
        {
            ::Address::resolve(args[0].toString(exec));
        }
...
struct DNSResolve : public Function
{
    virtual Value call(ExecState *exec, Object &, const List &args)
    {
        ...
        try
        {
            return String(Address::resolve(args[0].toString(exec)));
        }
        catch(const Address::Error &)
        {
            return Undefined();
...
struct MyIpAddress : public Function
{
    virtual Value call(ExecState *, Object &, const List &args)
    {
        ...
        try
        {
            return String(Address::resolve(hostname));
        }
        catch(const Address::Error &)
        {
            return Undefined();
...
struct DNSDomainLevels : public Function
{
    virtual Value call(ExecState *exec, Object &, const List &args)
    {
        ...
        if(host.isNull())
            return Number(0);
        return Number(std::count(host.data(), host.data() + host.size(), '.'));
    }
};
...
struct DateRange : public Function
{
    virtual Value call(ExecState *exec, Object &, const List &args)
    {
        if(args.size() < 1 || args.size() > 7)
            return Undefined();
        static const char *const months[] = {"jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "nov", "dec", 0};
        ...
        for(int i = 0; i < args.size(); ++i)
        {
            ...
            if(args[i].isA(NumberType))
                ...
                value = findString(args[i].toString(exec), months);
            if(value >= 0)
        ...
        if(values.size() == 6)
            return checkRange((now->tm_year + 1900) * 372 + now->tm_mon * 31 + now->tm_mday, values[2] * 372 + values[1] * 31 + values[0],
                              values[5] * 372 + values[4] * 31 + values[3]);

        // day1, month1, day2, month2
        else if(values.size() == 4 && values[1] < 12 && values[3] < 12)
            return checkRange(now->tm_mon * 31 + now->tm_mday, values[1] * 31 + values[0], values[3] * 31 + values[2]);

        // month1, year1, month2, year2
        else if(values.size() == 4)
            return checkRange((now->tm_year + 1900) * 12 + now->tm_mon, values[1] * 12 + values[0], values[3] * 12 + values[2]);
        ...
        // day1, day2
        else if(values.size() == 2 && args[0].isA(NumberType) && args[1].isA(NumberType))
...
struct TimeRange : public Function
{
    virtual Value call(ExecState *exec, Object &, const List &args)
    {
        ...
        if(values.size() == 6)
            return checkRange(now->tm_hour * 3600 + now->tm_min * 60 + now->tm_sec, values[0] * 3600 + values[1] * 60 + values[2],
                              values[3] * 3600 + values[4] * 60 + values[5]);

        // hour1, min1, hour2, min2
        else if(values.size() == 4)
            return checkRange(now->tm_hour * 60 + now->tm_min, values[0] * 60 + values[1], values[2] * 60 + values[3]);
...
{
    global.put(exec, "isPlainHostName", Object(new IsPlainHostName));
    global.put(exec, "dnsDomainIs", Object(new DNSDomainIs));
    global.put(exec, "localHostOrDomainIs", Object(new LocalHostOrDomainIs));
    global.put(exec, "isResolvable", Object(new IsResolvable));
    global.put(exec, "isInNet", Object(new IsInNet));
    global.put(exec, "dnsResolve", Object(new DNSResolve));
    global.put(exec, "myIpAddress", Object(new MyIpAddress));
    global.put(exec, "dnsDomainLevels", Object(new DNSDomainLevels));
    global.put(exec, "shExpMatch", Object(new ShExpMatch));
    global.put(exec, "weekdayRange", Object(new WeekdayRange));
    global.put(exec, "dateRange", Object(new DateRange));
    global.put(exec, "timeRange", Object(new TimeRange));
}
}
