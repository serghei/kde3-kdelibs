...
namespace KPAC {
Discovery::Discovery(QObject *parent) : Downloader(parent), m_helper(new KProcIO)
{
...
bool Discovery::checkDomain() const
{
    ...
    int len = res_query(m_hostname.local8Bit(), C_IN, T_SOA, response.buf, sizeof(response.buf));
...
void Discovery::failed()
{
    ...
    if((firstQuery && !initHostName()) || (!firstQuery && !checkDomain()))
    {
        emit result(false);
        return;
    }
