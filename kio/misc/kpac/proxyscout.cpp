...
namespace KPAC {
ProxyScout::QueuedRequest::QueuedRequest(const KURL &u) : transaction(kapp->dcopClient()->beginTransaction()), url(u)
{
}

ProxyScout::ProxyScout(const QCString &name)
    : KDEDModule(name), m_instance(new KInstance("proxyscout")), m_downloader(0), m_script(0), m_suspendTime(0)
{
}
