...
int init_socket()
{
    ...
    if(setsockopt(sock, SOL_SOCKET, SO_BROADCAST, &bcast, sizeof(bcast)) == -1
       || setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, &bcast, sizeof(bcast)) == -1 || bind(sock, (struct sockaddr *)&addr, sizeof(addr)) == -1)
        exit(1);
...
uint32_t send_request(int sock)
{
    ...
    hostname[0] = '\0';
    hostname[255] = '\0';
    if(gethostname(hostname, 255) == -1 || strlen(hostname) == 0 || (hent = gethostbyname(hostname)) == NULL)
        exit(1);
