...
static KCmdLineOptions options[] = {{"+file", "File name", 0},
...
void printKeyValues(KFileMetaInfo &info)
{
    ...
    for(it = l.begin(); it != l.end(); ++it)
    {
        ...
        {
            kdDebug() << item.key() << "(" << item.translatedKey() << ") -> " << item.string() << endl;
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    if(!thumbitem.isValid())
        kdDebug() << "no thumbnail\n";
    else
