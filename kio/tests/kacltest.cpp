...
template < typename T > static bool check(const QString &txt, T a, T b)
{
    if(a == b)
    {
        kdDebug() << txt << " : checking '" << a << "' against expected value '" << b << "'... "
...
static const QString s_testACLEffective = QString::fromLatin1("user::rwx\nuser:bin:rwx #effective:r-x\ngroup::rw- #effective:r--\n")
                                          ...
                                          + s_group2 + QString::fromLatin1(":r--\n") + QString::fromLatin1("mask::r-x\nother::r--\n");

KACLTest::KACLTest() : m_acl(s_testACL)
{
}
