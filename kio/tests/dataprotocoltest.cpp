...
public:
    ...
    void sendMetaData()
    {
        ...
        for(; it != end; ++it)
        {
            cout << endl << "Metadata[\"" << it.key() << "\"] was expected but not defined";
            total++;
        }
    }
...
private:
     ...
     */
    void testStrings(const QString &prefix, const QString &templat, const QString &s, bool /*casesensitive*/ = true)
    {
        if(templat == s)
            passed++;
        else
        {
            cout << endl << prefix << "expected \"" << templat << "\", found \"" << s << "\"";
            failure = true;
        } /*end if*/
        total++;
    }
...
public:
    ...
    void endTestcase()
    {
        ...
        cout << "(" << passed << " of " << total << ") " << (failed ? "failed" : "passed") << endl;
    }
...
const struct
{
    ...
    // whitespace test III
    {"arbitrary keys, reserved names as keys, whitespace test III",
     0,
     {{"base64", "nospace"}, {"key", "onespaceinner"}, {"key2", "onespaceouter"}, {"charset", "utf8"}, {"<<empty>>", ""}, {0, 0}},
    ...
    {"string literal with escaped chars, testing delimiters within "
     ...
     0,
     {{"fortune-cookie",
       "Master Leep say: \"Rabbit is humble, "
       "Rabbit is gentle; follow the Rabbit\""},
      {0, 0}},
...
int main(int /*argc*/, char * /*argv*/ [])
{
    ...
    for(uint i = 0; i < sizeof testcases / sizeof testcases[0]; i++)
    {
        ...
        kio_data.setExpectedMimeType(testcases[i].exp_mime_type != 0 ? testcases[i].exp_mime_type : "text/plain");
