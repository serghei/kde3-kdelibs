...
TestService::TestService(const QString &exec)
{
    ...
    connect(kapp->dcopClient(), SIGNAL(applicationRegistered(const QCString &)), this, SLOT(newApp(const QCString &)));
    connect(kapp->dcopClient(), SIGNAL(applicationRemoved(const QCString &)), this, SLOT(endApp(const QCString &)));
    connect(&proc, SIGNAL(processExited(KProcess *)), this, SLOT(appExit()));
...
int main(int argc, char *argv[])
{
    ...
    {
        if(((*it)->DCOPServiceType() == KService::DCOP_None) && !(*it)->desktopEntryPath().startsWith("SuSE") && (*it)->hasServiceType("Application"))
        {
            if((*it)->exec().startsWith((*it)->desktopEntryName()))
            {
                ...
                qWarning("%s %s", (*it)->desktopEntryPath().latin1(), result.latin1());
            }
        }
    }
