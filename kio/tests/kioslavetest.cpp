...
KioslaveTest::KioslaveTest(QString src, QString dest, uint op, uint pr) : KMainWindow(0, "")
{
...
void KioslaveTest::startJob()
{
    ...
    {
        QMessageBox::critical(this, "Kioslave Error Message", "Destination URL is malformed");
        return;
    }
    ...
    switch(selectedOperation)
    {
        case List:
            ...
            connect(myJob, SIGNAL(entries(KIO::Job *, const KIO::UDSEntryList &)), SLOT(slotEntries(KIO::Job *, const KIO::UDSEntryList &)));
            break;
        ...
        case Get:
            ...
            connect(myJob, SIGNAL(data(KIO::Job *, const QByteArray &)), SLOT(slotData(KIO::Job *, const QByteArray &)));
            break;

        case Put:
            putBuffer = 0;
            myJob = KIO::put(src, -1, true, false);
            connect(myJob, SIGNAL(dataReq(KIO::Job *, QByteArray &)), SLOT(slotDataReq(KIO::Job *, QByteArray &)));
            break;
    ...
    connect(job, SIGNAL(canceled(KIO::Job *)), SLOT(slotResult(KIO::Job *)));
...
{
    const char *fileDataArray[] = {"Hello world\n", "This is a test file\n", "You can safely delete it.\n", "BIG\n", 0};
...
static KCmdLineOptions options[] = {
    {"s", 0, 0},       {"src <src>", "Source URL", ""},
    {"d", 0, 0},       {"dest <dest>", "Destination URL", ""},
    {"o", 0, 0},       {"operation <operation>", "Operation (list,listrecursive,stat,get,put,copy,move,del,shred,mkdir)", "copy"},
    {"p", 0, 0},       {"progress <progress>", "Progress Type (none,default,status)", "default"},
    KCmdLineLastOption};
...
int main(int argc, char **argv)
{
    ...
    else if(tmps == "mkdir")
    {
        ...
        KCmdLineArgs::usage("unknown operation");
    ...
    else if(tmps == "status")
    {
        ...
        KCmdLineArgs::usage("unknown progress mode");
