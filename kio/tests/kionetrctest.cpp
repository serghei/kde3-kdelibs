...
void output(const KURL &u)
{
    ...
    kdDebug() << "Type: " << l.type << endl << "Machine: " << l.machine << endl << "Login: " << l.login << endl << "Password: " << l.password << endl;

    QMap< QString, QStringList >::ConstIterator it = l.macdef.begin();
    for(; it != l.macdef.end(); ++it)
    {
        kdDebug() << "Macro: " << it.key() << "= " << it.data().join("   ") << endl;
    }
}
...
int main(int argc, char **argv)
{
    ...
    KCmdLineOptions options[] = {{"+command", "[url1,url2 ,...]", 0}, KCmdLineLastOption};
