...
void recursive_print(const KArchiveDirectory *dir, const QString &path)
{
    ...
    for(; it != l.end(); ++it)
    {
        ...
        printf("mode=%07o %s %s size: %d pos: %d %s%s isdir=%d%s", entry->permissions(), entry->user().latin1(), entry->group().latin1(),
               entry->isDirectory() ? 0 : ((KArchiveFile *)entry)->size(), entry->isDirectory() ? 0 : ((KArchiveFile *)entry)->position(),
               path.latin1(), (*it).latin1(), entry->isDirectory(), !entry->symlink() ? "" : QString(" symlink: %1").arg(entry->symlink()).latin1());
...
void recursive_transfer(const KArchiveDirectory *dir, const QString &path, KZip *zip)
{
    ...
    for(; it != l.end(); ++it)
    {
        ...
        if(e->isFile())
        {
            ...
            {
                zip->writeFile(path + e->name().latin1(), "holgi", "holgrp", arr.size(), f->data());
            }
            else
                zip->writeSymLink(path + e->name(), e->symlink(), "leo", "leo", 0120777, 1000000000l, 1000000000l, 1000000000l);
        }
        else if(e->isDirectory())
        {
            recursive_transfer((KArchiveDirectory *)e, path + e->name() + "/", zip);
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    else if(command == "readwrite")
    {
        ...
        zip.writeSymLink("a_link", "mydir/test3", "leo", "leo", 0120777, 1000000000l, 1000000000l, 1000000000l);
