...
void KDirWatchTest::VERIFY_DIRTY(const QString &alert)
{
    ...
    VERIFY(m_s[sigCreated] == m_signals[sigCreated] && m_s[sigDeleted] == m_signals[sigDeleted] && m_s[sigDirty] + 1 == m_signals[sigDirty]
           && m_lastSignal == alert);
}
...
void KDirWatchTest::VERIFY_CREATED(const QString &alert)
{
    ...
    VERIFY(m_s[sigDirty] == m_signals[sigDirty] && m_s[sigDeleted] == m_signals[sigDeleted] && m_s[sigCreated] + 1 == m_signals[sigCreated]
           && m_lastSignal == alert);
}
...
void KDirWatchTest::VERIFY_DELETED(const QString &alert)
{
    ...
    VERIFY(m_s[sigDirty] == m_signals[sigDirty] && m_s[sigCreated] == m_signals[sigCreated] && m_s[sigDeleted] + 1 == m_signals[sigDeleted]
           && m_lastSignal == alert);
}
