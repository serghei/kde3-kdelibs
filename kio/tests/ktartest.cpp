...
void recursive_print(const KTarDirectory *dir, const QString &path)
{
    ...
    for(; it != l.end(); ++it)
    {
        ...
        printf("mode=%07o %s %s %s%s %d isdir=%d\n", entry->permissions(), entry->user().latin1(), entry->group().latin1(), path.latin1(),
               (*it).latin1(), entry->isFile() ? static_cast< const KArchiveFile * >(entry)->size() : 0, entry->isDirectory());
