...
*
* $Id: kimageio.cpp 344678 2004-09-07 12:51:11Z staniek $
*
...
KImageIOFormat::KImageIOFormat(const QString &path) : KSycocaEntry(path)
{
    bLibLoaded = false;
    mReadFunc = 0;
    mWriteFunc = 0;
...
KImageIOFormat::KImageIOFormat(QDataStream &_str, int offset) : KSycocaEntry(_str, offset)
{
    bLibLoaded = false;
    mReadFunc = 0;
    mWriteFunc = 0;
...
void KImageIOFormat::load(QDataStream &_str)
{
    Q_INT8 iRead, iWrite;
...
void KImageIOFormat::save(QDataStream &_str)
{
    KSycocaEntry::save(_str);
    Q_INT8 iRead = bRead ? 1 : 0;
    Q_INT8 iWrite = bWrite ? 1 : 0;

    _str << mType << mHeader << mFlags << iRead << iWrite << mSuffices << mMimetype << mLib << mPattern << rPaths;
}

void KImageIOFormat::callLibFunc(bool read, QImageIO *iio)
{
    if(!bLibLoaded)
    {
        if(mLib.isEmpty())
...
KImageIOFactory::KImageIOFactory() : KSycocaFactory(KST_KImageIO)
{
    ...
    if(m_str)
    {
        ...
        if(!formatList)
        {
            ...
            // Add rPaths.
            for(QStringList::Iterator it = rPath.begin(); it != rPath.end(); ++it)
            {
            } // lt_dladdsearchdir( QFile::encodeName(*it) ); FIXME check if this should be implemented
        }
        load();
    }
    else if(KSycoca::self()->isBuilding())
    {
        // Build database
        if(!formatList)
        {
...
QString KImageIOFactory::createPattern(KImageIO::Mode _mode)
{
    QStringList patterns;
    QString allPatterns;
    QString wildCard("*.");
    QString separator("|");
    for(KImageIOFormatList::ConstIterator it = formatList->begin(); it != formatList->end(); ++it)
    {
        KImageIOFormat *format = (*it);
        if(((_mode == KImageIO::Reading) && format->bRead) || ((_mode == KImageIO::Writing) && format->bWrite))
        {
            QString pattern;
            QStringList suffices = format->mSuffices;
            for(QStringList::ConstIterator it = suffices.begin(); it != suffices.end(); ++it)
            {
                if(!pattern.isEmpty())
                    pattern += " ";
...
void KImageIOFactory::readImage(QImageIO *iio)
{
...
void KImageIOFactory::writeImage(QImageIO *iio)
{
    (void)self(); // Make sure we exist
    const char *fm = iio->format();
    if(!fm)
    ...
    for(KSycocaEntry::List::Iterator it = list.begin(); it != list.end(); ++it)
    {
        ...
        if(!format->mHeader.isEmpty() && !format->mLib.isEmpty())
        {
            ...
            if(format->bWrite)
                ...
                writeFunc = 0;
            QImageIO::defineIOHandler(format->mType.ascii(), format->mHeader.ascii(), format->mFlags.ascii(), readFunc, writeFunc);
        }
...
KSycocaEntry *KImageIOFactory::createEntry(int offset)
{
    KImageIOFormat *format = 0;
    KSycocaType type;
...
QString KImageIO::pattern(Mode _mode)
{
    if(_mode == Reading)
...
QStringList KImageIO::types(Mode _mode)
{
    ...
    if(formatList)
    {
        ...
        {
            KImageIOFormat *format = (*it);
            if(((_mode == Reading) && format->bRead) || ((_mode == Writing) && format->bWrite))
