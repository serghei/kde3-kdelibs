...
#ifdef USE_POSIX_ACL
    KACLPrivate(acl_t acl) : m_acl(acl)
    {
        init();
    }
    ~KACLPrivate()
...
KACL::KACL(const QString &aclString) : d(new KACLPrivate)
{
    setACL(aclString);
}
...
KACL::KACL() : d(new KACLPrivate)
{
}

KACL::KACL(const KACL &rhs) : d(new KACLPrivate)
{
    setACL(rhs.asString());
}
