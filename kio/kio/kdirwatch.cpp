...
void KDirWatchPrivate::dnotify_sigio_handler(int sig, siginfo_t *si, void *p)
{
    ...
    {
        if((old_sigio_act.sa_handler != SIG_DFL) && (old_sigio_act.sa_handler != SIG_IGN))
...
KDirWatchPrivate::KDirWatchPrivate() : rescan_timer(0, "KDirWatchPrivate::rescan_timer")
{
...
void KDirWatchPrivate::slotActivated()
{
    ...
    while(pending > 0)
    {
        ...
        while(pending > 0)
        {
            ...
            for(EntryMap::Iterator it = m_mapEntries.begin(); it != m_mapEntries.end(); ++it)
            {
                Entry *e = &(*it);
                if(e->wd == event->wd)
                {
...
{
    for(QPtrListIterator< Entry > sub_entry(m_entries); sub_entry.current(); ++sub_entry)
    {
...
bool KDirWatchPrivate::useFAM(Entry *e)
{
    ...
    if(e->isDir)
    {
        ...
        {
            int res = FAMMonitorDirectory(&fc, QFile::encodeName(e->path), &(e->fr), e);
            if(res < 0)
            {
            ...
            kdDebug(7001) << " Setup FAM (Req " << FAMREQUEST_GETREQNUM(&(e->fr)) << ") for " << e->path << endl;
        }
    }
    else
    {
        ...
        {
            int res = FAMMonitorFile(&fc, QFile::encodeName(e->path), &(e->fr), e);
            if(res < 0)
            {
                e->m_mode = UnknownMode;
                use_fam = false;
...
bool KDirWatchPrivate::useDNotify(Entry *e)
{
    ...
    if(e->isDir)
    {
        if(e->m_status == Normal)
        {
            ...
            if(fcntl(fd, F_SETSIG, dnotify_signal) < 0 || fcntl(fd, F_NOTIFY, mask) < 0)
            {

                kdDebug(7001) << "Not using Linux Directory Notifications." << endl;
                supports_dnotify = false;
                ::close(fd);
                e->m_mode = UnknownMode;
                return false;
            ...
            kdDebug(7001) << " Setup DNotify (fd " << fd << ") for " << e->path << endl;
        }
        else
        { // NotExisting
            addEntry(0, QDir::cleanDirPath(e->path + "/.."), e, true);
...
bool KDirWatchPrivate::useINotify(Entry *e)
{
    ...
    if((e->wd = inotify_add_watch(m_inotify_fd, QFile::encodeName(e->path), mask)) > 0)
        return true;
...
bool KDirWatchPrivate::useStat(Entry *e)
{
    ...
    kdDebug(7001) << " Setup Stat (freq " << e->freq << ") for " << e->path << endl;
 ...
 */
void KDirWatchPrivate::addEntry(KDirWatch *instance, const QString &_path, Entry *sub_entry, bool isDir)
{
    QString path = _path;
    ...
    if(it != m_mapEntries.end())
    {
        if(sub_entry)
        {
            ...
            kdDebug(7001) << "Added already watched Entry " << path << " (for " << sub_entry->path << ")" << endl;
        ...
        else
        {
            ...
            kdDebug(7001) << "Added already watched Entry " << path << " (now " << (*it).clients() << " clients)"
    ...
    kdDebug(7001) << "Added " << (e->isDir ? "Dir " : "File ") << path << (e->m_status == NonExistent ? " NotExisting" : "")
                  << (sub_entry ? QString(" for %1").arg(sub_entry->path) : QString(""))
                  << (instance ? QString(" [%1]").arg(instance->name()) : QString("")) << endl;
...
void KDirWatchPrivate::removeEntry(KDirWatch *instance, const QString &_path, Entry *sub_entry)
{
    ...
    if(e->m_mode == FAMMode)
    {
        if(e->m_status == Normal)
        {
            ...
            kdDebug(7001) << "Cancelled FAM (Req " << FAMREQUEST_GETREQNUM(&(e->fr)) << ") for " << e->path << endl;
        }
        else
        {
            if(e->isDir)
    ...
    if(e->m_mode == INotifyMode)
    {
        if(e->m_status == Normal)
        {
            ...
            kdDebug(7001) << "Cancelled INotify (fd " << m_inotify_fd << ", " << e->wd << ") for " << e->path << endl;
        }
        else
        {
            if(e->isDir)
        ...
        { // isDir
            ...
            if(e->m_status == Normal)
            {
                if(e->dn_fd)
                {
                    ...
                    kdDebug(7001) << "Cancelled DNotify (fd " << e->dn_fd << ") for " << e->path << endl;
                    e->dn_fd = 0;
                }
            }
            else
    ...
    kdDebug(7001) << "Removed " << (e->isDir ? "Dir " : "File ") << e->path << (sub_entry ? QString(" for %1").arg(sub_entry->path) : QString(""))
                  << (instance ? QString(" [%1]").arg(instance->name()) : QString("")) << endl;
...
bool KDirWatchPrivate::stopEntryScan(KDirWatch *instance, Entry *e)
{
    ...
    kdDebug(7001) << instance->name() << " stopped scanning " << e->path << " (now " << stillWatching << " watchers)" << endl;
...
bool KDirWatchPrivate::restartEntryScan(KDirWatch *instance, Entry *e, bool notify)
{
    ...
    kdDebug(7001) << (instance ? instance->name() : "all") << " restarted scanning " << e->path << " (now " << wasWatching + newWatching
                  << " watchers)" << endl;
...
int KDirWatchPrivate::scanEntry(Entry *e)
{
    ...
    if(exists)
    {
        ...
        if((e->m_ctime != invalid_ctime) && ((stat_buf.st_ctime != e->m_ctime) || (stat_buf.st_nlink != (nlink_t)e->m_nlink)))
        {
            e->m_ctime = stat_buf.st_ctime;
            e->m_nlink = stat_buf.st_nlink;
            return Changed;
        ...
        switch(fe->code)
        {
            case FAMDeleted:
                ...
                if(!QDir::isRelativePath(fe->filename))
                {
                    ...
                    kdDebug(7001) << "Cancelled FAMReq " << FAMREQUEST_GETREQNUM(&(e->fr)) << " for " << e->path << endl;
            ...
            case FAMCreated:
            {
                ...
                Entry *sub_entry = e->m_entries.first();
                for(; sub_entry; sub_entry = e->m_entries.next())
                    if(sub_entry->path == e->path + "/" + fe->filename)
                        break;
                if(sub_entry && sub_entry->isDir)
                {
                    QString path = e->path;
...
void KDirWatchPrivate::statistics()
{
    ...
    else
    {
        ...
        {
            Entry *e = &(*it);
            kdDebug(7001) << "  " << e->path << " (" << ((e->m_status == Normal) ? "" : "Nonexistent ") << (e->isDir ? "Dir" : "File") << ", using "
                          << ((e->m_mode == FAMMode) ? "FAM" : (e->m_mode == INotifyMode)
                                                                   ? "INotify"
            ...
            for(; c; c = e->m_clients.next())
            {
                ...
                kdDebug(7001) << "    by " << c->instance->name() << " (" << c->count << " times)" << pending << endl;
            }
...
KDirWatch::KDirWatch(QObject *parent, const char *name) : QObject(parent, name)
{
    if(!name)
    {
...
void KDirWatch::addDir(const QString &_path, bool watchFiles, bool recursive)
{
