...
KServiceGroup::KServiceGroup(const QString &name) : KSycocaEntry(name), m_childCount(-1)
{
...
KServiceGroup::KServiceGroup(const QString &configFile, const QString &_relpath) : KSycocaEntry(_relpath), m_childCount(-1)
{
    d = new KServiceGroup::Private;
    m_bDeleted = false;
    m_bDeep = false;
...
KServiceGroup::KServiceGroup(QDataStream &_str, int offset, bool deep) : KSycocaEntry(_str, offset)
{
    d = new KServiceGroup::Private;
    m_bDeep = deep;
    load(_str);
...
int KServiceGroup::childCount()
{
    if(m_childCount == -1)
    {
        ...
        for(List::ConstIterator it = m_serviceList.begin(); it != m_serviceList.end(); it++)
        {
            KSycocaEntry *p = (*it);
...
void KServiceGroup::load(QDataStream &s)
{
    ...
    Q_INT8 _inlineAlias;
    Q_INT8 _allowInline;
    s >> m_strCaption >> m_strIcon >> m_strComment >> groupList >> m_strBaseGroupName >> m_childCount >> noDisplay >> d->suppressGenericNames
        >> d->directoryEntryPath >> d->sortOrder >> _showEmptyMenu >> inlineHeader >> _inlineAlias >> _allowInline;
...
KServiceGroup::List KServiceGroup::entries(bool sort, bool excludeNoDisplay, bool allowSeparators, bool sortByGenericName)
{
    ...
    {
        KSycocaEntry *p = (*it);
        bool noDisplay = p->isType(KST_KServiceGroup) ? static_cast< KServiceGroup * >(p)->noDisplay() : static_cast< KService * >(p)->noDisplay();
...
void KServiceGroup::parseAttribute(const QString &item, bool &showEmptyMenu, bool &showInline, bool &showInlineHeader, bool &showInlineAlias,
                                   int &inlineValue)
{
...
KServiceGroup::Ptr KServiceGroup::baseGroup(const QString &_baseGroupName)
{
...
KServiceSeparator::KServiceSeparator() : KSycocaEntry("separator")
{
}
