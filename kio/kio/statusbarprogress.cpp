...
StatusbarProgress::StatusbarProgress(QWidget *parent, bool button) : ProgressBase(parent)
{
...
{
    if(speed == 0)
    { // spped is measured in bytes-per-second
...
{
    if(!m_pJob)
    { // don't react when there isn't any job doing IO
        return true;
    }
