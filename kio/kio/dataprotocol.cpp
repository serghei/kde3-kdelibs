...
static int find(const QString &buf, int begin, QChar c1, QChar c2 = '\0', QChar c3 = '\0')
{
    ...
    {
        QChar ch = buf[pos];
        if(ch == c1 || (c2 != '\0' && ch == c2) || (c3 != '\0' && ch == c3))
            break;
        pos++;
    } /*wend*/
    return pos;
 ...
 * @param c3 alternative character to find or 0 to ignore
 */
inline QString extract(const QString &buf, int &pos, QChar c1, QChar c2 = '\0', QChar c3 = '\0')
{
    int oldpos = pos;
...
static void parseDataHeader(const KURL &url, DataHeader &header_info)
{
    ...
    header_info.charset = &header_info.attributes.insert(charset.string(), us_ascii.string()).data();
    ...
    if(header_info.data_offset >= raw_url_len)
        return;
    QString mime_type = extract(raw_url, header_info.data_offset, ';', ',').stripWhiteSpace();
    ...
    {
        // read attribute
        QString attribute = extract(raw_url, header_info.data_offset, '=', ';', ',').stripWhiteSpace();
        ...
        else
        {
            ...
            if(raw_url[header_info.data_offset] == '"')
            {
                ...
                value = extract(raw_url, header_info.data_offset, ';', ',').stripWhiteSpace();
...
#ifdef DATAKIOSLAVE
DataProtocol::DataProtocol(const QCString &pool_socket, const QCString &app_socket) : SlaveBase("kio_data", pool_socket, app_socket)
{
#else
