...
KService::KService(const QString &_name, const QString &_exec, const QString &_icon) : KSycocaEntry(QString::null)
{
...
KService::KService(const QString &_fullpath) : KSycocaEntry(_fullpath)
{
...
KService::KService(KDesktopFile *config) : KSycocaEntry(config->fileName())
{
    init(config);
}

void KService::init(KDesktopFile *config)
{
    d = new KServicePrivate;
    m_bValid = true;
    ...
    {
        kdWarning(7012) << "The desktop entry file " << entryPath() << " has Type=" << m_strType << " instead of \"Application\" or \"Service\""
                        << endl;
        m_bValid = false;
        return;
    }
    ...
    if((m_strType == "Application") && (!resource.isEmpty()) && (resource != "apps") && !absPath)
    {
        kdWarning(7012) << "The desktop entry file " << entryPath() << " has Type=" << m_strType << " but is located under \"" << resource
                        << "\" instead of \"apps\"" << endl;
        m_bValid = false;
        return;
    }

    if((m_strType == "Service") && (!resource.isEmpty()) && (resource != "services") && !absPath)
    {
        kdWarning(7012) << "The desktop entry file " << entryPath() << " has Type=" << m_strType << " but is located under \"" << resource
...
void KService::load(QDataStream &s)
{
    ...
    // number in ksycoca.h
    s >> m_strType >> m_strName >> m_strExec >> m_strIcon >> term >> m_strTerminalOptions >> m_strPath >> m_strComment >> m_lstServiceTypes >> def
        >> m_mapProps >> m_strLibrary >> dummyI1 >> dummyI2 >> dst >> m_strDesktopEntryName >> dummy1 >> dummyStr1 >> initpref >> dummyStr2 >> dummy2
        >> m_lstKeywords >> m_strInit >> dummyUI32 >> m_strGenName >> d->categories >> d->menuId;
...
public:
    KServiceReadProperty(const QString &_key, const QCString &_value) : key(_key), value(_value)
    {
    }

    bool internalHasGroup(const QCString &) const
    { /*qDebug("hasGroup(const QCString &)");*/
        return false;
    }
    ...
    KEntryMap internalEntryMap() const
    {
        return KEntryMap();
    }
...
bool KService::allowMultipleFiles() const
{
    ...
    if(m_strExec.find("%F") != -1 || m_strExec.find("%U") != -1 || m_strExec.find("%N") != -1 || m_strExec.find("%D") != -1)
        return true;
    else
        return false;
}
...
{
    if(d->menuId.isEmpty() || desktopEntryPath().startsWith(".hidden") || (QDir::isRelativePath(desktopEntryPath()) && d->categories.isEmpty()))
...
{
    KServiceProgressDialog dlg(parent, "ksycoca_progress", i18n("Updating System Configuration"), i18n("Updating system configuration."));

    QByteArray data;
    DCOPClient *client = kapp->dcopClient();

    int result = client->callAsync("kded", "kbuildsycoca", "recreate()", data, &dlg, SLOT(slotFinished()));
...
KServiceProgressDialog::KServiceProgressDialog(QWidget *parent, const char *name, const QString &caption, const QString &text)
...
void KServiceProgressDialog::slotProgress()
{
...
void KServiceProgressDialog::slotFinished()
{
