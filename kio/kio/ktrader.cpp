...
public:
    ...
    KTraderSorter(const KTraderSorter &s)
        : m_userPreference(s.m_userPreference)
        , m_bAllowAsDefault(s.m_bAllowAsDefault)
        , m_traderPreference(s.m_traderPreference)
        , m_pService(s.m_pService)
    {
    }
...
KTrader::OfferList KTrader::query(const QString &_servicetype, const QString &_constraint, const QString &_preferences) const
{
    return query(_servicetype, QString::null, _constraint, _preferences);
}

KTrader::OfferList KTrader::query(const QString &_servicetype, const QString &_genericServiceType, const QString &_constraint,
                                  const QString &_preferences) const
{
    ...
    kdDebug(7014) << "query for " << query << " : returning " << ret.count() << " offers" << endl;
#endif
    return ret;
}
