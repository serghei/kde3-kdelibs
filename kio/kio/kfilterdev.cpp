...
public:
    KFilterDevPrivate() : bNeedHeader(true), bSkipHeaders(false), autoDeleteFilterBase(false), bOpenedUnderlyingDevice(false), bIgnoreData(false)
    {
    }
    bool bNeedHeader;
    bool bSkipHeaders;
...
KFilterDev::KFilterDev(KFilterBase *_filter, bool autoDeleteFilterBase) : filter(_filter)
{
    assert(filter);
...
// static
QIODevice *KFilterDev::deviceForFile(const QString &fileName, const QString &mimetype, bool forceFilter)
{
    QFile *f = new QFile(fileName);
    KFilterBase *base = mimetype.isEmpty() ? KFilterBase::findFilterByFileName(fileName) : KFilterBase::findFilterByMimeType(mimetype);
    if(base)
    {
...
{
    return filter->device()->atEnd() && (d->result == KFilterBase::END) && d->ungetchBuffer.isEmpty();
}
...
Q_LONG KFilterDev::readBlock(char *data, Q_ULONG maxlen)
{
    ...
    while(dataReceived < maxlen)
    {
        if(filter->inBufferEmpty())
        {
            ...
            int size = filter->device()->readBlock(d->buffer.data(), d->buffer.size());
            if(size)
...
Q_LONG KFilterDev::writeBlock(const char *data /*0 to finish*/, Q_ULONG len)
{
    ...
    while(dataWritten < len || finish)
    {
        ...
        if(filter->inBufferEmpty() || (d->result == KFilterBase::END))
        {
            ...
            // kdDebug(7005) << " Wrote everything for now. avail_in = " << filter->inBufferAvailable() << " result=" << d->result << " wrote=" <<
            // wrote << endl;
