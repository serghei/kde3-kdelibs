...
DefaultProgress::DefaultProgress(bool showNow)
    : ProgressBase(0), m_iTotalSize(0), m_iTotalFiles(0), m_iTotalDirs(0), m_iProcessedSize(0), m_iProcessedDirs(0), m_iProcessedFiles(0)
{
    init();
...
DefaultProgress::DefaultProgress(QWidget *parent, const char * /*name*/)
    : ProgressBase(parent), m_iTotalSize(0), m_iTotalFiles(0), m_iTotalDirs(0), m_iProcessedSize(0), m_iProcessedDirs(0), m_iProcessedFiles(0)
{
    init();
}
...
void DefaultProgress::init()
{
    ...
    KWin::setIcons(winId(), KGlobal::iconLoader()->loadIcon("filesave", KIcon::NoGroup, 32),
...
// static
QString DefaultProgress::makePercentString(unsigned long percent, KIO::filesize_t totalSize, unsigned long totalFiles)
{
    if(totalSize)
...
void DefaultProgress::slotProcessedSize(KIO::Job *, KIO::filesize_t bytes)
{
    ...
    QString tmp = i18n("%1 of %2 complete").arg(KIO::convertSize(bytes)).arg(KIO::convertSize(m_iTotalSize));
...
void DefaultProgress::slotSpeed(KIO::Job *, unsigned long speed)
{
    ...
    {
        speedLabel->setText(i18n("%1/s ( %2 remaining )")
                                .arg(KIO::convertSize(speed))
