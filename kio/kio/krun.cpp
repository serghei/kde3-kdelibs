...
pid_t KRun::runURL(const KURL &u, const QString &_mimetype, bool tempFile, bool runExecutables, const QString &suggestedFileName)
{
    ...
    {
        KMessageBoxWrapper::error(0L, i18n("<qt>Unable to enter <b>%1</b>.\nYou do not have access rights to this location.</qt>").arg(u.htmlURL()));
        return 0;
    }
    ...
    if(noRun)
    {
        KMessageBox::sorry(0L, i18n("<qt>The file <b>%1</b> is an executable program. "
    ...
    if(noAuth)
    {
        KMessageBoxWrapper::error(0L, i18n("<qt>You do not have permission to run <b>%1</b>.</qt>").arg(u.htmlURL()));
        return 0;
    }
...
public:
    KRunMX1(const KService &_service) : KMacroExpanderBase('%'), hasUrls(false), hasSpec(false), service(_service)
    {
    }
...
int KRunMX1::expandEscapedMacro(const QString &str, uint pos, QStringList &ret)
{
...
public:
    KRunMX2(const KURL::List &_urls) : KMacroExpanderBase('%'), ignFile(false), urls(_urls)
    {
    }
    bool ignFile : 1;
...
void KRunMX2::subst(int option, const KURL &url, QStringList &ret)
{
    switch(option)
    {
        case 'u':
...
int KRunMX2::expandEscapedMacro(const QString &str, uint pos, QStringList &ret)
{
    uint option = str[pos + 1];
    switch(option)
    {
...
static pid_t runCommandInternal(KProcess *proc, const KService *service, const QString &binName, const QString &execName, const QString &iconName)
{
    if(service && !service->desktopEntryPath().isEmpty() && !KDesktopFile::isAuthorizedDesktopFile(service->desktopEntryPath()))
    {
...
static pid_t runTempService(const KService &_service, const KURL::List &_urls, bool tempFiles, const QString &suggestedFileName)
{
    ...
    *proc << args;
    ...
    return runCommandInternal(proc, &_service, KRun::binaryName(_service.exec(), false), _service.name(), _service.icon());
}
...
static KURL::List resolveURLs(const KURL::List &_urls, const KService &_service)
{
    ...
    else
    {
        if(supportedProtocols.isEmpty())
        {
            ...
            { // if no KDE app, be a bit over-generic
...
{
    if(!_service.desktopEntryPath().isEmpty() && !KDesktopFile::isAuthorizedDesktopFile(_service.desktopEntryPath()))
    {
    ...
    int i = KApplication::startServiceByDesktopPath(_service.desktopEntryPath(), urls.toStringList(), &error, 0L, &pid);
...
pid_t KRun::run(const QString &_exec, const KURL::List &_urls, const QString &_name, const QString &_icon, const QString &, const QString &)
{
...
KRun::KRun(const KURL &url, mode_t mode, bool isLocalFile, bool showProgressInfo) : m_timer(0, "KRun::timer")
{
    init(url, 0, mode, isLocalFile, showProgressInfo);
}

KRun::KRun(const KURL &url, QWidget *window, mode_t mode, bool isLocalFile, bool showProgressInfo) : m_timer(0, "KRun::timer")
{
    init(url, window, mode, isLocalFile, showProgressInfo);
}

void KRun::init(const KURL &url, QWidget *window, mode_t mode, bool isLocalFile, bool showProgressInfo)
{
    m_bFault = false;
    m_bAutoDelete = true;
        ...
        if(m_mode == 0)
        {
            ...
            if(KDE_stat(QFile::encodeName(m_strURL.path()), &buff) == -1)
            {
                ...
                KMessageBoxWrapper::error(
                    d->m_window,
                    i18n("<qt>Unable to run the command specified. The file or folder <b>%1</b> does not exist.</qt>").arg(m_strURL.htmlURL()));
                d->m_showingError = false;
                m_bFault = true;
                m_bFinished = true;
                m_timer.start(0, true);
...
void KRun::scanFile()
{
    ...
    job->setWindow(d->m_window);
    connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(slotScanFinished(KIO::Job *)));
    connect(job, SIGNAL(mimetype(KIO::Job *, const QString &)), this, SLOT(slotScanMimeType(KIO::Job *, const QString &)));
    m_job = job;
...
{
    return (serviceType == "application/x-desktop" || serviceType == "application/x-executable" || serviceType == "application/x-msdos-program"
            || serviceType == "application/x-shellscript");
}

/****************/

pid_t KProcessRunner::run(KProcess *p, const QString &binName)
{
    return (new KProcessRunner(p, binName))->pid();
}

#ifdef Q_WS_X11
pid_t KProcessRunner::run(KProcess *p, const QString &binName, const KStartupInfoId &id)
{
    return (new KProcessRunner(p, binName, id))->pid();
}
#endif

KProcessRunner::KProcessRunner(KProcess *p, const QString &_binName) : QObject(), process_(p), binName(_binName)
{
    QObject::connect(process_, SIGNAL(processExited(KProcess *)), this, SLOT(slotProcessExited(KProcess *)));
...
#ifdef Q_WS_X11
KProcessRunner::KProcessRunner(KProcess *p, const QString &_binName, const KStartupInfoId &id) : QObject(), process_(p), binName(_binName), id_(id)
{
    QObject::connect(process_, SIGNAL(processExited(KProcess *)), this, SLOT(slotProcessExited(KProcess *)));
...
void KProcessRunner::slotProcessExited(KProcess *p)
{
    if(p != process_)
        return; // Eh ?
    ...
    bool showErr = process_->normalExit() && (process_->exitStatus() == 127 || process_->exitStatus() == 1);
