...
bool NetAccess::download(const KURL &u, QString &target, QWidget *window)
{
    ...
    KURL dest;
    dest.setPath(target);
    return kioNet.filecopyInternal(u, dest, -1, true /*overwrite*/, false, window, false /*copy*/);
}
...
bool NetAccess::upload(const QString &src, const KURL &target, QWidget *window)
{
    ...
    KURL s;
    s.setPath(src);
    return kioNet.filecopyInternal(s, target, -1, true /*overwrite*/, false, window, false /*copy*/);
}
...
bool NetAccess::file_copy(const KURL &src, const KURL &target, int permissions, bool overwrite, bool resume, QWidget *window)
{
    NetAccess kioNet;
    return kioNet.filecopyInternal(src, target, permissions, overwrite, resume, window, false /*copy*/);
}
...
{
    NetAccess kioNet;
    return kioNet.filecopyInternal(src, target, permissions, overwrite, resume, window, true /*move*/);
}
...
bool NetAccess::synchronousRun(Job *job, QWidget *window, QByteArray *data, KURL *finalURL, QMap< QString, QString > *metaData)
{
    NetAccess kioNet;
...
bool NetAccess::filecopyInternal(const KURL &src, const KURL &target, int permissions, bool overwrite, bool resume, QWidget *window, bool move)
{
    bJobOK = true; // success unless further error occurs

    KIO::Scheduler::checkSlaveOnHold(true);
    KIO::Job *job = move ? KIO::file_move(src, target, permissions, overwrite, resume) : KIO::file_copy(src, target, permissions, overwrite, resume);
...
bool NetAccess::dircopyInternal(const KURL::List &src, const KURL &target, QWidget *window, bool move)
{
    bJobOK = true; // success unless further error occurs
...
bool NetAccess::statInternal(const KURL &url, int details, bool source, QWidget *window)
{
    bJobOK = true; // success unless further error occurs
...
bool NetAccess::synchronousRunInternal(Job *job, QWidget *window, QByteArray *data, KURL *finalURL, QMap< QString, QString > *metaData)
{
    job->setWindow(window);
