...
KDirListerCache::KDirListerCache(int maxCount) : itemsCached(maxCount)
{
    ...
    connect(kdirwatch, SIGNAL(dirty(const QString &)), this, SLOT(slotFileDirty(const QString &)));
    connect(kdirwatch, SIGNAL(created(const QString &)), this, SLOT(slotFileCreated(const QString &)));
    connect(kdirwatch, SIGNAL(deleted(const QString &)), this, SLOT(slotFileDeleted(const QString &)));
}
...
bool KDirListerCache::listDir(KDirLister *lister, const KURL &_u, bool _keep, bool _reload)
{
    ...
    printDebug();
#endif
    kdDebug(7004) << k_funcinfo << lister << " url=" << _url << " keep=" << _keep << " reload=" << _reload << endl;
...
void KDirListerCache::setAutoUpdate(KDirLister *lister, bool enable)
{
    ...
    for(KURL::List::Iterator it = lister->d->lstDirs.begin(); it != lister->d->lstDirs.end(); ++it)
    {
        if(enable)
...
void KDirListerCache::forgetDirs(KDirLister *lister)
{
    ...
    for(KURL::List::Iterator it = lstDirsCopy.begin(); it != lstDirsCopy.end(); ++it)
    {
...
void KDirListerCache::updateDirectory(const KURL &_dir)
{
    ...
    connect(job, SIGNAL(entries(KIO::Job *, const KIO::UDSEntryList &)), this, SLOT(slotUpdateEntries(KIO::Job *, const KIO::UDSEntryList &)));
    connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(slotUpdateResult(KIO::Job *)));
...
void KDirListerCache::printDebug()
{
    ...
    for(; itu.current(); ++itu)
    {
        kdDebug(7004) << "   " << itu.currentKey() << "  URL: " << itu.current()->url
                      ...
                      << " autoUpdates refcount: " << itu.current()->autoUpdates << " complete: " << itu.current()->complete
                      << (itu.current()->lstItems ? QString(" with %1 items.").arg(itu.current()->lstItems->count()) : QString(" lstItems=NULL"))
                      << endl;
    }
    ...
    QCacheIterator< DirItem > itc(itemsCached);
    for(; itc.current(); ++itc)
        kdDebug(7004) << "   " << itc.currentKey()
                      << "  rootItem: " << (itc.current()->rootItem ? itc.current()->rootItem->url().prettyURL() : QString("NULL"))
                      << (itc.current()->lstItems ? QString(" with %1 items.").arg(itc.current()->lstItems->count()) : QString(" lstItems=NULL"))
                      << endl;
}
#endif
...
{
    kdDebug(7003) << k_funcinfo << _url.prettyURL() << " keep=" << _keep << " reload=" << _reload << endl;
...
void KDirLister::emitChanges()
{
    ...
    for(KURL::List::Iterator it = d->lstDirs.begin(); it != d->lstDirs.end(); ++it)
    {
        ...
        for(; kit.current(); ++kit)
        {
            ...
            {
                oldMime = doMimeFilter((*kit)->mimetype(), d->oldMimeFilter) && doMimeExcludeFilter((*kit)->mimetype(), d->oldMimeExcludeFilter);
                newMime = doMimeFilter((*kit)->mimetype(), d->mimeFilter) && doMimeExcludeFilter((*kit)->mimetype(), d->mimeExcludeFilter);
            ...
            {
                bool oldName = (*kit)->isDir() || d->oldFilters.isEmpty() || doNameFilter((*kit)->text(), d->oldFilters);

                bool newName = (*kit)->isDir() || d->lstFilters.isEmpty() || doNameFilter((*kit)->text(), d->lstFilters);
...
void KDirLister::setMimeFilter(const QStringList &mimeFilter)
{
    ...
    if(mimeFilter.find("all/allfiles") != mimeFilter.end() || mimeFilter.find("all/all") != mimeFilter.end())
        d->mimeFilter.clear();
    else
