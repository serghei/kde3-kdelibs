...
KServiceType::KServiceType(const QString &_fullpath) : KSycocaEntry(_fullpath), d(0)
{
...
KServiceType::KServiceType(KDesktopFile *config) : KSycocaEntry(config->fileName()), d(0)
{
    init(config);
}

void KServiceType::init(KDesktopFile *config)
{
    // Is it a mimetype ?
    ...
    for(; gIt != tmpList.end(); ++gIt)
    {
        ...
        {
            config->setGroup(*gIt);
            QVariant v = config->readPropertyEntry("Value", QVariant::nameToType(config->readEntry("Type").ascii()));
            if(v.isValid())
    ...
    for(; gIt != tmpList.end(); ++gIt)
    {
        ...
        {
            config->setGroup(*gIt);
            m_mapPropDefs.insert((*gIt).mid(13), QVariant::nameToType(config->readEntry("Type").ascii()));
        }
    }
...
KServiceType::KServiceType(const QString &_fullpath, const QString &_type, const QString &_icon, const QString &_comment)
...
KServiceType::KServiceType(QDataStream &_str, int offset) : KSycocaEntry(_str, offset), d(0)
{
    load(_str);
}
...
void KServiceType::save(QDataStream &_str)
{
...
QVariant KServiceType::property(const QString &_name) const
{
    QVariant v;
