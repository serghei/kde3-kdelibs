...
TCPSlaveBase::TCPSlaveBase(unsigned short int defaultPort, const QCString &protocol, const QCString &poolSocket, const QCString &appSocket)
    : SlaveBase(protocol, poolSocket, appSocket), m_iSock(-1), m_iDefaultPort(defaultPort), m_sServiceName(protocol), fp(0)
{
...
TCPSlaveBase::TCPSlaveBase(unsigned short int defaultPort, const QCString &protocol, const QCString &poolSocket, const QCString &appSocket,
                           bool useSSL)
    : SlaveBase(protocol, poolSocket, appSocket), m_iSock(-1), m_bIsSSL(useSSL), m_iDefaultPort(defaultPort), m_sServiceName(protocol), fp(0)
{
    doConstructorStuff();
    if(useSSL)
...
void TCPSlaveBase::certificatePrompt()
{
    ...
    if(prompt || forcePrompt)
    {
        ...
        for(QStringList::Iterator it = certs.begin(); it != certs.end(); ++it)
        {
            ...
            if(pkcs && (!pkcs->getCertificate() || !pkcs->getCertificate()->x509V3Extensions().certTypeSSLClient()))
            {
                certs.remove(*it);
            }
            delete pkcs;
        ...
        bool rc = d->dcc->call("kio_uiserver", "UIServer", "showSSLCertDialog(QString, QStringList,int)", data, rettype, retval);
    ...
    if(!send)
    {
        if(save)
        {
            KSSLCertificateHome::setDefaultCertificate(certname, ourHost, false, false);
        }
        return;
    }
    ...
    { // We need the password
        ...
        do
        {
            ...
            if(showprompt)
            {
                if(!openPassDlg(ai, first ? QString::null : i18n("Unable to open the certificate. Try a new password?")))
                    break;
            }
    ...
    if(pkcs)
    {
        ...
        else
        {
            ...
            if(save)
            {
                KSSLCertificateHome::setDefaultCertificate(certname, ourHost, true, false);
            }
        }
        d->pkcs = pkcs;
    }
...
int TCPSlaveBase::verifyCertificate()
{
    ...
    QString errorStr;
    for(KSSLCertificate::KSSLValidationList::ConstIterator it = ksvl.begin(); it != ksvl.end(); ++it)
    {
                ...
                case KSSLCertificateCache::Prompt:
                {
                    do
                    {
                        if(ksv == KSSLCertificate::InvalidHost)
                        {
                            ...
                            result =
                                messageBox(WarningYesNoCancel, msg.arg(ourHost), i18n("Server Authentication"), i18n("&Details"), i18n("Co&ntinue"));
                        }
                        else
                        {
                            QString msg = i18n(
                        ...
                        if(result == KMessageBox::Yes)
                        {
                            ...
                            arg << metaData("window-id").toInt();
                            d->dcc->call("kio_uiserver", "UIServer", "showSSLInfoDialog(QString,KIO::MetaData,int)", data, ignoretype, ignore);
                        }
                    ...
                    if(result == KMessageBox::No)
                    {
                        ...
                        doAddHost = true;
                        result = messageBox(WarningYesNo, i18n("Would you like to accept this "
                                                               "certificate forever without "
                                                               "being prompted?"),
                                            i18n("Server Authentication"), i18n("&Forever"), i18n("&Current Sessions Only"));
                        if(result == KMessageBox::Yes)
                            permacache = true;
                        else
                            permacache = false;
    ...
    { // Child frame
        ...
        // as the parent frame
        bool certAndIPTheSame = (d->ip == metaData("ssl_parent_ip") && pc.toString() == metaData("ssl_parent_cert"));
        ...
        if(ksv == KSSLCertificate::Ok)
        {
            ...
            else
            {
                ...
                result = messageBox(WarningYesNo,
                                    i18n("The certificate is valid but does not appear to have been assigned to this server.  Do you wish to continue
                loading?"),
        ...
        else
        {
            ...
            if(cp == KSSLCertificateCache::Accept)
            {
                ...
                { // fail
                    result = messageBox(WarningYesNo, i18n("You have indicated that you wish to accept this certificate, but it is not issued to the "
                                                           "server who is presenting it. Do you wish to continue loading?"),
                                        i18n("Server Authentication"));
                    if(result == KMessageBox::Yes)
                    {
                        rc = 1;
            ...
            else
            {
                do
                {
                    QString msg = i18n(
                        "The server certificate failed the "
                        "authenticity test (%1).");
                    result = messageBox(WarningYesNoCancel, msg.arg(ourHost), i18n("Server Authentication"), i18n("&Details"), i18n("Co&nnect"));
                    if(result == KMessageBox::Yes)
                    {
                        if(!d->dcc)
                        {
        ...
        if(!isChild && metaData("ssl_was_in_use") == "FALSE" && d->kssl->settings()->warnOnEnter())
        {
            ...
            do
            {
                result = messageBox(i18n("You are about to "
                                         ...
                                         "easily observe your "
                                         "data in transit."),
                                    WarningYesNo, i18n("Security Information"), i18n("Display SSL "
                                                                                     "&Information"),
                                    i18n("C&onnect"), "WarnOnEnterSSLMode");
                // Move this setting into KSSL instead
                KConfig *config = new KConfig("kioslaverc");
                config->setGroup("Notification Messages");
    ...
    kdDebug(7029) << "SSL connection information follows:" << endl
                  ...
                  << "| Strength: " << d->kssl->connectionInfo().getCipherUsedBits() << " of " << d->kssl->connectionInfo().getCipherBits()
                  << " bits used." << endl
                  << "| PEER:" << endl
