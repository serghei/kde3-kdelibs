...
public:
    SlaveBaseConfig(SlaveBase *_slave) : slave(_slave)
    {
    }

    bool internalHasGroup(const QCString &) const
    {
        qWarning("hasGroup(const QCString &)");
        return false;
    }
...
SlaveBase::SlaveBase(const QCString &protocol, const QCString &pool_socket, const QCString &app_socket)
    ...
    while(true)
    {
        ...
        if((retval < 0) && (errno != EINTR))
        {
            kdDebug(7019) << "dispatchLoop(): select returned " << retval << " "
                          << (errno == EBADF ? "EBADF" : errno == EINTR ? "EINTR" : errno == EINVAL ? "EINVAL" : errno == ENOMEM ? "ENOMEM"
                                                                                                                                 : "unknown")
                          << " (" << errno << ")" << endl;
            return;
        }
...
void SlaveBase::listEntry(const UDSEntry &entry, bool _ready)
{
    ...
    if(!_ready)
    {
        ...
        if(pendingListEntries.count() > listEntryCurrentSize)
        {
            ...
            long diff = ((tp.tv_sec - listEntry_sec) * 1000000 + tp.tv_usec - listEntry_usec) / 1000;
            if(diff == 0)
                diff = 1;
...
void SlaveBase::sendAuthenticationKey(const QCString &key, const QCString &group, bool keepPass)
{
...
bool SlaveBase::openPassDlg(AuthInfo &info, const QString &errorMsg)
{
    ...
    bool callOK = d->dcopClient->call("kded", "kpasswdserver", "queryAuthInfo(KIO::AuthInfo, QString, long int, long int, unsigned long int)", params,
                                      replyType, reply);
    ...
    {
        kdError(7019) << "DCOP function queryAuthInfo(...) returns " << replyType << ", expected KIO::AuthInfo" << endl;
        return false;
    }
...
int SlaveBase::messageBox(const QString &text, MessageBoxType type, const QString &caption, const QString &buttonYes, const QString &buttonNo,
...
bool SlaveBase::checkCachedAuthentication(AuthInfo &info)
{
    ...
    if(!d->dcopClient->call("kded", "kpasswdserver", "checkAuthInfo(KIO::AuthInfo, long int, unsigned long int)", params, replyType, reply))
    {
        kdWarning(7019) << "Can't communicate with kded_kpasswdserver!" << endl;
        return false;
    }
    ...
    {
        kdError(7019) << "DCOP function checkAuthInfo(...) returns " << replyType << ", expected KIO::AuthInfo" << endl;
        return false;
    }
    if(!authResult.isModified())
    {
