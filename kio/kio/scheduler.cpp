...
KIO::Scheduler::ProtocolInfo *KIO::Scheduler::ProtocolInfoDict::get(const QString &protocol)
{
...
void Scheduler::startStep()
{
    ...
    while(it.current())
    {
        if(startJobScheduled(it.current()))
            return;
        ++it;
    }
}

void Scheduler::setupSlave(KIO::Slave *slave, const KURL &url, const QString &protocol, const QString &proxy, bool newSlave,
...
static Slave *searchIdleList(SlaveList *idleSlaves, const KURL &url, const QString &protocol, bool &exact)
{
    ...
    for(Slave *slave = idleSlaves->first(); slave; slave = idleSlaves->next())
    {
        if((protocol == slave->slaveProtocol()) && (host == slave->host()) && (port == slave->port()) && (user == slave->user()))
            return slave;
    }
...
Slave *Scheduler::createSlave(ProtocolInfo *protInfo, SimpleJob *job, const KURL &url)
{
    ...
    if(slave)
    {
        ...
        connect(slave, SIGNAL(slaveDied(KIO::Slave *)), SLOT(slotSlaveDied(KIO::Slave *)));
        ...
        connect(slave, SIGNAL(authorizationKey(const QCString &, const QCString &, bool)), sessionData,
                SLOT(slotAuthData(const QCString &, const QCString &, bool)));
        connect(slave, SIGNAL(delAuthorization(const QCString &)), sessionData, SLOT(slotDelAuthData(const QCString &)));
    }
    else
    {
...
Slave *Scheduler::_getConnectedSlave(const KURL &url, const KIO::MetaData &config)
{
    QString proxy;
    ...
    connect(slave, SIGNAL(connected()), SLOT(slotSlaveConnected()));
    connect(slave, SIGNAL(error(int, const QString &)), SLOT(slotSlaveError(int, const QString &)));
...
void Scheduler::slotScheduleCoSlave()
{
    Slave *nextSlave;
    slaveConfig = SlaveConfig::self();
    for(Slave *slave = coIdleSlaves->first(); slave; slave = nextSlave)
    {
...
void Scheduler::slotSlaveConnected()
{
    ...
    disconnect(slave, SIGNAL(connected()), this, SLOT(slotSlaveConnected()));
...
void Scheduler::slotSlaveError(int errorNr, const QString &errorMsg)
{
    Slave *slave = (Slave *)sender();
...
bool Scheduler::_assignJobToSlave(KIO::Slave *slave, SimpleJob *job)
{
    //    kdDebug(7006) << "_assignJobToSlave( " << job << ", " << slave << ")" << endl;
    QString dummy;
    if((slave->slaveProtocol() != KProtocolManager::slaveProtocol(job->url(), dummy)) || (!newJobs.removeRef(job)))
    {
...
bool Scheduler::_disconnectSlave(KIO::Slave *slave)
{
    ...
    assert(!coIdleSlaves->contains(slave));
    disconnect(slave, SIGNAL(connected()), this, SLOT(slotSlaveConnected()));
    disconnect(slave, SIGNAL(error(int, const QString &)), this, SLOT(slotSlaveError(int, const QString &)));
    if(slave->isAlive())
    {
        idleSlaves->append(slave);
...
void Scheduler::_checkSlaveOnHold(bool b)
{
    checkOnHold = b;
}

void Scheduler::_registerWindow(QWidget *wid)
{
    if(!wid)
        return;
    ...
    if(!m_windowList.contains(obj))
    {
        ...
        connect(wid, SIGNAL(destroyed(QObject *)), this, SLOT(slotUnregisterWindow(QObject *)));
        QByteArray params;
        QDataStream stream(params, IO_WriteOnly);
        stream << windowId;
        if(!kapp->dcopClient()->send("kded", "kded", "registerWindowId(long int)", params))
...
void Scheduler::slotUnregisterWindow(QObject *obj)
{
    ...
    disconnect(it.key(), SIGNAL(destroyed(QObject *)), this, SLOT(slotUnregisterWindow(QObject *)));
    ...
    if(kapp)
    {
        ...
        QDataStream stream(params, IO_WriteOnly);
        stream << windowId;
        kapp->dcopClient()->send("kded", "kded", "unregisterWindowId(long int)", params);
    }
}
