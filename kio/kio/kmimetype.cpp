...
void KMimeType::buildDefaultType()
{
    ...
    KServiceType *mime = KServiceTypeFactory::self()->findServiceTypeByName(defaultMimeType());
    ...
    else
    {
        ...
        s_pDefaultType = new KMimeType(sDefaultMimeType, defaultMimeType(), "unknown", "mime", QStringList());
    }
}
...
KMimeType::Ptr KMimeType::findByURL(const KURL &_url, mode_t _mode, bool _is_local_file, bool _fast_mode)
{
...
KMimeType::Ptr KMimeType::findByURL(const KURL &_url, mode_t _mode, bool _is_local_file, bool _fast_mode, bool *accurate)
{
    KMimeType::Ptr mime = findByURL(_url, _mode, _is_local_file, _fast_mode);
    if(accurate)
        *accurate = !(_fast_mode) || ((mime->patternsAccuracy() == 100) && mime != defaultMimeTypePtr());
    return mime;
}
...
KMimeType::KMimeType(const QString &_fullpath, const QString &_type, const QString &_icon, const QString &_comment, const QStringList &_patterns)
...
QPixmap KMimeType::pixmap(KIcon::Group _group, int _force_size, int _state, QString *_path) const
{
...
QPixmap KMimeType::pixmap(const KURL &_url, KIcon::Group _group, int _force_size, int _state, QString *_path) const
{
    KIconLoader *iconLoader = KGlobal::iconLoader();
...
QPixmap KMimeType::pixmapForURL(const KURL &_url, mode_t _mode, KIcon::Group _group, int _force_size, int _state, QString *_path)
{
    KIconLoader *iconLoader = KGlobal::iconLoader();
...
{
    const KMimeType::Ptr mt = findByURL(_url, _mode, _url.isLocalFile(), false /*HACK*/);
...
QString KMimeType::favIconForURL(const KURL &url)
{
    ...
    if(url.isLocalFile() || !url.protocol().startsWith("http") || !useFavIcons)
        return QString::null;
            ...
            if(dp)
            {
                ...
                else
                {
                    ...
                    { // only three
                        ...
                        isempty =
                            entries.find(".") != entries.end() && entries.find("..") != entries.end() && entries.find(".directory") != entries.end();
                    }
                }
...
QPixmap KDEDesktopMimeType::pixmap(const KURL &_url, KIcon::Group _group, int _force_size, int _state, QString *_path) const
{
    QString _icon = icon(_url, _url.isLocalFile());
    QPixmap pix = KGlobal::iconLoader()->loadIcon(_icon, _group, _force_size, _state, _path, false);
    if(pix.isNull())
        pix = KGlobal::iconLoader()->loadIcon("unknown", _group, _force_size, _state, _path, false);
    return pix;
}
                                                                                  ...
                                                                                  const KURL::List &file_list)
{
    ...
    if(cfg.hasKey("X-KDE-GetActionMenu"))
    {
        ...
        if(!function.endsWith("(KURL::List)"))
        {
            kdWarning()
                << "Desktop file " << path
                << " contains an invalid X-KDE-ShowIfDcopCall - the function must take the exact parameter (KURL::List) and must be specified."
                << endl;
        }
        else
        {
            if(kapp->dcopClient()->call(app, object, function.utf8(), dataToSend, replyType, replyData, true, -1) && replyType == "QStringList")
            {
...
void KDEDesktopMimeType::executeService(const KURL::List &urls, KDEDesktopMimeType::Service &_service)
{
    ...
    {
        kdDebug() << "KDEDesktopMimeType::executeService " << _service.m_strName << " first url's path=" << urls.first().path()
...
{
    static const QString &s_strDefaultMimeType = KGlobal::staticQString("application/octet-stream");
