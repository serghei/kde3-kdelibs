...
void KServiceTypeProfile::initStatic()
{
    ...
    for(QStringList::Iterator aIt = tmpList.begin(); aIt != tmpList.end(); ++aIt)
    {
        if(*aIt == defaultGroup)
            continue;
        ...
        if(pService)
        {
            ...
            {
                KServiceTypeProfile *p = KServiceTypeProfile::serviceTypeProfile(type, type2);
                ...
                // kdDebug(7014) << "KServiceTypeProfile::initStatic adding service " << application << " to profile for " << type << "," << type2 <<
                // " with preference " << pref << endl;
    ...
    for(; it != list.end(); ++it)
    {
        if(_genericServiceType.isEmpty() /*no constraint*/ || (*it)->hasServiceType(_genericServiceType))
        {
            ...
            if(serviceList.find((*it)->desktopEntryPath()) == serviceList.end())
            {
                ...
                offers.append(o);
                // kdDebug(7014) << "Appending offer " << (*it)->name() << " initial preference=" << (*it)->initialPreference() << "
                // allow-as-default=" << allow << endl;
            }
            // else
...
void KServiceTypeProfile::addService(const QString &_service, int _preference, bool _allow_as_default)
{
...
KServiceTypeProfile *KServiceTypeProfile::serviceTypeProfile(const QString &_servicetype, const QString &_genericServiceType)
{
    ...
    QPtrListIterator< KServiceTypeProfile > it(*s_lstProfiles);
    for(; it.current(); ++it)
        if((it.current()->m_strServiceType == _servicetype) && (it.current()->m_strGenericServiceType == _genservicetype))
            return it.current();
