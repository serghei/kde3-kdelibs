...
KDataToolInfo::KDataToolInfo(const KService::Ptr &service, KInstance *instance)
{
    ...
    {
        kdDebug(30003) << "The service " << m_service->name().latin1() << " does not feature the service type KDataTool" << endl;
        m_service = 0;
    }
}
...
QValueList< KDataToolInfo > KDataToolInfo::query(const QString &datatype, const QString &mimetype, KInstance *instance)
{
    ...
    for(; it != offers.end(); ++it)
    {
        ...
        if(!instance || !(*it)->property("ExcludeFrom").toStringList().contains(instance->instanceName()))
 ...
 *
 *************************************************/
KDataToolAction::KDataToolAction(const QString &text, const KDataToolInfo &info, const QString &command, QObject *parent, const char *name)
    : KAction(text, info.iconName(), 0, parent, name), m_command(command), m_info(info)
{
}
...
QPtrList< KAction > KDataToolAction::dataToolActionList(const QValueList< KDataToolInfo > &tools, const QObject *receiver, const char *slot)
{
    ...
    for(; entry != tools.end(); ++entry)
    {
        ...
        if(commands.count() != userCommands.count())
            kdWarning() << "KDataTool desktop file error (" << (*entry).service()->entryPath() << "). " << commands.count() << " commands and "
        ...
        for(; uit != userCommands.end() && cit != commands.end(); ++uit, ++cit)
        {
            ...
            connect(action, SIGNAL(toolActivated(const KDataToolInfo &, const QString &)), receiver, slot);
...
KDataTool::KDataTool(QObject *parent, const char *name) : QObject(parent, name), m_instance(0L)
{
}
