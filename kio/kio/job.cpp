...
Job::Job(bool showProgressInfo) : QObject(0, "job"), m_error(0), m_percent(0), m_progressId(0), m_speedTimer(0), d(new JobPrivate)
{
    ...
    if(showProgressInfo)
    {
        ...
        connect(this, SIGNAL(percent(KIO::Job *, unsigned long)), Observer::self(), SLOT(slotPercent(KIO::Job *, unsigned long)));
        connect(this, SIGNAL(infoMessage(KIO::Job *, const QString &)), Observer::self(), SLOT(slotInfoMessage(KIO::Job *, const QString &)));
        connect(this, SIGNAL(totalSize(KIO::Job *, KIO::filesize_t)), Observer::self(), SLOT(slotTotalSize(KIO::Job *, KIO::filesize_t)));
        connect(this, SIGNAL(processedSize(KIO::Job *, KIO::filesize_t)), Observer::self(), SLOT(slotProcessedSize(KIO::Job *, KIO::filesize_t)));
        connect(this, SIGNAL(speed(KIO::Job *, unsigned long)), Observer::self(), SLOT(slotSpeed(KIO::Job *, unsigned long)));
    }
...
SimpleJob::SimpleJob(const KURL &url, int command, const QByteArray &packedArgs, bool showProgressInfo)
    : Job(showProgressInfo), m_slave(0), m_packedArgs(packedArgs), m_url(url), m_command(command), m_totalSize(0)
{
    if(m_url.hasSubURL())
    {
...
void SimpleJob::start(Slave *slave)
{
    ...
    connect(m_slave, SIGNAL(error(int, const QString &)), SLOT(slotError(int, const QString &)));

    connect(m_slave, SIGNAL(warning(const QString &)), SLOT(slotWarning(const QString &)));

    connect(m_slave, SIGNAL(infoMessage(const QString &)), SLOT(slotInfoMessage(const QString &)));

    connect(m_slave, SIGNAL(connected()), SLOT(slotConnected()));

    connect(m_slave, SIGNAL(finished()), SLOT(slotFinished()));

    if((extraFlags() & EF_TransferJobDataSent) == 0)
    {
        connect(m_slave, SIGNAL(totalSize(KIO::filesize_t)), SLOT(slotTotalSize(KIO::filesize_t)));

        connect(m_slave, SIGNAL(processedSize(KIO::filesize_t)), SLOT(slotProcessedSize(KIO::filesize_t)));

        connect(m_slave, SIGNAL(speed(unsigned long)), SLOT(slotSpeed(unsigned long)));
    }

    connect(slave, SIGNAL(needProgressId()), SLOT(slotNeedProgressId()));

    connect(slave, SIGNAL(metaData(const KIO::MetaData &)), SLOT(slotMetaData(const KIO::MetaData &)));
...
//////////
MkdirJob::MkdirJob(const KURL &url, int command, const QByteArray &packedArgs, bool showProgressInfo)
...
{
    connect(slave, SIGNAL(redirection(const KURL &)), SLOT(slotRedirection(const KURL &)));
...
{
    KIO_ARGS << int(1) << Q_INT8(ro ? 1 : 0) << QString::fromLatin1(fstype) << dev << point;
...
StatJob::StatJob(const KURL &url, int command, const QByteArray &packedArgs, bool showProgressInfo)
    : SimpleJob(url, command, packedArgs, showProgressInfo), m_bSource(true), m_details(2)
{
}
...
void StatJob::start(Slave *slave)
{
    ...
    connect(slave, SIGNAL(statEntry(const KIO::UDSEntry &)), SLOT(slotStatEntry(const KIO::UDSEntry &)));
    connect(slave, SIGNAL(redirection(const KURL &)), SLOT(slotRedirection(const KURL &)));
...
TransferJob::TransferJob(const KURL &url, int command, const QByteArray &packedArgs, const QByteArray &_staticData, bool showProgressInfo)
...
void TransferJob::slotDataReq()
{
    ...
    {
        kdDebug(7007) << "send " << dataForSlave.size() / 1024 / 1024
...
{
    assert(slave);
    connect(slave, SIGNAL(data(const QByteArray &)), SLOT(slotData(const QByteArray &)));

    connect(slave, SIGNAL(dataReq()), SLOT(slotDataReq()));

    connect(slave, SIGNAL(redirection(const KURL &)), SLOT(slotRedirection(const KURL &)));

    connect(slave, SIGNAL(mimeType(const QString &)), SLOT(slotMimetype(const QString &)));

    connect(slave, SIGNAL(errorPage()), SLOT(slotErrorPage()));

    connect(slave, SIGNAL(needSubURLData()), SLOT(slotNeedSubURLData()));

    connect(slave, SIGNAL(canResume(KIO::filesize_t)), SLOT(slotCanResume(KIO::filesize_t)));
...
void TransferJob::slotNeedSubURLData()
{
    ...
    connect(m_subJob, SIGNAL(data(KIO::Job *, const QByteArray &)), SLOT(slotSubURLData(KIO::Job *, const QByteArray &)));
    addSubjob(m_subJob);
}
...
TransferJob *KIO::http_post(const KURL &url, const QByteArray &postData, bool showProgressInfo)
{
    ...
    if(_error)
    {
        ...
        for(QValueList< int >::ConstIterator it = overriden_ports->begin(); it != overriden_ports->end(); ++it)
    ...
    KIO_ARGS << (int)1 << _url;
    TransferJob *job = new TransferJob(_url, CMD_SPECIAL, packedArgs, postData, showProgressInfo);
...
TransferJob *KIO::put(const KURL &url, int permissions, bool overwrite, bool resume, bool showProgressInfo)
{
...
StoredTransferJob::StoredTransferJob(const KURL &url, int command, const QByteArray &packedArgs, const QByteArray &_staticData, bool showProgressInfo)
    : TransferJob(url, command, packedArgs, _staticData, showProgressInfo), m_uploadOffset(0)
{
    connect(this, SIGNAL(data(KIO::Job *, const QByteArray &)), SLOT(slotStoredData(KIO::Job *, const QByteArray &)));
    connect(this, SIGNAL(dataReq(KIO::Job *, QByteArray &)), SLOT(slotStoredDataReq(KIO::Job *, QByteArray &)));
}
...
StoredTransferJob *KIO::storedPut(const QByteArray &arr, const KURL &url, int permissions, bool overwrite, bool resume, bool showProgressInfo)
{
    KIO_ARGS << url << Q_INT8(overwrite ? 1 : 0) << Q_INT8(resume ? 1 : 0) << permissions;
...
MimetypeJob::MimetypeJob(const KURL &url, int command, const QByteArray &packedArgs, bool showProgressInfo)
...
DirectCopyJob::DirectCopyJob(const KURL &url, int command, const QByteArray &packedArgs, bool showProgressInfo)
    : SimpleJob(url, command, packedArgs, showProgressInfo)
{
}
 ...
 * and sending it away.
 */
FileCopyJob::FileCopyJob(const KURL &src, const KURL &dest, int permissions, bool move, bool overwrite, bool resume, bool showProgressInfo)
...
void FileCopyJob::slotStart()
{
    if(m_move)
    {
        ...
        if((m_src.protocol() == m_dest.protocol()) && (m_src.host() == m_dest.host()) && (m_src.port() == m_dest.port())
           && (m_src.user() == m_dest.user()) && (m_src.pass() == m_dest.pass()) && !m_src.hasSubURL() && !m_dest.hasSubURL())
        {
...
void FileCopyJob::startCopyJob(const KURL &slave_url)
{
    ...
    connect(m_copyJob, SIGNAL(canResume(KIO::Job *, KIO::filesize_t)), SLOT(slotCanResume(KIO::Job *, KIO::filesize_t)));
}
...
void FileCopyJob::startDataPump()
{
    ...
    {
        QDateTime dt;
        dt.setTime_t(d->m_modificationTime);
    ...
    connect(m_putJob, SIGNAL(canResume(KIO::Job *, KIO::filesize_t)), SLOT(slotCanResume(KIO::Job *, KIO::filesize_t)));
    connect(m_putJob, SIGNAL(dataReq(KIO::Job *, QByteArray &)), SLOT(slotDataReq(KIO::Job *, QByteArray &)));
    addSubjob(m_putJob);
}
...
void FileCopyJob::slotCanResume(KIO::Job *job, KIO::filesize_t offset)
{
    if(job == m_putJob || job == m_copyJob)
    {
        ...
        if(offset)
        {
            ...
            if(!KProtocolManager::autoResume() && !m_overwrite)
            {
                ...
                res = Observer::self()->open_RenameDlg(job, i18n("File Already Exists"), m_src.url(), m_dest.url(),
                                                       (RenameDlg_Mode)(M_OVERWRITE | M_RESUME | M_NORENAME), newPath, d->m_sourceSize, offset);
            }
        ...
        if(job == m_putJob)
        {
            ...
            if(offset)
            {
                ...
                connect(m_getJob, SIGNAL(canResume(KIO::Job *, KIO::filesize_t)), SLOT(slotCanResume(KIO::Job *, KIO::filesize_t)));
            }
            ...
            connect(m_getJob, SIGNAL(data(KIO::Job *, const QByteArray &)), SLOT(slotData(KIO::Job *, const QByteArray &)));
            connect(m_getJob, SIGNAL(mimetype(KIO::Job *, const QString &)), SLOT(slotMimetype(KIO::Job *, const QString &)));
        }
        else // copyjob
        {
    ...
    else if(job == m_getJob)
    {
        ...
        kdWarning(7007) << "FileCopyJob::slotCanResume from unknown job=" << job << " m_getJob=" << m_getJob << " m_putJob=" << m_putJob << endl;
}
...
FileCopyJob *KIO::file_copy(const KURL &src, const KURL &dest, int permissions, bool overwrite, bool resume, bool showProgressInfo)
{
    return new FileCopyJob(src, dest, permissions, false, overwrite, resume, showProgressInfo);
}

FileCopyJob *KIO::file_move(const KURL &src, const KURL &dest, int permissions, bool overwrite, bool resume, bool showProgressInfo)
{
...
void ListJob::slotListEntries(const KIO::UDSEntryList &list)
{
    ...
    if(recursive)
    {
        ...
        for(; it != end; ++it)
        {
            ...
            if(isDir && !isLink)
            {
                ...
                {
                    ListJob *job = new ListJob(itemURL, false /*no progress info!*/, true /*recursive*/, prefix + filename + "/", includeHidden);
                    Scheduler::scheduleJob(job);
                    connect(job, SIGNAL(entries(KIO::Job *, const KIO::UDSEntryList &)), SLOT(gotEntries(KIO::Job *, const KIO::UDSEntryList &)));
                    addSubjob(job);
                }
            }
        }
    ...
    else
    {
        ...
        for(; it != end; ++it)
        {
            ...
            if((prefix.isNull() || (filename != ".." && filename != ".")) && (includeHidden || (filename[0] != '.')))
...
void ListJob::start(Slave *slave)
{
    ...
    connect(slave, SIGNAL(listEntries(const KIO::UDSEntryList &)), SLOT(slotListEntries(const KIO::UDSEntryList &)));
    connect(slave, SIGNAL(totalSize(KIO::filesize_t)), SLOT(slotTotalSize(KIO::filesize_t)));
    connect(slave, SIGNAL(redirection(const KURL &)), SLOT(slotRedirection(const KURL &)));
    ...
    , m_reportTimer(0)
{
    ...
    if(showProgressInfo)
    {
        ...
        connect(this, SIGNAL(totalDirs(KIO::Job *, unsigned long)), Observer::self(), SLOT(slotTotalDirs(KIO::Job *, unsigned long)));
    }
    QTimer::singleShot(0, this, SLOT(slotStart()));
    /**
       States:
...
void CopyJob::slotResultStating(Job *job)
{
    ...
    if(bDir && !bLink     // treat symlinks as files (no recursion)
    ...
    for(; it != end; ++it)
    {
        ...
        if(displayName != ".." && displayName != ".")
        {
            ...
            if(!hasCustomURL)
            {
                ...
                { // Only if src is a directory. Otherwise uSource is fine as is
               ...
               (!(m_asMethod && state == STATE_STATING)))
            {
                ...
                if(hasCustomURL && KProtocolInfo::fileNameUsedForCopying(url) == KProtocolInfo::FromURL)
                {
                    ...
                    for(int n = 0; n < numberOfSlashes + 1; ++n)
                    {
                        ...
                        if(pos == -1)
                        { // error
                            kdWarning(7007) << "kioslave bug: not enough slashes in UDS_URL " << path << " - looking for " << numberOfSlashes
                                            << " slashes" << endl;
                            break;
                        }
                    }
...
void CopyJob::statCurrentSrc()
{
    if(m_currentStatSrc != m_srcList.end())
    {
        ...
        if(m_mode == Link)
        {
            ...
            if(destinationState == DEST_IS_DIR && !m_asMethod)
            {
                if((m_currentSrcURL.protocol() == info.uDest.protocol()) && (m_currentSrcURL.host() == info.uDest.host())
                   && (m_currentSrcURL.port() == info.uDest.port()) && (m_currentSrcURL.user() == info.uDest.user())
                                      ...
                                      || destinationState != DEST_IS_DIR || m_asMethod))
        {
            ...
            if((m_currentSrcURL.protocol() == m_dest.protocol()) && (m_currentSrcURL.host() == m_dest.host())
               && (m_currentSrcURL.port() == m_dest.port()) && (m_currentSrcURL.user() == m_dest.user()) && (m_currentSrcURL.pass() == m_dest.pass()))
            {
...
void CopyJob::startListing(const KURL &src)
{
    ...
    connect(newjob, SIGNAL(entries(KIO::Job *, const KIO::UDSEntryList &)), SLOT(slotEntries(KIO::Job *, const KIO::UDSEntryList &)));
    addSubjob(newjob);
}
...
void CopyJob::slotResultConflictCreatingDirs(KIO::Job *job)
{
    ...
    {
        if((*it).uSource == (*it).uDest || ((*it).uSource.protocol() == (*it).uDest.protocol() && (*it).uSource.path(-1) == linkDest))
    ...
    if(m_reportTimer)
        m_reportTimer->stop();
    RenameDlg_Result r = Observer::self()->open_RenameDlg(this, i18n("Folder Already Exists"), (*it).uSource.url(), (*it).uDest.url(), mode, newPath,
                                                          (*it).size, destsize, (*it).ctime, destctime, (*it).mtime, destmtime);
    if(m_reportTimer)
        m_reportTimer->start(REPORT_TIMEOUT, false);
    switch(r)
    {
        ...
        case R_RENAME:
        {
            ...
            for(; renamedirit != dirs.end(); ++renamedirit)
            {
                ...
                if(path.left(oldPath.length()) == oldPath)
                {
                    ...
                    kdDebug(7007) << "dirs list: " << (*renamedirit).uSource.path() << " was going to be " << path << ", changed into " << n << endl;
            ...
            for(; renamefileit != files.end(); ++renamefileit)
            {
                ...
                if(path.left(oldPath.length()) == oldPath)
                {
                    QString n = path;
                    n.replace(0, oldPath.length(), newPath);
                    kdDebug(7007) << "files list: " << (*renamefileit).uSource.path() << " was going to be " << path << ", changed into " << n
                                  << endl;
...
void CopyJob::createNextDir()
{
    ...
    else // we have finished creating dirs
    {
        ...
        if(m_progressId)
            Observer::self()->slotProcessedDirs(this, m_processedDirs);
...
void CopyJob::slotResultCopyingFiles(Job *job)
{
    ...
    else // no error
    {
        ...
        if(m_bCurrentOperationIsLink && m_mode == Move && !::qt_cast< KIO::DeleteJob * >(job) // Deleting source not already done
           )
        {
            subjobs.remove(job);
            assert(subjobs.isEmpty());
...
void CopyJob::slotResultConflictCopyingFiles(KIO::Job *job)
{
    ...
    if((m_conflictError == ERR_FILE_ALREADY_EXIST) || (m_conflictError == ERR_DIR_ALREADY_EXIST) || (m_conflictError == ERR_IDENTICAL_FILES))
    {
        ...
        res =
            Observer::self()->open_RenameDlg(this, !isDir ? i18n("File Already Exists") : i18n("Already Exists as Folder"), (*it).uSource.url(),
                                             (*it).uDest.url(), mode, newPath, (*it).size, destsize, (*it).ctime, destctime, (*it).mtime, destmtime);
    }
    else
    {
        ...
        {
            SkipDlg_Result skipResult = Observer::self()->open_SkipDlg(this, files.count() > 1, job->errorString());

            // Convert the return code from SkipDlg into a RenameDlg code
            res = (skipResult == S_SKIP) ? R_SKIP : (skipResult == S_AUTO_SKIP) ? R_AUTO_SKIP : R_CANCEL;
        }
    }
...
void CopyJob::copyNextFile()
{
    ...
    if(bCopyFile) // any file to create, finally ?
    {
        ...
        if(m_mode == Link)
        {
            ...
            if(((*it).uSource.protocol() == (*it).uDest.protocol()) && ((*it).uSource.host() == (*it).uDest.host())
               && ((*it).uSource.port() == (*it).uDest.port()) && ((*it).uSource.user() == (*it).uDest.user())
            ...
            else
            {
                ...
                if((*it).uDest.isLocalFile())
                {
                    ...
                    if((*it).uSource.protocol() == QString::fromLatin1("devices"))
                    {
                        ...
                        streamout << (*it).uDest;
                        if(kapp && kapp->dcopClient()->call("kded", "mountwatcher", "createLink(KURL, KURL)", param, retType, data, false))
                        {
        ...
        else if(!(*it).linkDest.isEmpty() && ((*it).uSource.protocol() == (*it).uDest.protocol()) && ((*it).uSource.host() == (*it).uDest.host())
                && ((*it).uSource.port() == (*it).uDest.port()) && ((*it).uSource.user() == (*it).uDest.user())
                && ((*it).uSource.pass() == (*it).uDest.pass()))
        ...
        connect(newjob, SIGNAL(totalSize(KIO::Job *, KIO::filesize_t)), this, SLOT(slotTotalSize(KIO::Job *, KIO::filesize_t)));
    }
    else
    {
        // We're done
...
void CopyJob::slotResultRenaming(Job *job)
{
    ...
    if(err)
    {
        ...
        if(m_currentSrcURL.isLocalFile() && m_currentSrcURL.url(-1) != dest.url(-1) && m_currentSrcURL.url(-1).lower() == dest.url(-1).lower()
           && (err == ERR_FILE_ALREADY_EXIST || err == ERR_DIR_ALREADY_EXIST || err == ERR_IDENTICAL_FILES))
        {
            kdDebug(7007) << "Couldn't rename directly, dest already exists. Detected special case of lower/uppercase renaming in same dir, try with "
                             "2 rename calls"
                          << endl;
    ...
    if(err)
    {
        ...
        // Existing dest?
        if((err == ERR_DIR_ALREADY_EXIST || err == ERR_FILE_ALREADY_EXIST || err == ERR_IDENTICAL_FILES) && isInteractive())
        {
            if(m_reportTimer)
                m_reportTimer->stop();
            ...
            else
            {
                QString newPath;
                // If src==dest, use "overwrite-itself"
                RenameDlg_Mode mode = (RenameDlg_Mode)((m_currentSrcURL == dest) ? M_OVERWRITE_ITSELF : M_OVERWRITE);
                ...
                if(m_currentSrcURL.isLocalFile() && KDE_stat(QFile::encodeName(m_currentSrcURL.path()), &stat_buf) == 0)
                {
                    sizeSrc = stat_buf.st_size;
                    ctimeSrc = stat_buf.st_ctime;
                    mtimeSrc = stat_buf.st_mtime;
                }
                if(dest.isLocalFile() && KDE_stat(QFile::encodeName(dest.path()), &stat_buf) == 0)
                {
                ...
                RenameDlg_Result r = Observer::self()->open_RenameDlg(
                    this, err != ERR_DIR_ALREADY_EXIST ? i18n("File Already Exists") : i18n("Already Exists as Folder"), m_currentSrcURL.url(),
                    dest.url(), mode, newPath, sizeSrc, sizeDest, ctimeSrc, ctimeDest, mtimeSrc, mtimeDest);
                if(m_reportTimer)
                    m_reportTimer->start(REPORT_TIMEOUT, false);
    ...
    if(!files.isEmpty() || !symlinks.isEmpty())
    {
        ...
        do
        {
            ...
            if((*it).isLocalFile() && unlink(QFile::encodeName((*it).path())) == 0)
            {
                ...
                job = 0;
                m_processedFiles++;
                if(m_processedFiles % 300 == 0 || m_totalFilesDirs < 300)
    ...
    if(!dirs.isEmpty()) // some dirs to delete ?
    {
        do
        {
            ...
            if((*it).isLocalFile() && ::rmdir(QFile::encodeName((*it).path())) == 0)
            {
                ...
                { // update progress info every 100 dirs
                    m_currentURL = *it;
                    slotReport();
                }
            }
...
MultiGetJob::MultiGetJob(const KURL &url, bool showProgressInfo) : TransferJob(url, 0, QByteArray(), QByteArray(), showProgressInfo)
{
...
void MultiGetJob::flushQueue(QPtrList< GetRequest > &queue)
{
    ...
    {
        if((m_url.protocol() == entry->url.protocol()) && (m_url.host() == entry->url.host()) && (m_url.port() == entry->url.port())
