...
bool ParseTreeIN::eval(ParseContext *_context) const
{
    ...
    if(c1.type == ParseContext::T_DOUBLE && c2.type == ParseContext::T_SEQ && (*(c2.seq.begin())).type() == QVariant::Double)
    {
        ...
        _context->b = false;
        for(; it != end; it++)
            if((*it).type() == QVariant::Double && (*it).toDouble() == c1.i)
            {
                _context->b = true;
                break;
            }
...
bool ParseTreeMIN2::eval(ParseContext *_context) const
{
    ...
    {
        _context->f = (double)(prop.toInt() - it.data().iMin) / (double)(it.data().iMax - it.data().iMin) * (-2.0) + 1.0;
        return true;
    }
    else if(prop.type() == QVariant::Double && it.data().type == PreferencesMaxima::PM_DOUBLE)
    {
        _context->f = (prop.toDouble() - it.data().fMin) / (it.data().fMax - it.data().fMin) * (-2.0) + 1.0;
        return true;
    }
...
bool ParseContext::initMaxima(const QString &_prop)
{
    ...
    if(it != maxima.end())
        return (it.data().type == PreferencesMaxima::PM_DOUBLE || it.data().type == PreferencesMaxima::PM_INT);
