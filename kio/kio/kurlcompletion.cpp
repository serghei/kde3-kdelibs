...
public:
    CompletionMatchEvent(CompletionThread *thread) : QCustomEvent(uniqueType()), m_completionThread(thread)
    {
    }
...
protected:
    CompletionThread(KURLCompletion *receiver) : QThread(), m_receiver(receiver), m_terminationRequested(false)
    {
    }
...
public:
    UserListThread(KURLCompletion *receiver) : CompletionThread(receiver)
    {
    }
...
public:
    DirectoryListThread(KURLCompletion *receiver, const QStringList &dirList, const QString &filter, bool onlyExe, bool onlyDir, bool noHidden,
...
public:
    ...
    QString dir() const
    {
        return m_kurl->directory(false, false);
    }
    QString file() const
    {
        return m_kurl->fileName(false);
    }
...
public:
    KURLCompletionPrivate() : url_auto_completion(true), userListThread(0), dirListThread(0)
    {
    }
    ~KURLCompletionPrivate();
 ...
 */
void KURLCompletion::setListedURL(int complType, const QString &dir, const QString &filter, bool no_hidden)
{
...
bool KURLCompletion::isListedURL(int complType, const QString &dir, const QString &filter, bool no_hidden)
{
    return d->last_compl_type == complType && (d->last_path_listed == dir || (dir.isEmpty() && d->last_path_listed.isEmpty()))
           && (filter.startsWith(d->last_file_listed) || (filter.isEmpty() && d->last_file_listed.isEmpty())) && d->last_no_hidden == (int)no_hidden
...
{
    return completionMode() == KGlobalSettings::CompletionAuto || completionMode() == KGlobalSettings::CompletionPopup
           || completionMode() == KGlobalSettings::CompletionMan || completionMode() == KGlobalSettings::CompletionPopupAuto;
}
//////////////////////////////////////////////////
//////////////////////////////////////////////////
// User directories
...
bool KURLCompletion::urlCompletion(const MyURL &url, QString *match)
{
    ...
    bool man_or_info = (url_dir.protocol() == QString("man") || url_dir.protocol() == QString("info"));

    if(!url_dir.isValid() || !KProtocolInfo::supportsListing(url_dir)
       || (!man_or_info && (url_dir.directory(false, false).isEmpty() || (isAutoCompletion() && !d->url_auto_completion))))
    {
        return false;
    }
 ...
 */
void KURLCompletion::listURLs(const QValueList< KURL * > &urls, const QString &filter, bool only_exe, bool no_hidden)
{
...
void KURLCompletion::slotIOFinished(KIO::Job *job)
{
    ...
    else
    {
        ...
        connect(d->list_job, SIGNAL(result(KIO::Job *)), SLOT(slotIOFinished(KIO::Job *)));

        connect(d->list_job, SIGNAL(entries(KIO::Job *, const KIO::UDSEntryList &)), SLOT(slotEntries(KIO::Job *, const KIO::UDSEntryList &)));
    ...
    while((pos = text.find('$', pos)) != -1)
    {
        ...
        else
        {
            ...
            if(pos2 >= 0)
            {
                ...
                QString value = QString::fromLocal8Bit(::getenv(key.local8Bit()));
