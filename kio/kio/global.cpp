...
KIO_EXPORT QStringList KIO::Job::detailedErrorStrings(const KURL *reqUrl /*= 0L*/, int method /*= -1*/) const
{
    ...
    ret << errorName;
    ret << QString::fromLatin1("<qt><p><b>") + errorName + QString::fromLatin1("</b></p><p>") + description + QString::fromLatin1("</p>");
...
KIO_EXPORT QByteArray KIO::rawErrorDetail(int errorCode, const QString &errorText, const KURL *reqUrl /*= 0L*/, int /*method = -1*/)
{
    ...
    if(reqUrl)
    {
        ...
        // add your hacks here...
        if(protocol == "http" || protocol == "https" || protocol == "ftp" || protocol == "sftp" || protocol == "webdav" || protocol == "webdavs"
           || protocol == "finger" || protocol == "fish" || protocol == "gopher" || protocol == "imap" || protocol == "imaps" || protocol == "lan"
           || protocol == "ldap" || protocol == "mailto" || protocol == "news" || protocol == "nntp" || protocol == "pop3" || protocol == "pop3s"
           || protocol == "smtp" || protocol == "smtps" || protocol == "telnet")
        {
    ...
    switch(errorCode)
    {
        ...
        case KIO::ERR_NOT_FILTER_PROTOCOL:
            ...
            description = i18n(
                              ...
                              "<strong>%1%2</strong>.")
                              .arg(!host.isNull() ? host + '/' : QString::null)
                              .arg(dir);
            causes << i18n(
                "KDE is able to communicate through a protocol within a "
        ...
        case KIO::ERR_COULD_NOT_AUTHENTICATE:
            errorName = i18n("Authentication Failed: Method %1 Not Supported").arg(errorText);
            description = i18n(
        ...
        case KIO::ERR_CANNOT_SYMLINK:
            ...
            description = i18n("The requested symbolic link %1 could not be created.").arg(errorText);
            causes << cAccess;
            solutions << sAccess;
            break;
...
QString KIO::findDeviceMountPoint(const QString &filename)
{
    ...
    while(getmntent(mnttab, &mnt) == 0)
    {
        ...
        if(strncmp(devname.data(), mnt.mnt_special, len) == 0
           || (strncmp(devname.data(), mnt.mnt_special, len - 3) == 0 && mnt.mnt_special[len - 3] == '/')
           || (strcmp(QFile::encodeName(filename).data(), mnt.mnt_special) == 0))
        {
...
static bool is_my_mountpoint(const char *mountpoint, const char *realname, int &max)
{
    ...
    if(!strncmp(mountpoint, realname, length) && length > max)
    {
        max = length;
 ...
 **/
static void check_mount_point(const char *mounttype, const char *fsname, MountState &isslow, MountState &isautofs)
{
...
static QString get_mount_info(const QString &filename, MountState &isautofs, MountState &isslow, MountState &ismanual, QString &fstype)
{
 ...
 */
    ...
    for(int i = 0; i < num_fs; i++)
    {
        ...
        if(is_my_mountpoint(mounted[i].f_mntonname, realname, max))
        {
            ...
            check_mount_point(mounttype, mounted[i].f_mntfromname, isautofs, isslow);
