...
KFileItem::KFileItem(const KIO::UDSEntry &_entry, const KURL &_url, bool _determineMimeTypeOnDemand, bool _urlIsDirectory)
    : m_entry(_entry)
    , m_url(_url)
    , m_pMimeType(0)
...
KFileItem::KFileItem(const KFileItem &item) : d(0)
{
    assign(item);
}
...
time_t KFileItem::time(unsigned int which, bool &hasTime) const
{
    ...
    if(m_bIsLocalURL)
    {
        ...
        if(KDE_stat(QFile::encodeName(m_url.path(-1)), &buf) == 0)
        {
            ...
            m_time[mappedWhich] = (which == KIO::UDS_MODIFICATION_TIME) ? buf.st_mtime :
...
int KFileItem::overlays() const
{
    ...
    {
        if(KSambaShare::instance()->isDirectoryShared(m_url.path()) || KNFSShare::instance()->isDirectoryShared(m_url.path()))
        {
...
QPixmap KFileItem::pixmap(int _size, int _state) const
{
    ...
    if(!m_pMimeType)
    {
        static const QString &defaultFolderIcon = KGlobal::staticQString(KMimeType::mimeType("inode/directory")->KServiceType::icon());
...
QString KFileItem::getToolTipText(int maxcount)
{
    ...
    if(!S_ISDIR(m_fileMode))
    {
        ...
        KIO::filesize_t sizeValue = size(hasSize);
        if(hasSize)
            tip += start + i18n("Size:") + mid + KIO::convertSizeWithBytes(sizeValue) + end;
    }
    ...
    if(info.isValid() && !info.isEmpty())
    {
        ...
        for(int count = 0; count < maxcount && it != keys.end(); ++it)
        {
            ...
            if(item.isValid())
            {
                ...
                if((item.attributes() & KFileMimeTypeInfo::SqueezeText) && s.length() > 50)
                {
                    s.truncate(47);
                    s.append("...");
                }
                ...
                {
                    count++;
                    tip += start + QStyleSheet::escape(item.translatedKey()) + ":" + mid + QStyleSheet::escape(s) + end;
                }
            }
        }
    }
...
bool KFileItem::cmp(const KFileItem &item)
{
    ...
    return (m_strName == item.m_strName && m_bIsLocalURL == item.m_bIsLocalURL && m_fileMode == item.m_fileMode && m_permissions == item.m_permissions
            && m_user == item.m_user && m_group == item.m_group && m_bLink == item.m_bLink && m_hidden == item.m_hidden
            && size(hasSize1) == item.size(hasSize2) && hasSize1 == hasSize2
            && time(KIO::UDS_MODIFICATION_TIME, hasTime1) == item.time(KIO::UDS_MODIFICATION_TIME, hasTime2) && hasTime1 == hasTime2
...
void KFileItem::setUDSEntry(const KIO::UDSEntry &_entry, const KURL &_url, bool _determineMimeTypeOnDemand, bool _urlIsDirectory)
{
    m_entry = _entry;
    m_url = _url;
...
QString KFileItem::parsePermissions(mode_t perm) const
{
    ...
    if(perm & QFileInfo::WriteUser)
        p[2] = 'w';
    if((perm & QFileInfo::ExeUser) && !(perm & S_ISUID))
        p[3] = 'x';
    else if((perm & QFileInfo::ExeUser) && (perm & S_ISUID))
        p[3] = 's';
    else if(!(perm & QFileInfo::ExeUser) && (perm & S_ISUID))
        p[3] = 'S';
    ...
    if(perm & QFileInfo::WriteGroup)
        p[5] = 'w';
    if((perm & QFileInfo::ExeGroup) && !(perm & S_ISGID))
        p[6] = 'x';
    else if((perm & QFileInfo::ExeGroup) && (perm & S_ISGID))
        p[6] = 's';
    else if(!(perm & QFileInfo::ExeGroup) && (perm & S_ISGID))
        p[6] = 'S';
    ...
    if(perm & QFileInfo::WriteOther)
        p[8] = 'w';
    if((perm & QFileInfo::ExeOther) && !(perm & S_ISVTX))
        p[9] = 'x';
    else if((perm & QFileInfo::ExeOther) && (perm & S_ISVTX))
        p[9] = 't';
    else if(!(perm & QFileInfo::ExeOther) && (perm & S_ISVTX))
        p[9] = 'T';
...
const KFileMetaInfo &KFileItem::metaInfo(bool autoget, int) const
{
    ...
    if(autoget && !m_metaInfo.isValid() && KGlobalSettings::showFilePreview(url))
    {
