...
static void transformToMsDos(const QDateTime &dt, char *buffer)
{
    if(dt.isValid())
    {
        const Q_UINT16 time = (dt.time().hour() << 11)     // 5 bit hour
        ...
        const Q_UINT16 date = ((dt.date().year() - 1980) << 9) // 7 bit year 1980-based
...
struct ParseFileInfo
{
    ...
    ParseFileInfo() : perm(0100644), uid(-1), gid(-1), extralen(0), exttimestamp_seen(false), newinfounix_seen(false)
    {
...
static bool parseExtTimestamp(const char *buffer, int size, bool islocal, ParseFileInfo &pfi)
{
    ...
    { // contains modification time
        ...
        pfi.mtime = time_t((uchar)buffer[0] | (uchar)buffer[1] << 8 | (uchar)buffer[2] << 16 | (uchar)buffer[3] << 24);
        buffer += 4;
        size -= 4;
    } /*end if*/
    ...
    if(flags & 2)
    { // contains last access time
        if(size < 4)
        {
            kdDebug(7040) << "premature end of extended timestamp (#3)" << endl;
            return true;
        } /*end if*/
        pfi.atime = time_t((uchar)buffer[0] | (uchar)buffer[1] << 8 | (uchar)buffer[2] << 16 | (uchar)buffer[3] << 24);
        buffer += 4;
        size -= 4;
    } /*end if*/
    ...
    { // contains creation time
        ...
        pfi.ctime = time_t((uchar)buffer[0] | (uchar)buffer[1] << 8 | (uchar)buffer[2] << 16 | (uchar)buffer[3] << 24);
        buffer += 4;
    } /*end if*/
...
static bool parseExtraField(const char *buffer, int size, bool islocal, ParseFileInfo &pfi)
{
    ...
    while(size >= 4)
    { // as long as a potential extra field can be read
...
public:
    KZipPrivate() : m_crc(0), m_currentFile(0L), m_currentDev(0L), m_compression(8), m_extraField(KZip::NoExtraField), m_offset(0L), m_saveFile(0)
    {
    }
...
KZip::KZip(const QString &filename) : KArchive(0L)
{
...
KZip::KZip(QIODevice *dev) : KArchive(dev)
{
...
KZip::~KZip()
{
    ...
    {                     // we created the device ourselves
...
bool KZip::openArchive(int mode)
{
    ...
    for(;;) // repeat until 'end of entries' signature is reached
    {
        ...
        if(!memcmp(buffer, "PK\3\4", 4)) // local file header
        {
            ...
            Q_LONG compr_size = (uchar)buffer[12] | (uchar)buffer[13] << 8 | (uchar)buffer[14] << 16 | (uchar)buffer[15] << 24;
            Q_LONG uncomp_size = (uchar)buffer[16] | (uchar)buffer[17] << 8 | (uchar)buffer[18] << 16 | (uchar)buffer[19] << 24;
            ...
            if(gpf & 8)
            {
                ...
                while(!foundSignature)
                {
                    ...
                    else if((buffer[0] == 'K' && buffer[1] == 1 && buffer[2] == 2) || (buffer[0] == 'K' && buffer[1] == 3 && buffer[2] == 4))
                    {
                        foundSignature = true;
                ...
                else
                {
                    ...
                    if(compr_size > (Q_LONG)dev->size())
                    {
                        ...
                        while(!foundSignature)
                        {
                            ...
                            if((buffer[0] == 'K' && buffer[1] == 1 && buffer[2] == 2) || (buffer[0] == 'K' && buffer[1] == 3 && buffer[2] == 4))
                            {
                                foundSignature = true;
                                dev->at(dev->at() - 4);
        ...
        else if(!memcmp(buffer, "PK\1\2", 4)) // central block
        {
            ...
            else
            {
                ...
                entry = new KZipFileEntry(this, entryName, access, pfi->mtime, rootDir()->user(), rootDir()->group(), symlink, name, dataoffset,
...
bool KZip::writeFile(const QString &name, const QString &user, const QString &group, uint size, const char *data)
{
    ...
    return KArchive::writeFile(name, user, group, size, mode, the_time, the_time, the_time, data);
}

// Doesn't need to be reimplemented anymore. Remove for KDE-4.0
bool KZip::writeFile(const QString &name, const QString &user, const QString &group, uint size, mode_t perm, time_t atime, time_t mtime, time_t ctime,
                     const char *data)
{
    return KArchive::writeFile(name, user, group, size, perm, atime, mtime, ctime, data);
}
...
bool KZip::prepareWriting(const QString &name, const QString &user, const QString &group, uint size)
{
    ...
    time_t the_time = time(0);
    return prepareWriting(name, user, group, size, dflt_perm, the_time, the_time, the_time);
}

// Doesn't need to be reimplemented anymore. Remove for KDE-4.0
bool KZip::prepareWriting(const QString &name, const QString &user, const QString &group, uint size, mode_t perm, time_t atime, time_t mtime,
                          time_t ctime)
{
    return KArchive::prepareWriting(name, user, group, size, perm, atime, mtime, ctime);
}

bool KZip::prepareWriting_impl(const QString &name, const QString &user, const QString &group, uint /*size*/, mode_t perm, time_t atime, time_t mtime,
                               time_t ctime)
{
    ...
    KZipFileEntry *e = new KZipFileEntry(this, fileName, perm, mtime, user, group, QString::null, name, device()->at() + 30 + name.length(), // start
...
bool KZip::doneWriting(uint size)
{
    ...
    int csize = device()->at() - d->m_currentFile->headerStart() - 30 - d->m_currentFile->path().length() - extra_field_len;
...
bool KZip::writeSymLink(const QString &name, const QString &target, const QString &user, const QString &group, mode_t perm, time_t atime,
                        time_t mtime, time_t ctime)
{
    return KArchive::writeSymLink(name, target, user, group, perm, atime, mtime, ctime);
}

bool KZip::writeSymLink_impl(const QString &name, const QString &target, const QString &user, const QString &group, mode_t perm, time_t atime,
                             time_t mtime, time_t ctime)
{
...
void KZip::virtual_hook(int id, void *data)
{
    switch(id)
    {
        ...
        case VIRTUAL_WRITE_SYMLINK:
        {
            ...
            params->retval = writeSymLink_impl(*params->name, *params->target, *params->user, *params->group, params->perm, params->atime,
                                               params->mtime, params->ctime);
            break;
        }
        case VIRTUAL_PREPARE_WRITING:
        {
            PrepareWritingParams *params = reinterpret_cast< PrepareWritingParams * >(data);
            params->retval = prepareWriting_impl(*params->name, *params->user, *params->group, params->size, params->perm, params->atime,
                                                 params->mtime, params->ctime);
            break;
        }
        default:
...
QIODevice *KZipFileEntry::device() const
{
    ...
    kdError() << "This zip file contains files compressed with method " << encoding() << ", this method is currently not supported by KZip,"
