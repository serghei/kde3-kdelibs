...
{
    s << a.url << a.username << a.password << a.prompt << a.caption << a.comment << a.commentLabel << a.realmValue << a.digestInfo
      << Q_UINT8(a.verifyPath ? 1 : 0) << Q_UINT8(a.readOnly ? 1 : 0) << Q_UINT8(a.keepPassword ? 1 : 0) << Q_UINT8(a.modified ? 1 : 0);
    return s;
}
...
QDataStream &KIO::operator>>(QDataStream &s, AuthInfo &a)
{
    ...
    s >> a.url >> a.username >> a.password >> a.prompt >> a.caption >> a.comment >> a.commentLabel >> a.realmValue >> a.digestInfo >> verify >> ro
        >> keep >> mod;
...
bool NetRC::lookup(const KURL &url, AutoLogin &login, bool userealnetrc, QString type, int mode)
{
    ...
    for(LoginList::Iterator it = l.begin(); it != l.end(); ++it)
    {
        ...
        if((mode & defaultOnly) == defaultOnly && log.machine == QString::fromLatin1("default")
        ...
        if((mode & presetOnly) == presetOnly && log.machine == QString::fromLatin1("preset") && (login.login.isEmpty() || login.login == log.login))
        {
            login.type = log.type;
            login.machine = log.machine;
            login.login = log.login;
