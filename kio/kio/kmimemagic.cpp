...
static const asc_type types[] = {{"text/html", 19, 2}, // 10 items but 10 different words only
                                 {"text/x-c", 13, 1},     {"text/x-makefile", 4, 1.9}, {"text/x-pli", 1, 3},  {"text/x-assembler", 6, 2.1},
                                 {"text/x-pascal", 1, 1}, {"text/x-java", 12, 1},      {"text/x-c++", 19, 1}, {"message/rfc822", 4, 1.9},
                                 {"message/news", 3, 2},  {"text/x-diff", 4, 2},       {"text/x-objc", 10, 1}};
...
static struct names
{
    const char *name;
    short type;
} const names[] = {{"<html", L_HTML | FLAG_STRONG},
...
static void test_table()
{
    ...
    {
        if(isprint((((unsigned long)m) >> 24) & 255) && isprint((((unsigned long)m) >> 16) & 255) && isprint((((unsigned long)m) >> 8) & 255)
...
#define EATAB                                                                                                                                        \
    {                                                                                                                                                \
        while(isascii((unsigned char)*l) && isspace((unsigned char)*l))                                                                              \
            ++l;                                                                                                                                     \
    }
...
int KMimeMagic::apprentice(const QString &magicfile)
{
    ...
    kdDebug(7018) << "apprentice: conf=" << conf << " file=" << magicfile << " m=" << (conf->magic ? "set" : "NULL")
                  << " m->next=" << ((conf->magic && conf->magic->next) ? "set" : "NULL") << " last=" << (conf->last ? "set" : "NULL") << endl;
...
#ifdef DEBUG_APPRENTICE
    kdDebug(7018) << "buff_apprentice: conf=" << conf << " m=" << (conf->magic ? "set" : "NULL")
                  << " m->next=" << ((conf->magic && conf->magic->next) ? "set" : "NULL") << " last=" << (conf->last ? "set" : "NULL") << endl;
...
#define NLEDATE 6
    ...
    switch(*l)
    {
        ...
        /* FALL THROUGH */
        default:
            if(*l == 'x' && isascii((unsigned char)l[1]) && isspace((unsigned char)l[1]))
            {
                m->reln = *l;
                ++l;
...
#ifdef DEBUG_APPRENTICE
    kdDebug(7018) << "parse: line=" << lineno << " m=" << m << " next=" << m->next << " cont=" << m->cont_level
                  << " desc=" << (m->desc ? m->desc : "NULL") << endl;
#endif
    return 0;
}
...
static int mconvert(union VALUETYPE *p, struct magic *m)
{
    switch(m->type)
    {
        ...
        case BELONG:
        case BEDATE:
            p->l = (long)((p->hl[0] << 24) | (p->hl[1] << 16) | (p->hl[2] << 8) | (p->hl[3]));
            return 1;
        ...
        case LELONG:
        case LEDATE:
            p->l = (long)((p->hl[3] << 24) | (p->hl[2] << 16) | (p->hl[1] << 8) | (p->hl[0]));
            return 1;
        default:
...
static int mget(union VALUETYPE *p, unsigned char *s, struct magic *m, int nbytes)
{
 ...
 */
static int match(struct config_rec *conf, unsigned char *s, int nbytes)
{
    int cont_level = 0;
    union VALUETYPE p;
    struct magic *m;
    ...
    kdDebug(7018) << "match: conf=" << conf << " m=" << (conf->magic ? "set" : "NULL")
                  << " m->next=" << ((conf->magic && conf->magic->next) ? "set" : "NULL") << " last=" << (conf->last ? "set" : "NULL") << endl;
    for(m = conf->magic; m; m = m->next)
    {
        if(isprint((((unsigned long)m) >> 24) & 255) && isprint((((unsigned long)m) >> 16) & 255) && isprint((((unsigned long)m) >> 8) & 255)
    ...
    for(m = conf->magic; m; m = m->next)
    {
        ...
        if(!mget(&p, s, m, nbytes) || !mcheck(&p, m))
        {
            ...
            {
#ifdef DEBUG_MIMEMAGIC
                kdDebug(7018) << "match: line=" << m->lineno << " cont=" << m_cont->cont_level << " mc=" << m_cont->lineno << " mc->next=" << m_cont
                              << " " << endl;
#endif
                /*
...
/* this will be the last run through the loop */
        ...
        {
#ifdef DEBUG_MIMEMAGIC
            kdDebug(7018) << "match: line=" << m->lineno << " cont=" << m->cont_level << " type=" << m->type << " "
                          << ((m->type == STRING) ? m->value.s : "") << endl;
#endif
...
static int ascmagic(struct config_rec *conf, unsigned char *buf, int nbytes)
{
    ...
    if(*buf == '.')
    {
        ...
        if((isascii(*tp) && (isalnum(*tp) || *tp == '\\') && isascii(*(tp + 1)) && (isalnum(*(tp + 1)) || *tp == '"')))
        {
            conf->resultBuf = MIME_APPL_TROFF;
            return 1;
        }
    }
    if((*buf == 'c' || *buf == 'C') && isascii(*(buf + 1)) && isspace(*(buf + 1)))
    {
        /* Fortran */
    ...
    while((token = tokenizer.nextToken())->length > 0)
    {
        ...
        for(p = names; p->name; p++)
        {
            if(STREQ(token, p->name))
            {
                ...
                {
                    if((1 << i) & p->type)
                        typecount[i] += p->type & FLAG_STRONG ? 2 : 1;
                }
            }
        }
    }
    ...
    {
        if(typecount[i] > 1)
        { // one word is not enough, we need at least two
            pct = (double)typecount[i] / (double)types[i].kwords * (double)types[i].weight;
            pcts[i] = pct;
            pctsum += pct;
            if(pct > maxpct)
            {
...
#ifdef DEBUG_MIMEMAGIC
            kdDebug(7018) << "" << types[i].type << " has " << typecount[i] << " hits, " << types[i].kwords << " kw, weight " << types[i].weight
                          << ", " << pct << " -> max = " << maxpct << "\n"
                          << endl;
#endif
        }
...
#ifdef DEBUG_MIMEMAGIC
            kdDebug(7018) << "mostaccurate=" << mostaccurate << " pcts=" << pcts[mostaccurate] << " pctsum=" << pctsum
...
bool KMimeMagic::mergeConfig(const QString &_configfile)
{
...
KMimeMagicResult *KMimeMagic::findBufferType(const QByteArray &array)
{
    unsigned char buf[HOWMANY + 1]; /* one extra for terminating '\0' */
...
static void refineResult(KMimeMagicResult *r, const QString &_filename)
{
    ...
    else if(tmp == "text/x-c++")
    {
        if(_filename.endsWith(".h") || _filename.endsWith(".hh") || _filename.endsWith(".H") || !_filename.right(4).contains('.'))
            tmp += "hdr";
        else
            tmp += "src";
        r->setMimeType(tmp);
    }
    else if(tmp == "application/x-sharedlib")
    {
...
KMimeMagicResult *KMimeMagic::findBufferFileType(const QByteArray &data, const QString &fn)
{
