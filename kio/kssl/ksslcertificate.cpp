...
KSSLCertificate *KSSLCertificate::crlFromString(QCString cert)
{
    KSSLCertificate *n = NULL;
#ifdef KSSL_HAVE_SSL
    if(cert.length() == 0)
...
KSSLCertificate::KSSLValidationList KSSLCertificate::validateVerbose(KSSLCertificate::KSSLPurpose purpose, KSSLCertificate *ca)
{
    ...
    for(QStringList::Iterator j = qsl.begin(); j != qsl.end(); ++j)
    {
        ...
        if((ksslv != KSSLCertificate::Ok) && (ksslv != KSSLCertificate::Irrelevant) && purpose == KSSLCertificate::SSLServer)
        {
...
KSSLCertificate::KSSLValidation KSSLCertificate::processError(int ec)
{
    ...
    switch(ec)
    {
        ...
        case X509_V_ERR_ERROR_IN_CRL_NEXT_UPDATE_FIELD:
            ...
            kdDebug(7029) << "KSSL apparently this is expired.  Not after: " << getNotAfter() << endl;
            break;
...
QDateTime KSSLCertificate::getQDTLastUpdate() const
{
#ifdef KSSL_HAVE_SSL
...
QDateTime KSSLCertificate::getQDTNextUpdate() const
{
#ifdef KSSL_HAVE_SSL
