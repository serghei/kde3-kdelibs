...
KSSLKeyGen::KSSLKeyGen(QWidget *parent, const char *name, bool modal) : KWizard(parent, name, modal)
{
    _idx = -1;
...
int KSSLKeyGen::generateCSR(const QString &name, const QString &pass, int bits, int e)
{
    ...
    kossl->i2d_PKCS8PrivateKey_fp(p8File.fstream(), pkey, kossl->EVP_bf_cbc(), pass.local8Bit().data(), pass.length(), 0L, 0L);
...
#ifdef KSSL_HAVE_SSL
    x << i18n("2048 (High Grade)") << i18n("1024 (Medium Grade)") << i18n("768  (Low Grade)") << i18n("512  (Low Grade)");
#else
