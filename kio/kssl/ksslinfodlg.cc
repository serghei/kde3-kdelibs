...
{
    setup(&ssl.peerInfo().getPeerCertificate(), ip, url, ssl.connectionInfo().getCipher(), ssl.connectionInfo().getCipherDescription(),
          ssl.connectionInfo().getCipherVersion(), ssl.connectionInfo().getCipherUsedBits(), ssl.connectionInfo().getCipherBits(),
...
void KSSLInfoDlg::setCertState(const QString &errorNrs)
{
    ...
    for(QStringList::ConstIterator it = errors.begin(); it != errors.end(); ++it)
    {
...
void KSSLInfoDlg::displayCert(KSSLCertificate *x)
{
    ...
    else
    {
        ...
        {
            if(x->getQDTNotAfter() > QDateTime::currentDateTime(Qt::UTC) && x->getQDTNotBefore() < QDateTime::currentDateTime(Qt::UTC))
            {
