...
int KSSL::seedWithEGD()
{
    ...
    if(m_cfg->useEGD() && !m_cfg->getEGDPath().isEmpty())
    {
        ...
        if(rc < 0)
            ...
            kdDebug(7029) << "KSSL: PRNG was seeded with " << rc << " bytes from the EGD." << endl;
    }
    else if(m_cfg->useEFile() && !m_cfg->getEGDPath().isEmpty())
    {
        ...
        if(rc < 0)
            ...
            kdDebug(7029) << "KSSL: PRNG was seeded with " << rc << " bytes from the entropy file." << endl;
    }
#endif
    return rc;
}
...
write_again:
    ...
    if(rc <= 0)
    { // OpenSSL returns 0 on error too
        int err = d->kossl->SSL_get_error(d->m_ssl, rc);
