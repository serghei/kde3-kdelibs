...
public:
    ...
    KSMIMECrypto::rc signMessage(BIO *clearText, BIO *cipherText, KSSLPKCS12 &privKey, QPtrList< KSSLCertificate > &certs, bool detached);

    KSMIMECrypto::rc encryptMessage(BIO *clearText, BIO *cipherText, KSMIMECrypto::algo algorithm, QPtrList< KSSLCertificate > &recip);

    KSMIMECrypto::rc checkSignature(BIO *clearText, BIO *signature, bool detached, QPtrList< KSSLCertificate > &recip);

    KSMIMECrypto::rc decryptMessage(BIO *cipherText, BIO *clearText, KSSLPKCS12 &privKey);
...
KSMIMECrypto::rc KSMIMECryptoPrivate::signMessage(BIO *clearText, BIO *cipherText, KSSLPKCS12 &privKey, QPtrList< KSSLCertificate > &certs,
                                                  bool detached)
{
...
KSMIMECrypto::rc KSMIMECryptoPrivate::encryptMessage(BIO *clearText, BIO *cipherText, KSMIMECrypto::algo algorithm,
...
KSMIMECrypto::rc KSMIMECryptoPrivate::checkSignature(BIO *clearText, BIO *signature, bool detached, QPtrList< KSSLCertificate > &recip)
{
...
KSMIMECrypto::rc KSMIMECryptoPrivate::decryptMessage(BIO *cipherText, BIO *clearText, KSSLPKCS12 &privKey)
{
    ...
    if(kossl->PKCS7_decrypt(p7, privKey.getPrivateKey(), privKey.getCertificate()->getCert(), clearText, 0))
    {
        rc = KSMIMECrypto::KSC_R_OK;
    }
    else
...
KSMIMECrypto::rc KSMIMECryptoPrivate::sslErrToRc(void)
{
    ...
    kdDebug(7029) << "KSMIMECrypto: uncaught error " << ERR_GET_LIB(cerr) << " " << ERR_GET_REASON(cerr) << endl;
...
KSMIMECrypto::rc KSMIMECrypto::signMessage(const QCString &clearText, QByteArray &cipherText, const KSSLPKCS12 &privKey,
                                           const QPtrList< KSSLCertificate > &certs, bool detached)
{
#ifdef KSSL_HAVE_SSL
    if(!kossl)
        return KSC_R_NO_SSL;
    ...
    rc rc = priv->signMessage(in, out, const_cast< KSSLPKCS12 & >(privKey), const_cast< QPtrList< KSSLCertificate > & >(certs), detached);

    if(!rc)
        priv->MemBIOToQByteArray(out, cipherText);
...
KSMIMECrypto::rc KSMIMECrypto::checkOpaqueSignature(const QByteArray &signedText, QCString &clearText, QPtrList< KSSLCertificate > &foundCerts)
{
#ifdef KSSL_HAVE_SSL
    if(!kossl)
        return KSC_R_NO_SSL;
...
KSMIMECrypto::rc KSMIMECrypto::encryptMessage(const QCString &clearText, QByteArray &cipherText, algo algorithm,
                                              const QPtrList< KSSLCertificate > &recip)
{
    ...
    rc rc = priv->encryptMessage(in, out, algorithm, const_cast< QPtrList< KSSLCertificate > & >(recip));
...
KSMIMECrypto::rc KSMIMECrypto::decryptMessage(const QByteArray &cipherText, QCString &clearText, const KSSLPKCS12 &privKey)
{
#ifdef KSSL_HAVE_SSL
    if(!kossl)
        return KSC_R_NO_SSL;
    ...
    rc rc = priv->decryptMessage(in, out, const_cast< KSSLPKCS12 & >(privKey));
