...
KDirSelectDialog::KDirSelectDialog(const QString &startDir, bool localOnly, QWidget *parent, const char *name, bool modal)
    : KDialogBase(parent, name, modal, i18n("Select Folder"), Ok | Cancel | User1, Ok, false, KGuiItem(i18n("New Folder..."), "folder_new"))
    , m_localOnly(localOnly)
{
    ...
    connect(d->speedBar, SIGNAL(activated(const KURL &)), SLOT(setCurrentURL(const KURL &)));
    ...
    connect(d->urlCombo, SIGNAL(textChanged(const QString &)), SLOT(slotComboTextChanged(const QString &)));
    ...
    m_showHiddenFolders = new KToggleAction(i18n("Show Hidden Folders"), 0, this, SLOT(slotShowHiddenFoldersToggled()), this);
    ...
    connect(m_treeView, SIGNAL(currentChanged(QListViewItem *)), SLOT(slotCurrentChanged()));
    connect(m_treeView, SIGNAL(contextMenu(KListView *, QListViewItem *, const QPoint &)),
            SLOT(slotContextMenu(KListView *, QListViewItem *, const QPoint &)));

    connect(d->urlCombo, SIGNAL(activated(const QString &)), SLOT(slotURLActivated(const QString &)));
    connect(d->urlCombo, SIGNAL(returnPressed(const QString &)), SLOT(slotURLActivated(const QString &)));
...
void KDirSelectDialog::setCurrentURL(const KURL &url)
{
    ...
    d->startURL = url;
    if(!d->branch || url.protocol() != d->branch->url().protocol() || url.host() != d->branch->url().host())
    {
        if(d->branch)
        {
    ...
    d->branch->disconnect(SIGNAL(populateFinished(KFileTreeViewItem *)), this, SLOT(slotNextDirToList(KFileTreeViewItem *)));
    connect(d->branch, SIGNAL(populateFinished(KFileTreeViewItem *)), SLOT(slotNextDirToList(KFileTreeViewItem *)));
...
{
    KConfigGroup conf(config, group);
    conf.writePathEntry("History Items", d->urlCombo->historyItems(), ',', true, true);
...
// static
KURL KDirSelectDialog::selectDirectory(const QString &startDir, bool localOnly, QWidget *parent, const QString &caption)
{
    KDirSelectDialog myDialog(startDir, localOnly, parent, "kdirselect dialog", true);
