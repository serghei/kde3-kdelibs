...
KFileDialog::KFileDialog(const QString &startDir, const QString &filter, QWidget *parent, const char *name, bool modal)
    : KDialogBase(parent, name, modal, QString::null, 0)
{
    init(startDir, filter, 0);
}

KFileDialog::KFileDialog(const QString &startDir, const QString &filter, QWidget *parent, const char *name, bool modal, QWidget *widget)
    : KDialogBase(parent, name, modal, QString::null, 0)
{
...
// deprecated
void KFileDialog::setFilterMimeType(const QString &label, const KMimeType::List &types, const KMimeType::Ptr &defaultType)
{
    d->mimetypes.clear();
...
void KFileDialog::setMimeFilter(const QStringList &mimeTypes, const QString &defaultType)
{
    ...
    filterWidget->setEditable(!d->hasDefaultFilter || d->operationMode != Saving);
...
void KFileDialog::slotOk()
{
    ...
    if(items && !locationEdit->lineEdit()->edited() && !(items->isEmpty() && !dirOnly))
    {
        ...
        if((mode() & KFile::LocalOnly) == KFile::LocalOnly && !url.isLocalFile())
        {
            // ### after message freeze, add message for directories!
            KMessageBox::sorry(d->mainWidget, i18n("You can only select local files."), i18n("Remote Files Not Accepted"));
            return;
        }
    ...
    if((mode() & KFile::Directory) == KFile::Directory)
    {
        ...
        if(d->url.isLocalFile())
        {
            if(locationEdit->currentText().stripWhiteSpace().isEmpty())
            {
                ...
                { // d->url is not a directory,
                    // maybe we are in File(s) | Directory mode
                    if((mode() & KFile::File) == KFile::File || (mode() & KFile::Files) == KFile::Files)
                        done = false;
                }
            }
        ...
        { // FIXME: remote directory, should we allow that?
    ...
    if((mode() & KFile::Files) == KFile::Files && !locationEdit->currentText().contains('/'))
    {
...
static bool isDirectory(const KIO::UDSEntry &t)
{
    ...
    for(KIO::UDSEntry::ConstIterator it = t.begin(); it != t.end(); it++)
    {
...
void KFileDialog::setLocationText(const QString &text)
{
    ...
    disconnect(locationEdit, SIGNAL(textChanged(const QString &)), this, SLOT(slotLocationChanged(const QString &)));
    locationEdit->setCurrentItem(0);
    connect(locationEdit, SIGNAL(textChanged(const QString &)), SLOT(slotLocationChanged(const QString &)));
...
void KFileDialog::updateLocationWhatsThis(void)
{
    ...
    {
        whatsThisText = "<qt>" + i18n("This is the name to save the file as.") + i18n(autocompletionWhatsThisText);
    }
    else if(ops->mode() & KFile::Files)
    {
        whatsThisText = "<qt>" + i18n(
    ...
    {
        whatsThisText = "<qt>" + i18n("This is the name of the file to open.") + i18n(autocompletionWhatsThisText);
    }
...
void KFileDialog::init(const QString &startDir, const QString &filter, QWidget *widget)
{
    ...
    d->pathCombo = new KURLComboBox(KURLComboBox::Directories, true, toolbar, "path combo");
    ...
    QWhatsThis::add(d->pathCombo, "<qt>" + i18n("This is the currently listed location. "
                                                ...
                                                "locations that have been visited recently.")
                                      + i18n(autocompletionWhatsThisText));

    KURL u;
    u.setPath(QDir::rootDirPath());
    QString text = i18n("Root Folder: %1").arg(u.path());
    d->pathCombo->addDefaultURL(u, KMimeType::pixmapForURL(u, 0, KIcon::Small), text);
    ...
    if((u.path(+1) != docPath.path(+1)) && QDir(docPath.path(+1)).exists())
    {
        text = i18n("Documents: %1").arg(docPath.path(+1));
        d->pathCombo->addDefaultURL(docPath, KMimeType::pixmapForURL(docPath, 0, KIcon::Small), text);
    }
    ...
    connect(ops, SIGNAL(urlEntered(const KURL &)), SLOT(urlEntered(const KURL &)));
    connect(ops, SIGNAL(fileHighlighted(const KFileItem *)), SLOT(fileHighlighted(const KFileItem *)));
    connect(ops, SIGNAL(fileSelected(const KFileItem *)), SLOT(fileSelected(const KFileItem *)));
    connect(ops, SIGNAL(finishedLoading()), SLOT(slotLoadingFinished()));

    ops->setupMenu(KDirOperator::SortActions | KDirOperator::FileActions | KDirOperator::ViewActions);
    ...
    KToggleAction *showSidebarAction = new KToggleAction(i18n("Show Quick Access Navigation Panel"), Key_F9, coll, "toggleSpeedbar");
    showSidebarAction->setCheckedState(i18n("Hide Quick Access Navigation Panel"));
    connect(showSidebarAction, SIGNAL(toggled(bool)), SLOT(toggleSpeedbar(bool)));

    KToggleAction *showBookmarksAction = new KToggleAction(i18n("Show Bookmarks"), 0, coll, "toggleBookmarks");
    showBookmarksAction->setCheckedState(i18n("Hide Bookmarks"));
    connect(showBookmarksAction, SIGNAL(toggled(bool)), SLOT(toggleBookmarks(bool)));

    KActionMenu *menu = new KActionMenu(i18n("Configure"), "configure", this, "extra menu");
    menu->setWhatsThis(
        i18n("<qt>This is the configuration menu for the file dialog. "
    ...
    connect(menu->popupMenu(), SIGNAL(aboutToShow()), ops, SLOT(updateSelectionDependentActions()));
    menu->plug(toolbar);

    // Insert a separator.
    KToolBarSeparator *spacerWidget = new KToolBarSeparator(Horizontal, false /*no line*/, toolbar);
    ...
    connect(d->pathCombo, SIGNAL(urlActivated(const KURL &)), this, SLOT(enterURL(const KURL &)));
    connect(d->pathCombo, SIGNAL(returnPressed(const QString &)), this, SLOT(enterURL(const QString &)));

    QString whatsThisText;

    // the Location label/edit
    d->locationLabel = new QLabel(i18n("&Location:"), d->mainWidget);
    locationEdit = new KURLComboBox(KURLComboBox::Files, true, d->mainWidget, "LocationEdit");
    connect(locationEdit, SIGNAL(textChanged(const QString &)), SLOT(slotLocationChanged(const QString &)));
    ...
    connect(fileCompletionObj, SIGNAL(match(const QString &)), SLOT(fileCompletion(const QString &)));

    connect(locationEdit, SIGNAL(returnPressed()), this, SLOT(slotOk()));
    connect(locationEdit, SIGNAL(activated(const QString &)), this, SLOT(locationActivated(const QString &)));
    ...
    filterWidget = new KFileFilterCombo(d->mainWidget, "KFileDialog::filterwidget");
...
void KFileDialog::initSpeedbar()
{
    ...
    connect(d->urlBar, SIGNAL(activated(const KURL &)), SLOT(enterURL(const KURL &)));
...
QString KFileDialog::getOpenFileName(const QString &startDir, const QString &filter, QWidget *parent, const QString &caption)
{
...
QString KFileDialog::getOpenFileNameWId(const QString &startDir, const QString &filter, WId parent_id, const QString &caption)
{
...
QStringList KFileDialog::getOpenFileNames(const QString &startDir, const QString &filter, QWidget *parent, const QString &caption)
{
    KFileDialog dlg(startDir, filter, parent, "filedialog", true);
    dlg.setOperationMode(Opening);
...
KURL KFileDialog::getOpenURL(const QString &startDir, const QString &filter, QWidget *parent, const QString &caption)
{
    KFileDialog dlg(startDir, filter, parent, "filedialog", true);
    dlg.setOperationMode(Opening);
...
KURL::List KFileDialog::getOpenURLs(const QString &startDir, const QString &filter, QWidget *parent, const QString &caption)
{
    KFileDialog dlg(startDir, filter, parent, "filedialog", true);
    dlg.setOperationMode(Opening);
...
KURL KFileDialog::getExistingURL(const QString &startDir, QWidget *parent, const QString &caption)
{
...
KURL KFileDialog::getImageOpenURL(const QString &startDir, QWidget *parent, const QString &caption)
{
    ...
    KFileDialog dlg(startDir, mimetypes.join(" "), parent, "filedialog", true);
    dlg.setOperationMode(Opening);
    dlg.setCaption(caption.isNull() ? i18n("Open") : caption);
    dlg.setMode(KFile::File);
...
KURL::List &KFileDialog::parseSelectedURLs() const
{
    ...
    { // assume _one_ absolute filename
        ...
        if(u.isValid())
            ...
            KMessageBox::error(d->mainWidget, i18n("The chosen filenames do not\n"
                                                   "appear to be valid."),
...
QString KFileDialog::getSaveFileName(const QString &dir, const QString &filter, QWidget *parent, const QString &caption)
{
...
QString KFileDialog::getSaveFileNameWId(const QString &dir, const QString &filter, WId parent_id, const QString &caption)
{
    bool specialDir = dir.at(0) == ':';
    QWidget *parent = QWidget::find(parent_id);
    KFileDialog dlg(specialDir ? dir : QString::null, filter, parent, "filedialog", true);
...
KURL KFileDialog::getSaveURL(const QString &dir, const QString &filter, QWidget *parent, const QString &caption)
{
    bool specialDir = dir.at(0) == ':';
    KFileDialog dlg(specialDir ? dir : QString::null, filter, parent, "filedialog", true);
    if(!specialDir)
...
void KFileDialog::readConfig(KConfig *kc, const QString &group)
{
    ...
    combo->setMaxItems(kc->readNumEntry(RecentURLsNumber, DefaultRecentURLsNumber));
    combo->setURL(ops->url());
    autoDirectoryFollowing = kc->readBoolEntry(AutoDirectoryFollowing, DefaultDirectoryFollowing);

    KGlobalSettings::Completion cm = (KGlobalSettings::Completion)kc->readNumEntry(PathComboCompletionMode, KGlobalSettings::completionMode());
    if(cm != KGlobalSettings::completionMode())
        combo->setCompletionMode(cm);

    cm = (KGlobalSettings::Completion)kc->readNumEntry(LocationComboCompletionMode, KGlobalSettings::completionMode());
    if(cm != KGlobalSettings::completionMode())
...
void KFileDialog::readRecentFiles(KConfig *kc)
{
    ...
    locationEdit->setMaxItems(kc->readNumEntry(RecentFilesNumber, DefaultRecentURLsNumber));
    locationEdit->setURLs(kc->readPathListEntry(RecentFiles), KURLComboBox::RemoveBottom);
...
{
    kdDebug(kfile_area) << "slotAutoSelectExtClicked(): " << d->autoSelectExtCheckBox->isChecked() << endl;
...
static QString getExtensionFromPatternList(const QStringList &patternList)
{
    ...
    for(QStringList::ConstIterator it = patternList.begin(); it != patternListEnd; it++)
    {
        ...
        // *.JP*G
        // *.JP?
        if((*it).startsWith("*.") && (*it).length() > 2 && (*it).find('*', 2) < 0 && (*it).find('?', 2) < 0)
        {
            ret = (*it).mid(1);
            break;
        }
    ...
    if((operationMode() == Saving) && (mode() & KFile::File))
    {
        ...
        if(!filter.isEmpty())
        {
            ...
            if(filter.find('/') < 0)
            {
                ...
                kdDebug(kfile_area) << "\tsetFilter-style: pattern ext=\'" << d->extension << "\'" << endl;
            }
            // e.g. "text/html"
            else
            {
                ...
                if(nativeExtension.at(0) == '.')
                {
                    ...
                    kdDebug(kfile_area) << "\tsetMimeFilter-style: native ext=\'" << d->extension << "\'" << endl;
                }
                ...
                if(d->extension.isEmpty())
                {
                    ...
                    kdDebug(kfile_area) << "\tsetMimeFilter-style: pattern ext=\'" << d->extension << "\'" << endl;
                }
            }
        }
        ...
        QWhatsThis::add(d->autoSelectExtCheckBox, "<qt>"
...
void KFileDialog::updateFilter()
{
    if((operationMode() == Saving) && (mode() & KFile::File))
    {
        ...
        {
            if(filterWidget->currentFilter() != mime->name() && filterWidget->filters.findIndex(mime->name()) != -1)
            {
...
void KFileDialog::toggleBookmarks(bool show)
{
    if(show)
    {
        ...
        connect(d->bookmarkHandler, SIGNAL(openURL(const QString &)), SLOT(enterURL(const QString &)));

        toolbar->insertButton(QString::fromLatin1("bookmark"), (int)HOTLIST_BUTTON, true, i18n("Bookmarks"), 5);
        toolbar->getButton(HOTLIST_BUTTON)->setPopup(d->bookmarkHandler->menu(), true);
        QWhatsThis::add(toolbar->getButton(HOTLIST_BUTTON), i18n("<qt>This button allows you to bookmark specific locations. "
...
// static
KURL KFileDialog::getStartURL(const QString &startDir, QString &recentDirClass)
{
    initStatic();
    ...
    if(useDefaultStartDir)
    {
        if(lastDirectory->isEmpty())
        {
            ...
            // does not exist
            if(lastDirectory->path(+1) == home.path(+1) || QDir::currentDirPath() != QDir::homeDirPath() || !QDir(lastDirectory->path(+1)).exists())
