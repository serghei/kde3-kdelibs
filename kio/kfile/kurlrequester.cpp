...
public:
    KURLDragPushButton(QWidget *parent, const char *name = 0) : KPushButton(parent, name)
    {
        setDragEnabled(true);
    }
    ~KURLDragPushButton()
...
public:
    ...
    void connectSignals(QObject *receiver)
    {
        ...
        connect(sender, SIGNAL(textChanged(const QString &)), receiver, SIGNAL(textChanged(const QString &)));
        connect(sender, SIGNAL(returnPressed()), receiver, SIGNAL(returnPressed()));
        connect(sender, SIGNAL(returnPressed(const QString &)), receiver, SIGNAL(returnPressed(const QString &)));
    }
...
KURLRequester::KURLRequester(QWidget *editWidget, QWidget *parent, const char *name) : QHBox(parent, name)
{
...
KURLRequester::KURLRequester(QWidget *parent, const char *name) : QHBox(parent, name)
{
    d = new KURLRequesterPrivate;
    init();
}
...
KURLRequester::KURLRequester(const QString &url, QWidget *parent, const char *name) : QHBox(parent, name)
{
    d = new KURLRequesterPrivate;
    init();
...
KFileDialog *KURLRequester::fileDialog() const
{
    if(!myFileDialog)
    {
        ...
        myFileDialog = new KFileDialog(QString::null, d->fileDialogFilter, p, "file dialog", true);
...
KURLComboRequester::KURLComboRequester(QWidget *parent, const char *name) : KURLRequester(new KComboBox(false), parent, name)
{
}
