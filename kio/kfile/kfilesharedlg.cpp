...
KFileSharePropsPlugin::KFileSharePropsPlugin(KPropertiesDialog *_props) : KPropsDlgPlugin(_props)
{
    d = new Private;
...
void KFileSharePropsPlugin::init()
{
    ...
    switch(KFileShare::authorization())
    {
        case KFileShare::Authorized:
        {
            ...
            if(!ok)
            {
                vbox->addWidget(new QLabel(i18n("Only folders in your home folder can be shared."), m_widget), 0);
            }
            else
            {
        ...
        case KFileShare::ErrorNotFound:
            vbox->addWidget(new QLabel(i18n("Error running 'filesharelist'. Check if installed and in $PATH or /usr/sbin."), m_widget), 0);
            break;
        case KFileShare::UserNotAllowed:
        {
            ...
            {
                vbox->addWidget(new QLabel(i18n("You need to be authorized to share folders."), m_widget), 0);
            }
            else
            {
                vbox->addWidget(new QLabel(i18n("File sharing is disabled."), m_widget), 0);
            }
...
void KFileSharePropsPlugin::slotConfigureFileSharing()
{
    ...
    connect(d->m_configProc, SIGNAL(processExited(KProcess *)), this, SLOT(slotConfigureFileSharingDone()));
...
void KFileSharePropsPlugin::applyChanges()
{
    ...
    if(m_rbShare && m_rbUnShare)
    {
        ...
        for(; it.current() && ok; ++it)
        {
            ...
            {
                if(share)
                    KMessageBox::detailedError(properties, i18n("Sharing folder '%1' failed.").arg(path),
                                               i18n("An error occurred while trying to share folder '%1'. "
                                                    "Make sure that the Perl script 'fileshareset' is set suid root.")
                                                   .arg(path));
                else
                    KMessageBox::error(properties, i18n("Unsharing folder '%1' failed.").arg(path),
