...
public:
    KFileIconViewPrivate(KFileIconView *parent)
    {
        ...
        smallColumns = new KRadioAction(i18n("Small Icons"), 0, parent, SLOT(slotSmallColumns()), parent->actionCollection(), "small columns");

        largeRows = new KRadioAction(i18n("Large Icons"), 0, parent, SLOT(slotLargeRows()), parent->actionCollection(), "large rows");
...
KFileIconView::KFileIconView(QWidget *parent, const char *name) : KIconView(parent, name), KFileView()
{
    ...
    connect(this, SIGNAL(contextMenuRequested(QIconViewItem *, const QPoint &)), SLOT(slotActivateMenu(QIconViewItem *, const QPoint &)));
...
void KFileIconView::writeConfig(KConfig *kc, const QString &group)
{
    ...
    QString viewMode = d->smallColumns->isChecked() ? QString::fromLatin1("SmallColumns") : QString::fromLatin1("LargeRows");
...
void KFileIconView::showToolTip(QIconViewItem *item)
{
    ...
    int w = maxItemWidth() - (itemTextPos() == QIconView::Bottom ? 0 : item->pixmapRect().width()) - 4;
...
void KFileIconView::showPreviews()
{
    ...
    connect(d->job, SIGNAL(gotPreview(const KFileItem *, const QPixmap &)), SLOT(gotPreview(const KFileItem *, const QPixmap &)));
...
void KFileIconView::setSorting(QDir::SortSpec spec)
{
    ...
    {
        for(; (item = it.current()); ++it)
            viewItem(item)->setKey(sortingKey(item->size(), item->isDir(), spec));
    }
    else
    { // Name or Unsorted
        for(; (item = it.current()); ++it)
            viewItem(item)->setKey(sortingKey(item->text(), item->isDir(), spec));
    }
...
void KFileIconView::initItem(KFileIconViewItem *item, const KFileItem *i, bool updateTextAndPixmap)
{
...
{
    return KURLDrag::canDecode(e) && (e->source() != const_cast< KFileIconView * >(this))
