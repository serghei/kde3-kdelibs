...
mode_t KFilePermissionsPropsPlugin::fperm[3][4] = {{S_IRUSR, S_IWUSR, S_IXUSR, S_ISUID},
...
KPropertiesDialog::KPropertiesDialog(KFileItem *item, QWidget *parent, const char *name, bool modal, bool autoShow)
    : KDialogBase(KDialogBase::Tabbed, i18n("Properties for %1").arg(KIO::decodeFileName(item->url().fileName())),
                  KDialogBase::Ok | KDialogBase::Cancel, KDialogBase::Ok, parent, name, modal)
{
...
KPropertiesDialog::KPropertiesDialog(const QString &title, QWidget *parent, const char *name, bool modal)
    : KDialogBase(KDialogBase::Tabbed, i18n("Properties for %1").arg(title), KDialogBase::Ok | KDialogBase::Cancel, KDialogBase::Ok, parent, name,
                  modal)
{
    d = new KPropertiesDialogPrivate;
...
KPropertiesDialog::KPropertiesDialog(KFileItemList _items, QWidget *parent, const char *name, bool modal, bool autoShow)
...
KPropertiesDialog::KPropertiesDialog(const KURL &_url, mode_t /* _mode is now unused */, QWidget *parent, const char *name, bool modal, bool autoShow)
    : KDialogBase(KDialogBase::Tabbed, i18n("Properties for %1").arg(KIO::decodeFileName(_url.fileName())), KDialogBase::Ok | KDialogBase::Cancel,
...
KPropertiesDialog::KPropertiesDialog(const KURL &_url, QWidget *parent, const char *name, bool modal, bool autoShow)
    : KDialogBase(KDialogBase::Tabbed, i18n("Properties for %1").arg(KIO::decodeFileName(_url.fileName())), KDialogBase::Ok | KDialogBase::Cancel,
                  KDialogBase::Ok, parent, name, modal)
    , m_singleUrl(_url)
{
...
KPropertiesDialog::KPropertiesDialog(const KURL &_tempUrl, const KURL &_currentDir, const QString &_defaultName, QWidget *parent, const char *name,
                                     bool modal, bool autoShow)
    : KDialogBase(KDialogBase::Tabbed, i18n("Properties for %1").arg(KIO::decodeFileName(_tempUrl.fileName())), KDialogBase::Ok | KDialogBase::Cancel,
                  KDialogBase::Ok, parent, name, modal)
    ,
...
bool KPropertiesDialog::showDialog(const KFileItemList &_items, QWidget *parent, const char *name, bool modal)
{
    if(_items.count() == 1)
...
bool KPropertiesDialog::canDisplay(KFileItemList _items)
{
    ...
    return KFilePropsPlugin::supports(_items) || KFilePermissionsPropsPlugin::supports(_items) || KDesktopPropsPlugin::supports(_items)
           || KBindingPropsPlugin::supports(_items) || KURLPropsPlugin::supports(_items) || KDevicePropsPlugin::supports(_items)
           || KFileMetaPropsPlugin::supports(_items) || KPreviewPropsPlugin::supports(_items);
}
...
void KPropertiesDialog::insertPages()
{
    ...
    if(kapp->authorizeKAction("sharefile") && KFileSharePropsPlugin::supports(m_items))
    {
    ...
    for(; it != end; ++it)
    {
        KPropsDlgPlugin *plugin =
            KParts::ComponentFactory::createInstanceFromLibrary< KPropsDlgPlugin >((*it)->library().local8Bit().data(), this, (*it)->name().latin1());
        if(!plugin)
            continue;
...
void KPropertiesDialog::updateUrl(const KURL &_newUrl)
{
    ...
    for(QPtrListIterator< KPropsDlgPlugin > it(m_pageList); it.current(); ++it)
        if(it.current()->isA("KExecPropsPlugin") || // KDE4 remove me
           it.current()->isA("KURLPropsPlugin") || it.current()->isA("KDesktopPropsPlugin"))
        {
...
KPropsDlgPlugin::KPropsDlgPlugin(KPropertiesDialog *_props) : QObject(_props, 0L)
{
...
KFilePropsPlugin::KFilePropsPlugin(KPropertiesDialog *_props) : KPropsDlgPlugin(_props)
{
    ...
    QVBoxLayout *vbl = new QVBoxLayout(d->m_frame, 0, KDialog::spacingHint(), "vbl");
    ...
    if(!d->bMultiple)
    {
        ...
        if(KExecPropsPlugin::supports(properties->items()) || // KDE4 remove me
           d->bDesktopFile || KBindingPropsPlugin::supports(properties->items()))
        {
    ...
    if(!isDevice && !isTrash && (bDesktopFile || S_ISDIR(mode)) && !d->bMultiple /*not implemented for multiple*/)
    {
        ...
        iconArea = iconButton;
        connect(iconButton, SIGNAL(iconChanged(QString)), this, SLOT(slotIconChanged()));
    }
    else
    {
    ...
    else
    {
        ...
        connect(d->m_lined, SIGNAL(textChanged(const QString &)), this, SLOT(nameFileChanged(const QString &)));
    }
    ...
    if(isLocal && hasDirs) // only for directories
    {
        ...
        connect(job, SIGNAL(foundMountPoint(const unsigned long &, const unsigned long &, const unsigned long &, const QString &)), this,
                SLOT(slotFoundMountPoint(const unsigned long &, const unsigned long &, const unsigned long &, const QString &)));
...
void KFilePropsPlugin::slotEditFileType()
{
    ...
    KRun::runCommand(keditfiletype + " --parent " + QString::number((ulong)properties->topLevelWidget()->winId()) + " " + KProcess::quote(mime),
...
void KFilePropsPlugin::slotFoundMountPoint(const unsigned long &kBSize, const unsigned long & /*kBUsed*/, const unsigned long &kBAvail,
                                           const QString &)
{
    d->m_freeSpaceLabel->setText(
        // xgettext:no-c-format  --  Don't warn about translating the %1 out of %2 part.
...
void KFilePropsPlugin::slotSizeDetermine()
{
    ...
    connect(d->dirSizeUpdateTimer, SIGNAL(timeout()), SLOT(slotDirSizeUpdate()));
    d->dirSizeUpdateTimer->start(500);
    connect(d->dirSizeJob, SIGNAL(result(KIO::Job *)), SLOT(slotDirSizeFinished(KIO::Job *)));
...
void KFilePropsPlugin::applyChanges()
{
    ...
    if(nameArea->inherits("QLineEdit"))
    {
        ...
        { // true for any from-template file
            ...
            connect(job, SIGNAL(renamed(KIO::Job *, const KURL &, const KURL &)), SLOT(slotFileRenamed(KIO::Job *, const KURL &, const KURL &)));
            // wait for job
...
void KFilePropsPlugin::applyIconChanges()
{
    ...
    if(url.isLocalFile())
    {
        ...
        QString str = KMimeType::findByURL(url, properties->item()->mode(), true)->KServiceType::icon();
...
const char *KFilePermissionsPropsPlugin::permissionsTexts[4][4] = {
    {I18N_NOOP("Forbidden"), I18N_NOOP("Can Read"), I18N_NOOP("Can Read & Write"), 0},
    {I18N_NOOP("Forbidden"), I18N_NOOP("Can View Content"), I18N_NOOP("Can View & Modify Content"), 0},
    {0, 0, 0, 0}, // no texts for links
    {I18N_NOOP("Forbidden"), I18N_NOOP("Can View Content & Read"), I18N_NOOP("Can View/Read & Modify/Write"), 0}};


KFilePermissionsPropsPlugin::KFilePermissionsPropsPlugin(KPropertiesDialog *_props) : KPropsDlgPlugin(_props)
{
    ...
    if(isLink)
        d->explanationLabel->setText(i18n("This file is a link and does not have permissions.", "All files are links and do not have permissions.",
    ...
    if(IamRoot && isLocal)
    {
        ...
        endpwent();
        usrEdit->setCompletionMode((i < maxEntries) ? KGlobalSettings::CompletionAuto : KGlobalSettings::CompletionNone);
        usrEdit->setText(strOwner);
        gl->addWidget(usrEdit, 1, 1);
        connect(usrEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    }
    else
    {
    ...
    if(IamRoot && isLocal)
    {
        ...
        connect(grpEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    }
    else if((groupList.count() > 1) && isMyFile && isLocal)
    {
        ...
        connect(grpCombo, SIGNAL(activated(int)), this, SIGNAL(changed()));
    }
    else
    {
...
#else
    fileSystemSupportsACLs = getxattr(pathCString.data(), "system.posix_acl_access", NULL, 0) >= 0 || errno == ENODATA;
#endif
...
void KFilePermissionsPropsPlugin::slotShowAdvancedPermissions()
{
    ...
    KDialogBase dlg(properties, 0, true, i18n("Advanced Permissions"), KDialogBase::Ok | KDialogBase::Cancel);
...
{
    setComboContent(d->ownerPermCombo, PermissionsOwner, permissions, d->partialPermissions);
    setComboContent(d->groupPermCombo, PermissionsGroup, permissions, d->partialPermissions);
    setComboContent(d->othersPermCombo, PermissionsOthers, permissions, d->partialPermissions);
    ...
    switch(d->pmode)
    {
            ...
            if(d->canChangePermissions)
                d->explanationLabel->setText(
                    d->isIrregular || d->hasExtendedACL
                        ? i18n("This file uses advanced permissions", "These files use advanced permissions.", properties->items().count())
                        : "");
        ...
        case PermissionsOnlyDirs:
            ...
            if(d->canChangePermissions)
                d->explanationLabel->setText(
                    d->isIrregular || d->hasExtendedACL
                        ? i18n("This folder uses advanced permissions.", "These folders use advanced permissions.", properties->items().count())
                        : "");
        ...
        case PermissionsMixed:
            enableAccessControls(d->canChangePermissions && !d->isIrregular && !d->hasExtendedACL);
            if(d->canChangePermissions)
                d->explanationLabel->setText(d->isIrregular || d->hasExtendedACL ? i18n("These files use advanced permissions.") : "");
            break;
            if(d->partialPermissions & S_ISVTX)
            {
                d->extraCheckbox->setTristate();
...
void KFilePermissionsPropsPlugin::getPermissionMasks(mode_t &andFilePermissions, mode_t &andDirPermissions, mode_t &orFilePermissions,
                                                     mode_t &orDirPermissions)
{
    ...
    if(m != (mode_t)-1)
    {
        ...
        if((m & UniOwner)
           && ((d->pmode == PermissionsMixed) || ((d->pmode == PermissionsOnlyFiles) && (d->extraCheckbox->state() == QButton::NoChange))))
    ...
    if(((d->pmode == PermissionsMixed) || (d->pmode == PermissionsOnlyDirs)) && (d->extraCheckbox->state() != QButton::NoChange))
    {
...
void KFilePermissionsPropsPlugin::applyChanges()
{
    ...
    if(files.count() > 0)
    {
        ...
        connect(job, SIGNAL(result(KIO::Job *)), SLOT(slotChmodResult(KIO::Job *)));
        // Wait for job
        QWidget dummy(0, 0, WType_Dialog | WShowModal);
        qt_enter_modal(&dummy);
        qApp->enter_loop();
...
KURLPropsPlugin::KURLPropsPlugin(KPropertiesDialog *_props) : KPropsDlgPlugin(_props)
{
    ...
    connect(URLEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
...
KBindingPropsPlugin::KBindingPropsPlugin(KPropertiesDialog *_props) : KPropsDlgPlugin(_props)
{
    ...
    connect(patternEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    connect(commentEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    connect(mimeEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    connect(cbAutoEmbed, SIGNAL(toggled(bool)), this, SIGNAL(changed()));
}
...
KDevicePropsPlugin::KDevicePropsPlugin(KPropertiesDialog *_props) : KPropsDlgPlugin(_props)
{
    ...
    for(KMountPoint::List::ConstIterator it = mountPoints.begin(); it != mountPoints.end(); ++it)
    {
        ...
        if((mountPoint != "-") && (mountPoint != "none") && !mountPoint.isEmpty() && device != "none")
        {
    ...
    QGridLayout *layout = new QGridLayout(d->m_frame, 0, 2, 0, KDialog::spacingHint());
    layout->setColStretch(1, 1);

    QLabel *label;
    label = new QLabel(d->m_frame);
    label->setText(devices.count() == 0 ? i18n("Device (/dev/fd0):") : // old style
    ...
    connect(device, SIGNAL(activated(int)), this, SLOT(slotActivated(int)));
    ...
    label = new QLabel(d->m_frame);
    label->setText(devices.count() == 0 ? i18n("Mount point (/mnt/floppy):") : // old style
    ...
    connect(device, SIGNAL(activated(int)), this, SIGNAL(changed()));
    connect(device, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    connect(readonly, SIGNAL(toggled(bool)), this, SIGNAL(changed()));
    connect(unmounted, SIGNAL(iconChanged(QString)), this, SIGNAL(changed()));

    connect(device, SIGNAL(textChanged(const QString &)), this, SLOT(slotDeviceChanged()));
}
...
void KDevicePropsPlugin::slotFoundMountPoint(const unsigned long &kBSize, const unsigned long & /*kBUsed*/, const unsigned long &kBAvail,
                                             const QString &)
{
...
KDesktopPropsPlugin::KDesktopPropsPlugin(KPropertiesDialog *_props) : KPropsDlgPlugin(_props)
{
...
{
    KDialogBase dlg(w, "KPropertiesMimetypes", true, i18n("Add File Type for %1").arg(properties->kurl().fileName()),
                    KDialogBase::Ok | KDialogBase::Cancel, KDialogBase::Ok);

    KGuiItem okItem(i18n("&Add"), QString::null /* no icon */, i18n("Add the selected file types to\nthe list of supported file types."),
...

    {
        ...
        connect(mw->listView, SIGNAL(selectionChanged()), this, SLOT(slotSelectMimetype()));
        connect(mw->listView, SIGNAL(doubleClicked(QListViewItem *, const QPoint &, int)), &dlg, SLOT(slotOk()));
...
{
    if(KRun::binaryName(w->commandEdit->text(), true) != KRun::binaryName(m_origCommandStr, true))
    {
...
void KDesktopPropsPlugin::applyChanges()
{
    ...
    for(QListViewItem *item = w->filetypeList->firstChild(); item; item = item->nextSibling())
    {
...
{
    KURL f = KFileDialog::getOpenURL(QString::null, QString::null, w);
    if(f.isEmpty())
        return;
...
{
    KDialogBase dlg(w, "KPropertiesDesktopAdv", true, i18n("Advanced Options for %1").arg(properties->kurl().fileName()),
                    KDialogBase::Ok | KDialogBase::Cancel, KDialogBase::Ok);
    ...
    connect(w->terminalEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    ...
    connect(w->suidEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    ...
    connect(w->dcopCombo, SIGNAL(highlighted(int)), this, SIGNAL(changed()));
    ...
    if(dlg.exec() == QDialog::Accepted)
    {
        ...
        {
            case 1:
                m_dcopServiceType = "multi";
                break;
            case 2:
                m_dcopServiceType = "unique";
                break;
            case 3:
                m_dcopServiceType = "wait";
                break;
            default:
                m_dcopServiceType = "none";
                break;
        }
    }
}
...
KExecPropsPlugin::KExecPropsPlugin(KPropertiesDialog *_props) : KPropsDlgPlugin(_props)
{
    d = new KExecPropsPluginPrivate;
    d->m_frame = properties->addPage(i18n("E&xecute"));
    QVBoxLayout *mainlayout = new QVBoxLayout(d->m_frame, 0, KDialog::spacingHint());
    ...
    QWhatsThis::add(execEdit, i18n("Following the command, you can have several place holders which will be replaced "
    ...
    connect(swallowExecEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    connect(swallowTitleEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    connect(execEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    connect(terminalEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    if(d->nocloseonexitCheck)
        connect(d->nocloseonexitCheck, SIGNAL(toggled(bool)), this, SIGNAL(changed()));
    connect(terminalCheck, SIGNAL(toggled(bool)), this, SIGNAL(changed()));
    connect(suidCheck, SIGNAL(toggled(bool)), this, SIGNAL(changed()));
    connect(suidEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
...
{
    KURL f = KFileDialog::getOpenURL(QString::null, QString::null, d->m_frame);
    if(f.isEmpty())
        return;
...
KApplicationPropsPlugin::KApplicationPropsPlugin(KPropertiesDialog *_props) : KPropsDlgPlugin(_props)
{
    ...
    connect(addExtensionButton, SIGNAL(clicked()), SLOT(slotAddExtension()));
    ...
    connect(delExtensionButton, SIGNAL(clicked()), SLOT(slotDelExtension()));
    ...
    connect(extensionsList, SIGNAL(highlighted(int)), this, SLOT(updateButton()));
    connect(availableExtensionsList, SIGNAL(highlighted(int)), this, SLOT(updateButton()));

    connect(addExtensionButton, SIGNAL(clicked()), this, SIGNAL(changed()));
    connect(delExtensionButton, SIGNAL(clicked()), this, SIGNAL(changed()));
    if(nameEdit)
        connect(nameEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    connect(commentEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    connect(genNameEdit, SIGNAL(textChanged(const QString &)), this, SIGNAL(changed()));
    connect(availableExtensionsList, SIGNAL(selected(int)), this, SIGNAL(changed()));
    connect(extensionsList, SIGNAL(selected(int)), this, SIGNAL(changed()));
}
