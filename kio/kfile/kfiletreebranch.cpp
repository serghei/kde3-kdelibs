...
KFileTreeBranch::KFileTreeBranch(KFileTreeView *parent, const KURL &url, const QString &name, const QPixmap &pix, bool showHidden,
    ...
    , m_showExtensions(true)
{
    ...
    if(!branchRoot)
    {
        m_root = new KFileTreeViewItem(parent, new KFileItem(url, "inode/directory", S_IFDIR), this);
    }
    ...
    connect(this, SIGNAL(refreshItems(const KFileItemList &)), this, SLOT(slotRefreshItems(const KFileItemList &)));

    connect(this, SIGNAL(newItems(const KFileItemList &)), this, SLOT(addItems(const KFileItemList &)));

    connect(this, SIGNAL(completed(const KURL &)), this, SLOT(slCompleted(const KURL &)));

    connect(this, SIGNAL(started(const KURL &)), this, SLOT(slotListerStarted(const KURL &)));

    connect(this, SIGNAL(deleteItem(KFileItem *)), this, SLOT(slotDeleteItem(KFileItem *)));

    connect(this, SIGNAL(canceled(const KURL &)), this, SLOT(slotCanceled(const KURL &)));

    connect(this, SIGNAL(clear()), this, SLOT(slotDirlisterClear()));

    connect(this, SIGNAL(clear(const KURL &)), this, SLOT(slotDirlisterClearURL(const KURL &)));

    connect(this, SIGNAL(redirection(const KURL &, const KURL &)), this, SLOT(slotRedirect(const KURL &, const KURL &)));
...
void KFileTreeBranch::addItems(const KFileItemList &list)
{
    ...
    while((currItem = it.current()) != 0)
    {
        ...
        KFileTreeViewItem *newKFTVI = static_cast< KFileTreeViewItem * >(currItem->extraData(this));
...
KFileTreeViewItem *KFileTreeBranch::createTreeViewItem(KFileTreeViewItem *parent, KFileItem *fileItem)
{
    KFileTreeViewItem *tvi = 0;
    if(parent && fileItem)
    {
        tvi = new KFileTreeViewItem(parent, fileItem, this);
    }
    else
    {
...
void KFileTreeBranch::slotDirlisterClearURL(const KURL &url)
{
    ...
    if(item)
    {
        KFileTreeViewItem *ftvi = static_cast< KFileTreeViewItem * >(item->extraData(this));
...
void KFileTreeBranch::slCompleted(const KURL &url)
{
    ...
    kdDebug(250) << "current parent " << currParent << " is already listed: " << currParent->alreadyListed() << endl;
    ...
    if(m_recurseChildren && (!m_startURL.isLocalFile() || !dirOnlyMode()))
    {
        ...
        for(KURL::List::Iterator it = m_openChildrenURLs.begin(); it != m_openChildrenURLs.end(); ++it)
        {
        ...
        if(wantRecurseUrl && currParent)
        {
            ...
            nextChild = static_cast< KFileTreeViewItem * >(static_cast< QListViewItem * >(currParent)->firstChild());
