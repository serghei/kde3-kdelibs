...
KAppTreeListItem::KAppTreeListItem(KListView *parent, const QString &name, const QPixmap &pixmap, bool parse, bool dir, const QString &p,
                                   const QString &c)
...
KAppTreeListItem::KAppTreeListItem(QListViewItem *parent, const QString &name, const QPixmap &pixmap, bool parse, bool dir, const QString &p,
                                   const QString &c)
    : QListViewItem(parent, name)
{
    init(pixmap, parse, dir, p, c);
...
{
    if(o && !parsed)
    { // fill the children before opening
...
KApplicationTree::KApplicationTree(QWidget *parent) : KListView(parent), currentitem(0)
{
...
{
    if(!currentitem)
        return false; // if currentitem isn't set
...
void KApplicationTree::addDesktopGroup(const QString &relPath, KAppTreeListItem *item)
{
    ...
    for(KServiceGroup::List::ConstIterator it = list.begin(); it != list.end(); it++)
    {
        ...
        if(item)
            newItem = new KAppTreeListItem(item, text, pixmap, false, isDir, relPath, exec);
        else
            newItem = new KAppTreeListItem(this, text, pixmap, false, isDir, relPath, exec);
        if(isDir)
...
KOpenWithDlg::KOpenWithDlg(const KURL::List &_urls, QWidget *parent) : QDialog(parent, "openwith", true)
{
...
KOpenWithDlg::KOpenWithDlg(const KURL::List &_urls, const QString &_text, const QString &_value, QWidget *parent) : QDialog(parent, "openwith", true)
{
...
KOpenWithDlg::KOpenWithDlg(const QString &serviceType, const QString &value, QWidget *parent) : QDialog(parent, "openwith", true)
{
...
KOpenWithDlg::KOpenWithDlg(QWidget *parent) : QDialog(parent, "openwith", true)
{
...
void KOpenWithDlg::init(const QString &_text, const QString &_value)
{
    ...
    if(!bReadOnly)
    {
        ...
        int mode = kc->readNumEntry(QString::fromLatin1("CompletionMode"), KGlobalSettings::completionMode());
    ...
    edit->setURL(_value);
    QWhatsThis::add(edit, i18n("Following the command, you can have several place holders which will be replaced "
    ...
    connect(m_pTree, SIGNAL(selected(const QString &, const QString &)), SLOT(slotSelected(const QString &, const QString &)));
    connect(m_pTree, SIGNAL(highlighted(const QString &, const QString &)), SLOT(slotHighlighted(const QString &, const QString &)));
    connect(m_pTree, SIGNAL(doubleClicked(QListViewItem *)), SLOT(slotDbClick()));
...
{
    if(m_pTree->isDirSel())
        return; // check if a directory is selected
    slotOK();
}
...
void KOpenWithDlg::accept()
{
    ...
    if(combo)
    {
        ...
        kc->writeEntry(QString::fromLatin1("CompletionMode"), combo->completionMode());
