...
static struct
{
    ...
    {I18N_NOOP("Owner"), "user-grey", 0}, {I18N_NOOP("Owning Group"), "group-grey", 0}, {I18N_NOOP("Others"), "others-grey", 0},
    {I18N_NOOP("Mask"), "mask", 0},       {I18N_NOOP("Named User"), "user", 0},         {I18N_NOOP("Named Group"), "group", 0},
};
...
KACLListViewItem::KACLListViewItem(QListView *parent, KACLListView::EntryType _type, unsigned short _value, bool defaults, const QString &_qualifier)
    : KListViewItem(parent, parent->lastItem())
    , // we want to append
    type(_type)
    , value(_value)
    , isDefault(defaults)
...
void KACLListViewItem::calcEffectiveRights()
{
    ...
    if(m_pACLListView->hasMaskEntry() && (type == KACLListView::NamedUser || type == KACLListView::Group || type == KACLListView::NamedGroup)
       && !isDefault)
    {
...
bool KACLListViewItem::isDeletable() const
{
    ...
    return type != KACLListView::User && type != KACLListView::Group && type != KACLListView::Others
...
void KACLListView::slotAddEntry()
{
    ...
    EditACLEntryDialog dlg(this, 0, allowedUsers(false), allowedGroups(false), allowedUsers(true), allowedGroups(true), allowedTypes,
                           allowedDefaultTypes, m_allowDefaults);
    dlg.exec();
...
void KACLListView::slotEditEntry()
{
    ...
    EditACLEntryDialog dlg(this, item, allowedUsers(false, item), allowedGroups(false, item), allowedUsers(true, item), allowedGroups(true, item),
