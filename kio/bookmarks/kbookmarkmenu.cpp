...
KBookmarkMenu::KBookmarkMenu(KBookmarkManager *mgr, KBookmarkOwner *_owner, KPopupMenu *_parentMenu, KActionCollection *collec, bool _isRoot,
                             bool _add, const QString &parentAddress)
    : QObject()
    , m_bIsRoot(_isRoot)
    , m_bAddBookmark(_add)
    ...
    , m_parentAddress(parentAddress)
{
    ...
    if(m_actionCollection)
    {
        ...
        connect(m_actionCollection, SIGNAL(actionHighlighted(KAction *)), this, SLOT(slotActionHighlighted(KAction *)));
    }
    ...
    if(!m_bNSBookmark) // not for the netscape bookmark
    {
        ...
        if(KBookmarkSettings::self()->m_contextmenu)
        {
            ...
            connect(_parentMenu, SIGNAL(aboutToShowContextMenu(KPopupMenu *, int, QPopupMenu *)), this,
                    SLOT(slotAboutToShowContextMenu(KPopupMenu *, int, QPopupMenu *)));
        }

        if(m_bIsRoot)
        {
            connect(m_pManager, SIGNAL(changed(const QString &, const QString &)), SLOT(slotBookmarksChanged(const QString &)));
        }
    }
...
static RMB *rmbSelf(KBookmarkMenu *m)
{
...
void RMB::slotRMBActionProperties(int val)
{
    ...
    KBookmarkEditDialog dlg(bookmark.fullText(), folder, m_pManager, KBookmarkEditDialog::ModifyMode, 0, 0, 0, i18n("Bookmark Properties"));
...
void RMB::slotRMBActionRemove(int val)
{
    ...
    if(KMessageBox::warningContinueCancel(m_parentMenu,
                                          folder ? i18n("Are you sure you wish to remove the bookmark folder\n\"%1\"?").arg(bookmark.text())
                                                 : i18n("Are you sure you wish to remove the bookmark\n\"%1\"?").arg(bookmark.text()),
                                          folder ? i18n("Bookmark Folder Deletion") : i18n("Bookmark Deletion"), KStdGuiItem::del())
...
{
    RMB::begin_rmb_action(this);
    rmbSelf(this)->slotRMBActionEditAt(val);
}
...
{
    RMB::begin_rmb_action(this);
    rmbSelf(this)->slotRMBActionInsert(val);
}

void KBookmarkMenu::slotRMBActionRemove(int val)
{
    RMB::begin_rmb_action(this);
    rmbSelf(this)->slotRMBActionRemove(val);
}

void KBookmarkMenu::slotRMBActionCopyLocation(int val)
{
    RMB::begin_rmb_action(this);
    rmbSelf(this)->slotRMBActionCopyLocation(val);
}
...
void KBookmarkMenu::addAddBookmarksList()
{
    ...
    KAction *paAddBookmarksList =
        new KAction(title, "bookmarks_list_add", 0, this, SLOT(slotAddBookmarksList()), m_actionCollection, m_bIsRoot ? "add_bookmarks_list" : 0);
...
void KBookmarkMenu::addAddBookmark()
{
    ...
    KAction *paAddBookmarks = new KAction(title, "bookmark_add", m_bIsRoot && m_bAddShortcuts ? KStdAccel::addBookmark() : KShortcut(), this,
                                          SLOT(slotAddBookmark()), m_actionCollection, m_bIsRoot ? "add_bookmark" : 0);
...
void KBookmarkMenu::addEditBookmarks()
{
    ...
    KAction *m_paEditBookmarks = KStdAction::editBookmarks(m_pManager, SLOT(slotEditBookmarks()), m_actionCollection, "edit_bookmarks");
...
void KBookmarkMenu::addNewFolder()
{
    ...
    KAction *paNewFolder = new KAction(title,
                                       "folder_new", //"folder",
                                       0, this, SLOT(slotNewFolder()), m_actionCollection);
...
void KBookmarkMenu::fillBookmarkMenu()
{
    ...
    if(m_bIsRoot && KBookmarkManager::userBookmarksFile() == m_pManager->path())
    {
        bool haveSep = false;
        ...
        for(it = keys.begin(); it != keys.end(); ++it)
        {
            ...
            actionMenu = new KImportedBookmarksActionMenu(info.name, info.type, m_actionCollection, "kbookmarkmenu");
            ...
            KBookmarkMenu *subMenu =
                new KBookmarkMenu(m_pManager, m_pOwner, actionMenu->popupMenu(), m_actionCollection, false, m_bAddBookmark, QString::null);
            connect(subMenu, SIGNAL(openBookmark(const QString &, Qt::ButtonState)), this, SIGNAL(openBookmark(const QString &, Qt::ButtonState)));
    ...
    for(KBookmark bm = parentBookmark.first(); !bm.isNull(); bm = parentBookmark.next(bm))
    {
        ...
        if(!bm.isGroup())
        {
            ...
            else
            {
                ...
                connect(action, SIGNAL(activated(KAction::ActivationReason, Qt::ButtonState)), this,
                        SLOT(slotBookmarkSelected(KAction::ActivationReason, Qt::ButtonState)));
        ...
        else
        {
            ...
            KActionMenu *actionMenu = new KBookmarkActionMenu(text, bm.icon(), m_actionCollection, "kbookmarkmenu");
            actionMenu->setProperty("address", bm.address());
            actionMenu->plug(m_parentMenu);
            m_actions.append(actionMenu);

            KBookmarkMenu *subMenu =
                new KBookmarkMenu(m_pManager, m_pOwner, actionMenu->popupMenu(), m_actionCollection, false, m_bAddBookmark, bm.address());

            connect(subMenu, SIGNAL(aboutToShowContextMenu(const KBookmark &, QPopupMenu *)), this,
...
KBookmarkEditDialog::KBookmarkEditDialog(const QString &title, const QString &url, KBookmarkManager *mgr, BookmarkEditType editType,
                                         const QString &address, QWidget *parent, const char *name, const QString &caption)
    : KDialogBase(parent, name, true, caption, (editType == InsertionMode) ? (User1 | Ok | Cancel) : (Ok | Cancel), Ok, false, KGuiItem())
    , m_folderTree(0)
    , m_mgr(mgr)
    , m_editType(editType)
    , m_address(address)
{
    ...
    KBookmarkEditFields::FieldsSet fs = folder ? KBookmarkEditFields::FolderFieldsSet : KBookmarkEditFields::BookmarkFieldsSet;
    ...
    if(editType == InsertionMode)
    {
        ...
        connect(m_folderTree, SIGNAL(doubleClicked(QListViewItem *)), this, SLOT(slotDoubleClicked(QListViewItem *)));
...
static void fillGroup(QListView *listview, KBookmarkFolderTreeItem *parentItem, KBookmarkGroup group, bool expandOpenGroups = true,
...
// toplevel item
KBookmarkFolderTreeItem::KBookmarkFolderTreeItem(QListView *parent, const KBookmark &gp) : QListViewItem(parent, i18n("Bookmarks")), m_bookmark(gp)
{
...
void KBookmarkMenuNSImporter::connectToImporter(const QObject &importer)
{
    ...
    connect(&importer, SIGNAL(newFolder(const QString &, bool, const QString &)), SLOT(newFolder(const QString &, bool, const QString &)));
...
void KBookmarkMenuNSImporter::newBookmark(const QString &text, const QCString &url, const QString &)
{
    ...
    connect(action, SIGNAL(activated(KAction::ActivationReason, Qt::ButtonState)), m_menu,
            SLOT(slotBookmarkSelected(KAction::ActivationReason, Qt::ButtonState)));
...
void KBookmarkMenuNSImporter::newFolder(const QString &text, bool, const QString &)
{
    ...
    KBookmarkMenu *subMenu =
        new KBookmarkMenu(m_pManager, m_menu->m_pOwner, actionMenu->popupMenu(), m_actionCollection, false, m_menu->m_bAddBookmark, QString::null);
    connect(subMenu, SIGNAL(openBookmark(const QString &, Qt::ButtonState)), m_menu, SIGNAL(openBookmark(const QString &, Qt::ButtonState)));
