...
KBookmarkManager::KBookmarkManager() : DCOPObject(QCString("KBookmarkManager-generated")), m_doc("xbel"), m_docIsLoaded(true)
{
...
bool KBookmarkManager::saveAs(const QString &filename, bool toolbarCache) const
{
    ...
    if(!hadSaveError)
    {
        QString error = i18n(
                            ...
                            .arg(filename)
                            .arg(QString::fromLocal8Bit(strerror(file.status())));
...
KBookmarkGroup KBookmarkManager::toolbar()
{
    ...
    if(!m_docIsLoaded)
    {
        ...
        if(m_toolbarDoc.isNull() && QFile::exists(cacheFilename) && bmInfo.lastModified() < cacheInfo.lastModified())
        {
...
static QString pickUnusedTitle(KBookmarkGroup parentBookmark, const QString &title, const QString &url)
{
...
KBookmarkGroup KBookmarkManager::addBookmarkDialog(const QString &_url, const QString &_title, const QString &_parentBookmarkAddress)
{
    QString url = _url;
...
void KBookmarkManager::setShowNSBookmarks(bool show)
{
    ...
    if(this->path() != userBookmarksFile())
        return;
    KBookmarkMenu::DynMenuInfo info = KBookmarkMenu::showDynamicBookmarks("netscape");
    info.show = show;
...
{
    KProcess proc;
    proc << QString::fromLatin1("keditbookmarks") << QString::fromLatin1("--address") << address << m_bookmarksFile;
    proc.start(KProcess::DontCare);
}
