...
public:
    ...
    void filter(const KBookmarkGroup &grp)
    {
        traverse(grp);
    }
...
KBookmarkBar::KBookmarkBar(KBookmarkManager *mgr, KBookmarkOwner *_owner, KToolBar *_toolBar, KActionCollection *coll, QObject *parent,
                           const char *name)
    : QObject(parent, name), m_pOwner(_owner), m_toolBar(_toolBar), m_actionCollection(coll), m_pManager(mgr)
{
    ...
    connect(mgr, SIGNAL(changed(const QString &, const QString &)), SLOT(slotBookmarksChanged(const QString &)));
...
#define CURRENT_TOOLBAR() (dptr()->m_filteredMgr ? dptr()->m_filteredMgr->root() : m_pManager->toolbar())

#define CURRENT_MANAGER() (dptr()->m_filteredMgr ? dptr()->m_filteredMgr : m_pManager)
...
KBookmarkGroup KBookmarkBar::getToolbar()
{
    if(KBookmarkSettings::self()->m_filteredtoolbar)
    {
        ...
        KBookmarkDomBuilder builder(dptr()->m_filteredMgr->root(), dptr()->m_filteredMgr);
...
void KBookmarkBar::fillBookmarkBar(KBookmarkGroup &parent)
{
    ...
    for(KBookmark bm = parent.first(); !bm.isNull(); bm = parent.next(bm))
    {
        ...
        {
            KActionMenu *action = new KBookmarkActionMenu(text, bm.icon(), m_actionCollection, "bookmarkbar-actionmenu");
            action->setProperty("address", bm.address());
            ...
            KBookmarkMenu *menu =
                new KBookmarkMenu(CURRENT_MANAGER(), m_pOwner, action->popupMenu(), m_actionCollection, false, addEntriesBookmarkBar, bm.address());
            connect(menu, SIGNAL(aboutToShowContextMenu(const KBookmark &, QPopupMenu *)), this,
                    SIGNAL(aboutToShowContextMenu(const KBookmark &, QPopupMenu *)));
            connect(menu, SIGNAL(openBookmark(const QString &, Qt::ButtonState)), this, SIGNAL(openBookmark(const QString &, Qt::ButtonState)));
 ...
 */
static QString handleToolbarDragMoveEvent(KBookmarkBarPrivate *p, KToolBar *tb, QPoint pos, QPtrList< KAction > actions, bool &atFirst,
...
bool KBookmarkBar::eventFilter(QObject *o, QEvent *e)
{
    ...
    else if(e->type() == QEvent::Drop)
    {
        ...
        kdDebug(7043) << "inserting " << QString(dptr()->m_atFirst ? "before" : "after") << " dptr()->m_dropAddress == " << dptr()->m_dropAddress
                      << endl;
        KBookmarkGroup parentBookmark = bookmark.parentGroup();
        Q_ASSERT(!parentBookmark.isNull());
        KBookmark newBookmark = parentBookmark.addBookmark(m_pManager, toInsert.fullText(), toInsert.url());
