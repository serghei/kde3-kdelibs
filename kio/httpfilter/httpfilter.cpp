...
HTTPFilterBase::HTTPFilterBase() : last(0)
{
}
...
{
    last = previous;
    connect(last, SIGNAL(output(const QByteArray &)), this, SLOT(slotInput(const QByteArray &)));
}

HTTPFilterChain::HTTPFilterChain() : first(0)
{
}
...
void HTTPFilterChain::addFilter(HTTPFilterBase *filter)
{
    ...
    last = filter;
    connect(filter, SIGNAL(output(const QByteArray &)), this, SIGNAL(output(const QByteArray &)));
    connect(filter, SIGNAL(error(int, const QString &)), this, SIGNAL(error(int, const QString &)));
}
...
// 1 : not gzip
// 2 : no header
int HTTPFilterGZip::checkHeader()
{
#ifdef DO_GZIP
    uInt len;
    int c;
