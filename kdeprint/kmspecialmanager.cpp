...
KMSpecialManager::KMSpecialManager(KMManager *parent, const char *name) : QObject(parent, name), m_mgr(parent), m_loaded(false)
{
}
...
bool KMSpecialManager::savePrinters()
{
    ...
    if(getuid() == 0)
    {
        ...
        if(confname.startsWith(KGlobal::dirs()->localkdedir()))
        {
            ...
            m_mgr->setErrorMsg(
                i18n("A file share/kdeprint/specials.desktop was found in your "
...
bool KMSpecialManager::loadDesktopFile(const QString &filename)
{
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        if(!KdeprintChecker::check(&conf)
           || !KXmlCommandManager::self()->checkCommand(printer->option("kde-special-command"), KXmlCommandManager::None, KXmlCommandManager::None,
                                                        0))
