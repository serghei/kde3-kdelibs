...
LPRngToolHandler::LPRngToolHandler(KMManager *mgr) : LprHandler("lprngtool", mgr)
{
}

bool LPRngToolHandler::validate(PrintcapEntry *entry)
{
    if(entry->comment.startsWith("##LPRNGTOOL##") && entry->comment.find("UNKNOWN") == -1)
        return true;
    return false;
}
...
bool LPRngToolHandler::completePrinter(KMPrinter *prt, PrintcapEntry *entry, bool shortmode)
{
    ...
    else if(l[1] == "SMB")
    {
        ...
        QString uri = buildSmbURI(opts["workgroup"], opts["host"], opts["printer"], user, pass);
        prt->setDevice(uri);
...
PrintcapEntry *LPRngToolHandler::createEntry(KMPrinter *prt)
{
    ...
    else if(prot == "smb")
    {
        ...
        {
            entry->addField(
                "xfer_options", Field::String,
                QString::fromLatin1(
                    "authfile=\"auth\" crlf=\"0\" hostip=\"\" host=\"%1\" printer=\"%2\" remote_mode=\"SMB\" share=\"//%3/%4\" workgroup=\"%5\"")
                    .arg(server)
                    .arg(printer)
                    .arg(server)
                    .arg(printer)
    ...
    if(prt->driver())
    {
        ...
        entry->addField("lprngtooloptions", Field::String,
                        QString::fromLatin1("FILTERTYPE=\"IFHP\" IFHP_OPTIONS=\"status@,sync@,pagecount@,waitend@\" PRINTERDB_ENTRY=\"%1\"")
                            .arg(driver->get("driverID")));
        QMap< QString, QString > opts;
        QString optstr;
