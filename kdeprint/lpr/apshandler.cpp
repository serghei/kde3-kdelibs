...
ApsHandler::ApsHandler(KMManager *mgr) : LprHandler("apsfilter", mgr)
{
    m_counter = 1;
}
    ...
    if(LprHandler::completePrinter(prt, entry, shortmode))
    {
        ...
        if(prt->device().isEmpty())
        {
            ...
            if(QFile::exists(smbname))
            {
                ...
                {
                    prt->setDevice(buildSmbURI(opts["SMB_WORKGROUP"], opts["SMB_SERVER"], opts["SMB_PRINTER"], opts["SMB_USER"], opts["SMB_PASSWD"]));
                }
                prot = "smb";
            }
            else if(QFile::exists(ncpname))
            {
                ...
                {
                    QString uri = buildSmbURI(QString::null, opts["NCP_SERVER"], opts["NCP_PRINTER"], opts["NCP_USER"], opts["NCP_PASSWD"]);
                    uri.replace(0, 3, "ncp");
                    prt->setDevice(uri);
                }
                prot = "ncp";
