...
KMLprManager::KMLprManager(QObject *parent, const char *name, const QStringList & /*args*/) : KMManager(parent, name)
{
    ...
    setPrinterOperationMask(KMManager::PrinterEnabling | KMManager::PrinterConfigure | KMManager::PrinterTesting | KMManager::PrinterCreation
                            | KMManager::PrinterRemoval | KMManager::PrinterTesting);
...
bool KMLprManager::savePrintcapFile()
{
    ...
    {
        setErrorMsg(
            i18n("Unable to save printcap file. Check that "
...
bool KMLprManager::createPrinter(KMPrinter *prt)
{
    ...
    if(!KStandardDirs::makeDir(sd, 0755))
    {
        setErrorMsg(i18n("Unable to create the spool directory %1. Check that you "
                         "have the required permissions for that operation.")
                        .arg(sd));
        return false;
    }
...
{
    if(!m_currentprinter
       || KMessageBox::warningContinueCancel(NULL, i18n("Editing a printcap entry manually should only be "
                                                        "done by confirmed system administrator. This may "
                                                        "prevent your printer from working. Do you want to "
                                                        "continue?"),
                                             QString::null, KStdGuiItem::cont(), "editPrintcap")
              == KMessageBox::Cancel)
        return;
