...
MaticHandler::MaticHandler(KMManager *mgr) : LprHandler("foomatic", mgr)
{
...
bool MaticHandler::savePrinterDriver(KMPrinter *prt, PrintcapEntry *entry, DrMain *driver, bool *)
{
    ...
    if(!result)
        manager()->setErrorMsg(
            i18n("You probably don't have the required permissions "
...
PrintcapEntry *MaticHandler::createEntry(KMPrinter *prt)
{
    KURL url(prt->device());
    QString prot = url.protocol();
    if((prot != "lpd" || m_rlprpath.isEmpty()) && (prot != "socket" || m_ncpath.isEmpty()) && (prot != "smb" || m_smbpath.isEmpty())
       && prot != "parallel")
    {
    ...
    {
        manager()->setErrorMsg(
            i18n("Unable to find executable lpdomatic. "
