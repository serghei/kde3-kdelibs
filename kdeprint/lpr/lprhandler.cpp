...
LprHandler::LprHandler(const QString &name, KMManager *mgr) : m_name(name), m_manager(mgr)
{
}
...
bool LprHandler::completePrinter(KMPrinter *prt, PrintcapEntry *entry, bool)
{
    ...
    {
        QString rm = entry->has("rm") ? entry->field("rm") : LprSettings::self()->defaultRemoteHost();
