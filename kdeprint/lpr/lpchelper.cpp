...
LpcHelper::LpcHelper(QObject *parent, const char *name) : QObject(parent, name)
{
...
void LpcHelper::parseStatusLPR(QTextStream &t)
{
    ...
    while(!t.atEnd())
    {
        ...
        {
            if(!printer.isEmpty() && (m_state[printer] & KMPrinter::StateMask) != KMPrinter::Stopped && line.find("no entries") == -1)
...
bool LpcHelper::changeJobState(KMJob *job, int state, QString &msg)
{
    ...
    QString result =
        execute(m_exepath + (state == KMJob::Held ? " hold " : " release ") + KProcess::quote(job->printer()) + " " + QString::number(job->id()));
