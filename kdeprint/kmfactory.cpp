...
KMFactory::KMFactory() : QObject(NULL, "Factory")
{
...
void KMFactory::createManager()
{
    loadFactory();
    if(m_factory)
        m_manager = (KMManager *)m_factory->create(this, "Manager", "KMManager");
    if(!m_manager)
        m_manager = new KMManager(this, "Manager");
}
...
void KMFactory::createJobManager()
{
    loadFactory();
    if(m_factory)
        m_jobmanager = (KMJobManager *)m_factory->create(this, "JobManager", "KMJobManager");
    if(!m_jobmanager)
        m_jobmanager = new KMJobManager(this, "JobManager");
}
...
void KMFactory::createUiManager()
{
    loadFactory();
    if(m_factory)
        m_uimanager = (KMUiManager *)m_factory->create(this, "UiManager", "KMUiManager");
    if(!m_uimanager)
        m_uimanager = new KMUiManager(this, "UiManager");
}
...
void KMFactory::createPrinterImpl()
{
    loadFactory();
    if(m_factory)
        m_implementation = (KPrinterImpl *)m_factory->create(this, "PrinterImpl", "KPrinterImpl");
    if(!m_implementation)
        m_implementation = new KPrinterImpl(this, "PrinterImpl");
}
...
void KMFactory::loadFactory(const QString &syst)
{
    if(!m_factory)
    {
        ...
        if(!m_factory)
        {
            KMessageBox::error(
                0,
                i18n("<qt>There was an error loading %1. The diagnostic is:<p>%2</p></qt>").arg(libname).arg(KLibLoader::self()->lastErrorMessage()));
        }
    }
}
