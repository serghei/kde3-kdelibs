...
KMVirtualManager::KMVirtualManager(KMManager *parent, const char *name) : QObject(parent, name), m_manager(parent)
{
}
...
void KMVirtualManager::copy(KMPrinter *p, const QString &src, const QString &name)
{
    ...
    if(!prsrc || findPrinter(instname) != NULL)
        return;
    KMPrinter *printer = new KMPrinter;
    printer->copy(*prsrc);
    printer->setName(instname);
...
void KMVirtualManager::setAsDefault(KMPrinter *p, const QString &name, QWidget *parent)
{
    ...
    if(p->isSpecial())
    {
        if(KMessageBox::warningContinueCancel(parent, i18n("<qt>You are about to set a pseudo-printer as your personal default. "
                                                           ...
                                                           "printing normally. Do you really want to set <b>%1</b> as your personal default?</qt>")
                                                          .arg(instname),
                                              QString::null, i18n("Set as Default"), "setSpecialAsDefault")
           == KMessageBox::No)
            return;
    }
