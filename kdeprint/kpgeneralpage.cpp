...
static const char *default_size[] = {
    "A4",    I18N_NOOP("ISO A4"), "Letter", I18N_NOOP("US Letter"),       "Legal", I18N_NOOP("US Legal"),        "Ledger",  I18N_NOOP("Ledger"),
    "Folio", I18N_NOOP("Folio"),  "Comm10", I18N_NOOP("US #10 Envelope"), "DL",    I18N_NOOP("ISO DL Envelope"), "Tabloid", I18N_NOOP("Tabloid"),
    "A3",    I18N_NOOP("ISO A3"), "A2",     I18N_NOOP("ISO A2"),          "A1",    I18N_NOOP("ISO A1"),          "A0",      I18N_NOOP("ISO A0")};
...
static const char *default_source[] = {"Upper",        I18N_NOOP("Upper Tray"),         "Lower",         I18N_NOOP("Lower Tray"),
                                       "MultiPurpose", I18N_NOOP("Multi-Purpose Tray"), "LargeCapacity", I18N_NOOP("Large Capacity Tray")};
#define DEFAULT_TYPE 4
static const char *default_type[] = {"Normal", I18N_NOOP("Normal"), "Transparency", I18N_NOOP("Transparency")};

//***************************************************************************************************

KPGeneralPage::KPGeneralPage(KMPrinter *pr, DrMain *dr, QWidget *parent, const char *name) : KPrintDialogPage(pr, dr, parent, name)
{
    // WhatsThis strings.... (added by pfeifle@kde.org)
    QString whatsThisPrintPropertiesGeneralPage = i18n(
        " <qt> "
        " <p><b>\"General\"</b> </p> "
        ...
        " </qt>");
    QString whatsThisGeneralPageSizeLabel = i18n(
        " <qt> "
    ...
    QGridLayout *lay3 = new QGridLayout(m_orientbox->layout(), 4, 2, KDialog::spacingHint());
    ...
    QGridLayout *lay4 = new QGridLayout(m_duplexbox->layout(), 3, 2, KDialog::spacingHint());
    ...
    QGridLayout *lay5 = new QGridLayout(m_nupbox->layout(), 3, 2, KDialog::spacingHint());
    lay5->addWidget(m_nup1, 0, 0);
    lay5->addWidget(m_nup2, 1, 0);
    lay5->addWidget(m_nup4, 2, 0);
    lay5->addMultiCellWidget(m_nuppix, 0, 2, 1, 1);
    QGridLayout *lay6 = new QGridLayout(m_bannerbox->layout(), 2, 2, KDialog::spacingHint());
...
void KPGeneralPage::initialize()
{
    if(driver())
    {
        ...
        if(opt)
            initCombo(m_pagesize, opt);
        else
            m_pagesize->setEnabled(false);
        opt = (DrListOption *)driver()->findOption("MediaType");
        if(opt)
            initCombo(m_papertype, opt);
        else
            m_papertype->setEnabled(false);
        opt = (DrListOption *)driver()->findOption("InputSlot");
        if(opt)
            initCombo(m_inputslot, opt);
        else
            m_inputslot->setEnabled(false);
        // Duplex
        ...
        if(opt)
        {
            ...
            if(opt->currentChoice())
            {
                ...
                if(opt->currentChoice()->name() == "DuplexNoTumble" || opt->currentChoice()->name() == "On")
                    ID = DUPLEX_LONG_ID;
                else if(opt->currentChoice()->name() == "DuplexTumble")
                    ID = DUPLEX_SHORT_ID;
...
void KPGeneralPage::setOptions(const QMap< QString, QString > &opts)
{
    ...
    if(driver())
    {
        ...
        for(QStringList::ConstIterator it = l.begin(); it != l.end(); ++it)
        {
            value = *it;
            DrBase *ch;
            if((ch = (driver()->findOption("PageSize"))) && (ch = (static_cast< DrListOption * >(ch))->findChoice(value)))
            {
                if(m_pagesize->isEnabled())
                    setComboItem(m_pagesize, ch->get("text"));
            }
            else if((ch = (driver()->findOption("MediaType"))) && (ch = (static_cast< DrListOption * >(ch))->findChoice(value)))
            {
                if(m_papertype->isEnabled())
                    setComboItem(m_papertype, ch->get("text"));
            }
            else if((ch = (driver()->findOption("InputSlot"))) && (ch = static_cast< DrListOption * >(ch)->findChoice(value)))
            {
                if(m_inputslot)
        ...
        {
            int ID(0);
            if(value == "DuplexNoTumble" || value == "On")
                ID = 1;
    ...
    if(!value.isEmpty())
    {
        QStringList l = QStringList::split(',', value, false);
        if(l.count() > 0)
            setComboItem(m_startbanner, l[0]);
        if(l.count() > 1)
            setComboItem(m_endbanner, l[1]);
    }
...
void KPGeneralPage::getOptions(QMap< QString, QString > &opts, bool incldef)
{
    ...
    if(driver())
    {
        ...
        if((opt = (DrListOption *)driver()->findOption("PageSize")) != NULL)
        {
            ...
            if(incldef || ch->name() != opt->get("default"))
        ...
        if(m_duplexbox->isEnabled() && (opt = (DrListOption *)driver()->findOption("Duplex")) != NULL)
        {
            ...
            {
                case DUPLEX_NONE_ID:
                    value = (twoChoices ? "Off" : "None");
                    break;
                case DUPLEX_LONG_ID:
                    value = (twoChoices ? "On" : "DuplexNoTumble");
                    break;
                case DUPLEX_SHORT_ID:
                    value = "DuplexTumble";
                    break;
            }
            if(incldef || value != opt->get("default"))
                opts["Duplex"] = value;
        }
    }
    else
    {
        value = QString("%1,%2,%3")
                    .arg(default_size[m_pagesize->currentItem() * 2])
                    .arg(default_type[m_papertype->currentItem() * 2])
                    .arg(default_source[m_inputslot->currentItem() * 2]);
        opts["media"] = value;
    ...
    if(value != "3" || incldef)
        opts["orientation-requested"] = value;
...
void KPGeneralPage::slotOrientationChanged(int ID)
{
    ...
    switch(ID)
    {
        case ORIENT_PORTRAIT_ID:
            iconstr = "kdeprint_portrait";
            break;
        case ORIENT_LANDSCAPE_ID:
            iconstr = "kdeprint_landscape";
            break;
        case ORIENT_REVPORTRAIT_ID:
            iconstr = "kdeprint_revportrait";
            break;
        case ORIENT_REVLANDSCAPE_ID:
            iconstr = "kdeprint_revlandscape";
            break;
        default:
            iconstr = "kdeprint_portrait";
            break;
...
void KPGeneralPage::slotNupChanged(int ID)
{
    ...
    {
        case NUP_1_ID:
            iconstr = "kdeprint_nup1";
            break;
        case NUP_2_ID:
            iconstr = "kdeprint_nup2";
            break;
        case NUP_4_ID:
            iconstr = "kdeprint_nup4";
            break;
        default:
            iconstr = "kdeprint_nup1";
            break;
...
void KPGeneralPage::slotDuplexChanged(int ID)
{
    if(m_duplexbox->isEnabled())
    {
        ...
        {
            case DUPLEX_NONE_ID:
                iconstr = "kdeprint_duplex_none";
                break;
            case DUPLEX_LONG_ID:
                iconstr = "kdeprint_duplex_long";
                break;
            case DUPLEX_SHORT_ID:
                iconstr = "kdeprint_duplex_short";
                break;
            default:
                iconstr = "kdeprint_duplex_none";
                break;
