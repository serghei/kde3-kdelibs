...
KPPosterPage::KPPosterPage(QWidget *parent, const char *name) : KPrintDialogPage(parent, name)
{
...
void KPPosterPage::setOptions(const QMap< QString, QString > &opts)
{
    ...
    if(opts["_kde-filters"].find("poster") != -1)
    {
        ...
        if(!ps.isEmpty())
        {
            ...
            m_lockbtn->setOn(!prtsize.isEmpty() && page_sizes[m_postersize->currentItem()].ID == prtsize.toInt());
            if(!m_lockbtn->isOn())
