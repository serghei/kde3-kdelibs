...
KPrinterWrapper::KPrinterWrapper(KPrinter *prt, QPrinter::PrinterMode m) : QPrinter(m), m_printer(prt)
{
}
...
KPrinter::KPrinter(bool restore, QPrinter::PrinterMode m) : QPaintDevice(QInternal::Printer | QInternal::ExternalDevice)
{
    init(restore, m);
}
...
void KPrinter::translateQtOptions()
{
    ...
    if(!option("kde-margin-top").isEmpty())
    {
        ...
        d->m_wrapper->setMargins(
            (int)((option("kde-margin-top").toFloat() * res + 71) / 72), (int)((option("kde-margin-left").toFloat() * res + 71) / 72),
            (int)((option("kde-margin-bottom").toFloat() * res + 71) / 72), (int)((option("kde-margin-right").toFloat() * res + 71) / 72));
    }
        ...
        else
        {
            ...
            if(!option("kde-range").isEmpty())
            {
                ...
                for(QStringList::ConstIterator it = ranges.begin(); it != ranges.end(); ++it)
                {
                    ...
                    else
                    {
                        ...
                        if(ok)
                            p2 = (*it).right((*it).length() - p - 1).toInt(&ok);
                        if(ok && p1 <= p2)
                        {
                            // clip to min/max
                            p1 = QMAX(mp, p1);
            ...
            if(pageSet() != AllPages)
            {
                ...
                for(QValueList< int >::Iterator it = list.begin(); it != list.end();)
                    if((((*it) % 2) != 0 && keepEven) || (((*it) % 2) == 0 && !keepEven))
                        it = list.remove(it);
                    else
                        ++it;
            }
...
void KPrinter::reload()
{
    ...
    if(global != -1)
        setOrientation((KPrinter::Orientation)global);
    global = KMFactory::self()->settings()->pageSize;
    if(global != -1)
        setPageSize((KPrinter::PageSize)global);
...
{
    if(!KNotifyClient::event(0, "printerror",
                             i18n("<p><nobr>A print error occurred. Error message received from system:</nobr></p><br>%1").arg(p->errorMessage())))
...
KPrinter::PageSize pageNameToPageSize(const QString &_name)
{
    ...
    if(name == "LETTER")
        ...
        return KPrinter::A0;
    else if(name == "B0" || name == "B0ISO")
        return KPrinter::B0;
    else if(name == "B1" || name == "B1ISO")
        return KPrinter::B1;
    else if(name == "B2" || name == "B2ISO")
        return KPrinter::B2;
    else if(name == "B3" || name == "B3ISO")
        return KPrinter::B3;
    else if(name == "B4" || name == "B4ISO")
        return KPrinter::B4;
    else if(name == "B5" || name == "B5ISO")
        return KPrinter::B5;
    else if(name == "B6" || name == "B6ISO")
        return KPrinter::B6;
    else if(name == "B7" || name == "B7ISO")
        return KPrinter::B7;
    else if(name == "B8" || name == "B8ISO")
        return KPrinter::B8;
    else if(name == "B9" || name == "B9ISO")
        return KPrinter::B9;
    else if(name == "B10" || name == "B10ISO")
        return KPrinter::B10;
    else if(name == "C5" || name == "C5E" || name == "ENVC5")
        return KPrinter::C5E;
    else if(name == "DL" || name == "DLE" || name == "ENVDL")
        return KPrinter::DLE;
    else if(name == "COMM10" || name == "COM10" || name == "ENV10")
...
{
    setOption("kde-minpage", QString::number(m));
    setOption("kde-maxpage", QString::number(M));
}
...
{
    setOption("kde-frompage", QString::number(m));
    setOption("kde-topage", QString::number(M));
    setOption("kde-range", (m > 0 && M > 0 ? QString("%1-%2").arg(m).arg(M) : QString::fromLatin1("")));
}
...
{
    setOption("kde-outputfilename", f);
