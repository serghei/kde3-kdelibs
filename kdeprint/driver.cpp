...
DrBase::DrBase() : m_type(DrBase::Base), m_conflict(false)
{
}
...
{
    QString grpname;
    if(optname == "PageSize" || optname == "InputSlot" || optname == "ManualFeed" || optname == "MediaType" || optname == "MediaColor"
       || optname == "MediaWeight" || optname == "Duplex" || optname == "DoubleSided" || optname == "Copies")
        grpname = i18n("General");
    else if(optname.startsWith("stp") || optname == "Cyan" || optname == "Yellow" || optname == "Magenta" || optname == "Black"
            || optname == "Density" || optname == "Contrast")
...
DrChoiceGroup::DrChoiceGroup() : DrGroup()
{
...
DrStringOption::DrStringOption() : DrBase()
{
...
DrIntegerOption::DrIntegerOption() : DrBase()
{
...
DrFloatOption::DrFloatOption() : DrBase()
{
    m_type = DrBase::Float;
    m_value = 0.0;
    set("minval", "0.0");
...
DrListOption::DrListOption() : DrBase()
{
    m_type = DrBase::List;
...
{
    if(!m_option1)
        m_option1 = (DrListOption *)driver->findOption(m_opt1);
    if(!m_option2)
        m_option2 = (DrListOption *)driver->findOption(m_opt2);
    if(m_option1 && m_option2 && m_option1->currentChoice() && m_option2->currentChoice())
    {
        ...
        // tag options
        QString s((f1 && f2 ? "1" : "0"));
        if(!m_option1->conflict())
            m_option1->setConflict(f1 && f2);
        if(!m_option2->conflict())
            m_option2->setConflict(f1 && f2);
        // return value
        return (f1 && f2);
    }
    return false;
...
DrPageSize::DrPageSize(const QString &s, float width, float height, float left, float bottom, float right, float top)
    : m_name(s), m_width(width), m_height(height), m_left(left), m_bottom(bottom), m_right(right), m_top(top)
{
}

DrPageSize::DrPageSize(const DrPageSize &d)
    : m_name(d.m_name), m_width(d.m_width), m_height(d.m_height), m_left(d.m_left), m_bottom(d.m_bottom), m_right(d.m_right), m_top(d.m_top)
{
}
