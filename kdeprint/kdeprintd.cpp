...
KDEPrintd::KDEPrintd(const QCString &obj) : KDEDModule(obj)
{
...
bool KDEPrintd::checkFiles(QString &cmd, const QStringList &files)
{
    for(QStringList::ConstIterator it = files.begin(); it != files.end(); ++it)
        ...
        {
            if(KMessageBox::warningContinueCancel(0, i18n("Some of the files to print are not readable by the KDE "
                                                  ...
                                                  QString::null, i18n("Provide root's Password"), "provideRootsPassword")
...
void KDEPrintd::initPassword(const QString &user, const QString &passwd, const QString &host, int port)
{
    ...
    if(!callingDcopClient()->call("kded", "kpasswdserver", "addAuthInfo(KIO::AuthInfo,long int)", params, replyType, reply))
