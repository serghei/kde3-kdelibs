...
KPreviewProc::KPreviewProc() : KProcess()
{
    m_bOk = false;
...
void KPreviewProc::slotProcessExited(KProcess *proc)
{
    ...
    if(proc->normalExit() && proc->exitStatus() == 0)
        ...
        kdDebug(500) << "KPreviewProc::slotProcessExited: normalExit=" << proc->normalExit() << " exitStatus=" << proc->exitStatus() << endl;
}
...
static KLibFactory *componentFactory()
{
    ...
    KTrader::OfferList offers = KTrader::self()->query(QString::fromLatin1("application/postscript"), QString::fromLatin1("KParts/ReadOnlyPart"),
...
KPrintPreview::KPrintPreview(QWidget *parent, bool previewOnly) : KDialogBase(parent, "PreviewDlg", true, i18n("Print Preview"), 0)
{
...
bool KPrintPreview::preview(const QString &file, bool previewOnly, WId parentId)
{
    ...
    {
        return (KMessageBox::questionYesNo(parentW, i18n("Do you want to continue printing?"), QString::null, KGuiItem(i18n("Print"), "fileprint"),
                                           KStdGuiItem::cancel(), "continuePrinting")
                == KMessageBox::Yes);
    }
    else
        return false;
