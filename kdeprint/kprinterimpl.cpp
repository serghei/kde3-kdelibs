...
KPrinterImpl::KPrinterImpl(QObject *parent, const char *name) : QObject(parent, name)
{
    loadAppOptions();
}
...
QString KPrinterImpl::tempFile()
{
    ...
    do
        f = locateLocal("tmp", "kdeprint_") + KApplication::randomString(8);
...
int KPrinterImpl::filterFiles(KPrinter *printer, QStringList &files, bool flag)
{
    ...
    if(printer->pageSelection() == KPrinter::SystemSide
       ...
       && (printer->pageOrder() == KPrinter::LastPageFirst || !printer->option("kde-range").isEmpty() || printer->pageSet() != KPrinter::AllPages))
    {
        if(flist.findIndex("psselect") == -1)
        {
            ...
            if(index == -1 || !KXmlCommandManager::self()->checkCommand("psselect"))
            {
                printer->setErrorMessage(
                    i18n("<p>Unable to perform the requested page selection. The filter <b>psselect</b> "
...
int KPrinterImpl::doFilterFiles(KPrinter *printer, QStringList &files, const QStringList &flist, const QMap< QString, QString > &opts, bool flag)
{
    ...
    QString ps = pageSizeToPageName(printer->option("kde-printsize").isEmpty() ? printer->pageSize()
                                                                               : (KPrinter::PageSize)printer->option("kde-printsize").toInt());
    for(QStringList::Iterator it = files.begin(); it != files.end(); ++it)
    {
        QString mime = KMimeMagic::self()->findFileType(*it)->mimeType();
        if(inputMimeTypes.find(mime) == inputMimeTypes.end())
        {
            if(KMessageBox::warningContinueCancel(0, "<p>"
               ...
               == KMessageBox::Continue)
            {
                ...
                {
                    bool ok(false);
                    QString targetMime = KInputDialog::getItem(i18n("Select MIME Type"), i18n("Select the target format for the conversion:"),
...
int KPrinterImpl::autoConvertFiles(KPrinter *printer, QStringList &files, bool flag)
{
    ...
    for(QStringList::Iterator it = files.begin(); it != files.end();)
    {
        ...
        if(mime == "application/x-zerosize")
        {
            ...
            KMessageBox::information(NULL, i18n("<qt>The print file is empty and will be ignored:<p>%1</p></qt>").arg(*it), QString::null,
                                     "emptyFileNotPrinted");
            if(flag)
                QFile::remove(*it);
            it = files.remove(it);
        ...
        {
            if((result = KMessageBox::warningYesNoCancel(
                    NULL, i18n("<qt>The file format <em> %1 </em> is not directly supported by the current print system. You "
                               "now have 3 options: "
                               "<ul> "
                              ...
                              .arg(mime)
                              .arg(primaryMimeType),
                    QString::null, i18n("Convert"), i18n("Keep"), QString::fromLatin1("kdeprintAutoConvert")))
               == KMessageBox::Yes)
            {
                ...
                if(flist.count() == 0)
                {
                    KMessageBox::error(NULL, i18n("<qt>No appropriate filter was found to convert the file format %1 into %2.<br>"
                                                  "<ul>"
