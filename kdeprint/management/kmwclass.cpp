...
KMWClass::KMWClass(QWidget *parent, const char *name) : KMWizardPage(parent, name)
{
    m_ID = KMWizard::Class;
...
void KMWClass::initPrinter(KMPrinter *p)
{
    ...
    if(list)
    {
        ...
        for(; it.current(); ++it)
            if(it.current()->instanceName().isEmpty() && !it.current()->isClass(true) && !it.current()->isSpecial()
               && !members.contains(it.current()->name()))
    ...
    for(QStringList::ConstIterator it = members.begin(); it != members.end(); ++it)
    {
        ...
        if(pr)
            m_list2->insertItem(SmallIcon(pr->pixmap()), *it);
    }
    m_list2->sort();
}
