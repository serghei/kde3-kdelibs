...
KMPropBackend::KMPropBackend(QWidget *parent, const char *name) : KMPropWidget(parent, name)
{
...
void KMPropBackend::setPrinter(KMPrinter *p)
{
    if(p && p->isPrinter())
    {
        ...
        if(prot == "ipp" || prot == "http")
            m_type->setText(i18n("IPP Printer"));
        else if(prot == "usb")
            m_type->setText(i18n("Local USB Printer"));
        else if(prot == "parallel")
            m_type->setText(i18n("Local Parallel Printer"));
        else if(prot == "serial")
            m_type->setText(i18n("Local Serial Printer"));
        else if(prot == "socket")
            m_type->setText(i18n("Network Printer (socket)"));
        else if(prot == "smb")
            m_type->setText(i18n("SMB printers (Windows)"));
        else if(prot == "lpd")
            m_type->setText(i18n("Remote LPD queue"));
        else if(prot == "file")
            m_type->setText(i18n("File printer"));
        else if(prot == "fax")
            m_type->setText(i18n("Serial Fax/Modem printer"));
        else
            m_type->setText(i18n("Unknown Protocol", "Unknown"));
        emit enable(true);
