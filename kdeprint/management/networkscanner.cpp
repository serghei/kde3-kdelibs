...
NetworkScanner::NetworkScanner(int port, QWidget *parent, const char *name) : QWidget(parent, name)
{
...
void NetworkScanner::slotScanClicked()
{
    if(!d->scanning)
    {
        if(d->localPrefix() == d->prefixaddress
           || KMessageBox::warningContinueCancel(this->parentWidget(), i18n("You are about to scan a subnet (%1.*) that does not "
                                                                            "correspond to the current subnet of this computer (%2.*). Do you want "
                                                                            "to scan the specified subnet anyway?")
                                                                           .arg(d->prefixaddress)
                                                                           .arg(d->localPrefix()),
                                                 QString::null, KGuiItem(i18n("&Scan"), "viewmag"), "askForScan")
...
bool NetworkScanner::checkPrinter(const QString &host, int port)
{
    ...
    for(; it.current(); ++it)
    {
        if(port == it.current()->Port && (host == it.current()->IP || host == it.current()->Name))
            return true;
    }
