...
KMIconViewItem::KMIconViewItem(QIconView *parent, KMPrinter *p) : QIconViewItem(parent)
{
    m_state = 0;
...
KMIconView::KMIconView(QWidget *parent, const char *name) : KIconView(parent, name)
{
...
KMIconViewItem *KMIconView::findItem(KMPrinter *p)
{
    if(p)
    {
        ...
        for(; it.current(); ++it)
            if(it.current()->text() == p->name() && it.current()->isClass() == p->isClass())
                return it.current();
    }
    return 0;
}
