...
KMListViewItem::KMListViewItem(QListView *parent, const QString &txt) : QListViewItem(parent, txt)
{
    init();
}

KMListViewItem::KMListViewItem(QListViewItem *parent, const QString &txt) : QListViewItem(parent, txt)
{
    init();
}

KMListViewItem::KMListViewItem(QListViewItem *parent, KMPrinter *p) : QListViewItem(parent)
{
    init(p);
}
...
KMListView::KMListView(QWidget *parent, const char *name) : QListView(parent, name)
{
...
KMListViewItem *KMListView::findItem(KMPrinter *p)
{
    if(p)
    {
        ...
        for(; it.current(); ++it)
            if(isVirtual)
            {
                if(it.current()->depth() == 3 && it.current()->text(0) == p->instanceName() && it.current()->parent()->text(0) == p->printerName())
                    return it.current();
            }
            else
            {
        ...
        for(; it.current(); ++it)
        {
            ...
            if(!item)
            {
                if(it.current()->isVirtual())
                {
                    ...
                    item = new KMListViewItem((it.current()->isSpecial() ? m_specials : (it.current()->isClass(false) ? m_classes : m_printers)),
                                              it.current());
                m_items.append(item);
                changed = true;
            }
