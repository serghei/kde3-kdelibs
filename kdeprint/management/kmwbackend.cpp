...
KRadioButton::KRadioButton(const QString &txt, QWidget *parent, const char *name) : QRadioButton(txt, parent, name)
{
...
KMWBackend::KMWBackend(QWidget *parent, const char *name) : KMWizardPage(parent, name)
{
...
void KMWBackend::initPrinter(KMPrinter *p)
{
    ...
    else
    {
        s = p->deviceProtocol();
        // extract protocol
        if(s == "parallel" || s == "serial" || s == "usb")
            ID = KMWizard::Local;
        else if(s == "smb")
            ID = KMWizard::SMB;
        else if(s == "ipp" || s == "http")
            ID = KMWizard::IPP;
        else if(s == "lpd")
            ID = KMWizard::LPD;
        else if(s == "socket")
...
void KMWBackend::addBackend(int ID, bool on, int nextpage)
{
    ...
    {
        case KMWizard::Local:
            addBackend(ID, i18n("&Local printer (parallel, serial, USB)"), on, i18n("<qt><p>Locally-connected printer</p>"
                                                                                    "<p>Use this for a printer connected "
                                                                                    "to the computer via a parallel, serial or USB port.</p></qt>"),
                       nextpage);
            break;
        case KMWizard::SMB:
            addBackend(ID, i18n("&SMB shared printer (Windows)"), on, i18n("<qt><p>Shared Windows printer</p>"
            ...
            break;
        case KMWizard::LPD:
            addBackend(ID, i18n("&Remote LPD queue"), on, i18n("<qt><p>Print queue on a remote LPD server</p>"
                                                               "<p>Use this for a print queue "
                                                               "existing on a remote machine running a LPD print server.</p></qt>"),
                       nextpage);
            break;
        case KMWizard::TCP:
            addBackend(ID, i18n("Ne&twork printer (TCP)"), on, i18n("<qt><p>Network TCP printer</p>"
