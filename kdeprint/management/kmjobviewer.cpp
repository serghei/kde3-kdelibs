...
KJobListView::KJobListView(QWidget *parent, const char *name) : KListView(parent, name)
{
    setAcceptDrops(true);
...
KMJobViewer::KMJobViewer(QWidget *parent, const char *name) : KMainWindow(parent, name)
{
    m_view = 0;
    m_pop = 0;
...
void KMJobViewer::refresh(bool reload)
{
    ...
    for(; it.current(); ++it)
        if((all || it.current()->printer() == m_prname)
           && ((it.current()->state() >= KMJob::Cancelled && !active) || (it.current()->state() < KMJob::Cancelled && active))
...
void KMJobViewer::init()
{
    if(!m_view)
    {
        ...
        connect(m_view, SIGNAL(rightButtonPressed(QListViewItem *, const QPoint &, int)),
                SLOT(slotRightClicked(QListViewItem *, const QPoint &, int)));
...
void KMJobViewer::buildPrinterMenu(QPopupMenu *menu, bool use_all, bool use_specials)
{
    ...
    for(; it.current(); ++it, i++)
    {
        if(!it.current()->instanceName().isEmpty() || (it.current()->isSpecial() && !use_specials))
            continue;
...
void KMJobViewer::slotSelectionChanged()
{
    ...
    for(; it.current(); ++it)
    {
        if(it.current()->isSelected())
        {
            ...
            if(it.current()->job()->type() == KMJob::Threaded)
                thread |= 0x1;
            else
                thread |= 0x2;

            if(state == -1)
                state = it.current()->job()->state();
            else if(state != 0 && state != it.current()->job()->state())
                state = 0;
    ...
    actionCollection()
        ->action("job_hold")
        ->setEnabled(/*!remote &&*/ !completed && (thread == 2) && (state > 0) && (state != KMJob::Held) && (acts & KMJob::Hold));
    actionCollection()
        ->action("job_resume")
        ->setEnabled(/*!remote &&*/ !completed && (thread == 2) && (state > 0) && (state == KMJob::Held) && (acts & KMJob::Resume));
...
void KMJobViewer::send(int cmd, const QString &name, const QString &arg)
{
    ...
    {
        KMessageBox::error(this, "<qt>" + i18n("Unable to perform action \"%1\" on selected jobs. Error received from manager:").arg(name) + "<p>"
                                     + KMManager::self()->errorMsg() + "</p></qt>");
...
void KMJobViewer::loadPrinters()
{
    ...
    for(; it.current(); ++it)
    {
        ...
        if((it.current()->isPrinter() || it.current()->isClass(false) || (it.current()->isSpecial() && it.current()->isValid()))
...
void KMJobViewer::slotDropped(QDropEvent *e, QListViewItem *)
{
    ...
    for(KURL::List::ConstIterator it = uris.begin(); it != uris.end(); ++it)
    {
