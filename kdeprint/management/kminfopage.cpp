...
KMInfoPage::KMInfoPage(QWidget *parent, const char *name) : QWidget(parent, name)
{
...
void KMInfoPage::setPrinter(KMPrinter *p)
{
    ...
    if(p && p->isClass(false))
        m_devlabel->setText(i18n("Members:"));
    else
        m_devlabel->setText(i18n("Device:"));
    ...
    else
    {
        ...
        if(p->isImplicit())
            m_type->setText(i18n("Implicit class"));
        else if(p->isClass(false))
            if(p->isRemote())
                m_type->setText(i18n("Remote class"));
            else
                m_type->setText(i18n("Local class"));
        else if(p->isPrinter())
            if(p->isRemote())
                m_type->setText(i18n("Remote printer"));
            else
                m_type->setText(i18n("Local printer"));
        else if(p->isSpecial())
            m_type->setText(i18n("Special (pseudo) printer"));
        else
            m_type->setText(i18n("Unknown class of printer", "Unknown"));
