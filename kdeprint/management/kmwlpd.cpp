...
KMWLpd::KMWLpd(QWidget *parent, const char *name) : KMWInfoBase(2, parent, name)
{
    m_ID = KMWizard::LPD;
    ...
    setInfo(
        i18n("<p>Enter the information concerning the remote LPD queue; "
...
bool KMWLpd::isValid(QString &msg)
{
    ...
    if(!checkLpdQueue(text(0).latin1(), text(1).latin1()))
    {
        if(KMessageBox::warningContinueCancel(this,
                                              i18n("Cannot find queue %1 on server %2; do you want to continue anyway?").arg(text(1)).arg(text(0)))
           == KMessageBox::Cancel)
            return false;
    }
    return true;
