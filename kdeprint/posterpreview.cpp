...
PosterPreview::PosterPreview(QWidget *parent, const char *name) : QFrame(parent, name)
{
...
PosterPreview::PosterPreview(const QString &postersize, const QString &mediasize, QWidget *parent, const char *name) : QFrame(parent, name)
{
...
void PosterPreview::updatePoster()
{
    ...
    *m_process << "poster"
               << "-F"
               << "-m" + m_mediasize << "-p" + m_postersize << "-c" + QString::number(m_cutmargin) + "%";
...
void PosterPreview::mousePressEvent(QMouseEvent *e)
{
    if(e->button() == Qt::LeftButton && m_boundingrect.isValid())
    {
        if(m_boundingrect.contains(e->pos()))
        {
            ...
            if(m_selectedpages.find(pagenum) == m_selectedpages.end() || !(e->state() & Qt::ShiftButton))
            {
