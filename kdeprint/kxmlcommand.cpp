...
KXmlCommand::KXmlCommand(const QString &xmlId) : QObject(KXmlCommandManager::self(), "XmlCommand")
{
    init();
    d->m_name = xmlId;
}
    ...
    if(d->m_driver)
    {
        ...
        for(QMap< QString, QString >::ConstIterator it = fopts.begin(); it != fopts.end(); ++it)
        {
            ...
            if(dopt)
            {
                ...
                {
                    if((value.right(1) == "'" && value.left(1) == "'") || (value.right(1) == "\"" && value.left(1) == "\""))
...
KXmlCommandManager::KXmlCommandManager() : QObject(KMFactory::self(), "XmlCommandManager")
{
...
bool KXmlCommandManager::checkCommand(const QString &xmlId, int inputCheck, int outputCheck, QString *msg)
{
    ...
    if(status && !cmd.isEmpty() && (inputCheck > None || outputCheck > None))
    {
        ...
        if(status && outputCheck > None && (cmd.find("%out") == -1 || outputCheck == Advanced) && cmd.find("filteroutput") == -1)
        {
            status = false;
            errmsg =
                i18n("The command does not contain the required tag %1.").arg(outputCheck == Advanced ? "%filteroutput" : "{%out,%filteroutput}");
        }
    }
