...
KMLpdManager::KMLpdManager(QObject *parent, const char *name) : KMManager(parent, name)
{
...
QString KMLpdManager::programName(int f)
{
    ...
    {
        case 0:
            return conf->readPathEntry("LpdCommand", "/usr/sbin/lpc");
        case 1:
            return conf->readPathEntry("LpdQueue", "lpq");
        case 2:
            return conf->readPathEntry("LpdRemove", "lprm");
    }
    return QString::null;
}
...
bool KMLpdManager::checkGsDriver(const QString &gsdriver)
{
    ...
    {
        setErrorMsg(
            i18n("The driver device <b>%1</b> is not compiled in your GhostScript distribution. Check your installation or use another driver.")
                .arg(gsdriver));
        return false;
    }
    return true;
...
bool KMLpdManager::savePrinterDriver(KMPrinter *printer, DrMain *driver)
{
    ...
    if(driver->get("drtype") == "printtool" && !spooldir.isEmpty())
    {
        ...
        ent->m_comment = QString::fromLatin1("##PRINTTOOL3## %1 %2 %3 %4 {} {%5} %6 {}")
                             .arg(options["PRINTER_TYPE"])
                             .arg(options["GSDEVICE"])
                             .arg((resol.isEmpty() ? QString::fromLatin1("NAxNA") : resol))
                             .arg(options["PAPERSIZE"])
                             .arg(driver->name())
                             .arg((color.isEmpty() ? QString::fromLatin1("Default") : color.right(color.length() - 15)));
        ...
        if(system(cmd.data()) == 0 && savePrinttoolCfgFile(driverDirectory() + "/general.cfg.in", spooldir, options)
