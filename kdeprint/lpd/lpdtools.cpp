...
static const char *pt_pagesize[] = {"ledger", I18N_NOOP("Ledger"), "legal", I18N_NOOP("US Legal"), "letter", I18N_NOOP("US Letter"),
                                    "a4",     I18N_NOOP("A4"),     "a3",    I18N_NOOP("A3"),       "b4",     I18N_NOOP("B4"),
                                    "b5",     I18N_NOOP("B5"),     0};
static int pt_nup[] = {1, 2, 4, 8, -1};
static const char *pt_bool[] = {"YES", I18N_NOOP("Enabled"), "NO", I18N_NOOP("Disabled"), 0};
...
{
    int p1(pos), p2(0);
    while(s[p1].isSpace() && p1 < (int)s.length())
        p1++;
    if(s[p1] == '{')
    {
        p1++;
    ...
    {
        p2 = p1;
        while(!s[p2].isSpace() && p2 < (int)s.length())
            p2++;
    }
    pos = (p2 + 1);
