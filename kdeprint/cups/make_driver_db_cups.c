...
int parsePpdFile(const char *filename, FILE *output_file)
{
    ...
    while(gzgets(ppd_file, line, 4095) != Z_NULL)
    {
        ...
        if(strncmp(line, "*Manufacturer:", 14) == 0)
        ...
        else if(strncmp(line, "*ShortNickName:", 15) == 0)
            ...
            strncat(desc, value, 255 - strlen(desc));
        else if(strncmp(line, "*pnpManufacturer:", 17) == 0)
            fprintf(output_file, "PNPMANUFACTURER=%s\n", value);
        else if(strncmp(line, "*pnpModel:", 10) == 0)
            fprintf(output_file, "PNPMODEL=%s\n", value);
        else if(strncmp(line, "*LanguageVersion:", 17) == 0)
            strncat(langver, value, 63 - strlen(langver));
        else
            count--;
