...
KMWIpp::KMWIpp(QWidget *parent, const char *name) : KMWInfoBase(2, parent, name)
{
    m_ID = KMWizard::IPP;
    ...
    setInfo(
        i18n("<p>Enter the information concerning the remote IPP server "
...
void KMWIpp::updatePrinter(KMPrinter *p)
{
    ...
    if(!p->option("kde-login").isEmpty())
        url.setUser(p->option("kde-login"));
    if(!p->option("kde-password").isEmpty())
        url.setPass(p->option("kde-password"));
    p->setDevice(url.url());
}
