...
KMCupsManager::KMCupsManager(QObject *parent, const char *name, const QStringList & /*args*/) : KMManager(parent, name)
{
...
bool KMCupsManager::completePrinterShort(KMPrinter *p)
{
    ...
    */
    ...
    {
        QString value;
        if(req.text("printer-info", value))
            p->setDescription(value);
        // disabled location
        // if (req.text("printer-location",value)) p->setLocation(value);
        if(req.text("printer-make-and-model", value))
            p->setDriverInfo(value);
        ...
        if(req.name("job-sheets-supported", values))
            p->setOption("kde-banners-supported", values.join(QString::fromLatin1(",")));

        // quotas
        int ival;
        if(req.integer("job-quota-period", ival))
            p->setOption("job-quota-period", QString::number(ival));
        if(req.integer("job-k-limit", ival))
            p->setOption("job-k-limit", QString::number(ival));
        if(req.integer("job-page-limit", ival))
            p->setOption("job-page-limit", QString::number(ival));
...
void KMCupsManager::processRequest(IppRequest *req)
{
    ...
    while(attr)
    {
        ...
        else if(attrname == "printer-type")
        {
            ...
            if((value & CUPS_PRINTER_IMPLICIT))
                printer->addType(KMPrinter::Implicit);
        ...
        else if(attrname == "printer-state")
        {
            switch(attr->values[0].integer)
            {
                ...
                case IPP_PRINTER_STOPPED:
                    printer->setState(KMPrinter::Stopped);
                    break;
            }
        }
...
DrMain *KMCupsManager::loadMaticDriver(const QString &drname)
{
    ...
    {
        setErrorMsg(
            i18n("Unable to find the executable foomatic-datafile "
...
QStringList KMCupsManager::detectLocalPrinters()
{
    ...
    if(req.doRequest("/"))
    {
        ...
        {
            QString attrname(attr->name);
            if(attrname == "device-info")
                desc = attr->values[0].string.text;
            else if(attrname == "device-make-and-model")
                printer = attr->values[0].string.text;
            else if(attrname == "device-uri")
                uri = attr->values[0].string.text;
            else if(attrname == "device-class")
                cl = attr->values[0].string.text;
            if(attrname.isEmpty() || attr == req.last())
            {
                if(!uri.isEmpty())
                {
                    if(printer == "Unknown")
                        printer = QString::null;
...
{
    if(m_currentprinter && m_currentprinter->isLocal() && !m_currentprinter->isClass(true) && !m_currentprinter->isSpecial())
    {
...
QString KMCupsManager::stateInformation()
{
    return QString("%1: %2")
        .arg(i18n("Server"))
        .arg(CupsInfos::self()->host()[0] != '/' ? QString("%1:%2").arg(CupsInfos::self()->host()).arg(CupsInfos::self()->port())
                                                 : CupsInfos::self()->host());
}
...
void KMCupsManager::checkUpdatePossibleInternal()
{
    ...
    connect(m_socket, SIGNAL(connected(const KResolverEntry &)), SLOT(slotConnectionSuccess()));
    ...
    else
    {
        ...
        else
        {
            setErrorMsg(i18n("Connection to CUPS server failed. Check that the CUPS server is correctly installed and running. "
                             "Error: %1.")
                            .arg(i18n("the IPP request failed for an unknown reason")));
