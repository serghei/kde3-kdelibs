...
ImagePosition::ImagePosition(QWidget *parent, const char *name) : QWidget(parent, name)
{
    position_ = Center;
...
{
    int pos(Center);
    if(strcmp(type, "top-left") == 0)
        pos = TopLeft;
    else if(strcmp(type, "top") == 0)
        pos = Top;
    else if(strcmp(type, "top-right") == 0)
        pos = TopRight;
        ...
        pos = Right;
    else if(strcmp(type, "bottom-left") == 0)
        pos = BottomLeft;
    else if(strcmp(type, "bottom") == 0)
        pos = Bottom;
    else if(strcmp(type, "bottom-right") == 0)
        pos = BottomRight;
...
void ImagePosition::paintEvent(QPaintEvent *)
{
    ...
    switch(horiz)
    {
        ...
        default:
        case 1:
            x = (page.left() + page.right() - img.width()) / 2;
            break;
        case 2:
            x = page.right() - margin - img.width();
            break;
    }
    switch(vert)
    {
        ...
        default:
        case 1:
            y = (page.top() + page.bottom() - img.height()) / 2;
            break;
        case 2:
            y = page.bottom() - margin - img.height();
            break;
    }
