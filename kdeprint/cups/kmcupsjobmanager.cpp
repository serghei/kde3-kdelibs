...
KMCupsJobManager::KMCupsJobManager(QObject *parent, const char *name, const QStringList & /*args*/) : KMJobManager(parent, name)
{
}
        ...
        */
        ...
        switch(action)
        {
            ...
            case KMJob::Move:
                ...
                uri = QString::fromLatin1("ipp://%1/printers/%2").arg(CupsInfos::self()->hostaddr(), argstr);
    ...
    if(!mp->uri().isEmpty())
    {
        ...
        */
        ...
        req.addURI(IPP_TAG_OPERATION, "printer-uri",
                   QString("ipp://%1/%2/%3").arg(infos->hostaddr(), (mp && mp->isClass()) ? "classes" : "printers", prname));
...
void KMCupsJobManager::parseListAnswer(IppRequest &req, KMPrinter *pr)
{
    ...
    while(attr)
    {
        ...
        if(name == "job-id")
            job->setId(attr->values[0].integer);
...
void KMCupsJobManager::validatePluginActions(KActionCollection *coll, const QPtrList< KMJob > &joblist)
{
    ...
    for(; it.current(); ++it)
    {
        flag = (flag && it.current()->type() == KMJob::System && (it.current()->state() == KMJob::Queued || it.current()->state() == KMJob::Held)
                /*&& !it.current()->isRemote()*/);
    }
...
bool KMCupsJobManager::changePriority(const QPtrList< KMJob > &jobs, bool up)
{
    ...
    for(; it.current() && result; ++it)
    {
        ...
        if(up)
            value = QMIN(value + 10, 100);
        else
            value = QMAX(value - 10, 1);
...
bool KMCupsJobManager::editJobAttributes(KMJob *j)
{
    ...
    */
    ...
    if(dlg.exec())
    {
        ...
        opts["multiple-document-handling"] =
            (opts["kde-collate"] == "Collate" ? "separate-documents-collated-copies" : "separate-documents-uncollated-copies");
