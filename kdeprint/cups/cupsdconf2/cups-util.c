    ...
    cupsGetConf(void)
{
    ...
    if((cups_server = httpConnectEncrypt(cupsServer(), ippPort(), cupsEncryption())) == NULL)
    {
...
#endif
            if(strncmp(www_authenticate, "Basic", 5) == 0 || digest_tries > 1 || !pwdstring[0])
            {
/*
 ...
 */
    ...
    while((bytes =
               ...
               httpRead2
#endif
           (cups_server, buffer, sizeof(buffer)))
          > 0)
    {
...
#else
    if(ntohl(*(int *)&http->hostaddr.sin_addr) != 0x7f000001 && strcasecmp(http->hostname, "localhost") != 0)
#endif
        return (0);
