...
int getServerPid()
{
    ...
    for(uint i = 0; i < dir.count(); i++)
    {
        ...
        if(f.exists() && f.open(IO_ReadOnly))
        {
            ...
            t >> line;
            f.close();
            if(line.right(5) == "cupsd" || line.right(6).left(5) == "cupsd") // second condition for 2.4.x kernels
...
{
    QPixmap icon = KGlobal::instance()->iconLoader()->loadIcon(page->pixmap(), KIcon::NoGroup, KIcon::SizeMedium);
...
bool CupsdDialog::restartServer(QString &msg)
{
    ...
    else
    {
        ...
        else
        {
            ...
            proc << "kdesu"
                 << "-c"
                 << "kill -SIGHUP " + QString::number(serverPid);
...
bool CupsdDialog::configure(const QString &filename, QWidget *parent, QString *msg)
{
    ...
    else
    {
        ...
        if(dlg.setConfigFile(fn) && dlg.exec())
        {
            ...
            if(!needUpload)
                KMessageBox::information(parent, i18n("The config file has not been uploaded to the "
