...
bool CupsdConf::saveToFile(const QString &filename)
{
    ...
    else
    {
        ...
        if(classification_ != CLASS_NONE)
            t << "ClassifyOverride " << (classoverride_ ? "Yes" : "No") << endl;
        ...
        for(locations_.first(); locations_.current(); locations_.next())
        {
            ...
            switch(loc->encryption_)
            {
                ...
                case ENCRYPT_NEVER:
                    t << "Never";
                    break;
                case ENCRYPT_REQUIRED:
                    t << "Required";
                    break;
                default:
                case ENCRYPT_IFREQUESTED:
                    t << "IfRequested";
                    break;
            }
            t << endl;
        ...
        if(keepjobhistory_)
            t << "PreserveJobFiles " << (keepjobfiles_ ? "On" : "Off") << endl;

        t << endl << comments_["autopurgejobs"] << endl;
        if(keepjobhistory_)
            t << "AutoPurgeJobs " << (autopurgejobs_ ? "Yes" : "No") << endl;
        ...
        if(browsing_)
            t << "BrowsePort " << browseport_ << endl;

        t << endl << comments_["browseinterval"] << endl;
        if(browsing_)
            t << "BrowseInterval " << browseinterval_ << endl;

        t << endl << comments_["browsetimeout"] << endl;
        if(browsing_)
            t << "BrowseTimeout " << browsetimeout_ << endl;
        ...
        if(browsing_)
            t << "BrowseOrder " << (browseorder_ == ORDER_ALLOW_DENY ? "allow,deny" : "deny,allow") << endl;

        t << endl << comments_["implicitclasses"] << endl;
        if(browsing_)
            t << "ImplicitClasses " << (useimplicitclasses_ ? "On" : "Off") << endl;

        t << endl << comments_["implicitanyclasses"] << endl;
        if(browsing_)
            t << "ImplicitAnyClasses " << (useanyclasses_ ? "On" : "Off") << endl;

        t << endl << comments_["hideimplicitmembers"] << endl;
        if(browsing_)
            t << "HideImplicitMembers " << (hideimplicitmembers_ ? "Yes" : "No") << endl;

        t << endl << comments_["browseshortnames"] << endl;
        if(browsing_)
            t << "BrowseShortNames " << (useshortnames_ ? "Yes" : "No") << endl;
...
bool CupsdConf::parseOption(const QString &line)
{
    ...
    if(keyword == "accesslog")
        accesslog_ = value;
    else if(keyword == "autopurgejobs")
        autopurgejobs_ = (value.lower() == "yes");
    else if(keyword == "browseaddress")
        browseaddresses_.append("Send " + value);
    else if(keyword == "browseallow")
        browseaddresses_.append("Allow " + value);
    else if(keyword == "browsedeny")
        browseaddresses_.append("Deny " + value);
    else if(keyword == "browseinterval")
        browseinterval_ = value.toInt();
    else if(keyword == "browseorder")
        browseorder_ = (value.lower() == "deny,allow" ? ORDER_DENY_ALLOW : ORDER_ALLOW_DENY);
    else if(keyword == "browsepoll")
        browseaddresses_.append("Poll " + value);
    else if(keyword == "browseport")
        browseport_ = value.toInt();
    ...
    else if(keyword == "browserelay")
        browseaddresses_.append("Relay " + value);
    else if(keyword == "browseshortnames")
        useshortnames_ = (value.lower() != "no");
    else if(keyword == "browsetimeout")
        browsetimeout_ = value.toInt();
    else if(keyword == "browsing")
        browsing_ = (value.lower() != "off");
    else if(keyword == "classification")
    {
        ...
        if(cl == "none")
            ...
            classification_ = CLASS_CLASSIFIED;
        else if(cl == "confidential")
            classification_ = CLASS_CONFIDENTIAL;
        else if(cl == "secret")
            classification_ = CLASS_SECRET;
        else if(cl == "topsecret")
            classification_ = CLASS_TOPSECRET;
        else if(cl == "unclassified")
            classification_ = CLASS_UNCLASSIFIED;
        else
        {
    ...
    else if(keyword == "classifyoverride")
        classoverride_ = (value.lower() == "yes");
    else if(keyword == "datadir")
        datadir_ = value;
    else if(keyword == "defaultcharset")
        charset_ = value;
    else if(keyword == "defaultlanguage")
        language_ = value;
    else if(keyword == "documentroot")
        documentdir_ = value;
    else if(keyword == "errorlog")
        errorlog_ = value;
    else if(keyword == "filterlimit")
        filterlimit_ = value.toInt();
    else if(keyword == "fontpath")
        fontpath_ += QStringList::split(':', value, false);
    else if(keyword == "group")
        group_ = value;
    else if(keyword == "hideimplicitmembers")
        hideimplicitmembers_ = (value.lower() != "no");
    else if(keyword == "hostnamelookups")
    {
        QString h = value.lower();
        if(h == "on")
            hostnamelookup_ = HOSTNAME_ON;
        else if(h == "double")
            hostnamelookup_ = HOSTNAME_DOUBLE;
        else
            hostnamelookup_ = HOSTNAME_OFF;
    }
    else if(keyword == "implicitclasses")
        useimplicitclasses_ = (value.lower() != "off");
    else if(keyword == "implicitanyclasses")
        useanyclasses_ = (value.lower() == "on");
    else if(keyword == "keepalive")
        keepalive_ = (value.lower() != "off");
    else if(keyword == "keepalivetimeout")
        keepalivetimeout_ = value.toInt();
    else if(keyword == "listen")
        listenaddresses_.append("Listen " + value);
    else if(keyword == "loglevel")
    {
        QString ll = value.lower();
        if(ll == "none")
            loglevel_ = LOGLEVEL_NONE;
        else if(ll == "error")
            loglevel_ = LOGLEVEL_ERROR;
        else if(ll == "warn")
            loglevel_ = LOGLEVEL_WARN;
        else if(ll == "info")
            loglevel_ = LOGLEVEL_INFO;
        else if(ll == "debug")
            loglevel_ = LOGLEVEL_DEBUG;
        else if(ll == "debug2")
            loglevel_ = LOGLEVEL_DEBUG2;
    }
    else if(keyword == "maxclients")
        maxclients_ = value.toInt();
    else if(keyword == "maxjobs")
        maxjobs_ = value.toInt();
    else if(keyword == "maxjobsperprinter")
        maxjobsperprinter_ = value.toInt();
    else if(keyword == "maxjobsperuser")
        maxjobsperuser_ = value.toInt();
    ...
    else if(keyword == "pagelog")
        pagelog_ = value;
    else if(keyword == "port")
        listenaddresses_.append("Listen *:" + value);
    else if(keyword == "preservejobhistory")
        keepjobhistory_ = (value != "off");
    else if(keyword == "preservejobfiles")
        keepjobfiles_ = (value == "on");
    else if(keyword == "printcap")
        printcap_ = value;
    else if(keyword == "printcapformat")
        printcapformat_ = (value.lower() == "solaris" ? PRINTCAP_SOLARIS : PRINTCAP_BSD);
    else if(keyword == "requestroot")
        requestdir_ = value;
    else if(keyword == "remoteroot")
        remoteroot_ = value;
    else if(keyword == "ripcache")
        ripcache_ = value;
    else if(keyword == "serveradmin")
        serveradmin_ = value;
    else if(keyword == "serverbin")
        serverbin_ = value;
    else if(keyword == "servercertificate")
        encryptcert_ = value;
    else if(keyword == "serverkey")
        encryptkey_ = value;
    else if(keyword == "servername")
        servername_ = value;
    else if(keyword == "serverroot")
        serverfiles_ = value;
    else if(keyword == "ssllisten")
        listenaddresses_.append("SSLListen " + value);
    else if(keyword == "sslport")
        listenaddresses_.append("SSLListen *:" + value);
    else if(keyword == "systemgroup")
        systemgroup_ = value;
    else if(keyword == "tempdir")
        tmpfiles_ = value;
    else if(keyword == "timeout")
        clienttimeout_ = value.toInt();
...
bool CupsdConf::loadAvailableResources()
{
    ...
    if(request_)
    {
        ...
        while(attr)
        {
            ...
            else if(strcmp(attr->name, "printer-name") == 0)
...
bool CupsLocation::parseOption(const QString &line)
{
    ...
    if(keyword == "authtype")
    {
        ...
        if(a == "basic")
            authtype_ = AUTHTYPE_BASIC;
        else if(a == "digest")
            authtype_ = AUTHTYPE_DIGEST;
        else
            authtype_ = AUTHTYPE_NONE;
    }
    else if(keyword == "authclass")
    {
        ...
        if(a == "user")
            ...
            authclass_ = AUTHCLASS_SYSTEM;
        else if(a == "group")
            authclass_ = AUTHCLASS_GROUP;
        else
            authclass_ = AUTHCLASS_ANONYMOUS;
    }
    else if(keyword == "authgroupname")
        authname_ = value;
    else if(keyword == "require")
    {
        ...
        addresses_.append("Allow " + value);
    else if(keyword == "deny")
        addresses_.append("Deny " + value);
    else if(keyword == "order")
        order_ = (value.lower() == "deny,allow" ? ORDER_DENY_ALLOW : ORDER_ALLOW_DENY);
    else if(keyword == "encryption")
    {
        QString e = value.lower();
        if(e == "always")
            encryption_ = ENCRYPT_ALWAYS;
        else if(e == "never")
            encryption_ = ENCRYPT_NEVER;
        else if(e == "required")
            encryption_ = ENCRYPT_REQUIRED;
        else
            encryption_ = ENCRYPT_IFREQUESTED;
    }
    else if(keyword == "satisfy")
        satisfy_ = (value.lower() == "any" ? SATISFY_ANY : SATISFY_ALL);
    else
        return false;
    return true;
}
...
{
    if(text == i18n("Base", "Root") || text == i18n("All printers") || text == i18n("All classes") || text == i18n("Print jobs"))
        return RESOURCE_GLOBAL;
    else if(text == i18n("Administration"))
...
int CupsResource::typeFromPath(const QString &path)
{
    if(path == "/admin")
        return RESOURCE_ADMIN;
    else if(path == "/printers" || path == "/classes" || path == "/" || path == "/jobs")
        return RESOURCE_GLOBAL;
    else if(path.left(9) == "/printers")
        return RESOURCE_PRINTER;
    else if(path.left(8) == "/classes")
        return RESOURCE_CLASS;
    else
        return RESOURCE_GLOBAL;
}
...
{
    QString path("/");
    if(text == i18n("Administration"))
        path = "/admin";
    else if(text == i18n("All printers"))
        path = "/printers";
    else if(text == i18n("All classes"))
        path = "/classes";
    else if(text == i18n("Print jobs"))
        path = "/jobs";
    else if(text == i18n("Base", "Root"))
        path = "/";
    else if(text.find(i18n("Printer")) == 0)
    {
        path = "/printers/";
...
QString CupsResource::pathToText(const QString &path)
{
    ...
    if(path == "/admin")
        ...
        text = i18n("All printers");
    else if(path == "/classes")
        text = i18n("All classes");
    else if(path == "/")
        text = i18n("Root");
    else if(path == "/jobs")
        text = i18n("Print jobs");
