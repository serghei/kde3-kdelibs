...
CupsdSecurityPage::CupsdSecurityPage(QWidget *parent, const char *name) : CupsdPage(parent, name)
{
...
void CupsdSecurityPage::slotAdd()
{
    ...
    if(LocationDialog::newLocation(loc, this, conf_))
    {
        ...
        for(locs_.first(); locs_.current(); locs_.next())
            ...
            {
                if(KMessageBox::warningContinueCancel(this, i18n("This location is already defined. Do you want to replace the existing one?"),
                                                      QString::null, i18n("Replace"))
