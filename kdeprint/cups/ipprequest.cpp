...
{
    if(!name.isEmpty())
        ippAddInteger(request_, (ipp_tag_t)group, (ipp_tag_t)type, name.latin1(), value);
}
...
{
    if(!name.isEmpty())
        ippAddBoolean(request_, (ipp_tag_t)group, name.latin1(), (char)value);
}
...
bool IppRequest::doFileRequest(const QString &res, const QString &filename)
{
    ...
    int myPort = port_;
    if(myHost.isEmpty())
        myHost = CupsInfos::self()->host();
    if(myPort <= 0)
        myPort = CupsInfos::self()->port();
        ...
        for(int i = 0; i < attr->num_values; i++)
        {
            switch(attr->value_tag)
            {
                ...
                case IPP_TAG_RESOLUTION:
                    output << "( " << attr->values[i].resolution.xres << ", " << attr->values[i].resolution.yres << " )";
                    break;
                case IPP_TAG_RANGE:
                ...
                case IPP_TAG_DATE:
                    ...
                    dateStr.sprintf("%.4d-%.2d-%.2d, %.2d:%.2d:%.2d %c%.2d%.2d", d[0] * 256 + d[1], d[2], d[3], d[4], d[5], d[6], d[8], d[9], d[10]);
                    output << dateStr;
                    break;
                default:
                    continue;
...
void IppRequest::setMap(const QMap< QString, QString > &opts)
{
    ...
    for(QMap< QString, QString >::ConstIterator it = opts.begin(); it != opts.end(); ++it)
    {
        ...
        else if(value.isEmpty() || lovalue == "off" || lovalue == "on" || lovalue == "yes" || lovalue == "no" || lovalue == "true"
                || lovalue == "false")
