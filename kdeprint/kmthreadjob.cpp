...
KMThreadJob::KMThreadJob(QObject *parent, const char *name) : QObject(parent, name)
{
...
bool KMThreadJob::saveJobs()
{
    ...
    if(f.open(IO_WriteOnly))
    {
        ...
        for(; it.current(); ++it)
            t << it.current()->id() << CHARSEP << it.current()->name() << CHARSEP << it.current()->printer() << CHARSEP << it.current()->owner()
              << CHARSEP << it.current()->size() << endl;
        return true;
    }
    return false;
}
