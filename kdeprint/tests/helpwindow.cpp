...
HelpWindow::HelpWindow(const QString &home_, const QString &_path, QWidget *parent, const char *name)
    : KMainWindow(parent, name, WDestructiveClose), pathCombo(0), selectedURL()
{
    readHistory();
    readBookmarks();
    ...
    backwardId = go->insertItem(icon_back, "&Backward", browser, SLOT(backward()), ALT | Key_Left);
    forwardId = go->insertItem(icon_forward, "&Forward", browser, SLOT(forward()), ALT | Key_Right);
...
void HelpWindow::print()
{
    ...
    if(printer.setup(this))
    {
        ...
        QFont font("times", 10);
#endif
        QSimpleRichText richText(browser->text(), font, browser->context(), browser->styleSheet(), browser->mimeSourceFactory(), body.height());
        ...
        do
        {
            ...
            p.setFont(font);
            p.drawText(view.right() - p.fontMetrics().width(QString::number(page)), view.bottom() + p.fontMetrics().ascent() + 5,
                       QString::number(page));
