...
KPrintDialog::KPrintDialog(QWidget *parent, const char *name) : KDialog(parent, name, true)
{
...
void KPrintDialog::initialize(KPrinter *printer)
{
    ...
    {
        KMessageBox::error(parentWidget(), "<qt><nobr>" + i18n("An error occurred while retrieving the printer list:") + "</nobr><br><br>"
                                               + KMManager::self()->errorMsg() + "</qt>");
    }
    ...
    if(plist)
    {
        ...
        for(; it.current(); ++it)
        {
            ...
            d->m_printers->insertItem(
                SmallIcon(it.current()->pixmap(), 0, (it.current()->isValid() ? (int)KIcon::DefaultState : (int)KIcon::LockOverlay)),
                it.current()->name(), false /*sep*/);
    ...
    else
    {
        ...
        do
        {
            ...
            if(f.exists())
            {
                if(f.isWritable())
                {
                    // value = (KMessageBox::warningYesNo(this,i18n("File \"%1\" already exists. Overwrite?").arg(f.absFilePath())) ==
                    // KMessageBox::Yes);
                    time_t mtimeDest = f.lastModified().toTime_t();
                    KIO::RenameDlg dlg(this, i18n("Print"), QString::null, d->m_file->url(), KIO::M_OVERWRITE, (time_t)-1, f.size(), (time_t)-1,
                                       f.created().toTime_t(), mtimeDest + 1, mtimeDest, true);
                    int result = dlg.exec();
                    switch(result)
                    {
