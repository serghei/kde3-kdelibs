...
KDockSplitter::KDockSplitter(QWidget *parent, const char *name, Orientation orient, int pos) : QWidget(parent, name)
{
    m_dontRecalc = false;
    divider = 0L;
    child0 = 0L;
...
void KDockSplitter::resizeEvent(QResizeEvent *ev)
{
    ...
    //  kdDebug(282)<<"ResizeEvent :"<< ((initialised) ? "initialised":"not initialised")<<", "<< ((ev) ? "real event":"")<<", "<<(isVisible()
    //  ?"visible":"")<<endl;
    ...
    if(initialised)
    {
        ...
        */
        ...
        {
            if((c0->getWidget()) && (dc = dynamic_cast< KDockContainer * >(c0->getWidget())) && (dc->isOverlapMode()))
            {
            ...
            {
                if((c1->getWidget()) && (dc = dynamic_cast< KDockContainer * >(c1->getWidget())) && (dc->isOverlapMode()))
                {
...
/*************************************************************************/
KDockButton_Private::KDockButton_Private(QWidget *parent, const char *name) : QPushButton(parent, name)
{
    moveMouse = false;
