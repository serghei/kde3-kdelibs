...
KRuler::KRuler(QWidget *parent, const char *name) : QFrame(parent, name), range(INIT_MIN_VALUE, INIT_MAX_VALUE, 1, 10, INIT_VALUE), dir(Horizontal)
{
    init();
...
KRuler::KRuler(Orientation orient, QWidget *parent, const char *name, WFlags f)
    : QFrame(parent, name, f), range(INIT_MIN_VALUE, INIT_MAX_VALUE, 1, 10, INIT_VALUE), dir(orient)
{
    init();
...
KRuler::KRuler(Orientation orient, int widgetWidth, QWidget *parent, const char *name, WFlags f)
    : QFrame(parent, name, f), range(INIT_MIN_VALUE, INIT_MAX_VALUE, 1, 10, INIT_VALUE), dir(orient)
{
    init();
...
void KRuler::drawContents(QPainter *p)
{
//  debug ("KRuler::drawContents, %s",(horizontal==dir)?"horizontal":"vertical");
    ...
    for(int profile = 0; profile < 10; profile++)
    {
        ...
        int value = range.value(), minval = range.minValue(), maxval;
        if(dir == Horizontal)
        {
            maxval = range.maxValue() + offset_ - (d->lengthFix ? (height() - d->endOffset_length) : d->endOffset_length);
        }
        else
        {
            maxval = range.maxValue() + offset_ - (d->lengthFix ? (width() - d->endOffset_length) : d->endOffset_length);
        }
        // ioffsetval = value-offset;
        //    pixelpm = (int)ppm;
        //    left  = clip.left(),
        //    right = clip.right();
        double f, fend, offsetmin = (double)(minval - offset_), offsetmax = (double)(maxval - offset_),
        ...
        if(d->showEndL)
        {
            ...
            { // rotate text +pi/2 and move down a bit
              // QFontMetrics fm(font);
