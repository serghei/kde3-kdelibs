...
void KDatePicker::fillWeeksCombo(const QDate &date)
{
    ...
    for(; day <= lastDay; day = calendar->addDays(day, 7 /*calendar->daysOfWeek()*/))
    {
        ...
        if(year != calendar->year(day))
            week += "*"; // show that this is a week from a different year
...
KDatePicker::KDatePicker(QWidget *parent, QDate dt, const char *name) : QFrame(parent, name)
{
    init(dt);
}

KDatePicker::KDatePicker(QWidget *parent, QDate dt, const char *name, WFlags f) : QFrame(parent, name, f)
{
    init(dt);
}

KDatePicker::KDatePicker(QWidget *parent, const char *name) : QFrame(parent, name)
{
...
bool KDatePicker::eventFilter(QObject *o, QEvent *e)
{
    if(e->type() == QEvent::KeyPress)
    {
        ...
        if((k->key() == Qt::Key_Prior) || (k->key() == Qt::Key_Next) || (k->key() == Qt::Key_Up) || (k->key() == Qt::Key_Down))
        {
...
{
} // ### in 3.2 obsolete; kept for binary compatibility
...
void KDatePicker::selectMonthClicked()
{
    ...
    if((month = popup.exec(selectMonth->mapToGlobal(QPoint(0, 0)), calendar->month(date) - 1)) == -1)
        return; // canceled
...
void KDatePicker::selectYearClicked()
{
    ...
    if(popup->exec(selectYear->mapToGlobal(QPoint(0, selectMonth->height()))))
    {
        ...
        calendar->setYMD(date, year, calendar->month(date), QMIN(day, calendar->daysInMonth(date)));
        // ----- set this month
        setDate(date);
    }
    else
...
void KDatePicker::ensureSelectYearIsUp()
{
...
{
    QWidget *widgets[] = {yearForward, yearBackward, monthForward, monthBackward, selectMonth,
                          selectYear,  line,         table,        d->selectWeek, d->todayButton};
...
void KDatePicker::setFontSize(int s)
{
    ...
    for(int i = 1;; ++i)
    {
        QString str = KGlobal::locale()->calendar()->monthName(i, KGlobal::locale()->calendar()->year(table->getDate()), false);
        if(str.isNull())
            break;
...
void KDatePicker::setCloseButton(bool enable)
{
    ...
    if(enable)
    {
        ...
        connect(d->closeButton, SIGNAL(clicked()), topLevelWidget(), SLOT(close()));
    }
    else
    {
