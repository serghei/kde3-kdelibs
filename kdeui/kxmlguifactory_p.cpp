...
ContainerNode::ContainerNode(QWidget *_container, const QString &_tagName, const QString &_name, ContainerNode *_parent, KXMLGUIClient *_client,
                             KXMLGUIBuilder *_builder, int id, const QString &_mergingName, const QString &_groupName, const QStringList &customTags,
 ...
 */
ContainerNode *ContainerNode::findContainer(const QString &name, const QString &tagName, const QPtrList< QWidget > *excludeList,
...
ContainerClient *ContainerNode::findChildContainerClient(KXMLGUIClient *currentGUIClient, const QString &groupName,
...
void ContainerNode::plugActionList(BuildState &state, const MergingIndexList::Iterator &mergingIdxIt)
{
    ...
    ContainerClient *client = findChildContainerClient(state.guiClient, QString::null, mergingIndices.end());
...
void ContainerNode::adjustMergingIndices(int offset, const MergingIndexList::Iterator &it)
{
...
bool ContainerNode::destruct(QDomElement element, BuildState &state)
{
    ...
    if(clients.count() == 0 && children.count() == 0 && container && client == state.guiClient)
    {
...
QDomElement ContainerNode::findElementForChild(const QDomElement &baseElement, ContainerNode *childNode)
{
    ...
    for(QDomNode n = baseElement.firstChild(); !n.isNull(); n = n.nextSibling())
    {
        ...
        if(e.tagName().lower() == childNode->tagName && e.attribute(attrName) == childNode->name)
            return e;
    }
...
void ContainerNode::unplugClient(ContainerClient *client)
{
    ...
    adjustMergingIndices(-int(client->actions.count() + client->customElements.count()), mergingIt);
...
int ContainerNode::calcMergingIndex(const QString &mergingName, MergingIndexList::Iterator &it, BuildState &state, bool ignoreDefaultMergingIndex)
{
    ...
    if((mergingIt == mergingEnd && state.currentDefaultMergingIt == mergingEnd) || ignoreDefaultMergingIndex)
        return index;
...
BuildHelper::BuildHelper(BuildState &state, ContainerNode *node)
    : containerClient(0), ignoreDefaultMergingIndex(false), m_state(state), parentNode(node)
{
    ...
    parentNode->calcMergingIndex(QString::null, m_state.currentClientMergingIt, m_state, /*ignoreDefaultMergingIndex*/ false);
}
...
bool BuildHelper::processActionElement(const QDomElement &e, int idx)
{
    ...
    // kdDebug() << "BuildHelper::processActionElement " << e.attribute( "name" ) << " -> " << action << " (in " <<
    // m_state.guiClient->actionCollection() << ")" << endl;
    if(!action)
        return false;
...
void BuildHelper::processStateElement(const QDomElement &element)
{
    ...
    for(QDomNode n = element.firstChild(); !n.isNull(); n = n.nextSibling())
    {
        ...
        for(QDomNode n2 = n.firstChild(); !n2.isNull(); n2 = n2.nextSibling())
        {
            ...
            if(actionEl.tagName().lower() != "action")
                continue;
...
void BuildHelper::processMergeElement(const QString &tag, const QString &name, const QDomElement &e)
{
    ...
    m_state.currentDefaultMergingIt = parentNode->findIndex(defaultMergingName);
    parentNode->calcMergingIndex(QString::null, m_state.currentClientMergingIt, m_state, ignoreDefaultMergingIndex);
}
...
void BuildHelper::processContainerElement(const QDomElement &e, const QString &tag, const QString &name)
{
    ...
    if(!containerNode)
    {
        ...
        containerNode = new ContainerNode(container, tag, name, parentNode, m_state.guiClient, builder, id, mergingName, group, cusTags, conTags);
    }
...
QWidget *BuildHelper::createContainer(QWidget *parent, int index, const QDomElement &element, int &id, KXMLGUIBuilder **builder)
{
    QWidget *res = 0L;
