...
KListView::Tooltip::Tooltip(KListView *parent, QToolTipGroup *group) : QToolTip(parent, group), mParent(parent)
{
}
...
KListViewLineEdit::KListViewLineEdit(KListView *parent) : KLineEdit(parent->viewport()), item(0), col(0), p(parent)
{
    setFrame(false);
    hide();
    connect(parent, SIGNAL(selectionChanged()), SLOT(slotSelectionChanged()));
    connect(parent, SIGNAL(itemRemoved(QListViewItem *)), SLOT(slotItemRemoved(QListViewItem *)));
}
...
void KListViewLineEdit::selectNextCell(QListViewItem *pitem, int column, bool forward)
{
    ...
    QListViewItem *top = (pitem && pitem->parent()) ? pitem->parent()->firstChild() : p->firstChild();
...
bool KListViewLineEdit::event(QEvent *pe)
{
    if(pe->type() == QEvent::KeyPress)
    {
        ...
        if((k->key() == Qt::Key_Backtab || k->key() == Qt::Key_Tab) && p->tabOrderedRenaming() && p->itemsRenameable()
...
KListView::KListView(QWidget *parent, const char *name) : QListView(parent, name), d(new KListViewPrivate(this))
{
    ...
    connect(this, SIGNAL(contentsMoving(int, int)), this, SLOT(cleanDropVisualizer()));
    connect(this, SIGNAL(contentsMoving(int, int)), this, SLOT(cleanItemHighlighter()));
    ...
    connect(&d->autoSelect, SIGNAL(timeout()), this, SLOT(slotAutoSelect()));
    connect(&d->dragExpand, SIGNAL(timeout()), this, SLOT(slotDragExpand()));

    // context menu handling
    if(d->showContextMenusOnPress)
    {
        connect(this, SIGNAL(rightButtonPressed(QListViewItem *, const QPoint &, int)), this,
                SLOT(emitContextMenu(QListViewItem *, const QPoint &, int)));
    }
    else
    {
    ...
    connect(this, SIGNAL(menuShortCutPressed(KListView *, QListViewItem *)), this, SLOT(emitContextMenu(KListView *, QListViewItem *)));
...
void KListView::slotSettingsChanged(int category)
{
    switch(category)
    {
        case KApplication::SETTINGS_MOUSE:
            ...
            disconnect(this, SIGNAL(mouseButtonClicked(int, QListViewItem *, const QPoint &, int)), this,
...
void KListView::focusInEvent(QFocusEvent *fe)
{
    ...
    if((d->selectedBySimpleMove) && (d->selectionMode == FileManager) && (fe->reason() != QFocusEvent::Popup)
       && (fe->reason() != QFocusEvent::ActiveWindow) && (currentItem()))
    {
...
void KListView::focusOutEvent(QFocusEvent *fe)
{
    ...
    if((d->selectedBySimpleMove) && (d->selectionMode == FileManager) && (fe->reason() != QFocusEvent::Popup)
       && (fe->reason() != QFocusEvent::ActiveWindow) && (currentItem()) && (!d->editor->isVisible()))
    {
...
void KListView::contentsMousePressEvent(QMouseEvent *e)
{
    ...
    bool rootDecoClicked =
        at && (p.x() <= header()->cellPos(header()->mapToActual(0)) + treeStepSize() * (at->depth() + (rootIsDecorated() ? 1 : 0)) + itemMargin())
...
void KListView::contentsMouseMoveEvent(QMouseEvent *e)
{
    ...
    if(item && d->bChangeCursorOverItem && d->bUseSingle)
    {
        ...
        if((item != d->pCurrentItem) || (isExecuteArea(vp) != d->cursorInExecuteArea))
        {
    ...
    QPoint newPos = e->pos();
    if(dragOn && d->validDrag && (newPos.x() > d->startDragPos.x() + d->dragDelay || newPos.x() < d->startDragPos.x() - d->dragDelay
                                  || newPos.y() > d->startDragPos.y() + d->dragDelay || newPos.y() < d->startDragPos.y() - d->dragDelay))
...
void KListView::contentsMouseReleaseEvent(QMouseEvent *e)
{
    if(e->button() == LeftButton)
    {
        ...
        if(d->pressedOnSelected && itemsRenameable())
        {
            ...
            {
                // true if the root decoration of the item "at" was clicked (i.e. the +/- sign)
                bool rootDecoClicked = (p.x() <= header()->cellPos(header()->mapToActual(0))
...
void KListView::findDrop(const QPoint &pos, QListViewItem *&parent, QListViewItem *&after)
{
    ...
    if(above)
    {
        ...
        if(above->isExpandable())
        {
            ...
            if(p.x() >= depthToPixels(above->depth() + 1) || (above->isOpen() && above->childCount() > 0))
            {
                parent = above;
                after = 0L;
                return;
...
QRect KListView::drawDropVisualizer(QPainter *p, QListViewItem *parent, QListViewItem *after)
{
    QRect insertmarker;
...
QRect KListView::drawItemHighlighter(QPainter *painter, QListViewItem *item)
{
    ...
    if(item)
    {
        ...
        if(painter)
            style().drawPrimitive(QStyle::PE_FocusRect, painter, r, colorGroup(), QStyle::Style_FocusAtBorder, colorGroup().highlight());
    }
...
void KListView::fileManagerKeyPressEvent(QKeyEvent *e)
{
    ...
    if((e->key() != Key_Shift) && (e->key() != Key_Control) && (e->key() != Key_Meta) && (e->key() != Key_Alt))
    {
    ...
    if(((!selectedItems) || ((selectedItems == 1) && (d->selectedUsingMouse))) && (e_state == NoButton)
       && ((e->key() == Key_Down) || (e->key() == Key_Up) || (e->key() == Key_Next) || (e->key() == Key_Prior) || (e->key() == Key_Home)
           || (e->key() == Key_End)))
    {
        d->selectedBySimpleMove = true;
        d->selectedUsingMouse = false;
    ...
    switch(e->key())
    {
        ...
        case Key_Plus:
            ...
            break;
        default:
            bool realKey = ((e->key() != Key_Shift) && (e->key() != Key_Control) && (e->key() != Key_Meta) && (e->key() != Key_Alt));
...
void KListView::viewportPaintEvent(QPaintEvent *e)
{
    ...
    if(d->mOldDropHighlighter.isValid() && e->rect().intersects(d->mOldDropHighlighter))
    {
        ...
        style().drawPrimitive(QStyle::PE_FocusRect, &painter, d->mOldDropHighlighter, colorGroup(), QStyle::Style_FocusAtBorder);
    }
    d->painting = false;
}
...
KListViewItem::KListViewItem(QListView *parent) : QListViewItem(parent)
{
    init();
}

KListViewItem::KListViewItem(QListViewItem *parent) : QListViewItem(parent)
{
    init();
}

KListViewItem::KListViewItem(QListView *parent, QListViewItem *after) : QListViewItem(parent, after)
{
    init();
}

KListViewItem::KListViewItem(QListViewItem *parent, QListViewItem *after) : QListViewItem(parent, after)
{
    init();
}

KListViewItem::KListViewItem(QListView *parent, QString label1, QString label2, QString label3, QString label4, QString label5, QString label6,
...
KListViewItem::KListViewItem(QListViewItem *parent, QString label1, QString label2, QString label3, QString label4, QString label5, QString label6,
                             QString label7, QString label8)
    : QListViewItem(parent, label1, label2, label3, label4, label5, label6, label7, label8)
{
    init();
...
KListViewItem::KListViewItem(QListView *parent, QListViewItem *after, QString label1, QString label2, QString label3, QString label4, QString label5,
...
KListViewItem::KListViewItem(QListViewItem *parent, QListViewItem *after, QString label1, QString label2, QString label3, QString label4,
...
QColor KListViewItem::backgroundColor(int column)
{
    ...
    QColor color = isAlternate() ? view->alternateBackground() : view->viewport()->colorGroup().base();
...
void KListViewItem::paintCell(QPainter *p, const QColorGroup &cg, int column, int width, int alignment)
{
    ...
    {
        _cg.setColor((lv->viewport()->backgroundMode() == Qt::FixedColor) ? QColorGroup::Background : QColorGroup::Base, backgroundColor(column));
    }
