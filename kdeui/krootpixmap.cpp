...
KRootPixmap::KRootPixmap(QWidget *widget, const char *name) : QObject(widget, name ? name : "KRootPixmap"), m_Desk(0), m_pWidget(widget)
{
    init();
}
...
{
    if(wallpaperForDesktop(m_Desk) == wallpaperForDesktop(desktop) && !wallpaperForDesktop(m_Desk).isNull())
        return;

#ifdef Q_WS_X11
    if(KWin::windowInfo(m_pWidget->topLevelWidget()->winId()).desktop() == NET::OnAllDesktops && pixmapName(m_Desk) != pixmapName(desktop))
#endif
        repaint(true);
}
...
{
#ifdef Q_WS_X11
    if(!(properties & NET::WMDesktop) || (window != m_pWidget->topLevelWidget()->winId()))
        return;
#endif
