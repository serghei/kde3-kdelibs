...
KPixmapIO::KPixmapIO()
{
    ...
    d->ximage = XShmCreateImage(qt_xdisplay(), (Visual *)QPaintDevice::x11AppVisual(), QPaintDevice::x11AppDepth(), ZPixmap, 0L, d->shminfo, 10, 10);
    ...
    else
    {
        ...
        kdWarning(290) << "red = " << red_shift << ", green = " << green_shift << ", blue = " << blue_shift << endl;
...
void KPixmapIO::putImage(QPixmap *dst, const QPoint &offset, const QImage *src)
{
...
void KPixmapIO::putImage(QPixmap *dst, int dx, int dy, const QImage *src)
{
    ...
    if(m_bShm && (src->depth() > 1) && (d->bpp > 8) && (size > d->threshold))
    {
        ...
        {
            convertToXImage(*src);
            XShmPutImage(qt_xdisplay(), dst->handle(), qt_xget_temp_gc(qt_xscreen(), false), d->ximage, dx, dy, 0, 0, src->width(), src->height(),
                         false);
...
{
    destroyXImage();
    d->ximage = XShmCreateImage(qt_xdisplay(), (Visual *)QPaintDevice::x11AppVisual(), QPaintDevice::x11AppDepth(), ZPixmap, 0L, d->shminfo, w, h);
...
QImage KPixmapIO::convertFromXImage()
{
    ...
    if(d->bpp == 8)
    {
        ...
        for(i = 0; i < ncells; i++)
            cmap[i].pixel = i;
        XQueryColors(qt_xdisplay(), QPaintDevice::x11AppColormap(), cmap, ncells);
    ...
    switch(d->byteorder)
    {
        ...
        case bo16_BGR_565:
        {
            ...
            for(y = 0; y < height; y++)
            {
                ...
                {
                    pixel = *src++;
                    val = ((pixel & 0xf800) << 8) | ((pixel & 0x7e0) << 5) | ((pixel & 0x1f) << 3);
                    *dst++ = val;
                    pixel >>= 16;
                    val = ((pixel & 0xf800) << 8) | ((pixel & 0x7e0) << 5) | ((pixel & 0x1f) << 3);
                    *dst++ = val;
        ...
        case bo16_BGR_555:
        {
            ...
            for(y = 0; y < height; y++)
            {
                ...
                {
                    pixel = *src++;
                    val = ((pixel & 0x7c00) << 9) | ((pixel & 0x3e0) << 6) | ((pixel & 0x1f) << 3);
                    *dst++ = val;
                    pixel >>= 16;
                    val = ((pixel & 0x7c00) << 9) | ((pixel & 0x3e0) << 6) | ((pixel & 0x1f) << 3);
                    *dst++ = val;
        ...
        case bo16_BGR_555:
            ...
            if(img.depth() == 32)
            {
                ...
                for(y = 0; y < height; y++)
                {
                    ...
                    {
                        pixel = *src++;
                        val = ((pixel & 0xf80000) >> 9) | ((pixel & 0xf800) >> 6) | ((pixel & 0xff) >> 3);
                        pixel = *src++;
                        val |= (((pixel & 0xf80000) >> 9) | ((pixel & 0xf800) >> 6) | ((pixel & 0xff) >> 3)) << 16;
                        *dst++ = val;
                    }
                    if(width % 2)
                    {
                        pixel = *src++;
                        *((Q_INT16 *)dst) = ((pixel & 0xf80000) >> 9) | ((pixel & 0xf800) >> 6) | ((pixel & 0xff) >> 3);
                    }
                }
            }
            else
        ...
        case bo16_BGR_565:
            ...
            if(img.depth() == 32)
            {
                ...
                for(y = 0; y < height; y++)
                {
                    ...
                    {
                        pixel = *src++;
                        val = ((pixel & 0xf80000) >> 8) | ((pixel & 0xfc00) >> 5) | ((pixel & 0xff) >> 3);
                        pixel = *src++;
                        val |= (((pixel & 0xf80000) >> 8) | ((pixel & 0xfc00) >> 5) | ((pixel & 0xff) >> 3)) << 16;
                        *dst++ = val;
                    }
                    if(width % 2)
                    {
                        pixel = *src++;
                        *((Q_INT16 *)dst) = ((pixel & 0xf80000) >> 8) | ((pixel & 0xfc00) >> 5) | ((pixel & 0xff) >> 3);
                    }
                }
            }
            else
...
bool KPixmapIO::initXImage(int, int)
{
    return false;
}
void KPixmapIO::doneXImage()
...
bool KPixmapIO::createXImage(int, int)
{
    return false;
}
void KPixmapIO::destroyXImage()
...
bool KPixmapIO::createShmSegment(int)
{
    return false;
}
void KPixmapIO::destroyShmSegment()
