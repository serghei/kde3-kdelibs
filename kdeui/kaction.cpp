...
public:
    ...
    struct Container
    {
        ...
        Container(const Container &s)
        {
            ...
            m_id = s.m_id;
            m_representative = s.m_representative;
        }
        QWidget *m_container;
        int m_id;
...
KAction::KAction(const QString &text, const KShortcut &cut, const QObject *receiver, const char *slot, KActionCollection *parent, const char *name)
    : QObject(parent, name), d(new KActionPrivate)
{
    initPrivate(text, cut, receiver, slot);
}

KAction::KAction(const QString &text, const QString &sIconName, const KShortcut &cut, const QObject *receiver, const char *slot,
...
KAction::KAction(const QString &text, const QIconSet &pix, const KShortcut &cut, const QObject *receiver, const char *slot, KActionCollection *parent,
                 const char *name)
    : QObject(parent, name), d(new KActionPrivate)
{
    initPrivate(text, cut, receiver, slot);
...
KAction::KAction(const KGuiItem &item, const KShortcut &cut, const QObject *receiver, const char *slot, KActionCollection *parent, const char *name)
    : QObject(parent, name), d(new KActionPrivate)
{
...
KAction::KAction(const QString &text, const KShortcut &cut, QObject *parent, const char *name) : QObject(parent, name), d(new KActionPrivate)
{
    initPrivate(text, cut, 0, 0);
}

KAction::KAction(const QString &text, const KShortcut &cut, const QObject *receiver, const char *slot, QObject *parent, const char *name)
    : QObject(parent, name), d(new KActionPrivate)
{
    initPrivate(text, cut, receiver, slot);
}

KAction::KAction(const QString &text, const QIconSet &pix, const KShortcut &cut, QObject *parent, const char *name)
    : QObject(parent, name), d(new KActionPrivate)
{
    initPrivate(text, cut, 0, 0);
    setIconSet(pix);
...
KAction::KAction(const QString &text, const QString &pix, const KShortcut &cut, QObject *parent, const char *name)
    : QObject(parent, name), d(new KActionPrivate)
{
    initPrivate(text, cut, 0, 0);
    d->setIconName(pix);
...
KAction::KAction(const QString &text, const QIconSet &pix, const KShortcut &cut, const QObject *receiver, const char *slot, QObject *parent,
                 const char *name)
    : QObject(parent, name), d(new KActionPrivate)
{
    initPrivate(text, cut, receiver, slot);
...
KAction::KAction(const QString &text, const QString &pix, const KShortcut &cut, const QObject *receiver, const char *slot, QObject *parent,
                 const char *name)
    : QObject(parent, name), d(new KActionPrivate)
{
    initPrivate(text, cut, receiver, slot);
...
KAction::KAction(QObject *parent, const char *name) : QObject(parent, name), d(new KActionPrivate)
{
...
void KAction::initPrivate(const QString &text, const KShortcut &cut, const QObject *receiver, const char *slot)
{
    d->m_cutDefault = cut;

    m_parentCollection = dynamic_cast< KActionCollection * >(parent());
    kdDebug(129) << "KAction::initPrivate(): this = " << this << " name = \"" << name() << "\" cut = " << cut.toStringInternal()
                 << " m_parentCollection = " << m_parentCollection << endl;
    if(m_parentCollection)
...
bool KAction::initShortcut(const KShortcut &cut)
{
    ...
    if(qstrcmp(name(), "unnamed") && m_parentCollection && m_parentCollection->isAutoConnectShortcuts() && m_parentCollection->kaccel())
    {
...
void KAction::plugShortcut()
{
    ...
    // kdDebug(129) << "KAction::plugShortcut(): this = " << this << " kaccel() = " << (m_parentCollection ? m_parentCollection->kaccel() : 0) <<
    // endl;
...
bool KAction::updateKAccelShortcut(KAccel *kaccel)
{
    ...
    if(!kaccel->actions().actionPtr(name()))
    {
        if(!d->m_cut.isNull())
        {
            ...
            b = kaccel->insert(name(), d->plainText(), QString::null, d->m_cut, this, SLOT(slotActivated()), isShortcutConfigurable(), isEnabled());
        }
    }
    else
...
void KAction::insertKAccel(KAccel *kaccel)
{
    ...
    if(!kaccel->actions().actionPtr(name()))
    {
        ...
        kdWarning(129) << "KAction::insertKAccel( kaccel = " << kaccel << " ): KAccel object already contains an action name \"" << name() << "\""
                       << endl; // -- ellis
}
...
void KAction::updateShortcut(QPopupMenu *menu, int id)
{
    ...
    else
    {
        ...
        kdDebug(129) << "KAction::updateShortcut(): name = \"" << name() << "\", cut = " << d->m_cut.toStringInternal()
                     << "; No KAccel, probably missing a parent collection." << endl;
    }
}
    ...
    if(::qt_cast< QPopupMenu * >(w))
    {
        ...
        if(d->hasIcon())
        {
            ...
            id = menu->insertItem(d->iconSet(KIcon::Small, 0, instance), d->text(), this, // dsweet
                                  SLOT(slotPopupActivated()), keyQt, -1, index);
        }
        else
            id = menu->insertItem(d->text(), this, SLOT(slotPopupActivated()), keyQt, -1, index);
    ...
    else if(::qt_cast< KToolBar * >(w))
    {
        ...
        if(icon().isEmpty() && !iconSet().pixmap().isNull()) // old code using QIconSet directly
        {
            bar->insertButton(iconSet().pixmap(), id_, SIGNAL(buttonClicked(int, Qt::ButtonState)), this,
                              SLOT(slotButtonClicked(int, Qt::ButtonState)), d->isEnabled(), d->plainText(), index);
        }
        else
        {
...
void KAction::plugAccel(KAccel *kacc, bool configurable)
{
    ...
    {
        d->m_kaccel = kacc;
        d->m_kaccel->insert(name(), d->plainText(), QString::null, KShortcut(d->m_cut), this, SLOT(slotActivated()), configurable, isEnabled());
        ...
        kdWarning(129) << "KAction::plugAccel( kacc = " << kacc << " ): KAccel object already contains an action name \"" << name() << "\""
                       << endl; // -- ellis
}
    ...
    if(::qt_cast< QSignal * >(sender()))
    {
        ...
        if(pos != -1)
        {
            ...
            if(qpm)
            {
                ...
                { // just a QPopupMenu? We'll ask for the state now then (small race condition?)
