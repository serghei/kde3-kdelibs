...
KSpellConfig::KSpellConfig(const KSpellConfig &_ksc)
    : QWidget(0, 0), nodialog(true), kc(0), cb1(0), cb2(0), dictlist(0), dictcombo(0), encodingcombo(0), clientcombo(0)
{
...
KSpellConfig::KSpellConfig(QWidget *parent, const char *name, KSpellConfig *_ksc, bool addHelpButton)
    : QWidget(parent, name), nodialog(false), kc(0), cb1(0), cb2(0), dictlist(0), dictcombo(0), encodingcombo(0), clientcombo(0)
{
    d = new KSpellConfigPrivate;
    kc = KGlobal::config();
    ...
    connect(dictcombo, SIGNAL(activated(int)), this, SLOT(sSetDictionary(int)));
    ...
    connect(encodingcombo, SIGNAL(activated(int)), this, SLOT(sChangeEncoding(int)));
    ...
    connect(clientcombo, SIGNAL(activated(int)), this, SLOT(sChangeClient(int)));
...
bool KSpellConfig::writeGlobalSettings()
{
    ...
    kc->writeEntry("KSpell_Encoding", (int)encoding(), true, true);
    kc->writeEntry("KSpell_Client", client(), true, true);
    kc->sync();
...
bool KSpellConfig::interpret(QString &fname, QString &lname, QString &hname)

{
    ...
    if(dname.endsWith("sml") || dname.endsWith("med") || dname.endsWith("lrg") || dname.endsWith("xlg"))
    ...
    else if(dname == "english" || dname == "american" || dname == "british" || dname == "canadian")
    {
        lname = "en";
        hname = i18n("English");
    }
    ...
    {
        lname = "de";
        hname = i18n("German (new spelling)");
    }
    ...
    {
        lname = "de";
        hname = i18n("Swiss German");
    }
    else if(dname == "ukrainian")
    {
        lname = "uk";
        hname = i18n("Ukrainian");
    }
    else if(dname == "lietuviu" || dname == "lithuanian")
    {
        lname = "lt";
        hname = i18n("Lithuanian");
    }
    ...
    else if(dname == "belarusian")
    { // waiting for post 2.2 to not dissapoint translators
        lname = "be";
        hname = i18n("Belarusian");
    }
    else if(dname == "magyar")
    {
        lname = "hu";
        hname = i18n("Hungarian");
        sChangeEncoding(KS_E_LATIN2);
    }
    else
    {
        lname = "";
        hname = i18n("Unknown ispell dictionary", "Unknown");
    }
    if(!extension.isEmpty())
    {
    ...
    if((KGlobal::locale()->language() == QString::fromLatin1("C") && lname == QString::fromLatin1("en")) || KGlobal::locale()->language() == lname)
        return true;
...
void KSpellConfig::getAvailDictsIspell()
{
    ...
    */
    ...
    kdDebug(750) << "KSpellConfig::getAvailDictsIspell " << dir.filePath() << " " << dir.dirPath() << endl;

    const QDir thedir(dir.filePath(), "*.hash");
    const QStringList entryList = thedir.entryList();

    kdDebug(750) << "KSpellConfig" << thedir.path() << "\n" << endl;
    kdDebug(750) << "entryList().count()=" << entryList.count() << endl;
    ...
    for(; entryListItr != entryListEnd; ++entryListItr)
    {
        ...
        { // This one is the KDE default language
            ...
            hname = i18n("default spelling dictionary", "Default - %1 [%2]").arg(hname).arg(fname);
...
void KSpellConfig::getAvailDictsAspell()
{
    ...
    kdDebug(750) << "KSpellConfig::getAvailDictsAspell " << dir.filePath() << " " << dir.dirPath() << endl;
    ...
    for(; entryListItr != entryListEnd; ++entryListItr)
    {
        ...
        if(fname[0] != '.')
        {
            ...
            // remove .multi
            if(fname.endsWith(".multi"))
                fname.remove(fname.length() - 6, 6);
            // remove .rws
            if(fname.endsWith(".rws"))
            ...
            { // This one is the KDE default language
                ...
                hname = i18n("default spelling dictionary", "Default - %1").arg(hname);
...
void KSpellConfig::fillDicts(QComboBox *box, QStringList *dictionaries)
{
    langfnames.clear();
    if(box)
    {
...
void KSpellConfig::setClient(int c)
{
    iclient = c;
...
// KDE 4: Make it const QStringList &
void KSpellConfig::setReplaceAllList(QStringList _replacelist)
{
