...
KBugReport::KBugReport(QWidget *parentw, bool modal, const KAboutData *aboutData)
    : KDialogBase(Plain, i18n("Submit Bug Report"), Ok | Cancel, Ok, parentw, "KBugReport",
                  modal, // modal
                  true   // separator
                  )
{
    ...
    m_aboutData = aboutData ? aboutData : (KGlobal::_activeInstance ? KGlobal::_activeInstance->aboutData() : KGlobal::instance()->aboutData());
    m_process = 0;
    ...
    if(!d->submitBugButton)
    {
        ...
        m_configureEmail = new QPushButton(i18n("Configure Email..."), parent);
        connect(m_configureEmail, SIGNAL(clicked()), this, SLOT(slotConfigureEmail()));
        ...
        setButtonOK(
            KGuiItem(i18n("&Send"), "mail_send", i18n("Send bug report."), i18n("Send this bug report to %1.").arg(m_aboutData->bugAddress())));
    }
    else
    {
        m_configureEmail = 0;
    ...
    d->appname = QString::fromLatin1(m_aboutData ? m_aboutData->productName() : qApp->name());
    ...
    else
    {
        ...
        connect(d->submitBugButton, SIGNAL(clicked()), this, SLOT(slotOk()));
    }
}
...
void KBugReport::slotOk(void)
{
    ...
    if(m_lineedit->text().isEmpty() || m_subject->text().isEmpty())
    {
        QString msg = i18n(
    ...
    {
        case 0: // critical
            if(KMessageBox::questionYesNo(this, i18n("<p>You chose the severity <b>Critical</b>. "
        ...
        case 1: // grave
            if(KMessageBox::questionYesNo(
                   this, i18n("<p>You chose the severity <b>Grave</b>. "
                              "Please note that this severity is intended only for bugs that</p>"
...
void KBugReport::slotCancel()
{
    ...
    {
        int rc = KMessageBox::warningYesNo(this, i18n("Close and discard\nedited message?"), i18n("Close Message"), KStdGuiItem::discard(),
                                           KStdGuiItem::cont());
...
QString KBugReport::text() const
{
    ...
    QString os = QString::fromLatin1("OS: %1 (%2)\n").arg(KDE_COMPILING_OS).arg(KDE_DISTRIBUTION_TEXT);
    QString bodyText;
...
{
    QString recipient(m_aboutData ? m_aboutData->bugAddress() : QString::fromLatin1("submit@bugs.kde.org"));
