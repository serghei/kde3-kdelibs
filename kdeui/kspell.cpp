...
KSpell::KSpell(QWidget *_parent, const QString &_caption, QObject *obj, const char *slot, KSpellConfig *_ksc, bool _progressbar, bool _modal)
{
    initialize(_parent, _caption, obj, slot, _ksc, _progressbar, _modal, Text);
}

KSpell::KSpell(QWidget *_parent, const QString &_caption, QObject *obj, const char *slot, KSpellConfig *_ksc, bool _progressbar, bool _modal,
               SpellerType type)
{
    initialize(_parent, _caption, obj, slot, _ksc, _progressbar, _modal, type);
}
...
// trystart = {0,1,2}
{
    ...
    {
        connect(proc, SIGNAL(receivedStderr(KProcess *, char *, int)), this, SLOT(ispellErrors(KProcess *, char *, int)));

        connect(proc, SIGNAL(processExited(KProcess *)), this, SLOT(ispellExit(KProcess *)));
...
void KSpell::ispellErrors(KProcess *, char *buffer, int buflen)
{
    buffer[buflen - 1] = '\0';
...
void KSpell::setUpDialog(bool reallyuseprogressbar)
{
    if(dialogsetup)
        return;

    // Set up the dialog box
    ksdlg = new KSpellDlg(parent, "dialog", progressbar && reallyuseprogressbar, modaldlg);
    ksdlg->setCaption(caption);

    connect(ksdlg, SIGNAL(command(int)), this, SLOT(slotStopCancel(int)));
    connect(this, SIGNAL(progress(unsigned int)), ksdlg, SLOT(slotProgress(unsigned int)));
...
bool KSpell::cleanFputsWord(const QString &s, bool appendCR)
{
    QString qs(s);
    bool empty = true;
...
bool KSpell::cleanFputs(const QString &s, bool appendCR)
{
    QString qs(s);
...
bool KSpell::checkWord(const QString &buffer, bool _usedialog)
{
    ...
    if(qs.find(' ') != -1 || qs.isEmpty())
    { // make sure it's a _word_
    ...
    QString blank_line;
    while(proc->readln(blank_line, true) != -1)
        ; // eat spurious blanks
...
// invoked by KProcIO, when data from ispell are read
{
    ...
    do
    {
        ...
        {
            if((e = parseOneResponse(line, word, sugg)) == MISTAKE || e == REPLACE)
            {
                dlgresult = -1;
...
// invoked by KProcIO when read from ispell
{
        ...
        if(tempe > 0)
        {
            if((e = parseOneResponse(line, word, sugg)) == MISTAKE || e == REPLACE)
            {
                ...
                if((ksconfig->encoding() == KS_E_UTF8) && !d->aspellV6)
                {
                    ...
                    posinline = (QString::fromUtf8(origbuffer.mid(lastlastline, lastline - lastlastline).utf8(), posinline)).length();
    ...
    if(tempe == -1)
    { // we were called, but no data seems to be ready...
...
void KSpell::slotStopCancel(int result)
{
...
int KSpell::modalCheck(QString &text, KSpellConfig *_kcs)
{
    modalreturn = 0;
    modaltext = text;

    KSpell *spell = new KSpell(0L, i18n("Spell Checker"), 0, 0, _kcs, true, true);
...
void KSpell::slotModalReady()
{
    ...
    QObject::connect(this, SIGNAL(corrected(const QString &, const QString &, unsigned int)), this,
                     SLOT(slotSpellCheckerCorrected(const QString &, const QString &, unsigned int)));
    QObject::connect(this, SIGNAL(death()), this, SLOT(slotModalSpellCheckerFinished()));
    check(modaltext);
}
...
void KSpell::initialize(QWidget *_parent, const QString &_caption, QObject *obj, const char *slot, KSpellConfig *_ksc, bool _progressbar, bool _modal,
                        SpellerType type)
{
    d = new KSpellPrivate;
    ...
    connect(d->checkNextTimer, SIGNAL(timeout()), this, SLOT(checkNext()));
    autoDelete = false;
    modaldlg = _modal;
