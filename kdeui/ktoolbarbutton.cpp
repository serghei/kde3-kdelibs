...
KToolBarButton::KToolBarButton(QWidget *_parent, const char *_name) : QToolButton(_parent, _name)
{
...
KToolBarButton::KToolBarButton(const QString &_icon, int _id, QWidget *_parent, const char *_name, const QString &_txt, KInstance *_instance)
    : QToolButton(_parent, _name), d(0)
{
    ...
    if(d->m_parent)
    {
        connect(d->m_parent, SIGNAL(modechange()), this, SLOT(modeChange()));
    }
...
KToolBarButton::KToolBarButton(const QPixmap &pixmap, int _id, QWidget *_parent, const char *name, const QString &txt)
...
void KToolBarButton::modeChange()
{
    ...
    mysize = style().sizeFromContents(QStyle::CT_ToolButton, this, mysize).expandedTo(QApplication::globalStrut());
...
void KToolBarButton::setIcon(const QString &icon)
{
    ...
    if(d->m_parent && !strcmp(d->m_parent->name(), "mainToolBar"))
        QToolButton::setIconSet(d->m_instance->iconLoader()->loadIconSet(d->m_iconName, KIcon::MainToolbar, d->m_iconSize));
    else
        QToolButton::setIconSet(d->m_instance->iconLoader()->loadIconSet(d->m_iconName, KIcon::Toolbar, d->m_iconSize));
}
...
void KToolBarButton::setDefaultIcon(const QString &icon)
{
    ...
    QPixmap pm;
    if(d->m_parent && !strcmp(d->m_parent->name(), "mainToolBar"))
        pm = d->m_instance->iconLoader()->loadIcon(icon, KIcon::MainToolbar, d->m_iconSize);
    else
        pm = d->m_instance->iconLoader()->loadIcon(icon, KIcon::Toolbar, d->m_iconSize);
...
bool KToolBarButton::eventFilter(QObject *o, QEvent *ev)
{
    if((KToolBarButton *)o == this)
    {
        ...
        if(QToolButton::popup())
        {
            ...
            {
                QMouseEvent *mev = static_cast< QMouseEvent * >(ev);
                if((mev->pos() - d->m_mousePressPos).manhattanLength() > KGlobalSettings::dndEventDelay())
                {
                    openPopup();
                    return true;
                }
        ...
        if(d->m_isRadio
           && (ev->type() == QEvent::MouseButtonPress || ev->type() == QEvent::MouseButtonRelease || ev->type() == QEvent::MouseButtonDblClick)
           && isOn())
            return true;
...
void KToolBarButton::drawButton(QPainter *_painter)
{
    ...
    style().drawComplexControl(QStyle::CC_ToolButton, _painter, this, rect(), colorGroup(), flags, QStyle::SC_ToolButton, active, QStyleOption());
    ...
    {
        QPixmap pixmap =
            iconSet().pixmap(QIconSet::Automatic, isEnabled() ? (d->m_isActive ? QIconSet::Active : QIconSet::Normal) : QIconSet::Disabled,
    ...
    if(QToolButton::popup())
    {
        ...
        style().drawPrimitive(QStyle::PE_ArrowDown, _painter, QRect(width() - 7, height() - 7, 7, 7), colorGroup(), arrowFlags, QStyleOption());
    }
}
