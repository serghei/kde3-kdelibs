...
public:
    ...
    class CustomEditor {
    public:
        KDEUI_EXPORT CustomEditor() : m_representationWidget(0L), m_lineEdit(0L)
        {
        }
        KDEUI_EXPORT CustomEditor(QWidget *repWidget, KLineEdit *edit) : m_representationWidget(repWidget), m_lineEdit(edit)
        {
        }
...
public:
    ...
    }; // separated so that it doesn't appear in Qt designer
     ...
     * @since 3.1
     */
    KEditListBox(const QString &title, const CustomEditor &customEditor, QWidget *parent = 0, const char *name = 0, bool checkAtEntering = false,
                 int buttons = All);
    ...
    {
        m_listBox->insertItem(text, index);
    }
    /**
