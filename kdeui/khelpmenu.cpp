...
KHelpMenu::KHelpMenu(QWidget *parent, const QString &aboutAppText, bool showWhatsThis)
    : QObject(parent), mMenu(0), mAboutApp(0), mAboutKDE(0), mBugReport(0), d(new KHelpMenuPrivate)
{
    mParent = parent;
...
KHelpMenu::KHelpMenu(QWidget *parent, const KAboutData *aboutData, bool showWhatsThis, KActionCollection *actions)
    : QObject(parent), mMenu(0), mAboutApp(0), mAboutKDE(0), mBugReport(0), d(new KHelpMenuPrivate)
{
    mParent = parent;
    mShowWhatsThis = showWhatsThis;
...
KPopupMenu *KHelpMenu::menu()
{
    if(!mMenu)
    {
        ...
        {
            mMenu->insertItem(BarIcon("contents", KIcon::SizeSmall), i18n("%1 &Handbook").arg(appName), menuHelpContents);
        ...
        {
            mMenu->insertItem(kapp->miniIcon(), i18n("&About %1").arg(appName), menuAboutApp);
    ...
    else
    {
        if(!mAboutApp)
        {
            mAboutApp = new KDialogBase(QString::null, // Caption is defined below
                                        KDialogBase::Yes, KDialogBase::Yes, KDialogBase::Yes, mParent, "about", false, true, KStdGuiItem::ok());
            connect(mAboutApp, SIGNAL(finished()), this, SLOT(dialogFinished()));
