...
KDateValidator::KDateValidator(QWidget *parent, const char *name) : QValidator(parent, name)
{
}
...
KDateTable::KDateTable(QWidget *parent, QDate date_, const char *name, WFlags f) : QGridView(parent, name, (f | WNoAutoErase))
{
...
KDateTable::KDateTable(QWidget *parent, const char *name, WFlags f) : QGridView(parent, name, (f | WNoAutoErase))
{
    d = new KDateTablePrivate;
    setFontSize(10);
    setFocusPolicy(QWidget::StrongFocus);
...
void KDateTable::paintEmptyArea(QPainter *paint, int, int, int, int)
{
...
void KDateTable::paintCell(QPainter *painter, int row, int col)
{
    ...
    { // we are drawing the headline
        ...
        bool normalday = true;
        int firstWeekDay = KGlobal::locale()->weekStartDay();
        int daynum = (col + firstWeekDay < 8) ? col + firstWeekDay : col + firstWeekDay - 7;
        if(daynum == calendar->weekDayOfPray() || (daynum == 6 && calendar->calendarName() == "gregorian"))
            normalday = false;
        ...
        painter->drawText(0, 0, w, h - 1, AlignCenter, calendar->weekDayName(daynum, true), -1, &rect);
...
void KDateTable::viewportResizeEvent(QResizeEvent *e)
{
...
void KDateTable::contentsMousePressEvent(QMouseEvent *e)
{
...
KDateInternalWeekSelector::KDateInternalWeekSelector(QWidget *parent, const char *name)
    : QLineEdit(parent, name), val(new QIntValidator(this)), result(0)
{
    QFont font;
    // -----
...
void KDateInternalWeekSelector::weekEnteredSlot()
{
    bool ok;
    int week;
...
int KDateInternalWeekSelector::getWeek()
{
    return result;
}

void KDateInternalWeekSelector::setWeek(int week)
{
    QString temp;
    // -----
    temp.setNum(week);
...
void KDateInternalWeekSelector::setMaxWeek(int max)
{
    val->setRange(1, max);
}
...
public:
    KDateInternalMonthPrivate(int y, int m, int d) : year(y), month(m), day(d)
    {
    }
    int year;
    int month;
...
KDateInternalMonthPicker::KDateInternalMonthPicker(const QDate &date, QWidget *parent, const char *name)
    : QGridView(parent, name), result(0) // invalid
{
    QRect rect;
    QFont font;
    // -----
    ...
    for(int i = 1;; ++i)
    {
        QString str = KGlobal::locale()->calendar()->monthName(i, KGlobal::locale()->calendar()->year(date), false);
        if(str.isNull())
            break;
...
void KDateInternalMonthPicker::setupPainter(QPainter *p)
{
    p->setPen(KGlobalSettings::textColor());
}

void KDateInternalMonthPicker::viewportResizeEvent(QResizeEvent *)
{
    setCellWidth(width() / numCols());
    setCellHeight(height() / numRows());
}

void KDateInternalMonthPicker::paintCell(QPainter *painter, int row, int col)
{
    int index;
    QString text;
    // ----- find the number of the cell:
    index = 3 * row + col + 1;
    text = KGlobal::locale()->calendar()->monthName(index, KGlobal::locale()->calendar()->year(QDate(d->year, d->month, d->day)), false);
    painter->drawText(0, 0, cellWidth(), cellHeight(), AlignCenter, text);
    if(activeCol == col && activeRow == row)
        painter->drawRect(0, 0, cellWidth(), cellHeight());
}

void KDateInternalMonthPicker::contentsMousePressEvent(QMouseEvent *e)
{
...
void KDateInternalMonthPicker::contentsMouseMoveEvent(QMouseEvent *e)
{
...
void KDateInternalMonthPicker::contentsMouseReleaseEvent(QMouseEvent *e)
{
    if(!isEnabled())
    {
        return;
...
KDateInternalYearSelector::KDateInternalYearSelector(QWidget *parent, const char *name)
    : QLineEdit(parent, name), val(new QIntValidator(this)), result(0)
{
    QFont font;
    // -----
...
int KPopupFrame::exec(QPoint pos)
{
    popup(pos);
    repaint();
    d->exec = true;
