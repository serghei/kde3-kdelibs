...
KShortcutDialog::KShortcutDialog(const KShortcut &shortcut, bool bQtShortcut, QWidget *parent, const char *name)
    : KDialogBase(parent, name, true, i18n("Configure Shortcut"), KDialogBase::Details | KDialogBase::Ok | KDialogBase::Cancel, KDialogBase::Cancel,
                  true)
{
    ...
    connect(m_simple->m_btnClearShortcut, SIGNAL(clicked()), this, SLOT(slotClearShortcut()));
    connect(m_adv->m_btnClearPrimary, SIGNAL(clicked()), this, SLOT(slotClearPrimary()));
    connect(m_adv->m_btnClearAlternate, SIGNAL(clicked()), this, SLOT(slotClearAlternate()));

    connect(m_adv->m_txtPrimary, SIGNAL(clicked()), m_adv->m_btnPrimary, SLOT(animateClick()));
    connect(m_adv->m_txtAlternate, SIGNAL(clicked()), m_adv->m_btnAlternate, SLOT(animateClick()));
    connect(m_adv->m_btnPrimary, SIGNAL(clicked()), this, SLOT(slotSelectPrimary()));
    connect(m_adv->m_btnAlternate, SIGNAL(clicked()), this, SLOT(slotSelectAlternate()));
    ...
    if(m_bRecording)
    {
        ...
        if(m_iKey == 0)
        {
            if(m_mod)
            {
                QString keyModStr;
                if(m_mod & KKey::WIN)
                    keyModStr += KKey::modFlagLabel(KKey::WIN) + "+";
                if(m_mod & KKey::ALT)
                    keyModStr += KKey::modFlagLabel(KKey::ALT) + "+";
                if(m_mod & KKey::CTRL)
                    keyModStr += KKey::modFlagLabel(KKey::CTRL) + "+";
                if(m_mod & KKey::SHIFT)
                    keyModStr += KKey::modFlagLabel(KKey::SHIFT) + "+";
                s[m_iSeq] = keyModStr;
            }
        }
...
{
    uint mod = 0;
    if(keyModX & KKeyNative::modX(KKey::SHIFT))
        mod += KKey::SHIFT;
    if(keyModX & KKeyNative::modX(KKey::CTRL))
        mod += KKey::CTRL;
    if(keyModX & KKeyNative::modX(KKey::ALT))
        mod += KKey::ALT;
    if(keyModX & KKeyNative::modX(KKey::WIN))
        mod += KKey::WIN;
    return mod;
}
...
void KShortcutDialog::keyPressEvent(QKeyEvent *e)
{
    ...
    int keyQt = QChar(e->key() & 0xff).isLetter() ? (QChar(e->key() & 0xff).lower().latin1() | (e->key() & 0xffff00)) : e->key();
