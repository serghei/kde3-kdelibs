...
KIntValidator::KIntValidator(QWidget *parent, int base, const char *name) : QValidator(parent, name)
{
    _base = base;
    if(_base < 2)
        _base = 2;
...
KIntValidator::KIntValidator(int bottom, int top, QWidget *parent, int base, const char *name) : QValidator(parent, name)
{
    _base = base;
    if(_base > 36)
        _base = 36;
...
KFloatValidator::KFloatValidator(QWidget *parent, const char *name) : QValidator(parent, name)
{
...
KFloatValidator::KFloatValidator(double bottom, double top, QWidget *parent, const char *name) : QValidator(parent, name)
{
    d = new KFloatValidatorPrivate;
    d->acceptLocalizedNumbers = false;
    _min = bottom;
...
KFloatValidator::KFloatValidator(double bottom, double top, bool localeAware, QWidget *parent, const char *name) : QValidator(parent, name)
{
    d = new KFloatValidatorPrivate;
...
KDoubleValidator::KDoubleValidator(QObject *parent, const char *name) : QDoubleValidator(parent, name), d(0)
{
    d = new Private();
}

KDoubleValidator::KDoubleValidator(double bottom, double top, int decimals, QObject *parent, const char *name)
...
QValidator::State KDoubleValidator::validate(QString &input, int &p) const
{
    ...
    if(acceptLocalizedNumbers())
    {
        ...
        QString d = l->decimalSymbol(), n = l->negativeSign(), p = l->positiveSign(), t = l->thousandsSeparator();
        ...
        if((!n.isEmpty() && n.find('.') != -1) || (!d.isEmpty() && d.find('-') != -1))
        {
