...
KPixmapRegionSelectorWidget::KPixmapRegionSelectorWidget(QWidget *parent, const char *name) : QWidget(parent, name)
{
...
void KPixmapRegionSelectorWidget::updatePixmap()
{
    ...
    if(m_linedPixmap.isNull())
    {
        ...
        painter.fillRect(0, 0, m_linedPixmap.width(), m_linedPixmap.height(), QBrush(QColor(255, 255, 255), Qt::BDiagPattern));
        painter.end();
...
KPopupMenu *KPixmapRegionSelectorWidget::createPopupMenu()
{
    ...
    KAction *action = new KAction(i18n("&Rotate Clockwise"), "rotate_cw", 0, this, SLOT(rotateClockwise()), popup, "rotateclockwise");
    action->plug(popup);

    action = new KAction(i18n("Rotate &Counterclockwise"), "rotate_ccw", 0, this, SLOT(rotateCounterclockwise()), popup, "rotatecounterclockwise");
    action->plug(popup);
...
bool KPixmapRegionSelectorWidget::eventFilter(QObject *obj, QEvent *ev)
{
    if(ev->type() == QEvent::MouseButtonPress)
    {
        ...
        if(m_selectedRegion.contains(mev->pos()) && m_selectedRegion != m_originalPixmap.rect())
        {
            m_state = Moving;
    ...
    if(ev->type() == QEvent::MouseMove)
    {
        ...
        if(m_state == Resizing)
        {
            setSelectedRegion(calcSelectionRectangle(m_tempFirstClick, mev->pos()));
        }
        else if(m_state == Moving)
        {
            ...
            m_selectedRegion.moveBy(mev->x() - m_tempFirstClick.x(), mev->y() - m_tempFirstClick.y());
...
{
    return QRect((int)(m_selectedRegion.x() / m_zoomFactor), (int)(m_selectedRegion.y() / m_zoomFactor),
                 (int)(m_selectedRegion.width() / m_zoomFactor), (int)(m_selectedRegion.height() / m_zoomFactor));
}
...
void KPixmapRegionSelectorWidget::setMaximumWidgetSize(int width, int height)
{
    ...
    if(m_selectedRegion == m_originalPixmap.rect())
    ...
    if(!m_originalPixmap.isNull() && (m_originalPixmap.width() > m_maxWidth || m_originalPixmap.height() > m_maxHeight))
    {
        ...
        {
            m_selectedRegion = QRect(
                (int)(m_selectedRegion.x() * m_zoomFactor / oldZoomFactor), (int)(m_selectedRegion.y() * m_zoomFactor / oldZoomFactor),
                (int)(m_selectedRegion.width() * m_zoomFactor / oldZoomFactor), (int)(m_selectedRegion.height() * m_zoomFactor / oldZoomFactor));
        }
    }
