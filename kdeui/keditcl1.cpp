...
KEdit::KEdit(QWidget *_parent, const char *name) : QMultiLineEdit(_parent, name)
{
    d = new KEditPrivate;
    ...
    connect(this, SIGNAL(cursorPositionChanged(int, int)), this, SLOT(slotCursorPositionChanged()));
}
...
void KEdit::saveText(QTextStream *stream, bool softWrap)
{
    ...
    else
    {
        for(int i = 0; i <= line_count; i++)
        {
            ...
            else
            {
                ...
                while(true)
                {
                    while(lineOfChar(i, pos) == current_line)
                        pos++;
...
void KEdit::doGotoLine()
{
    ...
    while(1 + line + lineOfChar(parag, col) < target_line)
        col++;
