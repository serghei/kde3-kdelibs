...
static const char *const dockback_xpm[] = {"6 6 2 1", "# c black", ". c None", "......", ".#....", "..#..#", "...#.#", "....##", "..####"};

static const char *const todesktop_xpm[] = {"5 5 2 1", "# c black", ". c None", "####.", "##...", "#.#..", "#..#.", "....#"};

static const char *const not_close_xpm[] = {"5 5 2 1", "# c black", ". c None", "#####", "#...#", "#...#", "#...#", "#####"};
 ...
 * @author Max Judin.
*/
KDockMainWindow::KDockMainWindow(QWidget *parent, const char *name, WFlags f) : KMainWindow(parent, name, f)
{
...
void KDockMainWindow::setView(QWidget *view)
{
    ...
    {
        if(view->parent() != this)
            ((KDockWidget *)view)->applyToWidget(this);
    }
...
void KDockMainWindow::slotDockWidgetUndocked()
{
    ...
    if(!pSender->inherits("KDockWidget"))
        return;
...
/*************************************************************************/
KDockWidgetAbstractHeader::KDockWidgetAbstractHeader(KDockWidget *parent, const char *name) : QFrame(parent, name)
{
}
/*************************************************************************/
KDockWidgetHeader::KDockWidgetHeader(KDockWidget *parent, const char *name) : KDockWidgetAbstractHeader(parent, name)
{
...
void KDockWidgetHeader::setTopLevel(bool isTopLevel)
{
    ...
    bool dontShowDummy = drag->isVisibleTo(this) || dockbackButton->isVisibleTo(this) || d->toDesktopButton->isVisibleTo(this)
                         || stayButton->isVisibleTo(this) || closeButton->isVisibleTo(this);
    ...
    if(!parent())
    {
        ...
        if(bright)
        {
            ...
            else
            {
                ...
                if(bbottom)
                    d->resizeMode = KDockWidgetPrivate::ResizeBottomRight;
                else
                    d->resizeMode = KDockWidgetPrivate::ResizeRight;
            }
        }
        else if(bleft)
        {
...
QWidget *KDockWidget::latestKDockContainer()
{
    if(!(d->container))
        return 0;
    if(dynamic_cast< KDockContainer * >(d->container.operator->()))
        return d->container;
    return 0;
}
...
#undef FocusIn
        ...
        case QEvent::ChildRemoved:
            if(widget == ((QChildEvent *)event)->child())
                widget = 0L;
            break;
        case QEvent::Show:
            if(widget)
...
KDockWidget *KDockWidget::findNearestDockWidget(DockPosition pos)
{
    ...
    {
        KDockWidget *neighbor = ((pos == DockLeft) || (pos == DockTop)) ? static_cast< KDockWidget * >(((KDockSplitter *)(parent()))->getFirst())
        ...
        else if(neighbor->getWidget() && (neighbor->getWidget()->qt_cast("KDockTabGroup")))
...
KDockWidget *KDockWidget::manualDock(KDockWidget *target, DockPosition dockPos, int spliPos, QPoint pos, bool check, int tabIndex)
{
    ...
    if(parent() && !parent()->inherits("KDockSplitter") && !parentDockTabGroup() && !(dynamic_cast< KDockContainer * >(parent()))
       && !parentDockContainer())
    {
        ...
        if(target && !check)
        {
            ...
            switch(dockPos)
            {
                case KDockWidget::DockLeft:
                    another__dockPos = KDockWidget::DockRight;
                    break;
                case KDockWidget::DockRight:
                    another__dockPos = KDockWidget::DockLeft;
                    break;
                case KDockWidget::DockTop:
                    another__dockPos = KDockWidget::DockBottom;
                    break;
                case KDockWidget::DockBottom:
                    another__dockPos = KDockWidget::DockTop;
                    break;
                default:
                    break;
            }
    ...
    if(parentTab)
    {
        ...
        parentTab->insertTab(this, icon() ? *icon() : QPixmap(), tabPageLabel(), tabIndex);
    ...
    else
    {
        ...
        if(contWid)
        {
            ...
            if(cont)
            {
                ...
                cont->insertWidget(this, icon() ? *icon() : QPixmap(), tabPageLabel(), tabIndex);
    ...
    if(dockPos == KDockWidget::DockCenter)
    {
        ...
        tab->insertTab(target, target->icon() ? *(target->icon()) : QPixmap(), target->tabPageLabel());


        if(!target->toolTipString().isEmpty())
            tab->setTabToolTip(target, target->toolTipString());

        tab->insertTab(this, icon() ? *icon() : QPixmap(), tabPageLabel(), tabIndex);
    ...
    else
    {
        ...
        if(dockPos == KDockWidget::DockTop || dockPos == KDockWidget::DockBottom)
            panner = new KDockSplitter(newDock, "_dock_split_", Horizontal, spliPos);
        if(dockPos == KDockWidget::DockLeft || dockPos == KDockWidget::DockRight)
            panner = new KDockSplitter(newDock, "_dock_split_", Vertical, spliPos);
...
KDockTabGroup *KDockWidget::parentDockTabGroup() const
{
    ...
    if(candidate && candidate->inherits("KDockTabGroup"))
        return (KDockTabGroup *)candidate;
    return 0L;
}
...
#ifdef BORDERLESS_WINDOWS
    if(!mw->ownCursor())
        mw->setCursor(QCursor(ArrowCursor));
#endif
    widget = mw;
    delete layout;
...
void KDockWidget::setFormerBrotherDockWidget(KDockWidget *dockWidget)
{
    ...
    if(formerBrotherDockWidget)
        QObject::connect(formerBrotherDockWidget, SIGNAL(iMBeingClosed()), this, SLOT(loseFormerBrotherDockWidget()));
}

void KDockWidget::loseFormerBrotherDockWidget()
{
    if(formerBrotherDockWidget)
        QObject::disconnect(formerBrotherDockWidget, SIGNAL(iMBeingClosed()), this, SLOT(loseFormerBrotherDockWidget()));
    formerBrotherDockWidget = 0L;
    repaint();
}
    ...
    if(obj->inherits("KDockWidgetAbstractHeaderDrag"))
    {
        ...
        switch(event->type())
        {
            case QEvent::MouseButtonDblClick:
                if(curdw->currentDockPos == KDockWidget::DockDesktop)
                    curdw->dockBack();
                else
                {
                    curdw->toDesktop();
...
// END TEST
                ...
                else
                {
                    ...
                    if((((QMouseEvent *)event)->state() == LeftButton) && (curdw->eDocking != (int)KDockWidget::DockNone))
                    {
                        startDrag(curdw);
                    }
                }
...
KDockWidget *KDockManager::findDockWidgetAt(const QPoint &pos)
{
    ...
    if(currentDragWidget->eDocking == (int)KDockWidget::DockNone)
        return 0L;
    ...
    if(qt_find_obj_child(w, "KDockSplitter", "_dock_split_"))
        return 0L;
    if(qt_find_obj_child(w, "KDockTabGroup", "_dock_tab"))
        return 0L;
    ...
    if(childDockWidgetList->find(w) != -1)
        return 0L;
    if(currentDragWidget->isGroup && ((KDockWidget *)w)->parentDockTabGroup())
        return 0L;

    KDockWidget *www = (KDockWidget *)w;
    if(www->sDocking == (int)KDockWidget::DockNone)
        return 0L;
    if(!www->widget)
        return 0L;
    ...
    if(!(www->sDocking & (int)curPos))
        return 0L;
    if(!(currentDragWidget->eDocking & (int)curPos))
        return 0L;
    if(www->manager != this)
        return 0L;
...
{
    if((w->currentDockPos == KDockWidget::DockLeft) || (w->currentDockPos == KDockWidget::DockRight) || (w->currentDockPos == KDockWidget::DockTop)
       || (w->currentDockPos == KDockWidget::DockBottom))
    {
...
void KDockManager::drop()
{
    ...
    else
    {
        ...
        if((curPos != previousPosition) && (curPos != KDockWidget::DockCenter) && (curPos != KDockWidget::DockDesktop))
        {
...
void KDockManager::writeConfig(QDomElement &base)
{
    ...
    while(nListIt != nList.end())
    {
        ...
        if((obj->isGroup && (!obj->d->isContainer)) && (nameList.find(obj->firstName.latin1()) == -1 || nameList.find(obj->lastName.latin1()) == -1))
        {
...
{
    if(base.namedItem("group").isNull() && base.namedItem("tabgroup").isNull() && base.namedItem("dock").isNull()
    ...
    for(QDomNode n = base.firstChild(); !n.isNull(); n = n.nextSibling())
    {
        ...
        if(childEl.tagName() == "dockContainer")
        {
            ...
            else
            {
                ...
                if(!dc)
                    kdDebug(282) << "Error while trying to handle dockcontainer configuration restoration" << endl;
                else
                {
                    dc->load(childEl);
        ...
        else if(childEl.tagName() == "splitGroup")
        {
            // Read a group
            ...
            if(first && second)
            {
                obj = first->manualDock(second, (orientation == (int)Vertical) ? KDockWidget::DockLeft : KDockWidget::DockTop, separatorPos);
                if(obj)
...
void KDockManager::drawDragRectangle()
{
    ...
    return
#endif
        if(d->oldDragRect == d->dragRect) return;
...
KDockArea::KDockArea(QWidget *parent, const char *name) : QWidget(parent, name)
{
    QString new_name = QString(name) + QString("_DockManager");
    dockManager = new KDockManager(this, new_name.latin1());
    mainDockWidget = 0L;
...
KDockWidget *KDockArea::createDockWidget(const QString &name, const QPixmap &pixmap, QWidget *parent, const QString &strCaption,
                                         const QString &strTabPageLabel)
{
    return new KDockWidget(dockManager, name.latin1(), pixmap, parent, strCaption, strTabPageLabel);
}
...
bool KDockContainer::dockDragEnter(KDockWidget *, QMouseEvent *)
{
    return false;
}
bool KDockContainer::dockDragMove(KDockWidget *, QMouseEvent *)
{
    return false;
}
bool KDockContainer::dockDragLeave(KDockWidget *, QMouseEvent *)
{
    return false;
}
...
void KDockContainer::removeWidget(KDockWidget *dw)
{
    for(struct ListItem *tmp = m_childrenListBegin; tmp; tmp = tmp->next)
    {
        if(!strcmp(tmp->data, dw->name()))
        {
            ...
            if(tmp == m_childrenListBegin)
                m_childrenListBegin = tmp->next;
            if(tmp == m_childrenListEnd)
                m_childrenListEnd = tmp->prev;
            delete tmp;
            break;
        }
    }
