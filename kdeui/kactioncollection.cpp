...
KActionCollection::KActionCollection(QWidget *parent, const char *name, KInstance *instance) : QObject(parent, name)
{
...
KActionCollection::KActionCollection(QWidget *watch, QObject *parent, const char *name, KInstance *instance) : QObject(parent, name)
{
...
// KDE 4: remove
KActionCollection::KActionCollection(QObject *parent, const char *name, KInstance *instance) : QObject(parent, name)
{
...
KActionCollection::KActionCollection(const KActionCollection &copy) : QObject()
{
...
KActionCollection::KActionCollection(const char *name, const KXMLGUIClient *parent) : QObject(0L, name)
{
    d = new KActionCollectionPrivate;
...
{
    kdDebug(129) << "KActionCollection::beginXMLPlug( buildWidget = " << widget << " ): this = " << this
                 << " d->m_builderKAccel = " << d->m_builderKAccel << endl;
...
KAction *KActionCollection::action(const char *name, const char *classname) const
{
    ...
    else
    {
        ...
        for(; it.current(); ++it)
        {
            if((!name || !strcmp(it.current()->name(), name)) && (!classname || !strcmp(it.current()->className(), classname)))
            {
                pAction = it.current();
                break;
            }
...
void KActionCollection::connectHighlight(QWidget *container, KAction *action)
{
    ...
    if(!actionList)
    {
        ...
        {
            connect(container, SIGNAL(highlighted(int)), this, SLOT(slotMenuItemHighlighted(int)));
            connect(container, SIGNAL(aboutToHide()), this, SLOT(slotMenuAboutToHide()));
        }
        else if(::qt_cast< KToolBar * >(container))
        {
            connect(container, SIGNAL(highlighted(int, bool)), this, SLOT(slotToolBarButtonHighlighted(int, bool)));
        }

        connect(container, SIGNAL(destroyed()), this, SLOT(slotDestroyed()));
...
KActionShortcutList::KActionShortcutList(KActionCollection *pColl) : m_actions(*pColl)
{
}
...
bool KActionShortcutList::save() const
{
    ...
    for(uint i = 0; i < nSize; i++)
    {
        ...
        // kdDebug(129) << "name = " << sName << " shortcut = " << shortcut(i).toStringInternal() << " def = " <<
        // shortcutDefault(i).toStringInternal() << endl;
...
KActionPtrShortcutList::KActionPtrShortcutList(KActionPtrList &list) : m_actions(list)
{
}
