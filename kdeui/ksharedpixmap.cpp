...
KSharedPixmap::KSharedPixmap() : QWidget(0L, "shpixmap comm window")
{
...
bool KSharedPixmap::loadFromShared(const QString &name, const QRect &rect)
{
    ...
    XConvertSelection(qt_xdisplay(), d->selection, d->pixmap, d->target, winId(), CurrentTime);
    return true;
}
...
bool KSharedPixmap::x11Event(XEvent *event)
{
    ...
    XGetWindowProperty(qt_xdisplay(), winId(), ev->property, 0, 1, false, d->pixmap, &type, &format, &nitems, &ldummy, &pixmap_id);
    ...
    if(d->rect.isEmpty())
    {
        ...
        XCopyArea(qt_xdisplay(), pixmap, ((KPixmap *)this)->handle(), qt_xget_temp_gc(qt_xscreen(), false), 0, 0, width, height, 0, 0);
    ...
    XCopyArea(qt_xdisplay(), pixmap, ((KPixmap *)this)->handle(), qt_xget_temp_gc(qt_xscreen(), false), xa, ya, t1w + origin.x(), t1h + origin.y(),
              origin.x(), origin.y());
    XCopyArea(qt_xdisplay(), pixmap, ((KPixmap *)this)->handle(), qt_xget_temp_gc(qt_xscreen(), false), 0, ya, tw - t1w, t1h, t1w, 0);
    XCopyArea(qt_xdisplay(), pixmap, ((KPixmap *)this)->handle(), qt_xget_temp_gc(qt_xscreen(), false), xa, 0, t1w, th - t1h, 0, t1h);
    XCopyArea(qt_xdisplay(), pixmap, ((KPixmap *)this)->handle(), qt_xget_temp_gc(qt_xscreen(), false), 0, 0, tw - t1w, th - t1h, t1w, t1h);
