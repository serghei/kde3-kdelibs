...
KActionSelector::KActionSelector(QWidget *parent, const char *name) : QWidget(parent, name)
{
    ...
    connect(d->availableListBox, SIGNAL(doubleClicked(QListBoxItem *)), this, SLOT(itemDoubleClicked(QListBoxItem *)));
    connect(d->selectedListBox, SIGNAL(doubleClicked(QListBoxItem *)), this, SLOT(itemDoubleClicked(QListBoxItem *)));
    connect(d->availableListBox, SIGNAL(currentChanged(QListBoxItem *)), this, SLOT(slotCurrentChanged(QListBoxItem *)));
    connect(d->selectedListBox, SIGNAL(currentChanged(QListBoxItem *)), this, SLOT(slotCurrentChanged(QListBoxItem *)));
...
void KActionSelector::setButtonsEnabled()
{
    ...
    d->btnDown->setEnabled(d->selectedListBox->currentItem() > -1 && d->selectedListBox->currentItem() < (int)d->selectedListBox->count() - 1);
}
...
void KActionSelector::moveItem(QListBoxItem *item)
{
    ...
    InsertionPolicy p = (lbTo == d->availableListBox) ? d->availableInsertionPolicy : d->selectedInsertionPolicy;
