...
public:
    KSpellingHighlighterPrivate() : alwaysEndsWithSpace(true), intraWordEditing(false)
    {
    }
...
KSyntaxHighlighter::KSyntaxHighlighter(QTextEdit *textEdit, bool colorQuoting, const QColor &depth0, const QColor &depth1, const QColor &depth2,
                                       const QColor &depth3, SyntaxMode mode)
...
KSpellingHighlighter::KSpellingHighlighter(QTextEdit *textEdit, const QColor &spellColor, bool colorQuoting, const QColor &depth0,
                                           const QColor &depth1, const QColor &depth2, const QColor &depth3)
...
int KSpellingHighlighter::highlightParagraph(const QString &text, int paraNo)
{
    if(paraNo == -2)
        paraNo = 0;
    ...
    if(!isCode)
    {
        ...
        for(int i = 0; i < len; i++)
        {
            ...
            {
                if((para != paraNo) || !intraWordEditing() || (i - d->currentWord.length() > (uint)index) || (i < index))
                {
                    flushCurrentWord();
                }
                else
        ...
        if(!text[len - 1].isLetter() || (uint)(index + 1) != text.length() || para != paraNo)
            flushCurrentWord();
    }
    return ++paraNo;
}
...
void KSpellingHighlighter::flushCurrentWord()
{
    ...
    QChar ch;
    while((ch = d->currentWord[(int)d->currentWord.length() - 1]).isPunct() && ch != '(' && ch != '@')
...
KDictSpellingHighlighter::KDictSpellingHighlighter(QTextEdit *textEdit, bool spellCheckingActive, bool autoEnable, const QColor &spellColor,
                                                   bool colorQuoting, const QColor &depth0, const QColor &depth1, const QColor &depth2,
                                                   const QColor &depth3, KSpellConfig *spellConfig)
    : KSpellingHighlighter(textEdit, spellColor, colorQuoting, depth0, depth1, depth2, depth3)
{
    ...
    connect(d->rehighlightRequest, SIGNAL(timeout()), this, SLOT(slotRehighlight()));
    d->spellTimeout = new QTimer(this);
    connect(d->spellTimeout, SIGNAL(timeout()), this, SLOT(slotKSpellNotResponding()));
    ...
    else
    {
        ...
        connect(d->mSpellConfig, SIGNAL(configChanged()), this, SLOT(slotLocalSpellConfigChanged()));
    }
...
void KDictSpellingHighlighter::slotSpellReady(KSpell *spell)
{
    ...
    if(cg.readEntry("KSpell_DoSpellChecking") != "0")
    {
        if(d->globalConfig)
        {
            connect(d->sDictionaryMonitor, SIGNAL(destroyed()), this, SLOT(slotDictionaryChanged()));
        }
        if(spell != d->spell)
        {
            delete d->spell;
        ...
        connect(spell, SIGNAL(misspelling(const QString &, const QStringList &, unsigned int)), this,
                SLOT(slotMisspelling(const QString &, const QStringList &, unsigned int)));
        connect(spell, SIGNAL(corrected(const QString &, const QString &, unsigned int)), this,
                SLOT(slotCorrected(const QString &, const QString &, unsigned int)));
        d->checksRequested = 0;
        d->checksDone = 0;
...
void KDictSpellingHighlighter::slotMisspelling(const QString &originalWord, const QStringList &suggestions, unsigned int pos)
{
...
void KDictSpellingHighlighter::slotDictionaryChanged()
{
    ...
    d->spell = new KSpell(0, i18n("Incremental Spellcheck"), this, SLOT(slotSpellReady(KSpell *)), d->mSpellConfig);
}
    ...
    if(d->active != savedActive)
    {
        if(d->wordCount > 1)
            if(d->active)
                ...
                emit activeChanged(
                    i18n("Too many misspelled words. "
...
bool KDictSpellingHighlighter::eventFilter(QObject *o, QEvent *e)
{
    ...
    if(o == textEdit() && (e->type() == QEvent::KeyPress))
    {
        ...
        if(k->key() == Key_Enter || k->key() == Key_Return || k->key() == Key_Up || k->key() == Key_Down || k->key() == Key_Left
           || k->key() == Key_Right || k->key() == Key_PageUp || k->key() == Key_PageDown || k->key() == Key_Home || k->key() == Key_End
           || ((k->state() & ControlButton)
               && ((k->key() == Key_A) || (k->key() == Key_B) || (k->key() == Key_E) || (k->key() == Key_N) || (k->key() == Key_P))))
        {
            if(intraWordEditing())
            {
        ...
        if(k->key() == Key_Space || k->key() == Key_Enter || k->key() == Key_Return)
        {
            QTimer::singleShot(0, this, SLOT(slotAutoDetection()));
        }
    }

    else if(o == textEdit()->viewport() && (e->type() == QEvent::MouseButtonPress))
    {
        d->autoReady = true;
        if(intraWordEditing())
        {
