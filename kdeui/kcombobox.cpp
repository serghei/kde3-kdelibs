...
KComboBox::KComboBox(QWidget *parent, const char *name) : QComboBox(parent, name), d(new KComboBoxPrivate)
{
    init();
}

KComboBox::KComboBox(bool rw, QWidget *parent, const char *name) : QComboBox(rw, parent, name), d(new KComboBoxPrivate)
{
    init();
...
{
    if(!editable() && edit && !qstrcmp(edit->className(), "QLineEdit"))
    {
    ...
    if(d->klineEdit)
    {
        ...
        connect(d->klineEdit, SIGNAL(returnPressed(const QString &)), SIGNAL(returnPressed(const QString &)));

        connect(d->klineEdit, SIGNAL(completion(const QString &)), SIGNAL(completion(const QString &)));

        connect(d->klineEdit, SIGNAL(substringCompletion(const QString &)), SIGNAL(substringCompletion(const QString &)));

        connect(d->klineEdit, SIGNAL(textRotation(KCompletionBase::KeyBindingType)), SIGNAL(textRotation(KCompletionBase::KeyBindingType)));

        connect(d->klineEdit, SIGNAL(completionModeChanged(KGlobalSettings::Completion)), SIGNAL(completionModeChanged(KGlobalSettings::Completion)));

        connect(d->klineEdit, SIGNAL(aboutToShowContextMenu(QPopupMenu *)), SIGNAL(aboutToShowContextMenu(QPopupMenu *)));

        connect(d->klineEdit, SIGNAL(completionBoxActivated(const QString &)), SIGNAL(activated(const QString &)));
    }
}
...
KHistoryCombo::KHistoryCombo(QWidget *parent, const char *name) : KComboBox(true, parent, name), d(0)
{
    init(true); // using completion
}

// we are always read-write
KHistoryCombo::KHistoryCombo(bool useCompletion, QWidget *parent, const char *name) : KComboBox(true, parent, name), d(0)
{
    init(useCompletion);
}
...
void KHistoryCombo::init(bool useCompletion)
{
    ...
    connect(this, SIGNAL(aboutToShowContextMenu(QPopupMenu *)), SLOT(addContextMenuItems(QPopupMenu *)));
...
void KHistoryCombo::setHistoryItems(QStringList items, bool setCompletionList)
{
    KComboBox::clear();
...
void KHistoryCombo::rotateUp()
{
    ...
    while(myIterateIndex < last && (currText == text(myIterateIndex) || text(myIterateIndex).isEmpty()))
        ++myIterateIndex;
...
void KHistoryCombo::rotateDown()
{
    ...
    const QString currText = currentText();
    // skip duplicates/empty items
    while(myIterateIndex >= 0 && (currText == text(myIterateIndex) || text(myIterateIndex).isEmpty()))
        --myIterateIndex;
    ...
    if(myIterateIndex < 0)
    {
        ...
        { // bottom of history
            if(myIterateIndex == -2)
            {
                KNotifyClient::event((int)winId(), KNotifyClient::notification, i18n("No further item in the history."));
            }
