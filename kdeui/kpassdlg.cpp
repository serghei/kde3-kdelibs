...
public:
    KPasswordDialogPrivate()
        ...
        , maximumPasswordLength(KPasswordEdit::PassLen - 1)
...
KPasswordEdit::KPasswordEdit(QWidget *parent, const char *name) : QLineEdit(parent, name)
{
    init();
...
KPasswordEdit::KPasswordEdit(QWidget *parent, const char *name, int echoMode) : QLineEdit(parent, name), m_EchoMode(echoMode)
{
    init();
}

KPasswordEdit::KPasswordEdit(EchoModes echoMode, QWidget *parent, const char *name) : QLineEdit(parent, name), m_EchoMode(echoMode)
{
    init();
}

KPasswordEdit::KPasswordEdit(EchoMode echoMode, QWidget *parent, const char *name)
    : QLineEdit(parent, name), m_EchoMode(echoMode == QLineEdit::NoEcho ? NoEcho : OneStar)
{
    init();
}
...
{
    if(newLength >= PassLen)
        newLength = PassLen - 1; // belt and braces
    if(newLength < 0)
        newLength = 0;
    int *t = ourMaxLength(this);
    *t = newLength;
...
KPasswordDialog::KPasswordDialog(Types type, bool enableKeep, int extraBttn, QWidget *parent, const char *name)
    : KDialogBase(parent, name, true, "", Ok | Cancel | extraBttn, Ok, true), m_Keep(enableKeep ? 1 : 0), m_Type(type), d(new KPasswordDialogPrivate)
{
    d->iconName = "password";
    init();
}

KPasswordDialog::KPasswordDialog(Types type, bool enableKeep, int extraBttn, const QString &icon, QWidget *parent, const char *name)
    : KDialogBase(parent, name, true, "", Ok | Cancel | extraBttn, Ok, true), m_Keep(enableKeep ? 1 : 0), m_Type(type), d(new KPasswordDialogPrivate)
{
...
KPasswordDialog::KPasswordDialog(int type, QString prompt, bool enableKeep, int extraBttn)
    : KDialogBase(0L, "Password Dialog", true, "", Ok | Cancel | extraBttn, Ok, true)
...
void KPasswordDialog::init()
{
    ...
    else if(m_Type == NewPassword)
    {
        ...
        const QString strengthBarWhatsThis(
            i18n("The password strength meter gives an indication of the security "
...
void KPasswordDialog::slotOk()
{
    if(m_Type == NewPassword)
    {
        ...
        {
            int retVal = KMessageBox::warningContinueCancel(this, i18n("The password you have entered has a low strength. "
...
int KPasswordDialog::getPassword(QCString &password, QString prompt, int *keep)
{
...
void KPasswordDialog::enableOkBtn()
{
    if(m_Type == NewPassword)
    {
        const bool match = strcmp(m_pEdit->password(), m_pEdit2->password()) == 0 && (d->allowEmptyPasswords || m_pEdit->password()[0]);
        ...
        else
        {
            if((int)pass.length() < minPasswordLength)
            {
                d->m_MatchLabel->setText(
                    i18n("Password must be at least 1 character long", "Password must be at least %n characters long", minPasswordLength));
            }
            else
            {
                d->m_MatchLabel->setText(match ? i18n("Passwords match") : i18n("Passwords do not match"));
            }
        }
...
void KPasswordDialog::setMaximumPasswordLength(int maxLength)
{
    ...
    if(maxLength < 0)
        maxLength = 0;
    if(maxLength >= KPasswordEdit::PassLen)
        maxLength = KPasswordEdit::PassLen - 1;

    d->maximumPasswordLength = maxLength;

    m_pEdit->setMaxPasswordLength(maxLength);
    if(m_pEdit2)
        m_pEdit2->setMaxPasswordLength(maxLength);
}
