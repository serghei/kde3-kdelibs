...
KSwitchLanguageDialog::KSwitchLanguageDialog(QWidget *parent, const char *name, bool modal)
    : KDialogBase(parent, name, modal, i18n("Switch application language"), Ok | Cancel, Ok, true), d(new KSwitchLanguageDialogPrivate(this))
{
...
KSwitchLanguageDialogPrivate::KSwitchLanguageDialogPrivate(KSwitchLanguageDialog *parent) : p(parent)
{
...
void KSwitchLanguageDialogPrivate::fillApplicationLanguages(KLanguageButton *button)
{
    ...
    for(QStringList::ConstIterator it = allLanguages.begin(); it != allLanguages.end(); ++it)
    {
        ...
        {
            button->insertItem(locale->twoAlphaToLanguageName(languageCode), languageCode);
        }
    }
}
...
void KSwitchLanguageDialogPrivate::addLanguageButton(const QString &languageCode, bool primaryLanguage)
{
    ...
    QObject::connect(languageButton, SIGNAL(activated(const QString &)), p, SLOT(languageOnButtonChanged(const QString &)));
    ...
    if(primaryLanguage == false)
    {
        ...
        QObject::connect(removeButton, SIGNAL(clicked()), p, SLOT(removeButtonClicked()));
    }
