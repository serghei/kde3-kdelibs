...
KArrowButton::KArrowButton(QWidget *parent, Qt::ArrowType arrow, const char *name) : QPushButton(parent, name)
{
...
void KArrowButton::drawButton(QPainter *p)
{
    ...
    style().drawPrimitive(QStyle::PE_Panel, p, QRect(0, 0, width(), height()), colorGroup(), isDown() ? QStyle::Style_Sunken : QStyle::Style_Default,
                          QStyleOption(2, 0));

    if(static_cast< unsigned int >(width()) < arrowSize + margin || static_cast< unsigned int >(height()) < arrowSize + margin)
    ...
    style().drawPrimitive(e, p, QRect(QPoint(x, y), QSize(arrowSize, arrowSize)), colorGroup(), flags);
}
