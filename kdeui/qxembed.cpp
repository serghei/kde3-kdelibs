...
static void sendXEmbedMessage(WId window, long message, long detail = 0, long data1 = 0, long data2 = 0)
{
    if(!window)
        return;
    XEvent ev;
...
bool QXEmbedAppFilter::eventFilter(QObject *o, QEvent *e)
{
    ...
    switch(e->type())
    {
        ...
        case QEvent::FocusIn:
            ...
            //        active (L0615).
            if(qApp->focusWidget() == o && ((QPublicWidget *)qApp->focusWidget()->topLevelWidget())->topData()->embedded)
            {
                QFocusEvent *fe = (QFocusEvent *)e;
                if(obeyFocus || fe->reason() == QFocusEvent::Mouse || fe->reason() == QFocusEvent::Shortcut)
                {
            ...
            if(e->xclient.message_type == xembed)
            {
                ...
                switch(message)
                {
                    ...
                    case XEMBED_FOCUS_OUT:
                        ...
                        {
                            focusMap->insert(w->topLevelWidget(), new QGuardedPtr< QWidget >(w->topLevelWidget()->focusWidget()));
...
QXEmbed::QXEmbed(QWidget *parent, const char *name, WFlags f) : QWidget(parent, name, f)
{
    ...
    XSelectInput(qt_xdisplay(), winId(),
                 KeyPressMask | KeyReleaseMask | ButtonPressMask | ButtonReleaseMask | KeymapStateMask | ButtonMotionMask | PointerMotionMask
                     | // may need this, too
...
void QXEmbed::embed(WId w)
{
    ...
    if(!has_window)
    {
        ...
        for(int i = 0; i < 50; i++)
        {
            ...
            {
                kdDebug() << QString("> Loop %1: ").arg(i) << QString("> reparent of 0x%1").arg(w, 0, 16) << QString(" into 0x%1").arg(winId(), 0, 16)
    ...
    switch(e->type)
    {
        ...
        case ReparentNotify:
            if(e->xreparent.window == d->focusProxy->winId())
                break; // ignore proxy
            if(window && e->xreparent.window == window && e->xreparent.parent != winId())
            {
...
void QXEmbed::checkGrab()
{
    ...
    {
        if(!d->xgrab)
            XGrabButton(qt_xdisplay(), AnyButton, AnyModifier, winId(), false, ButtonPressMask, GrabModeSync, GrabModeAsync, None, None);
        d->xgrab = true;
    }
    else
    {
