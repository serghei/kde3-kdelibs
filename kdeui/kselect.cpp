...
KXYSelector::KXYSelector(QWidget *parent, const char *name) : QWidget(parent, name)
{
    xPos = 0;
    yPos = 0;
    minX = 0;
...
void KXYSelector::paintEvent(QPaintEvent *ev)
{
    ...
    style().drawPrimitive(QStyle::PE_Panel, &painter, borderRect, colorGroup(), QStyle::Style_Sunken);

    drawContents(&painter);
    if(paintRect.contains(cursorRect))
    {
        bitBlt(&store, 0, 0, this, px - STORE_W, py - STORE_W, STORE_W2, STORE_W2, CopyROP);
...
void KXYSelector::setPosition(int xp, int yp)
{
    ...
    bitBlt(this, px - STORE_W, py - STORE_W, &store, 0, 0, STORE_W2, STORE_W2, CopyROP);
    bitBlt(&store, 0, 0, this, xp - STORE_W, yp - STORE_W, STORE_W2, STORE_W2, CopyROP);
...
KSelector::KSelector(QWidget *parent, const char *name) : QWidget(parent, name), QRangeControl()
{
    _orientation = Horizontal;
    _indent = true;
}

KSelector::KSelector(Orientation o, QWidget *parent, const char *name) : QWidget(parent, name), QRangeControl()
{
    _orientation = o;
    _indent = true;
}
...
void KSelector::paintEvent(QPaintEvent *)
{
    ...
    if(indent())
    {
        ...
        style().drawPrimitive(QStyle::PE_Panel, &painter, r, colorGroup(), QStyle::Style_Sunken);
    }
...
void KSelector::moveArrow(const QPoint &pos)
{
    ...
    if(orientation() == Vertical)
        val = (maxValue() - minValue()) * (height() - pos.y() - 5 + w) / (height() - iw * 2) + minValue();
    else
        val = (maxValue() - minValue()) * (width() - pos.x() - 5 + w) / (width() - iw * 2) + minValue();
...
QPoint KSelector::calcArrowPos(int val)
{
    ...
    if(orientation() == Vertical)
    {
        p.setY(height() - ((height() - 2 * iw) * val / (maxValue() - minValue()) + 5));
        p.setX(width() - 5);
    }
    else
    {
        p.setX(width() - ((width() - 2 * iw) * val / (maxValue() - minValue()) + 5));
        p.setY(height() - 5);
    }
...
KGradientSelector::KGradientSelector(QWidget *parent, const char *name) : KSelector(parent, name)
{
    init();
}


KGradientSelector::KGradientSelector(Orientation o, QWidget *parent, const char *name) : KSelector(o, parent, name)
{
    init();
}
...
void KGradientSelector::drawContents(QPainter *painter)
{
    ...
    if(orientation() == Vertical)
    {
        for(int y = 0; y < image.height(); y++)
        {
            ...
            col.setRgb(color1.red() + int(redDiff * scale), color1.green() + int(greenDiff * scale), color1.blue() + int(blueDiff * scale));
    ...
    else
    {
        ...
        for(int y = 1; y < image.height(); y++)
            memcpy(image.scanLine(y), image.scanLine(y - 1), sizeof(unsigned int) * image.width());
    }

    QColor ditherPalette[8];

    for(int s = 0; s < 8; s++)
        ditherPalette[s].setRgb(color1.red() + redDiff * s / 8, color1.green() + greenDiff * s / 8, color1.blue() + blueDiff * s / 8);
    ...
    if(orientation() == Vertical)
    {
        ...
        int xPos = contentsRect().left() + (contentsRect().width() - painter->fontMetrics().width(text2)) / 2;
        QPen pen(color2);
        painter->setPen(pen);
        painter->drawText(xPos, yPos, text2);

        yPos = contentsRect().bottom() - painter->fontMetrics().descent() - 2;
        xPos = contentsRect().left() + (contentsRect().width() - painter->fontMetrics().width(text1)) / 2;
        pen.setColor(color1);
        painter->setPen(pen);
    ...
    else
    {
        ...
        pen.setColor(color1);
        painter->setPen(pen);
        painter->drawText(contentsRect().right() - painter->fontMetrics().width(text2) - 2, yPos, text2);
    }
}
