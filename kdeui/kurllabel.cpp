...
KURLLabel::KURLLabel(const QString &url, const QString &text, QWidget *parent, const char *name)
    : QLabel(!text.isNull() ? text : url, parent, name), d(new Private(url, this))
{
    setFont(font());
    setUseCursor(true);
...
KURLLabel::KURLLabel(QWidget *parent, const char *name) : QLabel(parent, name), d(new Private(QString::null, this))
{
    setFont(font());
    setUseCursor(true);
    setLinkColor(d->LinkColor);
