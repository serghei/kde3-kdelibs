...
{
    return QColor(int(c1.red() * p1 + c2.red() * (1.0 - p1)), int(c1.green() * p1 + c2.green() * (1.0 - p1)),
...
void KWordWrap::drawText(QPainter *painter, int textX, int textY, int flags) const
{
    ...
    if((d->m_constrainingRect.height() < 0) || ((y + height) <= d->m_constrainingRect.height()))
    {
        if(it == m_breakPositions.end())
            ...
            drawFadeoutText(painter, textX, textY + y + ascent, d->m_constrainingRect.width(), m_text.mid(start));
        else if(flags & Truncate)
            drawTruncateText(painter, textX, textY + y + ascent, d->m_constrainingRect.width(), m_text.mid(start));
        else
            painter->drawText(x, textY + y + ascent, m_text.mid(start, (*it) - start + 1));
    }
}
