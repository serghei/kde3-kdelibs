...
KToolBarSeparator::KToolBarSeparator(Orientation o, bool l, QToolBar *parent, const char *name) : QFrame(parent, name), line(l)
{
    connect(parent, SIGNAL(orientationChanged(Orientation)), this, SLOT(setOrientation(Orientation)));
    setOrientation(o);
...
void KToolBarSeparator::drawContents(QPainter *p)
{
    if(line)
    {
        ...
        style().drawPrimitive(QStyle::PE_DockWindowSeparator, p, contentsRect(), colorGroup(), flags);
    }
    else
    {
...
KToolBar::KToolBar(QWidget *parent, const char *name, bool honorStyle, bool readConfig)
    : QToolBar(QString::fromLatin1(name), dynamic_cast< QMainWindow * >(parent), parent, false, name ? name : "mainToolBar")
{
...
KToolBar::KToolBar(QMainWindow *parentWindow, QMainWindow::ToolBarDock dock, bool newLine, const char *name, bool honorStyle, bool readConfig)
    : QToolBar(QString::fromLatin1(name), parentWindow, dock, newLine, name ? name : "mainToolBar")
{
    init(readConfig, honorStyle);
}
...
void KToolBar::init(bool readConfig, bool honorStyle)
{
    ...
    connect(layoutTimer, SIGNAL(timeout()), this, SLOT(rebuildLayout()));
    connect(&(d->repaintTimer), SIGNAL(timeout()), this, SLOT(slotRepaint()));

    if(kapp)
    { // may be null when started inside designer
    ...
    if(mainWindow())
        connect(mainWindow(), SIGNAL(toolBarPositionChanged(QToolBar *)), this, SLOT(toolBarPosChanged(QToolBar *)));
...
int KToolBar::insertButton(const QString &icon, int id, bool enabled, const QString &text, int index, KInstance *_instance)
{
...
int KToolBar::insertButton(const QString &icon, int id, const char *signal, const QObject *receiver, const char *slot, bool enabled,
...
int KToolBar::insertButton(const QPixmap &pixmap, int id, bool enabled, const QString &text, int index)
{
...
int KToolBar::insertButton(const QPixmap &pixmap, int id, const char *signal, const QObject *receiver, const char *slot, bool enabled,
...
int KToolBar::insertButton(const QString &icon, int id, QPopupMenu *popup, bool enabled, const QString &text, int index)
{
...
int KToolBar::insertButton(const QPixmap &pixmap, int id, QPopupMenu *popup, bool enabled, const QString &text, int index)
{
    KToolBarButton *button = new KToolBarButton(pixmap, id, this, 0, text);
    insertWidgetInternal(button, index, id);
    button->setEnabled(enabled);
...
int KToolBar::insertLined(const QString &text, int id, const char *signal, const QObject *receiver, const char *slot, bool enabled,
...
int KToolBar::insertCombo(const QStringList &list, int id, bool writable, const char *signal, const QObject *receiver, const char *slot, bool enabled,
                          const QString &tooltiptext, int size, int index, QComboBox::Policy policy)
{
...
int KToolBar::insertCombo(const QString &text, int id, bool writable, const char *signal, QObject *receiver, const char *slot, bool enabled,
                          const QString &tooltiptext, int size, int index, QComboBox::Policy policy)
{
    KComboBox *combo = new KComboBox(writable, this);
    insertWidgetInternal(combo, index, id);
...
int KToolBar::insertAnimatedWidget(int id, QObject *receiver, const char *slot, const QString &icons, int index)
{
...
void KToolBar::addConnection(int id, const char *signal, const QObject *receiver, const char *slot)
{
...
void KToolBar::saveState()
{
    ...
    if(d->m_xmlguiClient && !d->m_xmlguiClient->xmlFile().isEmpty())
    {
        ...
        QDomElement current;
        for(QDomNode n = d->m_xmlguiClient->domDocument().documentElement().firstChild(); !n.isNull(); n = n.nextSibling())
        {
            current = n.toElement();
        ...
        for(QDomNode n = local.documentElement().firstChild(); !n.isNull(); n = n.nextSibling())
        {
...
void KToolBar::saveSettings(KConfig *config, const QString &_configGroup)
{
    ...
    // kdDebug(220) << name() << "                position=" << position << " index=" << index << " offset=" << offset() << " newLine=" << newLine()
    // << endl;
    ...
    // kdDebug(220) << name() << "                icontext=" << icontext << " hasDefault:" << config->hasDefault( "IconText" ) << "
    // d->IconTextDefault=" << d->IconTextDefault << endl;
        ...
        case KToolBar::Bottom:
            for(QWidget *w = ncThis->widgets.first(); w; w = ncThis->widgets.next())
            {
                ...
                if(w->sizePolicy().verData() == QSizePolicy::Ignored)
                    sh.setHeight(1);
                sh = sh.boundedTo(w->maximumSize()).expandedTo(w->minimumSize()).expandedTo(QSize(1, 1));
...
void KToolBar::positionYourself(bool force)
{
    ...
    // kdDebug(220) << name() << " positionYourself  dock=" << d->toolBarInfo.dock << " newLine=" << d->toolBarInfo.newline << " index=" <<
    // d->toolBarInfo.index << " offset=" << d->toolBarInfo.offset << endl;
    mainWindow()->moveDockWindow(this, d->toolBarInfo.dock, d->toolBarInfo.newline, d->toolBarInfo.index, d->toolBarInfo.offset);
