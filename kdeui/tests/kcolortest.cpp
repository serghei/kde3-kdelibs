...
KColorWidget::KColorWidget(QWidget *parent, const char *name) : QWidget(parent, name)
{
...
{
    if(!pixmap.isNull())
        bitBlt(this, 0, 0, &pixmap, 0, 0, pixmap.width(), pixmap.height(), Qt::CopyROP, true);
}
    ...
    if(fullscreen)
    {
        ...
        for(count = 0; count < max; ++count)
        {
            ...
            else
            {
                ...
                {
                    qrgb = *(QRgb *)qptr;
                    *qptr = qRgb((int)(qRed(qrgb) * 1. / max), (int)(qGreen(qrgb) * 1. / max), (int)(qBlue(qrgb) * 1. / max));
                }
            }
        ...
        stop = t.elapsed();
        qDebug("Total fullscreen %s dim time for %d steps : %f s", oldway ? "(antonio)" : "(intensity)", count, (stop - start) * 1e-3);
        ...
        stop = t.elapsed();
        qDebug("Total fullscreen (fade) dim time for %d steps : %f s", count, (stop - start) * 1e-3);
    }
