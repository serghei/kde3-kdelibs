...
TopLevel::TopLevel(QWidget *parent, const char *name) : QWidget(parent, name)
{
    ...
    connect(i5, SIGNAL(relativeValueChanged(double)), i6, SLOT(setRelativeValue(double)));
    connect(i6, SIGNAL(relativeValueChanged(double)), i5, SLOT(setRelativeValue(double)));
    ...
    d5 = new KDoubleNumInput(d4, double(INT_MIN + 1) / 1e9, double(INT_MAX - 1) / 1e9, 0.1, 0.001, 9, b2, "d5");
    ...
    connect(d6, SIGNAL(relativeValueChanged(double)), d7, SLOT(setRelativeValue(double)));
    connect(d7, SIGNAL(relativeValueChanged(double)), d6, SLOT(setRelativeValue(double)));
