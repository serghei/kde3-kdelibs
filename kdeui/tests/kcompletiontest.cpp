 ...
 */
Form1::Form1(QWidget *parent, const char *name) : QWidget(parent, name, WDestructiveClose)
{
    if(!name)
        setName("Form1");
    resize(559, 465);
    ...
    connect(ListBox1, SIGNAL(highlighted(const QString &)), SLOT(slotHighlighted(const QString &)));
    ...
    connect(PushButton1_2, SIGNAL(clicked()), edit->completionObject(), SLOT(clear()));
