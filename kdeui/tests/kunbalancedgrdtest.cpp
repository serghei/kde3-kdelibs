...
KGradientWidget::KGradientWidget(QWidget *parent, const char *name) : QWidget(parent, name)
{
    time.start();
...
void KGradientWidget::paintEvent(QPaintEvent *)
{
    ...
    KPixmapEffect::unbalancedGradient(pix, ca, cb, KPixmapEffect::VerticalGradient, xbalance, ybalance);
    ...
    // horizontal
    it = time.elapsed();
    KPixmapEffect::unbalancedGradient(pix, ca, cb, KPixmapEffect::HorizontalGradient, xbalance, ybalance);
    ft = time.elapsed();
    say.setNum(ft - it);
    say = "Horizontal";
    p.drawPixmap(x * width() / cols, y * height() / rows, pix);
    ...
    // elliptic
    it = time.elapsed();
    KPixmapEffect::unbalancedGradient(pix, ca, cb, KPixmapEffect::EllipticGradient, xbalance, ybalance);
    ft = time.elapsed();
    say.setNum(ft - it);
    say = "Elliptic";
    p.drawPixmap(x * width() / cols, y * height() / rows, pix);
    ...
    // diagonal
    it = time.elapsed();
    KPixmapEffect::unbalancedGradient(pix, ca, cb, KPixmapEffect::DiagonalGradient, xbalance, ybalance);
    ft = time.elapsed();
    say.setNum(ft - it);
    say = "Diagonal";
    p.drawPixmap(x * width() / cols, y * height() / rows, pix);
    ...
    // crossdiagonal
    it = time.elapsed();
    KPixmapEffect::unbalancedGradient(pix, ca, cb, KPixmapEffect::CrossDiagonalGradient, xbalance, ybalance);
    ft = time.elapsed();
    say.setNum(ft - it);
    say = "CrossDiagonal";
    ...
    // pyramidal
    it = time.elapsed();
    KPixmapEffect::unbalancedGradient(pix, ca, cb, KPixmapEffect::PyramidGradient, xbalance, ybalance);
    ft = time.elapsed();
    say.setNum(ft - it);
    say = "Pyramid";
    p.drawPixmap(x * width() / cols, y * height() / rows, pix);
    ...
    // rectangular
    it = time.elapsed();
    KPixmapEffect::unbalancedGradient(pix, ca, cb, KPixmapEffect::RectangleGradient, xbalance, ybalance);
    ft = time.elapsed();
    say.setNum(ft - it);
    say = "Rectangle";
    p.drawPixmap(x * width() / cols, y * height() / rows, pix);
    ...
    // crosspipe
    it = time.elapsed();
    KPixmapEffect::unbalancedGradient(pix, ca, cb, KPixmapEffect::PipeCrossGradient, xbalance, ybalance);
    ft = time.elapsed();
    say.setNum(ft - it);
    say = "PipeCross";
    p.drawPixmap(x * width() / cols, y * height() / rows, pix);
...
myTopWidget::myTopWidget(QWidget *parent, const char *name) : QWidget(parent, name)
{
...
void myTopWidget::rebalance()
{
    ...
    QString val;
    val.sprintf("Balance: X = %3d; Y = %3d", xSlider->value(), ySlider->value());
