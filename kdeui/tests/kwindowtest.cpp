...
testWindow::testWindow(QWidget *parent, const char *name) : KMainWindow(parent, name)
{
    ena = false;
    ...
    fileMenu->insertItem("&Exit", KApplication::kApplication(), SLOT(quit()), ALT + Key_Q);
    ...
    // First four  buttons
    pix = BarIcon("filenew");
    itemId = tb->insertButton(pix, 0, SIGNAL(clicked()), this, SLOT(slotNew()), true, "Create.. (toggles upper button)", 50);
    pix = BarIcon("fileopen");
    tb->insertButton(pix, 1, SIGNAL(clicked()), this, SLOT(slotOpen()), false, "Open");
    pix = BarIcon("filefloppy");
    tb->insertButton(pix, 2, SIGNAL(clicked()), this, SLOT(slotSave()), true, "Save (beep or delayed popup)");
    tb->setDelayedPopup(2, itemsMenu);
    pix = BarIcon("fileprint");
    tb->insertButton(pix, 3, SIGNAL(clicked()), this, SLOT(slotPrint()), true, "Print (enables/disables open)");

    // And a combobox
    // arguments: text (or strList), ID, writable, signal, object, slot, enabled,
    //            tooltiptext, size
    tb->insertCombo(QString("one"), 4, true, SIGNAL(activated(const QString &)), this, SLOT(slotList(const QString &)), true, "ComboBox", 150);


    // Then one line editor
    // arguments: text, id, signal, object (this), slot, enabled, tooltiptext, size
    tb->insertLined("ftp://ftp.kde.org/pub/kde", 5, SIGNAL(returnPressed()), this, SLOT(slotReturn()), true, "Location", 200);
    ...
    pix = BarIcon("exit");
    tb->insertButton(pix, 6, SIGNAL(clicked()), KApplication::kApplication(), SLOT(quit()), true, "Exit");
    tb->alignItemRight(6);
    ...
    pix = BarIcon("fileopen");
    tb1->insertButton(pix, 1, SIGNAL(clicked()), this, SLOT(slotOpen()), true, "Open (starts progres in sb)");

    tb1->insertSeparator();

    pix = BarIcon("filefloppy");
    tb1->insertButton(pix, 2, SIGNAL(clicked()), this, SLOT(slotSave()), true, "Save file2 (autorepeat)");
    tb1->setAutoRepeat(2);
...
int main(int argc, char *argv[])
{
    ...
    i = QMessageBox::information(0, "Select", "Select type of mainwidget", "Fixed", "Y-fixed", "Resizable");
    if(i == 0)
        test->beFixed();
    else if(i == 1)
        test->beYFixed();
