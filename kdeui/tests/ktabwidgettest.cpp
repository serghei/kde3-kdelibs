...
void Test::leftPopupActivated(int item)
{
    switch(item)
    {
        case 0:
            ...
            break;
        case 1:
            mWidget->addTab(new QPushButton("Testbutton", mWidget), QString("Tab %1").arg(mWidget->count() + 1));
            break;
        case 2:
            mWidget->addTab(new QLabel("Testlabel", mWidget), QString("Tab %1").arg(mWidget->count() + 1));
            break;
        case 3:
            mWidget->insertTab(new QWidget(mWidget), QString("Tab %1").arg(mWidget->count() + 1), 1);
    }
}
...
void Test::contextMenuActivated(int item)
{
    ...
    {
        case 0:
            mWidget->changeTab(mContextWidget, SmallIcon("konsole"), mWidget->label(mWidget->indexOf(mContextWidget)));
            break;
        case 1:
            mWidget->changeTab(mContextWidget, SmallIcon("konqueror"), mWidget->label(mWidget->indexOf(mContextWidget)));
            break;
        case 2:
            mWidget->setTabEnabled(mContextWidget, !(mWidget->isTabEnabled(mContextWidget)));
            break;
        case 3:
            if(mWidget->tabToolTip(mContextWidget).isEmpty())
                mWidget->setTabToolTip(mContextWidget, "This is a tool tip.");
            else
                mWidget->removeTabToolTip(mContextWidget);
            break;
        case 4:
            mWidget->showPage(mContextWidget);
    }
    delete mContextPopup;
    mContextPopup = 0;
}
...
{
    bool ok;
    QString text = KInputDialog::getText("Rename Tab", "Enter new name:", mWidget->label(mWidget->indexOf(w)), &ok, this);
    if(ok && !text.isEmpty())
    {
