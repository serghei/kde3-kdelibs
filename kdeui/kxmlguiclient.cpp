...
KActionCollection *KXMLGUIClient::actionCollection() const
{
    ...
    {
        d->m_actionCollection = new KActionCollection("KXMLGUIClient-KActionCollection", this);
    }
...
bool KXMLGUIClient::mergeXML(QDomElement &base, const QDomElement &additive, KActionCollection *actionCollection)
{
    ...
    while(!n.isNull())
    {
        ...
        if(tag == tagAction)
        {
            ...
            if(!actionCollection->action(name) || (kapp && !kapp->authorizeKAction(name)))
            {
        ...
        else if(tag == tagSeparator)
        {
            ...
            if(prev.isNull() || (prev.tagName() == tagSeparator && !prev.attribute(attrWeakSeparator).isNull()) || (prev.tagName() == tagText))
            {
...
QDomElement KXMLGUIClient::findMatchingElement(const QDomElement &base, const QDomElement &additive)
{
    ...
    while(!n.isNull())
    {
        ...
        if((e.tagName() == base.tagName()) && (e.attribute(attrName) == base.attribute(attrName)))
        {
            return e;
        }
    }
...
void KXMLGUIClient::stateChanged(const QString &newstate, KXMLGUIClient::ReverseStateChange reverse)
{
    ...
    //
    for(QStringList::Iterator it = stateChange.actionsToEnable.begin(); it != stateChange.actionsToEnable.end(); ++it)
    {
    ...
    //
    for(QStringList::Iterator it = stateChange.actionsToDisable.begin(); it != stateChange.actionsToDisable.end(); ++it)
    {
