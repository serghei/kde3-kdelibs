...
void KTabBar::mouseMoveEvent(QMouseEvent *e)
{
    if(e->state() == LeftButton)
    {
        ...
        QPoint newPos = e->pos();
        if(newPos.x() > mDragStart.x() + delay || newPos.x() < mDragStart.x() - delay || newPos.y() > mDragStart.y() + delay
           || newPos.y() < mDragStart.y() - delay)
        {
            if(tab)
            {
...
void KTabBar::paintLabel(QPainter *p, const QRect &br, QTab *t, bool has_focus) const
{
    QRect r = br;
    ...
    if(t->iconSet())
    {
        ...
        p->drawPixmap(right + (selected ? 0 : inactiveXShift), br.center().y() - pixh / 2 + (selected ? 0 : inactiveYShift), pixmap);
    }
    ...
    style().drawControl(QStyle::CE_TabBarLabel, p, this, r, t->isEnabled() ? cg : palette().disabled(), flags, QStyleOption(t));
}
