...
KLineEdit::KLineEdit(const QString &string, QWidget *parent, const char *name) : QLineEdit(string, parent, name)
{
    init();
}

KLineEdit::KLineEdit(QWidget *parent, const char *name) : QLineEdit(parent, name)
{
    init();
}
...
void KLineEdit::init()
{
    ...
    d->autoSuggest =
        (mode == KGlobalSettings::CompletionMan || mode == KGlobalSettings::CompletionPopupAuto || mode == KGlobalSettings::CompletionAuto);
...
void KLineEdit::setCompletionMode(KGlobalSettings::Completion mode)
{
    ...
    if(oldMode != mode && (oldMode == KGlobalSettings::CompletionPopup || oldMode == KGlobalSettings::CompletionPopupAuto) && d->completionBox
    ...
    if(mode == KGlobalSettings::CompletionPopupAuto || mode == KGlobalSettings::CompletionAuto || mode == KGlobalSettings::CompletionMan)
        d->autoSuggest = true;
    else
        d->autoSuggest = false;
...
{
    KGlobalSettings::Completion mode = completionMode();
    bool marked = (mode == KGlobalSettings::CompletionAuto || mode == KGlobalSettings::CompletionMan || mode == KGlobalSettings::CompletionPopup
...
void KLineEdit::rotateText(KCompletionBase::KeyBindingType type)
{
    ...
    if(comp && (type == KCompletionBase::PrevCompletionMatch || type == KCompletionBase::NextCompletionMatch))
    {
        QString input;
...
void KLineEdit::keyPressEvent(QKeyEvent *e)
{
    ...
    if(echoMode() == QLineEdit::Normal && completionMode() != KGlobalSettings::CompletionNone)
    {
        KeyBindingMap keys = getKeyBindings();
        KGlobalSettings::Completion mode = completionMode();
        bool noModifier = (e->state() == NoButton || e->state() == ShiftButton || e->state() == Keypad);

        if((mode == KGlobalSettings::CompletionAuto || mode == KGlobalSettings::CompletionPopupAuto || mode == KGlobalSettings::CompletionMan)
           && noModifier)
        {
            if(!d->userSelection && hasSelectedText() && (e->key() == Key_Right || e->key() == Key_Left) && e->state() == NoButton)
            {
        ...
        if((mode == KGlobalSettings::CompletionAuto || mode == KGlobalSettings::CompletionMan) && noModifier)
        {
            QString keycode = e->text();
            if(!keycode.isEmpty() && (keycode.unicode()->isPrint() || e->key() == Key_Backspace || e->key() == Key_Delete))
            {
        ...
        else if((mode == KGlobalSettings::CompletionPopup || mode == KGlobalSettings::CompletionPopupAuto) && noModifier && !e->text().isEmpty())
        {
            QString old_txt = text();
            bool hasUserSelection = d->userSelection;
            bool hadSelection = hasSelectedText();
            ...
            // can set the new autocompletion again.
            if(hadSelection && !hasUserSelection && start > cPos
               && ((!keycode.isEmpty() && keycode.unicode()->isPrint()) || e->key() == Key_Backspace || e->key() == Key_Delete))
            {
                del();
                setCursorPosition(cPos);
                cursorNotAtEnd = true;
...
QPopupMenu *KLineEdit::createPopupMenu()
{
    ...
    if(compObj() && !isReadOnly() && kapp->authorize("lineedit_text_completion"))
    {
        ...
        connect(subMenu, SIGNAL(activated(int)), this, SLOT(completionMenuActivated(int)));

        popup->insertSeparator();
        popup->insertItem(SmallIconSet("completion"), i18n("Text Completion"), subMenu);
        ...
        subMenu->setItemChecked(ShortAutoCompletion, mode == KGlobalSettings::CompletionMan);
        subMenu->setItemChecked(PopupAutoCompletion, mode == KGlobalSettings::CompletionPopupAuto);
...
void KLineEdit::completionMenuActivated(int id)
{
    ...
    {
        if((oldMode == KGlobalSettings::CompletionPopup || oldMode == KGlobalSettings::CompletionPopupAuto) && d->completionBox
           && d->completionBox->isVisible())
            d->completionBox->hide();
    ...
    if(o == this)
    {
        ...
        else if(ev->type() == QEvent::KeyPress)
        {
            ...
            if(e->key() == Qt::Key_Return || e->key() == Qt::Key_Enter)
            {
                ...
                bool stopEvent = trap || (d->grabReturnKeyEvents && (e->state() == NoButton || e->state() == Keypad));
...
void KLineEdit::setCompletionBox(KCompletionBox *box)
{
    ...
    if(handleSignals())
    {
        connect(d->completionBox, SIGNAL(highlighted(const QString &)), SLOT(setTextWorkaround(const QString &)));
        connect(d->completionBox, SIGNAL(userCancelled(const QString &)), SLOT(userCancelled(const QString &)));

        // TODO: we need our own slot, and to call setModified(true) if Qt4 has that.
        connect(d->completionBox, SIGNAL(activated(const QString &)), SIGNAL(completionBoxActivated(const QString &)));
    }
}
...
bool KLineEdit::overrideAccel(const QKeyEvent *e)
{
    ...
    if(d->completionBox && d->completionBox->isVisible())
    {
        ...
        if((key == Key_Backtab || key == Key_Tab) && (state == NoButton || (state & ShiftButton)))
        {
            return true;
        }
    }
