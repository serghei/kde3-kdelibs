...
KDialog::KDialog(QWidget *parent, const char *name, bool modal, WFlags f) : QDialog(parent, name, modal, f), d(0)
{
...
static QRect screenRect(QWidget *w, int screen)
{
    ...
    gc.setGroup("Windows");
    if(desktop->isVirtualDesktop() && gc.readBoolEntry("XineramaEnabled", true) && gc.readBoolEntry("XineramaPlacementEnabled", true))
    {
...
void KDialog::centerOnScreen(QWidget *w, int screen)
{
    ...
    w->move(r.center().x() - w->width() / 2, r.center().y() - w->height() / 2);
}
