    ...
    , d(0)
{
    ...
    if(mFace == TreeList || mFace == IconList)
    {
        ...
        connect(mPageStack, SIGNAL(aboutToShow(QWidget *)), SIGNAL(aboutToShowPage(QWidget *)));
    ...
    else if(mFace == Tabbed)
    {
        ...
        connect(mTabControl, SIGNAL(currentChanged(QWidget *)), SIGNAL(aboutToShowPage(QWidget *)));
...
QFrame *KJanusWidget::addPage(const QStringList &items, const QString &header, const QPixmap &pixmap)
{
    if(!mValid)
    {
...
QVBox *KJanusWidget::addVBoxPage(const QStringList &items, const QString &header, const QPixmap &pixmap)
{
    if(!mValid)
    {
        kdDebug() << "addPage: Invalid object" << endl;
...
QVBox *KJanusWidget::addVBoxPage(const QString &itemName, const QString &header, const QPixmap &pixmap)
{
    QStringList items;
    items << itemName;
...
QHBox *KJanusWidget::addHBoxPage(const QStringList &items, const QString &header, const QPixmap &pixmap)
{
    if(!mValid)
    {
        kdDebug() << "addPage: Invalid object" << endl;
...
QHBox *KJanusWidget::addHBoxPage(const QString &itemName, const QString &header, const QPixmap &pixmap)
{
    QStringList items;
    items << itemName;
...
QGrid *KJanusWidget::addGridPage(int n, Orientation dir, const QStringList &items, const QString &header, const QPixmap &pixmap)
{
    if(!mValid)
    {
        kdDebug() << "addPage: Invalid object" << endl;
...
QGrid *KJanusWidget::addGridPage(int n, Orientation dir, const QString &itemName, const QString &header, const QPixmap &pixmap)
{
    QStringList items;
    items << itemName;
...
void KJanusWidget::addPageWidget(QFrame *page, const QStringList &items, const QString &header, const QPixmap &pixmap)
{
...
void KJanusWidget::setTreeListAutoResize(bool state)
{
    if(mFace == TreeList)
    {
        mTreeListResizeMode = !state ? QSplitter::KeepSize : QSplitter::Stretch;
        if(d->mSplitter)
...
KJanusWidget::IconListBox::IconListBox(QWidget *parent, const char *name, WFlags f)
    : KListBox(parent, name, f), mShowAll(false), mHeightValid(false), mWidthValid(false)
{
}
...
KJanusWidget::IconListItem::IconListItem(QListBox *listbox, const QPixmap &pixmap, const QString &text) : QListBoxItem(listbox)
{
    mPixmap = pixmap;
    if(mPixmap.isNull())
    {
