...
KTabCtl::KTabCtl(QWidget *parent, const char *name) : QWidget(parent, name)
{
...
QRect KTabCtl::getChildRect() const
{
    if((tabs->shape() == QTabBar::RoundedBelow) || (tabs->shape() == QTabBar::TriangularBelow))
    {
        return QRect(2, 1, width() - 4, height() - tabs->height() - 4);
    }
    else
    {
        return QRect(2, tabs->height() + 1, width() - 4, height() - tabs->height() - 4);
    }
}
