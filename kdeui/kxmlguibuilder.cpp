    ...
    if(element.tagName().lower() == d->tagSeparator)
    {
        if(::qt_cast< QPopupMenu * >(parent))
        {
            ...
            if(count)
            {
                ...
                if(previousId != -1)
                {
                    if(menu->text(previousId).isEmpty() && !menu->iconSet(previousId) && !menu->pixmap(previousId))
                        return 0;
                }
            }
        ...
        else if(::qt_cast< KToolBar * >(parent))
        {
            ...
            for(; i < attributes.length(); i++)
            {
                ...
                if(attr.name().lower() == d->attrLineSeparator && attr.value().lower() == QString::fromLatin1("false"))
                {
                    isLineSep = false;
                    break;
                }
