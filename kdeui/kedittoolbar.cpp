...
public:
    ToolbarItem(KListView *parent, const QString &tag = QString::null, const QString &name = QString::null, const QString &statusText = QString::null)
        : QListViewItem(parent), m_tag(tag), m_name(name), m_statusText(statusText)
    {
    }

    ToolbarItem(KListView *parent, QListViewItem *item, const QString &tag = QString::null, const QString &name = QString::null,
                const QString &statusText = QString::null)
        : QListViewItem(parent, item), m_tag(tag), m_name(name), m_statusText(statusText)
    {
    }
...
public:
    ToolbarItemDrag(ToolbarItem *toolbarItem, QWidget *dragSource = 0, const char *name = 0) : QStoredDrag(TOOLBARITEMMIMETYPE, dragSource, name)
    {
        if(toolbarItem)
        {
            QByteArray data;
...
public:
    ToolbarListView(QWidget *parent = 0, const char *name = 0) : KListView(parent, name)
    {
    }
...
protected:
    virtual QDragObject *dragObject()
    {
        ...
        if(item)
        {
            ToolbarItemDrag *obj = new ToolbarItemDrag(item, this, "ToolbarAction drag item");
...
public:
     ...
     */
    KEditToolbarWidgetPrivate(KInstance *instance, KActionCollection *collection) : m_collection(collection)
    {
        m_instance = instance;
        m_isPart = false;
        m_helpArea = 0L;
    ...
    {
        return xml_file.isNull() ? QString(m_instance->instanceName()) + "ui.rc" : xml_file;
    }
    ...
    QString toolbarName(const XmlData &xmlData, const QDomElement &it) const
    {
        ...
        // it is in kparts
        if((xmlData.m_type == XmlData::Shell) || (xmlData.m_type == XmlData::Part))
        {
    ...
    QDomElement findElementForToolbarItem(const ToolbarItem *item) const
    {
        ...
        {
            QDomElement elem = n.toElement();
            if((elem.attribute(attrName) == item->internalName()) && (elem.tagName() == item->internalTag()))
                return elem;
        }
        return QDomElement();
    }
    ...
    void dump()
    {
        ...
        for(; xit != m_xmlFiles.end(); ++xit)
        {
            ...
            for(QValueList< QDomElement >::Iterator it = (*xit).m_barList.begin(); it != (*xit).m_barList.end(); ++it)
            {
...
KEditToolbar::KEditToolbar(KActionCollection *collection, const QString &file, bool global, QWidget *parent, const char *name)
...
KEditToolbar::KEditToolbar(const QString &defaultToolbar, KActionCollection *collection, const QString &file, bool global, QWidget *parent,
                           const char *name)
    : KDialogBase(Swallow, i18n("Configure Toolbars"), Default | Ok | Apply | Cancel, Ok, parent, name)
...
KEditToolbar::KEditToolbar(const QString &defaultToolbar, KXMLGUIFactory *factory, QWidget *parent, const char *name)
    : KDialogBase(Swallow, i18n("Configure Toolbars"), Default | Ok | Apply | Cancel, Ok, parent, name)
...
void KEditToolbar::slotDefault()
{
    if(KMessageBox::warningContinueCancel(
           this, i18n("Do you really want to reset all toolbars of this application to their default? The changes will be applied immediately."),
           i18n("Reset Toolbars"), i18n("Reset"))
...
KEditToolbarWidget::KEditToolbarWidget(KActionCollection *collection, const QString &file, bool global, QWidget *parent)
    : QWidget(parent), d(new KEditToolbarWidgetPrivate(instance(), collection))
{
...
KEditToolbarWidget::KEditToolbarWidget(const QString &defaultToolbar, KActionCollection *collection, const QString &file, bool global,
                                       QWidget *parent)
    : QWidget(parent), d(new KEditToolbarWidgetPrivate(instance(), collection))
{
    initNonKPart(collection, file, global);
...
KEditToolbarWidget::KEditToolbarWidget(KXMLGUIFactory *factory, QWidget *parent)
    : QWidget(parent), d(new KEditToolbarWidgetPrivate(instance(), KXMLGUIClient::actionCollection() /*create new one*/))
{
    initKPart(factory);
    // now load in our toolbar combo box
    loadToolbarCombo();
...
KEditToolbarWidget::KEditToolbarWidget(const QString &defaultToolbar, KXMLGUIFactory *factory, QWidget *parent)
    : QWidget(parent), d(new KEditToolbarWidgetPrivate(instance(), KXMLGUIClient::actionCollection() /*create new one*/))
{
    initKPart(factory);
    // now load in our toolbar combo box
...
void KEditToolbarWidget::initNonKPart(KActionCollection *collection, const QString &file, bool global)
{
...
void KEditToolbarWidget::setupLayout()
{
    ...
    connect(m_toolbarCombo, SIGNAL(activated(const QString &)), this, SLOT(slotToolbarSelected(const QString &)));
    ...
    connect(m_inactiveList, SIGNAL(selectionChanged(QListViewItem *)), this, SLOT(slotInactiveSelected(QListViewItem *)));
    connect(m_inactiveList, SIGNAL(doubleClicked(QListViewItem *, const QPoint &, int)), this, SLOT(slotInsertButton()));
    ...
    connect(m_inactiveList, SIGNAL(dropped(KListView *, QDropEvent *, QListViewItem *)), this,
            SLOT(slotDropped(KListView *, QDropEvent *, QListViewItem *)));
    connect(m_activeList, SIGNAL(dropped(KListView *, QDropEvent *, QListViewItem *)), this,
            SLOT(slotDropped(KListView *, QDropEvent *, QListViewItem *)));
    connect(m_activeList, SIGNAL(selectionChanged(QListViewItem *)), this, SLOT(slotActiveSelected(QListViewItem *)));
    connect(m_activeList, SIGNAL(doubleClicked(QListViewItem *, const QPoint &, int)), this, SLOT(slotRemoveButton()));
    ...
    connect(d->m_changeIcon, SIGNAL(clicked()), this, SLOT(slotChangeIcon()));
        ...
        if(it.tagName() == tagActionList)
        {
            ToolbarItem *act =
                new ToolbarItem(m_activeList, tagActionList, it.attribute(attrName),
                                i18n("This is a dynamic list of actions. You can move it, but if you remove it you won't be able to re-add it."));
...
void KEditToolbarWidget::slotActiveSelected(QListViewItem *item)
{
    ...
    d->m_changeIcon->setEnabled(item && d->m_hasKDialog && toolitem->internalTag() == tagAction);
...
void KEditToolbarWidget::moveActive(ToolbarItem *item, QListViewItem *before)
{
    ...
    ToolbarItem *clone = new ToolbarItem(m_activeList, before, item->internalTag(), item->internalName(), item->statusText());
...
void KEditToolbarWidget::updateLocal(QDomElement &elem)
{
    ...
    for(; xit != d->m_xmlFiles.end(); ++xit)
    {
        ...
        if((*xit).m_type == XmlData::Shell || (*xit).m_type == XmlData::Part)
        {
...
void KEditToolbarWidget::slotChangeIcon()
{
    ...
    connect(d->m_kdialogProcess, SIGNAL(processExited(KProcess *)), this, SLOT(slotProcessExited(KProcess *)));
}
...
void KEditToolbarWidget::slotProcessExited(KProcess *)
{
    ...
    if(!d->m_kdialogProcess->normalExit() || d->m_kdialogProcess->exitStatus() || d->m_kdialogProcess->readln(icon, true) <= 0)
    {
        delete d->m_kdialogProcess;
        d->m_kdialogProcess = 0;
        return;
