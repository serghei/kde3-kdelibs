...
int KEdit::doSearch(QString s_pattern, bool case_sensitive, bool wildcard, bool forward, int line, int col)
{
...
again:
    ...
    while(result)
    {
        ...
        result = doReplace(to_find_string, replace_dialog->case_sensitive(), false, (!replace_dialog->get_direction()), replace_all_line,
                           replace_all_col, true);
    }
...
again:
    ...
    int result = doReplace(to_find_string, replace_dialog->case_sensitive(), false, (!replace_dialog->get_direction()), line, col, false);
    ...
    if(!result)
    {
        ...
        { // forward search
            ...
            int query = KMessageBox::questionYesNo(replace_dialog, i18n("End of document reached.\n"
                                                                        "Continue from the beginning?"),
        ...
        { // backward search
            ...
            int query = KMessageBox::questionYesNo(replace_dialog, i18n("Beginning of document reached.\n"
                                                                        "Continue from the end?"),
                                                   i18n("Replace"), KStdGuiItem::cont(), i18n("Stop"));
            if(query == KMessageBox::Yes)
            {
...
/* antlarr: KDE 4: make it const QString & */
int KEdit::doReplace(QString s_pattern, bool case_sensitive, bool wildcard, bool forward, int line, int col, bool replace_all)
{
        ...
        while(line_counter >= 0)
        {
            ...
            else
            {
                if((line == line_counter) && (col < 0))
                    ...
                    pos = string.findRev(s_pattern, line == line_counter ? col : line_length, case_sensitive);
            }
...
KEdFind::KEdFind(QWidget *parent, const char *name, bool modal)
    : KDialogBase(parent, name, modal, i18n("Find"), modal ? User1 | Cancel : User1 | Close, User1, false, KGuiItem(i18n("&Find"), "find"))
{
    ...
    connect(d->combo, SIGNAL(textChanged(const QString &)), this, SLOT(textSearchChanged(const QString &)));
...
KEdReplace::KEdReplace(QWidget *parent, const char *name, bool modal)
    : KDialogBase(parent, name, modal, i18n("Replace"), modal ? User3 | User2 | User1 | Cancel : User3 | User2 | User1 | Close, User3, false,
                  i18n("Replace &All"), i18n("&Replace"), KGuiItem(i18n("&Find"), "find"))
{
    ...
    connect(d->searchCombo, SIGNAL(textChanged(const QString &)), this, SLOT(textSearchChanged(const QString &)));
