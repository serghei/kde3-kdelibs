...
void HTMLImageElementImpl::parseAttribute(AttributeImpl *attr)
{
    switch(attr->id())
    {
        ...
        case ATTR_SRC:
        {
            ...
            if(!url.isEmpty())
            { //### why do we not hide or something when setting this?
        ...
        case ATTR_ONABORT: // ### add support for this
            setHTMLEventListener(EventImpl::ABORT_EVENT, getDocument()->createHTMLEventListener(attr->value().string(), "onabort", this));
            break;
        case ATTR_ONERROR:
            setHTMLEventListener(EventImpl::ERROR_EVENT, getDocument()->createHTMLEventListener(attr->value().string(), "onerror", this));
            break;
        case ATTR_ONLOAD:
...
long HTMLImageElementImpl::width() const
{
    ...
    return m_render ? m_render->contentWidth() : getAttribute(ATTR_WIDTH).toInt();
}
...
long HTMLImageElementImpl::height() const
{
    ...
    return m_render ? m_render->contentHeight() : getAttribute(ATTR_HEIGHT).toInt();
}
...
bool HTMLMapElementImpl::mapMouseEvent(int x_, int y_, int width_, int height_, RenderObject::NodeInfo &info)
{
...
HTMLAreaElementImpl::HTMLAreaElementImpl(DocumentImpl *doc) : HTMLAnchorElementImpl(doc)
{
    m_coords = 0;
    m_coordsLen = 0;
    nohref = false;
...
bool HTMLAreaElementImpl::mapMouseEvent(int x_, int y_, int width_, int height_, RenderObject::NodeInfo &info)
{
    bool inside = false;
...
QRegion HTMLAreaElementImpl::getRegion(int width_, int height_) const
{
    ...
    if((shape == Poly || shape == Unknown) && m_coordsLen > 5)
    {
        ...
        for(int i = 0; i < len; ++i)
            points.setPoint(i, m_coords[(i << 1)].minWidth(width_), m_coords[(i << 1) + 1].minWidth(height_));
    ...
    else if((shape == Circle && m_coordsLen >= 3) || (shape == Unknown && m_coordsLen == 3))
    {
        ...
        region = QRegion(m_coords[0].minWidth(width_) - r, m_coords[1].minWidth(height_) - r, 2 * r, 2 * r, QRegion::Ellipse);
    }
