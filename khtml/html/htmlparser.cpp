...
class HTMLStackElem {
public:
    HTMLStackElem(int _id, int _level, DOM::NodeImpl *_node, bool _inline, HTMLStackElem *_next)
        : id(_id), level(_level), strayTableContent(false), m_inline(_inline), node(_node), next(_next)
    {
        node->ref();
    }
...
void KHTMLParser::parseToken(Token *t)
{
    ...
    {
        if(inBody && !skipMode() && current->id() != ID_STYLE && current->id() != ID_TITLE && current->id() != ID_SCRIPT
...
{
    return (id == ID_TR || id == ID_TD || id == ID_TABLE || id == ID_TBODY || id == ID_TFOOT || id == ID_THEAD || id == ID_TH);
}
...
// this should work if the document is wellformed
    ...
    {
#ifdef PARSER_DEBUG
        kdDebug(6035) << "added " << n->nodeName().string() << " to " << tmp->nodeName().string() << ", new current=" << newNode->nodeName().string()
                      << endl;
#endif
            ...
            case ID_TR:
                switch(id)
                {
                    ...
                    default:
                    {
                        ...
                        if(n->isTextNode()
                           || (node->id() == ID_TR && (parent->id() == ID_THEAD || parent->id() == ID_TBODY || parent->id() == ID_TFOOT)
                               && parentparent->id() == ID_TABLE)
                           || (!checkChild(ID_TR, id) && (node->id() == ID_THEAD || node->id() == ID_TBODY || node->id() == ID_TFOOT)
                               && parent->id() == ID_TABLE))
                        {
                            node = (node->id() == ID_TABLE) ? node : ((node->id() == ID_TR) ? parentparent : parent);
                            NodeImpl *parent = node->parentNode();
                            if(!parent)
                                return false;
                            int exceptioncode = 0;
                            ...
                            if(n->isElementNode() && tagPriority[id] != 0 && !flat && endTag[id] != DOM::FORBIDDEN)
                            {
...
NodeImpl *KHTMLParser::getElement(Token *t)
{
    ...
    switch(t->tid)
    {
        ...
        case ID_FRAMESET:
            ...
            if(inBody && !haveFrameSet && !haveContent && !haveBody)
            {
                ...
                if(static_cast< HTMLDocumentImpl * >(document)->body())
                    static_cast< HTMLDocumentImpl * >(document)->body()->addCSSProperty(CSS_PROP_DISPLAY, CSS_VAL_NONE);
                inBody = false;
            }
        ...
        case ID_FIELDSET:
            ...
            break;
        case ID_INPUT:
            if(t->attrs && KHTMLFactory::defaultHTMLSettings()->isAdFilterEnabled() && KHTMLFactory::defaultHTMLSettings()->isHideAdsEnabled()
