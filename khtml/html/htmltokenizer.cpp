...
void HTMLTokenizer::parseSpecial(TokenizerString &src)
{
    ...
    while(!src.isEmpty())
    {
        ...
        if(!scriptCodeResync && !brokenComments && !textarea && !xmp && ch == '-' && scriptCodeSize >= 3 && !src.escaped()
        ...
        if(!scriptCodeResync && !escaped && !src.escaped() && (ch == '>' || ch == '/' || ch <= ' ') && ch && scriptCodeSize >= searchStopperLen
...
void HTMLTokenizer::scriptHandler()
{
    ...
    if(effectiveScript)
    {
        ...
        if(!currentScriptSrc.isEmpty() && javascript && (cs = parser->doc()->docLoader()->requestScript(currentScriptSrc, scriptSrcCharset)))
        {
...
void HTMLTokenizer::scriptExecution(const QString &str, const QString &scriptURL, int baseLine)
{
...
void HTMLTokenizer::parseServer(TokenizerString &src)
{
    ...
    {
        scriptCode[scriptCodeSize++] = *src;
        if(src->unicode() == '>' && scriptCodeSize > 1 && scriptCode[scriptCodeSize - 2] == '%')
        {
            ++src;
            server = false;
            scriptCodeSize = 0;
        ...
        switch(Entity)
        {
            ...
            case EntityName:
            {
                ...
                while(ll--)
                {
                    ...
                    if(csrc.row() || !((cc >= 'a' && cc <= 'z') || (cc >= '0' && cc <= '9') || (cc >= 'A' && cc <= 'Z')))
                    {
                        Entity = SearchSemicolon;
                        break;
                    }
...
void HTMLTokenizer::parseTag(TokenizerString &src)
{
    ...
    while(!src.isEmpty())
    {
        ...
        while(l < src.length() && (src.toString()[l]).latin1() != '>')
            l++;
        qDebug("src is now: *%s*, tquote: %d", src.toString().left(l).latin1(), tquote);
#endif
        switch(tag)
        {
            case NoTag:
            ...
            case AttributeName:
            {
                ...
                while(ll--)
                {
                    ...
                    cBuffer[cBufferPos++] = (curchar >= 'A' && curchar <= 'Z') ? curchar | 0x20 : curchar;
                    ++src;
                }
            ...
            case QuotedValue:
            {
                ...
                while(!src.isEmpty())
                {
                    ...
                    if(curchar <= '\'' && !src.escaped())
                    {
                        ...
                        else if((tquote == SingleQuote && curchar == '\'') || (tquote == DoubleQuote && curchar == '\"'))
                        {
            ...
            case SearchEnd:
            {
                ...
                if(tagID < ID_CLOSE_TAG && tagID != ID_SCRIPT && DOM::endTag[tagID] == DOM::REQUIRED
                ...
                Mozilla 1.5 accepts application/x-javascript, WinIE 6 doesn't.
                        ...
                        if(type.compare("text/javascript") != 0 && type.compare("text/javascript1.0") != 0 && type.compare("text/javascript1.1") != 0
                           && type.compare("text/javascript1.2") != 0 && type.compare("text/javascript1.3") != 0
                           && type.compare("text/javascript1.4") != 0 && type.compare("text/javascript1.5") != 0 && type.compare("text/jscript") != 0
                           && type.compare("text/ecmascript") != 0 && type.compare("text/livescript") != 0
                           && type.compare("application/x-javascript") != 0 && type.compare("application/x-ecmascript") != 0
                           && type.compare("application/javascript") != 0 && type.compare("application/ecmascript") != 0)
                            javascript = false;
                    }
                    else if(a)
                    {
                        ...
                        lang = lang.lower();
                        if(lang.compare("") != 0 && lang.compare("javascript") != 0 && lang.compare("javascript1.0") != 0
                           && lang.compare("javascript1.1") != 0 && lang.compare("javascript1.2") != 0 && lang.compare("javascript1.3") != 0
                           && lang.compare("javascript1.4") != 0 && lang.compare("javascript1.5") != 0 && lang.compare("ecmascript") != 0
                           && lang.compare("livescript") != 0 && lang.compare("jscript"))
                            javascript = false;
                    }
                }
...
void HTMLTokenizer::addPending()
{
    ...
    else if(textarea)
    {
        switch(pending)
        {
            ...
            case TabPending:
                *dest++ = '\t';
                prePos += TAB_SIZE - (prePos % TAB_SIZE);
                break;
            case NonePending:
                assert(0);
        }
