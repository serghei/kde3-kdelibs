...
HTMLFormElementImpl::HTMLFormElementImpl(DocumentImpl *doc, bool implicit) : HTMLElementImpl(doc)
{
    m_implicit = implicit;
    m_post = false;
    m_multipart = false;
...
QByteArray HTMLFormElementImpl::formData(bool &ok)
{
        ...
        if(!current->disabled() && current->encoding(codec, lst, m_multipart))
        {
            ...
            for(it = lst.begin(); it != itEnd; ++it)
            {
                ...
                else
                {
                    ...
                    if(current->id() == ID_INPUT && static_cast< HTMLInputElementImpl * >(current)->inputType() == HTMLInputElementImpl::FILE
                       && current->renderer())
                    {
                        KURL path;
                        QString val = static_cast< HTMLInputElementImpl * >(current)->value().string().stripWhiteSpace();
                        if(!val.isEmpty() && QDir::isRelativePath(val) && QFile::exists(KGlobalSettings::documentPath() + val))
                        {
                    ...
                    if(current->id() == ID_INPUT && static_cast< HTMLInputElementImpl * >(current)->inputType() == HTMLInputElementImpl::TEXT)
    ...
    {
        const int result = KMessageBox::warningContinueCancelList(0, i18n("The following files will not be uploaded"
                                                                          " because they could not be found.\n"
                                                                          "Do you want to continue?"),
                                                                  fileNotUploads, i18n("Submit Confirmation"), KGuiItem(i18n("&Submit Anyway")));
    ...
    {
        const int result = KMessageBox::warningContinueCancelList(0, i18n("You're about to transfer the following files from "
                                                                          "your local computer to the Internet.\n"
                                                                          "Do you really want to continue?"),
                                                                  fileUploads, i18n("Send Confirmation"), KGuiItem(i18n("&Send Files")));
...
void HTMLFormElementImpl::doAutoFill()
{
    ...
    if(KWallet::Wallet::keyDoesNotExist(KWallet::Wallet::NetworkWallet(), KWallet::Wallet::FormDataFolder(), key))
        return;
...
void HTMLFormElementImpl::walletOpened(KWallet::Wallet *w)
{
    ...
    for(QPtrListIterator< HTMLGenericFormElementImpl > it(formElements); it.current(); ++it)
    {
        if(it.current()->id() == ID_INPUT)
        {
            ...
            if((current->inputType() == HTMLInputElementImpl::PASSWORD || current->inputType() == HTMLInputElementImpl::TEXT) && !current->readOnly()
    ...
    if(view && !view->nonPasswordStorableSite(formUrl.host()))
    {
        for(QPtrListIterator< HTMLGenericFormElementImpl > it(formElements); it.current(); ++it)
        {
            if(it.current()->id() == ID_INPUT)
            {
                ...
                if((c->inputType() == HTMLInputElementImpl::TEXT || c->inputType() == HTMLInputElementImpl::PASSWORD) && !c->readOnly())
                {
                    m_walletMap.insert(c->name().string(), c->value().string());
                    if(c->inputType() == HTMLInputElementImpl::PASSWORD && !c->value().isEmpty())
                        m_havePassword = true;
                }
            }
    ...
    if(ok && view)
    {
        ...
        if(m_havePassword && !m_haveTextarea && KWallet::Wallet::isEnabled())
        {
            ...
            if(doesnotexist || !w || login_changed)
            {
                ...
                KDialogBase *const dialog = new KDialogBase(i18n("Save Login Information"), KDialogBase::Yes | KDialogBase::No | KDialogBase::Cancel,
                                                            KDialogBase::Yes, KDialogBase::Cancel, 0, "questionYesNoCancel", true, true,
                                                            i18n("Store"), KGuiItem(i18n("Ne&ver for This Site")), i18n("Do Not Store"));

                bool checkboxResult = false;
                const int savePassword =
        ...
        if(m_post)
        {
            view->part()->submitForm("post", url.string(), form_data, m_target.string(), enctype().string(), m_boundary);
        }
        else
        {
            view->part()->submitForm("get", url.string(), form_data, m_target.string());
        }
    }
...
void HTMLFormElementImpl::parseAttribute(AttributeImpl *attr)
{
    switch(attr->id())
    {
        ...
        case ATTR_AUTOCOMPLETE:
            ...
            break;
        case ATTR_ONSUBMIT:
            setHTMLEventListener(EventImpl::SUBMIT_EVENT, getDocument()->createHTMLEventListener(attr->value().string(), "onsubmit", this));
            break;
        case ATTR_ONRESET:
            setHTMLEventListener(EventImpl::RESET_EVENT, getDocument()->createHTMLEventListener(attr->value().string(), "onreset", this));
            break;
        case ATTR_NAME:
...
void HTMLFormElementImpl::radioClicked(HTMLGenericFormElementImpl *caller)
{
    ...
    {
        HTMLGenericFormElementImpl *const current = it.current();
        if(current->id() == ID_INPUT && static_cast< HTMLInputElementImpl * >(current)->inputType() == HTMLInputElementImpl::RADIO
...
HTMLGenericFormElementImpl::HTMLGenericFormElementImpl(DocumentImpl *doc, HTMLFormElementImpl *f) : HTMLElementImpl(doc)
{
        ...
        if(!evt->defaultHandled() && m_render && m_render->isWidget())
        {
            ...
            if(evt->id() == EventImpl::KEYPRESS_EVENT && evt->isKeyRelatedEvent())
            {
                ...
                if(k && (k->key() == Qt::Key_Tab || k->key() == Qt::Key_BackTab))
                {
                    ...
                    if(widget)
                        static_cast< FocusHandleWidget * >(widget)->focusNextPrev(k->key() == Qt::Key_Tab);
...
HTMLButtonElementImpl::HTMLButtonElementImpl(DocumentImpl *doc, HTMLFormElementImpl *f) : HTMLGenericFormElementImpl(doc, f)
{
    m_clicked = false;
    m_type = SUBMIT;
    m_dirty = true;
...
HTMLFieldSetElementImpl::HTMLFieldSetElementImpl(DocumentImpl *doc, HTMLFormElementImpl *f) : HTMLGenericFormElementImpl(doc, f)
{
}
...
HTMLInputElementImpl::HTMLInputElementImpl(DocumentImpl *doc, HTMLFormElementImpl *f) : HTMLGenericFormElementImpl(doc, f)
{
    m_type = TEXT;
    m_maxLen = -1;
    m_size = 20;
...
void HTMLInputElementImpl::parseAttribute(AttributeImpl *attr)
{
    switch(attr->id())
    {
        ...
        case ATTR_VALUE:
            if(m_value.isNull())
            {                 // We only need to setChanged if the form is looking
        ...
        case ATTR_HEIGHT:
            ...
            break;
        case ATTR_ONSELECT:
            setHTMLEventListener(EventImpl::SELECT_EVENT, getDocument()->createHTMLEventListener(attr->value().string(), "onselect", this));
            break;
        case ATTR_ONCHANGE:
            setHTMLEventListener(EventImpl::CHANGE_EVENT, getDocument()->createHTMLEventListener(attr->value().string(), "onchange", this));
            break;
        default:
            HTMLGenericFormElementImpl::parseAttribute(attr);
    }
...
void HTMLInputElementImpl::attach()
{
    ...
    if(parentNode()->renderer() && _style->display() != NONE)
    {
        switch(m_type)
        {
            ...
            case PASSWORD:
            case ISINDEX:
                m_render = new(getDocument()->renderArena()) RenderLineEdit(this);
                break;
            case CHECKBOX:
                m_render = new(getDocument()->renderArena()) RenderCheckBox(this);
                break;
            case RADIO:
                m_render = new(getDocument()->renderArena()) RenderRadioButton(this);
                break;
            case SUBMIT:
                m_render = new(getDocument()->renderArena()) RenderSubmitButton(this);
                break;
            case IMAGE:
                m_render = new(getDocument()->renderArena()) RenderImageButton(this);
                break;
            case RESET:
                m_render = new(getDocument()->renderArena()) RenderResetButton(this);
                break;
            case FILE:
                m_render = new(getDocument()->renderArena()) RenderFileButton(this);
                break;
            case BUTTON:
                m_render = new(getDocument()->renderArena()) RenderPushButton(this);
            case HIDDEN:
                break;
        }
    }
        ...
        case FILE: // hmm, we have the type FILE also.  bad choice here...
        {
            ...
            if(multipart)
            {
                ...
                if(KIO::NetAccess::stat(fileurl, filestat, toplevel))
                {
                    ...
                    if(fileitem.isFile() && KIO::NetAccess::download(fileurl, local, toplevel))
                    {
                        QFile file(local);
...
void HTMLInputElementImpl::defaultEventHandler(EventImpl *evt)
{
    if(!m_disabled)
    {
        ...
        if(evt->isMouseEvent())
        {
            ...
            if((m_type == RADIO || m_type == CHECKBOX) && me->id() == EventImpl::MOUSEUP_EVENT && me->detail() > 0)
            {
                // click will follow
        ...
        {
            bool check = false;
            if(active() && (evt->id() == EventImpl::KEYUP_EVENT || evt->id() == EventImpl::KEYPRESS_EVENT))
            {
...
HTMLLabelElementImpl::HTMLLabelElementImpl(DocumentImpl *doc) : HTMLGenericFormElementImpl(doc)
{
}
...
void HTMLLabelElementImpl::defaultEventHandler(EventImpl *evt)
{
    if(!m_disabled)
    {
        ...
        else if(evt->isKeyRelatedEvent() && (evt->id() == EventImpl::KEYUP_EVENT || evt->id() == EventImpl::KEYPRESS_EVENT))
        {
            QKeyEvent *const ke = static_cast< KeyEventBaseImpl * >(evt)->qKeyEvent();
            if(ke && active() && (ke->key() == Qt::Key_Return || ke->key() == Qt::Key_Enter || ke->key() == Qt::Key_Space))
                act = true;
...
HTMLLegendElementImpl::HTMLLegendElementImpl(DocumentImpl *doc, HTMLFormElementImpl *f) : HTMLGenericFormElementImpl(doc, f)
{
}
...
HTMLSelectElementImpl::HTMLSelectElementImpl(DocumentImpl *doc, HTMLFormElementImpl *f) : HTMLGenericFormElementImpl(doc, f)
{
    m_multiple = false;
...
bool HTMLSelectElementImpl::encoding(const QTextCodec *codec, khtml::encodingList &encoded_values, bool)
{
    ...
    if(!successful && !m_multiple && m_size <= 1 && itemsSize && (items[0]->id() == ID_OPTION))
    {
...
int HTMLSelectElementImpl::optionToListIndex(int optionIndex) const
{
    ...
    int listIndex = 0;
    int optionIndex2 = 0;
    for(; optionIndex2 < itemsSize && optionIndex2 <= optionIndex; ++listIndex)
    { // not a typo!
        if(items[listIndex]->id() == ID_OPTION)
            ++optionIndex2;
    }
...
{
    QMemArray< HTMLGenericFormElementImpl * > items = listItems();
    if(listIndex < 0 || listIndex >= int(items.size()) || items[listIndex]->id() != ID_OPTION)
        return -1;
...
HTMLKeygenElementImpl::HTMLKeygenElementImpl(DocumentImpl *doc, HTMLFormElementImpl *f) : HTMLSelectElementImpl(doc, f)
{
...
HTMLOptionElementImpl::HTMLOptionElementImpl(DocumentImpl *doc, HTMLFormElementImpl *f) : HTMLGenericFormElementImpl(doc, f)
{
    m_selected = false;
}
...
HTMLTextAreaElementImpl::HTMLTextAreaElementImpl(DocumentImpl *doc, HTMLFormElementImpl *f) : HTMLGenericFormElementImpl(doc, f)
{
...
HTMLIsIndexElementImpl::HTMLIsIndexElementImpl(DocumentImpl *doc, HTMLFormElementImpl *f) : HTMLInputElementImpl(doc, f)
{
    m_type = TEXT;
    setName("isindex");
}
