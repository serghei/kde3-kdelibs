...
RenderCanvas::RenderCanvas(DOM::NodeImpl *node, KHTMLView *view) : RenderBlock(node)
{
...
void RenderCanvas::updateDocumentSize()
{
    ...
    if(!m_pagedMode && m_view)
    {
        ...
        if(!vss && m_width - m_view->verticalScrollBar()->sizeHint().width() == s.width() && m_cachedDocWidth <= m_width)
            hDocW = kMin(m_cachedDocWidth, s.width());

        if(!hss && m_height - m_view->horizontalScrollBar()->sizeHint().height() == s.height() && m_cachedDocHeight <= m_height)
...
void RenderCanvas::updateDocSizeAfterLayerTranslation(RenderObject *o, bool posXOffset, bool posYOffset)
{
    ...
    //    kdDebug() << " posXOffset: " << posXOffset << " posYOffset " << posYOffset << " m_cachedDocWidth  " <<  m_cachedDocWidth << "
    //    m_cachedDocHeight  " << m_cachedDocHeight << endl;
    updateDocumentSize();
}
...
void RenderCanvas::repaint(Priority p)
{
    if(m_view && !m_staticMode)
    {
        if(p == RealtimePriority)
        {
            ...
            m_view->updateContents(m_view->contentsX(), m_view->contentsY(), m_view->visibleWidth(), m_view->visibleHeight() /*, true*/);
        }
        else if(p == HighPriority)
            m_view->scheduleRepaint(m_view->contentsX(), m_view->contentsY(), m_view->visibleWidth(), m_view->visibleHeight(), true /*asap*/);
        else
            m_view->scheduleRepaint(m_view->contentsX(), m_view->contentsY(), m_view->visibleWidth(), m_view->visibleHeight());
    }
}
...
void RenderCanvas::setSelection(RenderObject *s, int sp, RenderObject *e, int ep)
{
    ...
    if(oldStart != m_selectionStart || (oldStart == oldEnd && (oldStartPos != m_selectionStartPos || oldEndPos != m_selectionEndPos))
    ...
    if(/*(oldStart != oldEnd || !oldEnd) &&*/
       (oldEnd != m_selectionEnd || (oldEnd == m_selectionEnd && oldEndPos != m_selectionEndPos)))
    {
...
QRect RenderCanvas::viewRect() const
{
    if(m_pagedMode)
        ...
        return QRect(m_view->contentsX(), m_view->contentsY(), m_view->visibleWidth(), m_view->visibleHeight());
    else
...
int RenderCanvas::docHeight() const
{
    ...
    if(fc)
    {
        ...
        // kdDebug(6040) << "h " << h << " lowestPos " << lowestPos << " dh " << dh << " fc->rh " << fc->effectiveHeight() << " fc->height() " <<
        // fc->height() << endl;
        if(lowestPos > dh)
            dh = lowestPos;
    ...
    // kdDebug(6040) << "h " << h << " layer(" << layer->renderer()->renderName() << "@" << layer->renderer() << ")->height " << layer->height() << "
    // lp " << (layer->yPos() + layer->height()) << " height() " << layer->renderer()->height() << " rh " << layer->renderer()->effectiveHeight() <<
    // endl;
    return h;
}
...
int RenderCanvas::docWidth() const
{
    ...
    if(fc)
    {
        ...
        // kdDebug(6040) << "w " << w << " rightmostPos " << rightmostPos << " dw " << dw << " fc->rw " << fc->effectiveWidth() << " fc->width() " <<
        // fc->width() << endl;
        if(rightmostPos > dw)
            dw = rightmostPos;
    ...
    // kdDebug(6040) << "w " << w << " layer(" << layer->renderer()->renderName() << ")->width " << layer->width() << " rm " << (layer->xPos() +
    // layer->width()) << " width() " << layer->renderer()->width() << " rw " << layer->renderer()->effectiveWidth() << endl;
    return w;
}
