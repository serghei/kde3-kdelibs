...
QRegion RenderLayer::paintedRegion(RenderLayer *rootLayer)
{
    ...
    if(s->visibility() == VISIBLE)
    {
        ...
        if(s->backgroundImage() || s->backgroundColor().isValid() || s->hasBorder() || renderer()->scrollsOverflow() || renderer()->isReplaced())
        {
            r += cr;
        }
        else
...
void RenderLayer::insertOnlyThisLayer()
{
    if(!m_parent && renderer()->parent())
    {
        ...
        if(parentLayer)
            parentLayer->addChild(this, renderer()->parent()->findNextLayer(parentLayer, renderer()));
    }
...
void RenderLayer::positionScrollbars(const QRect &absBounds)
{
    ...
    if(m_hBar)
    {
        ...
        m_hBar->resize(absBounds.width() - (m_object->borderLeft() + m_object->borderRight()) - (m_vBar ? m_vBar->width() - 1 : 0), m_hBar->height());
    }
#else
    int tx = absBounds.x();
    int ty = absBounds.y();
...
void RenderLayer::paintLayer(RenderLayer *rootLayer, QPainter *p, const QRect &paintDirtyRect, bool selectionOnly)
{
    ...
    if(shouldPaint && !selectionOnly)
    {
        ...
        if(!damageRect.isEmpty())
        {
            ...
            renderer()->paint(paintInfo, x - renderer()->xPos(), y - renderer()->yPos() + renderer()->borderTopExtra());
...
RenderLayer *RenderLayer::nodeAtPointForLayer(RenderLayer *rootLayer, RenderObject::NodeInfo &info, int xMousePos, int yMousePos,
                                              const QRect &hitTestRect)
{
    // Calculate the clip rects we should use.
    ...
    if(containsPoint(xMousePos, yMousePos, fgRect)
       && renderer()->nodeAtPoint(info, xMousePos, yMousePos, layerBounds.x() - renderer()->xPos(),
                                  layerBounds.y() - renderer()->yPos() + m_object->borderTopExtra(), HitTestChildrenOnly))
    {
...
void RenderLayer::calculateClipRects(const RenderLayer *rootLayer, QRect &overflowClipRect, QRect &posClipRect, QRect &fixedClipRect)
{
    if(parent())
...
void RenderLayer::calculateRects(const RenderLayer *rootLayer, const QRect &paintDirtyRect, QRect &layerBounds, QRect &backgroundRect,
                                 QRect &foregroundRect)
{
    ...
    backgroundRect = m_object->style()->position() == FIXED ? fixedClipRect : (m_object->isPositioned() ? posClipRect : overflowClipRect);
...
bool RenderLayer::intersectsDamageRect(const QRect &layerBounds, const QRect &damageRect) const
{
    return (renderer()->isCanvas() || renderer()->isRoot() || renderer()->isBody()
            || (renderer()->hasOverhangingFloats() && !renderer()->hasOverflowClip()) || (renderer()->isInline() && !renderer()->isReplaced())
            || layerBounds.intersects(damageRect));
}

bool RenderLayer::containsPoint(int x, int y, const QRect &damageRect) const
{
    return (renderer()->isCanvas() || renderer()->isRoot() || renderer()->isBody() || renderer()->hasOverhangingFloats()
            || (renderer()->isInline() && !renderer()->isReplaced()) || damageRect.contains(x, y));
}
...
// sorted already).
static void sortByZOrder(QPtrVector< RenderLayer > *buffer, QPtrVector< RenderLayer > *mergeBuffer, uint start, uint end)
{
    if(start >= end)
        return; // Sanity check.
...
static void write(QTextStream &ts, const RenderLayer &l, const QRect &layerBounds, const QRect &backgroundClipRect, const QRect &clipRect,
...
static void writeLayers(QTextStream &ts, const RenderLayer *rootLayer, RenderLayer *l, const QRect &paintDirtyRect, const QString &indent)
{
    // Calculate the clip rects we should use.
    QRect layerBounds, damageRect, clipRectToApply;
...
{
    return renderer()->style() && renderer()->hasOverflowClip() && !renderer()->isPositioned() && !renderer()->isRelPositioned();
    /* && !isTransparent(); */
}
...
void RenderLayer::styleChanged()
{
    ...
    if(m_object->hasOverflowClip() && m_object->style()->overflowX() == OMARQUEE && m_object->style()->marqueeBehavior() != MNONE)
    {
        if(!m_marquee)
...
int Marquee::computePosition(EMarqueeDirection dir, bool stopAtContentEdge)
{
    ...
    {
        int contentHeight =
            m_layer->renderer()->lowestPosition(true, false) - m_layer->renderer()->borderTop() + m_layer->renderer()->paddingBottom();
...
void Marquee::timerEvent(QTimerEvent * /*evt*/)
{
    ...
    else
    {
        ...
        int clientSize = isUnfurlMarquee() ? abs(range) : (isHorizontal() ? m_layer->renderer()->clientWidth() : m_layer->renderer()->clientHeight());
        int increment = kMax(1, abs(m_layer->renderer()->style()->marqueeIncrement().width(clientSize)));
        int currentPos = isUnfurlMarquee() ? m_unfurlPos : (isHorizontal() ? m_layer->scrollXOffset() : m_layer->scrollYOffset());
