...
RenderFrameSet::RenderFrameSet(HTMLFrameSetElementImpl *frameSet) : RenderBox(frameSet)
{
...
#ifdef DEBUG_LAYOUT
            kdDebug(6040) << "child frame at (" << xPos << "/" << yPos << ") size (" << m_gridLayout[1][c] << "/" << m_gridLayout[0][r] << ")"
                          << endl;
#endif
...
bool RenderFrameSet::userResize(MouseEventImpl *evt)
{
    ...
    if((m_resizing || evt->id() == EventImpl::MOUSEUP_EVENT) && view)
    {
        ...
        {
            if(m_oldpos >= 0)
                paint.drawRect(m_oldpos + sw / 2 - rBord, r.y(), 2 * rBord, r.height());
            if(p >= 0)
        ...
        {
            if(m_oldpos >= 0)
                paint.drawRect(r.x(), m_oldpos + sw / 2 - rBord, r.width(), 2 * rBord);
            if(p >= 0)
...
RenderPart::RenderPart(DOM::HTMLElementImpl *node) : RenderWidget(node)
{
    // init RenderObject attributes
    setInline(false);
}
...
RenderFrame::RenderFrame(DOM::HTMLFrameElementImpl *frame) : RenderPart(frame)
{
    setInline(false);
}
...
RenderPartObject::RenderPartObject(DOM::HTMLElementImpl *element) : RenderPart(element)
{
    // init RenderObject attributes
    setInline(true);
}
...
void RenderPartObject::updateWidget()
{
    ...
    if(element()->id() == ID_IFRAME)
    {
        ...
        url = o->url.string();
        if(!o->getDocument()->isURLAllowed(url))
            return;
    ...
    else
    {
        ...
        for(NodeImpl *child = element()->firstChild(); child; child = child->nextSibling())
        {
            if(child->id() == ID_PARAM)
            {
                ...
                else if(url.isEmpty() && (name_lower == QString::fromLatin1("src") || name_lower == QString::fromLatin1("movie")
        ...
        { // if(element()->id() == ID_OBJECT || element()->id() == ID_APPLET)
            ...
            if((serviceType.isEmpty() || serviceType == "application/x-oleobject") && !objbase->classId.isEmpty())
            {
#if 0
        ...
        if((url.isEmpty() && !embed && (serviceType.isEmpty() || classId.isEmpty())) || !document()->isURLAllowed(url)
...
bool RenderPartObject::partLoadingErrorNotify(khtml::ChildFrame *childFrame, const KURL &url, const QString &serviceType)
{
    ...
    if(serviceType != "application/x-activex-handler" && element()->id() == ID_OBJECT)
    {
        ...
        if(embed && !o->classId.isEmpty() && !(static_cast< ElementImpl * >(o)->getAttribute(ATTR_CODEBASE).string()).isEmpty())
        {
            KParts::URLArgs args;
...
void RenderPartObject::slotPartLoadingErrorNotify()
{
    ...
    if(element() && (element()->id() == ID_OBJECT || element()->id() == ID_EMBED || element()->id() == ID_APPLET))
