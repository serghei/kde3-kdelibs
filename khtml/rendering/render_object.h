...
typedef enum { HitTestAll = 0, HitTestSelfOnly = 1, HitTestChildrenOnly = 2 } HitTestAction;
...
typedef enum { LowPriority = 0, NormalPriority = 1, HighPriority = 2, RealtimePriority = 3 } Priority;
...
public:
    ...
    {
        return -1;
    } // Tables and blocks implement this.
...
public:
    ...
    {
        return style()->display() == COMPACT;
    } // compact
    bool isRunIn() const
    {
        return style()->display() == RUN_IN;
    } // run-in object
    ...
    struct PaintInfo
    {
        PaintInfo(QPainter *_p, const QRect &_r, PaintAction _phase) : p(_p), r(_r), phase(_phase), outlineObjects(0)
        {
        }
        ~PaintInfo()
        {
    ...
    virtual void paintBackgroundExtended(QPainter * /*p*/, const QColor & /*c*/, const BackgroundLayer * /*bgLayer*/, int /*clipy*/, int /*cliph*/,
                                         int /*_tx*/, int /*_ty*/, int /*w*/, int /*height*/, int /*bleft*/, int /*bright*/, int /*pleft*/,
                                         int /*pright*/)
    {
    }
    ...
    public:
        NodeInfo(bool readonly, bool active) : m_innerNode(0), m_innerNonSharedNode(0), m_innerURLElement(0), m_readonly(readonly), m_active(active)
        {
        }
    ...
    virtual FindSelectionResult checkSelectionPoint(int _x, int _y, int _tx, int _ty, DOM::NodeImpl *&, int &offset, SelPointState &);
    ...
    {
        return yPos() + (hasOverflowClip() ? -borderTopExtra() : kMin(overflowTop(), -borderTopExtra()));
    }
    ...
    void drawBorder(QPainter *p, int x1, int y1, int x2, int y2, BorderSide s, QColor c, const QColor &textcolor, EBorderStyle style, int adjbw1,
                    int adjbw2, bool invalidisInvert = false);
