...
FixedTableLayout::FixedTableLayout(RenderTable *table) : TableLayout(table)
{
}
...
#ifdef DEBUG_LAYOUT
                qDebug("    col element: effCol=%d, span=%d: %d w=%d type=%d", cCol, span, effWidth, w.value(), w.type());
#endif
                int usedSpan = 0;
                int i = 0;
                while(usedSpan < span)
...
#ifdef DEBUG_LAYOUT
                        qDebug("    setting effCol %d (span=%d) to width %d(type=%d)", cCol + i, eSpan, width[cCol + i].value(),
                               width[cCol + i].type());
#endif
                    }
                    usedSpan += eSpan;
...
AutoTableLayout::AutoTableLayout(RenderTable *table) : TableLayout(table)
{
                    ...
                    switch(w.type())
                    {
                        case Fixed:
                            ...
                            if(w.value() > 0 && !l.width.isPercent())
                            {
                                ...
                                {
                                    // Nav/IE weirdness
                                    if((wval > l.width.value()) || ((l.width.value() == wval) && (maxContributor == cell)))
                                    {
...
#ifdef DEBUG_LAYOUT
                qDebug("    col element %d (eff=%d): Length=%d(%d), span=%d, effColSpan=%d", cCol, cEffCol, w.value(), w.type(), span,
                       table->spanOfEffCol(cEffCol));
#endif
...
static bool shouldScaleColumns(RenderTable *table)
{
    ...
    while(table)
    {
        ...
        if((tw.isVariable() || tw.isPercent()) && !table->isPositioned())
        {
            ...
            while(cb && !cb->isCanvas() && !cb->isTableCell() && cb->style()->width().isVariable() && !cb->isPositioned())
                cb = cb->containingBlock();

            table = 0;
            if(cb && cb->isTableCell() && (cb->style()->width().isVariable() || cb->style()->width().isPercent()))
            {
                if(tw.isPercent())
                    scale = false;
                else
...
int AutoTableLayout::calcEffectiveWidth()
{
    ...
    for(unsigned int i = 0; i < spanCells.size(); i++)
    {
...
#ifdef DEBUG_LAYOUT
        qDebug("    colspan cell %p at effCol %d, span %d, type %d, value %d cmin=%d min=%d fixedwidth=%d", cell, col, cSpan, w.type(), w.value(),
               cMinWidth, minWidth, fixedWidth);
#endif
...
#ifdef DEBUG_LAYOUT
                        qDebug("   col %d: setting percent value %d effMaxWidth=%d totalWidth=%d", pos, percent, layoutStruct[pos].effMaxWidth,
                               totalWidth);
#endif
                        totalWidth -= layoutStruct[pos].effMaxWidth;
        ...
        if(cMinWidth > minWidth)
        {
            ...
            {
#ifdef DEBUG_LAYOUT
                qDebug("extending minWidth of cols %d-%d to %dpx currentMin=%d accroding to fixed sum %d", col, lastCol - 1, cMinWidth, minWidth,
                       fixedWidth);
#endif
...
void AutoTableLayout::layout()
{
    ...
    for(int i = 0; i < nEffCols; i++)
    {
        qDebug("    effcol %d is of type %d value %d, minWidth=%d, maxWidth=%d", i, layoutStruct[i].width.type(), layoutStruct[i].width.value(),
               layoutStruct[i].minWidth, layoutStruct[i].maxWidth);
        qDebug("        effective: type %d value %d, minWidth=%d, maxWidth=%d", layoutStruct[i].effWidth.type(), layoutStruct[i].effWidth.value(),
