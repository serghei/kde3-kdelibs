...
RenderTable::RenderTable(DOM::NodeImpl *node) : RenderBlock(node)
{
...
{
#ifdef DEBUG_LAYOUT
    kdDebug(6040) << renderName() << "(Table)::addChild( " << child->renderName() << ", " << (beforeChild ? beforeChild->renderName() : "0") << " )"
                  << endl;
#endif
...
#ifdef DEBUG_LAYOUT
    kdDebug(6040) << renderName() << "(Table)::layout1() width=" << width() << ", marginLeft=" << marginLeft() << " marginRight=" << marginRight()
                  << endl;
#endif
...
void RenderTable::paint(PaintInfo &pI, int _tx, int _ty)
{
    ...
    if((pI.phase == PaintActionElementBackground || pI.phase == PaintActionChildBackground) && shouldPaintBackgroundOrBorder()
    ...
    if(collapseBorders() && (pI.phase == PaintActionElementBackground || pI.phase == PaintActionChildBackground) && style()->visibility() == VISIBLE)
    {
...
RenderTableSection *RenderTable::sectionAbove(const RenderTableSection *section, bool skipEmptySections) const
{
    ...
    while(prevSection)
    {
        if(prevSection->isTableSection() && prevSection != head && prevSection != foot
           && (!skipEmptySections || static_cast< RenderTableSection * >(prevSection)->numRows()))
            break;
...
RenderTableSection *RenderTable::sectionBelow(const RenderTableSection *section, bool skipEmptySections) const
{
    ...
    while(nextSection)
    {
        if(nextSection->isTableSection() && nextSection != head && nextSection != foot
           && (!skipEmptySections || static_cast< RenderTableSection * >(nextSection)->numRows()))
            break;
        ...
        switch(pos)
        {
            ...
            case SelectionPointBefore:
                ...
                else
                {
                    node = nod;
                    offset = off;
                    // kdDebug(6030) << "RenderTable::checkSelectionPoint " << this << " before us -> returning SelectionPointBefore " << node << "/"
                    // << offset << endl;
            ...
            case SelectionPointAfterInLine:
                ...
                // kdDebug(6030) << "RenderTable::checkSelectionPoint: selection after: " << nod << " offset: " << off << " afterInLine: " <<
...
RenderTableSection::RenderTableSection(DOM::NodeImpl *node) : RenderBox(node)
{
    // init RenderObject attributes
...
void RenderTableSection::addChild(RenderObject *child, RenderObject *beforeChild)
{
    ...
    kdDebug(6040) << renderName() << "(TableSection)::addChild( " << child->renderName()
                  << ", beforeChild=" << (beforeChild ? beforeChild->renderName() : "0") << " )" << endl;
#endif
    if(!child->isTableRow())
    {
        // TBODY > FORM quirk (???)
...
void RenderTableSection::addCell(RenderTableCell *cell, RenderTableRow *row)
{
    ...
    if(rSpan == 1)
    {
        ...
        if(height.value() > 0 || (height.isRelative() && height.value() >= 0))
        {
            ...
            {
                case Percent:
                    if(!cRowHeight.isPercent() || (cRowHeight.isPercent() && cRowHeight.value() < height.value()))
                        grid[cRow].height = height;
                    break;
                case Fixed:
                    if(cRowHeight.type() < Percent || (cRowHeight.isFixed() && cRowHeight.value() < height.value()))
                        grid[cRow].height = height;
                    break;
                case Relative:
#if 0
...
#ifdef DEBUG_LAYOUT
            kdDebug(6040) << "setting width of cell " << cell << " " << cell->row() << "/" << cell->col() << " to " << w
                          << " colspan=" << cell->colSpan() << " start=" << j << " end=" << endCol << endl;
#endif
...
int RenderTableSection::layoutRows(int toAdd)
{
    ...
    for(int r = 0; r < totalRows; r++)
    {
        ...
        for(int c = 0; c < nEffCols; c++)
        {
            ...
            {
                cell->setCellPercentageHeight(
                    kMax(0, rHeight - cell->borderTop() - cell->paddingTop() - cell->borderBottom() - cell->paddingBottom()));
                cell->layoutIfNeeded();
            }
            {
#ifdef DEBUG_LAYOUT
                kdDebug(6040) << "setting position " << r << "/" << c << ": " << table()->columnPos[c] /*+ padding */ << "/" << rowPos[rindx]
                              << " height=" << rHeight << endl;
#endif
                ...
                te = kMax(0, te);
#ifdef DEBUG_LAYOUT
//            kdDebug( 6040 ) << "CELL " << cell << " te=" << te << ", be=" << rHeight - cell->height() - te << ", rHeight=" << rHeight << ", valign="
//            << va << endl;
#endif
            ...
            {
                cell->setPos(table()->columnPos[(int)totalCols] - table()->columnPos[table()->colToEffCol(cell->col() + cell->colSpan())]
                                 + leftOffset,
                             rowPos[rindx]);
            }
            else
...
static unsigned int findRow(unsigned int first_row, unsigned int last_row, const QMemArray< int > &rowPos, int y)
{
...
static void findRowCover(unsigned int &startrow, unsigned int &endrow, const QMemArray< int > &rowPos, int min_y, int max_y)
{
    assert(max_y >= min_y);
    ...
    for(unsigned int r = 0; r < grid.size(); r++)
    {
        for(int c = 0; c < table()->numEffCols(); c++)
        {
            if(cellAt(r, c) && cellAt(r, c) != (RenderTableCell *)-1)
                stream << " (" << cellAt(r, c)->row() << "," << cellAt(r, c)->col() << "," << cellAt(r, c)->rowSpan() << ","
                       << cellAt(r, c)->colSpan() << ") ";
            else
                stream << " null cell";
        }
...
RenderTableRow::RenderTableRow(DOM::NodeImpl *node) : RenderContainer(node)
{
    // init RenderObject attributes
    setInline(false); // our object is not Inline
}
...
RenderTableCell::RenderTableCell(DOM::NodeImpl *_node) : RenderBlock(_node)
{
    _col = -1;
    _row = -1;
    updateFromElement();
...
void RenderTableCell::calcMinMaxWidth()
{
    ...
    if(element() && style()->whiteSpace() == NORMAL)
    {
        ...
        if(!nowrap.isNull() && w.isFixed() && m_minWidth < w.value())
...
bool RenderTableCell::nodeAtPoint(NodeInfo &info, int _x, int _y, int _tx, int _ty, HitTestAction hitTestAction, bool inside)
{
    ...
    inside |= style()->visibility() != HIDDEN && (_y >= ty) && (_y < ty + height() + _topExtra + _bottomExtra) && (_x >= tx) && (_x < tx + width());
...
void RenderTableCell::paint(PaintInfo &pI, int _tx, int _ty)
{
...
#ifdef TABLE_PRINT
    kdDebug(6040) << renderName() << "(RenderTableCell)::paint() w/h = (" << width() << "/" << height() << ")"
                  << " _y/_h=" << pI.r.y() << "/" << pI.r.height() << endl;
#endif
    ...
    if(!overhangingContents() && ((_ty >= pI.r.y() + pI.r.height() + os) || (_ty + _topExtra + m_height + _bottomExtra <= pI.r.y() - os)))
        return;
...
public:
    ...
    void addBorder(const CollapsedBorderValue &b, RenderObject::BorderSide s, bool paint, int _x1, int _y1, int _x2, int _y2, EBorderStyle _style)
    {
        if(b.exists() && paint)
        {
...
void RenderTableCell::paintCollapsedBorder(QPainter *p, int _tx, int _ty, int w, int h)
{
    ...
    for(CollapsedBorder *border = borders.nextBorder(); border; border = borders.nextBorder())
    {
        if(border->border == *table()->currentBorderStyle())
            drawBorder(p, border->x1, border->y1, border->x2, border->y2, border->side, border->border.color(), style()->color(), border->style, 0,
                       0);
    }
}
...
RenderTableCol::RenderTableCol(DOM::NodeImpl *node) : RenderContainer(node), m_span(1)
{
    // init RenderObject attributes
