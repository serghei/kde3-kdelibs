...
RenderFormElement::RenderFormElement(HTMLGenericFormElementImpl *element) : RenderWidget(element)
{
...
RenderButton::RenderButton(HTMLGenericFormElementImpl *element) : RenderFormElement(element)
{
}
...
RenderCheckBox::RenderCheckBox(HTMLInputElementImpl *element) : RenderButton(element)
{
...
void RenderCheckBox::calcMinMaxWidth()
{
    ...
    QSize s(cb->style().pixelMetric(QStyle::PM_IndicatorWidth), cb->style().pixelMetric(QStyle::PM_IndicatorHeight));
...
RenderRadioButton::RenderRadioButton(HTMLInputElementImpl *element) : RenderButton(element)
{
...
void RenderRadioButton::calcMinMaxWidth()
{
    ...
    QSize s(rb->style().pixelMetric(QStyle::PM_ExclusiveIndicatorWidth), rb->style().pixelMetric(QStyle::PM_ExclusiveIndicatorHeight));
    setIntrinsicWidth(s.width());
    setIntrinsicHeight(s.height());
...
RenderSubmitButton::RenderSubmitButton(HTMLInputElementImpl *element) : RenderButton(element)
{
...
void RenderSubmitButton::calcMinMaxWidth()
{
    ...
    QSize s(pb->style().sizeFromContents(QStyle::CT_PushButton, pb, ts).expandedTo(QApplication::globalStrut()));
    int margin = pb->style().pixelMetric(QStyle::PM_ButtonMargin, pb) + pb->style().pixelMetric(QStyle::PM_DefaultFrameWidth, pb) * 2;
...
RenderResetButton::RenderResetButton(HTMLInputElementImpl *element) : RenderSubmitButton(element)
{
}
...
void LineEditWidget::slotCheckSpelling()
{
    ...
    connect(m_spell, SIGNAL(misspelling(const QString &, const QStringList &, unsigned int)), this,
            SLOT(spellCheckerMisspelling(const QString &, const QStringList &, unsigned int)));
    connect(m_spell, SIGNAL(corrected(const QString &, const QString &, unsigned int)), this,
...
QPopupMenu *LineEditWidget::createPopupMenu()
{
    ...
    connect(popup, SIGNAL(activated(int)), this, SLOT(extendedMenuActivated(int)));
...
RenderLineEdit::RenderLineEdit(HTMLInputElementImpl *element) : RenderFormElement(element)
{
...
void RenderLineEdit::calcMinMaxWidth()
{
    ...
    s = QSize(w + 2 + 2 * widget()->frameWidth(), kMax(h, 14) + 2 + 2 * widget()->frameWidth()).expandedTo(QApplication::globalStrut());
...
RenderFieldset::RenderFieldset(HTMLGenericFormElementImpl *element) : RenderBlock(element)
{
}
...
RenderObject *RenderFieldset::findLegend()
{
    ...
    {
        if(!legend->isFloatingOrPositioned() && legend->element() && legend->element()->id() == ID_LEGEND)
            return legend;
    }
    return 0;
}
...
void RenderFieldset::paintBorderMinusLegend(QPainter *p, int _tx, int _ty, int w, int h, const RenderStyle *style, int lx, int lw)
{
    ...
    if(render_t)
    {
        ...
        drawBorder(p, _tx + lx + lw, _ty, _tx + w, _ty + style->borderTopWidth(), BSTop, tc, style->color(), ts, 0,
    ...
    if(render_l)
    {
        ...
        bool ignore_top = (tc == lc) && (ls >= OUTSET) && (ts == DOTTED || ts == DASHED || ts == SOLID || ts == OUTSET);

        bool ignore_bottom = (bc == lc) && (ls >= OUTSET) && (bs == DOTTED || bs == DASHED || bs == SOLID || bs == INSET);
    ...
    if(render_r)
    {
        ...
        bool ignore_top = (tc == rc) && (rs >= DOTTED || rs == INSET) && (ts == DOTTED || ts == DASHED || ts == SOLID || ts == OUTSET);

        bool ignore_bottom = (bc == rc) && (rs >= DOTTED || rs == INSET) && (bs == DOTTED || bs == DASHED || bs == SOLID || bs == INSET);
...
RenderFileButton::RenderFileButton(HTMLInputElementImpl *element) : RenderFormElement(element)
{
...
void RenderFileButton::calcMinMaxWidth()
{
    ...
    QSize s = edit->style()
                  .sizeFromContents(QStyle::CT_LineEdit, edit, QSize(w + 2 + 2 * edit->frameWidth(), kMax(h, 14) + 2 + 2 * edit->frameWidth()))
...
RenderLabel::RenderLabel(HTMLGenericFormElementImpl *element) : RenderFormElement(element)
{
}

// -------------------------------------------------------------------------

RenderLegend::RenderLegend(HTMLGenericFormElementImpl *element) : RenderBlock(element)
{
}

// -------------------------------------------------------------------------------

ComboBoxWidget::ComboBoxWidget(QWidget *parent) : KComboBox(false, parent, "__khtml")
{
    setAutoMask(true);
    if(listBox())
...
RenderSelect::RenderSelect(HTMLSelectElementImpl *element) : RenderFormElement(element)
{
    ...
    if(m_optionsChanged)
    {
        ...
        for(listIndex = 0; listIndex < int(listItems.size()); listIndex++)
        {
            if(listItems[listIndex]->id() == ID_OPTGROUP)
            {
                ...
                {
                    static_cast< KComboBox * >(m_widget)->insertItem(QString(text.implementation()->s, text.implementation()->l), listIndex);
...
void RenderSelect::slotSelectionChanged() // emitted by the listbox only
{
    ...
    for(unsigned i = 0; i < listItems.count(); i++)
        ...
        if(listItems[i]->id() == ID_OPTION)
            static_cast< HTMLOptionElementImpl * >(listItems[i])->m_selected = static_cast< KListBox * >(m_widget)->isSelected(i);
...
void RenderSelect::updateSelection()
{
    ...
    if(m_useListBox)
    {
        ...
        for(i = 0; i < int(listItems.size()); i++)
            listBox->setSelected(i, listItems[i]->id() == ID_OPTION && static_cast< HTMLOptionElementImpl * >(listItems[i])->selected());
    }
    else
    {
        bool found = false;
...
TextAreaWidget::TextAreaWidget(int wrap, QWidget *parent) : KTextEdit(parent, "__khtml"), m_findDlg(0), m_find(0), m_repDlg(0), m_replace(0)
{
...
void TextAreaWidget::slotDoReplace()
{
    ...
    // of found text.
    connect(m_replace, SIGNAL(highlight(const QString &, int, int)), this, SLOT(slotFindHighlight(const QString &, int, int)));
    connect(m_replace, SIGNAL(findNext()), this, SLOT(slotReplaceNext()));
    connect(m_replace, SIGNAL(replace(const QString &, int, int, int)), this, SLOT(slotReplaceText(const QString &, int, int, int)));
...
void TextAreaWidget::slotReplaceNext()
{
    ...
    while(res == KFind::NoMatch)
    {
        ...
        {
            if(m_repPara == paragraphs() - 1 && m_repIndex == paragraphLength(m_repPara) - 1)
            {
                break;
            }
        }
...
void TextAreaWidget::slotDoFind()
{
    ...
    // Connect highlight signal to code which handles highlighting
    // of found text.
    connect(m_find, SIGNAL(highlight(const QString &, int, int)), this, SLOT(slotFindHighlight(const QString &, int, int)));
...
void TextAreaWidget::slotFindNext()
{
    ...
    while(res == KFind::NoMatch)
    {
        ...
        {
            if(m_findPara == paragraphs() - 1 && m_findIndex == paragraphLength(m_findPara) - 1)
            {
                break;
            }
        }
...
void TextAreaWidget::slotReplace()
{
    ...
    {
        m_repDlg = new KReplaceDialog(this, "KHTMLText Area Replace Dialog", 0, QStringList(), QStringList(), false);
...
RenderTextArea::RenderTextArea(HTMLTextAreaElementImpl *element) : RenderFormElement(element)
{
...
void RenderTextArea::calcMinMaxWidth()
{
    ...
    QSize size(kMax(element()->cols(), 1L) * m.width('x') + w->frameWidth() + w->verticalScrollBar()->sizeHint().width(),
               kMax(element()->rows(), 1L) * m.lineSpacing() + w->frameWidth() * 4
                   + (w->wordWrap() == QTextEdit::NoWrap ? w->horizontalScrollBar()->sizeHint().height() : 0));
