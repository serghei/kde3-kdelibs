...
RenderListItem::RenderListItem(DOM::NodeImpl *node) : RenderBlock(node)
{
...
static RenderObject *getParentOfFirstLineBox(RenderObject *curr, RenderObject *marker)
{
    ...
    for(RenderObject *currChild = firstChild; currChild; currChild = currChild->nextSibling())
    {
        ...
        if(currChild->style()->htmlHacks() && currChild->element() && (currChild->element()->id() == ID_UL || currChild->element()->id() == ID_OL))
            break;
...
RenderListMarker::RenderListMarker(DOM::NodeImpl *node) : RenderBox(node), m_listImage(0), m_markerWidth(0)
{
    // init RenderObject attributes
            ...
            if(!m_item.isEmpty())
            {
                if(listPositionInside())
                {
                    if(style()->direction() == LTR)
                    {
                        ...
                        p->drawText(_tx + fm.width(m_item), _ty, 0, 0, Qt::AlignLeft | Qt::DontClip, QString::fromLatin1(". "));
                    }
                    else
                    {
