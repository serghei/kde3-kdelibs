...
RenderObject::SelectionState InlineTextBox::selectionState()
{
    ...
    if(state == RenderObject::SelectionStart || state == RenderObject::SelectionEnd || state == RenderObject::SelectionBoth)
    {
        ...
        else if((state == RenderObject::SelectionEnd || startPos < m_start) && (state == RenderObject::SelectionStart || endPos > m_start + m_len))
...
{
    if(object()->isBR() || object()->style()->visibility() != VISIBLE || m_truncation == cFullTruncation || i.phase == PaintActionOutline)
        return;
    ...
    if(m_len > 0 && i.phase != PaintActionSelection)
    {
        ...
        {
            font->drawText(i.p, m_x + tx, m_y + ty + m_baseline, renderText()->string()->s, renderText()->string()->l, m_start, endPoint, m_toAdd,
                                   ...
                                   int deco)
{
    ...
    // kdDebug( 6040 ) << "textRun::painting(" << QConstString(text->str->s + m_start, m_len).string().left(30) << ") at(" << m_x+tx << "/" << m_y+ty
    // << ")" << endl;
    ...
    f->drawText(p, m_x + tx, m_y + ty + m_baseline, text->str->s, text->str->l, m_start, m_len, m_toAdd, m_reversed ? QPainter::RTL : QPainter::LTR,
                needClipping ? 0 : startPos, needClipping ? m_len : endPos, hbg, m_y + ty, height(), deco);
...
void InlineTextBox::paintShadow(QPainter *pt, const Font *f, int _tx, int _ty, const ShadowData *shadow)
{
    ...
    if(shadow->blur <= 0)
    {
        ...
        f->drawText(pt, x, y + m_baseline, text->str->s, text->str->l, m_start, m_len, m_toAdd, m_reversed ? QPainter::RTL : QPainter::LTR);
        pt->setPen(c);
    }
    else
    {
        ...
        p.setFont(pt->font());
        f->drawText(&p, thickness, thickness + m_baseline, text->str->s, text->str->l, m_start, m_len, m_toAdd,
                    m_reversed ? QPainter::RTL : QPainter::LTR);
...
FindSelectionResult InlineTextBox::checkSelectionPoint(int _x, int _y, int _tx, int _ty, const Font *f, RenderText *text, int &offset,
                                                       short lineHeight)
{
...
RenderText::RenderText(DOM::NodeImpl *node, DOMStringImpl *_str) : RenderObject(node)
{
...
void RenderText::setStyle(RenderStyle *_style)
{
    if(style() != _style)
    {
        bool changedText = ((!style() && (_style->textTransform() != TTNONE || !_style->preserveLF() || !_style->preserveWS()))
                            || (style() && (style()->textTransform() != _style->textTransform() || style()->whiteSpace() != _style->whiteSpace())));
...
//        kdDebug(6040) << "checkFirstLetter: forcedMinOffset: " << forcedMinOffset() << endl;
    ...
    while(!(offset >= s->m_start && offset <= s->m_start + s->m_len) && ++si < count)
    {
...
bool RenderText::nodeAtPoint(NodeInfo &info, int _x, int _y, int _tx, int _ty, HitTestAction /*hitTestAction*/, bool /*inBox*/)
{
    ...
    if(style()->visibility() != HIDDEN)
    {
        ...
        while(s)
        {
            if((_y >= _ty + s->m_y) && (_y < _ty + s->m_y + s->m_height) && (_x >= _tx + s->m_x) && (_x < _tx + s->m_x + s->m_width))
            {
                inside = true;
                break;
            }
...
FindSelectionResult RenderText::checkSelectionPoint(int _x, int _y, int _tx, int _ty, DOM::NodeImpl *&node, int &offset, SelPointState &)
{
    ...
    // kdDebug(6040) << renderName() << "::checkSelectionPoint x=" << xPos() << " y=" << yPos() << " w=" << width() << " h=" << height() << "
    // m_lines.count=" << m_lines.count() << endl;
    ...
    for(unsigned int si = 0; si < m_lines.count(); si++)
    {
        ...
        //         kdDebug(6040) << "RenderText::checkSelectionPoint " << this << " line " << si << " result=" << result << " offset=" << offset <<
        //         endl;
...
void RenderText::calcMinMaxWidth()
{
    ...
    for(int i = 0; i < len; i++)
    {
        ...
        int wordlen = 0;
        while(i + wordlen < len && (i + wordlen == 0 || str->s[i + wordlen].unicode() != SOFT_HYPHEN) && !(isBreakable(str->s, i + wordlen, str->l)))
            wordlen++;
...
bool RenderText::containsOnlyWhitespace(unsigned int from, unsigned int len) const
{
    ...
    for(currPos = from; currPos < from + len && (str->s[currPos] == '\n' || str->s[currPos].direction() == QChar::DirWS); currPos++)
        ;
    return currPos >= (from + len);
}

void RenderText::trimmedMinMaxWidth(short &beginMinW, bool &beginWS, short &endMinW, bool &endWS, bool &hasBreakableChar, bool &hasBreak,
                                    short &beginMaxW, short &endMaxW, short &minW, short &maxW, bool &stripFrontSpaces)
{
...
RenderTextFragment::RenderTextFragment(DOM::NodeImpl *_node, DOM::DOMStringImpl *_str, int startOffset, int endOffset)
    : RenderText(_node, _str->substring(startOffset, endOffset)), m_start(startOffset), m_end(endOffset), m_generatedContentStr(0)
{
}

RenderTextFragment::RenderTextFragment(DOM::NodeImpl *_node, DOM::DOMStringImpl *_str) : RenderText(_node, _str), m_start(0)
{
