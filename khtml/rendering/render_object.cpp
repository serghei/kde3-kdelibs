...
RenderLayer *RenderObject::findNextLayer(RenderLayer *parentLayer, RenderObject *startPoint, bool checkParent)
{
    ...
    {
        for(RenderObject *curr = startPoint ? startPoint->nextSibling() : firstChild(); curr; curr = curr->nextSibling())
        {
...
int RenderObject::offsetLeft() const
{
    ...
    for(RenderObject *curr = parent(); curr && curr != offsetPar; curr = curr->parent())
        x += curr->xPos();
...
RenderObject *RenderObject::offsetParent() const
{
    ...
    while(curr
          && (!curr->element() || (!curr->isPositioned() && !curr->isRelPositioned() && !(strict && skipTables ? curr->isRoot() : curr->isBody()))))
    {
...
{
    return width() - borderLeft() - borderRight() - (layer() ? layer()->verticalScrollbarWidth() : 0);
}

int RenderObject::clientHeight() const
{
    return height() - borderTop() - borderBottom() - (layer() ? layer()->horizontalScrollbarHeight() : 0);
}
...
RenderBlock *RenderObject::containingBlock() const
{
    ...
    {
        while(o && (o->style()->position() == STATIC || (o->isInline() && !o->isReplaced())) && !o->isCanvas())
        {
    ...
    {
        while(o && ((o->isInline() && !o->isReplaced()) || o->isTableRow() || o->isTableSection() || o->isTableCol() || o->isFrameSet()))
            o = o->parent();
    }
...
bool RenderObject::sizesToMaxWidth() const
{
    ...
    if(isFloating() || isCompact() || (isInlineBlockOrInlineTable() && !isHTMLMarquee())
    ...
    if(parent()->isFlexibleBox() && (parent()->style()->boxOrient() == HORIZONTAL || parent()->style()->boxAlign() != BSTRETCH))
        return true;
#endif
...
static int brightness(int red, int green, int blue)
{
    ...
    int luminosity = ((RED_LUMINOSITY * red) / 100) + ((GREEN_LUMINOSITY * green) / 100) + ((BLUE_LUMINOSITY * blue) / 100);

    return ((intensity * INTENSITY_FACTOR) + (luminosity * LUMINOSITY_FACTOR)) / 100;
}
...
static void calc3DColor(QColor &color, bool darken)
{
    ...
    {
        f0 = COLOR_DARK_BS_FACTOR + (brightness_ * (COLOR_LITE_BS_FACTOR - COLOR_DARK_BS_FACTOR) / MAX_COLOR);
        f1 = COLOR_DARK_TS_FACTOR + (brightness_ * (COLOR_LITE_TS_FACTOR - COLOR_DARK_TS_FACTOR) / MAX_COLOR);
    }
...
void RenderObject::drawBorder(QPainter *p, int x1, int y1, int x2, int y2, BorderSide s, QColor c, const QColor &textcolor, EBorderStyle style,
    ...
    switch(style)
    {
        ...
        case DOUBLE:
        {
            ...
            else
            {
                ...
                {
                    case BSTop:
                        drawBorder(p, x1 + kMax((-adjbw1 * 2 + 1) / 3, 0), y1, x2 - kMax((-adjbw2 * 2 + 1) / 3, 0), y1 + third, s, c, textcolor,
                                   SOLID, adjbw1bigthird, adjbw2bigthird);
                        drawBorder(p, x1 + kMax((adjbw1 * 2 + 1) / 3, 0), y2 - third, x2 - kMax((adjbw2 * 2 + 1) / 3, 0), y2, s, c, textcolor, SOLID,
                                   adjbw1bigthird, adjbw2bigthird);
                        break;
                    case BSLeft:
                        drawBorder(p, x1, y1 + kMax((-adjbw1 * 2 + 1) / 3, 0), x1 + third, y2 - kMax((-adjbw2 * 2 + 1) / 3, 0), s, c, textcolor,
                                   SOLID, adjbw1bigthird, adjbw2bigthird);
                        drawBorder(p, x2 - third, y1 + kMax((adjbw1 * 2 + 1) / 3, 0), x2, y2 - kMax((adjbw2 * 2 + 1) / 3, 0), s, c, textcolor, SOLID,
                                   adjbw1bigthird, adjbw2bigthird);
                        break;
                    case BSBottom:
                        drawBorder(p, x1 + kMax((adjbw1 * 2 + 1) / 3, 0), y1, x2 - kMax((adjbw2 * 2 + 1) / 3, 0), y1 + third, s, c, textcolor, SOLID,
                                   adjbw1bigthird, adjbw2bigthird);
                        drawBorder(p, x1 + kMax((-adjbw1 * 2 + 1) / 3, 0), y2 - third, x2 - kMax((-adjbw2 * 2 + 1) / 3, 0), y2, s, c, textcolor,
                                   SOLID, adjbw1bigthird, adjbw2bigthird);
                        break;
                    case BSRight:
                        drawBorder(p, x1, y1 + kMax((adjbw1 * 2 + 1) / 3, 0), x1 + third, y2 - kMax((adjbw2 * 2 + 1) / 3, 0), s, c, textcolor, SOLID,
                                   adjbw1bigthird, adjbw2bigthird);
                        drawBorder(p, x2 - third, y1 + kMax((-adjbw1 * 2 + 1) / 3, 0), x2, y2 - kMax((-adjbw2 * 2 + 1) / 3, 0), s, c, textcolor,
                                   SOLID, adjbw1bigthird, adjbw2bigthird);
                        break;
                    default:
                        break;
        ...
        case GROOVE:
        {
            ...
            {
                case BSTop:
                    drawBorder(p, x1 + kMax(-adjbw1, 0) / 2, y1, x2 - kMax(-adjbw2, 0) / 2, (y1 + y2 + 1) / 2, s, c, textcolor, s1, adjbw1bighalf,
                               adjbw2bighalf);
                    drawBorder(p, x1 + kMax(adjbw1 + 1, 0) / 2, (y1 + y2 + 1) / 2, x2 - kMax(adjbw2 + 1, 0) / 2, y2, s, c, textcolor, s2, adjbw1 / 2,
                               adjbw2 / 2);
                    break;
                case BSLeft:
                    drawBorder(p, x1, y1 + kMax(-adjbw1, 0) / 2, (x1 + x2 + 1) / 2, y2 - kMax(-adjbw2, 0) / 2, s, c, textcolor, s1, adjbw1bighalf,
                               adjbw2bighalf);
                    drawBorder(p, (x1 + x2 + 1) / 2, y1 + kMax(adjbw1 + 1, 0) / 2, x2, y2 - kMax(adjbw2 + 1, 0) / 2, s, c, textcolor, s2, adjbw1 / 2,
                               adjbw2 / 2);
                    break;
                case BSBottom:
                    drawBorder(p, x1 + kMax(adjbw1, 0) / 2, y1, x2 - kMax(adjbw2, 0) / 2, (y1 + y2 + 1) / 2, s, c, textcolor, s2, adjbw1bighalf,
                               adjbw2bighalf);
                    drawBorder(p, x1 + kMax(-adjbw1 + 1, 0) / 2, (y1 + y2 + 1) / 2, x2 - kMax(-adjbw2 + 1, 0) / 2, y2, s, c, textcolor, s1,
                               adjbw1 / 2, adjbw2 / 2);
                    break;
                case BSRight:
                    drawBorder(p, x1, y1 + kMax(adjbw1, 0) / 2, (x1 + x2 + 1) / 2, y2 - kMax(adjbw2, 0) / 2, s, c, textcolor, s2, adjbw1bighalf,
                               adjbw2bighalf);
                    drawBorder(p, (x1 + x2 + 1) / 2, y1 + kMax(-adjbw1 + 1, 0) / 2, x2, y2 - kMax(-adjbw2 + 1, 0) / 2, s, c, textcolor, s1,
                               adjbw1 / 2, adjbw2 / 2);
                    break;
            }
            break;
        ...
        case INSET:
        case OUTSET:
            calc3DColor(c, (style == OUTSET && (s == BSBottom || s == BSRight)) || (style == INSET && (s == BSTop || s == BSLeft)));
        /* nobreak; */
        case SOLID:
            p->setPen(Qt::NoPen);
            p->setBrush(c);
            ...
            {
                case BSTop:
                    quad.setPoints(4, x1 + kMax(-adjbw1, 0), y1, x1 + kMax(adjbw1, 0), y2, x2 - kMax(adjbw2, 0), y2, x2 - kMax(-adjbw2, 0), y1);
                    break;
                case BSBottom:
                    quad.setPoints(4, x1 + kMax(adjbw1, 0), y1, x1 + kMax(-adjbw1, 0), y2, x2 - kMax(-adjbw2, 0), y2, x2 - kMax(adjbw2, 0), y1);
                    break;
                case BSLeft:
                    quad.setPoints(4, x1, y1 + kMax(-adjbw1, 0), x1, y2 - kMax(-adjbw2, 0), x2, y2 - kMax(adjbw2, 0), x2, y1 + kMax(adjbw1, 0));
                    break;
                case BSRight:
                    quad.setPoints(4, x1, y1 + kMax(adjbw1, 0), x1, y2 - kMax(adjbw2, 0), x2, y2 - kMax(-adjbw2, 0), x2, y1 + kMax(-adjbw1, 0));
                    break;
...
void RenderObject::paintBorder(QPainter *p, int _tx, int _ty, int w, int h, const RenderStyle *style, bool begin, bool end)
{
    ...
    if(render_t)
    {
        bool ignore_left = (tc == lc) && (tt == lt) && (ts >= OUTSET) && (ls == DOTTED || ls == DASHED || ls == SOLID || ls == OUTSET);

        bool ignore_right = (tc == rc) && (tt == rt) && (ts >= OUTSET) && (rs == DOTTED || rs == DASHED || rs == SOLID || rs == INSET);

        drawBorder(p, _tx, _ty, _tx + w, _ty + style->borderTopWidth(), BSTop, tc, style->color(), ts, ignore_left ? 0 : style->borderLeftWidth(),
                   ignore_right ? 0 : style->borderRightWidth());
    }

    if(render_b)
    {
        bool ignore_left = (bc == lc) && (bt == lt) && (bs >= OUTSET) && (ls == DOTTED || ls == DASHED || ls == SOLID || ls == OUTSET);

        bool ignore_right = (bc == rc) && (bt == rt) && (bs >= OUTSET) && (rs == DOTTED || rs == DASHED || rs == SOLID || rs == INSET);
    ...
    if(render_l)
    {
        bool ignore_top = (tc == lc) && (tt == lt) && (ls >= OUTSET) && (ts == DOTTED || ts == DASHED || ts == SOLID || ts == OUTSET);

        bool ignore_bottom = (bc == lc) && (bt == lt) && (ls >= OUTSET) && (bs == DOTTED || bs == DASHED || bs == SOLID || bs == INSET);

        drawBorder(p, _tx, _ty, _tx + style->borderLeftWidth(), _ty + h, BSLeft, lc, style->color(), ls, ignore_top ? 0 : style->borderTopWidth(),
                   ignore_bottom ? 0 : style->borderBottomWidth());
    }

    if(render_r)
    {
        bool ignore_top = (tc == rc) && (tt == rt) && (rs >= DOTTED || rs == INSET) && (ts == DOTTED || ts == DASHED || ts == SOLID || ts == OUTSET);

        bool ignore_bottom =
            (bc == rc) && (bt == rt) && (rs >= DOTTED || rs == INSET) && (bs == DOTTED || bs == DASHED || bs == SOLID || bs == INSET);
...
void RenderObject::paintOutline(QPainter *p, int _tx, int _ty, int w, int h, const RenderStyle *style)
{
    ...
    drawBorder(p, _tx - ow, _ty - ow, _tx, _ty + h + ow, BSLeft, QColor(oc), style->color(), os, ow, ow, true);

    drawBorder(p, _tx - ow, _ty - ow, _tx + w + ow, _ty, BSTop, QColor(oc), style->color(), os, ow, ow, true);

    drawBorder(p, _tx + w, _ty - ow, _tx + w + ow, _ty + h + ow, BSRight, QColor(oc), style->color(), os, ow, ow, true);

    drawBorder(p, _tx - ow, _ty + h, _tx + w + ow, _ty + h + ow, BSBottom, QColor(oc), style->color(), os, ow, ow, true);
}
...
QString RenderObject::information() const
{
    ...
    ts << renderName() << "(" << (style() ? style()->refCount() : 0) << ")"
       << ": " << (void *)this << "  ";
    ts << "{" << x << " " << y << "} ";
    if(isInline())
        ts << "il ";
    ...
    ts << " (" << xPos() << "," << yPos() << "," << width() << "," << height() << ")"
       << " [" << minWidth() << "-" << maxWidth() << "]"
       << " { mT: " << marginTop() << " qT: " << isTopMarginQuirk() << " mB: " << marginBottom() << " qB: " << isBottomMarginQuirk() << "}"
       << (isTableCell()
               ? (QString::fromLatin1(" [r=") + QString::number(static_cast< const RenderTableCell * >(this)->row()) + QString::fromLatin1(" c=")
                  + QString::number(static_cast< const RenderTableCell * >(this)->col()) + QString::fromLatin1(" rs=")
                  + QString::number(static_cast< const RenderTableCell * >(this)->rowSpan()) + QString::fromLatin1(" cs=")
                  + QString::number(static_cast< const RenderTableCell * >(this)->colSpan()) + QString::fromLatin1("]"))
               : QString::null);
    if(layer())
        ts << " layer=" << layer();
    if(continuation())
    ...
    if(isText())
        ts << " \""
           << QConstString(static_cast< const RenderText * >(this)->text(), kMin(static_cast< const RenderText * >(this)->length(), 10u)).string()
           << "\"";
    return str;
}
...
void RenderObject::setStyle(RenderStyle *style)
{
    ...
    if(m_style)
    {
        ...
        if(d >= RenderStyle::Visible && !isText() && m_parent
           && (d == RenderStyle::Position || m_style->outlineWidth() > style->outlineWidth() || (!m_style->hidesOverflow() && style->hidesOverflow())
        ...
        if((isFloating() && m_style->floating() != style->floating())
           || (isPositioned() && m_style->position() != style->position() && style->position() != ABSOLUTE && style->position() != FIXED))
            removeFromObjectLists();

        if(layer())
        {
            if((m_style->hasAutoZIndex() != style->hasAutoZIndex() || m_style->zIndex() != style->zIndex()
...
FindSelectionResult RenderObject::checkSelectionPoint(int _x, int _y, int _tx, int _ty, DOM::NodeImpl *&node, int &offset, SelPointState &state)
{
    ...
    for(RenderObject *child = firstChild(); child; child = child->nextSibling())
    {
        ...
        //        kdDebug(6040) << "iterating " << (child ? child->renderName() : "") << "@" << child << (child->isText() ? " contains: \"" +
        //        QConstString(static_cast<RenderText *>(child)->text(), kMin(static_cast<RenderText *>(child)->length(), 10u)).string() + "\"" :
        //        QString::null) << endl;
        ...
        switch(pos)
        {
            ...
            case SelectionPointBefore:
                ...
                else
                {
                    node = nod;
                    offset = off;
                    // kdDebug(6030) << "RenderObject::checkSelectionPoint " << this << " before us -> returning SelectionPointBefore " << node << "/"
                    // << offset << endl;
            ...
            case SelectionPointAfterInLine:
                ...
                // kdDebug(6030) << "RenderObject::checkSelectionPoint: selection after: " << nod << " offset: " << off << " afterInLine: " <<
                // state.m_afterInLine << endl;
...
bool RenderObject::nodeAtPoint(NodeInfo &info, int _x, int _y, int _tx, int _ty, HitTestAction hitTestAction, bool inside)
{
    ...
    inside |= (style()->visibility() != HIDDEN && (_y >= ty) && (_y < ty + height()) && (_x >= tx) && (_x < tx + width())) || isRoot() || isBody();
    ...
    if(hitTestAction != HitTestSelfOnly && ((!isRenderBlock() || !static_cast< RenderBlock * >(this)->isPointInScrollbar(_x, _y, _tx, _ty))
                                            && (overhangingContents() || inOverflowRect || isInline() || isRoot() || isCanvas() || isTableRow()
    ...
    if(!isTableCell() && isInline())
    {
        ...
        else
        {
            if(!ref)
                ref = parent();
            bool checkParent =
                ref->isInline() && !ref->isReplacedBlock() && !(ref->style()->verticalAlign() == TOP || ref->style()->verticalAlign() == BOTTOM);
...
void RenderObject::getTextDecorationColors(int decorations, QColor &underline, QColor &overline, QColor &linethrough, bool quirksMode)
{
    ...
    } while(curr && decorations && (!quirksMode || !curr->element() || (curr->element()->id() != ID_A && curr->element()->id() != ID_FONT)));
...
bool RenderObject::hasCounter(const QString &counter) const
{
    ...
    if(counter == "list-item")
    {
        if(isListItem())
            return true;
        if(element() && (element()->id() == ID_OL || element()->id() == ID_UL || element()->id() == ID_MENU || element()->id() == ID_DIR))
            return true;
    }
...
CounterNode *RenderObject::getCounter(const QString &counter, bool view, bool counters)
{
    ...
    else if(counter == "list-item")
    {
        ...
        else if(element() && (element()->id() == ID_UL || element()->id() == ID_MENU || element()->id() == ID_DIR))
        {
            i = new CounterReset(this);
            val = 0;
            //             kdDebug( 6040 ) << renderName() << " counter-reset: " << counter << " " << val << endl;
    ...
    for(RenderObject *curr = firstChild(); curr; curr = curr->nextSibling())
    {
        if(curr->isWidget() && static_cast< RenderWidget * >(curr)->needsMask())
        {
            ...
            if(!r.isNull() && curr->absolutePosition(x, y))
            {
                ...
                x += pbx;
                y += pby;
                r = r.intersect(QRect(x, y, curr->width() - pbx - curr->borderRight() - curr->paddingRight(),
