 ...
 * @param wordEnd relative index pointing one position after the word ended
 */
inline void closeAndDrawWord(QPainter *p, QPainter::TextDirection d, int &x, int y, const short widths[], const QChar *str, int pos, int &wordStart,
                             int wordEnd)
{
    if(wordEnd <= wordStart)
        return;
...
struct Font::ScalKey
{
    ...
    ScalKey(const QFont &font) : family(font.family()), weight(font.weight()), italic(font.italic())
    {
    }
    ...
    bool operator==(const ScalKey &other) const
    {
        return (weight == other.weight && italic == other.italic && family == other.family);
    }
};
