...
RenderReplaced::RenderReplaced(DOM::NodeImpl *node) : RenderBox(node)
{
...
void RenderReplaced::calcMinMaxWidth()
{
    ...
    if(style()->width().isPercent() || style()->height().isPercent() || style()->maxWidth().isPercent() || style()->maxHeight().isPercent()
...
RenderWidget::RenderWidget(DOM::NodeImpl *node) : RenderReplaced(node)
{
    m_widget = 0;
...
public:
    enum
    {
        Type = QEvent::User + 0xbee
    };
    QWidgetResizeEvent(int _w, int _h) : QEvent((QEvent::Type)Type), w(_w), h(_h)
    {
    }
    int w;
    int h;
        ...
        if(color.isValid() || backgroundColor.isValid())
        {
            ...
            if(color.isValid())
            {
                ...
                const struct ColorSet toSet[] = {
                    {QPalette::Active, QColorGroup::Foreground},   {QPalette::Active, QColorGroup::ButtonText},
                    {QPalette::Active, QColorGroup::Text},         {QPalette::Inactive, QColorGroup::Foreground},
                    {QPalette::Inactive, QColorGroup::ButtonText}, {QPalette::Inactive, QColorGroup::Text},
                    {QPalette::Disabled, QColorGroup::ButtonText}, {QPalette::NColorGroups, QColorGroup::NColorRoles},
                };
...
void RenderWidget::setStyle(RenderStyle *_style)
{
    ...
    if(!canHaveBorder() || (style()->borderLeftStyle() == BNATIVE && style()->borderRightStyle() == BNATIVE && style()->borderTopStyle() == BNATIVE
...
void RenderWidget::paint(PaintInfo &paintInfo, int _tx, int _ty)
{
    ...
    if(shouldPaintBackgroundOrBorder() && (paintInfo.phase == PaintActionChildBackground || paintInfo.phase == PaintActionChildBackgrounds))
    ...
    if((_ty > paintInfo.r.bottom()) || (_ty + m_height <= paintInfo.r.top()) || (_tx + m_width <= paintInfo.r.left()) || (_tx > paintInfo.r.right()))
        return;
...
protected:
    PaintBuffer() : m_overflow(false), m_grabbed(false), m_timer(0), m_resetWidth(0), m_resetHeight(0){};
    ...
    switch(e->type())
    {
        ...
        case QEvent::Wheel:
            if(widget()->parentWidget() == view()->viewport())
            {
                ...
                if(qApp->focusWidget() != widget() && widget()->focusPolicy() <= QWidget::StrongFocus)
                {
...
bool RenderWidget::handleEvent(const DOM::EventImpl &ev)
{
    ...
    switch(ev.id())
    {
        ...
        case EventImpl::MOUSEMOVE_EVENT:
        {
            ...
            QPoint p(me.clientX() - absx + m_view->contentsX(), me.clientY() - absy + m_view->contentsY());
        ...
        case EventImpl::KEYPRESS_EVENT:
        {
            ...
            if(ke->isAutoRepeat())
            {
                QKeyEvent releaseEv(QEvent::KeyRelease, ke->key(), ke->ascii(), ke->state(), ke->text(), ke->isAutoRepeat(), ke->count());
...
void RenderWidget::dump(QTextStream &stream, const QString &ind) const
{
    ...
    if(widget())
        stream << " color=" << widget()->foregroundColor().name() << " bg=" << widget()->backgroundColor().name();
    else
        stream << " null widget";
}
#endif
