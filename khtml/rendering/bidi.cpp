...
/* a small helper class used internally to resolve Bidi embedding levels.
   ...
   relayouting
*/
BidiContext::BidiContext(unsigned char l, QChar::Direction e, BidiContext *p, bool o) : level(l), override(o), dir(e)
{
    parent = p;
    if(p)
    {
...
static inline RenderObject *Bidinext(RenderObject *par, RenderObject *current, BidiState &bidi, bool skipInlines = true)
{
    RenderObject *next = 0;
        ...
        if(!current->isFloating() && !current->isReplaced() && !current->isPositioned())
        {
            ...
            if(next && adjustEmbedding)
            {
                ...
                if(ub != UBNormal && !emptyRun)
                {
                    ...
                    QChar::Direction d = (ub == Embed ? (dir == RTL ? QChar::DirRLE : QChar::DirLRE) : (dir == RTL ? QChar::DirRLO : QChar::DirLRO));
                    embed(d, bidi);
                }
            }
        }
        ...
        if(currpoint == lBreak)
        {
            ...
            if(!endpoint.obj->style()->preserveWS())
            {
                if(endpoint.obj->isText())
                {
                    ...
                    if(endpoint.pos + 1 < textObj->length() && textObj->text()[endpoint.pos + 1].unicode() == SOFT_HYPHEN)
                        return;
                }
                endpoint.pos--;
            }
...
{
    if(start > end || obj->isFloating() || (obj->isPositioned() && !obj->hasStaticX() && !obj->hasStaticY()))
        return;
    ...
    else
    {
        ...
        if(int(nextMidpoint.pos + 1) <= end)
        {
            ...
            sCurrMidpoint++;
            if(nextMidpoint.pos != UINT_MAX)
            { // UINT_MAX means stop at the object and don't include any of it.
                addRun(new(obj->renderArena()) BidiRun(start, nextMidpoint.pos + 1, obj, bidi.context, dir));
...
void RenderBlock::bidiReorderLine(const BidiIterator &start, const BidiIterator &end, BidiState &bidi)
{
    ...
    while(1)
    {
...
#if BIDI_DEBUG > 1
        kdDebug(6041) << "directions: dir=" << (int)dir << " current=" << (int)dirCurrent << " last=" << status.last << " eor=" << status.eor
                      << " lastStrong=" << status.lastStrong << " embedding=" << (int)context->dir << " level =" << (int)context->level << endl;
#endif
                ...
                if(!(bidi.status.lastStrong == QChar::DirAL))
                {
                    ...
                    switch(bidi.status.last)
                    {
                        ...
                        case QChar::DirON:
                            ...
                            else if(bidi.status.eor == QChar::DirL || (bidi.status.eor == QChar::DirEN && bidi.status.lastStrong == QChar::DirL))
                            {
                                bidi.eor = bidi.current;
                                bidi.status.eor = dirCurrent;
                            }
        ...
        // cout << "     after: dir=" << //        dir << " current=" << dirCurrent << " last=" << status.last << " eor=" << status.eor << "
        // lastStrong=" << status.lastStrong << " embedding=" << context->dir << endl;
...
#if BIDI_DEBUG > 0
    kdDebug(6041) << "reached end of line current=" << current.obj << "/" << current.pos << ", eor=" << eor.obj << "/" << eor.pos << endl;
#endif
...
static void setStaticPosition(RenderBlock *p, RenderObject *o, bool *needToSetStaticX = 0, bool *needToSetStaticY = 0)
{
    ...
    {
        static_cast< RenderBox * >(o)->setStaticX(o->parent()->style()->direction() == LTR ? p->borderLeft() + p->paddingLeft()
...
BidiIterator RenderBlock::findNextLineBreak(BidiIterator &start, BidiState &bidi)
{
    ...
    while(o)
    {
        ...
        else if(o->isInlineFlow())
        {
            ...
            tmpW += o->marginLeft() + o->borderLeft() + o->paddingLeft() + o->marginRight() + o->borderRight() + o->paddingRight();
        }
...
#endif
#ifdef DEBUG_LINEBREAKS
                    kdDebug(6041) << "found space at " << pos << " in string '" << QString(str, strlen).latin1() << "' adding " << tmpW
                                  << " new width = " << w << endl;
#endif
...
#ifdef DEBUG_LINEBREAKS
                        kdDebug(6041) << "forced break sol: " << start.obj << " " << start.pos << "   end: " << lBreak.obj << " " << lBreak.pos
                                      << "   width=" << w << endl;
#endif
                        return lBreak;
                    }
...
#ifdef DEBUG_LINEBREAKS
    kdDebug(6041) << "regular break sol: " << start.obj << " " << start.pos << "   end: " << lBreak.obj << " " << lBreak.pos << "   width=" << w
                  << endl;
#endif
...
{
    for(RootInlineBox *curr = static_cast< khtml::RootInlineBox * >(firstLineBox()); curr;
        curr = static_cast< khtml::RootInlineBox * >(curr->nextLineBox()))
    {
