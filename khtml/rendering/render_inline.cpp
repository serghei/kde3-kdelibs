...
static void appendPoint(QValueVector< QPoint > &pointArray, QPoint &pnt)
{
    ...
    reduceSegmentSeparator(pointArray) || reduceSpike(pointArray);
}
 ...
 */
static void paintOutlineSegment(RenderObject *o, QPainter *p, int tx, int ty, const QPoint &p1, const QPoint &p2, RenderObject::BorderSide prevBS,
                                RenderObject::BorderSide curBS, RenderObject::BorderSide nextBS)
{
    ...
    switch(curBS)
    {
        ...
        case RenderObject::BSRight:
            ...
            o->drawBorder(p, x1 - (curBS == RenderObject::BSLeft ? ow : 0), y1 - (prevBS == RenderObject::BSTop ? ow : 0),
                          x2 + (curBS == RenderObject::BSRight ? ow : 0), y2 + (nextBS == RenderObject::BSBottom ? ow : 0), curBS, oc,
                          o->style()->color(), os, prevBS == RenderObject::BSTop ? ow : prevBS == RenderObject::BSBottom ? -ow : 0,
                          nextBS == RenderObject::BSTop ? -ow : nextBS == RenderObject::BSBottom ? ow : 0, true);
            break;
        case RenderObject::BSBottom:
        case RenderObject::BSTop:
            //       kdDebug(6040) << "BSTop/BSBottom: prevBS " << prevBS << " curBS " << curBS << " nextBS " << nextBS << endl;
            o->drawBorder(p, x1 - (prevBS == RenderObject::BSLeft ? ow : 0), y1 - (curBS == RenderObject::BSTop ? ow : 0),
                          x2 + (nextBS == RenderObject::BSRight ? ow : 0), y2 + (curBS == RenderObject::BSBottom ? ow : 0), curBS, oc,
                          o->style()->color(), os, prevBS == RenderObject::BSLeft ? ow : prevBS == RenderObject::BSRight ? -ow : 0,
                          nextBS == RenderObject::BSLeft ? -ow : nextBS == RenderObject::BSRight ? ow : 0, true);
            break;
    }
}

void RenderInline::paintOutlinePath(QPainter *p, int tx, int ty, const QPoint *begin, const QPoint *end, BorderSide bs, int direction,
                                    BorderSide endingBS)
{
    ...
    for(const QPoint *it = begin; it != end; ++it)
    {
        ...
        if(it + 1 != end)
        {
            ...
            //             kdDebug(6040) << "newBorderSide*: " << bs << " " << direction << "d " << cur << " - " << *(it + 1) << " => " << nextBS <<
            //             endl;
        }
        else
            nextBS = endingBS;
        ...
        paintOutlineSegment(this, p, tx, ty, last, cur, lastBS, bs, nextBS);
        lastBS = bs;
        last = cur;
        bs = nextBS;
    }
