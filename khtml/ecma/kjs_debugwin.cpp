...
SourceDisplay::SourceDisplay(KJSDebugWin *debugWin, QWidget *parent, const char *name)
    : QScrollView(parent, name), m_currentLine(-1), m_sourceFile(0), m_debugWin(debugWin), m_font(KGlobalSettings::fixedFont())
{
...
void SourceDisplay::drawContents2(QPainter *p, int clipx, int clipy, int clipw, int cliph)
{
    ...
    for(int lineno = firstLine; lineno <= lastLine; lineno++)
    {
        ...
        p->setPen(textColor);
        p->drawText(linenoWidth + 4, height * lineno, contentsWidth() - linenoWidth - 4, height, Qt::AlignLeft, m_lines[lineno]);
    }

    int remainingTop = height * (lastLine + 1);
    p->fillRect(0, remainingTop, linenoWidth, bottom - remainingTop, palette().active().mid());

    p->fillRect(linenoWidth, remainingTop, right - linenoWidth, bottom - remainingTop, palette().active().base());
}
...
KJSErrorDialog::KJSErrorDialog(QWidget *parent, const QString &errorMessage, bool showDebug)
    : KDialogBase(parent, 0, true, i18n("JavaScript Error"), showDebug ? KDialogBase::Ok | KDialogBase::User1 : KDialogBase::Ok, KDialogBase::Ok,
                  false, KGuiItem("&Debug", "gear"))
{
    QWidget *page = new QWidget(this);
    setMainWidget(page);

    QLabel *iconLabel = new QLabel("", page);
    iconLabel->setPixmap(KGlobal::iconLoader()->loadIcon("messagebox_critical", KIcon::NoGroup, KIcon::SizeMedium, KIcon::DefaultState, 0, true));
...
//-------------------------------------------------------------------------
EvalMultiLineEdit::EvalMultiLineEdit(QWidget *parent) : QMultiLineEdit(parent)
{
}
...
void KJSDebugWin::slotToggleBreakpoint(int lineno)
{
    ...
    for(it = m_sourceFragments.begin(); it != m_sourceFragments.end(); ++it)
    {
        ...
        if(sourceFragment && sourceFragment->sourceFile == sourceFile && sourceFragment->baseLine <= lineno
...
bool KJSDebugWin::setBreakpoint(int sourceId, int lineno)
{
    ...
    m_breakpointCount++;
    m_breakpoints = static_cast< Breakpoint * >(realloc(m_breakpoints, m_breakpointCount * sizeof(Breakpoint)));
...
bool KJSDebugWin::haveBreakpoint(SourceFile *sourceFile, int line0, int line1)
{
    for(int i = 0; i < m_breakpointCount; i++)
    {
        ...
        if(m_sourceFragments.contains(sourceId) && m_sourceFragments[sourceId]->sourceFile == sourceFile)
        {
