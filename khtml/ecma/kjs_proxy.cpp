...
public:
    ...
    virtual QVariant evaluate(QString filename, int baseLine, const QString &, const DOM::Node &n, Completion *completion = 0);
    virtual void clear();
...
QVariant KJSProxyImpl::evaluate(QString filename, int baseLine, const QString &str, const DOM::Node &n, Completion *completion)
{
...
void KJSProxyImpl::clear()
{
    ...
    if(m_script)
    {
        ...
        if(debugWin)
        {
            if(debugWin->getExecState() && debugWin->getExecState()->interpreter() == m_script)
                debugWin->slotStop();
        ...
        if(win)
        {
            ...
            m_script->globalObject().put(m_script->globalExec(), "debug", Value(new TestFunctionImp()), Internal);
            if(win->part())
                applyUserAgent();
        }
...
// m_script->setDebuggingEnabled(m_debugEnabled);
    ...
    globalObject.put(m_script->globalExec(), "debug", Value(new TestFunctionImp()), Internal);
    applyUserAgent();
}
...
void KJSProxyImpl::applyUserAgent()
{
    ...
    if(userAgent.find(QString::fromLatin1("Microsoft")) >= 0 || userAgent.find(QString::fromLatin1("MSIE")) >= 0)
    {
        ...
        if(userAgent.find(QString::fromLatin1("Mozilla")) >= 0 && userAgent.find(QString::fromLatin1("compatible")) == -1
...
void KJSCPUGuard::start(unsigned int ms, unsigned int i_ms)
{
    ...
    itimerval tv = {{i_ms / 1000, (i_ms % 1000) * 1000}, {ms / 1000, (ms % 1000) * 1000}};
...
bool KJSCPUGuard::confirmTerminate()
{
    ...
    return KMessageBox::warningYesNo(0L, i18n("A script on this page is causing KHTML to freeze. If it continues to run, other applications may "
                                              "become less responsive.\nDo you want to abort the script?"),
                                     i18n("JavaScript"), i18n("&Abort"), KStdGuiItem::cont(), "kjscupguard_alarmhandler")
           == KMessageBox::Yes;
}
