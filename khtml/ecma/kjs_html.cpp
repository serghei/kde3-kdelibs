...
const ClassInfo KJS::HTMLDocument::info = {"HTMLDocument", &DOMDocument::info, &HTMLDocumentTable, 0};
...
KJS::HTMLDocument::HTMLDocument(ExecState *exec, const DOM::HTMLDocument &d) : DOMDocument(HTMLDocumentProto::self(exec), d)
{
}
...
Value KJS::HTMLDocument::tryGet(ExecState *exec, const Identifier &propertyName) const
{
    ...
    if(info)
    {
        ...
        if(info->nd && DOM::HTMLMappedNameCollectionImpl::matchesName(info->nd, HTMLCollectionImpl::DOCUMENT_NAMED_ITEMS, propertyDOMString))
        {
        ...
        switch(entry->value)
        {
            ...
            case All:
                ...
                if(exec->interpreter()->compatMode() == Interpreter::NetscapeCompat)
                    return Undefined();
                else if(exec->interpreter()->compatMode() == Interpreter::IECompat)
                    return getHTMLCollection(exec, doc.all());
                else // enabled but hidden
                    return getHTMLCollection(exec, doc.all(), true);
            case CompatMode:
                return String(static_cast< HTMLDocumentImpl * >(doc.handle())->parseMode() == DocumentImpl::Compat ? "BackCompat" : "CSS1Compat");
        }
    }
    // Look for overrides
...
KJS::HTMLElement::HTMLElement(ExecState *exec, const DOM::HTMLElement &e) : DOMElement(prototypeForID(exec, e.elementId()), e)
{
}
    ...
    switch(element.elementId())
    {
        ...
        case ID_META:
        {
            ...
            switch(token)
            {
                ...
                case MetaHttpEquiv:
                    return String(meta.httpEquiv());
                case MetaName:
                    return String(meta.name());
                case MetaScheme:
                    return String(meta.scheme());
            }
        }
        break;
        case ID_BASE:
        ...
        case ID_ISINDEX:
        {
            ...
            {
                case IsIndexForm:
                    return getDOMNode(exec, isindex.form()); // type HTMLFormElement
                case IsIndexPrompt:
                    return String(isindex.prompt());
            }
        }
        break;
        case ID_STYLE:
        {
            ...
            {
                case StyleDisabled:
                    return Boolean(style.disabled());
                case StyleMedia:
                    return String(style.media());
                case StyleType:
                    return String(style.type());
                case StyleSheet:
                    return getDOMStyleSheet(exec, style.sheet());
            }
        }
        break;
        case ID_BODY:
        ...
        case ID_FORM:
        {
            ...
            {
                case FormElements:
                    return getHTMLCollection(exec, form.elements());
                case FormLength:
                    return Number(form.length());
                case FormName:
                    return String(form.name()); // NOT getString (IE gives empty string)
                case FormAcceptCharset:
                    return String(form.acceptCharset());
                case FormAction:
        ...
        case ID_SELECT:
        {
            ...
            {
                case SelectType:
                    return String(select.type());
                case SelectSelectedIndex:
                    return Number(select.selectedIndex());
                case SelectValue:
                    return String(select.value());
                case SelectLength:
                    return Number(select.length());
                case SelectForm:
                    return getDOMNode(exec, select.form()); // type HTMLFormElement
                case SelectOptions:
                    return getSelectHTMLCollection(exec, select.options(), select); // type HTMLCollection
                case SelectDisabled:
                    return Boolean(select.disabled());
                case SelectMultiple:
                    return Boolean(select.multiple());
                case SelectName:
                    return String(select.name());
                case SelectSize:
                    return Number(select.size());
                case SelectTabIndex:
                    return Number(select.tabIndex());
            }
        }
        break;
        case ID_OPTGROUP:
        {
            ...
            switch(token)
            {
                case OptGroupDisabled:
                    return Boolean(optgroup.disabled());
                case OptGroupLabel:
                    return String(optgroup.label());
            }
        }
        break;
        case ID_OPTION:
        {
            ...
            {
                case OptionForm:
                    return getDOMNode(exec, option.form()); // type HTMLFormElement
                case OptionDefaultSelected:
        ...
        case ID_INPUT:
        {
            ...
            switch(token)
            {
                ...
                case InputDefaultChecked:
                    return Boolean(input.defaultChecked());
                case InputForm:
                    return getDOMNode(exec, input.form()); // type HTMLFormElement
                case InputAccept:
                    return String(input.accept());
                case InputAccessKey:
                    return String(input.accessKey());
                case InputAlign:
                    return String(input.align());
                case InputAlt:
                    return String(input.alt());
                case InputChecked:
                ...
                case InputMaxLength:
                    return Number(input.maxLength());
                case InputName:
                    return String(input.name()); // NOT getString (IE gives empty string)
                case InputReadOnly:
                    return Boolean(input.readOnly());
                case InputSize:
                    return Number(input.getSize());
                case InputSrc:
                    return String(input.src());
                case InputTabIndex:
                    return Number(input.tabIndex());
                case InputType:
                    return String(input.type());
                case InputUseMap:
                    return String(input.useMap());
                case InputValue:
        ...
        case ID_TEXTAREA:
        {
            ...
            switch(token)
            {
                case TextAreaDefaultValue:
                    return String(textarea.defaultValue());
                case TextAreaForm:
                    return getDOMNode(exec, textarea.form()); // type HTMLFormElement
                case TextAreaAccessKey:
                    return String(textarea.accessKey());
                case TextAreaCols:
                    return Number(textarea.cols());
                case TextAreaDisabled:
                    return Boolean(textarea.disabled());
                case TextAreaName:
                    return String(textarea.name());
                case TextAreaReadOnly:
                    return Boolean(textarea.readOnly());
                case TextAreaRows:
                    return Number(textarea.rows());
                case TextAreaTabIndex:
                    return Number(textarea.tabIndex());
                case TextAreaType:
                    return String(textarea.type());
                case TextAreaValue:
                    return String(textarea.value());
                case TextAreaSelectionStart:
                    return Number(textarea.selectionStart());
                case TextAreaSelectionEnd:
                    return Number(textarea.selectionEnd());
                case TextAreaTextLength:
                    return Number(textarea.textLength());
            }
        }
        break;
        case ID_BUTTON:
        {
            ...
            {
                case ButtonForm:
                    return getDOMNode(exec, button.form()); // type HTMLFormElement
                case ButtonAccessKey:
                    return String(button.accessKey());
                case ButtonDisabled:
                    return Boolean(button.disabled());
                case ButtonName:
                    return String(button.name());
                case ButtonTabIndex:
                    return Number(button.tabIndex());
                case ButtonType:
        ...
        case ID_LABEL:
        {
            ...
            {
                case LabelForm:
                    return getDOMNode(exec, label.form()); // type HTMLFormElement
                case LabelAccessKey:
                    return String(label.accessKey());
                case LabelHtmlFor:
                    return String(label.htmlFor());
            }
        }
        break;
        case ID_FIELDSET:
        ...
        case ID_LEGEND:
        {
            ...
            {
                case LegendForm:
                    return getDOMNode(exec, legend.form()); // type HTMLFormElement
                case LegendAccessKey:
                    return String(legend.accessKey());
                case LegendAlign:
                    return String(legend.align());
            }
        }
        break;
        case ID_UL:
        {
            ...
            {
                case UListCompact:
                    return Boolean(uList.compact());
                case UListType:
                    return String(uList.type());
            }
        }
        break;
        case ID_OL:
        {
            ...
            {
                case OListCompact:
                    return Boolean(oList.compact());
                case OListStart:
                    return Number(oList.start());
                case OListType:
                    return String(oList.type());
            }
        }
        break;
        case ID_DL:
        ...
        case ID_DIR:
        {
            ...
            switch(token)
            {
                case DirectoryCompact:
                    return Boolean(directory.compact());
            }
        }
        break;
        case ID_MENU:
        ...
        case ID_BASEFONT:
        {
            ...
            {
                case BaseFontColor:
                    return String(baseFont.color());
                case BaseFontFace:
                    return String(baseFont.face());
                case BaseFontSize:
                    return Number(baseFont.getSize());
            }
        }
        break;
        case ID_FONT:
        ...
        case ID_HR:
        {
            ...
            {
                case HRAlign:
                    return String(hr.align());
                case HRNoShade:
                    return Boolean(hr.noShade());
                case HRSize:
                    return String(hr.size());
                case HRWidth:
                    return String(hr.width());
            }
        }
        break;
        case ID_INS:
        case ID_DEL:
        {
            ...
            {
                case ModCite:
                    return String(mod.cite());
                case ModDateTime:
                    return String(mod.dateTime());
            }
        }
        break;
        case ID_A:
        {
            ...
            switch(token)
            {
                case AnchorAccessKey:
                    return String(anchor.accessKey());
                case AnchorCharset:
                    return String(anchor.charset());
                case AnchorCoords:
                    return String(anchor.coords());
                case AnchorHref:
                    return String(anchor.href());
                case AnchorHrefLang:
                    return String(anchor.hreflang());
                case AnchorHash:
                    return String('#' + KURL(anchor.href().string()).ref());
                case AnchorHost:
                    return String(KURL(anchor.href().string()).host());
                case AnchorHostname:
                {
                ...
                case AnchorPathName:
                    return String(KURL(anchor.href().string()).path());
                case AnchorPort:
                    return String(QString::number(KURL(anchor.href().string()).port()));
                case AnchorProtocol:
                    return String(KURL(anchor.href().string()).protocol() + ":");
                case AnchorSearch:
                {
                    KURL u(anchor.href().string());
                    QString q = u.query();
                    if(q.length() == 1)
                        return String();
                    return String(q);
                ...
                case AnchorRel:
                    return String(anchor.rel());
                case AnchorRev:
                    return String(anchor.rev());
                case AnchorShape:
                    return String(anchor.shape());
                case AnchorTabIndex:
                    return Number(anchor.tabIndex());
                case AnchorTarget:
                    return String(anchor.target());
        ...
        case ID_IMG:
        {
            ...
            {
                case ImageName:
                    return String(image.name()); // NOT getString (IE gives empty string)
                case ImageAlign:
                    return String(image.align());
                case ImageAlt:
                    return String(image.alt());
                case ImageBorder:
                    return String(image.getBorder());
                case ImageComplete:
                    return Boolean(static_cast< DOM::HTMLImageElementImpl * >(image.handle())->complete());
                case ImageHeight:
                    return Number(image.height());
                case ImageHspace:
                    return Number(image.hspace());
                case ImageIsMap:
                    return Boolean(image.isMap());
                case ImageLongDesc:
                    return String(image.longDesc());
                case ImageSrc:
                    return String(image.src());
                case ImageUseMap:
                    return String(image.useMap());
                case ImageVspace:
                    return Number(image.vspace());
                case ImageWidth:
        ...
        case ID_OBJECT:
        {
            ...
            {
                case ObjectForm:
                    return getDOMNode(exec, object.form()); // type HTMLFormElement
                case ObjectCode:
                    return String(object.code());
                case ObjectAlign:
                    return String(object.align());
                case ObjectArchive:
                    return String(object.archive());
                case ObjectBorder:
                    return String(object.border());
                case ObjectCodeBase:
                    return String(object.codeBase());
                case ObjectCodeType:
                    return String(object.codeType());
                case ObjectContentDocument:
                    return checkNodeSecurity(exec, object.contentDocument()) ? getDOMNode(exec, object.contentDocument()) : Undefined();
                case ObjectData:
                    return String(object.data());
                case ObjectDeclare:
                    return Boolean(object.declare());
                case ObjectHeight:
                    return String(object.height());
                case ObjectHspace:
                    return Number(object.getHspace());
                case ObjectName:
                    return String(object.name());
                case ObjectStandby:
                    return String(object.standby());
                case ObjectTabIndex:
                    return Number(object.tabIndex());
                case ObjectType:
                    return String(object.type());
                case ObjectUseMap:
                    return String(object.useMap());
                case ObjectVspace:
                    return Number(object.getVspace());
                case ObjectWidth:
                    return String(object.width());
            }
        }
        break;
        case ID_PARAM:
        ...
        case ID_APPLET:
        {
            ...
            {
                case AppletAlign:
                    return String(applet.align());
                case AppletAlt:
                    return String(applet.alt());
                case AppletArchive:
                    return String(applet.archive());
                case AppletCode:
                    return String(applet.code());
                case AppletCodeBase:
                    return String(applet.codeBase());
                case AppletHeight:
                    return String(applet.height());
                case AppletHspace:
                    return Number(applet.getHspace());
                case AppletName:
                    return String(applet.name());
                case AppletObject:
                    return String(applet.object());
                case AppletVspace:
                    return Number(applet.getVspace());
                case AppletWidth:
                    return String(applet.width());
            }
        }
        break;
        case ID_MAP:
        {
            ...
            {
                case MapAreas:
                    return getHTMLCollection(exec, map.areas()); // type HTMLCollection
                case MapName:
        ...
        case ID_AREA:
        {
            ...
            {
                case AreaAccessKey:
                    return String(area.accessKey());
                case AreaAlt:
                    return String(area.alt());
                case AreaCoords:
                    return String(area.coords());
                ...
                case AreaSearch:
                {
                    ...
                    switch(token)
                    {
                        ...
                        case AreaHash:
                            return String(url.isEmpty() ? "" : '#' + url.ref());
                        case AreaHost:
                            return String(url.host());
                        case AreaHostName:
                        {
                        ...
                        case AreaPort:
                            return String(QString::number(url.port()));
                        case AreaProtocol:
                            return String(url.isEmpty() ? "" : url.protocol() + ":");
                        case AreaSearch:
                            return String(url.query());
                    }
                }
                case AreaNoHref:
                    return Boolean(area.noHref());
                case AreaShape:
                    return String(area.shape());
                case AreaTabIndex:
                    return Number(area.tabIndex());
                case AreaTarget:
                    return String(area.target());
            }
        }
        break;
        case ID_SCRIPT:
        {
            ...
            switch(token)
            {
                ...
                case ScriptHtmlFor:
                    return String(script.htmlFor());
                case ScriptEvent:
                    return String(script.event());
                case ScriptCharset:
                    return String(script.charset());
                case ScriptDefer:
                    return Boolean(script.defer());
                case ScriptSrc:
        ...
        case ID_TABLE:
        {
            ...
            {
                case TableCaption:
                    return getDOMNode(exec, table.caption()); // type HTMLTableCaptionElement
                case TableTHead:
                    return getDOMNode(exec, table.tHead()); // type HTMLTableSectionElement
                case TableTFoot:
                    return getDOMNode(exec, table.tFoot()); // type HTMLTableSectionElement
                case TableRows:
                    return getHTMLCollection(exec, table.rows()); // type HTMLCollection
                case TableTBodies:
                    return getHTMLCollection(exec, table.tBodies()); // type HTMLCollection
                case TableAlign:
                    return String(table.align());
                case TableBgColor:
                    return String(table.bgColor());
                case TableBorder:
                    return String(table.border());
                case TableCellPadding:
                    return String(table.cellPadding());
                case TableCellSpacing:
                    return String(table.cellSpacing());
                case TableFrame:
                    return String(table.frame());
                case TableRules:
                    return String(table.rules());
                case TableSummary:
                    return String(table.summary());
                case TableWidth:
                    return String(table.width());
            }
        }
        break;
        case ID_CAPTION:
        ...
        case ID_COLGROUP:
        {
            ...
            {
                case TableColAlign:
                    return String(tableCol.align());
                case TableColCh:
                    return String(tableCol.ch());
                case TableColChOff:
                    return String(tableCol.chOff());
                case TableColSpan:
                    return Number(tableCol.span());
                case TableColVAlign:
                    return String(tableCol.vAlign());
                case TableColWidth:
                    return String(tableCol.width());
            }
        }
        break;
        case ID_THEAD:
        ...
        case ID_TFOOT:
        {
            ...
            switch(token)
            {
                case TableSectionAlign:
                    return String(tableSection.align());
                case TableSectionCh:
                    return String(tableSection.ch());
                case TableSectionChOff:
                    return String(tableSection.chOff());
                case TableSectionVAlign:
                    return String(tableSection.vAlign());
                case TableSectionRows:
                    return getHTMLCollection(exec, tableSection.rows()); // type HTMLCollection
            }
        }
        break;
        case ID_TR:
        {
            ...
            switch(token)
            {
                case TableRowRowIndex:
                    return Number(tableRow.rowIndex());
                case TableRowSectionRowIndex:
                    return Number(tableRow.sectionRowIndex());
                case TableRowCells:
                    return getHTMLCollection(exec, tableRow.cells()); // type HTMLCollection
                case TableRowAlign:
                    return String(tableRow.align());
                case TableRowBgColor:
                    return String(tableRow.bgColor());
                case TableRowCh:
                    return String(tableRow.ch());
                case TableRowChOff:
                    return String(tableRow.chOff());
                case TableRowVAlign:
                    return String(tableRow.vAlign());
            }
        }
        break;
        case ID_TH:
        case ID_TD:
        {
            ...
            switch(token)
            {
                case TableCellCellIndex:
                    return Number(tableCell.cellIndex());
                case TableCellAbbr:
                    return String(tableCell.abbr());
                case TableCellAlign:
                    return String(tableCell.align());
                case TableCellAxis:
                    return String(tableCell.axis());
                case TableCellBgColor:
                    return String(tableCell.bgColor());
                case TableCellCh:
                    return String(tableCell.ch());
                case TableCellChOff:
                    return String(tableCell.chOff());
                case TableCellColSpan:
                    return Number(tableCell.colSpan());
                case TableCellHeaders:
                    return String(tableCell.headers());
                case TableCellHeight:
                    return String(tableCell.height());
                case TableCellNoWrap:
                    return Boolean(tableCell.noWrap());
                case TableCellRowSpan:
                    return Number(tableCell.rowSpan());
                case TableCellScope:
                    return String(tableCell.scope());
                case TableCellVAlign:
                    return String(tableCell.vAlign());
                case TableCellWidth:
                    return String(tableCell.width());
            }
        }
        break;
        case ID_FRAMESET:
        {
            ...
            {
                case FrameSetCols:
                    return String(frameSet.cols());
                case FrameSetRows:
                    return String(frameSet.rows());
            }
        }
        break;
        case ID_LAYER:
        {
            ...
            {
                case LayerTop:
                    return Number(layerElement.top());
                case LayerLeft:
                    return Number(layerElement.left());
                case LayerVisibility:
                    return getString(layerElement.visibility());
                case LayerBgColor:
                    return getString(layerElement.bgColor());
                /*case LayerClip:           return getLayerClip(exec, layerElement); */
                case LayerDocument:
                    return Undefined();
                case LayerLayers:
                    return getHTMLCollection(exec, layerElement.layers());
            }
        }
        break;
        case ID_FRAME:
        {
            ...
            switch(token)
            {
                ...
                case FrameFrameBorder:
                    return String(frameElement.frameBorder());
                case FrameLongDesc:
                    return String(frameElement.longDesc());
                case FrameMarginHeight:
                    return String(frameElement.marginHeight());
                case FrameMarginWidth:
                    return String(frameElement.marginWidth());
                case FrameName:
                    return String(frameElement.name());
                case FrameNoResize:
                    return Boolean(frameElement.noResize());
                case FrameScrolling:
                    return String(frameElement.scrolling());
                case FrameSrc:
                case FrameLocation:
        ...
        case ID_IFRAME:
        {
            ...
            {
                case IFrameAlign:
                    return String(iFrame.align());
                case IFrameContentDocument:
                    return checkNodeSecurity(exec, iFrame.contentDocument()) ? getDOMNode(exec, iFrame.contentDocument()) : Undefined();
                ...
                case IFrameFrameBorder:
                    return String(iFrame.frameBorder());
                case IFrameHeight:
                    return String(iFrame.height());
                case IFrameLongDesc:
                    return String(iFrame.longDesc());
                case IFrameMarginHeight:
                    return String(iFrame.marginHeight());
                case IFrameMarginWidth:
                    return String(iFrame.marginWidth());
                case IFrameName:
                    return String(iFrame.name());
                case IFrameScrolling:
                    return String(iFrame.scrolling());
                case IFrameSrc:
                    return String(iFrame.src());
                case IFrameWidth:
                    return String(iFrame.width());
            }
            break;
        }
...
HTMLElementFunction::HTMLElementFunction(ExecState *exec, int i, int len) : DOMFunction(exec), id(i)
{
    Value protect(this);
...
Value KJS::HTMLElementFunction::tryCall(ExecState *exec, Object &thisObj, const List &args)
{
    ...
    switch(element.elementId())
    {
        case ID_FORM:
        {
            ...
            if(id == KJS::HTMLElement::FormSubmit)
            {
                ...
                if(policy != KHTMLSettings::KJSWindowOpenAllow)
                {
                    ...
                    if(trg.isEmpty() || trg == "_top" || trg == "_self" || trg == "_parent")
                        block = false;
                        ...
                        if(KMessageBox::questionYesNo(
                               ...
                               form.action().isEmpty()
                                   ...
                                   : i18n("<qt>This site is submitting a form which will open <p>%1</p> in a new browser window via JavaScript.<br />"
                                         ...
                                         .arg(KStringHandler::csqueeze(form.action().string(), 100)),
                               caption, i18n("Allow"), i18n("Do Not Allow"))
                           == KMessageBox::Yes)
                            block = false;
                    }
        ...
        case ID_MARQUEE:
        {
            if(id == KJS::HTMLElement::MarqueeStart && element.handle()->renderer() && element.handle()->renderer()->layer()
               && element.handle()->renderer()->layer()->marquee())
            {
                element.handle()->renderer()->layer()->marquee()->start();
                return Undefined();
            }
            else if(id == KJS::HTMLElement::MarqueeStop && element.handle()->renderer() && element.handle()->renderer()->layer()
                    && element.handle()->renderer()->layer()->marquee())
            {
...
void KJS::HTMLElement::tryPut(ExecState *exec, const Identifier &propertyName, const Value &value, int attr)
{
    ...
    DOM::HTMLElement element = static_cast< DOM::HTMLElement >(node);
#ifdef KJS_VERBOSE
    kdDebug(6070) << "KJS::HTMLElement::tryPut " << propertyName.qstring() << " thisTag=" << element.tagName().string() << " str=" << str.string()
                  << endl;
#endif
    // First look at dynamic properties
    switch(element.elementId())
...
void KJS::HTMLElement::putValueProperty(ExecState *exec, int token, const Value &value, int)
{
    ...
    kdDebug(6070) << "KJS::HTMLElement::putValueProperty "
                  << " thisTag=" << element.tagName().string() << " token=" << token << endl;
#endif
    ...
    switch(element.elementId())
    {
        ...
        case ID_LINK:
        {
            ...
            switch(token)
            {
                case LinkDisabled:
                {
                    link.setDisabled(value.toBoolean(exec));
                    return;
                }
                case LinkCharset:
                {
        ...
        case ID_STYLE:
        {
            ...
            switch(token)
            {
                case StyleDisabled:
                {
                    style.setDisabled(value.toBoolean(exec));
                    return;
                }
                case StyleMedia:
                {
        ...
        case ID_SELECT:
        {
            ...
            switch(token)
            {
                ...
                case SelectLength:
                { // read-only according to the NS spec, but webpages need it writeable
                    Object coll = Object::dynamicCast(getSelectHTMLCollection(exec, select.options(), select));
                    if(coll.isValid())
                ...
                case SelectDisabled:
                {
                    select.setDisabled(value.toBoolean(exec));
                    return;
                }
                case SelectMultiple:
                {
                    select.setMultiple(value.toBoolean(exec));
                    return;
                }
                case SelectName:
                {
                ...
                case SelectSize:
                {
                    select.setSize(value.toInteger(exec));
                    return;
                }
                case SelectTabIndex:
                {
                    select.setTabIndex(value.toInteger(exec));
                    return;
                }
            }
        }
        ...
        case ID_OPTGROUP:
        {
            ...
            switch(token)
            {
                case OptGroupDisabled:
                {
                    optgroup.setDisabled(value.toBoolean(exec));
                    return;
                }
                case OptGroupLabel:
                {
        ...
        case ID_OPTION:
        {
            ...
            switch(token)
            {
                ...
                case OptionDefaultSelected:
                {
                    option.setDefaultSelected(value.toBoolean(exec));
                    return;
                }
                ...
                case OptionText:
                {
                    DOM::NodeList nl(option.childNodes());
                ...
                case OptionDisabled:
                {
                    option.setDisabled(value.toBoolean(exec));
                    return;
                }
                case OptionLabel:
                {
        ...
        case ID_INPUT:
        {
            ...
            switch(token)
            {
                case InputDefaultValue:
                {
                    input.setDefaultValue(str);
                    return;
                }
                case InputDefaultChecked:
                {
                    input.setDefaultChecked(value.toBoolean(exec));
                    return;
                }
                // read-only: form
                case InputAccept:
                ...
                case InputDisabled:
                {
                    input.setDisabled(value.toBoolean(exec));
                    return;
                }
                case InputMaxLength:
                {
                    input.setMaxLength(value.toInteger(exec));
                    return;
                }
                case InputName:
                {
                ...
                case InputReadOnly:
                {
                    input.setReadOnly(value.toBoolean(exec));
                    return;
                }
                case InputSize:
                {
                    input.setSize(value.toInteger(exec));
                    return;
                }
                case InputSrc:
                {
                ...
                case InputTabIndex:
                {
                    input.setTabIndex(value.toInteger(exec));
                    return;
                }
                case InputType:
                {
        ...
        case ID_TEXTAREA:
        {
            ...
            switch(token)
            {
                ...
                case TextAreaAccessKey:
                {
                    textarea.setAccessKey(str);
                    return;
                }
                case TextAreaCols:
                {
                    textarea.setCols(value.toInteger(exec));
                    return;
                }
                case TextAreaDisabled:
                {
                    textarea.setDisabled(value.toBoolean(exec));
                    return;
                }
                case TextAreaName:
                {
                ...
                case TextAreaReadOnly:
                {
                    textarea.setReadOnly(value.toBoolean(exec));
                    return;
                }
                case TextAreaRows:
                {
                    textarea.setRows(value.toInteger(exec));
                    return;
                }
                case TextAreaTabIndex:
                {
                    textarea.setTabIndex(value.toInteger(exec));
                    return;
                }
                // read-only: type
                case TextAreaValue:
        ...
        case ID_BUTTON:
        {
            ...
            switch(token)
            {
                ...
                case ButtonAccessKey:
                {
                    button.setAccessKey(str);
                    return;
                }
                case ButtonDisabled:
                {
                    button.setDisabled(value.toBoolean(exec));
                    return;
                }
                case ButtonName:
                {
                ...
                case ButtonTabIndex:
                {
                    button.setTabIndex(value.toInteger(exec));
                    return;
                }
                // read-only: type
                case ButtonValue:
        ...
        case ID_UL:
        {
            ...
            switch(token)
            {
                case UListCompact:
                {
                    uList.setCompact(value.toBoolean(exec));
                    return;
                }
                case UListType:
                {
        ...
        case ID_OL:
        {
            ...
            switch(token)
            {
                case OListCompact:
                {
                    oList.setCompact(value.toBoolean(exec));
                    return;
                }
                case OListStart:
                {
                    oList.setStart(value.toInteger(exec));
                    return;
                }
                case OListType:
                {
        ...
        case ID_LI:
        {
            ...
            switch(token)
            {
                ...
                case LIValue:
                {
                    li.setValue(value.toInteger(exec));
                    return;
                }
            }
        }
        ...
        case ID_PRE:
        {
            ...
            switch(token)
            {
                case PreWidth:
                {
                    pre.setWidth(value.toInteger(exec));
                    return;
                }
            }
        }
        ...
        case ID_BASEFONT:
        {
            ...
            switch(token)
            {
                case BaseFontColor:
                {
                    baseFont.setColor(str);
                    return;
                }
                case BaseFontFace:
                {
                    baseFont.setFace(str);
                    return;
                }
                case BaseFontSize:
                {
                    baseFont.setSize(value.toInteger(exec));
                    return;
                }
            }
        }
        ...
        case ID_HR:
        {
            ...
            switch(token)
            {
                ...
                case HRNoShade:
                {
                    hr.setNoShade(value.toBoolean(exec));
                    return;
                }
                case HRSize:
                {
        ...
        case ID_A:
        {
            ...
            switch(token)
            {
                ...
                case AnchorSearch:
                {
                    KURL href(anchor.href().string());
                    QString q = str.isEmpty() ? QString() : str.string();
                    href.setQuery(q);
                    anchor.setHref(href.url());
                    return;
                }
                case AnchorName:
                {
                ...
                case AnchorTabIndex:
                {
                    anchor.setTabIndex(value.toInteger(exec));
                    return;
                }
                case AnchorTarget:
                {
        ...
        case ID_IMG:
        {
            ...
            switch(token)
            {
                ...
                case ImageHspace:
                {
                    image.setHspace(value.toInteger(exec));
                    return;
                }
                case ImageIsMap:
                {
                    image.setIsMap(value.toBoolean(exec));
                    return;
                }
                case ImageLongDesc:
                {
                ...
                case ImageVspace:
                {
                    image.setVspace(value.toInteger(exec));
                    return;
                }
                case ImageWidth:
                {
        ...
        case ID_OBJECT:
        {
            ...
            switch(token)
            {
                ...
                case ObjectArchive:
                {
                    object.setArchive(str);
                    return;
                }
                case ObjectBorder:
                {
                    object.setBorder(str);
                    return;
                }
                case ObjectCodeBase:
                {
                    object.setCodeBase(str);
                    return;
                }
                case ObjectCodeType:
                {
                    object.setCodeType(str);
                    return;
                }
                ...
                case ObjectDeclare:
                {
                    object.setDeclare(value.toBoolean(exec));
                    return;
                }
                case ObjectHeight:
                {
                    object.setHeight(str);
                    return;
                }
                case ObjectHspace:
                {
                    object.setHspace(value.toInteger(exec));
                    return;
                }
                case ObjectName:
                {
                ...
                case ObjectStandby:
                {
                    object.setStandby(str);
                    return;
                }
                case ObjectTabIndex:
                {
                    object.setTabIndex(value.toInteger(exec));
                    return;
                }
                case ObjectType:
                {
                ...
                case ObjectUseMap:
                {
                    object.setUseMap(str);
                    return;
                }
                case ObjectVspace:
                {
                    object.setVspace(value.toInteger(exec));
                    return;
                }
                case ObjectWidth:
                {
        ...
        case ID_APPLET:
        {
            ...
            switch(token)
            {
                ...
                case AppletHeight:
                {
                    applet.setHeight(str);
                    return;
                }
                case AppletHspace:
                {
                    applet.setHspace(value.toInteger(exec));
                    return;
                }
                case AppletName:
                {
                ...
                case AppletObject:
                {
                    applet.setObject(str);
                    return;
                }
                case AppletVspace:
                {
                    applet.setVspace(value.toInteger(exec));
                    return;
                }
                case AppletWidth:
                {
        ...
        case ID_AREA:
        {
            ...
            switch(token)
            {
                ...
                case AreaNoHref:
                {
                    area.setNoHref(value.toBoolean(exec));
                    return;
                }
                case AreaShape:
                {
                ...
                case AreaTabIndex:
                {
                    area.setTabIndex(value.toInteger(exec));
                    return;
                }
                case AreaTarget:
                {
        ...
        case ID_SCRIPT:
        {
            ...
            switch(token)
            {
                ...
                case ScriptCharset:
                {
                    script.setCharset(str);
                    return;
                }
                case ScriptDefer:
                {
                    script.setDefer(value.toBoolean(exec));
                    return;
                }
                case ScriptSrc:
                {
        ...
        case ID_TABLE:
        {
            ...
            switch(token)
            {
                ...
                case TableCellPadding:
                {
                    table.setCellPadding(str);
                    return;
                }
                case TableCellSpacing:
                {
                    table.setCellSpacing(str);
                    return;
                }
                case TableFrame:
                {
        ...
        case ID_COLGROUP:
        {
            ...
            switch(token)
            {
                ...
                case TableColChOff:
                {
                    tableCol.setChOff(str);
                    return;
                }
                case TableColSpan:
                {
                    tableCol.setSpan(value.toInteger(exec));
                    return;
                }
                case TableColVAlign:
                {
                    tableCol.setVAlign(str);
                    return;
                }
                case TableColWidth:
                {
                    tableCol.setWidth(str);
                    return;
                }
            }
        }
        ...
        case ID_TFOOT:
        {
            ...
            switch(token)
            {
                case TableSectionAlign:
                {
                    tableSection.setAlign(str);
                    return;
                }
                case TableSectionCh:
                {
                    tableSection.setCh(str);
                    return;
                }
                case TableSectionChOff:
                {
                    tableSection.setChOff(str);
                    return;
                }
                case TableSectionVAlign:
                {
                    tableSection.setVAlign(str);
                    return;
                }
                    // read-only: rows
            }
        ...
        case ID_TR:
        {
            ...
            switch(token)
            {
                ...
                case TableRowAlign:
                {
                    tableRow.setAlign(str);
                    return;
                }
                case TableRowBgColor:
                {
                    tableRow.setBgColor(str);
                    return;
                }
                case TableRowCh:
                {
                ...
                case TableRowChOff:
                {
                    tableRow.setChOff(str);
                    return;
                }
                case TableRowVAlign:
                {
                    tableRow.setVAlign(str);
                    return;
                }
            }
        }
        ...
        case ID_TD:
        {
            ...
            switch(token)
            {
                ...
                case TableCellAbbr:
                {
                    tableCell.setAbbr(str);
                    return;
                }
                case TableCellAlign:
                {
                    tableCell.setAlign(str);
                    return;
                }
                case TableCellAxis:
                {
                    tableCell.setAxis(str);
                    return;
                }
                case TableCellBgColor:
                {
                    tableCell.setBgColor(str);
                    return;
                }
                case TableCellCh:
                {
                ...
                case TableCellChOff:
                {
                    tableCell.setChOff(str);
                    return;
                }
                case TableCellColSpan:
                {
                    tableCell.setColSpan(value.toInteger(exec));
                    return;
                }
                case TableCellHeaders:
                {
                    tableCell.setHeaders(str);
                    return;
                }
                case TableCellHeight:
                {
                    tableCell.setHeight(str);
                    return;
                }
                case TableCellNoWrap:
                {
                    tableCell.setNoWrap(value.toBoolean(exec));
                    return;
                }
                case TableCellRowSpan:
                {
                    tableCell.setRowSpan(value.toInteger(exec));
                    return;
                }
                case TableCellScope:
                {
                    tableCell.setScope(str);
                    return;
                }
                case TableCellVAlign:
                {
                    tableCell.setVAlign(str);
                    return;
                }
                case TableCellWidth:
                {
                    tableCell.setWidth(str);
                    return;
                }
            }
        }
        ...
        case ID_FRAMESET:
        {
            ...
            switch(token)
            {
                case FrameSetCols:
                {
                    frameSet.setCols(str);
                    return;
                }
                case FrameSetRows:
                {
                    frameSet.setRows(str);
                    return;
                }
            }
        }
        ...
        case ID_LAYER:
        {
            ...
            switch(token)
            {
                case LayerTop:
                {
                    layerElement.setTop(value.toInteger(exec));
                    return;
                }
                case LayerLeft:
                {
                    layerElement.setLeft(value.toInteger(exec));
                    return;
                }
                case LayerVisibility:
                {
                    layerElement.setVisibility(str);
                    return;
                }
                case LayerBgColor:
                {
                    layerElement.setBgColor(str);
                    return;
                }
        ...
        case ID_FRAME:
        {
            ...
            switch(token)
            {
                ...
                case FrameFrameBorder:
                {
                    frameElement.setFrameBorder(str);
                    return;
                }
                case FrameLongDesc:
                {
                    frameElement.setLongDesc(str);
                    return;
                }
                case FrameMarginHeight:
                {
                    frameElement.setMarginHeight(str);
                    return;
                }
                case FrameMarginWidth:
                {
                    frameElement.setMarginWidth(str);
                    return;
                }
                case FrameName:
                {
                    frameElement.setName(str);
                    return;
                }
                case FrameNoResize:
                {
                    frameElement.setNoResize(value.toBoolean(exec));
                    return;
                }
                case FrameScrolling:
                {
                    frameElement.setScrolling(str);
                    return;
                }
                case FrameSrc:
                {
        ...
        case ID_IFRAME:
        {
            ...
            switch(token)
            {
                ...
                case IFrameLongDesc:
                {
                    iFrame.setLongDesc(str);
                    return;
                }
                case IFrameMarginHeight:
                {
                    iFrame.setMarginHeight(str);
                    return;
                }
                case IFrameMarginWidth:
                {
                    iFrame.setMarginWidth(str);
                    return;
                }
                case IFrameName:
                {
    ...
    switch(token)
    {
        ...
        case ElementInnerText:
            ...
            return;
        default:
            kdDebug(6070) << "WARNING: KJS::HTMLElement::putValueProperty unhandled token " << token << " thisTag=" << element.tagName().string()
                          << " str=" << str.string() << endl;
    }
}
...
KJS::HTMLCollection::HTMLCollection(const KJS::Object &proto, const DOM::HTMLCollection &c) : DOMObject(proto), collection(c), hidden(false)
{
}
...
bool KJS::HTMLCollection::hasProperty(ExecState *exec, const Identifier &p) const
{
    if(p == lengthPropertyName)
        return true;
    if(collection.handle()->getType() == HTMLCollectionImpl::SELECT_OPTIONS && (p == "selectedIndex" || p == "value"))
        return true;
...
Value KJS::HTMLCollection::getNamedItems(ExecState *exec, const Identifier &propertyName) const
{
    ...
    if(!matches.isEmpty())
    {
        ...
        else
        {
            ...
            for(QValueList< DOM::NodeImpl * >::const_iterator i = matches.begin(); i != matches.end(); ++i)
...
#ifdef KJS_VERBOSE
                kdDebug(6070) << "KJS::HTMLCollectionProtoFunc::tryCall document.tags(" << tagName.string() << ") -> " << list.length()
                              << " items in node list" << endl;
#endif
            }
            else
            {
...
#ifdef KJS_VERBOSE
                kdDebug(6070) << "KJS::HTMLCollectionProtoFunc::tryCall element.tags(" << tagName.string() << ") -> " << list.length()
                              << " items in node list" << endl;
#endif
            }
...
KJS::HTMLSelectCollection::HTMLSelectCollection(ExecState *exec, const DOM::HTMLCollection &c, const DOM::HTMLSelectElement &e)
...
OptionConstructorImp::OptionConstructorImp(ExecState *exec, const DOM::Document &d) : ObjectImp(), doc(d)
{
...
ImageConstructorImp::ImageConstructorImp(ExecState *, const DOM::Document &d) : ObjectImp(), doc(d)
{
}
