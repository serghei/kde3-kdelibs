...
public:
    DOMFunction(ExecState *exec) : InternalFunctionImp(static_cast< FunctionPrototypeImp * >(exec->interpreter()->builtinFunctionPrototype().imp()))
    {
    }
    virtual Value get(ExecState *exec, const Identifier &propertyName) const;
    virtual Value tryGet(ExecState *exec, const Identifier &propertyName) const
 ...
 */
template < class DOMObj, class KJSDOMObj > inline Value cacheDOMObject(ExecState *exec, DOMObj domObj)
{
    DOMObject *ret;
    if(domObj.isNull())
        return Null();
...
inline Value DOMObjectLookupGet(ExecState *exec, const Identifier &propertyName, const HashTable *table, const ThisImp *thisObj)
{
...
inline Value DOMObjectLookupGetValue(ExecState *exec, const Identifier &propertyName, const HashTable *table, const ThisImp *thisObj)
{
    const HashEntry *entry = Lookup::findEntry(table, propertyName);
 ...
 */
template < class ThisImp, class ParentImp >
inline void DOMObjectLookupPut(ExecState *exec, const Identifier &propertyName, const Value &value, int attr, const HashTable *table,
                               ThisImp *thisObj)
{
    const HashEntry *entry = Lookup::findEntry(table, propertyName);
...
#define KJS_DEFINE_PROTOTYPE_IMP(ClassProto, ProtoCode)                                                                                              \
    ...
    public:                                                                                                                                          \
        ...
        Value get(ExecState *exec, const Identifier &propertyName) const;                                                                            \
                                                                                                                                                     \
    protected:                                                                                                                                       \
        ClassProto(ExecState *exec) : ObjectImp(ProtoCode)                                                                                           \
        {                                                                                                                                            \
        }                                                                                                                                            \
                                                                                                                                                     \
...
#define KJS_DEFINE_PROTOTYPE(ClassProto) KJS_DEFINE_PROTOTYPE_IMP(ClassProto, exec->interpreter()->builtinObjectPrototype())

#define KJS_DEFINE_PROTOTYPE_WITH_PROTOTYPE(ClassProto, ClassProtoProto) KJS_DEFINE_PROTOTYPE_IMP(ClassProto, ClassProtoProto::self(exec))
...
#define KJS_EMPTY_PROTOTYPE_IMP(ClassName, ClassProto, ProtoCode)                                                                                    \
    ...
    protected:                                                                                                                                       \
        ...
        {                                                                                                                                            \
            if(!s_name)                                                                                                                              \
                s_name = new Identifier("[[" ClassName ".prototype]]");                                                                              \
            return s_name;                                                                                                                           \
        }                                                                                                                                            \
    };                                                                                                                                               \
...
#define IMPLEMENT_PROTOFUNC_DOM(ClassFunc)                                                                                                           \
    ...
    public:                                                                                                                                          \
        ClassFunc(ExecState *exec, int i, int len) : DOMFunction(exec), id(i)                                                                        \
        {                                                                                                                                            \
            Value protect(this);                                                                                                                     \
...
#define IMPLEMENT_CONSTANT_TABLE(Class, ClassName)                                                                                                   \
    ...
    {                                                                                                                                                \
        if(!s_name)                                                                                                                                  \
            s_name = new Identifier("[[" ClassName ".constant_table]]");                                                                             \
        return s_name;                                                                                                                               \
    }                                                                                                                                                \
