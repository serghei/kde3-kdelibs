...
public:
    Plugins(ExecState *exec, bool pluginsEnabled) : PluginBase(exec, pluginsEnabled), m_pluginsEnabled(pluginsEnabled){};
...
public:
    MimeTypes(ExecState *exec, bool pluginsEnabled) : PluginBase(exec, pluginsEnabled), m_pluginsEnabled(pluginsEnabled){};
    virtual Value get(ExecState *exec, const Identifier &propertyName) const;
    virtual const ClassInfo *classInfo() const
    {
        return &info;
...
public:
    Plugin(ExecState *exec, PluginBase::PluginInfo *info) : PluginBase(exec, true)
    {
        m_info = info;
    };
    virtual Value get(ExecState *exec, const Identifier &propertyName) const;
    ...
    Value getValueProperty(ExecState *exec, int token) const;
    PluginBase::PluginInfo *pluginInfo() const
    {
        return m_info;
    }
...
public:
    MimeType(ExecState *exec, PluginBase::MimeClassInfo *info) : PluginBase(exec, true)
    {
        m_info = info;
    };
    virtual Value get(ExecState *exec, const Identifier &propertyName) const;
...
Navigator::Navigator(ExecState *exec, KHTMLPart *p) : ObjectImp(exec->interpreter()->builtinObjectPrototype()), m_part(p)
{
}
...
Value PluginsFunc::tryCall(ExecState *exec, Object &thisObj, const List &args)
{
    ...
    if(!base->pluginsEnabled())
    {
        if(id == Plugins_Refresh || //## TODO
           id == Plugins_Item || id == Plugins_NamedItem)
            return Undefined();
    }
    else
    {
...
{
    for(PluginBase::MimeClassInfo *m = m_info->mimes.first(); m != 0; m = m_info->mimes.next())
    {
        if(m->type == name)
            return Value(new MimeType(exec, m));
    }
