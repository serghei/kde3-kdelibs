...
public:
    History(ExecState *exec, KHTMLPart *p) : ObjectImp(exec->interpreter()->builtinObjectPrototype()), part(p)
    {
    }
...
public:
    External(ExecState *exec, KHTMLPart *p) : ObjectImp(exec->interpreter()->builtinObjectPrototype()), part(p)
    {
    }
    virtual Value get(ExecState *exec, const Identifier &propertyName) const;
    virtual const ClassInfo *classInfo() const
...
public:
    FrameArray(ExecState *exec, KHTMLPart *p) : ObjectImp(exec->interpreter()->builtinObjectPrototype()), part(p)
    {
    }
    virtual Value get(ExecState *exec, const Identifier &propertyName) const;
...
Screen::Screen(ExecState *exec) : ObjectImp(exec->interpreter()->builtinObjectPrototype())
{
}
...
Window::Window(khtml::ChildFrame *p) : ObjectImp(/*no proto*/), m_frame(p), screen(0), history(0), external(0), m_frames(0), loc(0), m_evt(0)
{
...
Value Window::retrieve(KParts::ReadOnlyPart *p)
{
    ...
    {
#ifdef KJS_VERBOSE
        kdDebug(6070) << "Window::retrieve part=" << part << " '" << part->name() << "' interpreter=" << proxy->interpreter()
                      << " window=" << proxy->interpreter()->globalObject().imp() << endl;
#endif
...
ObjectImp *Window::frames(ExecState *exec) const
{
    ...
    if(part)
        return m_frames ? m_frames : (const_cast< Window * >(this)->m_frames = new FrameArray(exec, part));
    return 0L;
}
...
bool Window::hasProperty(ExecState *exec, const Identifier &p) const
{
    ...
    if(part->document().isHTMLDocument())
    { // might be XML
...
Value Window::get(ExecState *exec, const Identifier &p) const
{
    ...
    else if(!part)
    {
        ...
        if(m_frame->m_liveconnect && isSafeScript(exec) && m_frame->m_liveconnect->get(0, p.qstring(), rtype, robjid, rvalue))
    ...
    if(isSafeScript(exec) && entry)
    {
        ...
        switch(entry->value)
        {
            ...
            case _History:
                return Value(history ? history : (const_cast< Window * >(this)->history = new History(exec, part)));

            case _External:
                return Value(external ? external : (const_cast< Window * >(this)->external = new External(exec, part)));
            ...
            case OuterWidth:
            {
                ...
                return Number(entry->value == OuterHeight ? inf.geometry().height() : inf.geometry().width());
#else
                return Number(entry->value == OuterHeight ? part->view()->height() : part->view()->width());
#endif
            }
            case PageXOffset:
            ...
            case Scrollbars:
                return Undefined(); // ###
            case _Screen:
                return Value(screen ? screen : (const_cast< Window * >(this)->screen = new Screen(exec)));
            case Image:
    ...
    if(isSafeScript(exec) && part->document().isHTMLDocument())
    { // might be XML
        ...
        if(info)
        {
            ...
            if(info->nd && DOM::HTMLMappedNameCollectionImpl::matchesName(info->nd, DOM::HTMLCollectionImpl::WINDOW_NAMED_ITEMS, propertyDOMString))
            {
...
void Window::put(ExecState *exec, const Identifier &propertyName, const Value &value, int attr)
{
    ...
    if(entry && !m_frame.isNull() && !m_frame->m_part.isNull())
    {
        ...
        if(part)
        {
            switch(entry->value)
            {
                case Status:
                {
                    if(isSafeScript(exec) && part->settings()->windowStatusPolicy(part->url().host()) == KHTMLSettings::KJSWindowStatusAllow)
                    {
    ...
    if(m_frame->m_liveconnect && isSafeScript(exec) && m_frame->m_liveconnect->put(0, propertyName.qstring(), value.toString(exec).qstring()))
        return;
    if(isSafeScript(exec))
    {
...
{
    if(m_frame.isNull() || m_frame->m_part.isNull())
    { // part deleted ? can't grant access
...
// kdDebug(6070) << "JavaScript: access granted, domain is '" << actDomain.string() << "'" << endl;
    ...
    kdDebug(6070) << "WARNING: JavaScript: access denied for current frame '" << actDomain.string() << "' to frame '" << thisDomain.string() << "'"
                  << endl;
...
JSEventListener *Window::getJSEventListener(const Value &val, bool html)
{
    ...
    if(existingListener)
    {
        if(existingListener->isHTMLEventListener() != html)
            ...
            kdWarning() << "getJSEventListener: event listener already found but with html=" << !html
                        << " - please report this, we thought it would never happen" << endl;
...
void Window::goURL(ExecState *exec, const QString &url, bool lockHistory)
{
    ...
    if(active_part && part)
    {
        ...
        else
        {
            ...
            if(isSafeScript(exec) || dstUrl.find(QString::fromLatin1("javascript:"), 0, false) != 0)
...
Value Window::openWindow(ExecState *exec, const List &args)
{
    ...
    if(!str.isEmpty())
    {
        ...
        if(!p || !static_cast< DOM::DocumentImpl * >(p->htmlDocument().handle())->isURLAllowed(url.url()))
            return Undefined();
    }

    KHTMLSettings::KJSWindowOpenPolicy policy = part->settings()->windowOpenPolicy(part->url().host());
...
Value WindowFunc::tryCall(ExecState *exec, Object &thisObj, const List &args)
{
    ...
    switch(id)
    {
        ...
        case Window::Confirm:
            ...
            if(part)
                emit part->browserExtension()->requestFocus(part);
            return Boolean(
                (KMessageBox::warningYesNo(widget, QStyleSheet::convertFromPlainText(str), caption, KStdGuiItem::ok(), KStdGuiItem::cancel())
                 == KMessageBox::Yes));
        case Window::Prompt:
#ifndef KONQ_EMBEDDED
            if(!widget->dialogsAllowed())
            ...
            bool ok;
            if(args.size() >= 2)
                str2 = KInputDialog::getText(caption, QStyleSheet::convertFromPlainText(str), args[1].toString(exec).qstring(), &ok, widget);
            else
                str2 = KInputDialog::getText(caption, QStyleSheet::convertFromPlainText(str), QString::null, &ok, widget);
            if(ok)
                return String(str2);
            else
                return Null();
        ...
        case Window::GetComputedStyle:
        {
            ...
            if(arg0.nodeType() != DOM::Node::ELEMENT_NODE)
                ...
                return getDOMCSSStyleDeclaration(
                    exec, part->document().defaultView().getComputedStyle(static_cast< DOM::Element >(arg0), args[1].toString(exec).string()));
        }
        case Window::Open:
        ...
        case Window::Focus:
        {
            KHTMLSettings::KJSWindowFocusPolicy policy = part->settings()->windowFocusPolicy(part->url().host());
    ...
    switch(id)
    {
        ...
        case Window::MoveBy:
        {
            KHTMLSettings::KJSWindowMovePolicy policy = part->settings()->windowMovePolicy(part->url().host());
            if(policy == KHTMLSettings::KJSWindowMoveAllow && args.size() == 2 && widget)
            {
                ...
                if(ext)
                {
                    ...
                    if(dest.x() >= sg.x() && dest.y() >= sg.x() && dest.x() + tl->width() <= sg.width() + sg.x()
        ...
        case Window::MoveTo:
        {
            ...
            if(policy == KHTMLSettings::KJSWindowMoveAllow && args.size() == 2 && widget)
            {
                ...
                if(ext)
                {
                    ...
                    // Security check (the spec talks about UniversalBrowserWrite to disable this check...)
                    if(dest.x() >= sg.x() && dest.y() >= sg.y() && dest.x() + tl->width() <= sg.width() + sg.x()
                       && dest.y() + tl->height() <= sg.height() + sg.y())
                        emit ext->moveTopLevelWidget(dest.x(), dest.y());
                }
            }
        ...
        {
            KHTMLSettings::KJSWindowResizePolicy policy = part->settings()->windowResizePolicy(part->url().host());
            if(policy == KHTMLSettings::KJSWindowResizeAllow && args.size() == 2 && widget)
            {
                QWidget *tl = widget->topLevelWidget();
                QRect geom = tl->frameGeometry();
                window->resizeTo(tl, geom.width() + args[0].toInt32(exec), geom.height() + args[1].toInt32(exec));
            }
            return Undefined();
        }
...
ScheduledAction::ScheduledAction(Object _func, List _args, DateTimeMS _nextTime, int _interval, bool _singleShot, int _timerId)
{
...
WindowQObject::WindowQObject(Window *w) : parent(w)
{
    ...
    if(!parent->m_frame)
        ...
        connect(parent->m_frame, SIGNAL(destroyed()), this, SLOT(parentDestroyed()));
    pausedTime = 0;
    lastTimerId = 0;
...
Value LocationFunc::tryCall(ExecState *exec, Object &thisObj, const List &args)
{
    ...
    switch(id)
    {
        ...
        case Location::Replace:
            Window::retrieveWindow(part)->goURL(exec, args[0].toString(exec).qstring(), id == Location::Replace);
            break;
...
Value ExternalFunc::tryCall(ExecState *exec, Object &thisObj, const List &args)
{
    ...
    switch(id)
    {
        case External::AddFavorite:
        {
            ...
            QString question;
            if(title.isEmpty())
                question = i18n("Do you want a bookmark pointing to the location \"%1\" to be added to your collection?").arg(url);
            else
                question =
                    i18n("Do you want a bookmark pointing to the location \"%1\" titled \"%2\" to be added to your collection?").arg(url).arg(title);
            ...
            if(KMessageBox::warningYesNo(widget, question, caption, i18n("Insert"), i18n("Disallow")) == KMessageBox::Yes)
            {
