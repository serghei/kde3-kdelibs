...
int main(int argc, char *argv[])
{
    ...
    KCmdLineArgs::init(argc, argv, "testkhtml", "Testkhtml", "a basic web browser using the KHTML library", "1.0");
    ...
    QObject::connect(doc->browserExtension(), SIGNAL(openURLRequest(const KURL &, const KParts::URLArgs &)), dummy,
    ...
    QWidget::connect(doc, SIGNAL(setWindowCaption(const QString &)), doc->widget()->topLevelWidget(), SLOT(setCaption(const QString &)));
    doc->widget()->show();
    toplevel->show();
...
void Dummy::doBenchmark()
{
    ...
    QString directory =
        KFileDialog::getExistingDirectory(settings.readPathEntry("path"), m_part->view(), QString::fromLatin1("Please select directory with tests"));
