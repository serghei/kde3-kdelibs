...
void KHTMLPartBrowserExtension::editableWidgetBlurred(QWidget * /*widget*/)
{
    ...
    {
        disconnect(QApplication::clipboard(), SIGNAL(dataChanged()), this, SLOT(updateEditActions()));

        if(oldWidget)
        {
            if(oldWidget->inherits("QLineEdit") || oldWidget->inherits("QTextEdit"))
                disconnect(oldWidget, SIGNAL(selectionChanged()), this, SLOT(updateEditActions()));
        }
...
void KHTMLPartBrowserExtension::setExtensionProxy(KParts::BrowserExtension *proxy)
{
    if(m_extensionProxy)
    {
        disconnect(m_extensionProxy, SIGNAL(enableAction(const char *, bool)), this, SLOT(extensionProxyActionEnabled(const char *, bool)));
        if(m_extensionProxy->inherits("KHTMLPartBrowserExtension"))
        {
            disconnect(m_extensionProxy, SIGNAL(editableWidgetFocused()), this, SLOT(extensionProxyEditableWidgetFocused()));
            disconnect(m_extensionProxy, SIGNAL(editableWidgetBlurred()), this, SLOT(extensionProxyEditableWidgetBlurred()));
        }
    }
    ...
    if(m_extensionProxy)
    {
        connect(m_extensionProxy, SIGNAL(enableAction(const char *, bool)), this, SLOT(extensionProxyActionEnabled(const char *, bool)));
        if(m_extensionProxy->inherits("KHTMLPartBrowserExtension"))
        {
            connect(m_extensionProxy, SIGNAL(editableWidgetFocused()), this, SLOT(extensionProxyEditableWidgetFocused()));
            connect(m_extensionProxy, SIGNAL(editableWidgetBlurred()), this, SLOT(extensionProxyEditableWidgetBlurred()));
        }
...
void KHTMLPartBrowserExtension::extensionProxyActionEnabled(const char *action, bool enable)
{
    ...
    if(strcmp(action, "cut") == 0 || strcmp(action, "copy") == 0 || strcmp(action, "paste") == 0)
    {
...
KHTMLPopupGUIClient::KHTMLPopupGUIClient(KHTMLPart *khtml, const QString &doc, const KURL &url) : QObject(khtml)
{
    ...
    if(!e.isNull() && (e.elementId() == ID_IMG || (e.elementId() == ID_INPUT && !static_cast< DOM::HTMLInputElement >(e).src().isEmpty())))
    {
    ...
    if(hasSelection)
    {
        ...
        new KAction(i18n("Search for '%1' with %2").arg(selectedText, name), icon, 0, d->m_khtml->browserExtension(), SLOT(searchProvider()),
                    actionCollection(), "searchProvider");
        ...
        if(!favoriteEngines.isEmpty())
        {
            ...
            for(; it != favoriteEngines.end(); ++it)
            {
                ...
                if(KURIFilter::self()->filterURI(data, list))
                {
                    ...
                    providerList->insert(new KAction(name, icon, 0, d->m_khtml->browserExtension(), SLOT(searchProvider()), actionCollection(),
                                                     QString("searchProvider" + searchProviderPrefix).latin1()));
                }
            }
        }
        ...
        if(selectedText.contains("://") && KURL(selectedText).isValid())
            new KAction(i18n("Open '%1'").arg(selectedText), "window_new", 0, d->m_khtml->browserExtension(), SLOT(openSelection()),
                        actionCollection(), "openSelection");
    }
    else if(url.isEmpty() && !isImage)
    {
        ...
        new KAction(i18n("Stop Animations"), 0, this, SLOT(slotStopAnimations()), actionCollection(), "stopanimations");
    }
    ...
    if(!url.isEmpty())
    {
        ...
        {
            new KAction(i18n("Copy Email Address"), 0, this, SLOT(slotCopyLinkLocation()), actionCollection(), "copylinklocation");
        }
        else
        {
            new KAction(i18n("&Save Link As..."), 0, this, SLOT(slotSaveLinkAs()), actionCollection(), "savelinkas");
            new KAction(i18n("Copy &Link Address"), 0, this, SLOT(slotCopyLinkLocation()), actionCollection(), "copylinklocation");
        }
    }
    ...
    if(!hasSelection)
    {
        if(khtml->parentPart())
        {
            new KAction(i18n("Open in New &Window"), "window_new", 0, this, SLOT(slotFrameInWindow()), actionCollection(), "frameinwindow");
            new KAction(i18n("Open in &This Window"), 0, this, SLOT(slotFrameInTop()), actionCollection(), "frameintop");
            new KAction(i18n("Open in &New Tab"), "tab_new", 0, this, SLOT(slotFrameInTab()), actionCollection(), "frameintab");
            new KAction(i18n("Reload Frame"), 0, this, SLOT(slotReloadFrame()), actionCollection(), "reloadframe");
            ...
            new KAction(i18n("View Frame Source"), 0, d->m_khtml, SLOT(slotViewDocumentSource()), actionCollection(), "viewFrameSource");
    ...
    if(isImage)
    {
        ...
        new KAction(i18n("Save Image As..."), 0, this, SLOT(slotSaveImageAs()), actionCollection(), "saveimageas");
        new KAction(i18n("Send Image..."), 0, this, SLOT(slotSendImage()), actionCollection(), "sendimage");


#ifndef QT_NO_MIMECLIPBOARD
        (new KAction(i18n("Copy Image"), 0, this, SLOT(slotCopyImage()), actionCollection(), "copyimage"))->setEnabled(!d->m_pixmap.isNull());
#endif

        if(d->m_pixmap.isNull())
        { // fallback to image location if still loading the image.  this will always be true if ifdef QT_NO_MIMECLIPBOARD
            new KAction(i18n("Copy Image Location"), 0, this, SLOT(slotCopyImageLocation()), actionCollection(), "copyimagelocation");
        }

        QString name = KStringHandler::csqueeze(d->m_imageURL.fileName() + d->m_imageURL.query(), 25);
        new KAction(
            i18n("View Image (%1)").arg(d->m_suggestedFilename.isEmpty() ? name.replace("&", "&&") : d->m_suggestedFilename.replace("&", "&&")), 0,
            this, SLOT(slotViewImage()), actionCollection(), "viewimage");

        if(KHTMLFactory::defaultHTMLSettings()->isAdFilterEnabled())
        {
            new KAction(i18n("Block Image..."), 0, this, SLOT(slotBlockImage()), actionCollection(), "blockimage");

            if(!d->m_imageURL.host().isEmpty() && !d->m_imageURL.protocol().isEmpty())
            {
                new KAction(i18n("Block Images From %1").arg(d->m_imageURL.host()), 0, this, SLOT(slotBlockHost()), actionCollection(), "blockhost");
            }
        }
    }
...
void KHTMLPopupGUIClient::slotBlockImage()
{
    ...
    QString url = KInputDialog::getText(i18n("Add URL to Filter"), i18n("Enter the URL:"), d->m_imageURL.url(), &ok);
    if(ok)
    {
...
{
    bool ok = false;
    QString url = KInputDialog::getText(i18n("Add URL to Filter"), i18n("Enter the URL:"), d->m_khtml->url().url(), &ok);
    if(ok)
    {
        KHTMLFactory::defaultHTMLSettings()->addAdFilter(url);
        d->m_khtml->reparseConfiguration();
...
void KHTMLPopupGUIClient::saveURL(QWidget *parent, const QString &caption, const KURL &url, const QMap< QString, QString > &metadata,
                                  const QString &filter, long cacheId, const QString &suggestedFilename)
{
    ...
    do
    {
        ...
        if(destURL.isLocalFile())
        {
            ...
            if(info.exists())
            {
                ...
                query = KMessageBox::warningContinueCancel(parent, i18n("A file named \"%1\" already exists. "
...
void KHTMLPopupGUIClient::saveURL(const KURL &url, const KURL &destURL, const QMap< QString, QString > &metadata, long cacheId)
{
    if(destURL.isValid())
    {
        bool saved = false;
            ...
            if(!url.isLocalFile())
            {
                ...
                if(!downloadManger.isEmpty())
                {
                    ...
                    else
                    {
                        ...
                        cmd += " " + KProcess::quote(url.url()) + " " + KProcess::quote(cleanDest.url());
...
KHTMLPartBrowserHostExtension::KHTMLPartBrowserHostExtension(KHTMLPart *part) : KParts::BrowserHostExtension(part)
{
    m_part = part;
}
...
// BCI: remove in KDE 4
KHTMLZoomFactorAction::KHTMLZoomFactorAction(KHTMLPart *part, bool direction, const QString &text, const QString &icon, const QObject *receiver,
...
KHTMLZoomFactorAction::KHTMLZoomFactorAction(KHTMLPart *part, bool direction, const QString &text, const QString &icon, const KShortcut &cut,
