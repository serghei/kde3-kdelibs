...
KMultiPart::KMultiPart(QWidget *parentWidget, const char *widgetName, QObject *parent, const char *name, const QStringList &)
...
void KMultiPart::setPart(const QString &mimeType)
{
    ...
    m_part = KParts::ComponentFactory::createPartInstanceFromQuery< KParts::ReadOnlyPart >(m_mimeType, QString::null, widget(), 0L, this, 0L);
    if(!m_part)
    {
    ...
    connect(m_part, SIGNAL(completed()), this, SLOT(slotPartCompleted()));
    ...
    if(childExtension)
    {
        ...
        connect(childExtension, SIGNAL(openURLNotify()), m_extension, SIGNAL(openURLNotify()));

        connect(childExtension, SIGNAL(openURLRequestDelayed(const KURL &, const KParts::URLArgs &)), m_extension,
                SIGNAL(openURLRequest(const KURL &, const KParts::URLArgs &)));

        connect(childExtension, SIGNAL(createNewWindow(const KURL &, const KParts::URLArgs &)), m_extension,
        ...
        connect(childExtension, SIGNAL(popupMenu(const QPoint &, const KFileItemList &)), m_extension,
                SIGNAL(popupMenu(const QPoint &, const KFileItemList &)));
        connect(childExtension, SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KFileItemList &)), m_extension,
                SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KFileItemList &)));
        connect(
            childExtension,
            SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KFileItemList &, const KParts::URLArgs &, KParts::BrowserExtension::PopupFlags)),
            m_extension,
            SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KFileItemList &, const KParts::URLArgs &, KParts::BrowserExtension::PopupFlags)));
        connect(childExtension, SIGNAL(popupMenu(const QPoint &, const KURL &, const QString &, mode_t)), m_extension,
                SIGNAL(popupMenu(const QPoint &, const KURL &, const QString &, mode_t)));
        connect(childExtension, SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KURL &, const QString &, mode_t)), m_extension,
        ...
        if(m_isHTMLPart)
            connect(childExtension, SIGNAL(infoMessage(const QString &)), m_extension, SIGNAL(infoMessage(const QString &)));
        // For non-HTML we prefer to show our infoMessage ourselves.

        childExtension->setBrowserInterface(m_extension->browserInterface());

        connect(childExtension, SIGNAL(enableAction(const char *, bool)), m_extension, SIGNAL(enableAction(const char *, bool)));
        connect(childExtension, SIGNAL(setLocationBarURL(const QString &)), m_extension, SIGNAL(setLocationBarURL(const QString &)));
        connect(childExtension, SIGNAL(setIconURL(const KURL &)), m_extension, SIGNAL(setIconURL(const KURL &)));
        connect(childExtension, SIGNAL(loadingProgress(int)), m_extension, SIGNAL(loadingProgress(int)));
        if(m_isHTMLPart) // for non-HTML we have our own
            connect(childExtension, SIGNAL(speedProgress(int)), m_extension, SIGNAL(speedProgress(int)));
        connect(childExtension, SIGNAL(selectionInfo(const KFileItemList &)), m_extension, SIGNAL(selectionInfo(const KFileItemList &)));
        connect(childExtension, SIGNAL(selectionInfo(const QString &)), m_extension, SIGNAL(selectionInfo(const QString &)));
        connect(childExtension, SIGNAL(selectionInfo(const KURL::List &)), m_extension, SIGNAL(selectionInfo(const KURL::List &)));
        connect(childExtension, SIGNAL(mouseOverInfo(const KFileItem *)), m_extension, SIGNAL(mouseOverInfo(const KFileItem *)));
        connect(childExtension, SIGNAL(moveTopLevelWidget(int, int)), m_extension, SIGNAL(moveTopLevelWidget(int, int)));
        connect(childExtension, SIGNAL(resizeTopLevelWidget(int, int)), m_extension, SIGNAL(resizeTopLevelWidget(int, int)));
    }
...
{
    KAboutData *aboutData = new KAboutData("kmultipart", I18N_NOOP("KMultiPart"), "0.1", I18N_NOOP("Embeddable component for multipart/mixed"),
                                           KAboutData::License_GPL, "(c) 2001, David Faure <david@mandrakesoft.com>");
    return aboutData;
}
