...
static inline bool isIndicatedInlineBox(InlineBox *box)
{
    ...
    return s->borderLeftWidth() || s->borderRightWidth() || s->borderTopWidth() || s->borderBottomWidth() || s->paddingLeft().value()
           || s->paddingRight().value() || s->paddingTop().value() || s->paddingBottom().value()
...
static inline bool isIndicatedFlow(RenderObject *r)
{
    ...
    return s->borderLeftStyle() != BNONE || s->borderRightStyle() != BNONE || s->borderTopStyle() != BNONE || s->borderBottomStyle() != BNONE
           //	|| s->paddingLeft().value() || s->paddingRight().value()
           //	|| s->paddingTop().value() || s->paddingBottom().value()
           //	|| s->marginLeft().value() || s->marginRight().value()
           || s->hasClip() || s->hidesOverflow() || s->backgroundColor().isValid() || s->backgroundImage();
}
...
static inline bool isUnsuitable(RenderObject *r, ObjectTraversalState trav)
{
    if(!r)
        return false;
    return r->isTableCol() || r->isTableSection() || r->isTableRow() || (r->isText() && static_cast< RenderText * >(r)->inlineTextBoxCount() == 0);
    ;
    Q_UNUSED(trav);
}
 ...
 */
static inline RenderObject *advanceSuitableObject(RenderObject *r, ObjectTraversalState &trav, bool toBegin, RenderObject *base, int &state)
{
    do
    {
 ...
 */
void /*KDE_NO_EXPORT*/ mapDOMPosToRenderPos(NodeImpl *node, long offset, RenderObject *&r, long &r_ofs, bool &outside, bool &outsideEnd)
{
    ...
    {
        r = 0;
        kdWarning() << k_funcinfo << "Mapping from nodes of type " << node->nodeType() << " not supported!" << endl;
    }
}
 ...
 */
void /*KDE_NO_EXPORT*/ mapRenderPosToDOMPos(RenderObject *r, long r_ofs, bool outside, bool outsideEnd, NodeImpl *&node, long &offset)
{
    node = r->element();
    Q_ASSERT(node);
#if DEBUG_CARETMODE > 5
    kdDebug(6200) << "mapRTD: r " << r << "@" << (r ? r->renderName() : QString::null)
                  << (r && r->element() ? QString(".node ") + QString::number((unsigned)r->element(), 16) + "@" + r->element()->nodeName().string()
                                        : QString::null)
                  << " outside " << outside << " outsideEnd " << outsideEnd << endl;
#endif
    ...
    {
        offset = 0;
        kdWarning() << k_funcinfo << "Mapping to nodes of type " << node->nodeType() << " not supported!" << endl;
    }
}
 ...
 */
static inline void mapRenderPosToTraversalState(bool outside, bool atEnd, bool toBegin, ObjectTraversalState &trav)
{
    if(!outside)
        atEnd = !toBegin;
    if(!atEnd ^ toBegin)
 ...
 * @param atEnd whether the caret position is at the end
 */
static inline void mapTraversalStateToRenderPos(ObjectTraversalState trav, bool toBegin, bool &outside, bool &atEnd)
{
    outside = false;
    switch(trav)
    {
        case OutsideDescending:
            outside = true; // fall through
        case InsideDescending:
            atEnd = toBegin;
            break;
        case OutsideAscending:
...
#if DEBUG_CARETMODE > 2
    kdDebug(6200) << "findRenderer: node " << node << " " << (node ? node->nodeName().string() : QString::null) << " offset " << offset << " r " << r
                  << "[" << (r ? r->renderName() : QString::null) << "] r_ofs " << r_ofs << " outside " << outside << " outsideEnd " << outsideEnd
                  << endl;
#endif
    if(r)
        return r;
...
void CaretBoxLine::addConvertedInlineBox(InlineBox *box, SeekBoxParams &sbp) /*KDE_NO_EXPORT*/
{
    ...
    for(; box; box = box->nextOnLine())
    {
        ...
        kdDebug(6200) << "x " << box->m_x << " y " << box->m_y << " w " << box->m_width << " h " << box->m_height << " baseline " << box->m_baseline
                      << " ifb " << box->isInlineFlowBox() << " itb " << box->isInlineTextBox() << " rlb " << box->isRootInlineBox() << endl;
#endif
        ...
        RenderStyle *ps = box->parent() && box->parent()->object() ? box->parent()->object()->style(box->parent()->m_firstLine) : s;
        ...
        {
#if DEBUG_ACIB
            kdDebug(6200) << "isinlinetextbox " << box
                          << (box->object()
                                  ? QString(" contains \"%1\"")
                                        .arg(QConstString(static_cast< RenderText * >(box->object())->str->s + box->minOffset(),
                                                          kMin(box->maxOffset() - box->minOffset(), 15L))
                                                 .string())
                                  : QString::null)
                          << endl;
#endif
...
CaretBoxLine *CaretBoxLine::constructCaretBoxLine(CaretBoxLineDeleter *deleter, InlineFlowBox *basicFlowBox, InlineBox *seekBox, bool seekOutside,
...
CaretBoxLine *CaretBoxLine::constructCaretBoxLine(CaretBoxLineDeleter *deleter, RenderBox *cb, bool outside, bool outsideEnd,
                                                  CaretBoxIterator &iter) /*KDE_NO_EXPORT*/
{
    int _x = cb->xPos();
    int _y = cb->yPos();
    int height;
    ...
    if(outside)
    {
        ...
        RenderStyle *s = cb->element() && cb->element()->parent() && cb->element()->parent()->renderer()
    ...
    deleter->append(result);
    result->caret_boxes.append(new CaretBox(_x, _y, width, height, cb, outside, outsideEnd));
    iter = result->begin();
    return result;
}
 ...
 */
static CaretBoxLine *findCaretBoxLine(DOM::NodeImpl *node, long offset, CaretBoxLineDeleter *cblDeleter, RenderObject *base, long &r_ofs,
                                      CaretBoxIterator &caretBoxIt)
{
    ...
    kdDebug(6200) << "node " << node << " offset: " << offset << " r " << r->renderName() << "[" << r << "].node "
                  << r->element()->nodeName().string() << "[" << r->element() << "]"
                  << " r_ofs " << r_ofs << " outside " << outside << " outsideEnd " << outsideEnd << endl;
#endif
...
#if DEBUG_CARETMODE > 2
            kdDebug(6200) << "text-box b: " << b << " baseFlowBox: " << baseFlowBox
                          << (b && b->object()
                                  ? QString(" contains \"%1\"")
                                        .arg(QConstString(static_cast< RenderText * >(b->object())->str->s + b->minOffset(),
                                                          kMin(b->maxOffset() - b->minOffset(), 15L))
                                                 .string())
                                  : QString::null)
                          << endl;
#endif
...
#endif
            return CaretBoxLine::constructCaretBoxLine(cblDeleter, baseFlowBox, b, outside, outsideEnd, caretBoxIt);
        } while(false); /*end if*/
...
#endif
            Q_ASSERT(r->isBox());
            return CaretBoxLine::constructCaretBoxLine(cblDeleter, static_cast< RenderBox * >(r), outside, outsideEnd, caretBoxIt);
        } /*end if*/

        kdDebug(6200) << "firstlinebox " << firstLineBox << endl;
        InlineFlowBox *baseFlowBox = seekBaseFlowBox(firstLineBox, base);
        return CaretBoxLine::constructCaretBoxLine(cblDeleter, baseFlowBox, firstLineBox, outside, outsideEnd, caretBoxIt);
    } /*end if*/
...
//    kdWarning() << "containing block contains no inline flow boxes!!! crash imminent" << endl;
...
#endif
        return CaretBoxLine::constructCaretBoxLine(cblDeleter, cb, outside, outsideEnd, caretBoxIt);
    } /*end if*/
    ...
    for(; flowBox; flowBox = static_cast< InlineFlowBox * >(flowBox->nextLineBox()))
    {
        ...
        CaretBoxLine *cbl = CaretBoxLine::constructCaretBoxLine(cblDeleter, baseFlowBox, 0, outside, outsideEnd, caretBoxIt, r);
#if DEBUG_CARETMODE > 5
...
static bool containsEditableElement(KHTMLPart *part, RenderBlock *cb, RenderTable *&table, bool fromEnd = false)
{
    ...
    do
    {
        ...
        kdDebug(6201) << "cee: r " << (r ? r->renderName() : QString::null) << "@" << r << " cb " << cb << " withinCb " << withinCb << " modWithinCb "
                      << modWithinCb << " tempTable " << tempTable << endl;
#endif
        if(r && modWithinCb && r->element() && !isUnsuitable(r, trav)
           && (part->isCaretMode() || part->isEditable() || r->style()->userInput() == UI_ENABLED))
        {
            table = tempTable;
#if DEBUG_CARETMODE > 1
...
#if DEBUG_CARETMODE > 1
        kdDebug(6201) << "cece: r " << (r ? r->renderName() : QString::null) << "@" << r << " cb " << cb << " withinCb " << withinCb
                      << " modWithinCb " << modWithinCb << " tempTable " << tempTable << endl;
#endif
...
LinearDocument::LinearDocument(KHTMLPart *part, NodeImpl *node, long offset, CaretAdvancePolicy advancePolicy, ElementImpl *baseElem)
    : node(node), offset(offset), m_part(part), advPol(advancePolicy), base(0)
{
    if(node == 0)
        return;
...
LineIterator::LineIterator(LinearDocument *l, DOM::NodeImpl *node, long offset) : lines(l)
{
    ...
    cbl = findCaretBoxLine(node, offset, &lines->cblDeleter, l->baseObject(), currentOffset, currentBox);
// can happen on partially loaded documents
#if DEBUG_CARETMODE > 0
    if(!cbl)
        kdDebug(6200) << "no render object found!" << endl;
#endif
    if(!cbl)
        return;
#if DEBUG_CARETMODE > 1
...
void LineIterator::nextBlock()
{
...

    {
...
#if DEBUG_CARETMODE > 1
        kdDebug(6200) << "nextBlock: before adv r" << r << " " << (r ? r->renderName() : QString::null)
                      << (r && r->isText() ? " contains \"" + QString(((RenderText *)r)->str->s, QMIN(((RenderText *)r)->str->l, 15)) + "\""
                                           : QString::null)
                      << " trav " << trav << " cb_outside " << cb_outside << " cb_outside_end " << cb_outside_end << endl;
#endif
        r = advanceSuitableObject(r, trav, false, base, state);
        if(!r)
        {
...
#if DEBUG_CARETMODE > 1
        kdDebug(6200) << "nextBlock: after r" << r << " trav " << trav << " cb_outside " << cb_outside << " cb_outside_end " << cb_outside_end
                      << endl;
#endif
#if DEBUG_CARETMODE > 0
        ...
        if(r->isRenderBlock() || isrepl)
        {
            ...
            cbl = CaretBoxLine::constructCaretBoxLine(&lines->cblDeleter, cb, cb_outside, cb_outside_end, currentBox);
        ...
        InlineFlowBox *flowBox = cb->firstLineBox();
#if DEBUG_CARETMODE > 0
        kdDebug(6200) << "++: flowBox " << flowBox << " cb " << cb << "["
                      << (cb
                              ? cb->renderName() + QString(".node ") + QString::number((unsigned)cb->element(), 16)
        ...
        CaretBoxIterator it;
        cbl = CaretBoxLine::constructCaretBoxLine(&lines->cblDeleter, flowBox, flowBox->firstChild(), seekOutside, seekOutsideEnd, it);
    }
}
...
#if DEBUG_CARETMODE > 1
        kdDebug(6200) << "prevBlock: before adv r" << r << " " << (r ? r->renderName() : QString::null)
                      << (r && r->isText() ? " contains \"" + QString(((RenderText *)r)->str->s, QMIN(((RenderText *)r)->str->l, 15)) + "\""
                                           : QString::null)
                      << " trav " << trav << " cb_outside " << cb_outside << " cb_outside_end " << cb_outside_end << endl;
#endif
...
#if DEBUG_CARETMODE > 1
        kdDebug(6200) << "prevBlock: after r" << r << " trav " << trav << " cb_outside " << cb_outside << " cb_outside_end " << cb_outside_end
                      << endl;
#endif
#if DEBUG_CARETMODE > 0
        kdDebug(6200) << "--: r " << r << "[" << (r ? r->renderName() : QString::null) << "]" << endl;
...
#if DEBUG_CARETMODE > 4
//       kdDebug(6200) << "ebit::advance: before: " << (**this)->object() << "@" << (**this)->object()->renderName() << ".node " <<
//       (**this)->object()->element() << "[" << ((**this)->object()->element() ? (**this)->object()->element()->nodeName().string() : QString::null)
//       << "] inline " << (**this)->isInline() << " outside " << (**this)->isOutside() << " outsideEnd " << (**this)->isOutsideEnd() << endl;
#endif
    ...
    while(!curAtEnd)
    {
        ...
        {
#if DEBUG_CARETMODE > 3
            kdDebug(6200) << "ebit::advance: " << (*curbox)->object() << "@" << (*curbox)->object()->renderName() << ".node "
                          << (*curbox)->object()->element() << "["
                          << ((*curbox)->object()->element() ? (*curbox)->object()->element()->nodeName().string() : QString::null) << "] inline "
                          << (*curbox)->isInline() << " outside " << (*curbox)->isOutside() << " outsideEnd " << (*curbox)->isOutsideEnd() << endl;
#endif
...
#if DEBUG_CARETMODE > 5
                kdDebug(6200) << "prev " << prev << " haslast " << haslast << " islastuseable " << islastuseable << " left " << left << " next "
                              << next << " hascoming " << hascoming << " iscominguseable " << iscominguseable << " right " << right
...
// inside boxes are *always* valid
                ...
                if(box->isInline())
                {
                    InlineBox *ibox = box->inlineBox();
                    kdDebug(6200) << "inside "
                                  << (!ibox->isInlineFlowBox() || static_cast< InlineFlowBox * >(ibox)->firstChild() ? "non-empty" : "empty")
                                  << (isIndicatedInlineBox(ibox) ? " indicated" : "") << " adjacent=" << adjacent << endl;
                }
#if 0
...
bool EditableCaretBoxIterator::isEditable(const CaretBoxIterator &boxit, bool fromEnd)
{
    ...
    kdDebug(6200) << "isEditable r" << r << ": " << (r ? r->renderName() : QString::null)
                  << (r && r->isText() ? " contains \"" + QString(((RenderText *)r)->str->s, QMIN(((RenderText *)r)->str->l, 15)) + "\""
                                       : QString::null)
                  << endl;
#endif
...
void EditableLineIterator::advance(bool toBegin)
{
    ...
    while(cbl)
    {
        ...
        {
#if DEBUG_CARETMODE > 3
            kdDebug(6200) << "advance: " << cbl->enclosingObject() << "@" << cbl->enclosingObject()->renderName() << ".node "
                          << cbl->enclosingObject()->element() << "["
...
static inline bool isCaretBoxEmpty(CaretBox *box)
{
    if(!box->isInline())
        return false;
    InlineBox *ibox = box->inlineBox();
    return ibox->isInlineFlowBox() && !static_cast< InlineFlowBox * >(ibox)->firstChild() && !isIndicatedInlineBox(ibox);
}
...
EditableCharacterIterator &EditableCharacterIterator::operator++()
{
    ...
    if(!_end && ebit != (*_it)->end())
    {
        CaretBox *box = *ebit;
        RenderObject *_r = box->object();
        kdDebug(6200) << "echit++(1): box " << box
                      << (box && box->isInlineTextBox()
                              ? QString(" contains \"%1\"")
                                    .arg(QConstString(static_cast< RenderText * >(box->object())->str->s + box->minOffset(),
                                                      box->maxOffset() - box->minOffset())
                                             .string())
                              : QString::null)
                      << " _r " << (_r ? _r->element()->nodeName().string() : QString("<nil>")) << endl;
    }
#endif
    return *this;
}
...
EditableCharacterIterator &EditableCharacterIterator::operator--()
{
    ...
    prev:
        ...
        if(!_peekPrev)
        {
            ...
            --ebit;
            if(ebit == (*_it)->preBegin())
            { // end of line reached, go to previous line
                --_it;
#if DEBUG_CARETMODE > 3
                kdDebug(6200) << "--_it" << endl;
#endif
...
// 	  RenderObject *r = box->object();
                    ...
                    _char = -1;
#if DEBUG_CARETMODE > 0
                    kdDebug(6200) << "echit--(2): box " << box << " b " << box->inlineBox()
                                  << (box->isInlineTextBox()
                                          ? QString(" contains \"%1\"")
                                                .arg(QConstString(static_cast< RenderText * >(box->object())->str->s + box->minOffset(),
                                                                  box->maxOffset() - box->minOffset())
...
#if DEBUG_CARETMODE > 0
        bool adjacent = ebit.isAdjacent();
        kdDebug(6200) << "adjacent " << adjacent << " _peekNext " << _peekNext
                      << " _peekNext->isInlineTextBox: " << (_peekNext ? _peekNext->isInlineTextBox() : false)
                      << " !((*ebit)->isInlineTextBox): " << (*ebit ? !(*ebit)->isInlineTextBox() : true) << endl;
#endif
#if 0
...
#endif
#if DEBUG_CARETMODE > 0
        kdDebug(6200) << "(*ebit)->obj " << (*ebit)->object()->renderName() << "[" << (*ebit)->object() << "]"
                      << " minOffset: " << (*ebit)->minOffset() << " maxOffset: " << (*ebit)->maxOffset() << endl;
#endif
#if DEBUG_CARETMODE > 3
...
//     if (!_it.outside) _offset = (*ebit)->maxOffset()/* - adjacent*/;
    ...
    {
        CaretBox *box = *ebit;
        kdDebug(6200) << "echit--(1): box " << box << " b " << box->inlineBox()
                      << (box->isInlineTextBox()
                              ? QString(" contains \"%1\"")
                                    .arg(QConstString(static_cast< RenderText * >(box->object())->str->s + box->minOffset(),
                                                      box->maxOffset() - box->minOffset())
...
TableRowIterator::TableRowIterator(RenderTable *table, bool fromEnd, RenderTableSection::RowStruct *row) : sec(table, fromEnd)
{
    // set index
    if(*sec)
    {
 ...
 */
static inline RenderTableCell *findNearestTableCell(KHTMLPart *part, int x, TableRowIterator &it, bool fromEnd)
{
...
inline void ErgonomicEditableLineIterator::calcAndStoreNewLine(RenderBlock *newBlock, bool toBegin)
{
    // take the first/last editable element in the found cell as the new
    // value for the iterator
    CaretBoxIterator it;
    cbl = CaretBoxLine::constructCaretBoxLine(&lines->cblDeleter, newBlock, true, toBegin, it);
#if DEBUG_CARETMODE > 3
...
void ErgonomicEditableLineIterator::determineTopologicalElement(RenderTableCell *oldCell, RenderObject *newObject, bool toBegin)
{
    ...
    if(!cell)
    {
        ...
        RenderTable *nestedTable;
        bool editableChild = cell && containsEditableChildElement(lines->m_part, cell, nestedTable, toBegin, section->table());
...
//	kdDebug(6200) << "approximate r" << r << ": " << (r ? r->renderName() : QString::null) << (r && r->isText() ? " contains \"" +
// QString(((RenderText *)r)->str->s, ((RenderText *)r)->str->l) + "\"" : QString::null) << endl;
#endif
        xPos = b->xPos();
...
#if DEBUG_CARETMODE > 0
        if(!it.isEnd())
            kdDebug(6200) << "reading2 '" << (*it).latin1() << "' (" << (int)(*it).latin1() << ") box " << it.caretBox() << endl;
#endif
    } while(!it.isEnd() && !(*it).isSpace() && !(*it).isPunct());
...
#if DEBUG_CARETMODE > 1
    if(!it.isEnd())
        kdDebug(6200) << "effective '" << (*it).latin1() << "' (" << (int)(*it).latin1() << ") box " << it.caretBox() << endl;
#endif
}
