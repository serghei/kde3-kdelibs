...
KJavaApplet::KJavaApplet(KJavaAppletWidget *_parent, KJavaAppletContext *_context) : params()
{
...
void KJavaApplet::stateChange(const int newStateInt)
{
    ...
    {
        kdError(6100) << "KJavaApplet::stateChange : don't want to switch from state " << d->state << " to " << newState << endl;
    }
}
...
{
    return (!d->failed && d->state >= INSTANCIATED && d->state < STOPPED);
}
