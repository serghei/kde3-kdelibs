...
class JSStackFrame {
public:
    JSStackFrame(JSStack &stack, QStringList &a) : jsstack(stack), args(a), ticket(counter++), ready(false), exit(false)
    {
...
KJavaAppletServer::KJavaAppletServer()
{
    ...
    connect(process, SIGNAL(received(const QByteArray &)), this, SLOT(slotJavaRequest(const QByteArray &)));
...
void KJavaAppletServer::setupJava(KJavaProcess *p)
{
    ...
    if(config.readBoolEntry("UseSecurityManager", true))
    {
        ...
        p->setSystemProperty("java.security.manager", "org.kde.kjas.server.KJASSecurityManager");
    }
...
bool KJavaAppletServer::createApplet(int contextId, int appletId, const QString &name, const QString &clazzName, const QString &baseURL,
                                     const QString &user, const QString &password, const QString &authname, const QString &codeBase,
                                     const QString &jarFile, QSize size, const QMap< QString, QString > &params, const QString &windowTitle)
{
...
void KJavaAppletServer::slotJavaRequest(const QByteArray &qb)
{
    ...
    switch(cmd_code)
    {
        ...
        case KJAS_JAVASCRIPT_EVENT:
            ...
            kdDebug(6100) << "Javascript request: " << contextID << " code: " << args[0] << endl;
            break;
                    ...
                    if(cert)
                    {
                        ...
                        switch(cert->validate())
                        {
                            ...
                            case KSSLCertificate::InvalidPurpose:
                                text += i18n("InvalidPurpose");
                                break;
                            case KSSLCertificate::PathLengthExceeded:
                                text += i18n("PathLengthExceeded");
                                break;
                            ...
                            case KSSLCertificate::SelfSigned:
                                text += i18n("SelfSigned");
                                break;
                            case KSSLCertificate::ErrorReadingRoot:
                                text += i18n("ErrorReadingRoot");
                                break;
...
PermissionDialog::PermissionDialog(QWidget *parent) : QObject(parent), m_button("no")
{
}
