...
public:
    ...
    CaretBox(int x, int y, int w, int h, RenderBox *cb, bool outside, bool outsideEnd)
        : _box(0), _w((short)w), _h(h), _x(x), _y(y), cb(cb), _outside(outside), outside_end(outsideEnd)
    {
    }
...
public:
    ...
    ~CaretBoxLine()
    {
        kdDebug(6200) << k_funcinfo << "called" << endl;
    }
#endif
     ...
     */
    static CaretBoxLine *constructCaretBoxLine(MassDeleter< CaretBoxLine > *deleter, InlineFlowBox *baseFlowBox, InlineBox *seekBox, bool seekOutside,
                                               bool seekOutsideEnd, CaretBoxIterator &iter, RenderObject *seekObject = 0) /*KDE_NO_EXPORT*/;
     ...
     *	position for \c cb
     */
    static CaretBoxLine *constructCaretBoxLine(MassDeleter< CaretBoxLine > *deleter, RenderBox *cb, bool outside, bool outsideEnd,
...
protected:
     ...
     */
    void addCreatedInlineBoxEdge(InlineBox *box, const QFontMetrics &fm, bool left, bool rtl) /*KDE_NO_EXPORT*/;
     ...
     * @param rtl true if direction is rtl
     */
    void addCreatedFlowBoxEdge(InlineFlowBox *flowBox, const QFontMetrics &fm, bool left, bool rtl) /*KDE_NO_EXPORT*/;
...
public:
     ...
     *	editable line
     */
    EditableLineIterator(const LineIterator &it, bool fromEnd = false) : LineIterator(it)
    {
        if(!cbl)
            return;
        if(!isEditable(*this))
...
public:
     ...
     */
    TableRowIterator(RenderTableSection *section, int index) : sec(section), index(index)
    {
    }
...
public:
     ...
     */
    ErgonomicEditableLineIterator(const LineIterator &it, int x) : EditableLineIterator(it), xCoor(x)
    {
    }
...
public:
    ...
    EditableCharacterIterator(LinearDocument *ld)
        : _it(ld->current()), ebit(_it, false, &_it.currentCaretBox()), _offset(_it.currentModifiedOffset()), _char(-1), _end(false)
    {
