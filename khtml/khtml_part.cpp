...
public:
    ...
    {
        m_part = part;
        m_cachedSheet = dl->requestStyleSheet(url, QString::null, "text/css", true /* "user sheet" */);
        if(m_cachedSheet)
...
KHTMLPart::KHTMLPart(KHTMLView *view, QObject *parent, const char *name, GUIProfile prof) : KParts::ReadOnlyPart(parent, name)
{
    d = 0;
    KHTMLFactory::registerPart(this);
    setInstance(KHTMLFactory::instance(), prof == BrowserViewGUI && !parentPart());
...
void KHTMLPart::init(KHTMLView *view, GUIProfile prof)
{
    ...
    d->m_bCleared = false;
    d->m_paViewDocument =
        new KAction(i18n("View Do&cument Source"), CTRL + Key_U, this, SLOT(slotViewDocumentSource()), actionCollection(), "viewDocumentSource");
    d->m_paViewFrame = new KAction(i18n("View Frame Source"), 0, this, SLOT(slotViewFrameSource()), actionCollection(), "viewFrameSource");
    d->m_paViewInfo =
        new KAction(i18n("View Document Information"), CTRL + Key_I, this, SLOT(slotViewPageInfo()), actionCollection(), "viewPageInfo");
    d->m_paSaveBackground =
        new KAction(i18n("Save &Background Image As..."), 0, this, SLOT(slotSaveBackground()), actionCollection(), "saveBackground");
    ...
    d->m_paSecurity->setWhatsThis(
        i18n("Security Settings<p>"
             ...
             "secure connection."));
    d->m_paDebugRenderTree = new KAction(i18n("Print Rendering Tree to STDOUT"), ALT + CTRL + SHIFT + Key_A, this, SLOT(slotDebugRenderTree()),
                                         actionCollection(), "debugRenderTree");
    d->m_paDebugDOMTree =
        new KAction(i18n("Print DOM Tree to STDOUT"), ALT + CTRL + SHIFT + Key_D, this, SLOT(slotDebugDOMTree()), actionCollection(), "debugDOMTree");
    ...
    d->m_manualDetection =
        new KSelectAction(i18n("short for Manual Detection", "Manual"), 0, this, SLOT(slotSetEncoding()), actionCollection(), "manualDetection");
    ...
    {
        d->m_paIncZoomFactor = new KHTMLZoomFactorAction(this, true, i18n("Enlarge Font"), "viewmag+", "CTRL++;CTRL+=", this, SLOT(slotIncZoomFast()),
                                                         actionCollection(), "incFontSizes");
        d->m_paIncZoomFactor->setWhatsThis(
            i18n("Enlarge Font<p>"
                 "Make the font in this window bigger. "
                 "Click and hold down the mouse button for a menu with all available font sizes."));
        d->m_paDecZoomFactor = new KHTMLZoomFactorAction(this, false, i18n("Shrink Font"), "viewmag-", CTRL + Key_Minus, this,
                                                         SLOT(slotDecZoomFast()), actionCollection(), "decFontSizes");
        d->m_paDecZoomFactor->setWhatsThis(
            i18n("Shrink Font<p>"
    ...
    d->m_paFindPrev->setWhatsThis(
        i18n("Find previous<p>"
             "Find the previous occurrence of the text that you "
             "have found using the <b>Find Text</b> function"));

    d->m_paFindAheadText =
        new KAction(i18n("Find Text as You Type"), KShortcut('/'), this, SLOT(slotFindAheadText()), actionCollection(), "findAheadText");
    d->m_paFindAheadLinks =
        new KAction(i18n("Find Links as You Type"), KShortcut('\''), this, SLOT(slotFindAheadLink()), actionCollection(), "findAheadLink");
    ...
    d->m_paToggleCaretMode = new KToggleAction(i18n("Toggle Caret Mode"), Key_F7, this, SLOT(slotToggleCaretMode()), actionCollection(), "caretMode");
    ...
    connect(this, SIGNAL(completed()), this, SLOT(updateActions()));
    connect(this, SIGNAL(completed(bool)), this, SLOT(updateActions()));
    connect(this, SIGNAL(started(KIO::Job *)), this, SLOT(updateActions()));

    d->m_popupMenuXML = KXMLGUIFactory::readConfigFile(locate("data", "khtml/khtml_popupmenu.rc", KHTMLFactory::instance()));

    connect(khtml::Cache::loader(), SIGNAL(requestStarted(khtml::DocLoader *, khtml::CachedObject *)), this,
            SLOT(slotLoaderRequestStarted(khtml::DocLoader *, khtml::CachedObject *)));
    connect(khtml::Cache::loader(), SIGNAL(requestDone(khtml::DocLoader *, khtml::CachedObject *)), this,
            SLOT(slotLoaderRequestDone(khtml::DocLoader *, khtml::CachedObject *)));
    connect(khtml::Cache::loader(), SIGNAL(requestFailed(khtml::DocLoader *, khtml::CachedObject *)), this,
            SLOT(slotLoaderRequestDone(khtml::DocLoader *, khtml::CachedObject *)));
...
KHTMLPart::~KHTMLPart()
{
    ...
    slotWalletClosed();
    if(!parentPart())
    { // only delete it if the top khtml_part closes
    ...
    disconnect(khtml::Cache::loader(), SIGNAL(requestStarted(khtml::DocLoader *, khtml::CachedObject *)), this,
               SLOT(slotLoaderRequestStarted(khtml::DocLoader *, khtml::CachedObject *)));
    disconnect(khtml::Cache::loader(), SIGNAL(requestDone(khtml::DocLoader *, khtml::CachedObject *)), this,
               SLOT(slotLoaderRequestDone(khtml::DocLoader *, khtml::CachedObject *)));
    disconnect(khtml::Cache::loader(), SIGNAL(requestFailed(khtml::DocLoader *, khtml::CachedObject *)), this,
               SLOT(slotLoaderRequestDone(khtml::DocLoader *, khtml::CachedObject *)));
...
bool KHTMLPart::openURL(const KURL &url)
{
    ...
    connect(d->m_job, SIGNAL(result(KIO::Job *)), SLOT(slotFinished(KIO::Job *)));
    connect(d->m_job, SIGNAL(data(KIO::Job *, const QByteArray &)), SLOT(slotData(KIO::Job *, const QByteArray &)));
    connect(d->m_job, SIGNAL(infoMessage(KIO::Job *, const QString &)), SLOT(slotInfoMessage(KIO::Job *, const QString &)));
    connect(d->m_job, SIGNAL(redirection(KIO::Job *, const KURL &)), SLOT(slotRedirection(KIO::Job *, const KURL &)));
    ...
    connect(d->m_job, SIGNAL(speed(KIO::Job *, unsigned long)), this, SLOT(slotJobSpeed(KIO::Job *, unsigned long)));

    connect(d->m_job, SIGNAL(percent(KIO::Job *, unsigned long)), this, SLOT(slotJobPercent(KIO::Job *, unsigned long)));

    connect(d->m_job, SIGNAL(result(KIO::Job *)), this, SLOT(slotJobDone(KIO::Job *)));
    ...
    if(args.reload && !settings()->userStyleSheet().isEmpty())
    {
        ...
        connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(slotUserSheetStatDone(KIO::Job *)));
    }
    emit started(0L);
...
{
#ifdef KJS_VERBOSE
    kdDebug(6070) << "KHTMLPart::executeScript caller='" << name() << "' node=" << n.nodeName().string().latin1() << "("
                  << (n.isNull() ? 0 : n.nodeType()) << ") " /* << script */ << endl;
#endif
    KJSProxy *proxy = jScript();
...
void KHTMLPart::setAutoloadImages(bool enable)
{
    ...
    if(enable)
    {
        ...
        d->m_paLoadImages =
            new KAction(i18n("Display Images on Page"), "images_display", 0, this, SLOT(slotLoadImages()), actionCollection(), "loadImages");
...
void KHTMLPart::clear()
{
    ...
    disconnect(partManager(), SIGNAL(activePartChanged(KParts::Part *)), this, SLOT(slotActiveFrameChanged(KParts::Part *)));
    ...
    connect(partManager(), SIGNAL(activePartChanged(KParts::Part *)), this, SLOT(slotActiveFrameChanged(KParts::Part *)));
...
void KHTMLPart::htmlError(int errorCode, const QString &text, const KURL &reqUrl)
{
    ...
    d->m_bJScriptOverride = true;
    begin();
    QString errText = QString::fromLatin1("<HTML dir=%1><HEAD><TITLE>").arg(QApplication::reverseLayout() ? "rtl" : "ltr");
    ...
    datetime = KGlobal::locale()->formatDateTime(QDateTime::currentDateTime(), false);
...
void KHTMLPart::begin(const KURL &url, int xOffset, int yOffset)
{
    ...
    if(!parentPart())
    {
        ...
        d->m_openableSuppressedPopups = 0;
        for(QValueListIterator< QGuardedPtr< KHTMLPart > > i = d->m_suppressedPopupOriginParts.begin(); i != d->m_suppressedPopupOriginParts.end();
            ++i)
        {
...
void KHTMLPart::slotLoaderRequestStarted(khtml::DocLoader *dl, khtml::CachedObject *obj)
{
    if(obj && obj->type() == khtml::CachedObject::Image && d->m_doc && d->m_doc->docLoader() == dl)
    {
        ...
        while(p)
        {
            ...
            p = p->parentPart();
            if(!p && op->d->m_loadedObjects <= op->d->m_totalObjectCount && !op->d->m_progressUpdateTimer.isActive())
...
void KHTMLPart::slotLoaderRequestDone(khtml::DocLoader *dl, khtml::CachedObject *obj)
{
    if(obj && obj->type() == khtml::CachedObject::Image && d->m_doc && d->m_doc->docLoader() == dl)
    {
        ...
        while(p)
        {
            ...
            ++(p->d->m_loadedObjects);
            p = p->parentPart();
            if(!p && op->d->m_loadedObjects <= op->d->m_totalObjectCount && op->d->m_jobPercent <= 100 && !op->d->m_progressUpdateTimer.isActive())
                op->d->m_progressUpdateTimer.start(200, true);
        }
    }
...
void KHTMLPart::slotProgressUpdate()
{
    ...
    if(d->m_statusMessagesEnabled)
    {
        if(d->m_bComplete)
            ...
            emit d->m_extension->infoMessage(
                i18n("%n Image of %1 loaded.", "%n Images of %1 loaded.", d->m_loadedObjects).arg(d->m_totalObjectCount));
    }
...
void KHTMLPart::scheduleRedirection(int delay, const QString &url, bool doLockHistory)
{
    ...
    if(delay < 24 * 60 * 60 && (d->m_redirectURL.isEmpty() || delay <= d->m_delayRedirect))
    {
...
void KHTMLPartPrivate::setFlagRecursively(bool KHTMLPartPrivate::*flag, bool value)
{
...
void KHTMLPart::setCaretVisible(bool show)
{
    ...
    if(show)
    {
        ...
        if(isCaretMode() || isEditable() || (caretNode && caretNode->contentEditable()))
        {
            view()->caretOn();
        } /*end if*/
    }
...
bool KHTMLPart::findTextNext(const QString &str, bool forward, bool caseSensitive, bool isRegExp)
{
    ...
    while(1)
    {
        if((d->m_findNode->nodeType() == Node::TEXT_NODE || d->m_findNode->nodeType() == Node::CDATA_SECTION_NODE) && d->m_findNode->renderer())
        {
            ...
            {
                int x = 0, y = 0;
                if(static_cast< khtml::RenderText * >(d->m_findNode->renderer())->posOfChar(d->m_findPos, x, y))
                ...
                d->m_doc->setSelection(d->m_selectionStart.handle(), d->m_startOffset, d->m_selectionEnd.handle(), d->m_endOffset);
                emitSelectionChanged();
                return true;
            }
        }
...
void KHTMLPart::findText(const QString &str, long options, QWidget *parent, KFindDialog *findDialog)
{
    ...
    connect(d->m_find, SIGNAL(highlight(const QString &, int, int)), this, SLOT(slotHighlight(const QString &, int, int)));
    ...
    if(!findDialog)
    {
        ...
        initFindNode(options & KFindDialog::SelectedText, options & KFindDialog::FindBackwards, options & KFindDialog::FromCursor);
    }
}
...
bool KHTMLPart::findTextNext(bool reverse)
{
    ...
    if(d->m_findDialog) // 0 when we close the dialog
    {
        ...
        if(d->m_lastFindState.options != options)
        {
            ...
            if(difference & (KFindDialog::SelectedText | KFindDialog::FromCursor))
            {
                ...
                (void)initFindNode(options & KFindDialog::SelectedText, options & KFindDialog::FindBackwards, options & KFindDialog::FromCursor);
            }
            d->m_lastFindState.options = options;
        }
    }
    ...
    if(d->m_lastFindState.last_dir != -1 && bool(d->m_lastFindState.last_dir) != bool(options & KFindDialog::FindBackwards))
    {
        ...
        if(d->m_find->needData())
        {
            ...
            while(obj && !newLine)
            {
                ...
                if(!s.isEmpty())
                {
                    ...
                    {
                        for(QValueList< KHTMLPartPrivate::StringPortion >::Iterator it = d->m_stringPortions.begin(); it != d->m_stringPortions.end();
                            ++it)
                            (*it).index += s.length();
...
DOM::Range KHTMLPart::selection() const
{
    ...
    if(!n->parentNode() || !n->renderer() || (!n->renderer()->isReplaced() && !n->renderer()->isBR()))
    {
...
void KHTMLPart::overURL(const QString &url, const QString &target, bool /*shiftPressed*/)
{
    ...
    else
    {
        ...
        else if(!target.isEmpty() && (target.lower() != "_top") && (target.lower() != "_self") && (target.lower() != "_parent"))
        {
            KHTMLPart *p = this;
            while(p->parentPart())
                p = p->parentPart();
        ...
        {
            QString
                mailtoMsg /* = QString::fromLatin1("<img src=%1>").arg(locate("icon", QString::fromLatin1("locolor/16x16/actions/mail_send.png")))*/;
...
bool KHTMLPart::urlSelectedIntern(const QString &url, int button, int state, const QString &_target, KParts::URLArgs args)
{
    ...
    if(!checkLinkSecurity(cURL, i18n("<qt>This untrusted page links to<BR><B>%1</B>.<BR>Do you want to follow the link?"), i18n("Follow")))
        return false;
...
void KHTMLPart::slotSecurity()
{
    ...
    if(d->m_ssl_in_use)
    {
        ...
        if(x)
        {
            ...
            kid->setup(x, d->m_ssl_peer_ip, m_url.url(), d->m_ssl_cipher, d->m_ssl_cipher_desc, d->m_ssl_cipher_version,
                       d->m_ssl_cipher_used_bits.toInt(), d->m_ssl_cipher_bits.toInt(), (KSSLCertificate::KSSLValidation)d->m_ssl_cert_state.toInt());
            kid->exec();
            delete x;
        }
        else
...
bool KHTMLPart::requestFrame(khtml::RenderPart *frame, const QString &url, const QString &frameName, const QStringList &params, bool isIFrame)
{
...
bool KHTMLPart::requestObject(khtml::RenderPart *frame, const QString &url, const QString &serviceType, const QStringList &params)
{
...
bool KHTMLPart::requestObject(khtml::ChildFrame *child, const KURL &url, const KParts::URLArgs &_args)
{
    ...
    child->m_args.metaData().insert("main_frame_request", parentPart() == 0 ? "TRUE" : "FALSE");
    child->m_args.metaData().insert("ssl_was_in_use", d->m_ssl_in_use ? "TRUE" : "FALSE");
...
bool KHTMLPart::processObjectRequest(khtml::ChildFrame *child, const KURL &_url, const QString &mimetype)
{
    ...
    if(child->m_serviceType != mimetype || !child->m_part)
    {
        ...
        if(child->m_type != khtml::ChildFrame::Object)
        {
            ...
            KParts::BrowserRun::AskSaveResult res = KParts::BrowserRun::askEmbedOrSave(url, mimetype, suggestedFilename);
            switch(res)
            {
        ...
        KParts::ReadOnlyPart *part = createPart(d->m_view->viewport(), child->m_name.ascii(), this, child->m_name.ascii(), mimetype,
                                                child->m_serviceName, dummy, child->m_params);
        ...
        if(child->m_part)
        {
            ...
            {
                disconnect(child->m_liveconnect,
                           SIGNAL(partEvent(const unsigned long, const QString &, const KParts::LiveConnectExtension::ArgList &)), child,
        ...
        else if(child->m_frame)
        {
            ...
            if(child->m_liveconnect)
                connect(child->m_liveconnect, SIGNAL(partEvent(const unsigned long, const QString &, const KParts::LiveConnectExtension::ArgList &)),
                        child, SLOT(liveConnectEvent(const unsigned long, const QString &, const KParts::LiveConnectExtension::ArgList &)));
        }
        KParts::StatusBarExtension *sb = KParts::StatusBarExtension::childObject(part);
        if(sb)
            sb->setStatusBar(d->m_statusBarExtension->statusBar());

        connect(part, SIGNAL(started(KIO::Job *)), this, SLOT(slotChildStarted(KIO::Job *)));
        connect(part, SIGNAL(completed()), this, SLOT(slotChildCompleted()));
        connect(part, SIGNAL(completed(bool)), this, SLOT(slotChildCompleted(bool)));
        connect(part, SIGNAL(setStatusBarText(const QString &)), this, SIGNAL(setStatusBarText(const QString &)));
        if(part->inherits("KHTMLPart"))
        {
            connect(this, SIGNAL(completed()), part, SLOT(slotParentCompleted()));
            connect(this, SIGNAL(completed(bool)), part, SLOT(slotParentCompleted()));
            ...
            connect(part, SIGNAL(docCreated()), this, SLOT(slotChildDocCreated()));
        }
        ...
        {
            connect(child->m_extension, SIGNAL(openURLNotify()), d->m_extension, SIGNAL(openURLNotify()));

            connect(child->m_extension, SIGNAL(openURLRequestDelayed(const KURL &, const KParts::URLArgs &)), this,
                    SLOT(slotChildURLRequest(const KURL &, const KParts::URLArgs &)));

            connect(child->m_extension, SIGNAL(createNewWindow(const KURL &, const KParts::URLArgs &)), d->m_extension,
                    SIGNAL(createNewWindow(const KURL &, const KParts::URLArgs &)));
            connect(child->m_extension,
                    SIGNAL(createNewWindow(const KURL &, const KParts::URLArgs &, const KParts::WindowArgs &, KParts::ReadOnlyPart *&)),
                    d->m_extension,
                    SIGNAL(createNewWindow(const KURL &, const KParts::URLArgs &, const KParts::WindowArgs &, KParts::ReadOnlyPart *&)));

            connect(child->m_extension, SIGNAL(popupMenu(const QPoint &, const KFileItemList &)), d->m_extension,
                    SIGNAL(popupMenu(const QPoint &, const KFileItemList &)));
            connect(child->m_extension, SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KFileItemList &)), d->m_extension,
                    SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KFileItemList &)));
            connect(child->m_extension, SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KFileItemList &, const KParts::URLArgs &,
                                                         KParts::BrowserExtension::PopupFlags)),
                    d->m_extension, SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KFileItemList &, const KParts::URLArgs &,
                                                     KParts::BrowserExtension::PopupFlags)));
            connect(child->m_extension, SIGNAL(popupMenu(const QPoint &, const KURL &, const QString &, mode_t)), d->m_extension,
                    SIGNAL(popupMenu(const QPoint &, const KURL &, const QString &, mode_t)));
            connect(child->m_extension, SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KURL &, const QString &, mode_t)), d->m_extension,
                    SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KURL &, const QString &, mode_t)));
            connect(child->m_extension, SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KURL &, const KParts::URLArgs &,
                                                         KParts::BrowserExtension::PopupFlags, mode_t)),
                    d->m_extension, SIGNAL(popupMenu(KXMLGUIClient *, const QPoint &, const KURL &, const KParts::URLArgs &,
                                                     KParts::BrowserExtension::PopupFlags, mode_t)));

            connect(child->m_extension, SIGNAL(infoMessage(const QString &)), d->m_extension, SIGNAL(infoMessage(const QString &)));

            connect(child->m_extension, SIGNAL(requestFocus(KParts::ReadOnlyPart *)), this, SLOT(slotRequestFocus(KParts::ReadOnlyPart *)));

            child->m_extension->setBrowserInterface(d->m_extension->browserInterface());
        }
    }
    else if(child->m_frame && child->m_part && child->m_frame->widget() != child->m_part->widget())
        child->m_frame->setWidget(child->m_part->widget());
...
KParts::ReadOnlyPart *KHTMLPart::createPart(QWidget *parentWidget, const char *widgetName, QObject *parent, const char *name, const QString &mimetype,
                                            QString &serviceName, QStringList &serviceTypes, const QStringList &params)
{
    QString constr;
        ...
        if(factory)
        {
            ...
            if(factory->inherits("KParts::Factory"))
                res = static_cast< KParts::ReadOnlyPart * >(
                    static_cast< KParts::Factory * >(factory)->createPart(parentWidget, widgetName, parent, name, className, params));
            else
...
KParts::PartManager *KHTMLPart::partManager()
{
    if(!d->m_manager && d->m_view)
    {
        ...
        connect(d->m_manager, SIGNAL(activePartChanged(KParts::Part *)), this, SLOT(slotActiveFrameChanged(KParts::Part *)));
        connect(d->m_manager, SIGNAL(partRemoved(KParts::Part *)), this, SLOT(slotPartRemoved(KParts::Part *)));
    }
...
void KHTMLPart::submitFormAgain()
{
    ...
    if(d->m_doc && !d->m_doc->parsing() && d->m_submitForm)
        KHTMLPart::submitForm(d->m_submitForm->submitAction, d->m_submitForm->submitUrl, d->m_submitForm->submitFormData, d->m_submitForm->target,
                              d->m_submitForm->submitContentType, d->m_submitForm->submitBoundary);

    delete d->m_submitForm;
    d->m_submitForm = 0;
}

void KHTMLPart::submitFormProxy(const char *action, const QString &url, const QByteArray &formData, const QString &_target,
                                const QString &contentType, const QString &boundary)
{
    submitForm(action, url, formData, _target, contentType, boundary);
}

void KHTMLPart::submitForm(const char *action, const QString &url, const QByteArray &formData, const QString &_target, const QString &contentType,
                           const QString &boundary)
{
    ...
    if(!d->m_submitForm)
    {
        if(u.protocol() != "https" && u.protocol() != "mailto")
        {
            ...
            { // Going from SSL -> nonSSL
                int rc = KMessageBox::warningContinueCancel(
                    NULL, i18n("Warning:  This is a secure form but it is attempting to send your data back unencrypted."
            ...
            { // Going from nonSSL -> nonSSL
                KSSLSettings kss(true);
                if(kss.warnOnUnencrypted())
                {
                    int rc = KMessageBox::warningContinueCancel(
                        NULL, i18n("Warning: Your data is about to be transmitted across the network unencrypted."
                                   "\nAre you sure you wish to continue?"),
                        i18n("Network Transmission"), KGuiItem(i18n("&Send Unencrypted")), "WarnOnUnencryptedForm");
        ...
        {
            int rc = KMessageBox::warningContinueCancel(NULL, i18n("This site is attempting to submit form data via email.\n"
                                                                   "Do you want to continue?"),
                                                        i18n("Network Transmission"), KGuiItem(i18n("&Send Email")), "WarnTriedEmailSubmit");
    ...
    if(!checkLinkSecurity(u, i18n("<qt>The form will be submitted to <BR><B>%1</B><BR>on your local filesystem.<BR>Do you want to submit the form?"),
                          i18n("Submit")))
        return;
    ...
    if(u.protocol() == "mailto")
    {
        ...
        if(triedToAttach)
            KMessageBox::information(NULL, i18n("This site attempted to attach a file from your computer in the form submission. The attachment was "
                                                "removed for your protection."),
                                     i18n("KDE"), "WarnTriedAttach");
        ...
        if(contentType.lower() == "multipart/form-data")
        {
            ...
            bodyEnc = KURL::encode_string(QString::fromLatin1(formData.data(), formData.size()));
        }
        else if(contentType.lower() == "text/plain")
        {
            // Convention seems to be to decode, and s/&/\n/
            QString tmpbody = QString::fromLatin1(formData.data(), formData.size());
...
void KHTMLPart::popupMenu(const QString &linkUrl)
{
    ...
    { // click on link
        ...
        if(!(d->m_strSelectedURLTarget).isEmpty() && (d->m_strSelectedURLTarget.lower() != "_top") && (d->m_strSelectedURLTarget.lower() != "_self")
    ...
    if(!linkUrl.isEmpty()) // over a link
    {
        ...
        else // look at "extension" of link
        {
            ...
            if(!fname.isEmpty() && !popupURL.hasRef() && popupURL.query().isEmpty())
            {
                ...
                if(pmt->name() != KMimeType::defaultMimeType() && !pmt->is("application/x-perl") && !pmt->is("application/x-perl-module")
                   && !pmt->is("application/x-php") && !pmt->is("application/x-python-bytecode") && !pmt->is("application/x-python")
...
bool KHTMLPart::checkFrameAccess(KHTMLPart *callingHtmlPart)
{
    ...
    // now compare the domains
    if(callingHtmlPart && !callingHtmlPart->htmlDocument().isNull() && !htmlDocument().isNull())
    {
        DOM::DOMString actDomain = callingHtmlPart->htmlDocument().domain();
        DOM::DOMString destDomain = htmlDocument().domain();

#ifdef DEBUG_FINDFRAME
        kdDebug(6050) << "KHTMLPart::checkFrameAccess: actDomain = '" << actDomain.string() << "' destDomain = '" << destDomain.string() << "'"
                      << endl;
#endif
...
KHTMLPart *KHTMLPart::findFrameParent(KParts::ReadOnlyPart *callingPart, const QString &f, khtml::ChildFrame **childFrame)
{
#ifdef DEBUG_FINDFRAME
    kdDebug(6050) << "KHTMLPart::findFrameParent: this = " << this << " URL = " << m_url << " name = " << name() << " findFrameParent( " << f << " )"
                  << endl;
#endif
    // Check access
...
KParts::ReadOnlyPart *KHTMLPart::currentFrame() const
{
    ...
    while(part && part->inherits("KHTMLPart") && static_cast< KHTMLPart * >(part)->d->m_frames.count() > 0)
    {
...
khtml::ChildFrame *KHTMLPart::recursiveFrameRequest(KHTMLPart *callingHtmlPart, const KURL &url, const KParts::URLArgs &args, bool callParent)
{
#ifdef DEBUG_FINDFRAME
...
void KHTMLPart::saveState(QDataStream &stream)
{
    ...
    stream << m_url << (Q_INT32)d->m_view->contentsX() << (Q_INT32)d->m_view->contentsY() << (Q_INT32)d->m_view->contentsWidth()
           << (Q_INT32)d->m_view->contentsHeight() << (Q_INT32)d->m_view->marginWidth() << (Q_INT32)d->m_view->marginHeight();
    ...
    // Save ssl data
    stream << d->m_ssl_in_use << d->m_ssl_peer_certificate << d->m_ssl_peer_chain << d->m_ssl_peer_ip << d->m_ssl_cipher << d->m_ssl_cipher_desc
           << d->m_ssl_cipher_version << d->m_ssl_cipher_used_bits << d->m_ssl_cipher_bits << d->m_ssl_cert_state << d->m_ssl_parent_ip
           << d->m_ssl_parent_cert;
...
void KHTMLPart::restoreState(QDataStream &stream)
{
    ...
    stream >> frameCount >> frameNames >> frameServiceTypes >> frameServiceNames >> frameURLs >> frameStateBuffers;
...
QString KHTMLPart::pageReferrer() const
{
    ...
    if(referrerURL.isValid())
    {
        ...
        if((protocol == "http") || ((protocol == "https") && (m_url.protocol() == "https")))
        {
...
void KHTMLPart::khtmlMousePressEvent(khtml::MousePressEvent *event)
{
    ...
    if(_mouse->button() == LeftButton || _mouse->button() == MidButton)
    {
        ...
        {
            if((!d->m_strSelectedURL.isNull() && !isEditable()) || (!d->m_mousePressNode.isNull() && d->m_mousePressNode.elementId() == ID_IMG))
                return;
            if(!innerNode.isNull() && innerNode.handle()->renderer())
            {
                ...
                innerNode.handle()->renderer()->checkSelectionPoint(event->x(), event->y(), event->absX() - innerNode.handle()->renderer()->xPos(),
        ...
        if(!innerNode.isNull() && innerNode.handle()->renderer())
        {
            ...
            if(node && node->renderer())
            {
                ...
                if(_mouse->state() & ShiftButton)
                {
                    ...
                    d->m_startBeforeEnd =
                        RangeImpl::compareBoundaryPoints(d->m_selectionStart.handle(), d->m_startOffset, d->m_selectionEnd.handle(), d->m_endOffset)
                        <= 0;
                ...
                // Extend the start
                extendSelection(d->m_selectionStart.handle(), d->m_startOffset, d->m_selectionStart, d->m_startOffset, !d->m_startBeforeEnd,
                                selectLine);
                // Extend the end
...
void KHTMLPart::extendSelection(DOM::NodeImpl *node, long offset, DOM::Node &selectionNode, long &selectionOffset, bool right, bool selectLines)
{
    ...
    int len = 0;
    if(obj->isText())
    { // can be false e.g. when double-clicking on a disabled submit button
...
void KHTMLPart::extendSelectionTo(int x, int y, int absX, int absY, const DOM::Node &innerNode)
{
    ...
    DOM::NodeImpl *node = 0;
    khtml::RenderObject::SelPointState state;
    innerNode.handle()->renderer()->checkSelectionPoint(x, y, absX - innerNode.handle()->renderer()->xPos(),
    ...
    if(d->m_selectionStart.isNull() || d->m_selectionEnd.isNull() || d->m_initialNode.isNull() || !d->m_selectionStart.handle()->renderer()
    ...
    if(d->m_extendMode != d->ExtendByChar)
    {
        ...
        bool caretBeforeInit =
            RangeImpl::compareBoundaryPoints(d->caretNode().handle(), d->caretOffset(), d->m_initialNode.handle(), d->m_initialOffset) <= 0;
        bool nodeBeforeInit = RangeImpl::compareBoundaryPoints(node, offset, d->m_initialNode.handle(), d->m_initialOffset) <= 0;
        // have to fix up start to point to the original end
        if(caretBeforeInit != nodeBeforeInit)
        {
            //         kdDebug(6000) << "extto cbi: " << caretBeforeInit << " startBefEnd " << d->m_startBeforeEnd << " extAtEnd " << d->m_extendAtEnd
            //         << " (" << d->m_startOffset << ") - (" << d->m_endOffset << ")" << " initOfs " << d->m_initialOffset << endl;
            extendSelection(d->m_initialNode.handle(), d->m_initialOffset, d->m_extendAtEnd ? d->m_selectionStart : d->m_selectionEnd,
                            d->m_extendAtEnd ? d->m_startOffset : d->m_endOffset, nodeBeforeInit, d->m_extendMode == d->ExtendByLine);
        }
    }
    ...
    d->m_startBeforeEnd =
        RangeImpl::compareBoundaryPoints(d->m_selectionStart.handle(), d->m_startOffset, d->m_selectionEnd.handle(), d->m_endOffset) <= 0;

    if(!d->m_selectionStart.isNull() && !d->m_selectionEnd.isNull())
    {
        //         kdDebug(6000) << "extto: startBefEnd " << d->m_startBeforeEnd << " extAtEnd " << d->m_extendAtEnd << " (" << d->m_startOffset << ")
        //         - (" << d->m_endOffset << ")" << " initOfs " << d->m_initialOffset << endl;
        if(d->m_extendMode != d->ExtendByChar && withinNode)
            extendSelection(node, offset, d->caretNode(), d->caretOffset(), d->m_startBeforeEnd ^ !d->m_extendAtEnd,
                            d->m_extendMode == d->ExtendByLine);

        if(d->m_selectionEnd == d->m_selectionStart && d->m_endOffset < d->m_startOffset)
            d->m_doc->setSelection(d->m_selectionStart.handle(), d->m_endOffset, d->m_selectionEnd.handle(), d->m_startOffset);
        else if(d->m_startBeforeEnd)
            d->m_doc->setSelection(d->m_selectionStart.handle(), d->m_startOffset, d->m_selectionEnd.handle(), d->m_endOffset);
        else
            d->m_doc->setSelection(d->m_selectionEnd.handle(), d->m_endOffset, d->m_selectionStart.handle(), d->m_startOffset);
    }
#ifndef KHTML_NO_CARET
    d->m_view->placeCaret();
#endif
...
void KHTMLPart::khtmlMouseMoveEvent(khtml::MouseMoveEvent *event)
{
    ...
    if(d->m_bDnd && d->m_bMousePressed
       && ((!d->m_strSelectedURL.isEmpty() && !isEditable()) || (!d->m_mousePressNode.isNull() && d->m_mousePressNode.elementId() == ID_IMG)))
    {
...
#ifndef KHTML_NO_SELECTION
        // selection stuff
        if(d->m_bMousePressed && innerNode.handle() && innerNode.handle()->renderer() && ((_mouse->state() & LeftButton) != 0))
        {
            extendSelectionTo(event->x(), event->y(), event->absX(), event->absY(), innerNode);
#else
        if(d->m_doc && d->m_view)
        {
...
void KHTMLPart::khtmlMouseReleaseEvent(khtml::MouseReleaseEvent *event)
{
    ...
    {
        kdDebug(6050) << "KHTMLPart::khtmlMouseReleaseEvent() MMB shouldOpen=" << d->m_bOpenMiddleClick << endl;
    ...
    {
        // we have to get to know if end is before start or not...
        //     kdDebug(6000) << "rel: startBefEnd " << d->m_startBeforeEnd << " extAtEnd " << d->m_extendAtEnd << " (" << d->m_startOffset << ") - ("
        //     << d->m_endOffset << ")" << endl;
        DOM::Node n = d->m_selectionStart;
...
#else
            // shouldn't be null but it can happen with dynamic updating of nodes
            if(d->m_selectionStart.isNull() || d->m_selectionEnd.isNull() || !d->m_selectionStart.handle()->renderer()
               || !d->m_selectionEnd.handle()->renderer())
                return;
            d->m_startBeforeEnd =
                RangeImpl::compareBoundaryPoints(d->m_selectionStart.handle(), d->m_startOffset, d->m_selectionEnd.handle(), d->m_endOffset) <= 0;
...
// get selected text and paste to the clipboard
        ...
        emitSelectionChanged();
        // kdDebug(6000) << "rel2: startBefEnd " << d->m_startBeforeEnd << " extAtEnd " << d->m_extendAtEnd << " (" << d->m_startOffset << ") - (" <<
        // d->m_endOffset << "), caretOfs " << d->caretOffset() << endl;
    }
#endif
...
void KHTMLPart::runAdFilter()
{
    ...
    if(KHTMLFactory::defaultHTMLSettings()->isHideAdsEnabled())
    {
        for(NodeImpl *nextNode, *node = d->m_doc; node; node = nextNode)
        {
            ...
            if(node->id() == ID_IMG || node->id() == ID_IFRAME
               || (node->id() == ID_INPUT && static_cast< HTMLInputElementImpl * >(node)->inputType() == HTMLInputElementImpl::IMAGE))
            {
                if(KHTMLFactory::defaultHTMLSettings()->isAdFiltered(
                       d->m_doc->completeURL(static_cast< ElementImpl * >(node)->getAttribute(ATTR_SRC).string())))
                {
...
void KHTMLPart::selectAll()
{
    ...
    while(first
          && !(first->renderer() && ((first->nodeType() == Node::TEXT_NODE || first->nodeType() == Node::CDATA_SECTION_NODE)
    ...
    while(last
          && !(last->renderer() && ((last->nodeType() == Node::TEXT_NODE || last->nodeType() == Node::CDATA_SECTION_NODE)
...
bool KHTMLPart::checkLinkSecurity(const KURL &linkURL, const QString &message, const QString &button)
{
    ...
    if(!linkAllowed)
    {
        ...
        if(!message.isEmpty())
        {
            response = KMessageBox::warningContinueCancel(0, message.arg(linkURL.htmlURL()), i18n("Security Warning"), button);
        }
        else
        {
            KMessageBox::error(0, i18n("<qt>Access by untrusted page to<BR><B>%1</B><BR> denied.").arg(linkURL.htmlURL()), i18n("Security Alert"));
        }
...
khtml::Decoder *KHTMLPart::createDecoder()
{
    ...
    if(!d->m_encoding.isNull())
        dec->setEncoding(d->m_encoding.latin1(), d->m_haveEncoding ? khtml::Decoder::UserChosenEncoding : khtml::Decoder::EncodingFromHTTPHeader);
    else
    {
        // Inherit the default encoding from the parent frame if there is one.
        const char *defaultEncoding =
            (parentPart() && parentPart()->d->m_decoder) ? parentPart()->d->m_decoder->encoding() : settings()->encoding().latin1();
...
void KHTMLPart::restoreScrollPosition()
{
    ...
    // during loading).
    if(d->m_view->contentsHeight() - d->m_view->visibleHeight() >= args.yOffset || d->m_bComplete)
    {
...
void KHTMLPart::openWallet(DOM::HTMLFormElementImpl *form)
{
    ...
    if(onlyLocalReferences())
    { // avoid triggering on local apps, thumbnails
        return;
    }
    ...
    if(!d->m_wq)
    {
        KWallet::Wallet *wallet = KWallet::Wallet::openWallet(KWallet::Wallet::NetworkWallet(), widget() ? widget()->topLevelWidget()->winId() : 0,
...
void KHTMLPart::walletOpened(KWallet::Wallet *wallet)
{
    ...
    QToolTip::add(d->m_statusBarWalletLabel,
                  i18n("The wallet '%1' is open and being used for form data and passwords.").arg(KWallet::Wallet::NetworkWallet()));
#endif // KHTML_NO_WALLET
}
...
void KHTMLPart::setSuppressedPopupIndicator(bool enable, KHTMLPart *originPart)
{
    ...
    if(enable && !d->m_statusBarPopupLabel)
    {
        ...
        if(d->m_settings->jsPopupBlockerPassivePopup())
        {
            ...
            KPassivePopup::message(i18n("Popup Window Blocked"),
                                   i18n("This page has attempted to open a popup window but was blocked.\nYou can click on this icon in the status "
                                        "bar to control this behavior\nor to open the popup."),
                                   px, d->m_statusBarPopupLabel);
        }
    }
...
void KHTMLPart::suppressedPopupMenu()
{
    ...
    if(d->m_openableSuppressedPopups)
        m->insertItem(i18n("&Show Blocked Popup Window", "Show %n Blocked Popup Windows", d->m_openableSuppressedPopups), this,
                      SLOT(showSuppressedPopups()));
...
{
    for(QValueListIterator< QGuardedPtr< KHTMLPart > > i = d->m_suppressedPopupOriginParts.begin(); i != d->m_suppressedPopupOriginParts.end(); ++i)
    {
        if(KHTMLPart *part = *i)
        {
            KJS::Window *w = KJS::Window::retrieveWindow(part);
