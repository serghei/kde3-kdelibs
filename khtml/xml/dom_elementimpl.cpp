...
AttrImpl::AttrImpl(ElementImpl *element, DocumentImpl *docPtr, NodeImpl::Id attrId, DOMStringImpl *value, DOMStringImpl *prefix)
    : NodeBaseImpl(docPtr), m_element(element), m_attrId(attrId)
{
    m_value = value;
    m_value->ref();
...
XMLElementImpl::XMLElementImpl(DocumentImpl *doc, NodeImpl::Id id) : ElementImpl(doc)
{
    // Called from createElement(). In this case localName, prefix, and namespaceURI all need to be null.
    m_id = id;
}

XMLElementImpl::XMLElementImpl(DocumentImpl *doc, NodeImpl::Id id, DOMStringImpl *_prefix) : ElementImpl(doc)
{
...
NamedAttrMapImpl::NamedAttrMapImpl(ElementImpl *element) : m_element(element), m_attrs(0), m_attrCount(0)
{
}
...
DOMStringImpl *NamedAttrMapImpl::getValue(NodeImpl::Id id, bool nsAware, DOMStringImpl *qName) const
{
    ...
    for(unsigned long i = 0; i < m_attrCount; i++)
        ...
        {
            // if we are called with a qualified name, filter out NS-aware elements with non-matching name.
            if(qName && (namespacePart(m_attrs[i].id()) != defaultNamespace) && strcasecmp(m_attrs[i].name(), qName))
                continue;
...
void NamedAttrMapImpl::setValue(NodeImpl::Id id, DOMStringImpl *value, DOMStringImpl *qName, DOMStringImpl *prefix, bool nsAware, bool hasNS)
{
    ...
    else
    {
        ...
        if(!m_element)
            return;
        m_attrs[m_attrCount - 1].m_data.attr = new AttrImpl(m_element, m_element->docPtr(), id, value, prefix);
        m_attrs[m_attrCount - 1].m_attrId = 0; /* "has implementation" flag */
        m_attrs[m_attrCount - 1].m_data.attr->ref();
        m_attrs[m_attrCount - 1].m_data.attr->setHTMLCompat(!hasNS && m_element->getDocument()->htmlMode() != DocumentImpl::XHtml);
    }
    if(m_element)
    {
        if(id == ATTR_ID)
...
NodeImpl::Id NamedAttrMapImpl::mapId(DOMStringImpl *namespaceURI, DOMStringImpl *localName, bool readonly)
{
    ...
    return m_element->getDocument()->getId(NodeImpl::AttributeId, namespaceURI, 0, localName, readonly, true /*lookupHTML*/);
}
