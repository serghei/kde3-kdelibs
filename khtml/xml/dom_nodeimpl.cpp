...
namespace khtml {
void /*KDE_NO_EXPORT*/ mapDOMPosToRenderPos(DOM::NodeImpl *node, long offset, khtml::RenderObject *&r, long &r_ofs, bool &outside, bool &outsideEnd);
}
...
kdDebug(6200) << "getCaret: r " << r << " " << (r?r->renderName():QString::null) << " r_ofs: " << r_ofs << " outside " << outside << " outsideEnd " << outsideEnd << endl;
        ...
        if(r)
        {
            r->caretPos(r_ofs, override * RenderObject::CFOverride + outside * RenderObject::CFOutside + outsideEnd * RenderObject::CFOutsideEnd, _x,
                        _y, width, height);
        }
        else
...
QRect NodeImpl::getRect() const
{
    ...
    if(m_render && m_render->absolutePosition(_x, _y))
        return QRect(_x + m_render->inlineXPos(), _y + m_render->inlineYPos(), m_render->width(),
                     m_render->height() + renderer()->borderTopExtra() + renderer()->borderBottomExtra());
...
void NodeImpl::dispatchGenericEvent(EventImpl *evt, int & /*exceptioncode */)
{
    ...
    if(evt->bubbles())
    {
        ...
        for(it.toLast(); it.current() && it.current() != propagationSentinel && !evt->defaultPrevented() && !evt->defaultHandled(); --it)
            it.current()->defaultEventHandler(evt);

        if(evt->id() == EventImpl::CLICK_EVENT && !evt->defaultPrevented() && static_cast< MouseEventImpl * >(evt)->button() == 0) // LMB click
...
void NodeImpl::dispatchWindowEvent(int _id, bool canBubbleArg, bool cancelableArg)
{
    ...
    if(_id == EventImpl::LOAD_EVENT && !evt->propagationStopped() && doc)
    {
        ...
        if(elt && (elt->getDocument()->domain().isNull() || elt->getDocument()->domain() == doc->domain()))
        {
...
void NodeImpl::dispatchMouseEvent(QMouseEvent *_mouse, int overrideId, int overrideDetail)
{
    ...
    EventImpl *const evt = new MouseEventImpl(evtId, true, cancelable, getDocument()->defaultView(), detail, screenX, screenY, clientX, clientY,
                                              pageX, pageY, ctrlKey, altKey, shiftKey, metaKey, button, 0);
    evt->ref();
    dispatchEvent(evt, exceptioncode, true);
    evt->deref();
}
...
{
    assert(!((_id != EventImpl::DOMFOCUSIN_EVENT && _id != EventImpl::DOMFOCUSOUT_EVENT && _id != EventImpl::DOMACTIVATE_EVENT)));
    ...
    int exceptioncode = 0;
    UIEventImpl *const evt = new UIEventImpl(static_cast< EventImpl::EventId >(_id), true, cancelable, getDocument()->defaultView(), detail);
    evt->ref();
    dispatchEvent(evt, exceptioncode, true);
    evt->deref();
}
...
void NodeImpl::dispatchSubtreeModifiedEvent()
{
    ...
    MutationEventImpl *const evt =
        new MutationEventImpl(EventImpl::DOMSUBTREEMODIFIED_EVENT, true, false, 0, DOMString(), DOMString(), DOMString(), 0);
    evt->ref();
    dispatchEvent(evt, exceptioncode, true);
    evt->deref();
}
...
bool NodeImpl::isReadOnly()
{
    ...
    while(n)
    {
        if(n->nodeType() == Node::ENTITY_NODE || n->nodeType() == Node::ENTITY_REFERENCE_NODE)
            return true;
        n = n->parentNode();
    }
    return false;
...
void NodeBaseImpl::dispatchChildInsertedEvents(NodeImpl *child, int &exceptioncode)
{
    ...
    {
        MutationEventImpl *const evt =
            new MutationEventImpl(EventImpl::DOMNODEINSERTED_EVENT, true, false, this, DOMString(), DOMString(), DOMString(), 0);
        evt->ref();
    ...
    if(p->nodeType() == Node::DOCUMENT_NODE)
    {
        for(NodeImpl *c = child; c; c = c->traverseNextNode(child))
        {
            ...
            {
                MutationEventImpl *const evt =
                    new MutationEventImpl(EventImpl::DOMNODEINSERTEDINTODOCUMENT_EVENT, false, false, 0, DOMString(), DOMString(), DOMString(), 0);
                evt->ref();
...
void NodeBaseImpl::dispatchChildRemovalEvents(NodeImpl *child, int &exceptioncode)
{
    ...
    {
        MutationEventImpl *const evt =
            new MutationEventImpl(EventImpl::DOMNODEREMOVED_EVENT, true, false, this, DOMString(), DOMString(), DOMString(), 0);
        evt->ref();
        child->dispatchEvent(evt, exceptioncode, true);
        evt->deref();
        if(exceptioncode)
    ...
    if(p->nodeType() == Node::DOCUMENT_NODE)
    {
        for(NodeImpl *c = child; c; c = c->traverseNextNode(child))
        {
            ...
            {
                MutationEventImpl *const evt =
                    new MutationEventImpl(EventImpl::DOMNODEREMOVEDFROMDOCUMENT_EVENT, false, false, 0, DOMString(), DOMString(), DOMString(), 0);
                evt->ref();
                c->dispatchEvent(evt, exceptioncode, true);
                evt->deref();
                if(exceptioncode)
...
NodeImpl *NodeListImpl::item(unsigned long index) const
{
    ...
    if(m_cache->current.node)
    {
        ...
        if(cacheDist < (long)index)
        { // Closer to the cached position
            usedCache = true;
            if(index >= m_cache->position)
            { // Go ahead
...
NodeListImpl::NodeListImpl(NodeImpl *n, int type, CacheFactory *factory)
{
    ...
    m_cache = m_refNode->getDocument()->acquireCachedNodeListInfo(factory ? factory : Cache::make, n, type);
}
...
TagNodeListImpl::TagNodeListImpl(NodeImpl *n, NodeImpl::Id id) : NodeListImpl(n, UNCACHEABLE), m_id(id), m_namespaceAware(false)
{
...
TagNodeListImpl::TagNodeListImpl(NodeImpl *n, const DOMString &namespaceURI, const DOMString &localName)
    : NodeListImpl(n, UNCACHEABLE), m_id(0), m_namespaceURI(namespaceURI), m_localName(localName), m_namespaceAware(true)
{
...
bool TagNodeListImpl::nodeMatches(NodeImpl *testNode, bool & /*doRecurse*/) const
{
    if(testNode->nodeType() != Node::ELEMENT_NODE)
        return false;
    if(m_namespaceAware)
        return (m_matchAllNamespaces || testNode->namespaceURI() == m_namespaceURI) && (m_matchAllNames || testNode->localName() == m_localName);
    else
    {
        NodeImpl::Id testId = testNode->id();
        // we have to strip the namespaces if we compare in a namespace unaware fashion
        if(!m_namespaceAware)
            testId = localNamePart(testId);
        return (m_id == 0 || m_id == testId);
    }
}

NameNodeListImpl::NameNodeListImpl(NodeImpl *n, const DOMString &t) : NodeListImpl(n, UNCACHEABLE), nodeName(t)
{
}
...
GenericRONamedNodeMapImpl::GenericRONamedNodeMapImpl(DocumentImpl *doc) : NamedNodeMapImpl()
{
    m_doc = doc;
...
NodeImpl::Id GenericRONamedNodeMapImpl::mapId(DOMStringImpl *namespaceURI, DOMStringImpl *localName, bool readonly)
{
    if(!m_doc)
        return 0;

    return m_doc->getId(NodeImpl::ElementId, namespaceURI, 0, localName, readonly, false /*don't lookupHTML*/);
}
