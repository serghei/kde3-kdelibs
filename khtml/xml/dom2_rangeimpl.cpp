...
RangeImpl::RangeImpl(DocumentImpl *_ownerDocument, NodeImpl *_startContainer, long _startOffset, NodeImpl *_endContainer, long _endOffset)
{
    m_ownerDocument = _ownerDocument;
    m_ownerDocument->ref();
...
short RangeImpl::compareBoundaryPoints(Range::CompareHow how, RangeImpl *sourceRange, int &exceptioncode)
{
    ...
    switch(how)
    {
        case Range::START_TO_START:
            return compareBoundaryPoints(m_startContainer, m_startOffset, sourceRange->startContainer(exceptioncode),
                                         sourceRange->startOffset(exceptioncode));
            break;
        case Range::START_TO_END:
            return compareBoundaryPoints(m_startContainer, m_startOffset, sourceRange->endContainer(exceptioncode),
                                         sourceRange->endOffset(exceptioncode));
            break;
        case Range::END_TO_END:
            return compareBoundaryPoints(m_endContainer, m_endOffset, sourceRange->endContainer(exceptioncode),
                                         sourceRange->endOffset(exceptioncode));
            break;
        case Range::END_TO_START:
            return compareBoundaryPoints(m_endContainer, m_endOffset, sourceRange->startContainer(exceptioncode),
                                         sourceRange->startOffset(exceptioncode));
            break;
        default:
...
DocumentFragmentImpl *RangeImpl::processContents(ActionType action, int &exceptioncode)
{
    ...
    {
        if(m_startContainer->nodeType() == Node::TEXT_NODE || m_startContainer->nodeType() == Node::CDATA_SECTION_NODE
        ...
        else
        {
            ...
            for(i = 0; i < m_startOffset; i++) // skip until m_startOffset
                n = n->nextSibling();
            while(n && i < m_endOffset)
            { // delete until m_endOffset
    ...
    if(m_startContainer != cmnRoot)
    {
           ...
           || m_startContainer->nodeType() == Node::COMMENT_NODE)
        {
            ...
            if(action == EXTRACT_CONTENTS || action == DELETE_CONTENTS)
                static_cast< CharacterDataImpl * >(m_startContainer)
                    ->deleteData(m_startOffset, static_cast< CharacterDataImpl * >(m_startContainer)->length() - m_startOffset, exceptioncode);
        }
        else if(m_startContainer->nodeType() == Node::PROCESSING_INSTRUCTION_NODE)
        {
            // ### operate just on data ?
    ...
    if(m_endContainer != cmnRoot)
    {
        ...
        if(m_endContainer->nodeType() == Node::TEXT_NODE || m_endContainer->nodeType() == Node::CDATA_SECTION_NODE
...
void RangeImpl::insertNode(NodeImpl *newNode, int &exceptioncode)
{
    ...
    if(newNode->nodeType() == Node::ATTRIBUTE_NODE || newNode->nodeType() == Node::ENTITY_NODE || newNode->nodeType() == Node::NOTATION_NODE
...
DOMString RangeImpl::toHTML(int &exceptioncode)
{
    ...
    while(n)
    {
        ...
        if(n->nodeType() == DOM::Node::ELEMENT_NODE)
        {
            ...
            if(elementId == ID_LI)
                in_li = true;
            if(num_tables == 0 && (elementId == ID_TD || elementId == ID_TR || elementId == ID_TH || elementId == ID_TBODY || elementId == ID_TFOOT
                                   || elementId == ID_THEAD))
                num_tables++;
            if(!(!n->hasChildNodes() && (elementId == ID_H1 || elementId == ID_H2 || elementId == ID_H3 || elementId == ID_H4 || elementId == ID_H5)))
            { // Don't add <h1/>  etc.  Just skip these nodes just to make the output html a bit nicer.
        ...
        else if(n->nodeType() == DOM::Node::TEXT_NODE || n->nodeType() == DOM::Node::CDATA_SECTION_NODE)
        {
        ...
        while(!next && n->parentNode())
        {
            ...
            if(n->nodeType() == DOM::Node::ELEMENT_NODE)
            {
                ...
                if(num_tables == 0 && (elementId == ID_TD || elementId == ID_TR || elementId == ID_TH || elementId == ID_TBODY
                                       || elementId == ID_TFOOT || elementId == ID_THEAD))
                    num_tables--;
...
void RangeImpl::checkNodeBA(NodeImpl *n, int &exceptioncode) const
{
    ...
    if(!(root->nodeType() == Node::ATTRIBUTE_NODE || root->nodeType() == Node::DOCUMENT_NODE || root->nodeType() == Node::DOCUMENT_FRAGMENT_NODE))
    {
        exceptioncode = RangeException::INVALID_NODE_TYPE_ERR + RangeException::_EXCEPTION_OFFSET;
        return;
    }

    if(n->nodeType() == Node::DOCUMENT_NODE || n->nodeType() == Node::DOCUMENT_FRAGMENT_NODE || n->nodeType() == Node::ATTRIBUTE_NODE
       || n->nodeType() == Node::ENTITY_NODE || n->nodeType() == Node::NOTATION_NODE)
        exceptioncode = RangeException::INVALID_NODE_TYPE_ERR + RangeException::_EXCEPTION_OFFSET;
}
...
void RangeImpl::selectNode(NodeImpl *refNode, int &exceptioncode)
{
    ...
    {
        if(anc->nodeType() == Node::ENTITY_NODE || anc->nodeType() == Node::NOTATION_NODE || anc->nodeType() == Node::DOCUMENT_TYPE_NODE)
        {
    ...
    if(refNode->nodeType() == Node::DOCUMENT_NODE || refNode->nodeType() == Node::DOCUMENT_FRAGMENT_NODE
       || refNode->nodeType() == Node::ATTRIBUTE_NODE || refNode->nodeType() == Node::ENTITY_NODE || refNode->nodeType() == Node::NOTATION_NODE)
    {
...
void RangeImpl::selectNodeContents(NodeImpl *refNode, int &exceptioncode)
{
    ...
    {
        if(n->nodeType() == Node::ENTITY_NODE || n->nodeType() == Node::NOTATION_NODE || n->nodeType() == Node::DOCUMENT_TYPE_NODE)
        {
...
void RangeImpl::surroundContents(NodeImpl *newParent, int &exceptioncode)
{
    ...
    if(newParent->nodeType() == Node::ATTRIBUTE_NODE || newParent->nodeType() == Node::ENTITY_NODE || newParent->nodeType() == Node::NOTATION_NODE
       || newParent->nodeType() == Node::DOCUMENT_TYPE_NODE || newParent->nodeType() == Node::DOCUMENT_NODE
    ...
    if(m_startContainer->nodeType() != Node::TEXT_NODE && m_startContainer->nodeType() != Node::COMMENT_NODE
       && m_startContainer->nodeType() != Node::CDATA_SECTION_NODE && m_startContainer->nodeType() != Node::PROCESSING_INSTRUCTION_NODE)
    {
    ...
    if(m_endContainer->nodeType() != Node::TEXT_NODE && m_endContainer->nodeType() != Node::COMMENT_NODE
       && m_endContainer->nodeType() != Node::CDATA_SECTION_NODE && m_endContainer->nodeType() != Node::PROCESSING_INSTRUCTION_NODE)
    {
...
void RangeImpl::checkDeleteExtract(int &exceptioncode)
{
    ...
    NodeImpl *start;
    if(m_startContainer->nodeType() != Node::TEXT_NODE && m_startContainer->nodeType() != Node::CDATA_SECTION_NODE
       && m_startContainer->nodeType() != Node::COMMENT_NODE && m_startContainer->nodeType() != Node::PROCESSING_INSTRUCTION_NODE)
    {
    ...
    NodeImpl *end;
    if(m_endContainer->nodeType() != Node::TEXT_NODE && m_endContainer->nodeType() != Node::CDATA_SECTION_NODE
       && m_endContainer->nodeType() != Node::COMMENT_NODE && m_endContainer->nodeType() != Node::PROCESSING_INSTRUCTION_NODE)
    {
    ...
    for(n = start; n != end; n = n->traverseNextNode())
    {
        ...
        { // ### is this for only directly under the DF, or anywhere?
            exceptioncode = DOMException::HIERARCHY_REQUEST_ERR;
            return;
        }
    }
