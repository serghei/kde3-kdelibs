...
public:
    AttrImpl(ElementImpl *element, DocumentImpl *docPtr, NodeImpl::Id attrId, DOMStringImpl *value, DOMStringImpl *prefix = 0);
    ~AttrImpl();
...
{
    NodeImpl::Id id() const
    {
        return m_attrId ? m_attrId : m_data.attr->id();
    }
    DOMStringImpl *val() const
    {
        return m_attrId ? m_data.value : m_data.attr->val();
    }
    DOMString value() const
    {
        return val();
    ...
    AttrImpl *attr() const
    {
        return m_attrId ? 0 : m_data.attr;
    }
    DOMString namespaceURI()
    {
        return m_attrId ? DOMString() : m_data.attr->namespaceURI();
    }
    DOMString prefix()
    {
        return m_attrId ? DOMString() : m_data.attr->prefix();
    }
    DOMString localName()
    {
        return m_attrId ? DOMString() : m_data.attr->localName();
    }
    DOMString name()
    {
        return m_attrId ? DOMString() : m_data.attr->name();
    }
...
public:
    ...
    void setAttribute(NodeImpl::Id id, const DOMString &value, const DOMString &qName, int &exceptioncode);
    void setAttributeNS(const DOMString &namespaceURI, const DOMString &qualifiedName, const DOMString &value, int &exceptioncode);
    virtual DOMString prefix() const;
...
public:
    ...
    AttributeImpl *attrAt(unsigned long index) const
    {
    ...
    void setValue(NodeImpl::Id id, DOMStringImpl *value, DOMStringImpl *qName = 0, DOMStringImpl *prefix = 0, bool nsAware = false,
                  bool hasNS = false);
...
inline bool checkQualifiedName(const DOMString &qualifiedName, const DOMString &namespaceURI, int *colonPos, bool nameCanBeNull, bool nameCanBeEmpty,
                               int *pExceptioncode)
{
    ...
    if(!qualifiedName.isNull() && !Element::khtmlValidQualifiedName(qualifiedName) && (!qualifiedName.isEmpty() || !nameCanBeEmpty))
    {
        if(pExceptioncode)
    ...
    if((!qualifiedName.isNull() && Element::khtmlMalformedQualifiedName(qualifiedName)) || (colonpos >= 0 && namespaceURI.isNull())
