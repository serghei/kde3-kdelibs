...
KHTMLRun::KHTMLRun(KHTMLPart *part, khtml::ChildFrame *child, const KURL &url, const KParts::URLArgs &args, bool hideErrorDialog)
    : KParts::BrowserRun(url, args, part, part->widget() ? part->widget()->topLevelWidget() : 0, false, false, hideErrorDialog), m_child(child)
{
...
void KHTMLRun::foundMimeType(const QString &_type)
{
    ...
    else
    {
        ...
        if(m_bFinished)
        { // saved or canceled -> flag completed
