...
CSSStyleSelector::CSSStyleSelector(DocumentImpl *doc, QString userStyleSheet, StyleSheetListImpl *styleSheets, const KURL &url, bool _strictParsing)
{
...
void CSSStyleSelector::computeFontSizesFor(QPaintDeviceMetrics *paintDeviceMetrics, int zoomFactor, QValueVector< int > &fontSizes, bool isFixed)
{
    ...
    for(int i = 0; i < MAXFONTSIZES; i++)
    {
        ...
        // kdDebug( 6080 ) << "index: " << i << " factor: " << factors[i] << " font pix size: " << int(KMAX( mediumFontSize*factor +.5f, minFontSize))
        // << endl;
    }
}
...
// correct font
                ...
                //		if (prop->m_id == CSS_PROP__KONQ_USER_INPUT) kdDebug(6080) << "El: "<<e->nodeName().string() << " user-input:
                //"<<((CSSPrimitiveValueImpl *)prop->value())->getIdent() << endl;
...
void CSSStyleSelector::adjustRenderStyle(RenderStyle *style, DOM::ElementImpl *e)
{
    ...
    if(style->display() == TABLE || style->display() == INLINE_TABLE || style->display() == TABLE_ROW_GROUP || style->display() == TABLE_ROW)
    {
...
unsigned int CSSStyleSelector::addInlineDeclarations(DOM::ElementImpl *e, DOM::CSSStyleDeclarationImpl *decl, unsigned int numProps)
{
    ...
    if(sel->attr)
    {
        ...
        switch(sel->match)
        {
            ...
            case CSSSelector::Exact:
                return (caseSensitive && !strcmp(sel->value, value)) || (!caseSensitive && !strcasecmp(sel->value, value));
                break;
            ...
            case CSSSelector::List:
            {
                ...
                if((sel->attr != ATTR_CLASS || e->hasClassList()) && sel->value.find(' ') != -1)
                    return false;
                if(sel_len == val_len)
                    return (caseSensitive && !strcmp(sel->value, value)) || (!caseSensitive && !strcasecmp(sel->value, value));
                // else the value is longer and can be a list
                if(sel->match == CSSSelector::Class && !e->hasClassList())
                    return false;
            ...
            case CSSSelector::Hyphen:
            {
                ...
                if(str.find(selStr, 0, caseSensitive) != 0)
                    return false;
                // It does. Check for exact match or following '-':
                if(str.length() != selStr.length() && str[selStr.length()] != '-')
                    return false;
                break;
            }
        ...
        switch(sel->pseudoType())
        {
            ...
            case CSSSelector::PseudoEmpty:
                ...
                else
                {
                    ...
                    while(t && t->isTextNode() && static_cast< TextImpl * >(t)->length() == 0)
                        t = t->nextSibling();
            ...
            case CSSSelector::PseudoLang:
            {
                ...
                if(langAttr.length() < langSel.length())
                    return false;
...
{
    int diff = static_cast< CSSOrderedProperty * >(i1)->priority - static_cast< CSSOrderedProperty * >(i2)->priority;
    return diff ? diff : static_cast< CSSOrderedProperty * >(i1)->position - static_cast< CSSOrderedProperty * >(i2)->position;
}

void CSSOrderedPropertyList::append(DOM::CSSStyleDeclarationImpl *decl, uint selector, uint specificity, Source regular, Source important)
{
...
static const colorMap cmap[] = {{CSS_VAL_AQUA, 0xFF00FFFF},
...
static inline int nextFontSize(const QValueVector< int > &a, int v, bool smaller)
{
    ...
    while(l <= r)
    {
        m = (l + r) / 2;
        if(a[m] == v)
            return smaller ? (m ? a[m - 1] : (v * 5) / 6) : (m + 1 < int(a.count()) ? a[m + 1] : (v * 6) / 5);
        else if(v < a[m])
            r = m - 1;
        else
            l = m + 1;
...
void CSSStyleSelector::applyRule(int id, DOM::CSSValueImpl *value)
{
    ...
    bool isInitial = (value->cssValueType() == CSSValue::CSS_INITIAL) || (!parentNode && value->cssValueType() == CSSValue::CSS_INHERIT);
        ...
        case CSS_PROP_SCROLLBAR_ARROW_COLOR:
        {
            ...
            else
            {
                ...
                if(ident)
                {
                    ...
                    else if(ident == CSS_VAL_TRANSPARENT && id != CSS_PROP_BORDER_TOP_COLOR && id != CSS_PROP_BORDER_RIGHT_COLOR
                            && id != CSS_PROP_BORDER_BOTTOM_COLOR && id != CSS_PROP_BORDER_LEFT_COLOR)
                        col = QColor();
                    else
            ...
            // +inherit
            {
                ...
                if(primitiveValue && !apply)
                {
                    ...
                    if(type > CSSPrimitiveValue::CSS_PERCENTAGE && type < CSSPrimitiveValue::CSS_DEG)
                        ...
                        l = Length(primitiveValue->computeLength(style, paintDeviceMetrics), Fixed, primitiveValue->isQuirkValue());
                    else if(type == CSSPrimitiveValue::CSS_PERCENTAGE)
                        l = Length((int)primitiveValue->floatValue(CSSPrimitiveValue::CSS_PERCENTAGE), Percent);
                    else if(type == CSSPrimitiveValue::CSS_HTML_RELATIVE)
                        l = Length(int(primitiveValue->floatValue(CSSPrimitiveValue::CSS_HTML_RELATIVE)), Relative);
...
// based off the font family.
#ifdef APPLE_CHANGES
                const QValueVector< int > &fontSizes = (fontDef.genericFamily == FontDef::eMonospace) ? m_fixedFontSizes : m_fontSizes;
#else
            ...
            else
            {
                ...
                if(type > CSSPrimitiveValue::CSS_PERCENTAGE && type < CSSPrimitiveValue::CSS_DEG)
                {
                    if(!khtml::printpainter && type != CSSPrimitiveValue::CSS_EMS && type != CSSPrimitiveValue::CSS_EXS && view && view->part())
                        size = int(primitiveValue->computeLengthFloat(parentStyle, paintDeviceMetrics) * view->part()->zoomFactor()) / 100;
                    else
                        size = int(primitiveValue->computeLengthFloat(parentStyle, paintDeviceMetrics));
                }
                else if(type == CSSPrimitiveValue::CSS_PERCENTAGE)
                    size = int(primitiveValue->floatValue(CSSPrimitiveValue::CSS_PERCENTAGE) * parentStyle->font().pixelSize()) / 100;
                else
                    return;
            }
        ...
        case CSS_PROP_LINE_HEIGHT:
        {
            ...
            else if(type > CSSPrimitiveValue::CSS_PERCENTAGE && type < CSSPrimitiveValue::CSS_DEG)
            {
                ...
                if(!khtml::printpainter && type != CSSPrimitiveValue::CSS_EMS && type != CSSPrimitiveValue::CSS_EXS && view && view->part())
                    lineHeight = Length(primitiveValue->computeLength(style, paintDeviceMetrics) * view->part()->zoomFactor() / 100, Fixed);
                else
        ...
        case CSS_PROP_CONTENT:
            // list of string, uri, counter, attr, i
            {
                ...
                if(style->styleType() == RenderStyle::FIRST_LETTER || style->styleType() == RenderStyle::FIRST_LINE
...
void CSSStyleSelector::checkForGenericFamilyChange(RenderStyle *aStyle, RenderStyle *aParentStyle)
{
    ...
    if(childFont.genericFamily != FontDef::eMonospace && parentFont.genericFamily != FontDef::eMonospace)
        return;
