...
Value RegTestFunction::call(ExecState *exec, Object & /*thisObj*/, const List &args)
{
    ...
    switch(id)
    {
        ...
        case CheckOutput:
        {
            ...
            {
                if(!m_regTest->reportResult(m_regTest->checkOutput(filename + "-dom"), "Script-generated " + filename + "-dom"))
                    failures |= RegressionTest::DomFailure;
                if(!m_regTest->reportResult(m_regTest->checkOutput(filename + "-render"), "Script-generated " + filename + "-render"))
        ...
        case Quit:
            m_regTest->reportResult(true, "Called quit");
...
Value KHTMLPartFunction::call(ExecState *exec, Object & /*thisObj*/, const List &args)
{
    ...
    switch(id)
    {
        ...
        case OpenPageAsUrl:
        {
            ...
            {
                exec->setException(Error::create(exec, GeneralError, QString("Error reading " + filename).latin1()));
            }
            else
            {
                QByteArray fileData;
...
int main(int argc, char *argv[])
{
    ...
    KCmdLineArgs::init(argc, argv, "testregression", "TestRegression", "Regression tester for khtml", "1.0");
    ...
    if(args->isSet("xvfb"))
    {
        ...
        const char *const fontpaths[] = {"/usr/share/fonts/", "/usr/X11/lib/X11/fonts/", "/usr/lib/X11/fonts/", "/usr/share/fonts/X11/"};
        ...
        if(!xvfb)
        {
            ...
            execl(xvfbPath8.data(), xvfbPath8.data(), "-once", "-dpi", "100", "-screen", "0", "1024x768x16", "-ac", "-fp", buffer.data(), ":47",
                  (char *)NULL);
        }
    ...
    if(!args->isSet("debug"))
    {
        ...
        static int areas[] = {1000, 6000, 6005, 6010, 6020, 6030, 6031, 6035, 6036, 6040, 6041, 6045, 6050, 6060, 6061,
                              7000, 7006, 170,  171,  7101, 7002, 7019, 7027, 7014, 7011, 6070, 6080, 6090, 0};
    ...
    RegressionTest *regressionTest =
        new RegressionTest(part, baseDir, args->getOption("output"), args->isSet("genoutput"), !args->isSet("html"), !args->isSet("js"));
...
RegressionTest::RegressionTest(KHTMLPart *part, const QString &baseDir, const QString &outputDir, bool _genOutput, bool runJS, bool runHTML)
    : QObject(part)
{
    m_part = part;
    m_baseDir = baseDir;
    ...
    for(int y = 0; y < h; ++y)
    {
        ...
        if(memcmp(ls, rs, bytes))
        {
            for(int x = 0; x < w; ++x)
            {
                QRgb l = ls[x];
                QRgb r = rs[x];
                if((abs(qRed(l) - qRed(r)) < 20) && (abs(qGreen(l) - qGreen(r)) < 20) && (abs(qBlue(l) - qBlue(r)) < 20))
                    continue;
...
void RegressionTest::doFailureReport(const QString &test, int failures)
{
    ...
    {
        renderDiff += "<pre>";
        FILE *pipe = popen(QString::fromLatin1("diff -u baseline/%1-render %3/%2-render").arg(test, test, relOutputDir).latin1(), "r");
    ...
    {
        domDiff += "<pre>";
        FILE *pipe = popen(QString::fromLatin1("diff -u baseline/%1-dom %3/%2-dom").arg(test, test, relOutputDir).latin1(), "r");
        QTextIStream *is = new QTextIStream(pipe);
        for(int line = 0; line < 100 && !is->eof(); ++line)
        {
            QString line = is->readLine();
...
void RegressionTest::testStaticFile(const QString &filename)
{
    ...
    if(filename.endsWith(".html") || filename.endsWith(".htm"))
        ...
        args.serviceType = "application/xhtml+xml";
    ...
    if(filename.startsWith("domts/"))
    {
        ...
        {
            if(comp.value().isValid() && comp.value().isA(ObjectType) && (Object::dynamicCast(comp.value()).className() == "Array"))
            {
