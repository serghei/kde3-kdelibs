...
static KCmdLineOptions options[] = {{"debug", I18N_NOOP("Keep output results from scripts"), 0},
...
public:
    ...
    void copyGroup(KConfigBase *cfg1, const QString &grp1, KConfigBase *cfg2, const QString &grp2);
...
KonfUpdate::KonfUpdate() : m_textStream(0), m_file(0)
{
    ...
    for(QStringList::ConstIterator it = updateFiles.begin(); it != updateFiles.end(); ++it)
    {
        QString file = *it;
        updateFile(file);
    }
...
QStringList KonfUpdate::findUpdateFiles(bool dirtyOnly)
{
    ...
    for(QStringList::ConstIterator it = list.begin(); it != list.end(); ++it)
    {
        ...
        if(stat(QFile::encodeName(file), &buff) == 0)
        {
            ...
            if(!dirtyOnly || (ctime != buff.st_ctime) || (mtime != buff.st_mtime))
            {
                result.append(*it);
            }
        }
...
void KonfUpdate::gotKey(const QString &_key)
{
    ...
    // Delete old entry
    if((oldConfig2 == newConfig) && (oldGroup == newGroup) && (oldKey == newKey))
        return; // Don't delete!
    ...
    if(oldConfig2->deleteGroup(oldGroup, false))
...
void KonfUpdate::copyGroup(KConfigBase *cfg1, const QString &grp1, KConfigBase *cfg2, const QString &grp2)
{
    cfg1->setGroup(grp1);
    cfg2->setGroup(grp2);
        ...
        if(output.open(IO_ReadOnly))
        {
            ...
            while(!ts.atEnd())
            {
                ...
                else if(line.startsWith("# DELETE "))
                {
                    ...
                    if(oldConfig2->deleteGroup(group, false))
                    { // Delete group if empty.
                ...
                else if(line.startsWith("# DELETEGROUP"))
                {
                    ...
                    if(oldConfig2->deleteGroup(group, true))
                    { // Delete group
...
{
    KAboutData aboutData("kconf_update", I18N_NOOP("KConf Update"), "1.0.2", I18N_NOOP("KDE Tool for updating user configuration files"),
