...
#define INDEXOF(r, g, b) (((r) * (MAX_G + 1) + (g)) * (MAX_B + 1) + (b))
    ...
    for(rc = 0; rc <= MAX_R; rc++) // build 2x2x2 color cube
        for(gc = 0; gc <= MAX_G; gc++)
            ...
            {
                dst->setColor(INDEXOF(rc, gc, bc), qRgb(rc * 255 / MAX_R, gc * 255 / MAX_G, bc * 255 / MAX_B));
            }
...
bool KPixmap::load(const QString &fileName, const char *format, int conversion_flags)
{
...
bool KPixmap::load(const QString &fileName, const char *format, ColorMode mode)
{
...
bool KPixmap::convertFromImage(const QImage &img, int conversion_flags)
{
    ...
    if((conversion_flags & KColorMode_Mask) != LowOnly && (conversion_flags & KColorMode_Mask) != WebOnly)
    {
...
bool KPixmap::checkColorTable(const QImage &image)
{
    ...
    for(i = 0; i < ncols; i++)
    {
        for(j = 0; j < 40; j++)
        {
            if(kpixmap_iconPalette[j].red() == qRed(ctable[i]) && kpixmap_iconPalette[j].green() == qGreen(ctable[i])
...
KPixmap::KPixmap(const QPixmap &p) : QPixmap(p)
{
}
