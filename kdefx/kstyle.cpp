...
const double top_right_corner[16] = {0.949, 0.965, 0.980, 0.992, 0.851, 0.890, 0.945, 0.980, 0.706, 0.780, 0.890, 0.960, 0.608, 0.706, 0.851, 0.949};

const double bottom_right_corner[16] = {0.608, 0.706, 0.851, 0.949, 0.706, 0.780, 0.890, 0.960,
                                        0.851, 0.890, 0.945, 0.980, 0.949, 0.965, 0.980, 0.992};

const double bottom_left_corner[16] = {0.949, 0.851, 0.706, 0.608, 0.965, 0.890, 0.780, 0.706,
                                       0.980, 0.945, 0.890, 0.851, 0.992, 0.980, 0.960, 0.949};

const double shadow_strip[4] = {0.565, 0.675, 0.835, 0.945};
}
...
KStyle::KStyle(KStyleFlags flags, KStyleScrollBarType sbtype) : QCommonStyle(), d(new KStylePrivate)
{
    d->flags = flags;
    ...
    if(useMenuTransparency)
    {
        ...
        if(d->transparencyEngine != Disabled)
        {
            ...
            d->menuHandler = new TransparencyHandler(this, d->transparencyEngine, d->menuOpacity, d->menuDropShadow);
        }
    }
...
void KStyle::renderMenuBlendPixmap(KPixmap &pix, const QColorGroup &cg, const QPopupMenu * /* popup */) const
{
    pix.fill(cg.button()); // Just tint as the default behavior
}


void KStyle::drawKStylePrimitive(KStylePrimitive kpe, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags flags,
...
void KStyle::drawPrimitive(PrimitiveElement pe, QPainter *p, const QRect &r, const QColorGroup &cg, SFlags flags, const QStyleOption &opt) const
{
    ...
    if(pe == PE_DockWindowHandle)
    {
        ...
        if(parent && (parent->inherits("QToolBar") ||     // Normal toolbar
...
void KStyle::drawControl(ControlElement element, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags flags,
                         const QStyleOption &opt) const
{
    switch(element)
    {
        ...
        case CE_ProgressBarContents:
        {
            ...
            if(progress > 0 || steps == 0)
            {
                ...
                { // Busy indicator
                    ...
                    if(reverse)
                        p->fillRect(cr.x() + cr.width() - width - pstep, cr.y(), width, cr.height(), cg.brush(QColorGroup::Highlight));
                    else
                        p->fillRect(cr.x() + pstep, cr.y(), width, cr.height(), cg.brush(QColorGroup::Highlight));
                ...
                if(d->highcolor)
                {
                    ...
                    KPixmapEffect::gradient(pix, reverse ? c.light(150) : c.dark(150), reverse ? c.dark(150) : c.light(150),
                                            KPixmapEffect::HorizontalGradient);
                    if(reverse)
                        p->drawPixmap(cr.x() + (cr.width() - width), cr.y(), pix, cr.width() - width, 0, width, cr.height());
                    else
                        p->drawPixmap(cr.x(), cr.y(), pix, 0, 0, width, cr.height());
                }
                else if(reverse)
                    p->fillRect(cr.x() + (cr.width() - width), cr.y(), width, cr.height(), cg.brush(QColorGroup::Highlight));
                else
                    p->fillRect(cr.x(), cr.y(), width, cr.height(), cg.brush(QColorGroup::Highlight));
            }
            break;
        }
...
int KStyle::pixelMetric(PixelMetric m, const QWidget *widget) const
{
    switch(m)
    {
        ...
        case PM_DockWindowHandleExtent:
        {
            ...
            if(widget && (parent = widget->parentWidget()) && !parent->inherits("QToolBar") && !parent->inherits("QMainWindow")
        ...
        case PM_TabBarTabOverlap:
        {
            ...
            if((tbs == QTabBar::RoundedAbove) || (tbs == QTabBar::RoundedBelow))
                return 0;
            else
                return 2;
        }
        ...
        case PM_SliderControlThickness:
        {
            ...
            int thickness = (slider->orientation() == Horizontal) ? slider->height() : slider->width();
            switch(ts)
            {
...
void KStyle::drawComplexControl(ComplexControl control, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags flags,
                                SCFlags controls, SCFlags active, const QStyleOption &opt) const
{
    switch(control)
    {
        ...
        case CC_ScrollBar:
        {
            ...
            {
                drawPrimitive(PE_ScrollBarSubLine, p, subline, cg, sflags | (active == SC_ScrollBarSubLine ? Style_Down : Style_Default));

                if(useThreeButtonScrollBar && subline2.isValid())
                    drawPrimitive(PE_ScrollBarSubLine, p, subline2, cg, sflags | (active == SC_ScrollBarSubLine ? Style_Down : Style_Default));
            }

            if((controls & SC_ScrollBarAddLine) && addline.isValid())
                drawPrimitive(PE_ScrollBarAddLine, p, addline, cg, sflags | ((active == SC_ScrollBarAddLine) ? Style_Down : Style_Default));

            if((controls & SC_ScrollBarSubPage) && subpage.isValid())
                drawPrimitive(PE_ScrollBarSubPage, p, subpage, cg, sflags | ((active == SC_ScrollBarSubPage) ? Style_Down : Style_Default));

            if((controls & SC_ScrollBarAddPage) && addpage.isValid())
                drawPrimitive(PE_ScrollBarAddPage, p, addpage, cg, sflags | ((active == SC_ScrollBarAddPage) ? Style_Down : Style_Default));

            if((controls & SC_ScrollBarFirst) && first.isValid())
                drawPrimitive(PE_ScrollBarFirst, p, first, cg, sflags | ((active == SC_ScrollBarFirst) ? Style_Down : Style_Default));

            if((controls & SC_ScrollBarLast) && last.isValid())
                drawPrimitive(PE_ScrollBarLast, p, last, cg, sflags | ((active == SC_ScrollBarLast) ? Style_Down : Style_Default));

            if((controls & SC_ScrollBarSlider) && slider.isValid())
            {
                drawPrimitive(PE_ScrollBarSlider, p, slider, cg, sflags | ((active == SC_ScrollBarSlider) ? Style_Down : Style_Default));
                // Draw focus rect
                if(sb->hasFocus())
                {
        ...
        case CC_Slider:
        {
            ...
            if(slider->parentWidget() && slider->parentWidget()->backgroundPixmap() && !slider->parentWidget()->backgroundPixmap()->isNull())
            {
            ...
            if(controls & SC_SliderTickmarks)
                QCommonStyle::drawComplexControl(control, &p2, widget, r, cg, flags, SC_SliderTickmarks, active, opt);
        ...
        case CC_ListView:
        {
            ...
            if(controls & (SC_ListViewBranch | SC_ListViewExpand))
            {
                ...
                else
                {
                    ...
                    int lh = QMAX(p->fontMetrics().height() + 2 * v->itemMargin(), QApplication::globalStrut().height());
                    if(lh % 2 > 0)
                        lh++;
...
QStyle::SubControl KStyle::querySubControl(ComplexControl control, const QWidget *widget, const QPoint &pos, const QStyleOption &opt) const
{
...
QRect KStyle::querySubControlMetrics(ComplexControl control, const QWidget *widget, SubControl sc, const QStyleOption &opt) const
{
    QRect ret;
    ...
    if(control == CC_ScrollBar)
    {
        ...
        int maxlen = (horizontal ? sb->width() : sb->height()) - (sbextent * (threeButtonScrollBar ? 3 : 2));
        int sliderlen;
        ...
        switch(sc)
        {
            ...
            case SC_ScrollBarAddPage:
            {
                ...
                if(horizontal)
                    ret.setRect(sliderstart + sliderlen, 0, maxlen - sliderstart - sliderlen + fudge, sbextent);
                else
                    ret.setRect(0, sliderstart + sliderlen, sbextent, maxlen - sliderstart - sliderlen + fudge);
                break;
            }
...
static const char *const kstyle_close_xpm[] = {"12 12 2 1",    "# c #000000",  ". c None",     "............", "............",
                                               "..##....##..", "...##..##...", "....####....", ".....##.....", "....####....",
                                               "...##..##...", "..##....##..", "............", "............", "............"};

static const char *const kstyle_maximize_xpm[] = {"12 12 2 1",    "# c #000000",  ". c None",     "............", "............",
                                                  ".##########.", ".##########.", ".#........#.", ".#........#.", ".#........#.",
                                                  ".#........#.", ".#........#.", ".#........#.", ".##########.", "............"};


static const char *const kstyle_minimize_xpm[] = {"12 12 2 1",    "# c #000000",  ". c None",     "............", "............",
                                                  "............", "............", "............", "............", "............",
                                                  "...######...", "...######...", "............", "............", "............"};

static const char *const kstyle_normalizeup_xpm[] = {"12 12 2 1",    "# c #000000",  ". c None",     "............", "...#######..",
                                                     "...#######..", "...#.....#..", ".#######.#..", ".#######.#..", ".#.....#.#..",
                                                     ".#.....###..", ".#.....#....", ".#.....#....", ".#######....", "............"};


static const char *const kstyle_shade_xpm[] = {"12 12 2 1",    "# c #000000",  ". c None",     "............", "............",
                                               "............", "............", "............", ".....#......", "....###.....",
                                               "...#####....", "..#######...", "............", "............", "............"};

static const char *const kstyle_unshade_xpm[] = {"12 12 2 1",    "# c #000000",  ". c None",     "............", "............",
                                                 "............", "............", "..#######...", "...#####....", "....###.....",
                                                 ".....#......", "............", "............", "............", "............"};

static const char *const dock_window_close_xpm[] = {"8 8 2 1",  "# c #000000", ". c None", "##....##", ".##..##.", "..####..",
                                                    "...##...", "..####..",    ".##..##.", "##....##", "........"};
...
/* XPM */
static const char *const information_xpm[] = {"32 32 5 1",
                                              ". c None",
                                              "c c #000000",
                                              "* c #999999",
                                              "a c #ffffff",
                                              ...
                                              "...................**..........."};
/* XPM */
static const char *const warning_xpm[] = {"32 32 4 1",
                                          ". c None",
                                          "a c #ffff00",
                                          "* c #000000",
                                          "b c #999999",
...
/* XPM */
static const char *const critical_xpm[] = {"32 32 4 1",
                                           ". c None",
                                           "a c #999999",
                                           "* c #ff0000",
                                           "b c #ffffff",
...
QPixmap KStyle::stylePixmap(StylePixmap stylepixmap, const QWidget *widget, const QStyleOption &opt) const
{
    switch(stylepixmap)
    {
...
int KStyle::styleHint(StyleHint sh, const QWidget *w, const QStyleOption &opt, QStyleHintReturn *shr) const
{
    switch(sh)
    {
...
bool KStyle::eventFilter(QObject *object, QEvent *event)
{
    if(d->useFilledFrameWorkaround)
    {
        ...
        // -- Karol.
        QFrame *frame = 0;
        if(event->type() == QEvent::Paint && (frame = ::qt_cast< QFrame * >(object)))
        {
...
TransparencyHandler::TransparencyHandler(KStyle *style, TransparencyEngine tEngine, float menuOpacity, bool useDropShadow) : QObject()
{
    te = tEngine;
    kstyle = style;
    opacity = menuOpacity;
...
void TransparencyHandler::createShadowWindows(const QPopupMenu *p)
{
    ...
    QPixmap pix_shadow1 = QPixmap::grabWindow(qt_xrootwin(), shadow1.x(), shadow1.y(), shadow1.width(), shadow1.height());
    QPixmap pix_shadow2 = QPixmap::grabWindow(qt_xrootwin(), shadow2.x(), shadow2.y(), shadow2.width(), shadow2.height());
...
bool TransparencyHandler::eventFilter(QObject *object, QEvent *event)
{
    ...
    if(et == QEvent::Show)
    {
        ...
        if(te != Disabled)
        {
            pix = QPixmap::grabWindow(qt_xrootwin(), p->x(), p->y(), p->width(), p->height());
...
void TransparencyHandler::blendToPixmap(const QColorGroup &cg, const QPopupMenu *p)
{
    ...
    if(blendPix.width() != pix.width() || blendPix.height() != pix.height())
        return;
...
void TransparencyHandler::XRenderBlendToPixmap(const QPopupMenu *p)
{
    ...
    XRenderPictFormat *xformat = XRenderFindFormat(dpy, PictFormatType | PictFormatDepth | PictFormatAlphaMask, &Rpf, 0);
    ...
    XRenderComposite(dpy, PictOpOver, renderPix.x11RenderHandle(), alphaPicture, pix.x11RenderHandle(), // src, mask, dst
                     0, 0,                                                                              // srcx,  srcy
                     0, 0,                                                                              // maskx, masky
                     0, 0,                                                                              // dstx,  dsty
