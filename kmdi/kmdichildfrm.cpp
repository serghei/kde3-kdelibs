...
KMdiWin32IconButton::KMdiWin32IconButton(QWidget *parent, const char *name) : QLabel(parent, name)
{
}
...
void KMdiChildFrm::resizeWindow(int resizeCorner, int xPos, int yPos)
{
    ...
    if(m_pClient)
    {
        ...
        minHeight = m_pClient->minimumSize().height() + KMDI_CHILDFRM_DOUBLE_BORDER + m_pCaption->heightHint() + KMDI_CHILDFRM_SEPARATOR;
        maxWidth = m_pClient->maximumSize().width() + KMDI_CHILDFRM_DOUBLE_BORDER;
        maxHeight = m_pClient->maximumSize().height() + KMDI_CHILDFRM_DOUBLE_BORDER + m_pCaption->heightHint() + KMDI_CHILDFRM_SEPARATOR;
    }
        ...
        case Maximized:
            switch(m_state)
            {
                case Minimized:
                {
                    ...
                    int nFrameHeight = KMDI_CHILDFRM_DOUBLE_BORDER + KMDI_CHILDFRM_SEPARATOR + m_pCaption->heightHint();
                    setGeometry(-m_pClient->x(), -m_pClient->y(), m_pManager->width() + nFrameWidth, m_pManager->height() + nFrameHeight);
                    raise();
                }
                break;
                case Normal:
                {
                    ...
                    int nFrameWidth = KMDI_CHILDFRM_DOUBLE_BORDER;
                    int nFrameHeight = KMDI_CHILDFRM_DOUBLE_BORDER + KMDI_CHILDFRM_SEPARATOR + m_pCaption->heightHint();
                    QRect maximizedFrmRect(-m_pClient->x(), -m_pClient->y(), m_pManager->width() + nFrameWidth, m_pManager->height() + nFrameHeight);
...
void KMdiChildFrm::setClient(KMdiChildView *w, bool bAutomaticResize)
{
    ...
    {
        setMinimumHeight(m_pClient->minimumSize().height() + KMDI_CHILDFRM_DOUBLE_BORDER + m_pCaption->heightHint() + KMDI_CHILDFRM_SEPARATOR);
    }
}
...
void KMdiChildFrm::doResize(bool captionOnly)
{
    ...
    {
        QSize newClientSize(captionWidth, height() - (KMDI_CHILDFRM_DOUBLE_BORDER + captionHeight + KMDI_CHILDFRM_SEPARATOR));
        if(newClientSize != m_pClient->size())
        {
            m_pClient->setGeometry(KMDI_CHILDFRM_BORDER, m_pCaption->heightHint() + KMDI_CHILDFRM_SEPARATOR + KMDI_CHILDFRM_BORDER,
