...
KMdiChildArea::KMdiChildArea(QWidget *parent) : QFrame(parent, "kmdi_childarea")
{
...
QPoint KMdiChildArea::getCascadePoint(int indexOfWindow)
{
    if(indexOfWindow < 0)
    {
        ...
        kdDebug(760) << k_funcinfo << "indexOfWindow was less than zero, using " << indexOfWindow << " as new index" << endl;
    }
...
void KMdiChildArea::cascadeMaximized()
{
    ...
    while(!list.isEmpty())
    {
        ...
        if(childFrm->state() != KMdiChildFrm::Minimized)
        {
            ...
            if((childFrm->minimumSize().width() > curSize.width()) || (childFrm->minimumSize().height() > curSize.height()))
            {
...
// KDE4: remove
void KMdiChildArea::getCaptionColors(const QPalette & /*pal*/, QColor &activeBG, QColor &activeFG, QColor &inactiveBG, QColor &inactiveFG)
{
