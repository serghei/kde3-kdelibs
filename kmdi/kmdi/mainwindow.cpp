...
KMDI::ToolViewAccessor *MainWindow::addToolWindow(QWidget *pWnd, KDockWidget::DockPosition pos, QWidget *pTargetWnd, int percent,
...
{
    for(KDockWidget *dw = widgetsToReparent.first(); dw; dw = widgetsToReparent.next())
    {
        ...
        if(widget)
        {
            ...
            if(tg)
            {
                ...
                for(int i = 0; i < tg->count(); i++)
                    list->append((KDockWidget *)static_cast< KDockWidget * >(tg->page(i)));
            }
            else
