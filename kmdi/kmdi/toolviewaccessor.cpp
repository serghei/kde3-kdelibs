    ...
    : QObject(parent)
{
    ...
    else
    {
        ...
        d->widgetContainer = parent->createDockWidget(widgetToWrap->name(), (widgetToWrap->icon() ? (*(widgetToWrap->icon())) : QPixmap()),
                                                      0L, // parent
...
void ToolViewAccessor::setWidgetToWrap(QWidget *widgetToWrap, const QString &tabToolTip, const QString &tabCaption)
{
    ...
    if(!tmp)
    {
        tmp = mdiMainFrm->createDockWidget(widgetToWrap->name(), widgetToWrap->icon() ? (*(widgetToWrap->icon())) : QPixmap(),
                                           0L, // parent
                                           widgetToWrap->caption(), finalTabCaption);
