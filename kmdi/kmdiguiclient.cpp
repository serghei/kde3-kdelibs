...
ToggleToolViewAction::ToggleToolViewAction(const QString &text, const KShortcut &cut, KDockWidget *dw, KMdiMainFrm *mdiMainFrm, QObject *parent,
                                           const char *name)
...
void ToggleToolViewAction::anDWChanged()
{
    if(isChecked() && m_dw->mayBeShow())
        ...
        setChecked(true);
                            ...
                            && ((static_cast< KDockWidget * >(m_dw->parentDockTabGroup()->parent()->qt_cast("KDockWidget")))->mayBeShow())))
        setChecked(false);
}
...
KMDIGUIClient::KMDIGUIClient(KMdiMainFrm *mdiMainFrm, bool showMDIModeAction, const char *name) : QObject(mdiMainFrm, name), KXMLGUIClient(mdiMainFrm)
{
    m_mdiMode = KMdi::ChildframeMode;
    m_mdiMainFrm = mdiMainFrm;
    connect(mdiMainFrm->guiFactory(), SIGNAL(clientAdded(KXMLGUIClient *)), this, SLOT(clientAdded(KXMLGUIClient *)));
    ...
    */
    ...
    connect(m_mdiMainFrm, SIGNAL(mdiModeHasBeenChangedTo(KMdi::MdiMode)), this, SLOT(mdiModeHasBeenChangedTo(KMdi::MdiMode)));

    m_gotoToolDockMenu = new KActionMenu(i18n("Tool &Docks"), actionCollection(), "kmdi_tooldock_menu");
    m_gotoToolDockMenu->insert(
        new KAction(i18n("Switch Top Dock"), ALT + CTRL + SHIFT + Key_T, this, SIGNAL(toggleTop()), actionCollection(), "kmdi_activate_top"));
    m_gotoToolDockMenu->insert(
        new KAction(i18n("Switch Left Dock"), ALT + CTRL + SHIFT + Key_L, this, SIGNAL(toggleLeft()), actionCollection(), "kmdi_activate_left"));
    m_gotoToolDockMenu->insert(
        new KAction(i18n("Switch Right Dock"), ALT + CTRL + SHIFT + Key_R, this, SIGNAL(toggleRight()), actionCollection(), "kmdi_activate_right"));
    m_gotoToolDockMenu->insert(new KAction(i18n("Switch Bottom Dock"), ALT + CTRL + SHIFT + Key_B, this, SIGNAL(toggleBottom()), actionCollection(),
                                           "kmdi_activate_bottom"));
...
void KMDIGUIClient::addToolView(KMdiToolViewAccessor *mtva)
{
    ...
    KAction *a = new ToggleToolViewAction(i18n("Show %1").arg(mtva->wrappedWidget()->caption()),
                                          /*QString::null*/ sc, dynamic_cast< KDockWidget * >(mtva->wrapperWidget()), m_mdiMainFrm,
                                          actionCollection(), aname.latin1());
