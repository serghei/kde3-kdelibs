    ...
    , m_documentTabWidget(0)
{
    ...
    d->closeWindowAction = new KAction(i18n("&Close"), KStdAccel::close(), this, SLOT(closeActiveView()), actionCollection(), "window_close");
    ...
    connect(guiFactory(), SIGNAL(clientAdded(KXMLGUIClient *)), this, SLOT(verifyToplevelHeight()));
    connect(guiFactory(), SIGNAL(clientRemoved(KXMLGUIClient *)), this, SLOT(verifyToplevelHeight()));
}
...
void KMdiMainFrm::createMdiManager()
{
    ...
    QObject::connect(m_pMdi, SIGNAL(sysButtonConnectionsMustChange(KMdiChildFrm *, KMdiChildFrm *)), this,
                     SLOT(updateSysButtonConnections(KMdiChildFrm *, KMdiChildFrm *)));
...
KMdiChildView *KMdiMainFrm::createWrapper(QWidget *view, const QString &name, const QString &shortName)
{
    ...
    KMdiChildView *pMDICover = new KMdiChildView(name /*caption*/, 0L /*parent*/, name.latin1());
...
//============ addWindow ============//
KMdiToolViewAccessor *KMdiMainFrm::addToolWindow(QWidget *pWnd, KDockWidget::DockPosition pos, QWidget *pTargetWnd, int percent,
                                                 const QString &tabToolTip, const QString &tabCaption)
{
    QWidget *tvta = pWnd;
...
void KMdiMainFrm::attachWindow(KMdiChildView *pWnd, bool bShow, bool bAutomaticResize)
{
    ...
    if(topLeftMdiChildArea.x() < 0 || topLeftMdiChildArea.y() < 0 || (topLeftMdiChildArea.x() + frameGeo.width() > childAreaGeo.width())
    ...
    if(m_pMdi->topChild() && m_pMdi->topChild()->isMaximized())
    {
        ...
        lpC->setGeometry(-lpC->m_pClient->x(), -lpC->m_pClient->y(), m_pMdi->width() + KMDI_CHILDFRM_DOUBLE_BORDER,
    ...
    else if(e->type() == QEvent::KeyRelease)
    {
        if(switching())
        {
            ...
            if(a)
            {
                 ...
                 **/
                if(state != ((QKeyEvent *)e)->stateAfter() && ((modFlags & KKey::CTRL) > 0) == ((state & Qt::ControlButton) > 0)
                   && ((modFlags & KKey::ALT) > 0) == ((state & Qt::AltButton) > 0) && ((modFlags & KKey::WIN) > 0) == ((state & Qt::MetaButton) > 0))
                {
                    activeWindow()->updateTimeStamp();
                    setSwitching(false);
                }
    ...
    while((*it))
    {
        ...
        if(rootDockWindow)
        {
            ...
            if(!found || rootDockWidgetList->isEmpty())
            {
                ...
                QRect r(p.x(), p.y() + m_undockPositioningOffset.y(), rootDockWindow->width() - windowTitleHeight - frameBorderWidth * 2,
...
void KMdiMainFrm::setupTabbedDocumentViewSpace()
{
    ...
    for(; it4.current(); ++it4)
    {
        ...
        connect(pView, SIGNAL(captionUpdated(QWidget *, const QString &)), m_documentTabWidget,
                SLOT(updateCaptionInView(QWidget *, const QString &)));
    }
}
