...
static int dlopen_flag = RTLD_LAZY;
//#if defined Q_WS_X11 && ! defined K_WS_QTONLY
#ifdef Q_WS_X11
static int X11fd = -1;
...
static int get_current_desktop(Display *disp)
{
    ...
    if(XGetWindowProperty(disp, DefaultRootWindow(disp), net_current_desktop, 0l, 1l, False, XA_CARDINAL, &type_ret, &format_ret, &nitems_ret,
                          &unused, &data_ret)
       == Success)
    {
...
QCString execpath_avoid_loops(const QCString &exec, int envc, const char *envs, bool avoid_loops)
{
    ...
    QCString execpath = QFile::encodeName(s_instance->dirs()->findExe(exec, paths.join(QString(":"))));
    if(avoid_loops && !execpath.isEmpty())
    {
        ...
        execpath = QFile::encodeName(s_instance->dirs()->findExe(exec, paths.join(QString(":"))));
    }
    return execpath;
}
...
static pid_t launch(int argc, const char *_name, const char *args, const char *cwd = 0, int envc = 0, const char *envs = 0, bool reset_env = false,
                    const char *tty = 0, bool avoid_loops = false, const char *startup_id_str = "0")
{
    int launcher = 0;
    QCString lib;
    QCString name;
    ...
    if(0 > pipe(d.fd))
    {
        ...
        d.errorMsg = i18n(
                         ...
                         "The system may have reached the maximum number of open files possible or the maximum number of open files that you are "
                         "allowed to use has been reached.")
                         .utf8();
        close(d.fd[0]);
        close(d.fd[1]);
        d.fork = 0;
    ...
    switch(d.fork)
    {
        case -1:
            ...
            d.errorMsg = i18n(
                             ...
                             "The system may have reached the maximum number of processes possible or the maximum number of processes that you are "
                             "allowed to use has been reached.")
                             .utf8();
            close(d.fd[0]);
            close(d.fd[1]);
        ...
        case 0:
            ...
            if(reset_env) // KWRAPPER/SHELL
            {
                ...
                QStrList unset_envs;
                for(int tmp_env_count = 0; environ[tmp_env_count]; tmp_env_count++)
                    unset_envs.append(environ[tmp_env_count]);
                for(QStrListIterator it(unset_envs); it.current() != NULL; ++it)
                {
...
/** Give the process a new name **/
            ...
            if(!libpath.isEmpty())
            {
                d.handle = dlopen(QFile::encodeName(libpath), RTLD_LAZY);
                if(!d.handle)
                {
...
static void handle_launcher_request(int sock = -1)
{
    ...
    if(request_header.cmd == LAUNCHER_OK)
    {
        d.launcher_ok = true;
            ...
            && ((request_header.cmd == LAUNCHER_EXEC) || (request_header.cmd == LAUNCHER_EXT_EXEC) || (request_header.cmd == LAUNCHER_SHELL)
                || (request_header.cmd == LAUNCHER_KWRAPPER) || (request_header.cmd == LAUNCHER_EXEC_NEW)))
    {
        pid_t pid;
...
#ifndef NDEBUG
        fprintf(
            stderr, "kdeinit: Got %s '%s' from %s.\n",
            (request_header.cmd == LAUNCHER_EXEC
                 ? "EXEC"
                 : (request_header.cmd == LAUNCHER_EXT_EXEC
                        ? "EXT_EXEC"
                        : (request_header.cmd == LAUNCHER_EXEC_NEW ? "EXEC_NEW" : (request_header.cmd == LAUNCHER_SHELL ? "SHELL" : "KWRAPPER")))),
        ...
        if(request_header.cmd == LAUNCHER_SHELL || request_header.cmd == LAUNCHER_KWRAPPER || request_header.cmd == LAUNCHER_EXT_EXEC
        ...
        bool reset_display = (!olddisplay.isEmpty() && !kdedisplay.isEmpty() && olddisplay != kdedisplay);

        if(reset_display)
            setenv(DISPLAY, kdedisplay, true);

        pid = launch(argc, name, args, cwd, envc, envs, request_header.cmd == LAUNCHER_SHELL || request_header.cmd == LAUNCHER_KWRAPPER, tty,
    ...
    else if(request_header.arg_length && request_header.cmd == LAUNCHER_SETENV)
    {
        ...
        if(request_header.arg_length != (int)(strlen(env_name) + strlen(env_value) + 2))
        {
#ifndef NDEBUG
...
{
    QStringList ltdl_library_path = QStringList::split(':', QFile::decodeName(getenv("LTDL_LIBRARY_PATH")));
    QStringList ld_library_path = QStringList::split(':', QFile::decodeName(getenv("LD_LIBRARY_PATH")));
    ...
    // FIXME check if is need to be implemented
    // if (!extra_path.isEmpty())
...
int kdeinit_x_errhandler(Display *dpy, XErrorEvent *err)
{
    ...
    fprintf(stderr,
            "kdeinit: KDE detected X Error: %s %d\n"
            "         Major opcode: %d\n"
            "         Minor opcode: %d\n"
            "         Resource id:  0x%lx\n",
...
int main(int argc, char **argv, char **envp)
{
    ...
    if(!d.suicide && !getenv("KDE_IS_PRELINKED"))
    {
        ...
        if(!konq.isEmpty())
            (void)dlopen(QFile::encodeName(konq).data(), dlopen_flag);
    }
#endif
    if(launch_klauncher)
    {
