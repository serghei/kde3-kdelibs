...
bool IdleSlave::match(const QString &protocol, const QString &host, bool connected)
{
    if(mOnHold)
        return false;
    if(protocol != mProtocol)
        return false;
    if(host.isEmpty())
        return true;
    if(host != mHost)
...
KLauncher::KLauncher(int _kdeinitSocket, bool new_startup)
    ...
    , mAutoStart(new_startup)
    , dontBlockReading(false)
    , newStartup(new_startup)
{
#ifdef Q_WS_X11
    mCached_dpy = NULL;
#endif
    ...
    connect(dcopClient(), SIGNAL(applicationRegistered(const QCString &)), this, SLOT(slotAppRegistered(const QCString &)));
    dcopClient()->connectDCOPSignal("DCOPServer", "", "terminateKDE()", objId(), "terminateKDE()", false);
    ...
    connect(mPoolSocket, SIGNAL(accepted(KSocket *)), SLOT(acceptSlave(KSocket *)));

    connect(&mTimer, SIGNAL(timeout()), SLOT(idleTimeout()));

    kdeinitNotifier = new QSocketNotifier(kdeinitSocket, QSocketNotifier::Read);
    connect(kdeinitNotifier, SIGNAL(activated(int)), this, SLOT(slotKDEInitData(int)));
...
void KLauncher::destruct(int exit_code)
{
    if(kapp)
        ((KLauncher *)kapp)->close();
    // We don't delete kapp here, that's intentional.
    ::exit(exit_code);
}

bool KLauncher::process(const QCString &fun, const QByteArray &data, QCString &replyType, QByteArray &replyData)
{
    if((fun == "exec_blind(QCString,QValueList<QCString>)") || (fun == "exec_blind(QCString,QValueList<QCString>,QValueList<QCString>,QCString)"))
    {
        QDataStream stream(data, IO_ReadOnly);
        replyType = "void";
        QCString name;
    ...
    if((fun == "start_service_by_name(QString,QStringList)") || (fun == "start_service_by_desktop_path(QString,QStringList)")
       || (fun == "start_service_by_desktop_name(QString,QStringList)") || (fun == "kdeinit_exec(QString,QStringList)")
       || (fun == "kdeinit_exec_wait(QString,QStringList)") || (fun == "start_service_by_name(QString,QStringList,QValueList<QCString>,QCString)")
       ...
       || (fun == "kdeinit_exec(QString,QStringList,QValueList<QCString>)") || (fun == "kdeinit_exec_wait(QString,QStringList,QValueList<QCString>)")
       ...
       || (fun == "kdeinit_exec_wait(QString,QStringList,QValueList<QCString>,QCString)"))
    {
        ...
        else if((fun == "kdeinit_exec(QString,QStringList)") || (fun == "kdeinit_exec(QString,QStringList,QValueList<QCString>)")
                || (fun == "kdeinit_exec(QString,QStringList,QValueList<QCString>,QCString)"))
        {
...
void KLauncher::slotKDEInitData(int)
{
    klauncher_header request_header;
    ...
    dontBlockReading = false;
    int result = read_socket(kdeinitSocket, (char *)&request_header, sizeof(request_header));
    if(result == -1)
    {
    ...
    result = read_socket(kdeinitSocket, (char *)requestData.data(), request_header.arg_length);
    ...
    if(lastRequest && (request_header.cmd == LAUNCHER_OK))
    {
        ...
        kdDebug(7016) << lastRequest->name << " (pid " << lastRequest->pid << ") up and running." << endl;
    ...
    kdWarning(7016) << "Unexpected command from KDEInit (" << (unsigned int)request_header.cmd << ")" << endl;
}

void KLauncher::processDied(pid_t pid, long /* exitStatus */)
{
    ...
    for(; request; request = requestList.next())
    {
        if(request->pid == pid)
        {
            ...
            else if((request->dcop_service_type == KService::DCOP_Unique) && (dcopClient()->isApplicationRegistered(request->dcop_name)))
...
void KLauncher::slotAppRegistered(const QCString &appId)
{
    ...
    for(; request; request = nextRequest)
    {
        ...
        if((request->dcop_service_type == KService::DCOP_Unique)
           && ((appId == request->dcop_name) || dcopClient()->isApplicationRegistered(request->dcop_name)))
        {
            request->status = KLaunchRequest::Running;
            requestDone(request);
            continue;
        ...
        int l = strlen(rAppId);
        if((strncmp(rAppId, cAppId, l) == 0) && ((cAppId[l] == '\0') || (cAppId[l] == '-')))
        {
...
void KLauncher::autoStart(int phase)
{
...
void KLauncher::requestDone(KLaunchRequest *request)
{
    if((request->status == KLaunchRequest::Running) || (request->status == KLaunchRequest::Done))
    {
        DCOPresult.result = 0;
    ...
    else
    {
        ...
        {
            Display *dpy = NULL;
            if((mCached_dpy != NULL) && (request->startup_dpy == XDisplayString(mCached_dpy)))
                dpy = mCached_dpy;
            if(dpy == NULL)
...
void KLauncher::requestStart(KLaunchRequest *request)
{
                              ...
                              bool blind, bool autoStart)
{
    ...
    if((request->dcop_service_type == KService::DCOP_Unique) || (request->dcop_service_type == KService::DCOP_Multi))
    {
                                          ...
                                          const QValueList< QCString > &envs)
{
    ...
    Display *dpy = NULL;
    if(dpy_str != NULL && mCached_dpy != NULL && qstrcmp(dpy_str, XDisplayString(mCached_dpy)) == 0)
        dpy = mCached_dpy;
    if(dpy == NULL)
...
void KLauncher::queueRequest(KLaunchRequest *request)
{
...
void KLauncher::createArgs(KLaunchRequest *request, const KService::Ptr service, const QStringList &urls)
{
...
pid_t KLauncher::requestHoldSlave(const KURL &url, const QString &app_socket)
{
    IdleSlave *slave;
    for(slave = mSlaveList.first(); slave; slave = mSlaveList.next())
    {
...
pid_t KLauncher::requestSlave(const QString &protocol, const QString &host, const QString &app_socket, QString &error)
{
    IdleSlave *slave;
    for(slave = mSlaveList.first(); slave; slave = mSlaveList.next())
    {
...
void KLauncher::waitForSlave(pid_t pid)
{
    IdleSlave *slave;
    for(slave = mSlaveList.first(); slave; slave = mSlaveList.next())
    {
...
void KLauncher::acceptSlave(KSocket *slaveSocket)
{
    ...
    connect(slave, SIGNAL(statusUpdate(IdleSlave *)), this, SLOT(slotSlaveStatus(IdleSlave *)));
    if(!mTimer.isActive())
    {
...
void KLauncher::slotSlaveStatus(IdleSlave *slave)
{
