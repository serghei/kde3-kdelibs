...
private:
     ...
     */
    typedef enum { statusSuccess, statusClientError, statusServerError } StatusCode;

    /**
     * Login Mode for ftpOpenConnection
     */
    typedef enum { loginDefered, loginExplicit, loginImplicit } LoginMode;
     ...
     */
    bool ftpOpenCommand(const char *command, const QString &path, char mode, int errorcode, KIO::fileoffset_t offset = 0);
