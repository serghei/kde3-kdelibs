...
int FtpSocket::connectSocket(int iTimeOutSec, bool bControl)
{
    ...
    int iOpt = bControl ? KExtendedSocket::inetSocket : KExtendedSocket::noResolve;
    ...
    if(iCon < 0)
    {
        int iErrorCode = (status() == IO_LookupError) ? ERR_UNKNOWN_HOST : ERR_COULD_NOT_CONNECT;
...
Ftp::Ftp(const QCString &pool, const QCString &app) : SlaveBase("ftp", pool, app)
{
...
bool Ftp::ftpOpenConnection(LoginMode loginMode)
{
    ...
    kdDebug(7102) << "ftpOpenConnection " << m_host << ":" << m_port << " " << m_user << " [password hidden]" << endl;
...
bool Ftp::ftpLogin()
{
    ...
    do
    {
        ...
        if(needPass)
        {
            tempbuf = "pass ";
            tempbuf += pass.latin1();
            kdDebug(7102) << "Sending Login password: "
                          << "[protected]" << endl;
...
bool Ftp::ftpSendCmd(const QCString &cmd, int maxretries)
{
    ...
    {
        kdWarning(7102) << "Invalid command received (contains CR or LF):" << cmd.data() << endl;
    ...
    if((m_iRespType <= 0) || (m_iRespCode == 421))
    {
        ...
        else
        {
            ...
            {
                kdDebug(7102) << "Was not able to communicate with " << m_host << endl << "Attempting to re-establish connection." << endl;
...
int Ftp::ftpOpenPASVDataConnection()
{
    ...
    if(!start || (sscanf(start, "(%d,%d,%d,%d,%d,%d)", &i[0], &i[1], &i[2], &i[3], &i[4], &i[5]) != 6
...
int Ftp::ftpOpenEPRTDataConnection()
{
    ...
    m_data->setSocketFlags(KExtendedSocket::noResolve | KExtendedSocket::passiveSocket | KExtendedSocket::inetSocket);
    ...
    //  .arg(sin->port());
    QCString command;
    command.sprintf("eprt |%d|%s|%d|", sin->ianaFamily(), sin->nodeName().latin1(), sin->port());
...
int Ftp::ftpOpenPortDataConnection()
{
    ...
    QCString portCmd;
    portCmd.sprintf("port %d,%d,%d,%d,%d,%d", pData[2], pData[3], pData[4], pData[5], pData[0], pData[1]);
...
bool Ftp::ftpOpenCommand(const char *_command, const QString &_path, char _mode, int errorcode, KIO::fileoffset_t _offset)
{
    int errCode = 0;
...
{
    kdDebug(7102) << "Got slave_status host = " << (m_host.ascii() ? m_host.ascii() : "[None]") << " ["
                  << (m_bLoggedOn ? "Connected" : "Not connected") << "]" << endl;
...
bool Ftp::ftpReadDir(FtpEntry &de)
{
    ...
    while(!m_data->textEOF())
    {
        ...
        // kdDebug(7102) << "p_access=" << p_access << " p_junk=" << p_junk << " p_owner=" << p_owner << " p_group=" << p_group << " p_size=" <<
        // p_size << endl;

        de.access = 0;
        if(strlen(p_access) == 1 && p_junk[0] == '[')
        {                                            // Netware
        ...
        if(p_date_1 != 0 && (p_date_2 = strtok(NULL, " ")) != 0 && (p_date_3 = strtok(NULL, " ")) != 0 && (p_name = strtok(NULL, "\r\n")) != 0)
        {
            {
                QCString tmp(p_name);
                if(p_access[0] == 'l')
            ...
            static const char *s_months[12] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
            for(int c = 0; c < 12; c++)
...
Ftp::StatusCode Ftp::ftpGet(int &iError, int iCopyFile, const KURL &url, KIO::fileoffset_t llOffset)
{
    ...
    // the request...
    if(!ftpSize(url.path(), '?') && (m_iRespCode == 550) && ftpFolder(url.path(), false))
    {
...
Ftp::StatusCode Ftp::ftpPut(int &iError, int iCopyFile, const KURL &dest_url, int permissions, bool overwrite, bool resume)
{
    if(!ftpOpenConnection(loginImplicit))
        return statusServerError;
    ...
    if(result != 0) // error
    {
        ...
        if(bMarkPartial)
        {
            ...
            if(ftpSize(dest, 'I') && (processed_size < (unsigned long)config()->readNumEntry("MinimumKeepSize", DEFAULT_MINIMUM_KEEP_SIZE)))
            {
                QCString cmd = "DELE ";
...
Ftp::StatusCode Ftp::ftpCopyPut(int &iError, int &iCopyFile, QString sCopyFile, const KURL &url, int permissions, bool overwrite)
{
...
Ftp::StatusCode Ftp::ftpCopyGet(int &iError, int &iCopyFile, const QString sCopyFile, const KURL &url, int permissions, bool overwrite)
{
...
// doesn't work for copy? -> design flaw?
    ...
    if(iCopyFile == -1)
    {
        ...
        iError = (errno == EACCES) ? ERR_WRITE_ACCESS_DENIED : ERR_CANNOT_OPEN_FOR_WRITING;
        return statusClientError;
    }
