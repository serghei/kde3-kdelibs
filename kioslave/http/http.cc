...
static QString sanitizeCustomHTTPHeader(const QString &_header)
{
    ...
    for(QStringList::Iterator it = headers.begin(); it != headers.end(); ++it)
    {
        ...
        if(header.find(':') == -1 || header.startsWith("host") || header.startsWith("via"))
            continue;
...
void HTTPProtocol::resetSessionSettings()
{
    ...
    if(m_strProxyRealm.isEmpty() || !proxy.isValid() || m_proxyURL.host() != proxy.host()
       || (!proxy.user().isNull() && proxy.user() != m_proxyURL.user()) || (!proxy.pass().isNull() && proxy.pass() != m_proxyURL.pass()))
    {
        m_bProxyAuthValid = false;
        m_proxyURL = proxy;
        m_bUseProxy = m_proxyURL.isValid();

        kdDebug(7113) << "(" << m_pid << ") Using proxy: " << m_bUseProxy << " URL: " << m_proxyURL.url() << " Realm: " << m_strProxyRealm << endl;
    }

    m_bPersistentProxyConnection = config()->readBoolEntry("PersistentProxyConnection", false);
    kdDebug(7113) << "(" << m_pid << ") Enable Persistent Proxy Connection: " << m_bPersistentProxyConnection << endl;
    ...
    kdDebug(7113) << "(" << m_pid << ") ssl_was_in_use = " << metaData("ssl_was_in_use") << endl;

    m_request.referrer = QString::null;
    if(config()->readBoolEntry("SendReferrer", true) && (m_protocol == "https" || m_protocol == "webdavs" || metaData("ssl_was_in_use") != "TRUE"))
    {
    ...
    if(m_bIsSSL && m_bUseProxy && m_proxyURL.protocol() != "https" && m_proxyURL.protocol() != "webdavs")
    {
        m_bNeedTunnel = true;
        setRealHost(m_request.hostname);
        kdDebug(7113) << "(" << m_pid << ") SSL tunnel: Setting real hostname to: " << m_request.hostname << endl;
    }
    else
    {
        m_bNeedTunnel = false;
...
void HTTPProtocol::setHost(const QString &host, int port, const QString &user, const QString &pass)
{
    ...
    kdDebug(7113) << "(" << m_pid << ") Hostname is now: " << m_request.hostname << " (" << m_request.encoded_hostname << ")" << endl;
}
...
bool HTTPProtocol::checkRequestURL(const KURL &u)
{
    ...
    if(m_protocol != u.protocol().latin1())
    {
        ...
        reparseConfiguration();
        if(m_iDefaultPort != oldDefaultPort && m_request.port == oldDefaultPort)
...
void HTTPProtocol::retrieveContent(bool dataInternal /* = false */)
{
    ...
    if(!dataInternal)
    {
        if((m_responseCode == 204) && ((m_request.method == HTTP_GET) || (m_request.method == HTTP_POST)))
...
bool HTTPProtocol::retrieveHeader(bool close_connection)
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            kdDebug(7113) << "(" << m_pid << ") Previous Response: " << m_prevResponseCode << endl;
            kdDebug(7113) << "(" << m_pid << ") Current Response: " << m_responseCode << endl;
            ...
            if(m_responseCode < 400 && (m_prevResponseCode == 401 || m_prevResponseCode == 407))
                saveAuthorization();
            break;
        }
    }
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::stat " << url.prettyURL() << endl;
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::listDir " << url.url() << endl;
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::davGeneric " << url.url() << endl;
    ...
    for(uint i = 0; i < propstats.count(); i++)
    {
        ...
        for(QDomNode n = prop.firstChild(); !n.isNull(); n = n.nextSibling())
        {
            ...
            else if(property.tagName() == "source")
            {
                ...
                QDomElement source = property.namedItem("link").toElement().namedItem("dst").toElement();
                if(!source.isNull())
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::mkdir " << url.url() << endl;
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::get " << url.url() << endl;
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::put " << url.prettyURL() << endl;
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::copy " << src.prettyURL() << " -> " << dest.prettyURL() << endl;
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::rename " << src.prettyURL() << " -> " << dest.prettyURL() << endl;
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::del " << url.prettyURL() << endl;
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::post " << url.prettyURL() << endl;
...
void HTTPProtocol::davLock(const KURL &url, const QString &scope, const QString &type, const QString &owner)
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::davLock " << url.prettyURL() << endl;
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::davUnlock " << url.prettyURL() << endl;
        ...
        case 207:
            // 207 Multi-status
            {
                ...
                errorString = i18n(
                                  ...
                                  "summary of the reasons is below.<ul>")
                                  .arg(action)
                                  .arg(url);
...
bool HTTPProtocol::isOffline(const KURL &url)
{
    ...
    if(dcopClient()->call("kded", "networkstatus", "status(QString)", params, replyType, reply) && (replyType == "int"))
    {
        int result;
...
{
    m_linePtrUnget = m_rewindBuf, m_lineCountUnget = m_rewindCount;
    m_rewindCount = 0;
}
...
void HTTPProtocol::httpCheckConnection()
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::httpCheckConnection: "
                  << " Socket status: " << m_iSock << " Keep Alive: " << m_bKeepAlive << " First: " << m_bFirstRequest << endl;
    ...
    if(!m_bFirstRequest && (m_iSock != -1))
    {
        ...
        {
            if(m_state.hostname != m_request.hostname || m_state.port != m_request.port || m_state.user != m_request.user
...
bool HTTPProtocol::httpOpenConnection()
{
    ...
    if(m_state.doProxy)
    {
        ...
        kdDebug(7113) << "(" << m_pid << ") Connecting to proxy server: " << proxy_host << ", port: " << proxy_port << endl;
...
bool HTTPProtocol::httpOpen()
{
    ...
    if(!m_bIsTunneled && m_bNeedTunnel)
    {
        ...
        header = QString(
                     ...
                     "\r\n")
                     .arg(m_request.encoded_hostname)
                     .arg(m_request.port);
    ...
    else
    {
        ...
        if(!m_request.bNoAuth && m_responseCode != 401 && m_responseCode != 407 && Authentication != AUTH_Negotiate)
        {
            ...
            {
                Authentication =
                    info.digestInfo.startsWith("Basic") ? AUTH_Basic : info.digestInfo.startsWith("NTLM")
                                                                           ? AUTH_NTLM
...
try_again:
    ...
    do
    {
        ...
        if((strncasecmp(buf, "HTTP", 4) == 0) || (strncasecmp(buf, "ICY ", 4) == 0)) // Shoutcast support
        {
            ...
            else if(m_responseCode == 401 || m_responseCode == 407)
            {
                ...
                if(m_prevResponseCode != m_responseCode && (m_prevResponseCode == 401 || m_prevResponseCode == 407))
                    saveAuthorization();
        ...
        {
            QStringList options = QStringList::split(',', QString::fromLatin1(trimLead(buf + 11)));
        ...
        {
            QStringList cacheControls = QStringList::split(',', QString::fromLatin1(trimLead(buf + 14)));
            for(QStringList::ConstIterator it = cacheControls.begin(); it != cacheControls.end(); it++)
            {
        ...
        {
            setMetaData("content-location", QString::fromLatin1(trimLead(buf + 17)).stripWhiteSpace());
        }
        ...
        else if(strncasecmp(buf, "Content-type:", 13) == 0)
        {
            ...
            while(*pos)
            {
                ...
                if(*pos)
                {
                    ...
                    pos = end;
                    if(mediaValue.length() && (mediaValue[0] == '"') && (mediaValue[mediaValue.length() - 1] == '"'))
                        mediaValue = mediaValue.mid(1, mediaValue.length() - 2);

                    kdDebug(7113) << "(" << m_pid << ") Media-Parameter Attribute: " << mediaAttribute << endl;
                    kdDebug(7113) << "(" << m_pid << ") Media-Parameter Value: " << mediaValue << endl;
        ...
        else if(strncasecmp(buf, "Content-Disposition:", 20) == 0)
        {
            ...
            if(!dispositionFilename.isEmpty())
            {
                ...
                kdDebug(7113) << "(" << m_pid << ") Content-Disposition: filename=" << dispositionFilename << endl;
            }
        }
        ...
        else if(strncasecmp(buf, "Link:", 5) == 0)
        {
            ...
            QStringList link = QStringList::split(";", QString(buf).replace(QRegExp("^Link:[ ]*"), ""));
            if(link.count() == 2)
            {
        ...
        else if(strncasecmp(buf, "P3P:", 4) == 0)
        {
            ...
            for(QStringList::Iterator it = policyfields.begin(); it != policyfields.end(); ++it)
            {
                ...
                if(policy.count() == 2)
                {
                    ...
                    {
                        policyrefs << policy[1].replace(QRegExp("[\"\']"), "").stripWhiteSpace();
                    }
                    else if(policy[0].lower() == "cp")
                    {
                        ...
                        QStringList cps = QStringList::split(" ", policy[1].replace(QRegExp("[\"\']"), "").simplifyWhiteSpace());
    ...
    if((m_responseCode == 401 && Authentication == AUTH_None) || (m_responseCode == 407 && ProxyAuthentication == AUTH_None))
    {
        m_bUnauthorized = false;
        if(m_request.bErrorPage)
            errorPage();
    ...
    if(!locationStr.isEmpty())
    {
        ...
        if((u.protocol() != "http") && (u.protocol() != "https") && (u.protocol() != "ftp") && (u.protocol() != "webdav")
           && (u.protocol() != "webdavs"))
        {
            redirection(u);
        ...
        // the fragment:
        if(m_request.url.hasRef() && !u.hasRef() && (m_request.url.host() == u.host()) && (m_request.url.protocol() == u.protocol()))
        ...
        kdDebug(7113) << "(" << m_pid << ") request.url: " << m_request.url.url() << endl << "LocationStr: " << locationStr.data() << endl;

        kdDebug(7113) << "(" << m_pid << ") Requesting redirection to: " << u.url() << endl;
    ...
    if(m_strMimeType.startsWith("text/") && (m_strMimeType != "text/css") && (m_strMimeType != "text/x-javascript") && !hasCacheDirective)
    {
    ...
    if(m_qContentEncodings.last() == "gzip")
    {
        ...
        else if(m_request.allowCompressedPage && m_strMimeType != "application/x-tgz" && m_strMimeType != "application/x-targz"
                && m_strMimeType != "application/x-gzip" && m_request.url.path().right(6) == ".ps.gz")
        {
            m_qContentEncodings.remove(m_qContentEncodings.fromLast());
            m_strMimeType = QString::fromLatin1("application/x-gzpostscript");
        }
        else if((m_request.allowCompressedPage && m_strMimeType == "text/html")
                || (m_request.allowCompressedPage && m_strMimeType != "application/x-tgz" && m_strMimeType != "application/x-targz"
                    && m_strMimeType != "application/x-gzip" && m_request.url.path().right(3) != ".gz"))
        {
            // Unzip!
        }
        else
    ...
    // Crypto ones....
    else if(m_strMimeType == "application/pkix-cert" || m_strMimeType == "application/binary-certificate")
    {
    ...
    {
        if((m_request.url.path().right(7) == ".tar.gz") || (m_request.url.path().right(4) == ".tar"))
            m_strMimeType = QString::fromLatin1("application/x-tgz");
  ...
  if (!m_qContentEncodings.isEmpty())
  {
    ...
    {
        kdDebug(7113) << "(" << m_pid << ") Setting Content-Disposition type to: " << dispositionType << endl;
        setMetaData("content-disposition-type", dispositionType);
    }
    if(!dispositionFilename.isEmpty())
    {
        kdDebug(7113) << "(" << m_pid << ") Setting Content-Disposition filename to: " << dispositionFilename << endl;
    ...
    if(locationStr.isEmpty() && (!m_strMimeType.isEmpty() || m_request.method == HTTP_HEAD))
    {
...
void HTTPProtocol::addEncoding(QString encoding, QStringList &encs)
{
    ...
    else
    {
        kdDebug(7113) << "(" << m_pid << ") Unknown encoding encountered.  "
                      << "Please write code. Encoding = \"" << encoding << "\"" << endl;
    }
}
...
bool HTTPProtocol::sendBody()
{
    ...
    if(!sendOk)
    {
        kdDebug(7113) << "(" << m_pid << ") Connection broken when sending message body: (" << m_state.hostname << ")" << endl;
        error(ERR_CONNECTION_BROKEN, m_state.hostname);
        return false;
    }
...
void HTTPProtocol::httpClose(bool keepAlive)
{
    ...
    // khtml.
    if(keepAlive && (!m_bUseProxy || m_bPersistentProxyConnection || m_bIsTunneled))
    {
...
{
    kdDebug(7113) << "(" << m_pid << ") HTTPProtocol::mimetype: " << url.prettyURL() << endl;
    ...
    kdDebug(7113) << "(" << m_pid << ") http: mimetype = " << m_strMimeType << endl;
}
...
void HTTPProtocol::slotData(const QByteArray &_d)
{
    ...
    if(!m_dataInternal)
    {
        ...
        if(m_strMimeType.isEmpty() && !m_bRedirect && !(m_responseCode >= 300 && m_responseCode <= 399))
        {
            ...
            kdDebug(7113) << "(" << m_pid << ") Mimetype buffer size: " << m_mimeTypeBuffer.size() << endl;

            KMimeMagicResult *result;
            result = KMimeMagic::self()->findBufferFileType(m_mimeTypeBuffer, m_request.url.fileName());
            if(result)
            {
            ...
            if(m_strMimeType.isEmpty())
            {
                ...
                kdDebug(7113) << "(" << m_pid << ") Using default mimetype: " << m_strMimeType << endl;
            }
...
bool HTTPProtocol::readBody(bool dataInternal /* = false */)
{
    ...
    if(!dataInternal)
    {
        ...
        {
            totalSize(m_iSize);
            infoMessage(i18n("Retrieving %1 from %2...").arg(KIO::convertSize(m_iSize)).arg(m_request.hostname));
        }
        else
        {
            totalSize(0);
    ...
    QObject::connect(&chain, SIGNAL(output(const QByteArray &)), this, SLOT(slotData(const QByteArray &)));
    QObject::connect(&chain, SIGNAL(error(int, const QString &)), this, SLOT(error(int, const QString &)));
    ...
    while(!m_bEOF)
    {
        ...
        if(bytesReceived == -1)
        {
            ...
            kdDebug(7113) << "(" << m_pid << ") readBody: bytesReceived==-1 sz=" << (int)sz << " Connnection broken !" << endl;
            error(ERR_CONNECTION_BROKEN, m_state.hostname);
            return false;
        }
    ...
    if(useMD5)
    {
        ...
        if(m_sContentMD5 == calculatedMD5)
            ...
            kdDebug(7113) << "(" << m_pid << ") MD5 checksum MISMATCH! Expected: " << calculatedMD5 << ", Got: " << m_sContentMD5 << endl;
    }
    ...
    {
        kdDebug(7113) << "(" << m_pid << ") still " << KIO::number(m_iBytesLeft) << " bytes left! can't close cache entry!\n";
    }
...
void HTTPProtocol::addCookies(const QString &url, const QCString &cookieHeader)
{
    ...
    kdDebug(7113) << "(" << m_pid << ") "
                  << "Window ID: " << windowId << ", for host = " << url << endl;
...
QString HTTPProtocol::findCookies(const QString &url)
{
    ...
    if(!dcopClient()->call("kded", "kcookiejar", "findCookies(QString,long int)", params, replyType, reply))
    {
        kdWarning(7113) << "(" << m_pid << ") Can't communicate with kded_kcookiejar!" << endl;
        return result;
    }
    ...
    {
        kdError(7113) << "(" << m_pid << ") DCOP function findCookies(...) returns " << replyType << ", expected QString" << endl;
    }
    return result;
}
...
void HTTPProtocol::writeCacheEntry(const char *buffer, int nbytes)
{
    ...
    {
        kdDebug(7113) << "writeCacheEntry: File size reaches " << file_pos << "Kb, exceeds cache limits. (" << m_maxCacheSize << "Kb)" << endl;
        fclose(m_request.fcache);
        m_request.fcache = 0;
        QString filename = m_request.cef + ".new";
        ::unlink(QFile::encodeName(filename));
...
void HTTPProtocol::closeCacheEntry()
{
    ...
    if(result == 0)
    {
        ...
        kdWarning(7113) << "(" << m_pid << ") closeCacheEntry: error renaming "
                        << "cache entry. (" << filename << " -> " << m_request.cef << ")" << endl;
    }
...
void HTTPProtocol::configAuth(char *p, bool isForProxy)
{
    ...
    */
    if(isForProxy)
    {
        if((f == AUTH_None) || ((m_iProxyAuthCount > 0) && (f < ProxyAuthentication)))
        {
    ...
    {
        if((f == AUTH_None) || ((m_iWWWAuthCount > 0) && (f < Authentication)))
        {
...
bool HTTPProtocol::getAuthorization()
{
    ...
    if(repeatFailure)
    {
        ...
        if(Authentication == AUTH_Digest || ProxyAuthentication == AUTH_Digest)
        {
            ...
            if(pos != -1)
            {
                pos += 5;
                int len = auth.length();
                while(pos < len && (auth[pos] == ' ' || auth[pos] == '='))
                    pos++;
    ...
    if(!result)
    {
        ...
        // did not fail before.
        if(!repeatFailure && !info.username.isNull() && !info.password.isNull())
            result = true;
        else
        {
...
QString HTTPProtocol::createNegotiateAuth()
{
    ...
    major_status = gss_init_sec_context(&minor_status, GSS_C_NO_CREDENTIAL, &ctx, server, mech_oid, req_flags, GSS_C_INDEFINITE,
                                        GSS_C_NO_CHANNEL_BINDINGS, GSS_C_NO_BUFFER, NULL, &output_token, NULL, NULL);
...
QString HTTPProtocol::createNTLMAuth(bool isForProxy)
{
    ...
    if(len > 4)
    {
        ...
        KNTLM::getAuth(buf, challenge, user, passwd, domain, KNetwork::KResolver::localHostName(), false, false);
    }
    else
    {
...
void HTTPProtocol::calculateResponse(DigestAuthInfo &info, QCString &Response)
{
    ...
    kdDebug(7113) << "(" << m_pid << ") calculateResponse(): A2 => " << HA2 << endl;
    ...
    kdDebug(7113) << "(" << m_pid << ") calculateResponse(): Response => " << Response << endl;
}
...
QString HTTPProtocol::createDigestAuth(bool isForProxy)
{
    ...
    while(*p)
    {
        ...
        if(strncasecmp(p, "realm=", 6) == 0)
        {
            p += 6;
            while(*p == '"')
                p++; // Go past any number of " mark(s) first
            while(p[i] != '"')
                i++; // Read everything until the last " mark
    ...
    else
    {
        ...
        kdDebug(7113) << "(" << m_pid << ") createDigestAuth(): passed digest "
                                         "authentication credential test: "
                      << send << endl;
