...
static void qDBusRemoveTimeout(DBusTimeout *timeout, void *data)
{
    ...
    QDBusConnectionPrivate *d = static_cast< QDBusConnectionPrivate * >(data);
    for(QValueList< DBusTimeout * >::iterator it = d->pendingTimeouts.begin(); it != d->pendingTimeouts.end();)
    {
        if((*it) == timeout)
        {
...
static void qDBusRemoveWatch(DBusWatch *watch, void *data)
{
    ...
    if(it != d->watchers.end())
    {
        ...
        for(QDBusConnectionPrivate::WatcherList::iterator wit = list.begin(); wit != list.end(); ++wit)
        {
            if((*wit).watch == watch)
            {
...
QDBusConnectionPrivate::QDBusConnectionPrivate(QObject *parent) : QObject(parent), ref(1), mode(InvalidMode), connection(0), server(0), dispatcher(0)
{
...
void QDBusConnectionPrivate::setServer(DBusServer *s)
{
    ...
    dbus_server_set_watch_functions(server, qDBusAddWatch, qDBusRemoveWatch, qDBusToggleWatch, this, 0); // ### check return type?
    dbus_server_set_timeout_functions(server, qDBusAddTimeout, qDBusRemoveTimeout, qDBusToggleTimeout, this, 0);
...
void QDBusConnectionPrivate::setConnection(DBusConnection *dbc)
{
    ...
    dbus_connection_set_watch_functions(connection, qDBusAddWatch, qDBusRemoveWatch, qDBusToggleWatch, this, 0);
    dbus_connection_set_timeout_functions(connection, qDBusAddTimeout, qDBusRemoveTimeout, qDBusToggleTimeout, this, 0);
...
static void qDBusResultReceived(DBusPendingCall *pending, void *user_data)
{
    ...
    if(it != d->pendingCalls.end())
    {
        ...
        QObject::connect(d, SIGNAL(dbusPendingCallReply(const QDBusMessage &)), it.data()->receiver, it.data()->method.data());

        d->emitPendingCallReply(reply);

        QObject::disconnect(d, SIGNAL(dbusPendingCallReply(const QDBusMessage &)), it.data()->receiver, it.data()->method.data());
    }
...
int QDBusConnectionPrivate::sendWithReplyAsync(const QDBusMessage &message, QObject *receiver, const char *method)
{
    if(!receiver || !method)
        return 0;

    if(!QObject::connect(receiver, SIGNAL(destroyed(QObject *)), this, SLOT(objectDestroyed(QObject *))))
        return false;
