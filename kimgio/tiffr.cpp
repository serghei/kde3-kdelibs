...
KDE_EXPORT void kimgio_tiff_read(QImageIO *io)
{
    ...
    // open file
    tiff = TIFFClientOpen(QFile::encodeName(io->fileName()), "r", (thandle_t)io->ioDevice(), tiff_read, tiff_write, tiff_seek, tiff_close, tiff_size,
                          tiff_map, tiff_unmap);
    ...
    for(unsigned ctr = 0; ctr < (height >> 1);)
    {
        ...
        unsigned *line2 = (unsigned *)image.scanLine(height - (++ctr));
