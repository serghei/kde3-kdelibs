...
jas_image_t *read_image(const QImageIO *io)
{
    jas_stream_t *in = 0;
    ...
    else
    {
        ...
        while((size = io->ioDevice()->readBlock(b.data(), 4096)) > 0)
        {
            ...
            if((out->writeBlock(b.data(), size)) == -1)
                break;
        } // while
...
static bool render_view(gs_t &gs, QImage &qti)
{
    ...
    for(int i = 1; i < 3; ++i)
    {
        if(jas_image_cmptwidth(gs.altimage, cmptlut[i]) != width || jas_image_cmptheight(gs.altimage, cmptlut[i]) != height)
            return false;
    } // for

    if(!qti.create(jas_image_width(gs.altimage), jas_image_height(gs.altimage), 32))
        return false;
...
KDE_EXPORT void kimgio_jp2_read(QImageIO *io)
{
    if(jas_init())
        return;

    gs_t gs;
    if(!(gs.image = read_image(io)))
        return;
...
KDE_EXPORT void kimgio_jp2_write(QImageIO *io)
{
    if(jas_init())
        return;
    ...
    ts << "rate=" << ((io->quality() < 0) ? DEFAULT_RATE : io->quality() / 100.0F);
