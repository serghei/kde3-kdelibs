...
int main(int argc, char *argv[])
{
    ...
    for(; it.current(); ++it)
    {
        ...
        if(v.has(EntityNickname))
        {
            ...
            cerr << "Nickname value == " << v.contentLine(EntityNickname)->value()->asString() << endl;
        }
        ...
        if(v.has(EntityRevision))
        {
            ...
            DateValue *d = (DateValue *)v.contentLine(EntityRevision)->value();
            ...
            if(d->hasTime())
            {
                ...
                cerr << "Revision date does NOT have a time component" << endl;
        }
        ...
        if(v.has(EntityURL))
        {
            ...
            cerr << "URL Value == " << v.contentLine(EntityURL)->value()->asString() << endl;

            URIValue *urlVal = (URIValue *)v.contentLine(EntityURL)->value();

            assert(urlVal != 0);

            cerr << "URL scheme == " << urlVal->scheme() << endl;

            cerr << "URL scheme specific part == " << urlVal->schemeSpecificPart() << endl;
        }
        ...
        if(v.has(EntityN))
        {
            ...
            NValue *n = (NValue *)(v.contentLine(EntityN)->value());
