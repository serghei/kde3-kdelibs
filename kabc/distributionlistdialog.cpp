...
public:
    EntryItem(QListView *parent, const Addressee &addressee, const QString &email = QString::null)
...
{
    bool ok;
    QString name = KInputDialog::getText(i18n("New Distribution List"), i18n("Please enter &name:"), QString::null, &ok);
    if(!ok)
        return;
...
void DistributionListEditorWidget::editList()
{
    ...
    bool ok;
    QString name = KInputDialog::getText(i18n("Distribution List"), i18n("Please change &name:"), oldName, &ok);
    if(!ok)
        return;
...
{
    int result = KMessageBox::warningContinueCancel(this, i18n("Delete distribution list '%1'?").arg(mNameCombo->currentText()), QString::null,
                                                    KStdGuiItem::del());
...
{
    AddresseeItem *addresseeItem = static_cast< AddresseeItem * >(mAddresseeView->selectedItem());
...
void DistributionListEditorWidget::changeEmail()
{
    ...
    QString email = EmailSelector::getEmail(entryItem->addressee().emails(), entryItem->email(), this);
    list->removeEntry(entryItem->addressee(), entryItem->email());
