...
VCard::List VCardParser::parseVCards(const QString &text)
{
    ...
    for(it = lines.begin(); it != linesEnd; ++it)
    {
        ...
        {
            if(inVCard && !currentLine.isEmpty())
            { // now parse the line
                ...
                { // find all parameters
                    ...
                    for(++paramIt; paramIt != params.end(); ++paramIt)
                    {
                        ...
                        if(pair[1].find(',') != -1)
                        { // parameter in type=x,y,z format
                ...
                { // have to decode the data
                    ...
                    if(vCardLine.parameter("encoding").lower() == "b" || vCardLine.parameter("encoding").lower() == "base64")
                    {
                ...
                if(params.findIndex("charset") != -1)
                { // have to convert the data
                    QTextCodec *codec = QTextCodec::codecForName(vCardLine.parameter("charset").latin1());
                    if(codec)
                    {
                ...
                { // if charset not given, assume it's in UTF-8 (as used in previous KDE versions)
                    vCardLine.setValue(QString::fromUtf8(output));
                }
