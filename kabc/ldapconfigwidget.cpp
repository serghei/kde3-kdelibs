...
LdapConfigWidget::LdapConfigWidget(QWidget *parent, const char *name, WFlags fl) : QWidget(parent, name, fl)
{
    mProg = 0;
    mFlags = 0;
    mainLayout = new QGridLayout(this, 12, 4, 0, KDialog::spacingHint());
}

LdapConfigWidget::LdapConfigWidget(int flags, QWidget *parent, const char *name, WFlags fl) : QWidget(parent, name, fl)
{
    mFlags = flags;
    mProg = 0;
    mainLayout = new QGridLayout(this, 12, 4, 0, KDialog::spacingHint());
...
void LdapConfigWidget::initWidget()
{
    ...
    if(mFlags & (W_SIZELIMIT | W_TIMELIMIT))
        row++;
    ...
    if(mFlags & W_AUTHBOX)
    {
        ...
        QButtonGroup *authbox = new QButtonGroup(3, Qt::Horizontal, i18n("Authentication"), this);
...
void LdapConfigWidget::sendQuery()
{
    ...
    connect(job, SIGNAL(data(KIO::Job *, const QByteArray &)), this, SLOT(loadData(KIO::Job *, const QByteArray &)));
    connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(loadResult(KIO::Job *)));
...
LDAPUrl LdapConfigWidget::url() const
{
    ...
    if(mUser)
        _url.setUser(mUser->text());
    if(mPassword)
        _url.setPass(mPassword->text());
    if(mHost)
        _url.setHost(mHost->text());
    if(mPort)
        _url.setPort(mPort->value());
...
void LdapConfigWidget::setMech(const QString &mech)
{
    ...
    if(!mech.isEmpty())
    {
        ...
        while(i < mMech->count())
        {
            if(mMech->text(i) == mech)
                break;
            i++;
        }
        if(i == mMech->count())
            mMech->insertItem(mech);
