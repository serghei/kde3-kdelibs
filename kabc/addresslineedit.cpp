...
// $Id: addresslineedit.cpp 465272 2005-09-29 09:47:40Z mueller $
...
AddressLineEdit::AddressLineEdit(QWidget *parent, bool useCompletion, const char *name) : KLineEdit(parent, name)
{
...
void AddressLineEdit::init()
{
    ...
    if(m_useCompletion)
    {
        ...
        connect(s_LDAPSearch, SIGNAL(searchData(const QStringList &)), SLOT(slotLDAPSearchData(const QStringList &)));
    }

    if(m_useCompletion && !m_completionInitialized)
    {
        setCompletionObject(s_completion, false); // we handle it ourself
        connect(this, SIGNAL(completion(const QString &)), this, SLOT(slotCompletion()));

        KCompletionBox *box = completionBox();
        connect(box, SIGNAL(highlighted(const QString &)), this, SLOT(slotPopupCompletion(const QString &)));
        connect(box, SIGNAL(userCancelled(const QString &)), SLOT(userCancelled(const QString &)));
...
QStringList AddressLineEdit::addresses()
{
    ...
    for(it = addressBook->begin(); it != addressBook->end(); ++it)
    {
        ...
        QString n = (*it).prefix() + space + (*it).givenName() + space + (*it).additionalName() + space + (*it).familyName() + space + (*it).suffix();
        ...
        for(mit = emails.begin(); mit != emails.end(); ++mit)
        {
            ...
            if(!email.isEmpty())
            {
                ...
                { /* do we really need quotes around this name ? */
                ...
                if(!addr.isEmpty() && (email.find('<') == -1) && (email.find('>') == -1) && (email.find(',') == -1))
                    addr += '<' + email + '>';
                else
                    addr += email;
