...
ResourceLDAPKIO::ResourceLDAPKIO(const KConfig *config) : Resource(config)
{
    ...
    d->mCacheDst = KGlobal::dirs()->saveLocation("cache", "ldapkio") + "/" + type() + "_" + identifier();
    init();
}
...
QString ResourceLDAPKIO::findUid(const QString &uid)
{
    ...
    kdDebug(7125) << "ResourceLDAPKIO::findUid() uid: " << uid << " url " << url.prettyURL() << endl;

    KIO::ListJob *listJob = KIO::listDir(url, false /* no GUI */);
    connect(listJob, SIGNAL(entries(KIO::Job *, const KIO::UDSEntryList &)), SLOT(entries(KIO::Job *, const KIO::UDSEntryList &)));
    connect(listJob, SIGNAL(result(KIO::Job *)), this, SLOT(listResult(KIO::Job *)));
...
QCString ResourceLDAPKIO::addEntry(const QString &attr, const QString &value, bool mod)
{
    ...
    {
        if(mod)
            tmp += LDIF::assembleLine("replace", attr) + "\n";
...
bool ResourceLDAPKIO::AddresseeToLDIF(QByteArray &ldif, const Addressee &addr, const QString &olddn)
{
    QCString tmp;
    QString dn;
    QByteArray data;
    ...
    else
    {
        ...
        {
            dn = mAttributes["commonName"] + "=" + addr.assembledName() + "," + olddn.section(',', 1);
        }
        else
        {
            dn = olddn;
    ...
    {
        if(mod)
            tmp += LDIF::assembleLine("replace", mAttributes["mail"]) + "\n";
    ...
    {
        if(mod && mAttributes["mail"] != mAttributes["mailAlias"])
            tmp += LDIF::assembleLine("replace", mAttributes["mailAlias"]) + "\n";
    ...
    if(!mAttributes["jpegPhoto"].isEmpty())
    {
        ...
        if(mod)
            tmp += LDIF::assembleLine("replace", mAttributes["jpegPhoto"]) + "\n";
...
{
    KIO::Job *job = NULL;
    if(d->mCachePolicy == Cache_Always || (d->mCachePolicy == Cache_NoConnection && d->mError == KIO::ERR_COULD_NOT_CONNECT))
    {
...
bool ResourceLDAPKIO::load()
{
    ...
    if(d->mCachePolicy != Cache_Always)
    {
        ...
        connect(job, SIGNAL(data(KIO::Job *, const QByteArray &)), this, SLOT(data(KIO::Job *, const QByteArray &)));
        connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(syncLoadSaveResult(KIO::Job *)));
        enter_loop();
    }
    ...
    do
    {
        ...
        switch(ret)
        {
            ...
            case LDIF::Item:
                ...
                {
                    PhoneNumber phone(QString::fromUtf8(value, value.size()), PhoneNumber::Work);
                    d->mAddr.insertPhoneNumber(phone);
                }
                else if(name == mAttributes["facsimileTelephoneNumber"].lower())
                {
                    PhoneNumber phone(QString::fromUtf8(value, value.size()), PhoneNumber::Fax);
                    d->mAddr.insertPhoneNumber(phone);
                }
                else if(name == mAttributes["mobile"].lower())
                {
                    PhoneNumber phone(QString::fromUtf8(value, value.size()), PhoneNumber::Cell);
                    d->mAddr.insertPhoneNumber(phone);
                }
                else if(name == mAttributes["pager"].lower())
                {
                    PhoneNumber phone(QString::fromUtf8(value, value.size()), PhoneNumber::Pager);
                    d->mAddr.insertPhoneNumber(phone);
                }
...
void ResourceLDAPKIO::result(KIO::Job *job)
{
    ...
    if(cjob)
    {
        connect(cjob, SIGNAL(result(KIO::Job *)), this, SLOT(loadCacheResult(KIO::Job *)));
    }
    else
    {
        if(!mErrorMsg.isEmpty())
...
bool ResourceLDAPKIO::save(Ticket *)
{
    ...
    connect(job, SIGNAL(dataReq(KIO::Job *, QByteArray &)), this, SLOT(saveData(KIO::Job *, QByteArray &)));
    connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(syncLoadSaveResult(KIO::Job *)));
    enter_loop();
    if(mErrorMsg.isEmpty())
    {
...
bool ResourceLDAPKIO::asyncSave(Ticket *)
{
    ...
    KIO::Job *job = KIO::put(d->mLDAPUrl, -1, true, false, false);
    connect(job, SIGNAL(dataReq(KIO::Job *, QByteArray &)), this, SLOT(saveData(KIO::Job *, QByteArray &)));
    connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(saveResult(KIO::Job *)));
    return true;
}
