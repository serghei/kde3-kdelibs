...
ResourceLDAPKIOConfig::ResourceLDAPKIOConfig(QWidget *parent, const char *name) : KRES::ConfigWidget(parent, name)
{
...
AttributesDialog::AttributesDialog(const QMap< QString, QString > &attributes, int rdnprefix, QWidget *parent, const char *name)
    : KDialogBase(Plain, i18n("Attributes Configuration"), Ok | Cancel, Ok, parent, name, true, true)
{
    ...
    QGridLayout *layout = new QGridLayout(page, 4, (attributes.count() + 4) >> 1, 0, spacingHint());
    ...
    for(i = 1; i < mMapCombo->count(); i++)
    {
        ...
        for(; it2.current(); ++it2)
        {
            ...
            {
                if(mMapList[i][it2.currentKey()] != it2.current()->text())
                    break;
            }
            else
            {
                if(mDefaultMap[it2.currentKey()] != it2.current()->text())
                    break;
            }
        }
        if(!it2.current())
...
OfflineDialog::OfflineDialog(bool autoCache, int cachePolicy, const KURL &src, const QString &dst, QWidget *parent, const char *name)
    : KDialogBase(Plain, i18n("Offline Configuration"), Ok | Cancel, Ok, parent, name, true, true)
{
    QFrame *page = plainPage();
    ...
    mSrc = src;
    mDst = dst;
    mCacheGroup = new QButtonGroup(1, Qt::Horizontal, i18n("Offline Cache Policy"), page);
    ...
    mAutoCache = new QCheckBox(i18n("Refresh offline cache automatically"), page);
...
void OfflineDialog::loadCache()
{
    ...
    {
        KMessageBox::information(this, i18n("Successfully downloaded directory server contents!"));
    }
    else
    {
        KMessageBox::error(this, i18n("An error occurred downloading directory server contents into file %1.").arg(mDst));
    }
}
