...
ResourceFile::ResourceFile(const KConfig *config) : Resource(config), mFormat(0), mAsynchronous(false)
{
...
ResourceFile::ResourceFile(const QString &fileName, const QString &formatName) : Resource(0), mFormat(0), mAsynchronous(false)
{
    init(fileName, formatName);
}
...
Ticket *ResourceFile::requestSaveTicket()
{
    ...
    else
    {
        ...
        kdDebug(5700) << "ResourceFile::requestSaveTicket(): Unable to lock file '" << mFileName << "': " << mLock->error() << endl;
        return 0;
    }
...
bool ResourceFile::save(Ticket *)
{
    ...
    (void)KSaveFile::backupFile(mFileName, QString::null /*directory*/, extension);
