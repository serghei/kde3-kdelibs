...
static const KCmdLineOptions options[] = {{"disable-autostart", I18N_NOOP("Disable automatic startup on login"), 0},
                                          {"quiet", "", 0},
                                          {"o", 0, 0},
...
void readKAddressBookEntries(const QString &dataString, Addressee &a)
{
    ...
    for(it = entries.begin(); it != entries.end(); ++it)
    {
        ...
        {
            a.insertPhoneNumber(PhoneNumber(fieldValue, PhoneNumber::Home | PhoneNumber::Fax));
        }
        ...
        {
            a.insertPhoneNumber(PhoneNumber(fieldValue, PhoneNumber::Work | PhoneNumber::Fax));
        }
    ...
    if(!homeAddress.isEmpty())
        a.insertAddress(homeAddress);
    if(!businessAddress.isEmpty())
        a.insertAddress(businessAddress);
    if(!otherAddress.isEmpty())
        a.insertAddress(otherAddress);
}
...
void importKab(KABC::AddressBook *ab, bool override, bool quiet)
{
    ...
    if(!QFile::exists(fileName))
    {
        if(!quiet)
        {
            KMessageBox::error(0, "<qt>"
                                      + i18n("Address book file <b>%1</b> not found! Make sure the old address book is located there and you have "
    ...
    for(int i = 0; i < num; ++i)
    {
        ...
        for(phoneIt = entry.telephone.begin(); phoneIt != entry.telephone.end(); ++phoneIt)
        {
            ...
            int type = 0;
            if(kabType == ::AddressBook::Fixed)
                type = PhoneNumber::Voice;
            else if(kabType == ::AddressBook::Mobile)
                type = PhoneNumber::Cell | PhoneNumber::Voice;
            else if(kabType == ::AddressBook::Fax)
                type = PhoneNumber::Fax;
            else if(kabType == ::AddressBook::Modem)
        ...
        if(!entry.user1.isEmpty())
            note += "\nUser1: " + entry.user1;
        if(!entry.user2.isEmpty())
            note += "\nUser2: " + entry.user2;
        if(!entry.user3.isEmpty())
            note += "\nUser3: " + entry.user3;
