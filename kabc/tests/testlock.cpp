...
LockWidget::LockWidget(const QString &identifier)
{
    ...
    else
    {
        ...
        QLabel *pidLabel = new QLabel("Process ID: " + QString::number(pid), this);
    ...
    connect(watch, SIGNAL(dirty(const QString &)), SLOT(updateLockView()));
    connect(watch, SIGNAL(created(const QString &)), SLOT(updateLockView()));
    connect(watch, SIGNAL(deleted(const QString &)), SLOT(updateLockView()));
...
static const KCmdLineOptions options[] = {{"a", 0, 0},
