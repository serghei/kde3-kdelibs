...
void TestLDAPClient::testIntevation()
{
    ...
    connect(mClient, SIGNAL(result(const KABC::LdapObject &)), this, SLOT(slotLDAPResult(const KABC::LdapObject &)));
    connect(mClient, SIGNAL(done()), this, SLOT(slotLDAPDone()));
    connect(mClient, SIGNAL(error(const QString &)), this, SLOT(slotLDAPError(const QString &)));
