...
HighContrastStyle::HighContrastStyle() : KStyle(0, ThreeButtonScrollBar)
{
    QSettings settings;
...
{
    if(widget->inherits("QButton") || widget->inherits("QComboBox") || widget->inherits("QSpinWidget") || widget->inherits("QLineEdit")
...
{
    if(widget->inherits("QWidget") || widget->inherits("QComboBox") || widget->inherits("QSpinWidget") || widget->inherits("QLineEdit")
       || widget->inherits("QTextEdit"))
...
void HighContrastStyle::drawPrimitive(PrimitiveElement pe, QPainter *p, const QRect &r, const QColorGroup &cg, SFlags flags,
...
void HighContrastStyle::drawKStylePrimitive(KStylePrimitive kpe, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg,
...
void HighContrastStyle::drawControl(ControlElement element, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags flags,
                                    const QStyleOption &opt) const
{
    if(widget == hoverWidget)
        flags |= Style_MouseOver;
    ...
    switch(element)
    {
        ...
        case CE_TabBarTab:
        {
            ...
            if(shape == QTabBar::TriangularBelow || shape == QTabBar::RoundedBelow)
            {
                p->fillRect(r.left(), r.top(), r.width(), 2 * basicLineWidth, p->pen().color());
                p->fillRect(r.left() + basicLineWidth, flags & Style_Selected ? basicLineWidth : 2 * basicLineWidth, r.width() - 2 * basicLineWidth,
                            basicLineWidth, p->backgroundColor());
            }
            else
            {
                p->fillRect(r.left(), r.bottom() - 2 * basicLineWidth + 1, r.width(), 2 * basicLineWidth, p->pen().color());
                p->fillRect(r.left() + basicLineWidth, r.bottom() - 2 * basicLineWidth + 1, r.width() - 2 * basicLineWidth,
                            flags & Style_Selected ? 2 * basicLineWidth : basicLineWidth, p->backgroundColor());
            }
            break;
        }
        ...
        case CE_PushButtonLabel:
        {
            ...
            if(!icon.isNull())
            {
                ...
                if(text.isEmpty() && ((pixmap == 0) || pixmap->isNull()))
                    p->drawPixmap(x + (w - icon.width()) / 2, y + (h - icon.height()) / 2, icon);
                else
            ...
            drawItem(p, QRect(x, y, w, h), (element == CE_RadioButtonLabel || element == CE_CheckBoxLabel || element == CE_ProgressBarLabel)
                                               ? AlignVCenter | AlignLeft | ShowPrefix
        ...
        case CE_MenuBarItem:
        {
            ...
            if(!opt.isDefault())
            {
                ...
                QColor color = p->pen().color();
                drawItem(p, r, AlignCenter | AlignVCenter | ShowPrefix | DontClip | SingleLine, cg, flags, mi->pixmap(), mi->text(), -1, &color);
            }
            break;
        }
        ...
        case CE_ProgressBarGroove:
        {
            ...
            {
                QRect r2(r);
                r2.setLeft(p->boundingRect(r, AlignVCenter | AlignLeft | ShowPrefix, progressbar->progressString()).right() + 4 * basicLineWidth);
                drawRoundRect(p, r2);
            }
            break;
        }
...
void HighContrastStyle::drawControlMask(ControlElement element, QPainter *p, const QWidget *w, const QRect &r, const QStyleOption &opt) const
{
    switch(element)
    {
        case CE_PushButton:
...
void HighContrastStyle::drawComplexControl(ComplexControl control, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg,
...
void HighContrastStyle::drawComplexControlMask(ComplexControl c, QPainter *p, const QWidget *w, const QRect &r, const QStyleOption &o) const
{
    switch(c)
    {
        case CC_SpinWidget:
...
void HighContrastStyle::drawItem(QPainter *p, const QRect &r, int flags, const QColorGroup &cg, bool enabled, const QPixmap *pixmap,
...
QRect HighContrastStyle::querySubControlMetrics(ComplexControl control, const QWidget *widget, SubControl subcontrol, const QStyleOption &opt) const
{
    switch(control)
    {
        case CC_ComboBox:
        {
            ...
            switch(subcontrol)
            {
                ...
                case SC_ComboBoxEditField:
                    return QRect(2 * basicLineWidth, 2 * basicLineWidth, widget->width() - arrow - 3 * basicLineWidth,
                                 widget->height() - 4 * basicLineWidth);
        ...
        case CC_SpinWidget:
        {
            ...
            switch(subcontrol)
            {
                ...
                case SC_SpinWidgetDown:
                    return QRect(widget->width() - arrow, widget->height() / 2, arrow, widget->height() - widget->height() / 2);
...
QSize HighContrastStyle::sizeFromContents(ContentsType contents, const QWidget *widget, const QSize &contentSize, const QStyleOption &opt) const
{
    switch(contents)
    {
        // PUSHBUTTON SIZE
...
bool HighContrastStyle::eventFilter(QObject *object, QEvent *event)
{
    ...
    if(widget)
    {
        ...
        if(event->type() == QEvent::Enter && (widget->inherits("QButton") || widget->inherits("QComboBox") || widget->inherits("QSpinWidget")))
        {
            hoverWidget = widget;
            widget->repaint(false);
        }
        else if(event->type() == QEvent::Leave && (widget->inherits("QButton") || widget->inherits("QComboBox") || widget->inherits("QSpinWidget")))
        {
        ...
        else if(event->type() == QEvent::FocusIn || event->type() == QEvent::FocusOut)
        {
            ...
            while(widgetparent && !widgetparent->inherits("QComboBox") && !widgetparent->inherits("QSpinWidget"))
            {
