/*
 $Id: kthemebase.cpp 465272 2005-09-29 09:47:40Z mueller $
...
static QColor readColorEntry(QSettings *s, const char *pKey, const QColor *pDefault)
{
    QColor aRetColor;
...
static const char *const widgetEntries[] = {
    ...
    "PushButton", "ComboBox", "HSBarSlider", "VSBarSlider", "Bevel", "ToolButton", "ScrollButton", "HScrollDeco", "VScrollDeco", "ComboDeco",
    "MenuItem", "Tab", "ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight",
    // sunken widgets
    "PushButtonDown", "ComboBoxDown", "HSBarSliderDown", "VSBarSliderDown", "BevelDown", "ToolButtonDown", "ScrollButtonDown", "HScrollDecoDown",
    "VScrollDecoDown", "ComboDecoDown", "MenuItemDown", "TabDown", "SunkenArrowUp", "SunkenArrowDown", "SunkenArrowLeft", "SunkenArrowRight",
    // everything else
    "HScrollGroove", "VScrollGroove", "Slider", "SliderGroove", "CheckBoxDown", "CheckBox", "CheckBoxTri", "RadioDown", "Radio", "HBarHandle",
    "VBarHandle", "ToolBar", "Splitter", "CheckMark", "MenuBar", "DisableArrowUp", "DisableArrowDown", "DisableArrowLeft", "DisableArrowRight",
    "ProgressBar", "ProgressBackground", "MenuBarItem", "Background", "RotSlider", "RotInactiveTab", "RotActiveTab",
};
...
void KThemeBase::generateBorderPix(int i)
{
    ...
    if(pbPixmaps[i])
    {
        ...
        bitBlt(&tmp, 0, 0, pbPixmaps[i], 0, 0, pbWidth[i], pbWidth[i], Qt::CopyROP, false);
        if(srcMask)
        {
            bitBlt(&destMask, 0, 0, srcMask, 0, 0, pbWidth[i], pbWidth[i], Qt::CopyROP, false);
            tmp.setMask(destMask);
        }
        pbPixmaps[i]->setBorder(KThemePixmap::TopLeft, tmp);

        bitBlt(&tmp, 0, 0, pbPixmaps[i], pbPixmaps[i]->width() - pbWidth[i], 0, pbWidth[i], pbWidth[i], Qt::CopyROP, false);
        if(srcMask)
        {
            bitBlt(&destMask, 0, 0, srcMask, pbPixmaps[i]->width() - pbWidth[i], 0, pbWidth[i], pbWidth[i], Qt::CopyROP, false);
            tmp.setMask(destMask);
        }
        pbPixmaps[i]->setBorder(KThemePixmap::TopRight, tmp);

        bitBlt(&tmp, 0, 0, pbPixmaps[i], 0, pbPixmaps[i]->height() - pbWidth[i], pbWidth[i], pbWidth[i], Qt::CopyROP, false);
        if(srcMask)
        {
            bitBlt(&destMask, 0, 0, srcMask, 0, pbPixmaps[i]->height() - pbWidth[i], pbWidth[i], pbWidth[i], Qt::CopyROP, false);
            tmp.setMask(destMask);
        }
        pbPixmaps[i]->setBorder(KThemePixmap::BottomLeft, tmp);

        bitBlt(&tmp, 0, 0, pbPixmaps[i], pbPixmaps[i]->width() - pbWidth[i], pbPixmaps[i]->height() - pbWidth[i], pbWidth[i], pbWidth[i], Qt::CopyROP,
               false);
        if(srcMask)
        {
            bitBlt(&destMask, 0, 0, srcMask, pbPixmaps[i]->width() - pbWidth[i], pbPixmaps[i]->height() - pbWidth[i], pbWidth[i], pbWidth[i],
                   Qt::CopyROP, false);
            tmp.setMask(destMask);
        }
        ...
        bitBlt(&tmp, 0, 0, pbPixmaps[i], pbWidth[i], 0, pbPixmaps[i]->width() - pbWidth[i] * 2, pbWidth[i], Qt::CopyROP, false);
        if(srcMask)
        {
            bitBlt(&destMask, 0, 0, srcMask, pbWidth[i], 0, pbPixmaps[i]->width() - pbWidth[i] * 2, pbWidth[i], Qt::CopyROP, false);
            tmp.setMask(destMask);
        }
        pbPixmaps[i]->setBorder(KThemePixmap::Top, tmp);

        bitBlt(&tmp, 0, 0, pbPixmaps[i], pbWidth[i], pbPixmaps[i]->height() - pbWidth[i], pbPixmaps[i]->width() - pbWidth[i] * 2, pbWidth[i],
               Qt::CopyROP, false);
        if(srcMask)
        {
            bitBlt(&destMask, 0, 0, srcMask, pbWidth[i], pbPixmaps[i]->height() - pbWidth[i], pbPixmaps[i]->width() - pbWidth[i] * 2, pbWidth[i],
                   Qt::CopyROP, false);
            tmp.setMask(destMask);
        }
        ...
        bitBlt(&tmp, 0, 0, pbPixmaps[i], 0, pbWidth[i], pbWidth[i], pbPixmaps[i]->height() - pbWidth[i] * 2, Qt::CopyROP, false);
        if(srcMask)
        {
            bitBlt(&destMask, 0, 0, srcMask, 0, pbWidth[i], pbWidth[i], pbPixmaps[i]->height() - pbWidth[i] * 2, Qt::CopyROP, false);
            tmp.setMask(destMask);
        }

        pbPixmaps[i]->setBorder(KThemePixmap::Left, tmp);

        bitBlt(&tmp, 0, 0, pbPixmaps[i], pbPixmaps[i]->width() - pbWidth[i], pbWidth[i], pbWidth[i], pbPixmaps[i]->height() - pbWidth[i] * 2,
               Qt::CopyROP, false);
        if(srcMask)
        {
            bitBlt(&destMask, 0, 0, srcMask, pbPixmaps[i]->width() - pbWidth[i], pbWidth[i], pbWidth[i], pbPixmaps[i]->height() - pbWidth[i] * 2,
                   Qt::CopyROP, false);
            tmp.setMask(destMask);
        }
...
void KThemeBase::copyWidgetConfig(int sourceID, int destID, QString *pixnames, QString *brdnames)
{
...
#define PREBLEND_ITEMS 12
    static const WidgetType preBlend[] = {Slider,      IndicatorOn,     IndicatorOff,    ExIndicatorOn, ExIndicatorOff, HScrollDeco,
                                          VScrollDeco, HScrollDecoDown, VScrollDecoDown, ComboDeco,     ComboDecoDown,  CheckMark};
...
KThemeBase::KThemeBase(const QString &dir, const QString &configFile) : KStyle(FilledFrameWorkaround), configFileName(configFile)
{
...
KThemePixmap *KThemeBase::scale(int w, int h, WidgetType widget) const
{
    ...
    {
        if(!pixmaps[widget] || pixmaps[widget]->width() != w || pixmaps[widget]->height() != h)
        {
    ...
    else if(scaleHints[widget] == HorizontalScale)
    {
        if(pixmaps[widget]->width() != w)
        {
            ...
            {
                cache->insert(pixmaps[widget], KThemeCache::HorizontalScale, widget);
                QImage tmpImg = images[widget]->smoothScale(w, images[widget]->height());
                pixmaps[widget] = new KThemePixmap;
                pixmaps[widget]->convertFromImage(tmpImg);
                if(blends[widget] != 0.0)
                    blend(widget);
    ...
    else if(scaleHints[widget] == VerticalScale)
    {
        if(pixmaps[widget]->height() != h)
        {
            ...
            {
                cache->insert(pixmaps[widget], KThemeCache::VerticalScale, widget);
                QImage tmpImg = images[widget]->smoothScale(images[widget]->width(), h);
                pixmaps[widget] = new KThemePixmap;
                pixmaps[widget]->convertFromImage(tmpImg);
                if(blends[widget] != 0.0)
                    blend(widget);
...
KThemePixmap *KThemeBase::scaleBorder(int w, int h, WidgetType widget) const
{
    ...
    else
    {
        ...
        bitBlt(pixmap, 0, 0, tmp, 0, 0, bdWidth, bdWidth, Qt::CopyROP, false);
        if(srcMask)
            bitBlt(&mask, 0, 0, srcMask, 0, 0, bdWidth, bdWidth, Qt::CopyROP, false);
        else
        ...
        srcMask = tmp->mask();
        bitBlt(pixmap, w - bdWidth, 0, tmp, 0, 0, bdWidth, bdWidth, Qt::CopyROP, false);
        if(srcMask)
            bitBlt(&mask, w - bdWidth, 0, srcMask, 0, 0, bdWidth, bdWidth, Qt::CopyROP, false);
        else
            mPainter.fillRect(w - bdWidth, 0, bdWidth, bdWidth, color1);

        tmp = borderPixmap(widget)->border(KThemePixmap::BottomLeft);
        srcMask = tmp->mask();
        bitBlt(pixmap, 0, h - bdWidth, tmp, 0, 0, bdWidth, bdWidth, Qt::CopyROP, false);
        if(srcMask)
            bitBlt(&mask, 0, h - bdWidth, srcMask, 0, 0, bdWidth, bdWidth, Qt::CopyROP, false);
        else
            mPainter.fillRect(0, h - bdWidth, bdWidth, bdWidth, color1);

        tmp = borderPixmap(widget)->border(KThemePixmap::BottomRight);
        srcMask = tmp->mask();
        bitBlt(pixmap, w - bdWidth, h - bdWidth, tmp, 0, 0, bdWidth, bdWidth, Qt::CopyROP, false);
        if(srcMask)
            bitBlt(&mask, w - bdWidth, h - bdWidth, srcMask, 0, 0, bdWidth, bdWidth, Qt::CopyROP, false);
        else
        ...
        if(w - bdWidth * 2 > 0)
        {
            ...
            if(srcMask)
                bitBlt(&mask, bdWidth, 0, srcMask, 0, 0, w - bdWidth * 2, bdWidth, Qt::CopyROP, false);
            else
                mPainter.fillRect(bdWidth, 0, w - bdWidth * 2, bdWidth, color1);

            tmp = borderPixmap(widget)->border(KThemePixmap::Bottom);
            srcMask = tmp->mask();
            p.drawTiledPixmap(bdWidth, h - bdWidth, w - bdWidth * 2, bdWidth, *tmp);
            if(srcMask)
                bitBlt(&mask, bdWidth, h - bdWidth, srcMask, 0, 0, w - bdWidth * 2, bdWidth, Qt::CopyROP, false);
            else
                mPainter.fillRect(bdWidth, h - bdWidth, w - bdWidth * 2, bdWidth, color1);
        }
        if(h - bdWidth * 2 > 0)
        {
            ...
            if(srcMask)
                bitBlt(&mask, 0, bdWidth, srcMask, 0, 0, bdWidth, h - bdWidth * 2, Qt::CopyROP, false);
            else
                mPainter.fillRect(0, bdWidth, bdWidth, h - bdWidth * 2, color1);

            tmp = borderPixmap(widget)->border(KThemePixmap::Right);
            srcMask = tmp->mask();
            p.drawTiledPixmap(w - bdWidth, bdWidth, bdWidth, h - bdWidth * 2, *tmp);
            if(srcMask)
                bitBlt(&mask, w - bdWidth, bdWidth, srcMask, 0, 0, bdWidth, h - bdWidth * 2, Qt::CopyROP, false);
            else
...
KThemePixmap *KThemeBase::blend(WidgetType widget) const
{
    ...
    KPixmapEffect::blend(*pixmaps[widget], blends[widget], *grLowColors[widget], g, false);
    return (pixmaps[widget]);
}
    ...
    if(gradients[widget] == GrVertical)
    {
        if(!pixmaps[widget] || pixmaps[widget]->height() != h)
        {
            ...
            else
            {
                ...
                pixmaps[widget] = new KThemePixmap;
                pixmaps[widget]->resize(w, h);
                KPixmapEffect::gradient(*pixmaps[widget], *grHighColors[widget], *grLowColors[widget], KPixmapEffect::VerticalGradient);
            }
        }
    }
    else if(gradients[widget] == GrHorizontal)
    {
        if(!pixmaps[widget] || pixmaps[widget]->width() != w)
        {
            ...
            else
            {
                ...
                pixmaps[widget] = new KThemePixmap;
                pixmaps[widget]->resize(w, h);
                KPixmapEffect::gradient(*pixmaps[widget], *grHighColors[widget], *grLowColors[widget], KPixmapEffect::HorizontalGradient);
            }
        }
    }
    else if(gradients[widget] == GrReverseBevel)
    {
        if(!pixmaps[widget] || pixmaps[widget]->width() != w || pixmaps[widget]->height() != h)
        {
            ...
            else
            {
                ...
                KPixmapEffect::gradient(*pixmaps[widget], lc, hc, KPixmapEffect::DiagonalGradient);
                KPixmapEffect::gradient(s, *grHighColors[widget], *grLowColors[widget], KPixmapEffect::DiagonalGradient);
                bitBlt(pixmaps[widget], offset, offset, &s, 0, 0, w - offset * 2, h - offset * 2, Qt::CopyROP);
            }
        }
    }
    else
    {
        ...
        if(!pixmaps[widget] || pixmaps[widget]->width() != w || pixmaps[widget]->height() != h)
        {
            ...
            else
            {
                ...
                pixmaps[widget] = new KThemePixmap;
                pixmaps[widget]->resize(w, h);
                KPixmapEffect::gradient(*pixmaps[widget], *grHighColors[widget], *grLowColors[widget], g);
            }
        }
    }
    return (pixmaps[widget]);
...
QColorGroup *KThemeBase::makeColorGroup(const QColor &fg, const QColor &bg, Qt::GUIStyle)
{
    if(shading == Motif)
    {
        ...
        return (new QColorGroup(fg, bg, bg.light(highlightVal), bg.dark(lowlightVal), bg.dark(120), fg, QApplication::palette().active().base()));
    }
    else
        return (new QColorGroup(fg, bg, bg.light(150), bg.dark(), bg.dark(120), fg, QApplication::palette().active().base()));
}
...
void KThemeBase::readMiscResourceGroup()
{
    ...
    arrowStyle = (ArrowStyle)readNumEntry(prop, "ArrowType", (int)LargeArrow);
...
void KThemeBase::applyResourceGroup(QSettings *config, int i)
{
    ...
    if(keys.contains("GradientLow"))
        prop["GrLow"] = readColorEntry(config, QString(base + "GradientLow").latin1(), &QApplication::palette().active().background()).name();

    // Gradient high color
    if(keys.contains("GradientHigh"))
        prop["GrHigh"] = readColorEntry(config, QString(base + "GradientHigh").latin1(), &QApplication::palette().active().foreground()).name();
    ...
    if(!tmpStr.isEmpty())
    {
        ...
        prop["PixmapBWidth"] = QString::number(config->readNumEntry(base + "PixmapBWidth", 0));
    }
    ...
    if(i == SliderGroove)
        prop["SmallGroove"] = QString::number(config->readBoolEntry(base + "SmallGroove", false));
    else if(i == ActiveTab || i == InactiveTab)
        prop["BottomLine"] = QString::number(config->readBoolEntry(base + "BottomLine", true));
    else if(i == Splitter)
    ...
    else if(i == PushButton || i == PushButtonDown)
    {
        ...
        if(keys.contains("3DFocusRect"))
            prop["3DFRect"] = QString::number(config->readBoolEntry(base + "3DFocusRect", false));
        else
            prop["3DFRect"] = "5000";
        if(keys.contains("3DFocusOffset"))
            prop["3DFOffset"] = QString::number(config->readBoolEntry(base + "3DFocusOffset", 0));
        else
            prop["3DFOffset"] = "5000";
        if(keys.contains("Round"))
            prop["Round"] = QString::number(config->readBoolEntry(base + "Round", false));
...
void KThemeBase::readResourceGroup(int i, QString *pixnames, QString *brdnames, bool *loadArray)
{
        ...
        for(sIndex = 0; sIndex < WIDGETS; ++sIndex)
        {
            if(tmpStr == widgetEntries[sIndex])
            {
                if(!loadArray[sIndex]) // hasn't been loaded yet
                    readResourceGroup(sIndex, pixnames, brdnames, loadArray);
                break;
            }
        }
        if(loadArray[sIndex])
    ...
    for(tmpVal = DisArrowUp; tmpVal <= DisArrowRight; ++tmpVal)
    {
        if(tmpVal == i)
        {
            ...
            if(tmpStr.isEmpty())
            {
                copyWidgetConfig(ArrowUp + (tmpVal - DisArrowUp), i, pixnames, brdnames);
                return;
            }
        }
    }
    ...
    if(gradients[i] != GrNone || blends[i] != 0.0)
        grLowColors[i] = new QColor(readColorEntry(prop, "GrLow", QApplication::palette().active().background()));
    else
        grLowColors[i] = NULL;

    // Gradient high color
    if(gradients[i] != GrNone)
        grHighColors[i] = new QColor(readColorEntry(prop, "GrHigh", QApplication::palette().active().background()));
    else
        grHighColors[i] = NULL;
    ...
    if(!tmpStr.isEmpty())
    {
        ...
        {
            if(tmpStr == pixnames[existing] && scaleHints[i] == TileScale && scaleHints[existing] == TileScale && blends[existing] == 0.0
               && blends[i] == 0.0)
            {
...
QPalette KThemeBase::overridePalette(const QPalette &pal)
{
    ...
    if(isPixmap(Background) || isColor(Background))
    {
        ...
        {
            background = colorGroup(pal.active(), Background)->background();
        }
        ...
        QColorGroup pre(foreground, button, background.light(highlightVal), background.dark(lowlightVal), background.dark(120), baseText,
                        buttonText /*CHECKME: BrightText*/, base, background);
    ...
    QColorGroup disabledgrp(disfg, background, // TODO:Convert this to the new ctor.
                            background.light(highlightVal), background.dark(lowlightVal), background.dark(120), background.dark(120), base);


    QColorGroup colgrp(foreground, button, background.light(highlightVal), background.dark(lowlightVal), background.dark(120), baseText,
                       buttonText /*CHECKME: BrightText*/, base, background);
...
KThemePixmap::KThemePixmap(bool timer) : KPixmap()
{
    if(timer)
    {
        t = new QTime;
...
KThemePixmap::KThemePixmap(const KThemePixmap &p) : KPixmap(p)
{
    if(p.t)
    {
        t = new QTime;
...
KThemePixmap::KThemePixmap(const KThemePixmap &p, const QPixmap &p2) : KPixmap(p2)
{
    if(p.t)
    {
        t = new QTime;
...
KThemeCache::KThemeCache(int maxSize, QObject *parent, const char *name) : QObject(parent, name)
{
...
KThemePixmap *KThemeCache::pixmap(int w, int h, int widgetID, bool border, bool mask)
{
...
bool KThemeCache::insert(KThemePixmap *pixmap, ScaleHint scale, int widgetID, bool border, bool mask)
{
    kthemeKey key;
    key.cacheKey = 0; // shut up, gcc
    key.data.id = widgetID;
    key.data.width = (scale == FullScale || scale == HorizontalScale) ? pixmap->width() : 0;
    key.data.height = (scale == FullScale || scale == VerticalScale) ? pixmap->height() : 0;
    key.data.border = border;
    key.data.mask = mask;
    ...
    return (cache.insert((unsigned long)key.cacheKey, pixmap, pixmap->width() * pixmap->height() * pixmap->depth() / 8));
}
