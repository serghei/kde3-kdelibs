...
void kDrawWindowsArrow(QPainter *p, const QStyle *style, QStyle::PrimitiveElement pe, bool down, int x, int y, int w, int h, const QColorGroup &cg,
                       bool enabled)
{
    QPointArray a;
    switch(pe)
    ...
    if(down)
    {
        p->translate(style->pixelMetric(QStyle::PM_ButtonShiftHorizontal), style->pixelMetric(QStyle::PM_ButtonShiftVertical));
    }
...
QSize KThemeStyle::sizeFromContents(ContentsType contents, const QWidget *widget, const QSize &contentSize, const QStyleOption &opt) const
{
    switch(contents)
    {
        // PUSHBUTTON SIZE
...
int KThemeStyle::pixelMetric(PixelMetric metric, const QWidget *widget) const
{
    switch(metric)
    {
        ...
        case PM_ButtonMargin:
            return decoWidth(PushButton) > decoWidth(PushButtonDown) ? 3 + decoWidth(PushButton) : 3 + decoWidth(PushButtonDown);
...
KThemeStyle::KThemeStyle(const QString &configDir, const QString &configFile)
    : KThemeBase(configDir, configFile)
    , paletteSaved(false)
    , polishLock(false)
    , menuCache(0)
    , vsliderCache(0)
    , brushHandle(0)
    , brushHandleSet(false)
    , kickerMode(false)
{
...
void KThemeStyle::polish(QPalette &p)
{
    ...
    {
        QBrush bgBrush(p.color(QPalette::Normal, QColorGroup::Background), *uncached(Background));
...
bool KThemeStyle::eventFilter(QObject *object, QEvent *event)
{
    if(object->inherits("KActiveLabel"))
    {
        if(event->type() == QEvent::Move || event->type() == QEvent::Resize || event->type() == QEvent::Show)
        {
            ...
            QPainter p;
            p.begin(&pix);
            p.drawTiledPixmap(0, 0, uncached(Background)->width(), uncached(Background)->height(), *uncached(Background), pos.x(), pos.y());
            p.end();
            QPalette pal(w->palette());
            QBrush brush(pal.color(QPalette::Normal, QColorGroup::Background), pix);
...
void KThemeStyle::polish(QWidget *w)
{
    ...
    if(w->backgroundPixmap() && !w->isTopLevel()
       && (!kickerMode || (!w->inherits("TaskBar") && !w->inherits("TaskBarContainer") && !w->inherits("TaskbarApplet")
    ...
    if(w->inherits("QTipLabel"))
    {
        ...
        QColorGroup clrGroup(Qt::black, QColor(255, 255, 220), QColor(96, 96, 96), Qt::black, Qt::black, Qt::black, QColor(255, 255, 220));
    ...
    else if(::qt_cast< QRadioButton * >(w))
    {
        if(isColor(ExIndicatorOff) || isColor(ExIndicatorOn))
        {
            ...
            if(isColor(ExIndicatorOff))
            {
                ...
                newPal.setDisabled(*colorGroup(newPal.active(), ExIndicatorOff));
            }
...
void KThemeStyle::drawBaseButton(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool sunken, bool rounded, WidgetType type) const
{
    ...
    {
        int i;
        bitBlt(p->device(), x, y, scalePixmap(w, h, type), 0, 0, w, h, Qt::CopyROP, true);
        p->setPen(g.text());
    ...
    else if(!borderPixmap(type) && shade() == KDE)
    {
        ...
        if(isPixmap(type))
            p->drawTiledPixmap(x + 4, y + 4, w - 6, h - 6, *scalePixmap(w - 6, h - 6, type));
        else
            p->fillRect(x + 4, y + 4, w - 6, h - offset * 6, g.brush(QColorGroup::Button));
    }
    else
    {
        if((w - offset * 2) > 0 && (h - offset * 2) > 0)
        {
            if(isPixmap(type))
                if(rounded)
                    ...
                    p->drawTiledPixmap(x + offset, y + offset, w - offset * 2, h - offset * 2, *scalePixmap(w - offset * 2, h - offset * 2, type));
            else if(1) //##### TODO - Get this optimization working... !borderPixmap( type ) || (( w - decoWidth(type) * 2 ) > 0 && ( h -
                       // decoWidth(type) * 2 ) > 0) )
            // Sometimes border covers the whole thing - in that case, avoid drawing the base.
            {
                p->fillRect(x + offset, y + offset, w - offset * 2, h - offset * 2, g.brush(QColorGroup::Button));
            }
        }
        if(borderPixmap(type))
        {
            bitBlt(p->device(), x, y, scaleBorder(w, h, type), 0, 0, w, h, Qt::CopyROP, false);
        }
        else
            drawShade(p, x, y, w, h, g, sunken, rounded, highlightWidth(type), borderWidth(type), shade());
    }
    p->setPen(oldPen);
}

void KThemeStyle::drawPrimitive(PrimitiveElement pe, QPainter *p, const QRect &r, const QColorGroup &g_base, SFlags flags,
                                const QStyleOption &opt) const
{
    ...
    switch(pe)
    {
        ...
        case PE_ArrowLeft:
        {
            ...
            if(isPixmap(widget))
            {
                bitBlt(p->device(), x + (w - uncached(widget)->width()) / 2, y + (h - uncached(widget)->height()) / 2, uncached(widget));
        ...
        {
            drawBaseButton(p, x, y, w, h, g, (sunken || on || down), roundButton(), (sunken || on || down) ? PushButtonDown : PushButton);
            handled = true;
            break;
        }
        case PE_PanelDockWindow:
        {
            drawBaseButton(p, x, y, w, h, *colorGroup(g, ToolBar), false, false, ToolBar);
            handled = true;
            break;
        }
        case PE_CheckMark:
        {
            if(isPixmap(CheckMark))
            {
                if(flags & Style_Enabled || flags & Style_On)
                    bitBlt(p->device(), x + (w - uncached(CheckMark)->width()) / 2, y + (h - uncached(CheckMark)->height()) / 2, uncached(CheckMark));
                handled = true;
            }
        ...
        case PE_ExclusiveIndicator:
        {
            ...
            {
                p->drawPixmap(x, y, *uncached((flags & Style_On || flags & Style_Down) ? ExIndicatorOn : ExIndicatorOff));
                handled = true;
            }
        ...
        case PE_IndicatorMask:
        {
            ...
            {
                const QBitmap *mask = uncached((flags & Style_On) ? IndicatorOn : IndicatorOff)->mask();
                if(mask)
                {
                    p->setPen(Qt::color1);
                    p->drawPixmap(x, y, *mask);
        ...
        case PE_Indicator:
        {
            if(isPixmap((flags & Style_On || flags & Style_Down) ? IndicatorOn : IndicatorOff))
            {
                p->drawPixmap(x, y, *uncached((flags & Style_On || flags & Style_Down) ? IndicatorOn : IndicatorOff));
                handled = true;
            }
            break;
        }
        case PE_Splitter:
        {
            drawBaseButton(p, x, y, w, h, *colorGroup(g, Splitter), false, false, Splitter);
            handled = true;
            break;
        }
        case PE_FocusRect:
        ...
        case PE_ScrollBarSubPage:
        {
            ...
            if(!isPixmap(widget))
            {
                ...
                drawShade(p, r.x(), r.y(), r.width(), r.height(), *colorGroup(g, widget), true, false, highlightWidth(widget), borderWidth(widget),
                          shade());
            }
            else
            {
                ...
                p->drawTiledPixmap(r.x(), r.y(), r.width(), r.height(), *scalePixmap(r.width(), r.height(), widget));
                drawShade(p, r.x(), r.y(), r.width(), r.height(), *colorGroup(g, widget), true, false, highlightWidth(widget), borderWidth(widget),
                          shade());
            }
        ...
        case PE_ScrollBarAddLine:
        {
            ...
            drawBaseButton(p, r.x(), r.y(), r.width(), r.height(), *colorGroup(g, down ? ScrollButtonDown : ScrollButton), down, false,
                           down ? ScrollButtonDown : ScrollButton);

            drawPrimitive((horizontal) ? PE_ArrowRight : PE_ArrowDown, p, QRect(r.x() + 3, r.y() + 3, r.width() - 6, r.height() - 6),
                          *colorGroup(g, down ? ScrollButtonDown : ScrollButton), flags);
        ...
        case PE_ScrollBarSubLine:
        {
            ...
            drawPrimitive((horizontal) ? PE_ArrowLeft : PE_ArrowUp, p, QRect(r.x() + 3, r.y() + 3, r.width() - 6, r.height() - 6),
                          *colorGroup(g, down ? ScrollButtonDown : ScrollButton), flags);
            handled = true;
            break;
        }
        case PE_ScrollBarSlider:
        {
            ...
            WidgetType widget = horizontal ? active ? HScrollBarSliderDown : HScrollBarSlider : active ? VScrollBarSliderDown : VScrollBarSlider;
            drawBaseButton(p, r.x() + offsetH, r.y() + offsetV, r.width() - 2 * offsetH, r.height() - 2 * offsetV, *colorGroup(g, widget), active,
                           false, widget);

            int spaceW = horizontal ? r.width() - decoWidth(widget) - 4 : r.width();
            int spaceH = horizontal ? r.height() : r.height() - decoWidth(widget) - 4;

            widget = active ? horizontal ? HScrollDecoDown : VScrollDecoDown : horizontal ? HScrollDeco : VScrollDeco;
            if(isPixmap(widget))
            {
                if(spaceW >= uncached(widget)->width() && spaceH >= uncached(widget)->height())
                {
                    bitBlt(p->device(), r.x() + (r.width() - uncached(widget)->width()) / 2, r.y() + (r.height() - uncached(widget)->height()) / 2,
                           uncached(widget));
                }
            }
            handled = true;
    ...
    if(!handled)
        KThemeBase::drawPrimitive(pe, p, r, g, flags, opt);
}
...
void KThemeStyle::drawControl(ControlElement element, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags how,
                              const QStyleOption &opt) const
{
    bool handled = false;
    int x, y, w, h;
    ...
    switch(element)
    {
        case CE_PushButton:
        {
            ...
            drawBaseButton(p, diw, diw, w - 2 * diw, h - 2 * diw, *colorGroup(btn->colorGroup(), sunken ? PushButtonDown : PushButton), sunken,
                           roundButton(), sunken ? PushButtonDown : PushButton);
        ...
        case CE_PushButtonLabel:
        {
            ...
            if(button->isMenuButton())
            {
                ...
                drawPrimitive(PE_ArrowDown, p, QRect(x + w - dx - 2, y + 2, dx, h - 4), cg, how, opt);
                w -= dx;
            }
            ...
            if(button->iconSet() && !button->iconSet()->isNull())
            {
                ...
                if(button->text().isEmpty() && !button->pixmap())
                    p->drawPixmap(x + (w - pixmap.width()) / 2, y + (h - pixmap.height()) / 2, pixmap);
                else
            ...
            if(active || button->isDefault() && button->isEnabled())
            {
                ...
                // Text shadow
                for(i = 0; i < 2; i++)
                    drawItem(p, QRect(x + i + 1, y + 1, w, h), AlignCenter | ShowPrefix, button->colorGroup(), button->isEnabled(), NULL,
                             button->text(), -1, active ? &button->colorGroup().dark() : &button->colorGroup().mid());

                // Normal Text
                for(i = 0; i < 2; i++)
                    drawItem(p, QRect(x + i, y, w, h), AlignCenter | ShowPrefix, button->colorGroup(), true, i == 0 ? button->pixmap() : NULL,
            ...
            else
            {
                if(button->isEnabled())
                {
                    drawItem(p, QRect(x, y, w, h), AlignCenter | ShowPrefix, button->colorGroup(), true, button->pixmap(), button->text(), -1,
                             active ? &button->colorGroup().light() : &button->colorGroup().buttonText());
                }
                else
                {
                    // TODO: Handle reversed
                    drawItem(p, QRect(x + 1, y + 1, w, h), AlignCenter | ShowPrefix, button->colorGroup(), true, button->pixmap(), button->text(), -1,
            ...
            if(how & Style_HasFocus)
                drawPrimitive(PE_FocusRect, p, QStyle::visualRect(subRect(SR_PushButtonFocusRect, widget), widget), cg, how);
            handled = true;
            break;
        }
        ...
        case CE_MenuBarEmptyArea:
        {
            ...
            drawPrimitive(PE_PanelMenuBar, p, QRect(0, 0, r.width() + r.x() * 2, r.height() + r.y() * 2), cg, Style_Default);
            handled = true;
            break;
        }
        ...
        case CE_TabBarTab:
        {
            ...
            if(tbs == QTabBar::RoundedAbove || tbs == QTabBar::TriangularAbove)
            {
                if(!selected)
                {
                    p->fillRect(x, y, x2 - x + 1, 2, tb->palette().active().brush(QColorGroup::Background));
                    y += 2;
                }
                p->setPen(cg->text());
                i = 0;
                ...
                if(isPixmap(widget))
                    p->drawTiledPixmap(x, y, x2 - x + 1, y2 - y + 1, *scalePixmap(x2 - x + 1, y2 - y + 1, widget));
                else
                    p->fillRect(x, y, x2 - x + 1, y2 - y + 1, cg->background());
            }
            else if(tb->shape() == QTabBar::RoundedBelow || tb->shape() == QTabBar::TriangularBelow)
            {
                if(widget == ActiveTab)
                    widget = RotActiveTab;
                else
                ...
                if(!selected)
                {
                    p->fillRect(x, y2 - 2, x2 - x + 1, 2, tb->palette().active().brush(QColorGroup::Background));
                    y2 -= 2;
                }
                p->setPen(cg->text());
                i = 0;
        ...
        case CE_MenuBarItem:
        {
            ...
            drawItem(p, QRect(x, y, w, h), AlignCenter | AlignVCenter | ShowPrefix | DontClip | SingleLine, *g, mi->isEnabled(), mi->pixmap(),
                     mi->text(), -1, &btext);
            handled = true;
            break;
        }
        case CE_PopupMenuItem:
        {
            ...
            {
                drawShade(p, x, y, w, h, *colorGroup(cg_ours, MenuItem), false, false, highlightWidth(MenuItem), borderWidth(MenuItem), shade());
                int dw = decoWidth(MenuItem);
                if(!isPixmap(MenuItem))
                {
                    p->fillRect(x + dw, y + dw, w - dw * 2, h - dw * 2, cg_ours.brush(QColorGroup::Background));
                ...
                else
                {
                    ...
                    p->drawTiledPixmap(x + dw, y + dw, w - dw * 2, h - dw * 2, *scalePixmap(w, p->window().height(), MenuItem), x, y);
                }
            }
            ...
            else if(checkable && mi->isChecked())
            {
                ...
                drawPrimitive(PE_CheckMark, p, QRect(cx + itemFrame, y + itemFrame, checkcol - itemFrame * 2, h - itemFrame * 2), cg_ours, cflags);
            }
            ...
            else
            {
                ...
                if(!s.isNull())
                {
                    ...
                    if(t >= 0)
                    {
                        int tabx = reverse ? x + rightBorder + itemHMargin + itemFrame : x + w - tab - rightBorder - itemHMargin - itemFrame;
            ...
            if(mi->popup())
            {
                ...
                if(active)
                {
                    ...
                    QColorGroup g2(discol, cg_ours.highlight(), white, white, enabled ? white : discol, discol, white);

                    drawPrimitive(arrow, p, vr, g2, Style_Enabled | Style_Down);
                }
                else
                    drawPrimitive(arrow, p, vr, cg_ours, enabled ? Style_Enabled : Style_Default);
            }
            handled = true;
            break;
        }
    ...
    if(!handled)
        KThemeBase::drawControl(element, p, widget, r, cg, how, opt);
}


void KThemeStyle::drawControlMask(ControlElement element, QPainter *p, const QWidget *widget, const QRect &r, const QStyleOption &opt) const
{
    bool handled = false;
    int x, y, w, h;
    r.rect(&x, &y, &w, &h);
    ...
    if(!handled)
        KThemeBase::drawControlMask(element, p, widget, r, opt);
}


void KThemeStyle::drawKStylePrimitive(KStylePrimitive kpe, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags flags,
                                      const QStyleOption &opt) const
{
    bool handled = false;
    int x, y, w, h;
    ...
    switch(kpe)
    {
        case KPE_SliderGroove:
        {
            if(!roundSlider())
            {
                ...
                if(horizontal)
                {
                    drawBaseButton(p, x, y, w, h, *colorGroup(cg, SliderGroove), true, false, SliderGroove);
                }
                else
                {
                    drawBaseButton(p, x, y, w, h, *colorGroup(cg, RotSliderGroove), true, false, RotSliderGroove);
                }
            }
            else
            {
                ...
                p->fillRect(x + 2, y + 2, w - 4, h - 4, slider->isEnabled() ? cg.dark() : cg.mid());
                p->setPen(cg.shadow());
        ...
        case KPE_SliderHandle:
        {
            if(isPixmap(Slider))
            {
                ...
                if(horizontal)
                {
                    bitBlt(p->device(), x, y + (h - uncached(Slider)->height()) / 2, uncached(Slider));
                }
                else
                {
                    if(!vsliderCache)
                    ...
                    bitBlt(p->device(), x + (w - vsliderCache->width()) / 2, y, vsliderCache);
                }
            }
            else
            {
                ...
                p->fillRect(QRect(x + 3, y + 3, w - 6, h - 6), cg.button());
        ...
        {
            if(w > h)
                drawBaseButton(p, x, y, w, h, *colorGroup(cg, HBarHandle), false, false, HBarHandle);
            else
                drawBaseButton(p, x, y, w, h, *colorGroup(cg, VBarHandle), false, false, VBarHandle);
    ...
    if(!handled)
    {
        KThemeBase::drawKStylePrimitive(kpe, p, widget, r, cg, flags, opt);
    }
}


void KThemeStyle::drawComplexControl(ComplexControl control, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &g, SFlags how,
                                     SCFlags controls, SCFlags active, const QStyleOption &opt) const
{
    ...
    switch(control)
    {
        case CC_ToolButton:
        {
            ...
            if(controls & SC_ToolButton)
            {
                ...
                drawBaseButton(p, button.x(), button.y(), button.width(), button.height(), *colorGroup(g, widget), down || on, false, widget);
        ...
        case CC_ComboBox:
        {
            if(controls & SC_ComboBoxFrame)
            {
                ...
                drawBaseButton(p, x, y, w, h, *colorGroup(g, widget), sunken, roundComboBox(), widget);
            ...
            if(controls & SC_ComboBoxArrow)
            {
                ...
                if(!sunken && isPixmap(ComboDeco))
                    bitBlt(p->device(), x + (w - uncached(ComboDeco)->width() - decoWidth(ComboBox) / 2), y + (h - uncached(ComboDeco)->height()) / 2,
                           uncached(ComboDeco));
                else if(sunken && isPixmap(ComboDecoDown))
                    bitBlt(p->device(), x + (w - uncached(ComboDecoDown)->width() - decoWidth(ComboBoxDown)) / 2,
                           y + (h - uncached(ComboDecoDown)->height()) / 2, uncached(ComboDecoDown));
                else
                {
        ...
        case CC_ScrollBar:
        {
            ...
            SFlags sflags = ((horizontal ? Style_Horizontal : Style_Default) | (maxedOut ? Style_Default : Style_Enabled));
            ...
            if(groove.isValid())
            {
                ...
                drawPrimitive(PE_ScrollBarSubPage, &p2, groove, g, sflags | ((active == SC_ScrollBarSubPage) ? Style_Down : Style_Default));
            }
            ...
            if(subline.isValid())
            {
                drawPrimitive(PE_ScrollBarSubLine, &p2, subline, g, sflags | (active == SC_ScrollBarSubLine ? Style_Down : Style_Default));
            }

            if(addline.isValid())
                drawPrimitive(PE_ScrollBarAddLine, &p2, addline, g, sflags | ((active == SC_ScrollBarAddLine) ? Style_Down : Style_Default));

            if(slider.isValid())
            { //(controls & SC_ScrollBarSlider) &&
                drawPrimitive(PE_ScrollBarSlider, &p2, slider, g, sflags | ((active == SC_ScrollBarSlider) ? Style_Down : Style_Default));
                // Draw focus rect
                if(sb->hasFocus())
                {
    ...
    if(!handled)
    {
        KThemeBase::drawComplexControl(control, p, widget, r, g, how, controls, active, opt);
    }
}


void KThemeStyle::drawBaseMask(QPainter *p, int x, int y, int w, int h, bool round) const
{
    // round edge fills
    static const QCOORD btm_left_fill[] = {0, 0, 1, 0, 2, 0, 3, 0, 4, 0, 0, 1, 1, 1, 2, 1, 3, 1, 4,
                                           1, 1, 2, 2, 2, 3, 2, 4, 2, 2, 3, 3, 3, 4, 3, 3, 4, 4, 4};

    static const QCOORD btm_right_fill[] = {0, 0, 1, 0, 2, 0, 3, 0, 4, 0, 0, 1, 1, 1, 2, 1, 3, 1, 4,
                                            1, 0, 2, 1, 2, 2, 2, 3, 2, 0, 3, 1, 3, 2, 3, 0, 4, 1, 4};

    static const QCOORD top_left_fill[] = {3, 0, 4, 0, 2, 1, 3, 1, 4, 1, 1, 2, 2, 2, 3, 2, 4, 2, 0,
 ...
 * have to test.
 */
void KThemeStyle::drawShade(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool sunken, bool rounded, int hWidth, int bWidth,
                            ShadeStyle style) const
{
    int i, sc, bc, x2, y2;
    QPen highPen, lowPen;
    ...
    if(rounded && w > 19 && h > 19)
    {
        ...
        {
            bPntArray.putPoints(bc, 24, x + 4, y + 1, x + 5, y + 1, x + 3, y + 2, x + 2, y + 3, x + 1, y + 4, x + 1, y + 5, x + 1, y2 - 5, x + 1,
                                y2 - 4, x + 2, y2 - 3, x2 - 5, y + 1, x2 - 4, y + 1, x2 - 3, y + 2, x2 - 5, y2 - 1, x2 - 4, y2 - 1, x2 - 3, y2 - 2,
                                x2 - 2, y2 - 3, x2 - 1, y2 - 5, x2 - 1, y2 - 4, x + 3, y2 - 2, x + 4, y2 - 1, x + 5, y2 - 1, x2 - 2, y + 3, x2 - 1,
                                y + 4, x2 - 1, y + 5);
            bc += 24;
            // ellispe edges don't match exactly, so fill in blanks
            if(i < bWidth - 1 || hWidth != 0)
            {
                bPntArray.putPoints(bc, 20, x + 6, y + 1, x + 4, y + 2, x + 3, y + 3, x + 2, y + 4, x + 1, y + 6, x2 - 6, y + 1, x2 - 4, y + 2,
                                    x2 - 3, y + 3, x + 2, y2 - 4, x + 1, y2 - 6, x2 - 6, y2 - 1, x2 - 4, y2 - 2, x2 - 3, y2 - 3, x2 - 2, y2 - 4,
                                    x2 - 1, y2 - 6, x + 6, y2 - 1, x + 4, y2 - 2, x + 3, y2 - 3, x2 - 1, y + 6, x2 - 2, y + 4);
                bc += 20;
            }
            bLineArray.putPoints(i * 8, 8, x + 6, y, x2 - 6, y, x, y + 6, x, y2 - 6, x + 6, y2, x2 - 6, y2, x2, y + 6, x2, y2 - 6);
            ++x, ++y;
            --x2, --y2;
        }
        // highlights
        for(i = 0, sc = 0; i < hWidth; ++i)
        {
            hPntArray.putPoints(sc, 12, x + 4, y + 1, x + 5, y + 1,                                                                  // top left
                                x + 3, y + 2, x + 2, y + 3, x + 1, y + 4, x + 1, y + 5, x + 1, y2 - 5, x + 1, y2 - 4, x + 2, y2 - 3, // half corners
                                x2 - 5, y + 1, x2 - 4, y + 1, x2 - 3, y + 2);
            lPntArray.putPoints(sc, 12, x2 - 5, y2 - 1, x2 - 4, y2 - 1, // btm right
                                x2 - 3, y2 - 2, x2 - 2, y2 - 3, x2 - 1, y2 - 5, x2 - 1, y2 - 4, x + 3, y2 - 2, x + 4, y2 - 1, x + 5,
                                y2 - 1, // half corners
            ...
            if(i < hWidth - 1)
            {
                hPntArray.putPoints(sc, 10, x + 6, y + 1, x + 4, y + 2,                                     // top left
                                    x + 3, y + 3, x + 2, y + 4, x + 1, y + 6, x2 - 6, y + 1, x2 - 4, y + 2, // half corners
                                    x2 - 3, y + 3, x + 2, y2 - 4, x + 1, y2 - 6);
                lPntArray.putPoints(sc, 10, x2 - 6, y2 - 1, x2 - 4, y2 - 2,                                       // btm right
                                    x2 - 3, y2 - 3, x2 - 2, y2 - 4, x2 - 1, y2 - 6, x + 6, y2 - 1, x + 4, y2 - 2, // half corners
...
int KThemeStyle::popupMenuItemHeight(bool /*checkable*/, QMenuItem *mi, const QFontMetrics &fm)
{
    int h2, h = 0;
    ...
    if(mi->isSeparator())
        return (2);
    if(mi->isChecked())
        h = isPixmap(CheckMark) ? uncached(CheckMark)->height() + offset : offset + 16;
    if(mi->pixmap())
    {
    ...
    if(mi->iconSet())
    {
        h2 = mi->iconSet()->pixmap(QIconSet::Small, QIconSet::Normal).height() + offset;
        h = h2 > h ? h2 : h;
    }
    h2 = fm.height() + offset;
    h = h2 > h ? h2 : h;
