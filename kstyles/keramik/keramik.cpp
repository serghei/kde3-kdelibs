...
// XXX
/* reimp. */
void KeramikStyle::renderMenuBlendPixmap(KPixmap &pix, const QColorGroup &cg, const QPopupMenu * /* popup */) const
{
...
QPixmap KeramikStyle::stylePixmap(StylePixmap stylepixmap, const QWidget *widget, const QStyleOption &opt) const
{
    switch(stylepixmap)
    {
        case SP_TitleBarMinButton:
            return Keramik::PixmapLoader::the().pixmap(keramik_title_iconify, Qt::black, Qt::black, false, false);
        // return qpixmap_from_bits( iconify_bits, "title-iconify.png" );
        case SP_TitleBarMaxButton:
            return Keramik::PixmapLoader::the().pixmap(keramik_title_maximize, Qt::black, Qt::black, false, false);
        case SP_TitleBarCloseButton:
            if(widget && widget->inherits("KDockWidgetHeader"))
                return Keramik::PixmapLoader::the().pixmap(keramik_title_close_tiny, Qt::black, Qt::black, false, false);
            else
                return Keramik::PixmapLoader::the().pixmap(keramik_title_close, Qt::black, Qt::black, false, false);
        case SP_TitleBarNormalButton:
            return Keramik::PixmapLoader::the().pixmap(keramik_title_restore, Qt::black, Qt::black, false, false);
        default:
            break;
    }
...
void KeramikStyle::polish(QWidget *widget)
{
    ...
    if(widget->inherits("QPushButton") || widget->inherits("QComboBox") || widget->inherits("QToolButton"))
    {
        ...
        widget->setBackgroundMode(NoBackground);
            ...
            && ((widget->inherits("QListBox") && widget->parentWidget()->inherits("QComboBox")) || widget->inherits("KCompletionBox")))
    {
...
*/
static void renderToolbarEntryBackground(QPainter *paint, const QToolBar *parent, QRect r, const QColorGroup &cg, bool horiz)
{
    ...
    Keramik::GradientPainter::renderGradient(paint, r, cg.button(), horiz, false /*Not a menubar*/, xoff, yoff, toolWidth, toolHeight);
}
...
static void renderToolbarWidgetBackground(QPainter *painter, const QWidget *widget)
{
    ...
    if(painter)
    {
        Keramik::GradientPainter::renderGradient(painter, widget->rect(), widget->colorGroup().button(), horiz_grad, false, x_offset, y_offset,
                                                 toolWidth, toolHeight);
    }
    else
    {
        QPainter p(widget);
        Keramik::GradientPainter::renderGradient(&p, widget->rect(), widget->colorGroup().button(), horiz_grad, false, x_offset, y_offset, toolWidth,
                                                 toolHeight);
    }
}

// This function draws primitive elements as well as their masks.
void KeramikStyle::drawPrimitive(PrimitiveElement pe, QPainter *p, const QRect &r, const QColorGroup &cg, SFlags flags, const QStyleOption &opt) const
{
    ...
    switch(pe)
    {
        ...
        case PE_ButtonDefault:
        {
            ...
            int id;
            if(sunken)
                id = keramik_pushbutton_default_pressed;
            else
                id = keramik_pushbutton_default;
        ...
        case PE_ButtonTool:
        {
            if(titleBarMode)
            {
                ...
                Keramik::ScaledPainter(flags & Style_Down ? keramik_titlebutton_pressed : keramik_titlebutton, Keramik::ScaledPainter::Both)
                    .draw(p, nr, cg.button(), cg.background());
                return;
            }
            ...
            else
            {
                ...
                {
                    Keramik::GradientPainter::renderGradient(p, QRect(r.x(), 0, r.width(), r.height()), Keramik::ColorUtil::lighten(cg.button(), 115),
                                                             flags & Style_Horizontal, false);
                }
                else
                    Keramik::GradientPainter::renderGradient(p, QRect(r.x(), 0, r.width(), r.height()), cg.button(), flags & Style_Horizontal, false);
        ...
        case PE_ButtonCommand:
        {
            ...
            if(toolbarBlendWidget && !flatMode)
            {
                ...
                Keramik::RectTilePainter(name, false).draw(p, r, cg.button(), cg.background(), disabled, Keramik::TilePainter::PaintFullBlend);
            }
            else if(!flatMode)
                Keramik::RectTilePainter(name, false).draw(p, r, cg.button(), cg.background(), disabled, pmode());
            else
            {
                Keramik::ScaledPainter(name + KeramikTileCC, Keramik::ScaledPainter::Vertical)
        ...
        case PE_ButtonBevel:
        {
            ...
            if(w > 4 && h > 4)
            {
                if(sunken)
                    ...
                    Keramik::GradientPainter::renderGradient(p, QRect(x + 2, y + 2, w - 4, h - 4), cg.button(), flags & Style_Horizontal);
            }
            break;
        }
        ...
        case PE_ScrollBarSlider:
        {
            ...
            if(horizontal)
            {
                if(w > (loader.size(keramik_scrollbar_hbar + KeramikSlider1).width() + loader.size(keramik_scrollbar_hbar + KeramikSlider4).width()
                        + loader.size(keramik_scrollbar_hbar + KeramikSlider3).width() + 2))
                    count = 5;
            }
            else if(h > (loader.size(keramik_scrollbar_vbar + KeramikSlider1).height() + loader.size(keramik_scrollbar_vbar + KeramikSlider4).height()
            ...
            if(!active)
                ...
                Keramik::ScrollBarPainter(name, count, horizontal).draw(p, r, Keramik::ColorUtil::lighten(col, 110), cg.background(), false, pmode());
            break;
        }
        ...
        case PE_ScrollBarAddLine:
        {
            ...
            if(flags & Style_Horizontal)
            {
                ...
                p->setPen(cg.buttonText());
                p->drawLine(r.x() + r.width() / 2 - 1, r.y() + r.height() / 2 - 3, r.x() + r.width() / 2 - 1, r.y() + r.height() / 2 + 3);


                drawKeramikArrow(p, cg, QRect(r.x(), r.y(), r.width() / 2, r.height()), PE_ArrowLeft, down, !disabled);

                drawKeramikArrow(p, cg, QRect(r.x() + r.width() / 2, r.y(), r.width() - r.width() / 2, r.height()), PE_ArrowRight, down, !disabled);
            }
            else
            {
                ...
                p->setPen(cg.buttonText());
                p->drawLine(r.x() + r.width() / 2 - 4, r.y() + r.height() / 2, r.x() + r.width() / 2 + 2, r.y() + r.height() / 2);


                drawKeramikArrow(p, cg, QRect(r.x(), r.y(), r.width(), r.height() / 2), PE_ArrowUp, down, !disabled);

                drawKeramikArrow(p, cg, QRect(r.x(), r.y() + r.height() / 2, r.width(), r.height() - r.height() / 2), PE_ArrowDown, down, !disabled);
        ...
        case PE_PanelDockWindow: // Toolbar
        {
            ...
            if(lw)
            {
                ...
                if(horiz)
                {
                    ...
                    // Left line
                    Keramik::GradientPainter::renderGradient(p, QRect(r.x(), r.y(), 1, r.height()), cg.button(), true);
                ...
                else
                {
                    ...
                    // Top line
                    Keramik::GradientPainter::renderGradient(p, QRect(r.x(), r.y(), r.width(), 1), cg.button(), false);
...
void KeramikStyle::drawKStylePrimitive(KStylePrimitive kpe, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags flags,
                                       const QStyleOption &opt) const
{
    ...
    switch(kpe)
    {
        ...
        case KPE_SliderHandle:
        {
            ...
            if(horizontal)
                Keramik::ScaledPainter(keramik_slider)
                    .draw(p, r, disabled ? cg.button() : hl, Qt::black, disabled, Keramik::TilePainter::PaintFullBlend);
            else
                Keramik::ScaledPainter(keramik_vslider)
                    .draw(p, r, disabled ? cg.button() : hl, Qt::black, disabled, Keramik::TilePainter::PaintFullBlend);
            break;
        }
...
void KeramikStyle::drawControl(ControlElement element, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags flags,
                               const QStyleOption &opt) const
{
    bool disabled = (flags & Style_Enabled) == 0;
    int x, y, w, h;
    ...
    switch(element)
    {
        ...
        case CE_PushButtonLabel:
        {
            ...
            if(button->iconSet() && !button->iconSet()->isNull())
            {
                ...
                else
                {
                    // Icon only. Center it.
                    if(!button->pixmap())
                        p->drawPixmap(x + w / 2 - icon.width() / 2, y + h / 2 - icon.height() / 2, icon);
        ...
        case CE_TabBarTab:
        {
            ...
            else
            {
                ...
                if(index == 0)
                    mode = Keramik::InactiveTabPainter::First;
                else if(index == tabBar->count() - 1)
                    mode = Keramik::InactiveTabPainter::Last;
                else
                    mode = Keramik::InactiveTabPainter::Middle;
        ...
        case CE_DockWindowEmptyArea:
        {
            ...
            if(widget && widget->inherits("QToolBar"))
            {
                ...
                if(tb->place() == QDockWindow::OutsideDock)
                {
                    ...
                    pr = QRect(r.x(), titleBarH + tb->frameWidth(), r.width(), tb->height() - titleBarH - 2 * tb->frameWidth() + 2);
                    // 2 at the end = the 2 pixels of border of a "regular"
                    // toolbar we normally paint over.
                }
                Keramik::GradientPainter::renderGradient(p, pr, cg.button(), tb->orientation() == Qt::Horizontal);
            }
            else
                KStyle::drawControl(CE_DockWindowEmptyArea, p, widget, r, cg, flags, opt);
            break;
        }
        ...
        case CE_ProgressBarContents:
        {
            ...
            if(progress > 0 || steps == 0)
            {
                ...
                { // Busy indicator
                    ...
                    Keramik::RowPainter(keramik_progressbar).draw(p, progressRect, cg.highlight(), cg.background());
                    return;
                }
                ...
                if(animateProgressBar)
                {
                    ...
                    if(reverse)
                    {
                        ...
                        if(endDim.width() < progressRect.width())
                        {
                            ...
                            QPixmap endline =
                                loader.scale(keramik_progressbar + 3, endDim.width(), progressRect.height(), cg.highlight(), cg.background());
                            ...
                            // Paint the stripes.
                            QPixmap stripe = loader.scale(keramik_progressbar + 4, 28, progressRect.height(), cg.highlight(), cg.background());
                            // 4 = 3*1 + 1 = (1,1) = cc
                ...
                Keramik::ProgressBarPainter(keramik_progressbar, reverse).draw(p, progressRect, cg.highlight(), cg.background());
            }
            break;
        }
...
void KeramikStyle::drawControlMask(ControlElement element, QPainter *p, const QWidget *widget, const QRect &r, const QStyleOption &opt) const
{
    p->fillRect(r, color1);
    maskMode = true;
...
void KeramikStyle::drawComplexControl(ComplexControl control, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags flags,
    ...
    switch(control)
    {
        ...
        case CC_ComboBox:
        {
            ...
            if(controls == SC_All)
            {
                ...
                if(widget->parent() && (widget->parent()->inherits("QToolBar") || !qstrcmp(widget->parent()->name(), kdeToolbarWidget)))
                {
            ...
            if(controls & SC_ComboBoxArrow)
            {
                ...
                if(!compact)
                {
                    ...
                    ar = visualRect(QRect(ar.x() + loader.size(keramik_ripple).width() + 4, ar.y(), 11, ar.height()), widget);
            ...
            if(controls & SC_ComboBoxEditField)
            {
                if(cb->editable())
                {
                    ...
                    Keramik::RectTilePainter(keramik_frame_shadow, false, false, 2, 2).draw(p2, er, cg.button(), Qt::black, false, pmodeFullBlend());
                }
                else if(cb->hasFocus())
                {
        ...
        case CC_SpinWidget:
        {
            ...
            if(controls & SC_SpinWidgetButtonField)
            {
                ...
                if(active & SC_SpinWidgetUp)
                    Keramik::CenteredPainter(keramik_spinbox_pressed_arrow_up)
                        .draw(p, br.x(), br.y() + 3, br.width(), br.height() / 2, cg.button(), cg.background());
                else
                    Keramik::CenteredPainter(keramik_spinbox_arrow_up)
                        .draw(p, br.x(), br.y() + 3, br.width(), br.height() / 2, cg.button(), cg.background(), !sw->isUpEnabled());
                if(active & SC_SpinWidgetDown)
                    Keramik::CenteredPainter(keramik_spinbox_pressed_arrow_down)
                        .draw(p, br.x(), br.y() + br.height() / 2, br.width(), br.height() / 2 - 8, cg.button(), cg.background());
                else
                    Keramik::CenteredPainter(keramik_spinbox_arrow_down)
                        .draw(p, br.x(), br.y() + br.height() / 2, br.width(), br.height() / 2 - 8, cg.background(), cg.button(),
                              !sw->isDownEnabled());
            }
        ...
        case CC_ToolButton:
        {
            ...
            if(!onToolbar && !onExtender && widget->parentWidget() && !qstrcmp(widget->parentWidget()->name(), "qt_maxcontrols"))
            {
            ...
            if(controls & SC_ToolButton)
            {
                ...
                else if(onExtender)
                {
                    ...
                    Keramik::GradientPainter::renderGradient(p, r, cg.button(), horiz, false, /*Not a menubar*/
                                                             xoff, yoff, tr.width(), tr.height());
                }
            }
...
void KeramikStyle::drawComplexControlMask(ComplexControl control, QPainter *p, const QWidget *widget, const QRect &r, const QStyleOption &opt) const
{
    if(control == CC_ComboBox)
    {
        maskMode = true;
        drawComplexControl(CC_ComboBox, p, widget, r, QApplication::palette().active(), Style_Default, SC_ComboBoxFrame, SC_None, opt);
        maskMode = false;
    }
    else
        p->fillRect(r, color1);
...
int KeramikStyle::pixelMetric(PixelMetric m, const QWidget *widget) const
{
    switch(m)
    {
        ...
        case PM_ScrollBarSliderMin:
            return loader.size(keramik_scrollbar_vbar + KeramikSlider1).height() + loader.size(keramik_scrollbar_vbar + KeramikSlider3).height();
...
QSize KeramikStyle::sizeFromContents(ContentsType contents, const QWidget *widget, const QSize &contentSize, const QStyleOption &opt) const
{
    switch(contents)
    {
        // PUSHBUTTON SIZE
        ...
        case CT_PushButton:
        {
            ...
            if(btn->text().isEmpty() && contentSize.width() < 32)
                return QSize(w, h);
...
QStyle::SubControl KeramikStyle::querySubControl(ComplexControl control, const QWidget *widget, const QPoint &point, const QStyleOption &opt) const
{
    ...
    if(control == CC_ScrollBar && result == SC_ScrollBarAddLine)
    {
        ...
        {
            if(point.x() < addline.center().x())
                result = SC_ScrollBarSubLine;
        }
        else if(point.y() < addline.center().y())
            result = SC_ScrollBarSubLine;
    }
    return result;
}

QRect KeramikStyle::querySubControlMetrics(ComplexControl control, const QWidget *widget, SubControl subcontrol, const QStyleOption &opt) const
{
    switch(control)
    {
        case CC_ComboBox:
        ...
        case CC_ScrollBar:
        {
            ...
            switch(subcontrol)
            {
                case SC_ScrollBarGroove:
                    if(horizontal)
                        return QRect(subline, 0, maxlen, sb->height());
                    else
                        return QRect(0, subline, sb->width(), maxlen);

                case SC_ScrollBarSlider:
                    if(horizontal)
                        return QRect(sliderpos, 0, sliderlen, sb->height());
                    else
                        return QRect(0, sliderpos, sb->width(), sliderlen);

                case SC_ScrollBarSubLine:
                    if(horizontal)
                        return QRect(0, 0, subline, sb->height());
                    else
                        return QRect(0, 0, sb->width(), subline);

                case SC_ScrollBarAddLine:
                    if(horizontal)
                        return QRect(sb->width() - addline, 0, addline, sb->height());
                    else
                        return QRect(0, sb->height() - addline, sb->width(), addline);

                case SC_ScrollBarSubPage:
                    if(horizontal)
                        return QRect(subline, 0, sliderpos - subline, sb->height());
                    else
                        return QRect(0, subline, sb->width(), sliderpos - subline);
...
bool KeramikStyle::eventFilter(QObject *object, QEvent *event)
{
    ...
    if(event->type() == QEvent::Paint && ::qt_cast< QLineEdit * >(object))
    {
        ...
        QPainter p(widget);
        Keramik::RectTilePainter(keramik_frame_shadow, false, false, 2, 2)
            .draw(&p, widget->rect(), widget->palette().color(QPalette::Normal, QColorGroup::Button), Qt::black, false,
    ...
    else if(::qt_cast< QListBox * >(object))
    {
        ...
        switch(event->type())
        {
            ...
            case QEvent::Resize:
            {
                ...
                XRectangle rects[5] = {{0, 0, uint16_t(resize->size().width() - 2), uint16_t(resize->size().height() - 6)},
                ...
                XShapeCombineRectangles(qt_xdisplay(), listbox->handle(), ShapeBounding, 0, 0, rects, 5, ShapeSet, YXSorted);
            }
            break;
#endif
            ...
            case QEvent::Paint:
            {
                ...
                {
                    QPainter p(listbox);
                    Keramik::RectTilePainter(keramik_combobox_list, false, false)
                        .draw(&p, 0, 0, listbox->width(), listbox->height(), listbox->palette().color(QPalette::Normal, QColorGroup::Button),
    ...
    else if(event->type() == QEvent::Paint && object->parent() && ::qt_cast< QToolBar * >(object->parent()) && !::qt_cast< QPopupMenu * >(object))
    {
        ...
        {
            Keramik::GradientPainter::renderGradient(&p, wr, widget->colorGroup().button(), true /*horizontal*/, false /*not a menu*/, 0, widget->y(),
                                                     wr.width(), tr.height());
        }
        else
        {
            Keramik::GradientPainter::renderGradient(&p, wr, widget->colorGroup().button(), false /*vertical*/, false /*not a menu*/, widget->x(), 0,
                                                     tr.width(), wr.height());
        }
