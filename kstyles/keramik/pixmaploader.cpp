    ...
    if(edata->haveAlpha)
    {
        if(blend)
        {
            ...
            for(int pos = 0; pos < size; pos += 3)
            {
                ...
                *write = qRgb(((rr * alpha + 127) >> 8) + ((br * destAlpha + 127) >> 8), ((rg * alpha + 127) >> 8) + ((bg * destAlpha + 127) >> 8),
...
void TilePainter::draw(QPainter *p, int x, int y, int width, int height, const QColor &color, const QColor &bg, bool disabled, PaintMode mode)
{
    ...
    for(unsigned int col = 0; col < columns(); ++col)
        if(columnMode(col) != Fixed)
        {
            ...
            scaleWidth -= PixmapLoader::the().size(absTileName(col, 0)).width();
    ...
    for(unsigned int row = 0; row < rows(); ++row)
        if(rowMode(row) != Fixed)
        {
            ...
            scaleHeight -= PixmapLoader::the().size(absTileName(0, row)).height();
    ...
    for(unsigned int row = 0; row < rows(); ++row)
    {
        ...
        for(unsigned int col = 0; col < columns(); ++col)
        {
            ...
            if(scaledColumns && col == lastScaledColumn)
                w += scaleWidth - scaleWidth / scaledColumns * scaledColumns;
...
RectTilePainter::RectTilePainter(int name, bool scaleH, bool scaleV, unsigned int columns, unsigned int rows)
    : TilePainter(name), m_scaleH(scaleH), m_scaleV(scaleV)
{
    m_columns = columns;
    m_rows = rows;
...
ActiveTabPainter::ActiveTabPainter(bool bottom)
    : RectTilePainter(bottom ? keramik_tab_bottom_active : keramik_tab_top_active, false), m_bottom(bottom)
{
    m_rows = 2;
    if(m_bottom)
    {
...
InactiveTabPainter::InactiveTabPainter(Mode mode, bool bottom)
    : RectTilePainter(bottom ? keramik_tab_bottom_inactive : keramik_tab_top_inactive, false), m_mode(mode), m_bottom(bottom)
{
    m_rows = 2;
    if(m_bottom)
    {
...
ScrollBarPainter::ScrollBarPainter(int type, int count, bool horizontal)
    : TilePainter(name(horizontal)), m_type(type), m_count(count), m_horizontal(horizontal)
{
    for(int c = 0; c < 5; c++)
    {
        if(!m_horizontal || !(c % 2))
