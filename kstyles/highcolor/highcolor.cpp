/*
 * $Id: highcolor.cpp 465272 2005-09-29 09:47:40Z mueller $
 *
...
KPixmap *GradientSet::gradient(GradientType type)
{
    ...
    switch(type)
    {
        case VSmall:
        {
            ...
            KPixmapEffect::gradient(*gradients[VSmall], c.light(110), c.dark(110), KPixmapEffect::VerticalGradient);
            break;
        }
        ...
        case VMed:
        {
            ...
            KPixmapEffect::gradient(*gradients[VMed], c.light(110), c.dark(110), KPixmapEffect::VerticalGradient);
            break;
        }
        ...
        case VLarge:
        {
            ...
            KPixmapEffect::gradient(*gradients[VLarge], c.light(110), c.dark(110), KPixmapEffect::VerticalGradient);
            break;
        }
        ...
        case HMed:
        {
            ...
            KPixmapEffect::gradient(*gradients[HMed], c.light(110), c.dark(110), KPixmapEffect::HorizontalGradient);
            break;
        }
        ...
        case HLarge:
        {
            ...
            KPixmapEffect::gradient(*gradients[HLarge], c.light(110), c.dark(110), KPixmapEffect::HorizontalGradient);
            break;
        }
...
HighColorStyle::HighColorStyle(StyleType styleType) : KStyle(AllowMenuTransparency | FilledFrameWorkaround, ThreeButtonScrollBar)
{
    type = styleType;
...
/* reimp. */
void HighColorStyle::renderMenuBlendPixmap(KPixmap &pix, const QColorGroup &cg, const QPopupMenu * /* popup */) const
{
...
void HighColorStyle::drawPrimitive(PrimitiveElement pe, QPainter *p, const QRect &r, const QColorGroup &cg, SFlags flags,
                                   const QStyleOption &opt) const
{
    ...
    switch(pe)
    {
        ...
        case PE_ButtonCommand:
        {
            ...
            if(sunken)
                kDrawBeButton(p, x, y, w, h, cg, true, &cg.brush(QColorGroup::Mid));
            ...
            else if(flat)
            {
                if(flags & Style_MouseOver)
                    ...
                    renderGradient(p, QRect(x, y, w - 1, h - 1), cg.button(), false);
            ...
            else if(highcolor)
            {
                ...
                renderGradient(p, QRect(x + 4, y + 4, w - 6, h - 6), cg.button(), false);
            }
            else
                kDrawBeButton(p, x, y, w, h, cg, false, &cg.brush(QColorGroup::Button));
            break;
        }
        ...
        case PE_HeaderSection:
        {
            ...
            if(sunken)
                ...
                renderGradient(p, QRect(x + 1, y + 1, w - 3, h - 3), cg.button(), !horizontal);
            p->setPen(oldPen);
            break;
        }
        ...
        case PE_ScrollBarSlider:
        {
            ...
            else
            {
                ...
                if(flags & Style_Horizontal)
                {
                    ...
                    int x = r.x() + (r.width() - 7) / 2;
                    int y = r.y() + (r.height() - (buttons * 5) - (buttons - 1)) / 2;
                    int x2 = x + 7;
                ...
                else
                {
                    ...
                    int x = r.x() + (r.width() - (buttons * 5) - (buttons - 1)) / 2;
                    int y = r.y() + (r.height() - 7) / 2;
                    int y2 = y + 7;
        ...
        case PE_ScrollBarSubPage:
        {
            ...
            if(type != B3)
            {
                ...
                if(flags & Style_Horizontal)
                {
                    p->drawLine(x, y, x2, y);
                    p->drawLine(x, y2, x2, y2);
                    renderGradient(p, QRect(x, y + 1, w, h - 2), cg.mid(), false);
                }
                else
                {
                    p->drawLine(x, y, x, y2);
                    p->drawLine(x2, y, x2, y2);
                    renderGradient(p, QRect(x + 1, y, w - 2, h), cg.mid(), true);
                }
            }
            else
            {
                ...
                if(flags & Style_Horizontal)
                {
                    p->drawLine(x, y, x2, y);
                    p->drawLine(x, y2, x2, y2);
                    p->fillRect(QRect(x, y + 1, w, h - 2), flags & Style_Down ? cg.button() : cg.midlight());
                }
                else
                {
                    p->drawLine(x, y, x, y2);
                    p->drawLine(x2, y, x2, y2);
                    p->fillRect(QRect(x + 1, y, w - 2, h), flags & Style_Down ? cg.button() : cg.midlight());
                }
            }
            break;
        }
        ...
        {
            drawPrimitive(PE_ButtonBevel, p, r, cg, (flags & Style_Enabled) | ((flags & Style_Down) ? Style_Down : Style_Raised));
        ...
        case PE_Indicator:
        {
            ...
            if(enabled)
                p->fillRect(x + 2, y + 2, w - 4, h - 4, down ? cg.button() : cg.base());
            else
        ...
        case PE_ExclusiveIndicator:
        {
            ...
            // Bevel
            kColorBitmaps(p, cg, r.x(), r.y(), &lightBmp, &grayBmp, NULL, &dgrayBmp);
            ...
            if(on)
            {
                QColor color = flags & Style_NoChange ? cg.dark() : cg.text();
        ...
        { // Toolbar
            ...
            if(lw)
            {
                ...
                renderGradient(p, QRect(r.x() + 1, r.y() + 1, r.width() - 2, r.height() - 2), cg.button(),
                               (r.width() < r.height()) && (pe != PE_PanelMenuBar));
            }
            else
                renderGradient(p, r, cg.button(), (r.width() < r.height()) && (pe != PE_PanelMenuBar));
        ...
        {
            renderGradient(p, r, cg.button(), !(flags & Style_Horizontal));
            if(!(flags & Style_Horizontal))
            {
                p->setPen(cg.mid());
...
void HighColorStyle::drawKStylePrimitive(KStylePrimitive kpe, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags flags,
                                         const QStyleOption &opt) const
{
    switch(kpe)
    {
        ...
        case KPE_SliderHandle:
        {
            ...
            p->drawLine(x2 - 2, y + 3, x2 - 2, y2 - 2);
            renderGradient(p, QRect(x + 3, y + 3, w - 6, h - 6), cg.button(), !horizontal);
...
void HighColorStyle::drawControl(ControlElement element, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg, SFlags flags,
                                 const QStyleOption &opt) const
{
    switch(element)
    {
        ...
        case CE_PushButtonLabel:
        {
            ...
            if(button->isMenuButton())
            {
                ...
                drawPrimitive(PE_ArrowDown, p, QRect(x + w - dx - 2, y + 2, dx, h - 4), cg, flags, opt);
                w -= dx;
            }
            ...
            if(active || button->isDefault())
            {
                ...
                // Normal Text
                for(i = 0; i < 2; i++)
                    drawItem(p, QRect(x + i, y, w, h), AlignCenter | ShowPrefix, button->colorGroup(), button->isEnabled(),
                             i == 0 ? button->pixmap() : NULL, button->text(), -1,
                             active ? &button->colorGroup().light() : &button->colorGroup().buttonText());
            }
            else
                drawItem(p, QRect(x, y, w, h), AlignCenter | ShowPrefix, button->colorGroup(), button->isEnabled(), button->pixmap(), button->text(),
                         -1, active ? &button->colorGroup().light() : &button->colorGroup().buttonText());
        ...
        case CE_ToolBoxTab:
        {
            ...
            if(pressed)
                ...
                renderGradient(p, QRect(x + 1, y + 1, r.width() - 2, r.height() - 2), fill, false);
            break;
        }
        ...
        case CE_MenuBarItem:
        {
            ...
            if(active && focused)
                ...
                renderGradient(p, r, cg.button(), false, r.x(), r.y() - 1, pr.width() - 2, pr.height() - 2);
        ...
        case CE_PopupMenuItem:
        {
            ...
            else if(checkable && mi->isChecked())
            {
                ...
                if(!active)
                    qDrawShadePanel(p, cx, y, checkcol, h, cg, true, 1, &cg.brush(QColorGroup::Midlight));

                // Draw the checkmark
                SFlags cflags = Style_Default;
                cflags |= active ? Style_Enabled : Style_On;

                drawPrimitive(PE_CheckMark, p, QRect(cx + itemFrame, y + itemFrame, checkcol - itemFrame * 2, h - itemFrame * 2), cg, cflags);
            }
            ...
            if(mi->popup())
            {
                ...
                QRect vr = visualRect(QRect(x + w - arrowHMargin - 2 * itemFrame - dim, y + h / 2 - dim / 2, dim, dim), r);
...
void HighColorStyle::drawControlMask(ControlElement element, QPainter *p, const QWidget *widget, const QRect &r, const QStyleOption &opt) const
{
    switch(element)
    {
        // PUSHBUTTON MASK
...
void HighColorStyle::drawComplexControl(ComplexControl control, QPainter *p, const QWidget *widget, const QRect &r, const QColorGroup &cg,
                                        SFlags flags, SCFlags controls, SCFlags active, const QStyleOption &opt) const
{
    switch(control)
    {
        // COMBOBOX
        ...
        case CC_ComboBox:
        {
            ...
            if(controls & SC_ComboBoxArrow)
            {
                ...
                renderGradient(p, QRect(x + 2, y + 2, w - 4, h - 4), cg.button(), false);
            ...
            if(controls & SC_ComboBoxEditField)
            {
                ...
                QRect re = QStyle::visualRect(querySubControlMetrics(CC_ComboBox, widget, SC_ComboBoxEditField), widget);
                ...
                {
                    // Draw the contents
                    p->fillRect(re.x(), re.y(), re.width(), re.height(), cg.brush(QColorGroup::Highlight));
            ...
            if(controls & SC_ToolButton)
            {
                ...
                else if(widget->parent())
                {
                    if(widget->parent()->inherits("QToolBar"))
                    {
                        ...
                        renderGradient(p, r, cg.button(), parent->orientation() == Qt::Vertical, r.x(), r.y(), pr.width() - 2, pr.height() - 2);
                    }
                    else if(widget->parent()->inherits("QToolBarExtensionWidget"))
                    {
                        ...
                        {
                            renderGradient(p, r, cg.button(), false, r.x(), r.y(), r.width(), tr.height());
                        }
                        else
                        {
                            renderGradient(p, r, cg.button(), true, r.x(), r.y(), tr.width(), r.height());
                        }
                    }
                }
            }
...
void HighColorStyle::drawComplexControlMask(ComplexControl control, QPainter *p, const QWidget *widget, const QRect &r, const QStyleOption &opt) const
{
    switch(control)
    {
...
void HighColorStyle::drawItem(QPainter *p, const QRect &r, int flags, const QColorGroup &cg, bool enabled, const QPixmap *pixmap, const QString &text,
                              int len, const QColor *penColor) const
{
    ...
    KStyle::drawItem(p, r, flags, cg, enabled, pixmap, text, len, col);
}
...
QRect HighColorStyle::subRect(SubRect r, const QWidget *widget) const
{
    ...
    if(r == SR_PushButtonFocusRect)
    {
        ...
        int dfw1 = pixelMetric(PM_DefaultFrameWidth, widget) * 2, dfw2 = dfw1 * 2;

        return QRect(wrect.x() + dfw1 + dbw1 + 1, wrect.y() + dfw1 + dbw1 + 1, wrect.width() - dfw2 - dbw2 - 1, wrect.height() - dfw2 - dbw2 - 1);
    }
    else
...
QSize HighColorStyle::sizeFromContents(ContentsType contents, const QWidget *widget, const QSize &contentSize, const QStyleOption &opt) const
{
    switch(contents)
    {
        // PUSHBUTTON SIZE
...
QPixmap HighColorStyle::stylePixmap(StylePixmap stylepixmap, const QWidget *widget, const QStyleOption &opt) const
{
    switch(stylepixmap)
    {
...
bool HighColorStyle::eventFilter(QObject *object, QEvent *event)
{
    ...
    else if(object->parent() && !qstrcmp(object->name(), kdeToolbarWidget))
    {
        ...
        if(event->type() == QEvent::Paint)
        {
            ...
            if(tb)
                horiz_grad = tb->orientation() == Qt::Vertical;

            QPainter p(widget);
            renderGradient(&p, r, parent->colorGroup().button(), horiz_grad, x_offset, y_offset, pr.width(), pr.height());

            return false; // Now draw the contents
        }
    }
    else if(object->parent() && (toolbar = dynamic_cast< QToolBar * >(object->parent())))
    {
        ...
        if(event->type() == QEvent::Paint)
        {
            ...
            QPainter p(widget);
            renderGradient(&p, wr, toolbar->colorGroup().button(), toolbar->orientation() == Qt::Vertical, wr.x(), wr.y(), tr.width() - 2,
                           tr.height() - 2);
...
void HighColorStyle::renderGradient(QPainter *p, const QRect &r, QColor clr, bool horizontal, int px, int py, int pwidth, int pheight) const
{
    ...
    if(horizontal)
    {
        ...
        else
        {
            ...
            if(hLarge->width() > px)
            {
                ...
                // Draw the gradient
                p->drawTiledPixmap(r.x(), r.y(), pixmapWidth, r.height(), *hLarge, px, 0);
                // Draw the remaining fill
                p->fillRect(r.x() + pixmapWidth, r.y(), r.width() - pixmapWidth, r.height(), clr.dark(110));
            }
            else
    ...
    else
    {
        ...
        else
        {
            ...
            if(vLarge->height() > py)
            {
                ...
                // Draw the gradient
                p->drawTiledPixmap(r.x(), r.y(), r.width(), pixmapHeight, *vLarge, 0, py);
                // Draw the remaining fill
                p->fillRect(r.x(), r.y() + pixmapHeight, r.width(), r.height() - pixmapHeight, clr.dark(110));
            }
            else
                p->fillRect(r, clr.dark(110));
        }
