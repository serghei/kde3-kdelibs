...
static void scrollBarControlsMetrics(const QScrollBar *sb, int sliderStart, int /* sliderMin */, int sliderMax, int sliderLength, int buttonDim,
                                     QRect &rSub, QRect &rAdd, QRect &rSubPage, QRect &rAddPage, QRect &rSlider)
{
...
static void drawFunkyRect(QPainter *p, int x, int y, int w, int h, bool small)
{
    ...
    if(small)
    {
        ...
        QCOORD pointList[] = {1, 1, w - 2, 1, w - 2, h - 2, 1, h - 2};
    ...
    else
    {
        ...
        QCOORD pointList[] = {1, 2, 2, 1, w - 3, 1, w - 2, 2, w - 2, h - 3, w - 3, h - 2, 2, h - 2, 1, h - 3};
...
bool WebStyle::eventFilter(QObject *o, QEvent *e)
{
...
void WebStyle::drawButton(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool sunken, const QBrush *fill)
{
    p->save();
...
QRect WebStyle::buttonRect(int x, int y, int w, int h)
{
    return QRect(x + 2, y + 2, w - 4, h - 4);
}

void WebStyle::drawBevelButton(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool sunken, const QBrush *fill)
{
    drawButton(p, x, y, w, h, g, sunken, fill);
}

void WebStyle::drawPushButton(QPushButton *b, QPainter *p)
{
    ...
    p->fillRect(4, 4, b->width() - 8, b->height() - 8, b->colorGroup().brush(QColorGroup::Button));
...
void WebStyle::drawPushButtonLabel(QPushButton *b, QPainter *p)
{
...
void WebStyle::drawScrollBarControls(QPainter *p, const QScrollBar *sb, int sliderStart, uint controls, uint activeControl)
{
    p->save();
    ...
    scrollBarControlsMetrics(sb, sliderStart, sliderMin, sliderMax, sliderLength, buttonDim, rSub, rAdd, rSubPage, rAddPage, rSlider);
    ...
    if(controls & AddLine && rAdd.isValid())
    {
        ...
        Qt::ArrowType t = sb->orientation() == Horizontal ? Qt::RightArrow : Qt::DownArrow;

        // Is it me or is KStyle::drawArrow broken ?

        drawArrow(p, t,
                  true, // FIXME - down ?
                  rAdd.x(), rAdd.y(), rAdd.width(), rAdd.height(), g,
                  true // FIXME - enabled ?
                  );
    }
    ...
    if(controls & SubLine && rSub.isValid())
    {
        ...
        Qt::ArrowType t = sb->orientation() == Horizontal ? Qt::LeftArrow : Qt::UpArrow;

        drawArrow(p, t,
                  true, // FIXME - down ?
                  rSub.x(), rSub.y(), rSub.width(), rSub.height(), g,
                  true // FIXME - enabled ?
                  );
    }
    ...
    if(controls & Slider && rSlider.isValid())
    {
        ...
        {
            int ellipseSize = Horizontal == sb->orientation() ? rSlider.height() - 4 : rSlider.width() - 4;

            QPoint center(rSlider.center());

            if(Horizontal == sb->orientation())
            {
                p->drawEllipse(center.x() - ellipseSize / 2, rSlider.y() + 2, ellipseSize, ellipseSize);
            }
            else
            {
                p->drawEllipse(rSlider.x() + 2, center.y() - ellipseSize / 2, ellipseSize, ellipseSize);
            }
        }
    }
...
QStyle::ScrollControl WebStyle::scrollBarPointOver(const QScrollBar *sb, int sliderStart, const QPoint &point)
{
...
void WebStyle::scrollBarMetrics(const QScrollBar *sb, int &sliderMin, int &sliderMax, int &sliderLength, int &buttonDim)
{
    int maxlen;
    ...
    sliderLength = (sb->pageStep() * maxlen) / (sb->maxValue() - sb->minValue() + sb->pageStep());
...
void WebStyle::drawIndicator(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, int state, bool down, bool enabled)
{
    p->save();
...
void WebStyle::drawExclusiveIndicator(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool on, bool down, bool enabled)
{
    p->save();
...
void WebStyle::drawIndicatorMask(QPainter *p, int x, int y, int w, int h, int /* state */
                                 )
{
    p->fillRect(x, y, w, h, Qt::color1);
}

void WebStyle::drawExclusiveIndicatorMask(QPainter *p, int x, int y, int w, int h, bool /* on */
                                          )
{
    if(0 == w % 2)
        --w;
...
void WebStyle::drawComboButton(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool sunken, bool editable, bool enabled,
                               const QBrush *fill)
{
    p->save();
...
QRect WebStyle::comboButtonRect(int x, int y, int w, int h)
{
    return QRect(x + 2, y + 2, w - 20, h - 4);
}

QRect WebStyle::comboButtonFocusRect(int x, int y, int w, int h)
{
    return QRect(x + 2, y + 2, w - 20, h - 4);
}
...
void WebStyle::drawSliderGroove(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, QCOORD /* c */, Orientation o)
{
    p->save();
...
void WebStyle::drawArrow(QPainter *p, Qt::ArrowType type, bool down, int x, int y, int w, int h, const QColorGroup &g, bool enabled,
                         const QBrush *fill)
{
    KStyle::drawArrow(p, type, down, x, y, w, h, g, enabled, fill);
}

void WebStyle::drawSlider(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, Orientation o, bool /* tickAbove */, bool /* tickBelow */
                          )
{
    p->save();
...
void WebStyle::drawKToolBar(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, KToolBarPos /* pos */, QBrush * /* fill */
                            )
{
    p->save();
...
void WebStyle::drawKBarHandle(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, KToolBarPos /* pos */, QBrush * /* fill */
                              )
{
    p->save();
    p->setPen(g.mid());
...
void WebStyle::drawKMenuBar(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool /* macMode */, QBrush * /* fill */
                            )
{
    p->save();
    p->setPen(g.mid());
...
void WebStyle::drawKToolBarButton(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool sunken, bool raised, bool enabled, bool popup,
                                  KToolButtonType type, const QString &btext, const QPixmap *pixmap, QFont *font, QWidget *button)
{
    ...
    switch(type)
    {
        ...
        case Text:
            ...
            if(!btext.isNull())
            {
                ...
                p->drawText(x, y, w, h, AlignCenter, btext);
            }
        ...
        case IconTextRight:
            ...
            if(0 != pixmap)
                p->drawPixmap(x + 2, y + (h - pixmap->height()) / 2, *pixmap);
            ...
            if(!btext.isNull())
            {
                ...
                if(0 != pixmap)
                {
                    ...
                    p->drawText(x + textLeft, y, w - textLeft, h, AlignVCenter | AlignLeft, btext);
                }
                else
                {
                    p->drawText(x, y, w, h, AlignVCenter | AlignLeft, btext);
                }
            }
            break;
        ...
        case IconTextBottom:
            ...
            if(0 != pixmap)
                p->drawPixmap(x + (w - pixmap->width()) / 2, y + 2, *pixmap);
            ...
            if(!btext.isNull())
            {
                ...
                if(0 != pixmap)
                {
                    ...
                    p->drawText(x + 2, textTop, w - 4, h - x - textTop, AlignCenter, btext);
                }
                else
                {
                    p->drawText(x, y, w, h, AlignCenter, btext);
...
void WebStyle::drawKMenuItem(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool active, QMenuItem *mi, QBrush * /* fill */
                             )
{
    p->save();
    ...
    QColor textColour = active ? contrastingForeground(g.highlightedText(), bg) : contrastingForeground(g.text(), bg);

    QApplication::style().drawItem(p, x, y, w, h, AlignCenter | ShowPrefix | DontClip | SingleLine, g, mi->isEnabled(), mi->pixmap(), mi->text(), -1,
                                   &textColour);

    p->restore();
}

void WebStyle::drawPopupMenuItem(QPainter *p, bool checkable, int maxpmw, int tab, QMenuItem *mi, const QPalette &pal, bool act, bool enabled, int x,
                                 int y, int w, int h)
{
    // TODO
...
void WebStyle::drawKProgressBlock(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, QBrush *fill)
{
    p->save();
...
void WebStyle::drawFocusRect(QPainter *p, const QRect &r, const QColorGroup &g, const QColor *pen, bool atBorder)
{
    p->save();
...
void WebStyle::drawPanel(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool /* sunken */, int /* lineWidth */, const QBrush *fill)
{
    p->save();
...
void WebStyle::drawPopupPanel(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, int /* lineWidth */, const QBrush *fill)
{
    p->save();
...
void WebStyle::drawSeparator(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, bool /* sunken */, int /* lineWidth */,
                             int /* midLineWidth */
                             )
{
    p->save();
...
void WebStyle::drawTab(QPainter *p, const QTabBar *tabBar, QTab *tab, bool selected)
{
    QRect r(tab->rect());
...
void WebStyle::drawTabMask(QPainter *p, const QTabBar *, QTab *tab, bool)
{
    p->fillRect(tab->rect(), Qt::color1);
}

void WebStyle::drawKickerHandle(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, QBrush *fill)
{
    p->save();
...
void WebStyle::drawKickerAppletHandle(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, QBrush *fill)
{
    p->save();
...
void WebStyle::drawKickerTaskButton(QPainter *p, int x, int y, int w, int h, const QColorGroup &g, const QString &text, bool active, QPixmap *icon,
                                    QBrush * /* fill */
                                    )
{
    p->save();
...
int WebStyle::popupMenuItemHeight(bool, QMenuItem *i, const QFontMetrics &fm)
{
    if(i->isSeparator())
        return 1;
    ...
    if(0 != i->iconSet())
    {
        h = QMAX(i->iconSet()->pixmap(QIconSet::Small, QIconSet::Normal).height(), h);
    }
