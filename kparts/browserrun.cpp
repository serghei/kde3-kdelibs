...
BrowserRun::BrowserRun(const KURL &url, const KParts::URLArgs &args, KParts::ReadOnlyPart *part, QWidget *window, bool removeReferrer,
                       bool trustedSource)
    : KRun(url, window, 0 /*mode*/, false /*is_local_file known*/, false /* no GUI */)
    , m_args(args)
    , m_part(part)
    , m_window(window)
    , m_bRemoveReferrer(removeReferrer)
    , m_bTrustedSource(trustedSource)
{
...
BrowserRun::BrowserRun(const KURL &url, const KParts::URLArgs &args, KParts::ReadOnlyPart *part, QWidget *window, bool removeReferrer,
                       bool trustedSource, bool hideErrorDialog)
    : KRun(url, window, 0 /*mode*/, false /*is_local_file known*/, false /* no GUI */)
    , m_args(args)
    , m_part(part)
    , m_window(window)
...
void BrowserRun::scanFile()
{
    ...
    connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(slotBrowserScanFinished(KIO::Job *)));
    connect(job, SIGNAL(mimetype(KIO::Job *, const QString &)), this, SLOT(slotBrowserMimetype(KIO::Job *, const QString &)));
    m_job = job;
}
       ...
       !m_strURL.isLocalFile())
    {
        ...
        else // "Open" chosen (done by KRun::foundMimeType, called when returning NotHandled)
        {
            ...
            if(m_args.doPost())
            {
                ...
                job->setWindow(m_window);
                connect(job, SIGNAL(result(KIO::Job *)), this, SLOT(slotCopyToTempFileResult(KIO::Job *)));
...
bool BrowserRun::allowExecution(const QString &serviceType, const KURL &url)
{
    ...
    return (KMessageBox::warningContinueCancel(0, i18n("Do you really want to execute '%1'? ").arg(url.prettyURL()), i18n("Execute File?"),
                                               i18n("Execute"))
...
BrowserRun::AskSaveResult BrowserRun::askSave(const KURL &url, KService::Ptr offer, const QString &mimeType, const QString &suggestedFilename)
{
    ...
    QString openText = (offer && !offer->name().isEmpty()) ? i18n("&Open with '%1'").arg(offer->name()) : i18n("&Open With...");

    int choice = KMessageBox::questionYesNoCancel(0L, question, url.host(), KStdGuiItem::saveAs(), openText,
...
BrowserRun::AskSaveResult BrowserRun::askEmbedOrSave(const KURL &url, const QString &mimeType, const QString &suggestedFilename, int flags)
{
    ...
    if(flags != (int)AttachmentDisposition
       && (mime->is("text/html") || mime->is("text/xml") || mime->is("inode/directory") || mimeType.startsWith("image")
           || mime->is("multipart/x-mixed-replace") || mime->is("multipart/replace") || mimeType.startsWith("print")))
        return Open;

    QString question = makeQuestion(url, mimeType, suggestedFilename);

    int choice = KMessageBox::questionYesNoCancel(0L, question, url.host(), KStdGuiItem::saveAs(), KGuiItem(i18n("&Open"), "fileopen"),
...
void BrowserRun::simpleSave(const KURL &url, const QString &suggestedFilename, QWidget *window)
{
    ...
    KFileDialog *dlg = new KFileDialog(QString::null, QString::null /*all files*/, window, "filedialog", true);
...
void BrowserRun::redirectToError(int error, const QString &errorText)
{
     ...
     */
    KURL newURL(QString("error:/?error=%1&errText=%2").arg(error).arg(KURL::encode_string(errorText)), 106);
...
{
    return (serviceType == "application/x-desktop" || serviceType == "application/x-shellscript");
}
