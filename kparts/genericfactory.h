 ...
 * @internal
 */
template < class T > class GenericFactoryBase : public KParts::Factory {
public:
    GenericFactoryBase()
    {
        if(s_self)
 ...
 */
template < class T > class GenericFactory : public GenericFactoryBase< T > {
public:
    GenericFactory()
    {
    }
    ...
    {
        T *part = KDEPrivate::ConcreteFactory< T >::create(parentWidget, widgetName, parent, name, className, args);
...
template < class T1, class T2 > class GenericFactory< KTypeList< T1, T2 > > : public GenericFactoryBase< T1 > {
public:
    GenericFactory()
    {
    }
    ...
    virtual KParts::Part *createPartObject(QWidget *parentWidget, const char *widgetName, QObject *parent, const char *name, const char *className,
                                           const QStringList &args)
    {
        QObject *object = KDEPrivate::MultiFactory< KTypeList< T1, T2 > >::create(parentWidget, widgetName, parent, name, className, args);
 ...
 * @internal
 */
template < class T > GenericFactoryBase< T > *GenericFactoryBase< T >::s_self = 0;

/**
 * @internal
 */
template < class T > KInstance *GenericFactoryBase< T >::s_instance = 0;

/**
 * @internal
 */
template < class T > KAboutData *GenericFactoryBase< T >::s_aboutData = 0;

/**
 * @internal
 */
template < class T > KInstance *GenericFactoryBase< T >::instance()
{
    if(!s_instance)
    {
        if(s_self)
 ...
 * @internal
 */
template < class T > KAboutData *GenericFactoryBase< T >::aboutData()
{
    if(!s_aboutData)
