        ...
        switch(ev->type())
        {
            ...
            case QEvent::MouseButtonDblClick:
            {
                ...
                m_reason = mev->button() == Qt::LeftButton
                               ...
                               : (mev->button() == Qt::MidButton ? PartManager::ReasonMidClick : PartManager::ReasonRightClick);
                break;
            }
            case QEvent::FocusIn:
...
PartManager::PartManager(QWidget *parent, const char *name) : QObject(parent, name)
{
...
PartManager::PartManager(QWidget *topLevel, QObject *parent, const char *name) : QObject(parent, name)
{
    d = new PartManagerPrivate;
...
{
    for(QPtrListIterator< QWidget > it(d->m_managedTopLevelWidgets); it.current(); ++it)
        disconnect(it.current(), SIGNAL(destroyed()), this, SLOT(slotManagedTopLevelWidgetDestroyed()));
...
bool PartManager::eventFilter(QObject *obj, QEvent *ev)
{
    ...
    if((w->testWFlags(WType_Dialog) && w->isModal()) || w->testWFlags(WType_Popup) || w->testWFlags(WStyle_Tool))
        return false;
    ...
    if(ev->type() == QEvent::MouseButtonPress || ev->type() == QEvent::MouseButtonDblClick)
    {
        ...
        kdDebug(1000) << "PartManager::eventFilter button: " << mev->button() << " "
                      << "d->m_activationButtonMask=" << d->m_activationButtonMask << endl;
#endif
    ...
    while(w)
    {
...
#ifdef DEBUG_PARTMANAGER
        QCString evType =
            (ev->type() == QEvent::MouseButtonPress) ? "MouseButtonPress" : (ev->type() == QEvent::MouseButtonDblClick)
                                                                                ? "MouseButtonDblClick"
        ...
        if(part) // We found a part whose widget is w
        {
            if(d->m_policy == PartManager::TriState)
            {
                ...
                if((d->m_selectedWidget != w || d->m_selectedPart != part) && (d->m_activeWidget != w || d->m_activePart != part))
                {
                    ...
                    {
#ifdef DEBUG_PARTMANAGER
                        kdDebug(1000) << "Part " << part << " (non-selectable) made active because " << w->className() << " got event"
                                      << " " << evType << endl;
#endif
                        d->setReason(ev);
                        setActivePart(part, w);
                ...
                {
#ifdef DEBUG_PARTMANAGER
                    kdDebug(1000) << "Part " << part << " made active (from selected) because " << w->className() << " got event"
                                  << " " << evType << endl;
#endif
                    d->setReason(ev);
                    setActivePart(part, w);
            ...
            {
#ifdef DEBUG_PARTMANAGER
                kdDebug(1000) << "Part " << part << " made active because " << w->className() << " got event"
                              << " " << evType << endl;
#endif
                d->setReason(ev);
                setActivePart(part, w);
        ...
        if(w && ((w->testWFlags(WType_Dialog) && w->isModal()) || w->testWFlags(WType_Popup) || w->testWFlags(WStyle_Tool)))
        {
#ifdef DEBUG_PARTMANAGER
...
void PartManager::addPart(Part *part, bool setActive)
{
    ...
    {
        kdWarning(1000) << "Part '" << part->name() << "' has a widget " << part->widget()->name()
                        << " with a focus policy of NoFocus. It should have at least a ClickFocus policy, for part activation to work well." << endl;
    }
    if(part->widget() && part->widget()->focusPolicy() == QWidget::TabFocus)
    {
        kdWarning(1000) << "Part '" << part->name() << "' has a widget " << part->widget()->name()
                        << " with a focus policy of TabFocus. It should have at least a ClickFocus policy, for part activation to work well." << endl;
    }
...
void PartManager::addManagedTopLevelWidget(const QWidget *topLevel)
{
    ...
    connect(topLevel, SIGNAL(destroyed()), this, SLOT(slotManagedTopLevelWidgetDestroyed()));
}
