...
Part::Part(QObject *parent, const char *name) : QObject(parent, name)
{
    d = new PartPrivate;
    m_widget = 0L;
    m_manager = 0L;
...
Part::~Part()
{
    ...
    if(m_widget)
    {
        ...
        disconnect(m_widget, SIGNAL(destroyed()), this, SLOT(slotWidgetDestroyed()));
    }
...
ReadOnlyPart::ReadOnlyPart(QObject *parent, const char *name) : Part(parent, name), m_bTemp(false)
{
...
ReadWritePart::ReadWritePart(QObject *parent, const char *name) : ReadOnlyPart(parent, name), m_bModified(false), m_bClosing(false)
{
    m_bReadWrite = true;
}
...
bool ReadWritePart::queryClose()
{
    ...
    int res = KMessageBox::warningYesNoCancel(widget(), i18n("The document \"%1\" has been modified.\n"
