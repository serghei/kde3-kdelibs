...
public:
    ...
    StatusBarItem(QWidget *widget, int stretch, bool permanent) : m_widget(widget), m_stretch(stretch), m_permanent(permanent), m_visible(false)
    {
    }
...
StatusBarExtension::StatusBarExtension(KParts::ReadOnlyPart *parent, const char *name) : QObject(parent, name), m_statusBar(0), d(0)
{
...
{
    if(!GUIActivateEvent::test(ev) || !watched->inherits("KParts::ReadOnlyPart"))
