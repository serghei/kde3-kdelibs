...
AutoBookmarker::AutoBookmarker(QObject *parent, const char *name, const QStringList & /*args*/)
    : KTextEditor::Plugin((KTextEditor::Document *)parent, name), KTextEditor::ConfigInterfaceExtension()
{
    if(parent)
...
void AutoBookmarker::slotCompleted()
{
    ...
    KTextEditor::DocumentInfoInterface *di =
        static_cast< KTextEditor::DocumentInfoInterface * >(document()->qt_cast("KTextEditor::DocumentInfoInterface"));
    QString mt;
...
void ABGlobal::readConfig()
{
    ...
    while(config->hasGroup(QString("autobookmark%1").arg(n)))
    {
        ...
        AutoBookmarkEnt *e = new AutoBookmarkEnt(config->readEntry("pattern", ""), filemask, mimemask, flags);
...
public:
    AutoBookmarkEntItem(KListView *lv, AutoBookmarkEnt *e) : QListViewItem(lv), ent(e)
    {
        redo();
    };
    ~AutoBookmarkEntItem(){};
...
AutoBookmarkerEntEditor::AutoBookmarkerEntEditor(QWidget *parent, AutoBookmarkEnt *e)
    : KDialogBase(parent, "autobookmark_ent_editor", true, i18n("Edit Entry"), KDialogBase::Ok | KDialogBase::Cancel), e(e)
{
    ...
    QWhatsThis::add(lePattern, i18n("<p>A regular expression. Matching lines will be bookmarked.</p>"));
    ...
    QWhatsThis::add(cbCS, i18n("<p>If enabled, the pattern matching will be case sensitive, otherwise "
                               "not.</p>"));
    ...
    QWhatsThis::add(cbMM, i18n("<p>If enabled, the pattern matching will use minimal matching; if you "
    ...
    QWhatsThis::add(leFileMask, i18n("<p>A list of filename masks, separated by semicolons. This can be used "
    ...
    QWhatsThis::add(leMimeTypes, i18n("<p>A list of mime types, separated by semicolon. This can be used to "
    ...
    QWhatsThis::add(btnMTW, i18n("<p>Click this button to display a checkable list of mimetypes available "
...
void AutoBookmarkerEntEditor::apply()
{
    ...
    e->flags = 0;
    if(cbCS->isOn())
        e->flags |= AutoBookmarkEnt::CaseSensitive;
    if(cbMM->isOn())
        e->flags |= AutoBookmarkEnt::MinimalMatching;
}

void AutoBookmarkerEntEditor::showMTDlg()
{
    QString text =
        i18n("Select the MimeTypes for this pattern.\nPlease note that this will automatically edit the associated file extensions as well.");
...
AutoBookmarkerConfigPage::AutoBookmarkerConfigPage(QWidget *parent, const char *name) : KTextEditor::ConfigPage(parent, name)
{
    ...
    QWhatsThis::add(lvPatterns, i18n("<p>This list shows your configured autobookmark entities. When a document "
    ...
    QWhatsThis::add(btnNew, i18n("Press this button to create a new autobookmark entity."));

    btnDel = new QPushButton(i18n("&Delete"), this);
    lo1->addWidget(btnDel);
    QWhatsThis::add(btnDel, i18n("Press this button to delete the currently selected entity."));

    btnEdit = new QPushButton(i18n("&Edit..."), this);
    lo1->addWidget(btnEdit);
    QWhatsThis::add(btnEdit, i18n("Press this button to edit the currently selected entity."));
...
AutoBookmarkEnt::AutoBookmarkEnt(const QString &p, const QStringList &f, const QStringList &m, int fl)
    : pattern(p), filemask(f), mimemask(m), flags(fl)
{
    ;
}
// END
