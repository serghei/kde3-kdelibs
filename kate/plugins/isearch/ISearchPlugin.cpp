    ...
    , m_toolBarWasHidden(false)
{
    ...
    m_searchForwardAction =
        new KAction(i18n("Search Incrementally"), CTRL + ALT + Key_F, this, SLOT(slotSearchForwardAction()), actionCollection(), "edit_isearch");
    m_searchBackwardAction = new KAction(i18n("Search Incrementally Backwards"), CTRL + ALT + SHIFT + Key_F, this, SLOT(slotSearchBackwardAction()),
                                         actionCollection(), "edit_isearch_reverse");

    m_label = new QLabel(i18n("I-Search:"), 0L, "kde toolbar widget");
    KWidgetAction *labelAction = new KWidgetAction(m_label, i18n("I-Search:"), 0, 0, 0, actionCollection(), "isearch_label");
    ...
    connect(m_combo, SIGNAL(textChanged(const QString &)), this, SLOT(slotTextChanged(const QString &)));
    connect(m_combo, SIGNAL(returnPressed(const QString &)), this, SLOT(slotReturnPressed(const QString &)));
    connect(m_combo, SIGNAL(aboutToShowContextMenu(QPopupMenu *)), this, SLOT(slotAddContextMenuItems(QPopupMenu *)));
    m_comboAction = new KWidgetAction(m_combo, i18n("Search"), 0, 0, 0, actionCollection(), "isearch_combo");
    m_comboAction->setAutoSized(true);
    m_comboAction->setShortcutConfigurable(false);

    KActionMenu *optionMenu = new KActionMenu(i18n("Search Options"), "configure", actionCollection(), "isearch_options");
    optionMenu->setDelayed(false);

    KToggleAction *action = new KToggleAction(i18n("Case Sensitive"), KShortcut(), actionCollection(), "isearch_case_sensitive");
    ...
    action = new KToggleAction(i18n("From Beginning"), KShortcut(), actionCollection(), "isearch_from_beginning");
    action->setShortcutConfigurable(false);
    connect(action, SIGNAL(toggled(bool)), this, SLOT(setFromBeginning(bool)));
    action->setChecked(m_fromBeginning);
    optionMenu->insert(action);

    action = new KToggleAction(i18n("Regular Expression"), KShortcut(), actionCollection(), "isearch_reg_exp");
    action->setShortcutConfigurable(false);
    connect(action, SIGNAL(toggled(bool)), this, SLOT(setRegExp(bool)));
...
bool ISearchPluginView::eventFilter(QObject *o, QEvent *e)
{
    ...
    if(e->type() == QEvent::FocusIn)
    {
        ...
        if(focusEvent->reason() == QFocusEvent::ActiveWindow || focusEvent->reason() == QFocusEvent::Popup)
            return false;
        startSearch();
    }
...
// Sigh... i18n hell.
void ISearchPluginView::updateLabelText(bool failing /* = false */, bool reverse /* = false */, bool wrapped /* = false */,
...
void ISearchPluginView::slotAddContextMenuItems(QPopupMenu *menu)
{
    if(menu)
    {
        ...
        menu->insertItem(i18n("Case Sensitive"), this, SLOT(setCaseSensitive(bool)));
        menu->insertItem(i18n("From Beginning"), this, SLOT(setFromBeginning(bool)));
        menu->insertItem(i18n("Regular Expression"), this, SLOT(setRegExp(bool)));
        // menu->insertItem( i18n("Auto-Wrap Search"), this,
        //		  SLOT(setAutoWrap(bool)));
    }
}

bool ISearchPluginView::iSearch(uint startLine, uint startCol, const QString &text, bool reverse, bool autoWrap)
{
    if(!m_view)
        return false;
    ...
    if(!m_regExp)
    {
        found = m_searchIF->searchText(startLine, startCol, text, &m_foundLine, &m_foundCol, &m_matchLen, m_caseSensitive, reverse);
    }
    else
    {
        found = m_searchIF->searchText(startLine, startCol, QRegExp(text), &m_foundLine, &m_foundCol, &m_matchLen, reverse);
    }
    if(found)
    {
    ...
    // FIXME
    bool overwrapped = (m_wrapped && ((m_foundLine > m_startLine) || (m_foundLine == m_startLine && m_foundCol >= m_startCol)));
    //	kdDebug() << "Overwrap = " << overwrapped << ". Start was " << m_startLine << ", " << m_startCol << endl;
    updateLabelText(!found, reverse, m_wrapped, overwrapped);
    return found;
}

ISearchPlugin::ISearchPlugin(QObject *parent, const char *name, const QStringList &) : KTextEditor::Plugin((KTextEditor::Document *)parent, name)
{
}
