...
DocWordCompletionPlugin::DocWordCompletionPlugin(QObject *parent, const char *name, const QStringList & /*args*/)
...
DocWordCompletionPluginView::DocWordCompletionPluginView(uint treshold, bool autopopup, KTextEditor::View *view, const char *name)
    : QObject(view, name), KXMLGUIClient(view), m_view(view), d(new DocWordCompletionPluginViewPrivate)
{
    d->treshold = treshold;
    view->insertChildClient(this);
    setInstance(KGenericFactory< DocWordCompletionPlugin >::instance());

    (void)new KAction(i18n("Reuse Word Above"), CTRL + Key_8, this, SLOT(completeBackwards()), actionCollection(), "doccomplete_bw");
    (void)new KAction(i18n("Reuse Word Below"), CTRL + Key_9, this, SLOT(completeForwards()), actionCollection(), "doccomplete_fw");
    (void)new KAction(i18n("Pop Up Completion List"), 0, this, SLOT(popupCompletionList()), actionCollection(), "doccomplete_pu");
    (void)new KAction(i18n("Shell Completion"), 0, this, SLOT(shellComplete()), actionCollection(), "doccomplete_sh");
    d->autopopup = new KToggleAction(i18n("Automatic Completion Popup"), 0, this, SLOT(toggleAutoPopup()), actionCollection(), "enable_autopopup");
    ...
    if(vi)
    {
        ...
        connect(view->document(), SIGNAL(variableChanged(const QString &, const QString &)), this,
                SLOT(slotVariableChanged(const QString &, const QString &)));
    }
}
...
void DocWordCompletionPluginView::complete(bool fw)
{
    ...
    /* IF the current line is equal to the previous line
       ...
       AND the lastinsertedlength last characters of the word is
            ...
            */
    if(cline == d->cline && ccol - d->lilen == d->ccol && wrd.endsWith(d->lastIns))
    {
    ...
    while(true)
    {
        pos = fw ? d->re.search(ln, d->col) : d->re.searchRev(ln, d->col);
...
QString DocWordCompletionPluginView::word()
{
    ...
    KTextEditor::EditInterface *ei = KTextEditor::editInterface(m_view->document());
    d->re.setPattern("\\b(\\w+)$");
    if(d->re.searchRev(ei->text(cline, 0, cline, ccol)) < 0)
        return QString::null; // no word
    return d->re.cap(1);
}
...
DocWordCompletionConfigPage::DocWordCompletionConfigPage(DocWordCompletionPlugin *completion, QWidget *parent, const char *name)
    : KTextEditor::ConfigPage(parent, name), m_completion(completion)
{
    ...
    lo->addWidget(hb);
    QLabel *l = new QLabel(i18n("Translators: This is the first part of two strings wich will comprise the "
    ...
    lSbRight = new QLabel(i18n("This is the second part of two strings that will comprise teh sentence "
                               "'Show completions when a word is at least N characters'",
                               "characters long."),
                          hb);

    QWhatsThis::add(cbAutoPopup, i18n("Enable the automatic completion list popup as default. The popup can "
                                      "be disabled on a view basis from the 'Tools' menu."));
    QWhatsThis::add(sbAutoPopup, i18n("Define the length a word should have before the completion list "
                                      "is displayed."));
