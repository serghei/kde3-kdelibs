...
KateSuperCursor::KateSuperCursor(KateDocument *doc, bool privateC, const KateTextCursor &cursor, QObject *parent, const char *name)
    : QObject(parent, name), KateDocCursor(cursor.line(), cursor.col(), doc), Kate::Cursor(), m_doc(doc)
{
...
KateSuperCursor::KateSuperCursor(KateDocument *doc, bool privateC, int lineNum, int col, QObject *parent, const char *name)
    : QObject(parent, name), KateDocCursor(lineNum, col, doc), Kate::Cursor(), m_doc(doc)
{
    m_moveOnInsert = false;
    m_lineRemoved = false;
    m_privateCursor = privateC;
...
{
    return isValid() && ((int)lineNum > superStart().line() || ((int)lineNum == superStart().line() && superStart().atStartOfLine()))
           && ((int)lineNum < superEnd().line() || ((int)lineNum == superEnd().line() && superEnd().atEndOfLine()));
}
...
KateSuperRangeList::KateSuperRangeList(bool autoManage, QObject *parent, const char *name)
    : QObject(parent, name), m_autoManage(autoManage), m_connect(true), m_trackingBoundaries(false)
{
...
KateSuperRangeList::KateSuperRangeList(const QPtrList< KateSuperRange > &rangeList, QObject *parent, const char *name)
    : QObject(parent, name), m_autoManage(false), m_connect(false), m_trackingBoundaries(false)
{
