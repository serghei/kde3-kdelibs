...
#define katelua_registerFunc(n, f, t) (lua_pushstring(m_interpreter, n), lua_pushcfunction(m_interpreter, f), lua_settable(m_interpreter, t))

#define katelua_registerNumConst(n, v, t) (lua_pushstring(m_interpreter, n), lua_pushnumber(m_interpreter, v), lua_settable(m_interpreter, t))
...
static int katelua_katedebug(lua_State *L)
{
    ...
    {
        if(lua_isnil(L, i))
            kdDebug() << "NIL VALUE" << endl;
        else if(lua_isstring(L, i))
            kdDebug() << lua_tostring(L, i) << endl;
        else if(lua_isboolean(L, i))
            kdDebug() << (bool)lua_toboolean(L, i) << endl;
        else if(lua_isnumber(L, i))
            kdDebug() << lua_tonumber(L, i) << endl;
        else
            kdDebug() << "Invalid type for katedebug:" << lua_type(L, i) << endl;
    }
    return 0;
}
...
static int katelua_document_removeText(lua_State *L)
{
    ...
    lua_pushboolean(L,
                    katelua_doc->removeText((uint)lua_tonumber(L, 1), (uint)lua_tonumber(L, 2), (uint)lua_tonumber(L, 3), (uint)lua_tonumber(L, 4)));
    return 1;
}
...
static const struct KATELUA_FUNCTIONS katelua_documenttable[4] = {{"textLine", katelua_document_textline},
                                                                  {"removeText", katelua_document_removeText},
                                                                  {"insertText", katelua_document_insertText},
                                                                  {0, 0}};

static const struct KATELUA_FUNCTIONS katelua_viewtable[5] = {{"cursorLine", katelua_view_cursorline},
...
KateLUAIndentScriptImpl::KateLUAIndentScriptImpl(const QString &internalName, const QString &filePath, const QString &niceName,
...
void KateLUAIndentScriptManager::collectScripts(bool force)
{
    ...
    kdDebug() << "=================================================" << endl
              << "Trying to find Lua scripts" << endl
              << "=================================================" << endl;
    ...
    for(QStringList::Iterator it = list.begin(); it != list.end(); ++it)
    {
        ...
        if(!force && config.hasGroup(Group) && (sbuf.st_mtime == config.readNumEntry("lastModified")))
        {
            ...
            else
            {
                ...
                KateLUAIndentScriptImpl *s = new KateLUAIndentScriptImpl(internalName, filePath, niceName, copyright, version);
...
void KateLUAIndentScriptManager::parseScriptHeader(const QString &filePath, QString *niceName, QString *copyright, double *version)
{
#if 0
