...
KateAutoIndent::KateAutoIndent(KateDocument *_doc) : QObject(), doc(_doc)
{
}
...
bool KateNormalIndent::skipBlanks(KateDocCursor &cur, KateDocCursor &max, bool newline) const
{
    ...
    do
    {
        ...
        if(attrib != commentAttrib && attrib != regionAttrib && attrib != alertAttrib && attrib != preprocessorAttrib
           && !hlFile.endsWith("doxygen.xml"))
        {
...
KateCSmartIndent::KateCSmartIndent(KateDocument *doc) : KateNormalIndent(doc), allowSemi(false), processingBlock(false)
{
...
void KateCSmartIndent::processLine(KateDocCursor &line)
{
    ...
    {
        if(textLine->stringAtPos(firstChar, "case") || textLine->stringAtPos(firstChar, "default") || textLine->stringAtPos(firstChar, "public")
           || textLine->stringAtPos(firstChar, "private") || textLine->stringAtPos(firstChar, "protected")
           || textLine->stringAtPos(firstChar, "signals") || textLine->stringAtPos(firstChar, "Q_SIGNALS")
           || textLine->stringAtPos(firstChar, "Q_SLOTS") || textLine->stringAtPos(firstChar, "slots"))
        {
    ...
    {
        // c# regions
        if(textLine->stringAtPos(firstChar, "#region") || textLine->stringAtPos(firstChar, "#endregion"))
        {
            KateDocCursor temp = line;
            indent = calcIndent(temp, true);
        }
    ...
    if(indent != measureIndent(line) || first == '}' || first == '{' || first == '#')
    {
        ...
        if(indent > 0)
            doc->insertText(line.line(), 0, filler);
        if(!processingBlock)
            line.setCol(filler.length());
    }
}
...
bool KateCSmartIndent::handleDoxygen(KateDocCursor &begin)
{
    ...
    if(first >= 0)
    {
        ...
        if(insideDoxygen)
        {
            ...
            if(doxygenAutoInsert && ((first < 0) || (!textLine->stringAtPos(first, "*/") && !textLine->stringAtPos(first, "*"))))
            {
                filler = filler + " * ";
            }
 ...
 */
static inline bool isColonImmune(const KateNormalIndent &indenter, uchar attr1, uchar attr2, QChar prev1, QChar prev2)
{
 ...
 */
static inline bool colonPermitsReindent(const KateNormalIndent &indenter, const KateTextLine::Ptr &line, int curCol)
{
    ...
    return !isColonImmune(indenter, line->attribute(curCol - 1), line->attribute(curCol - 2), txt[curCol - 1], txt[curCol - 2]);
}
...
void KateCSmartIndent::processChar(QChar c)
{
    ...
    if(c == '/')
    {
        ...
        if(textLine->attribute(begin.col()) == doxyCommentAttrib)
        {
            ...
            if(first != -1 && firstChar == '*' && textLine->nextNonSpaceChar(first + 1) == view->cursorColumnReal() - 1)
    ...
    int pos;
    if(((c == firstChar && firstTriggers.find(firstChar) >= 0) || (c == lastChar && lastTriggers.find(lastChar) >= 0))
...
uint KateCSmartIndent::calcIndent(KateDocCursor &begin, bool needContinue)
{
    ...
    while(cur.validPosition() && cur < begin)
    {
        ...
        // kdDebug(13030) << "  cur.line:" << cur.line() << " cur.col:" << cur.col() << " currentChar '" << tc << "' " <<
        // textLine->attribute(cur.col()) << endl;
...
KatePythonIndent::KatePythonIndent(KateDocument *doc) : KateNormalIndent(doc)
{
}
...
KateXmlIndent::KateXmlIndent(KateDocument *doc) : KateNormalIndent(doc)
{
}
...
void KateXmlIndent::processChar(QChar c)
{
    ...
    if(text.find(startsWithCloseTag) == -1)
        return;
...
void KateXmlIndent::getLineInfo(uint line, uint &prevIndent, int &numTags, uint &attrCol, bool &unclosedTag)
{
    prevIndent = 0;
    int firstChar;
            ...
            case '>':
                if(!seenOpen)
                {
                    ...
                    for(uint backLine = line; backLine;)
                    {
                        ...
                        if(x->string().find(unclosedDoctype) != -1)
                            --numTags;
...
uint KateXmlIndent::processLine(uint line)
{
    ...
    if(unclosedTag)
        ...
        indent = prevIndent + numTags * indentWidth;
    if(indent < 0)
        indent = 0;
...
KateCSAndSIndent::KateCSAndSIndent(KateDocument *doc) : KateNormalIndent(doc)
{
}
...
QString KateCSAndSIndent::calcIndent(const KateDocCursor &begin)
{
    ...
    if(currLineFirst >= 0 && (currLine->attribute(currLineFirst) == commentAttrib || currLine->attribute(currLineFirst) == doxyCommentAttrib))
    ...
    while(cur.gotoPreviousLine())
    {
        ...
        for(pos = lastChar; pos >= firstChar; --pos)
        {
            ...
            if(lookingForScopeKeywords && openParenCount == 0 && textLine->attribute(pos) == keywordAttrib
...
QString KateCSAndSIndent::calcIndentAfterKeyword(const KateDocCursor &indentCursor, const KateDocCursor &keywordCursor, int keywordPos,
                                                 bool blockKeyword)
{
...
QString KateCSAndSIndent::calcIndentInBrace(const KateDocCursor &indentCursor, const KateDocCursor &braceCursor, int bracePos)
{
    ...
    if(indentFirst >= 0 && indentLine->attribute(indentFirst) == symbolAttrib && indentLine->getChar(indentFirst) == ':'
...
void KateCSAndSIndent::processChar(QChar c)
{
    ...
    if(textLine->attribute(begin.col()) == doxyCommentAttrib)
    {
        ...
        if(c == '/')
        {
            ...
            // if the first char exists and is a '*', and the next non-space-char
            // is already the just typed '/', concatenate it to "*/".
            if(first != -1 && textLine->getChar(first) == '*' && textLine->nextNonSpaceChar(first + 1) == view->cursorColumnReal() - 1)
                doc->removeText(view->cursorLine(), first + 1, view->cursorLine(), view->cursorColumnReal() - 1);
        }
...
KateVarIndent::KateVarIndent(KateDocument *doc) : KateNormalIndent(doc)
{
    ...
    connect(doc, SIGNAL(variableChanged(const QString &, const QString &)), this, SLOT(slotVariableChanged(const QString &, const QString &)));
}
...
void KateVarIndent::processLine(KateDocCursor &line)
{
    ...
    // Currently, the check is done.
    {
        ...
        if(i > -1)
        {
            ...
            if(d->couples & Parens && ch == ')' && (at == d->coupleAttrib || (!at && hasRelevantOpening(KateDocCursor(line.line(), i, doc)))))
                adjustment--;
            else if(d->couples & Braces && ch == '}' && (at == d->coupleAttrib || (!at && hasRelevantOpening(KateDocCursor(line.line(), i, doc)))))
                adjustment--;
            else if(d->couples & Brackets && ch == ']' && (at == d->coupleAttrib || (!at && hasRelevantOpening(KateDocCursor(line.line(), i, doc)))))
                adjustment--;
        }
    }
...
#define ISCOMMENT (ISCOMMENTATTR(ktl->attribute(ktl->firstChar())) || ISCOMMENTATTR(ktl->attribute(matchpos)))
    ...
    int matchpos = 0;
    if(ktl && !d->reIndentAfter.isEmpty() && (matchpos = d->reIndentAfter.search(doc->textLine(ln))) > -1 && !ISCOMMENT)
        adjustment++;

    // else, check if this line should indent unless ...
    ktl = doc->plainKateTextLine(line.line());
    if(!d->reIndent.isEmpty() && (matchpos = d->reIndent.search(doc->textLine(line.line()))) > -1 && !ISCOMMENT)
        adjustment++;

    // else, check if the current line indicates if we should remove indentation unless ...
    if(!d->reUnindent.isEmpty() && (matchpos = d->reUnindent.search(doc->textLine(line.line()))) > -1 && !ISCOMMENT)
        adjustment--;
...
void KateVarIndent::slotVariableChanged(const QString &var, const QString &val)
{
    ...
    else if(var == "var-indent-handle-couples")
    {
        ...
        if(l.contains("parens"))
            d->couples |= Parens;
        if(l.contains("braces"))
            d->couples |= Braces;
        if(l.contains("brackets"))
            d->couples |= Brackets;
    }
...
KateScriptIndent::KateScriptIndent(KateDocument *doc) : KateNormalIndent(doc)
{
...
#include <qlabel.h>
ScriptIndentConfigPage::ScriptIndentConfigPage(QWidget *parent, const char *name) : IndenterConfigPage(parent, name)
{
