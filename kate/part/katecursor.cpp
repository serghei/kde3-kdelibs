...
KateDocCursor::KateDocCursor(int line, int col, KateDocument *doc) : KateTextCursor(line, col), m_doc(doc)
{
}
...
bool KateDocCursor::removeText(uint nbChar)
{
    ...
    // Remove the text
    return m_doc->removeText((uint)line(), (uint)col(), (uint)endCursor.line(), (uint)endCursor.col());
}
