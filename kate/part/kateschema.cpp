...
public:
    KateStyleListItem(QListViewItem *parent = 0, const QString &stylename = 0, class KateAttribute *defaultstyle = 0, class KateHlItemData *data = 0);
    KateStyleListItem(QListView *parent, const QString &stylename = 0, class KateAttribute *defaultstyle = 0, class KateHlItemData *data = 0);
    ~KateStyleListItem()
    {
        if(st)
            delete is;
...
KateSchemaManager::KateSchemaManager() : m_config("kateschemarc", false, false)
{
    update();
}
...
KateSchemaConfigColorTab::KateSchemaConfigColorTab(QWidget *parent, const char *) : QWidget(parent)
{
    m_schema = -1;
    ...
    QWhatsThis::add(m_linenumber, i18n("<p>This color will be used to draw the line numbers (if enabled) and the "
    ...
    QWhatsThis::add(m_wwmarker, i18n("<p>Sets the color of Word Wrap-related markers:</p>"
    ...
    QWhatsThis::add(m_tmarker, i18n("<p>Sets the color of the tabulator marks:</p>"));
}
...
KateSchemaConfigFontTab::KateSchemaConfigFontTab(QWidget *parent, const char *) : QWidget(parent)
{
    // sizemanagment
...
KateSchemaConfigFontColorTab::KateSchemaConfigFontColorTab(QWidget *parent, const char *) : QWidget(parent)
{
    ...
    QWhatsThis::add(m_defaultStyles, i18n("This list displays the default styles for the current schema and "
...
void KateSchemaConfigFontColorTab::schemaChanged(uint schema)
{
    ...
    p.setColor(QColorGroup::Base, KateFactory::self()->schemaManager()->schema(schema)->readColorEntry("Color Background", &_c));
    _c = KGlobalSettings::highlightColor();
    p.setColor(QColorGroup::Highlight, KateFactory::self()->schemaManager()->schema(schema)->readColorEntry("Color Selection", &_c));
    ...
    : QWidget(parent)
{
    ...
    QWhatsThis::add(m_styles, i18n("This list displays the contexts of the current syntax highlight mode and "
...
void KateSchemaConfigHighlightTab::schemaChanged(uint schema)
{
    ...
    QColor _c(KGlobalSettings::baseColor());
    p.setColor(QColorGroup::Base, KateFactory::self()->schemaManager()->schema(m_schema)->readColorEntry("Color Background", &_c));
    _c = KGlobalSettings::highlightColor();
    p.setColor(QColorGroup::Highlight, KateFactory::self()->schemaManager()->schema(m_schema)->readColorEntry("Color Selection", &_c));
    _c = l->at(0)->textColor(); // not quite as much of an assumption ;)
    p.setColor(QColorGroup::Text, _c);
    m_styles->viewport()->setPalette(p);

    QDict< KateStyleListCaption > prefixes;
    for(KateHlItemData *itemData = m_hlDict[m_schema]->find(m_hl)->last(); itemData != 0L; itemData = m_hlDict[m_schema]->find(m_hl)->prev())
    {
...
KateSchemaConfigPage::KateSchemaConfigPage(QWidget *parent, KateDocument *doc) : KateConfigPage(parent), m_lastSchema(-1)
{
    QVBoxLayout *layout = new QVBoxLayout(this, 0, KDialog::spacingHint());
    ...
    connect(schemaCombo, SIGNAL(activated(int)), this, SLOT(schemaChanged(int)));
...
KateStyleListView::KateStyleListView(QWidget *parent, bool showUseDefaults) : QListView(parent)
{
    ...
    connect(this, SIGNAL(contextMenuRequested(QListViewItem *, const QPoint &, int)), this, SLOT(showPopupMenu(QListViewItem *, const QPoint &)));
...
KateStyleListItem::KateStyleListItem(QListViewItem *parent, const QString &stylename, KateAttribute *style, KateHlItemData *data)
    : QListViewItem(parent, stylename), ds(style), st(data)
{
    initStyle();
}

KateStyleListItem::KateStyleListItem(QListView *parent, const QString &stylename, KateAttribute *style, KateHlItemData *data)
    : QListViewItem(parent, stylename), ds(style), st(data)
{
    initStyle();
}
...
void KateStyleListItem::toggleDefStyle()
{
    if(*is == *ds)
    {
        KMessageBox::information(listView(), i18n("\"Use Default Style\" will be automatically unset when you change any style properties."),
                                 i18n("Kate Styles"), "Kate hl config use defaults");
    }
    else
    {
        delete is;
...
void KateStyleListItem::setColor(int column)
{
    ...
    if(KColorDialog::getColor(c, d, listView()) != QDialog::Accepted)
        return;
...
void KateStyleListItem::paintCell(QPainter *p, const QColorGroup & /*cg*/, int col, int width, int align)
{
    ...
    switch(col)
    {
        ...
        case UseDefStyle:
        {
            ...
            x++;
            y++;
            if((col == Bold && is->bold()) || (col == Italic && is->italic()) || (col == Underline && is->underline())
               || (col == Strikeout && is->strikeOut()) || (col == UseDefStyle && *is == *ds))
            {
                QPointArray a(7 * 2);
                int i, xx, yy;
                xx = x + 1 + marg;
...
KateStyleListCaption::KateStyleListCaption(QListView *parent, const QString &name) : QListViewItem(parent, name)
{
}
