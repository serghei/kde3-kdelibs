...
public:
    KateCompletionItem(QListBox *lb, KTextEditor::CompletionEntry entry) : QListBoxText(lb), m_entry(entry)
    {
        if(entry.postfix == "()")
...
KateCodeCompletion::KateCodeCompletion(KateView *view) : QObject(view, "Kate Code Completion"), m_view(view), m_commentLabel(0)
{
    ...
    connect(m_pArgHint, SIGNAL(argHintHidden()), this, SIGNAL(argHintHidden()));
...
void KateCodeCompletion::showCompletionBox(QValueList< KTextEditor::CompletionEntry > complList, int offset, bool casesensitive)
{
...
void KateCodeCompletion::handleKey(QKeyEvent *e)
{
    ...
    // keyboard movement
    if((e->key() == Key_Up) || (e->key() == Key_Down) || (e->key() == Key_Home) || (e->key() == Key_End) || (e->key() == Key_Prior)
       || (e->key() == Key_Next))
    {
        QTimer::singleShot(0, this, SLOT(showComment()));
...
void KateCodeCompletion::updateBox(bool)
{
    ...
    */
    ...
    if(m_completionListBox->count() == 0 || (m_completionListBox->count() == 1 && // abort if we equaled the last item
...
void KateCodeCompletion::showComment()
{
    ...
    finalPoint.setY(m_completionListBox->viewport()
                        ->mapToGlobal(m_completionListBox->itemRect(m_completionListBox->item(m_completionListBox->currentItem())).topLeft())
                        .y());
...
KateArgHint::KateArgHint(KateView *parent, const char *name) : QFrame(parent, name, WType_Popup)
{
...
void KateArgHint::cursorPositionChanged(KateView *view, int line, int col)
{
    ...
    text = text.replace(strconst_rx, "\"\"").replace(chrconst_rx, "''");
