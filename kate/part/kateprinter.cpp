...
bool KatePrinter::print(KateDocument *doc)
{
    ...
    if(printer.setup(kapp->mainWidget(), i18n("Print %1").arg(printer.docName())))
    {
        ...
        */
        ...
        // Text Settings Page
        bool selectionOnly = (doc->hasSelection() && (printer.option("app-kate-printselection") == "true"));
        int selStartCol = 0;
        int selEndCol = 0;
        ...
        // Layout Page
        renderer.config()->setSchema(KateFactory::self()->schemaManager()->number(printer.option("app-kate-colorscheme")));
        ...
        */
        {
            ...
            if(useGuide)
            {
                ...
                {
                    _widest = kMax(_widest, ((QFontMetrics)(_d->bold()
                                                                ? _d->italic() ? renderer.config()->fontStruct()->myFontMetricsBI
                                                                               : renderer.config()->fontStruct()->myFontMetricsBold
                                                                : _d->italic() ? renderer.config()->fontStruct()->myFontMetricsItalic
            ...
            if(headerTagList.grep("%P").count() || footerTagList.grep("%P").count())
            {
                ...
                for(uint i = firstline; i < lastline; i++)
                {
                    ...
                    while(_lw >= 0)
                    {
                        ...
                        _lw -= maxWidth;
                        if(!_lw)
                            _lw--; // skip lines matching exactly!
                    }
                }
                if(_lt)
                    _pages++; // last page
            ...
            while(needWrap)
            {
                ...
                if(pageStarted)
                {
                    ...
                    if(useHeader)
                    {
                        ...
                        {
                            int valign = ((useBox || headerDrawBg || useBackground) ? Qt::AlignVCenter : Qt::AlignTop);
                    ...
                    { // FIXME - this may span more pages...
                        ...
                        QRect _r;
                        paint.drawText(_marg, y, pdmWidth - (2 * _marg), maxHeight - y, Qt::AlignTop | Qt::AlignHCenter,
                        ...
                        while((_d = _it.current()) != 0)
                        {
                            ...
                            paint.drawText((_x + ((_i % guideCols) * _cw)), y, _cw, renderer.fontHeight(), Qt::AlignVCenter | Qt::AlignLeft, _d->name,
                                           -1, &_r);
                            _i++;
                            if(_i && !(_i % guideCols))
                        ...
                        if(_i % guideCols)
                            y += renderer.fontHeight(); // last row not full
                ...
                if(printLineNumbers && !startCol) // don't repeat!
                {
                    ...
                    paint.drawText(((useBox || useBackground) ? innerMargin : 0), y, lineNumberWidth, renderer.fontHeight(), Qt::AlignRight,
                                   QString("%1").arg(lineCount + 1));
                }
                endCol = renderer.textWidth(doc->kateTextLine(lineCount), startCol, maxWidth, &needWrap);

                if(endCol < startCol)
                {
                    // kdDebug(13020)<<"--- Skipping garbage, line: "<<lineCount<<" start: "<<startCol<<" end: "<<endCol<<" real EndCol; "<<
                    // buffer->line(lineCount)->length()<< " !?"<<endl;
                    lineCount++;
...
KatePrintTextSettings::KatePrintTextSettings(KPrinter * /*printer*/, QWidget *parent, const char *name) : KPrintDialogPage(parent, name)
{
    ...
    // whatsthis
    QWhatsThis::add(cbSelection, i18n("<p>This option is only available if some text is selected in the document.</p>"
                                      "<p>If available and enabled, only the selected text is printed.</p>"));
    QWhatsThis::add(cbLineNumbers, i18n("<p>If enabled, line numbers will be printed on the left side of the page(s).</p>"));
    QWhatsThis::add(cbGuide, i18n("<p>Print a box displaying typographical conventions for the document type, as "
...
KatePrintHeaderFooter::KatePrintHeaderFooter(KPrinter * /*printer*/, QWidget *parent, const char *name) : KPrintDialogPage(parent, name)
{
...
KatePrintLayout::KatePrintLayout(KPrinter * /*printer*/, QWidget *parent, const char *name) : KPrintDialogPage(parent, name)
{
    ...
    QWhatsThis::add(cbDrawBackground, i18n("<p>If enabled, the background color of the editor will be used.</p>"
                                           "<p>This may be useful if your color scheme is designed for a dark background.</p>"));
    QWhatsThis::add(cbEnableBox, i18n("<p>If enabled, a box as defined in the properties below will be drawn "
                                      "around the contents of each page. The Header and Footer will be separated "
                                      "from the contents with a line as well.</p>"));
    QWhatsThis::add(sbBoxWidth, i18n("The width of the box outline"));
    QWhatsThis::add(sbBoxMargin, i18n("The margin inside boxes, in pixels"));
    QWhatsThis::add(kcbtnBoxColor, i18n("The line color to use for boxes"));
}
