    ...
    , m_imPreeditSelStart(0)
{
    ...
    connect(m_columnScroll, SIGNAL(valueChanged(int)), this, SLOT(scrollColumns(int)));
    ...
    connect(leftBorder, SIGNAL(toggleRegionVisibility(unsigned int)), m_doc->foldingTree(), SLOT(toggleRegionVisibility(unsigned int)));

    connect(doc->foldingTree(), SIGNAL(regionVisibilityChangedAt(unsigned int)), this, SLOT(slotRegionVisibilityChangedAt(unsigned int)));
    ...
    connect(m_view, SIGNAL(selectionChanged()), this, SLOT(viewSelectionChanged()));
...
void KateViewInternal::scrollPos(KateTextCursor &c, bool force, bool calledExternally)
{
    ...
    bool viewLinesScrolledUsable =
        !force && (c.line() >= (int)startLine() - (int)linesDisplayed() - 1) && (c.line() <= (int)endLine() + (int)linesDisplayed() + 1);
            ...
            else
            {
                ...
                if(lineRanges[z].dirty || changed || alreadyDirty)
                {
                    ...
                    if(wrap)
                    {
                        ...
                        if((lineRanges[z].startX != startX) || (lineRanges[z].endX != endX) || (lineRanges[z].startCol != startCol)
                           || (lineRanges[z].endCol != endCol) || (lineRanges[z].shiftX != shiftX))
                            lineRanges[z].dirty = true;
...
{
    // kdDebug() << "MakeVisible start [" << startPos().line << "," << startPos().col << "] end [" << endPos().line << "," << endPos().col << "] ->
    // request: [" << c.line << "," << c.col << "]" <<endl;// , new start [" << scroll.line << "," << scroll.col << "] lines " << (linesDisplayed() -
    // 1) << " height " << height() << endl;
...
void KateViewInternal::updateMicroFocusHint()
{
    ...
    uint preeditStrLen =
        renderer->textWidth(textLine(m_imPreeditStartLine), cursor.col()) - renderer->textWidth(textLine(m_imPreeditStartLine), m_imPreeditSelStart);
...
public:
    CalculatingCursor(KateViewInternal *vi) : KateTextCursor(), m_vi(vi)
    {
        Q_ASSERT(valid());
    }

    CalculatingCursor(KateViewInternal *vi, const KateTextCursor &c) : KateTextCursor(c), m_vi(vi)
    {
        Q_ASSERT(valid());
    }

    // This one constrains its arguments to valid positions
    CalculatingCursor(KateViewInternal *vi, uint line, uint col) : KateTextCursor(line, col), m_vi(vi)
    {
        makeValid();
    }
...
protected:
    bool valid() const
    {
        return line() >= 0 && uint(line()) < m_vi->m_doc->numLines() && col() >= 0
...
public:
    BoundedCursor(KateViewInternal *vi) : CalculatingCursor(vi){};
    BoundedCursor(KateViewInternal *vi, const KateTextCursor &c) : CalculatingCursor(vi, c){};
    BoundedCursor(KateViewInternal *vi, uint line, uint col) : CalculatingCursor(vi, line, col){};
    virtual CalculatingCursor &operator+=(int n)
    {
        ...
        if(n > 0 && m_vi->m_view->dynWordWrap())
        {
            ...
            if(m_col > m_vi->m_doc->lineLength(m_line))
            {
                ...
                int endX;
                bool crap;
                m_vi->m_view->renderer()->textWidth(m_vi->textLine(m_line), currentRange.startCol, m_vi->width() - currentRange.xOffset(), &crap,
                                                    &endX);
...
public:
    WrappingCursor(KateViewInternal *vi) : CalculatingCursor(vi){};
    WrappingCursor(KateViewInternal *vi, const KateTextCursor &c) : CalculatingCursor(vi, c){};
    WrappingCursor(KateViewInternal *vi, uint line, uint col) : CalculatingCursor(vi, line, col){};
...
KateLineRange KateViewInternal::range(const KateTextCursor &realCursor)
{
    ...
    do
    {
        ...
        first = false;
            ...
            && thisRange.startCol != thisRange.endCol);
...
int KateViewInternal::displayViewLine(const KateTextCursor &virtualCursor, bool limitToVisible)
{
    ...
    if(realCursor.col() == -1)
        realCursor.setCol(m_doc->lineLength(realCursor.line()));
...
void KateViewInternal::cursorUp(bool sel)
{
    ...
    if(m_view->dynWordWrap())
    {
        ...
        Q_ASSERT((cursor.line() == thisRange.line) && (cursor.col() >= thisRange.startCol) && (!thisRange.wrap || cursor.col() < thisRange.endCol));
...
void KateViewInternal::updateSelection(const KateTextCursor &_newCursor, bool keepSel)
{
    ...
    if(keepSel)
    {
        if(!m_view->hasSelection() || (selectAnchor.line() == -1)
           || (m_view->config()->persistentSelection() && ((cursor < m_view->selectStart) || (cursor > m_view->selectEnd))))
        {
            selectAnchor = cursor;
            ...
            switch(m_selectionMode)
            {
                case Word:
                {
                    ...
                    else if(newCursor < selStartCached)
                    {
                        ...
                        c = newCursor.col();
                        if(c > 0 && c < m_doc->textLine(newCursor.line()).length() && m_doc->highlight()->isInWord(l->getChar(c))
...
void KateViewInternal::updateCursor(const KateTextCursor &newCursor, bool force, bool center, bool calledExternally)
{
    ...
    // kdDebug(13030) << "Cursor now located at real " << cursor.line << "," << cursor.col << ", virtual " << displayCursor.line << ", " <<
    // displayCursor.col << "; Top is " << startLine() << ", " << startPos().col <<  endl;
...
bool KateViewInternal::tagLines(KateTextCursor start, KateTextCursor end, bool realCursors)
{
    ...
    for(uint z = 0; z < lineRanges.size(); z++)
    {
        if((lineRanges[z].virtualLine > start.line()
            || (lineRanges[z].virtualLine == start.line() && lineRanges[z].endCol >= start.col() && start.col() != -1))
           && (lineRanges[z].virtualLine < end.line()
               || (lineRanges[z].virtualLine == end.line() && (lineRanges[z].startCol <= end.col() || end.col() == -1))))
        {
...
bool KateViewInternal::eventFilter(QObject *obj, QEvent *e)
{
    ...
    switch(e->type())
    {
        ...
        case QEvent::DragMove:
        {
            ...
            QRect doNotScrollRegion(scrollMargin, scrollMargin, width() - scrollMargin * 2, height() - scrollMargin * 2);
...
void KateViewInternal::keyPressEvent(QKeyEvent *e)
{
    ...
    if(codeComp)
    {
        ...
        if(e->key() == Key_Enter || e->key() == Key_Return || (key == SHIFT + Qt::Key_Return) || (key == SHIFT + Qt::Key_Enter))
        {
    ...
    if((key == SHIFT + Qt::Key_Return) || (key == SHIFT + Qt::Key_Enter))
    {
        ...
        if(pos != -1)
        {
            while((int)line->length() > pos && !line->getChar(pos).isLetterOrNumber() && pos < cursor.col())
                ++pos;
        }
        else
        {
        ...
        m_doc->editStart();
        m_doc->insertText(cursor.line(), line->length(), "\n" + line->string(0, pos) + line->string().right(line->length() - cursor.col()));
    ...
    if(key == Qt::Key_Tab || key == SHIFT + Qt::Key_Backtab || key == Qt::Key_Backtab)
    {
        ...
        else if(m_doc->configFlags() & KateDocumentConfig::cfTabIndents)
        {
            if(key == Qt::Key_Tab)
            {
    ...
    if(!(e->state() & ControlButton) && !(e->state() & AltButton) && m_doc->typeChars(m_view, e->text()))
    {
        e->accept();
    ...
    switch(e->button())
    {
        case LeftButton:
            ...
            if(possibleTripleClick)
            {
                ...
                if(m_view->selectStart < selectAnchor && selectAnchor.line() != m_view->selectStart.line())
    ...
    switch(e->button())
    {
        case LeftButton:
            ...
            if(e->state() & Qt::ShiftButton)
            {
                ...
                if(cs < m_doc->textLine(selectAnchor.line()).length() && m_doc->highlight()->isInWord(l->getChar(cs)))
                {
                    for(cs--; cs >= 0; cs--)
...
void KateViewInternal::resizeEvent(QResizeEvent *e)
{
    ...
    if(m_view->dynWordWrap())
    {
        ...
        for(uint i = 0; i < lineRanges.count(); i++)
        {
            ...
            if(lineRanges[i].wrap || (!expandedHorizontally && (lineRanges[i].endX - lineRanges[i].startX) > width()))
            {
        ...
        if(width() < e->oldSize().width())
        {
            if(!m_view->wrapCursor())
            {
                ...
                if(cursor.col() > m_doc->lineLength(cursor.line()))
                {
                    ...
                    KateTextCursor newCursor(cursor.line(), thisRange.endCol + ((width() - thisRange.xOffset() - (thisRange.endX - thisRange.startX))
                                                                                / m_view->renderer()->spaceWidth())
                                                                - 1);
...
void KateViewInternal::textHintTimeout()
{
    ...
    if(!tmp.isEmpty())
        kdDebug(13030) << "Hint text: " << tmp << endl;
}
...
{
    event->accept((QTextDrag::canDecode(event) && m_doc->isReadWrite()) || KURLDrag::canDecode(event));
}
...
void KateViewInternal::imComposeEvent(QIMEvent *e)
{
    ...
    if(m_imPreeditLength > 0)
    {
        ...
        m_doc->removeText(m_imPreeditStartLine, m_imPreeditStart, m_imPreeditStartLine, m_imPreeditStart + m_imPreeditLength);
    }
    ...
    // update selection
    m_view->setIMSelectionValue(m_imPreeditStartLine, m_imPreeditStart, m_imPreeditStart + m_imPreeditLength, m_imPreeditSelStart,
                                m_imPreeditSelStart + e->selectionLength(), true);
