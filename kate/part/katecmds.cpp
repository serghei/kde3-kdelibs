...
bool KateCommands::CoreCommands::exec(Kate::View *view, const QString &_cmd, QString &errorMsg)
{
#define KCC_ERR(s)                                                                                                                                   \
    {                                                                                                                                                \
        errorMsg = s;                                                                                                                                \
    ...
    else if(cmd == "set-tab-width" || cmd == "set-indent-width" || cmd == "set-word-wrap-column" || cmd == "goto")
    {
        ...
        int val(args.first().toInt(&ok));
        if(!ok)
            KCC_ERR(i18n("Failed to convert argument '%1' to integer.").arg(args.first()));
    ...
    else if(cmd == "set-icon-border" || cmd == "set-folding-markers" || cmd == "set-line-numbers" || cmd == "set-replace-tabs"
            || cmd == "set-remove-trailing-space" || cmd == "set-show-tabs" || cmd == "set-indent-spaces" || cmd == "set-mixed-indent"
            || cmd == "set-word-wrap" || cmd == "set-wrap-cursor" || cmd == "set-replace-tabs-save" || cmd == "set-remove-trailing-space-save"
            || cmd == "set-show-indent")
    {
        if(!args.count())
...
int KateCommands::SedReplace::sedMagic(KateDocument *doc, int &line, const QString &find, const QString &repOld, const QString &delim, bool noCase,
                                       bool repeat, uint startcol, int endcol)
{
