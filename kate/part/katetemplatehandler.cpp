...
KateTemplateHandler::KateTemplateHandler(KateDocument *doc, uint line, uint column, const QString &templateString,
                                         const QMap< QString, QString > &initialValues)
    : QObject(doc), KateKeyInterceptorFunctor(), m_doc(doc), m_currentTabStop(-1), m_currentRange(0), m_initOk(false), m_recursion(false)
{
...
void KateTemplateHandler::slotDocumentDestroyed()
{
    m_doc = 0;
}

void KateTemplateHandler::generateRangeTable(uint insertLine, uint insertCol, const QString &insertString,
                                             const QValueList< KateTemplateHandlerPlaceHolderInfo > &buildList)
{
    ...
    for(QValueList< KateTemplateHandlerPlaceHolderInfo >::const_iterator it = buildList.begin(); it != buildList.end(); ++it)
    {
        ...
        KateArbitraryHighlightRange *hlr = new KateArbitraryHighlightRange(m_doc, KateTextCursor(line, col), KateTextCursor(line, (*it).len + col));
        colInText += (*it).len;
        col += (*it).len;
        hlr->allowZeroLength();
        hlr->setUnderline(true);
...
#warning FIXME undo/redo detection
    ...
    QString sourceText =
        m_doc->text(m_currentRange->start().line(), m_currentRange->start().col(), m_currentRange->end().line(), m_currentRange->end().col(), false);
...
bool KateTemplateHandler::operator()(KKey key)
{
    ...
    if(m_tabOrder.at(m_currentTabStop)->isInitialValue)
    {
        ...
        m_doc->activeView()->setSelection(m_currentRange->end(), m_currentRange->end());
...
void KateTemplateHandler::slotAboutToRemoveText(const KateTextRange &range)
{
    ...
    if(m_currentRange && (!m_currentRange->includes(range.start())))
