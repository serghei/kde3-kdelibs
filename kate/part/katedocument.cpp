...
//
KateDocument::KateDocument(bool bSingleViewMode, bool bBrowserView, bool bReadOnly, QWidget *parentWidget, const char *widgetName, QObject *parent,
                           const char *name)
    ...
    , m_tabInterceptor(0)
{
    ...
    connect(KateFactory::self()->dirWatch(), SIGNAL(dirty(const QString &)), this, SLOT(slotModOnHdDirty(const QString &)));

    connect(KateFactory::self()->dirWatch(), SIGNAL(created(const QString &)), this, SLOT(slotModOnHdCreated(const QString &)));

    connect(KateFactory::self()->dirWatch(), SIGNAL(deleted(const QString &)), this, SLOT(slotModOnHdDeleted(const QString &)));
...
{
    if(m_editCurrentUndo || (m_activeView && m_activeView->imComposeEvent()))
        return;
...
bool KateDocument::editUnWrapLine(uint line, bool removeLine, uint length)
{
    ...
    {
        l->insertText(col, (nextLine->length() < length) ? nextLine->length() : length, nextLine->text(), nextLine->attributes());
...
void KateDocument::updateModified()
{
    ...
    */
    ...
    kdDebug(13020) << k_funcinfo << "Pattern:" << static_cast< unsigned int >(currentPattern) << endl;
...
bool KateDocument::searchText(unsigned int startLine, unsigned int startCol, const QString &text, unsigned int *foundAtLine, unsigned int *foundAtCol,
...
bool KateDocument::searchText(unsigned int startLine, unsigned int startCol, const QRegExp &regexp, unsigned int *foundAtLine,
...
void KateDocument::writeSessionConfig(KConfig *kconfig)
{
    ...
    for(QIntDictIterator< KTextEditor::Mark > it(m_marks); it.current() && it.current()->type & KTextEditor::MarkInterface::markType01; ++it)
...
bool KateDocument::openURL(const KURL &url)
{
    ...
    else
    {
        ...
        // connect to slots
        connect(m_job, SIGNAL(data(KIO::Job *, const QByteArray &)), SLOT(slotDataKate(KIO::Job *, const QByteArray &)));

        connect(m_job, SIGNAL(result(KIO::Job *)), SLOT(slotFinishedKate(KIO::Job *)));
...
bool KateDocument::openFile(KIO::Job *job)
{
    ...
    if(s_openErrorDialogsActivated)
    {
        if(!success && m_buffer->loadingBorked())
            KMessageBox::error(
                widget(), i18n("The file %1 could not be loaded completely, as there is not enough temporary disk storage for it.").arg(m_url.url()));
        else if(!success)
            KMessageBox::error(
                widget(),
    ...
    if(m_buffer->binary())
    {
        ...
        KMessageBox::information(widget(), i18n("The file %1 is a binary, saving it will result in a corrupt file.").arg(m_url.url()),
                                 i18n("Binary File Opened"), "Binary File Opened Warning");
    }
...
bool KateDocument::save()
{
    ...
    if((l && config()->backupFlags() & KateDocumentConfig::LocalFiles) || (!l && config()->backupFlags() & KateDocumentConfig::RemoteFiles))
    {
        KURL u(url());
...
bool KateDocument::saveFile()
{
    ...
    if(m_buffer->loadingBorked()
       && (KMessageBox::warningContinueCancel(widget(), i18n("This file could not be loaded correctly due to lack of temporary disk space. Saving it "
                                                             "could cause data loss.\n\nDo you really want to save it?"),
                                              i18n("Possible Data Loss"), i18n("Save Nevertheless"))
    ...
    //
    if(m_buffer->binary()
       && (KMessageBox::warningContinueCancel(widget(), i18n("The file %1 is a binary, saving it will result in a corrupt file.").arg(m_url.url()),
                                              i18n("Trying to Save Binary File"), i18n("Save Nevertheless"), "Binary File Save Warning")
           != KMessageBox::Continue))
        return false;
        ...
        if(s_fileChangedDialogsActivated && m_modOnHd)
        {
            ...
            if(!isModified())
            {
                if(KMessageBox::warningContinueCancel(
                       0, str + i18n("Do you really want to save this unmodified file? You could overwrite changed data in the file on disk."),
                       i18n("Trying to Save Unmodified File"), i18n("Save Nevertheless"))
                   != KMessageBox::Continue)
                    return false;
            }
            else
            {
                if(KMessageBox::warningContinueCancel(0, str + i18n("Do you really want to save this file? Both your open file and the file on disk "
    ...
    if(!m_buffer->canEncode()
       && (KMessageBox::warningContinueCancel(0, i18n("The selected encoding cannot encode every unicode character in this document. Do you really "
                                                      "want to save it? There could be some data lost."),
                                              i18n("Possible Data Loss"), i18n("Save Nevertheless"))
           != KMessageBox::Continue))
    {
        return false;
    ...
    if(!success)
        KMessageBox::error(widget(), i18n("The document could not be saved, as it was not possible to write to %1.\n\nCheck that you have write "
                                          "access to this file or that enough disk space is available.")
                                         .arg(m_url.url()));
...
bool KateDocument::closeURL()
{
    ...
    if(!m_reloading && !url().isEmpty())
    {
        ...
        {
            if(!(KMessageBox::warningContinueCancel(
                     widget(), reasonedMOHString() + "\n\n" + i18n("Do you really want to continue to close this file? Data loss may occur."),
                     i18n("Possible Data Loss"), i18n("Close Nevertheless"), QString("kate_close_modonhd_%1").arg(m_modOnHdReason))
                 == KMessageBox::Continue))
                return false;
        }
    }
...
bool KateDocument::typeChars(KateView *view, const QString &chars)
{
            ...
            if(!bracketInserted && (config()->configFlags() & KateDocument::cfAutoBrackets))
            {
                ...
                if(complete)
                {
                    ...
                    { // no selection, check whether we should better refuse to complete
                        if(((ch == '\'' || ch == '"') && (prevChar.isLetterOrNumber() || prevChar == ch)) || nextChar.isLetterOrNumber()
    ...
    if(config()->configFlags() & KateDocument::cfOvr)
        removeText(view->cursorLine(), view->cursorColumnReal(), view->cursorLine(),
                   kMin(view->cursorColumnReal() + buf.length(), textLine->length()));
...
void KateDocument::paste(KateView *view)
{
    ...
    if(m_indenter->canProcessLine() && config()->configFlags() & KateDocumentConfig::cfIndentPastedText)
    {
        editStart();
    ...
    if(!view->blockSelectionMode())
        emit charactersSemiInteractivelyInserted(line, column, s);
    m_undoDontMerge = true;
}
...
void KateDocument::indent(KateView *v, uint line, int change)
{
    ...
    else
    {
        ...
        {
            if((v->lineSelected(line) || v->lineHasSelected(line)) && (!rts || lineLength(line) > 0))
            {
                optimizeLeadingSpace(line, config()->configFlags(), change);
            }
        }
...
bool KateDocument::removeStartLineCommentFromSingleLine(int line, int attrib)
{
    ...
    bool removed = (removeStringFromBegining(line, longCommentMark) || removeStringFromBegining(line, shortCommentMark));
...
#warning "that's a bad idea, can lead to stray endings, FIXME"
    ...
    bool removedStart = (removeStringFromBegining(line, longStartCommentMark) || removeStringFromBegining(line, shortStartCommentMark));

    bool removedStop = false;
    if(removedStart)
    {
        // Try to remove the long stop comment mark first
        removedStop = (removeStringFromEnd(line, longStopCommentMark) || removeStringFromEnd(line, shortStopCommentMark));
    }
...
bool KateDocument::removeStartStopCommentFromSelection(KateView *view, int attrib)
{
    ...
    bool remove = nextNonSpaceCharPos(sl, sc) && m_buffer->plainLine(sl)->stringAtPos(sc, startComment) && previousNonSpaceCharPos(el, ec)
                  && ((ec - endCommentLen + 1) >= 0) && m_buffer->plainLine(el)->stringAtPos(ec - endCommentLen + 1, endComment);
...
bool KateDocument::removeStartStopCommentFromRegion(const KateTextCursor &start, const KateTextCursor &end, int attrib)
{
    ...
    bool remove = m_buffer->plainLine(start.line())->stringAtPos(start.col(), startComment) && ((end.col() - endCommentLen) >= 0)
...
bool KateDocument::removeStartLineCommentFromSelection(KateView *view, int attrib)
{
    ...
    {
        // Try to remove the long comment mark first
        removed = (removeStringFromBegining(z, longCommentMark) || removeStringFromBegining(z, shortCommentMark) || removed);
    }
...
void KateDocument::comment(KateView *v, uint line, uint column, int change)
{
    ...
    bool hasStartStopCommentMark = (!(highlight()->getCommentStart(startAttrib).isEmpty()) && !(highlight()->getCommentEnd(endAttrib).isEmpty()));
    ...
    if(change > 0) // comment
    {
        ...
        else
        {
            ...
            // line ignored
            if(hasStartStopCommentMark && (!hasStartLineCommentMark || ((v->selStartCol() > m_buffer->plainLine(v->selStartLine())->firstChar())
            ...
            if((!removed) && foldingTree())
            {
                ...
                if(commentRegion)
                {
                    ...
                    if(n)
                    {
                        ...
                        {
                            kdDebug(13020) << "Enclosing region found:" << start.col() << "/" << start.line() << "-" << end.col() << "/" << end.line()
                                           << endl;
                        ...
                        kdDebug(13020) << "No enclosing region found" << endl;
                }
                else
                    kdDebug(13020) << "No comment region specified for current hl" << endl;
            }
        }
        else
        {
            // anders: this seems like it will work with above changes :)
            removed = (hasStartLineCommentMark && removeStartLineCommentFromSelection(v, startAttrib))
                      || (hasStartStopCommentMark && removeStartStopCommentFromSelection(v, startAttrib));
        }
    }
}

void KateDocument::transform(KateView *v, const KateTextCursor &c, KateDocument::TextTransform t)
{
    editStart();
    ...
    if(hasSelection())
    {
        ...
        {
            uint start, end;
            start = (ln == selstart.line() || v->blockSelectionMode()) ? selstart.col() : 0;
            end = (ln == selend.line() || v->blockSelectionMode()) ? selend.col() : lineLength(ln);
            if(start > end)
            {
                uint t = start;
                start = end;
            ...
            else // Capitalize
            {
                ...
                while(p < s.length())
                {
                    ...
                    if((!start && !p) || ((ln == selstart.line() || v->blockSelectionMode()) && !p && !highlight()->isInWord(l->getChar(start - 1)))
...
QString KateDocument::getWord(const KateTextCursor &cursor)
{
    ...
    while(start > 0 && highlight()->isInWord(textLine->getChar(start - 1), textLine->attribute(start - 1)))
        start--;
    while(end < len && highlight()->isInWord(textLine->getChar(end), textLine->attribute(end)))
        end++;
    len = end - start;
        ...
        switch(p.exec())
        {
            case KateModOnHdPrompt::Save:
            {
                ...
                KEncodingFileDialog::Result res =
                    KEncodingFileDialog::getSaveURLAndEncoding(config()->encoding(), url().url(), QString::null, widget(), i18n("Save File"));
...
void KateDocument::reloadFile()
{
    if(!url().isEmpty())
    {
        ...
        {
            int i = KMessageBox::warningYesNoCancel(0, reasonedMOHString() + "\n\n" + i18n("What do you want to do?"),
...
void KateDocument::removeTrailingSpace(uint line)
{
    ...
    if(config()->configFlags() & KateDocumentConfig::cfRemoveTrailingDyn)
    {
        ...
        if(line == activeView()->cursorLine() && activeView()->cursorColumnReal() >= (uint)kMax(0, ln->lastChar()))
            return;
...
void KateDocument::slotQueryClose_save(bool *handled, bool *abortClosing)
{
    ...
    *abortClosing = true;
    ...
    {
        KEncodingFileDialog::Result res =
            KEncodingFileDialog::getSaveURLAndEncoding(config()->encoding(), QString::null, QString::null, 0, i18n("Save File"));
...
bool KateDocument::checkOverwrite(KURL u)
{
    ...
    return KMessageBox::Cancel != KMessageBox::warningContinueCancel(0, i18n("A file named \"%1\" already exists. "
...
bool KateDocument::insertTemplateTextImplementation(uint line, uint column, const QString &templateString,
