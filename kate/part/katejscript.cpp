...
public:
    ...
    const KJS::ClassInfo *classInfo() const
    {
        return &info;
    }
    ...
    , m_view(new KJS::Object(wrapView(m_interpreter->globalExec(), 0)))
{
    ...
    m_interpreter->globalObject().put(m_interpreter->globalExec(), "debug", KJS::Object(new KateJSGlobalFunctions(KateJSGlobalFunctions::Debug, 1)));
}
...
KateJSDocument::KateJSDocument(KJS::ExecState *exec, KateDocument *_doc) : KJS::ObjectImp(KateJSDocumentProto::self(exec)), doc(_doc)
{
}
...
KJS::Value KJS::KateJSViewProtoFunc::call(KJS::ExecState *exec, KJS::Object &thisObj, const KJS::List &args)
{
    ...
    switch(id)
    {
        ...
        case KateJSView::SetSelection:
            return KJS::Boolean(view->setSelection(args[0].toUInt32(exec), args[1].toUInt32(exec), args[2].toUInt32(exec), args[3].toUInt32(exec)));
...
KateJSView::KateJSView(KJS::ExecState *exec, KateView *_view) : KJS::ObjectImp(KateJSViewProto::self(exec)), view(_view)
{
}
...
KateJSIndenter::KateJSIndenter(KJS::ExecState *exec) : KJS::ObjectImp(KateJSViewProto::self(exec))
{
}
...
KateIndentJScriptImpl::KateIndentJScriptImpl(const QString &internalName, const QString &filePath, const QString &niceName, const QString &copyright,
                                             double version)
    ...
    for(QStringList::Iterator it = list.begin(); it != list.end(); ++it)
    {
        ...
        if(!force && config.hasGroup(Group) && (sbuf.st_mtime == config.readNumEntry("lastModified")))
        {
            ...
            else
            {
                ...
                KateIndentJScriptImpl *s = new KateIndentJScriptImpl(internalName, filePath, niceName, copyright, version);
...
void KateIndentJScriptManager::parseScriptHeader(const QString &filePath, QString *niceName, QString *copyright, double *version)
{
    ...
    while((line = st.readLine()) != QString::null)
    {
        ...
        if(currentState == NOTHING)
        {
            if(keyValue.exactMatch(line))
            {
                ...
                QString value = sl[2];
                if(key == "NAME")
                    (*niceName) = value.stripWhiteSpace();
                else if(key == "VERSION")
                    (*version) = value.stripWhiteSpace().toDouble(0);
                else if(key == "COPYRIGHT")
                {
                    tmpblockdata = "";
                    if(value.stripWhiteSpace().length() > 0)
                        tmpblockdata = value;
                    currentState = COPYRIGHT;
                }
                else
                    kdDebug(13050) << "ignoring key" << endl;
            }
        }
        else
        {
