...
KateArbitraryHighlightRange::KateArbitraryHighlightRange(KateSuperCursor *start, KateSuperCursor *end, QObject *parent, const char *name)
...
KateArbitraryHighlightRange::KateArbitraryHighlightRange(KateDocument *doc, const KateTextCursor &start, const KateTextCursor &end, QObject *parent,
                                                         const char *name)
...
KateArbitraryHighlight::KateArbitraryHighlight(KateDocument *parent, const char *name) : QObject(parent, name)
{
}
...
void KateArbitraryHighlight::slotRangeListDeleted(QObject *obj)
{
    ...
    for(QMap< KateView *, QPtrList< KateSuperRangeList > * >::Iterator it = m_viewHLs.begin(); it != m_viewHLs.end(); ++it)
        for(KateSuperRangeList *l = (*it)->first(); l; l = (*it)->next())
            ...
            {
                l->take();
                break; // should we check if one list is stored more than once for a view ?? I don't think adding the same list 2 or more times is
                       // sane, but who knows (jowenn)
            }
}
