...
public:
    ...
    inline bool removeTrailingSpaces() const
    {
            ...
            if(m_position == m_text.length())
            {
                ...
                if(!m_eof)
                {
                    ...
                    if(c > 0)
                    {
                        ...
                        m_text = m_text.mid(m_lastLineStart, m_position - m_lastLineStart) + str;
                    }
                    else
...
void KateBuffer::editEnd()
{
    ...
    {
        // hl update !!!
        if(m_highlight && !m_highlight->noHighlighting() && (editTagLineStart <= editTagLineEnd) && (editTagLineEnd <= m_lineHighlighted))
        {
            ...
            {
                needContinue = doHighlight(buf2, (editTagLineStart > buf2->startLine()) ? editTagLineStart : buf2->startLine(),
                                           (editTagLineEnd > buf2->endLine()) ? buf2->endLine() : editTagLineEnd, true);
...
KateTextLine::Ptr KateBuffer::line_internal(KateBufBlock *buf, uint i)
{
    ...
    while((i >= m_lineHighlighted) && (buf2 = findBlock(m_lineHighlighted)))
    {
        ...
        doHighlight(buf2, kMax(m_lineHighlighted, buf2->startLine()), end, false);
...
KateBufBlock *KateBuffer::findBlock_internal(uint i, uint *index)
{
    ...
    if(lastLine > i) // we are in a allready known area !
    {
        while(true)
        {
            ...
            if((buf->startLine() <= i) && (buf->endLine() > i))
            {
                if(index)
...
void KateBuffer::addIndentBasedFoldingInformation(QMemArray< uint > &foldingList, bool addindent, uint deindent)
{
    if(addindent)
    {
        // kdDebug(13020)<<"adding indent for line :"<<current_line + buf->startLine()<<"
        // textLine->noIndentBasedFoldingAtStart"<<textLine->noIndentBasedFoldingAtStart()<<endl;
...
bool KateBuffer::doHighlight(KateBufBlock *buf, uint startLine, uint endLine, bool invalidate)
{
    ...
    {
        {
            if(KateHlManager::self()->resetDynamicCtxs())
            {
                ...
                while((endLine > m_lineHighlighted) && (buf = findBlock(m_lineHighlighted)))
                {
                    ...
                    doHighlight(buf, kMax(m_lineHighlighted, buf->startLine()), end, false);
    ...
    while((current_line < buf->lines()) && (stillcontinue || ((current_line + buf->startLine()) <= endLine)))
    {
        // current line
        ...
        if(m_highlight->foldingIndentationSensitive())
        {
            ...
            kdDebug(13020) << "current_line:" << current_line + buf->startLine() << " textLine->noIndentBasedFoldingAtStart"
                           << textLine->noIndentBasedFoldingAtStart() << endl;
            ...
            if(!textLine->noIndentBasedFolding())
            {
                if(nextLineIndentationValid)
                {
                    // if (textLine->firstChar()!=-1)
                    {
                        ...
                        /*        }
                                if (textLine->noIndentBasedFolding()) kdDebug(13020)<<"=============================indentation based folding
                           disabled======================"<<endl;
                                if (!textLine->noIndentBasedFolding()) {*/
                        if((textLine->firstChar() == -1))
                        {
        ...
        if(foldingChanged || foldingColChanged)
        {
            ...
            if(foldingChanged == false)
            {
                ...
                textLine->setFoldingColumnsOutdated(false);
        }
        bool retVal_folding = false;
...
KateBufBlock::KateBufBlock(KateBuffer *parent, KateBufBlock *prev, KateBufBlock *next, KateFileLoader *stream)
...
KateBufBlockList::KateBufBlockList() : m_count(0), m_first(0), m_last(0)
{
}
