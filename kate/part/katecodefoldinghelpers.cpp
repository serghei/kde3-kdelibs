...
void KateCodeFoldingTree::getLineInfo(KateLineInfo *info, unsigned int line)
{
    ...
    for(uint i = 0; i < m_root.childCount(); ++i)
    {
        ...
        if((node->startLineRel <= line)
           && (line <= node->startLineRel + node->endLineRel)) // we found a node, which contains the given line -> do a complete lookup
        {
...
KateCodeFoldingNode *KateCodeFoldingTree::findNodeForLineDescending(KateCodeFoldingNode *node, unsigned int line, unsigned int offset,
                                                                    bool oneStepOnly)
{
    if(node->noChildren())
        return node;
    ...
    for(uint i = 0; i < node->childCount(); ++i)
    {
        ...
        if((subNode->startLineRel + offset <= line)
           && (line <= subNode->endLineRel + subNode->startLineRel + offset)) // warning fix me for invalid ends
        {
...
void KateCodeFoldingTree::dumpNode(KateCodeFoldingNode *node, const QString &prefix)
{
    ...
    kdDebug(13000) << prefix
                   << QString("Type: %1, startLineValid %2, startLineRel %3, endLineValid %4, endLineRel %5, visible %6")
                          .arg(node->type)
                          .arg(node->startLineValid)
                          .arg(node->startLineRel)
                          .arg(node->endLineValid)
                          .arg(node->endLineRel)
                          .arg(node->visible)
                   << endl;
...
*/
void KateCodeFoldingTree::updateLine(unsigned int line, QMemArray< uint > *regionChanges, bool *updated, bool changed, bool colsChanged)
{
...
void KateCodeFoldingTree::addOpening_further_iterations(KateCodeFoldingNode *node, signed char /* nType */, QMemArray< uint > *list,
                                                        unsigned int line, int current, unsigned int startLine, unsigned int charPos)
{
    while(!(list->isEmpty()))
    {
        if(list->isEmpty())
...
void KateCodeFoldingTree::addHiddenLineBlock(KateCodeFoldingNode *node, unsigned int line)
{
    ...
    for(QValueList< KateHiddenLineBlock >::Iterator it = hiddenLines.begin(); it != hiddenLines.end(); ++it)
    {
        if(((*it).start >= data.start)
           && ((*it).start
               <= data.start + data.length - 1)) // another hidden block starting at the within this block already exits -> adapt new block
        {
...
bool KateCodeFoldingTree::existsOpeningAtLineAfter(unsigned int line, KateCodeFoldingNode *node)
{
    for(KateCodeFoldingNode *tmp = node->parentNode; tmp; tmp = tmp->parentNode)
    {
        ...
        if((tmp2 = tmp->child(tmp->findChild(node) + 1)) && ((tmp2->startLineRel + startLine) == line))
            return true;
