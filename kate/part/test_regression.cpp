...
KateViewObject::KateViewObject(ExecState *exec, KateView *v, ObjectImp *fallback) : view(v), fallback(fallback)
{
// put a function
...
OutputFunction::OutputFunction(KJS::ExecState *exec, OutputObject *output, int _id, int length) : o(output)
{
    id = _id;
    if(length >= 0)
        putDirect("length", length);
...
{
    QDir dir(kapp->dirs()->saveLocation("config"), QString(failureSnapshotPrefix) + "*", QDir::Time, QDir::Files);
...
int main(int argc, char *argv[])
{
    ...
    KCmdLineArgs::init(argc, argv, "testregression", "TestRegression", "Regression tester for kate", "1.0");
    ...
    if(args->count() < 1 && baseDir.isEmpty())
    {
        ...
        printf(
            ...
            "",
            KCmdLineArgs::appName(), (const char *)baseDirConfigFile, (const char *)baseDirConfigFile);
        ::exit(1);
    }

    int testcase_index = 0;
    if(baseDir.isEmpty())
        baseDir = args->arg(testcase_index++);
    ...
    cfg.writeEntry("Basic Config Flags", KateDocumentConfig::cfBackspaceIndents
                                             ...
                                             | KateDocumentConfig::cfKeepIndentProfile | KateDocumentConfig::cfKeepExtraSpaces
                                             | KateDocumentConfig::cfTabIndents | KateDocumentConfig::cfShowTabs | KateDocumentConfig::cfSpaceIndent
                                             | KateDocumentConfig::cfSmartHome | KateDocumentConfig::cfTabInsertsTab
...

    {
        ...
        // FIXME adapt to kate
        static int areas[] = {1000, 13000, 13001, 13002, 13010, 13020, 13025, 13030, 13033, 13035, 13040, 13050, 13051, 7000,
                              7006, 170,   171,   7101,  7002,  7019,  7027,  7014,  7001,  7011,  6070,  6080,  6090,  0};
    ...
    // run the tests
    RegressionTest *regressionTest = new RegressionTest(part, &cfg, baseDir, args->getOption("output"), args->isSet("genoutput"));
    QObject::connect(part->browserExtension(), SIGNAL(openURLRequest(const KURL &, const KParts::URLArgs &)), regressionTest,
                     SLOT(slotOpenURL(const KURL &, const KParts::URLArgs &)));
    QObject::connect(part->browserExtension(), SIGNAL(resizeTopLevelWidget(int, int)), regressionTest, SLOT(resizeTopLevelWidget(int, int)));
...

    {
        ...
        if(!failureSnapshot.isEmpty())
            regressionTest->setFailureSnapshotConfig(new KSimpleConfig(failureSnapshotPrefix + failureSnapshot, true), failureSnapshot);
    }

    if(args->isSet("save-failures"))
    {
        QString failureSaver = args->getOption("save-failures");
        regressionTest->setFailureSnapshotSaver(new KSimpleConfig(failureSnapshotPrefix + failureSaver, false), failureSaver);
    }
    ...
    if(result)
    {
        ...
        {
            printf("\nTests completed.\n");
            printf("Total:    %d\n", regressionTest->m_passes_work + regressionTest->m_passes_fail + regressionTest->m_failures_work
                                         + regressionTest->m_failures_fail + regressionTest->m_errors);
            ...
            QString link, cl;
            link = QString("<hr>%1 failures. (%2 expected failures)").arg(regressionTest->m_failures_work).arg(regressionTest->m_failures_fail);
            if(regressionTest->m_failures_new)
...
RegressionTest::RegressionTest(KateDocument *part, KConfig *baseConfig, const QString &baseDir, const QString &outputDir, bool _genOutput)
    : QObject(part)
{
    m_part = part;
...
void RegressionTest::createLink(const QString &test, int failures)
{
    ...
    list.open(IO_WriteOnly | IO_Append);
    QString link;
    link = QString("<a href=\"%1\" target=\"content\" title=\"%2\">").arg(test + "-compare.html").arg(test);
    link += m_currentTest;
    link += "</a> ";
...
static QString makeRelativePath(const QString &base, const QString &path)
{
    ...
    do
    {
        ...
        if(cmpPathComp.string() != cmpBaseComp.string())
        {
            pos--;
            break;
        }
...
void RegressionTest::doFailureReport(const QString &test, int failures)
{
    ...
    {
        domDiff += "<pre>";
        FILE *pipe = popen(QString::fromLatin1("diff -u baseline/%1-result %3/%2-result").arg(test, test, relOutputDir).latin1(), "r");
    ...
    QString relpath = makeRelativePath(m_outputDir + "/" + QFileInfo(test).dirPath(), m_baseDir);
    ...
    cl += QString(
        ...
        ".diff { position: absolute; left: 10px; top: 100px; visibility: hidden; border: 1px black solid; background-color: white; color: black; /* "
        "width: 800; height: 600; overflow: scroll; */ }\n"
        "</style>\n");
    ...
    if(QFile::exists(m_baseDir + "/tests/" + test))
        cl += QString("<a class=button href=\"%1\">HTML</a>&nbsp;").arg(relpath + "/tests/" + test);
...
void RegressionTest::createMissingDirs(const QString &filename)
{
    ...
    {
        if(!QFileInfo(pathComponents[pathno]).exists() && !QDir(pathComponents[pathno - 1]).mkdir(pathComponents[pathno]))
        {
