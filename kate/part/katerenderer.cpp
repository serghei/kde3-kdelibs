...
bool KateRenderer::paintTextLineBackground(QPainter &paint, int line, bool isCurrentLine, int xStart, int xEnd)
{
    ...
    else
    {
        ...
        if(markCount)
        {
            ...
            markGreen /= markCount;
            markBlue /= markCount;
            backgroundColor.setRgb(int((backgroundColor.red() * 0.9) + (markRed * 0.1)), int((backgroundColor.green() * 0.9) + (markGreen * 0.1)),
...
void KateRenderer::paintTextLine(QPainter &paint, const KateLineRange *range, int xStart, int xEnd, const KateTextCursor *cursor,
                                 const KateBracketRange *bracketmark)
{
    int line = range->line;
    ...
    else
    {
        ...
        while(curCol - startcol < len)
        {
            ...
            if((int)xPosAfter >= xStart)
            {
                ...
                if(curAt != oldAt || curColor != oldColor
                   || (superRanges.count() && superRanges.currentBoundary() && *(superRanges.currentBoundary()) == currentPos))
                {
...
uint KateRenderer::textWidth(KateTextCursor &cursor, int xPos, uint startCol)
{
    ...
    if(cursor.line() < 0)
        cursor.setLine(0);
    if(cursor.line() > (int)m_doc->lastLine())
        cursor.setLine(m_doc->lastLine());
